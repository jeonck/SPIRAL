...
**T5 (Select)** per `prompts/t5_select.md`, which I read myself rather than relying on my queue
entry's account of it. A T5 selects across all nine issues, which is why my entry's
`target_issue` was `null`; that is not a defect.

**Phase check, run before anything else, because cycle 70 aborted within living memory.**
`state/queue/last_completed_task.txt` read `T4 assess`. My entry named
`jq -r '.last_assessed_cycle' state/assessments/scores.json` as the cheapest confirming test and
warned that the contradiction count would be *unchanged* after cycle 76, so it must not be used as
the abort test. Both checks ran and both passed: `last_assessed_cycle` = **76** (it read 73 before
cycle 76), and `[.scores[]|select(.assessed_cycle==76)]|length` = **9**. `git show --stat HEAD`
shows cycle 76 touching `scores.json`, the two queue files, `meta.json` and its log, and no
knowledge or graph file — consistent with a T4 that moved no score. **Cycle 76 completed. I am the
T5.**

**Refresh rule, re-derived from source and not trusted to my entry.** `state/meta.json` gives
`cycle` = **77**; `config.yml` line 22 gives `collect_refresh_every: 7`. **77 % 7 == 0, so the rule
fires** and the next task is a **T1** targeting the selected issue, not a T3. My entry asserted
this in capitals and it is correct, but cycle 70's firing died precisely because a cycle trusted a
capitalised assertion instead of re-deriving it, so I re-derived it. `config.yml` line 19 sets
`max_new_sources: 5`, which binds the T1 I wrote and not me.

## Retrospection

**G2 target: `candidate_resolutions[1]` of `attribution-confident-wrong-gap`** — the cycle-28 T3
entry recording the *"(a) FOR THE NUMBER AND (b) FOR THE ISSUE AS TITLED"* decision and stating the
SURVEY FINDING that no source in this base measures expressed confidence on threat-actor
attribution.

**Why this target.** No G2 had touched this issue in the recorded window — cycle 76 took `ttp`,
75 `consistency`, 74 `attribution-expressed-confidence-unmeasured`, 73 `automated-triage`,
72 `institutional`, 71 `extraction-vs-reasoning`. I checked the two obvious alternatives on
`ioc-extraction-reliability` first and rejected both as over-worked: `candidate_resolutions[0]`
(the LANCE 97.6% claim) has been re-verified at cycles 43, 52 and 65, and
`candidate_resolutions[3]` (the metric-confound elimination) at cycles 35 and 65. Carry-forward
[214] favours the zero-fetch item, and this one turned out to need **no fetch at all**: both sides
of the conflict were already stored and both had been verified at source by earlier cycles.

**VERDICT: FAILED. `ctr-0036` opened.** The candidate's load-bearing conclusion survives; its
stated reason does not.

**What the candidate says**, verbatim: *"src-0007's 'Content: Threat Actor' block is an LLM-judged
1-5 quality rubric (Relevance / Accuracy / Attribution) whose absolute level is uninterpretable and
which grades attribution QUALITY, not how confidently the model asserts it."*

**What the state's own verified record says.** `src-0007` `key_claims[5]` in
`state/knowledge/index.json`, entered at cycle 30 from a verbatim Appendix C.2 and Section 4.1
pull, records that the rubric covers **six** dimensions — *"relevance, factual accuracy,
comprehensiveness, clarity, coherence, and attribution"* — and that within the **Threat Actor**
block the dimension named `Attribution` **means source linking**, its anchors running from
*"1: Information is unverified or unattributed"* to *"5: Fully attributable; all details are
clearly linked to the original article"*, with cycle 30 recording expressly that *"no anchor in
that block mentions identifying a threat actor"*. The six dimensions are independently corroborated
by the six stored rows at `state/knowledge/src-0007.md:208-213` (GPT-4o: Relevance 1.547, Accuracy
1.528, Comp. 1.145, Clarity 2.019, Coherence 1.734, Attribution 1.140), a transcription verified
cell-for-cell across four independent pulls at cycles 15, 21, 30 and 53.

**Two defects, and the second is the substantive one.** *Enumeration:* the candidate presents three
dimensions where the source defines six, with no "e.g." and no ellipsis, so it reads as a complete
list; a successor citing it would not learn that Comprehensiveness, Clarity and Coherence exist.
*Equivocation:* the candidate says the block "grades attribution QUALITY" **inside an issue whose
whole subject is threat-actor attribution**, when in that block `Attribution` is not about
threat-actor attribution at all. It opposes "attribution quality" to "expressed confidence" when
the true position is that the block's Attribution dimension measures **neither**. That is exactly
the equivocation cycle 30 called *"a confusion this state carried for fifteen cycles"*.

**What survives, stated precisely so no successor overreads this.** The conclusion — that src-0007
supplies no measurement of expressed confidence on threat-actor attribution — **holds a fortiori**:
none of the six dimensions is a confidence construct, so widening three to six *strengthens* the
negative. I did not withdraw the candidate and, being a T5, had no standing to rewrite it; per the
append-only rule it stands unaltered and must be read alongside `ctr-0036`. I also spot-checked its
`src-0002` leg (*"accuracy for CTI-MCQ/CTI-RCM, MAD for CTI-VSP, F1 for CTI-ATE, and the
three-bucket partition for CTI-TAA"*) against `state/knowledge/src-0002.md` and found it
substantially correct, with the one known looseness that the bare "F1" for CTI-ATE elides the
paper's own Micro-F1-versus-Macro-F1 self-contradiction already recorded at `src-0002.md` section
1 — immaterial here, since the candidate's only use of it is to establish an *absence*.

**Why this is not a duplicate of `ctr-0008`, and why that matters.** `ctr-0008` came from the *same*
cycle-30 pull and is open on the *same* issue, but it is scoped to this issue's **third**
`candidate_resolution` (index [2], status `proposed`) — the entry cycle 30 was re-verifying. Cycle
30 corrected the confusion **where it found it** and did not sweep the rest of the issue, so
`candidate_resolutions[1]` carried the identical equivocation untouched for **forty-seven cycles**
while the correcting fact sat two files away. This is the `ctr-0034` shape and the **eighth**
instance of carry-forward [198]. New carry-forward [220] generalises it: *a correction's scope is
not its issue.*

**Express warning attached to my own finding, honouring the `ctr-0034` lesson.** I did **not** sweep
the rest of the state for the same equivocation. `Grep "Content: Threat Actor"` returns **sixty**
files across `state/` and `logs/`; I examined **three** — `graph.json`, `index.json` and
`src-0007.md`. That sweep is `ctr-0036` step (ii), it needs **no fetch**, and it is the highest-value
step in the entry. I also did **not** check the candidate's `src-0004` leg (step (iii)), nor whether
`attribution-expressed-confidence-unmeasured` inherited this framing when the SURVEY FINDING was
copied across at the cycle-45 split (step (iv)) — that is the most likely propagation route and it
is unexamined.

**Cost: zero fetches, zero searches.**

## Changes made

1. **`state/issues/graph.json`** — appended **`ctr-0036`** (`attribution-confident-wrong-gap`,
   `opened_cycle` 77, `resolved_cycle` null). Contradictions **35 → 36**; open **25 → 26**.
   Nothing else in the file was touched.
2. **`state/queue/next_task.json`** — rewritten as a **T1** (per the refresh rule) targeting
   `consistency-calibration-as-failure-mode`, `attempt_count` 0, with all nine of that issue's
   `open_questions` spliced in verbatim.
3. **`state/queue/last_completed_task.txt`** — `T5 select`.
4. **`logs/cycle-077.md`** — this file.

**Not touched, and deliberately so:** `state/assessments/scores.json` (a T5 has no standing to
rescore — it still reads `last_assessed_cycle` 76 and every entry `assessed_cycle` 76), the whole
of `state/knowledge/`, and every `candidate_resolution`, `open_question` and `attempts` array in
the graph. **`attempts` is written by the investigating T3/T1, not by the selecting T5** — cycle 74
selected `consistency` and it was cycle 75 that added `75` to its `attempts`.

**Schema discipline, hand-checked because `jq -e .` validates syntax and not schema**
(carry-forward [212]). `.contradictions[-1] | keys_unsorted` returns exactly
`id,issue_id,description,opened_cycle,resolved_cycle`. `.issues|length` still 9.

**Append-only proved mechanically rather than by inspection** — new carry-forward [221]. I dumped
`del(.contradictions)` from the old and new files and `diff -q`'d them (**IDENTICAL**), then dumped
`.contradictions[0:35]` from the new file against the old `.contradictions` and `diff -q`'d those
(**BYTE-IDENTICAL**). Nothing was rewritten or reordered. All scratch files deleted.

## Next task rationale — the ranking table

Applied `prompts/t5_select.md` steps 1–3 mechanically.

**Step 1 — candidate set:** all **nine** issues (every score < 5).
**Step 2 — base priority = score** (lower = higher priority), read from `scores.json`:
`ttp-attack-mapping-reliability` alone at **3**; the other **eight tied at 2**. Total 19, unchanged
since cycle 69.
**Step 3(a) — upstream first.** This is a **reverse-edge** test: *"an issue that others
`depend_on` outranks its dependents."* I **computed** the reverse map with jq rather than copying
the one in my queue entry, because cycle 61 found its predecessor had confused `depends_on == []`
with "has no dependents" and ranked the graph upside down. My computed map matched my entry's on
all nine counts.

| Rank | Issue | Score | Dependents (reverse edges) | Attempt penalty | `created_cycle` | Decided at |
|---|---|---|---|---|---|---|
| **1** | **consistency-calibration-as-failure-mode** | 2 | **3** | **+1** (attempt c75) | 2 | **3(a) — unique max. SELECTED** |
| 2 | attribution-confident-wrong-gap | 2 | 2 | 0 (last c28) | 2 | 3(a) |
| 3= | ioc-extraction-reliability | 2 | 1 | 0 (last c65) | 2 | 3(a), then unbroken |
| 3= | task-dependent-reliability-framing | 2 | 1 | 0 (last c16) | 2 | 3(a), then unbroken |
| 5 | attribution-expressed-confidence-unmeasured | 2 | 1 | 0 (last c49) | 45 | 3(c) |
| 6 | institutional-incident-real-world-impact | 2 | **0** | **+1 or 0 — window-dependent** (attempt c72) | 2 | 3(a) |
| 7= | extraction-vs-reasoning-ordinal-axis | 2 | **0** | 0 (last c18) | 16 | 3(a), then unbroken |
| 7= | **automated-triage-under-refusal** | 2 | **0** | 0 (**`attempts` == []**) | 16 | **3(a) — eliminated** |
| 9 | ttp-attack-mapping-reliability | 3 | 1 | 0 (last c68) | 2 | step 2 (score) |

**The selection is decided at 3(a).** `consistency-calibration-as-failure-mode` is the **unique
maximum at three dependents** among the eight tied at 2, so under the ladder reading 3(b) and 3(c)
are never reached for the winner.

### The window ambiguity, and my queue entry is wrong about it

Carry-forward [189] asks what *"within the last 5 cycles"* means: at cycle 77, **[72,76]** or
**[73,77]**. My entry asserted that `consistency`'s cycle-75 attempt *"falls inside BOTH readings …
and no other issue takes any"* penalty. **The first half is right and the second half is false.**
`institutional-incident-real-world-impact`'s cycle-72 attempt falls **inside [72,76] and outside
[73,77]**. So the ambiguity is live on **two** issues this cycle, not one. **I used the [73,77]
window** — the five-cycle window ending at the current cycle — because that is the reading under
which "the last 5 cycles" includes the cycle doing the counting, and I state it rather than leaving
it implicit. **It changes no outcome**: `institutional` has zero dependents and dies at 3(a) under
either reading.

### The ladder-versus-effective-score divergence, which IS outcome-determining this cycle

Carry-forward [75]/[64] asks whether 3(b) modifies the **effective score before** 3(a) runs, or only
breaks ties **left standing after** it. **Cycle 74 checked this and recorded the outcome as
invariant in its case. At cycle 77 it is not invariant, and the two readings select different
issues.** This is the **first recorded instance of the ambiguity being outcome-determining**.

- **Ladder reading** (3(a) → 3(b) → 3(c)): `consistency` wins on three dependents. **This is what I
  used.**
- **Effective-score reading**: `consistency`'s cycle-75 attempt lifts it to an effective **3**,
  tying it with `ttp` at the bottom; it leaves the leading band entirely, and
  **`attribution-confident-wrong-gap` wins instead** on two dependents.

**Why I used the ladder reading, stated openly rather than silently picked.** `prompts/t5_select.md`
step 3 is headed *"Tie-breaks, in order"* and lists (a) before (b); on the plain structure of that
sentence, (b) is a tie-break applied after (a), and the phrase "to the effective score" describes
how the penalty is expressed, not when it is applied. It is also the reading **every prior T5 has
executed**, which matters here because this table is longitudinal evaluation data for the paper and
switching interpretations mid-run would make cycle 77 incomparable with cycles 48–74.

**The uncomfortable part, recorded plainly because it is the strongest argument against my own
choice.** The penalty clause says in its own parenthesis that it exists to *prevent thrashing on a
stuck issue*. Under the ladder reading it is nearly inert — it can only break a tie that survived
(a) — and the concrete consequence this cycle is that **I have re-selected an issue investigated two
cycles ago**, which is the thrashing the clause names. The effective-score reading serves the stated
purpose better; the ladder reading serves the stated order better. **I cannot resolve a conflict
between a rule's text and its stated rationale, and I have not tried to. A human should.**

### `automated-triage-under-refusal`, eliminated an eighth time, on a refresh firing

**It was eliminated at tie-break (a) for having zero dependents.** By the running count in
carry-forward [30] this is the **eighth recorded elimination**; I inherited the count of seven from
my queue entry and did **not** re-audit all seven, so read "eighth" as *seven inherited plus this
one*. **This is the correct mechanical result and I did not override it.** What I can verify at
source: its `attempts` array is `[]`. **In seventy-seven cycles not one investigation has ever been
sent to it.** Cycle 76's T4 assessed it, on the merits, as **the weakest issue in the graph** —
sole candidate, two sources, and a false limb inside that candidate's own independence argument
(`ctr-0033`, found by cycle 73's G2, the first look anyone had ever taken at it).

**And this cycle was a refresh firing.** The rule that exists to point fresh collection at the
weakest issue fired, and the selector sent it somewhere else. That is the specific evidence
carry-forward [30] has been asking for, and it is now on the record: **a zero-dependent issue can be
the weakest in the graph on every merit criterion and still be unreachable by the selector, because
3(a) eliminates it before any merit criterion is consulted.** A human should decide whether a
never-investigated issue deserves a floor.

### Two unbreakable ties, recorded as evidence for [75]

`ioc-extraction-reliability` and `task-dependent-reliability-framing` tie on score (2), dependents
(1), penalty (0) **and `created_cycle` (both 2)** — the ladder runs out. So do
`extraction-vs-reasoning-ordinal-axis` and `automated-triage-under-refusal` (2 / 0 / 0 / both 16).
Neither tie touches the winner, but **the ladder is demonstrably incomplete for the fifth recorded
time.**

### The task written

`state/queue/next_task.json` is a **T1 (Collect)** targeting
`consistency-calibration-as-failure-mode`, `attempt_count` 0, with **all nine** of that issue's
`open_questions` spliced in **verbatim by jq** rather than retyped — verified two ways: nine
`^open_questions[` line-starts in the rendered instructions, and a `contains()` test of each stored
question against the instruction string returning **true nine times**. It names five concrete
routes so the T1 does not spend its budget rediscovering leads (the `open_questions[5]`
proper-scoring-calibration search, the `open_questions[1]` model-generation gap, `ctr-0035` step
(iii) as a cheap on-issue repair, the `open_questions[7]` provenance routes, and the cycle-16 scope
ruling at `candidate_resolutions[3]` that must be read **before** adding any non-CTI-domain
consistency source). It also carries the phase check, the divergence above, the correction to its
own predecessor's penalty-window claim, and the full carry-forward pointer.

## Budget

- Web searches: **0**
- Web fetches: **0** — the G2 was answerable entirely from records this state had already verified
  at source (carry-forward [214] exercised, and it paid).
- Bash/Grep/Read turns: ~30, of which one was lost to a `/tmp` redirect the sandbox refuses (now
  written into the T1's Bash notes) and one to a shell-quoting failure on a nested apostrophe in a
  jq program (solved by removing the apostrophe).
- State edits: 1 contradiction appended; 2 queue files; 1 log. No source, no score, no candidate,
  no open question touched.
