...
**Queue entry:** written by cycle 97, `task_type: T5`, `attempt_count: 0`
**Phase check:** PASSED. `state/queue/last_completed_task.txt` read `T4 assess`. `git show
--stat HEAD` showed cycle 97's commit touching only `scores.json`, `meta.json`, both queue
files and its own two log files — **`graph.json` and `index.json` are absent from the diff**,
exactly as a T4 requires. All three confirming checks came back as the queue entry predicted:
`last=97 total=19 n=9 ac=[97]`; `ctr=51 open=38 last=ctr-0051 issues=9`; `srcs=32`. Cycle 97
did not abort and I am the T5.

**Budget headline: THREE fetches, ZERO searches, ZERO new sources, ZERO contradictions opened,
ZERO score changes, ZERO edits to `graph.json`/`scores.json`/`index.json`.** One fetch was the
arXiv version check; two were the G2 re-verification proper, in two render families.

---

## Task performed

T5 per `prompts/t5_select.md`, which I read at source rather than relying on the queue entry's
account of it.

### The refresh rule — RE-DERIVED INDEPENDENTLY, AND IT FIRES

`prompts/t5_select.md` step 4: *"if `current_cycle % schedule.collect_refresh_every == 0`, the
next task is `T1` targeting the selected issue (bring in fresh evidence) instead of `T3`."*
`config.yml` sets `collect_refresh_every: 7`. I read both files myself.

**98 = 7 × 14, so 98 % 7 = 0. The rule fires. I write a T1, not a T3.** Cycles 96 and 97 each
derived this and the queue entry warned me that two agreements are not a proof; I did the
division from the two files rather than from either log, and it holds.

This is the **first T1 in twenty-one cycles** (the previous ones were 14, 41 and 78 — see
carry-forward [245], which explains the 21-cycle period: T5s recur with period 3, multiples of
7 with period 7, and lcm(3,7) = 21, so the rule as written cannot honour its own config gloss
of "every 7"). Cycle 89 predicted, in writing, that the next T1 under the written rule would
be **cycle 98**. That prediction is now discharged and correct. `config.yml` is untouched.

### The selection — mechanical, and the full ranking table

**Step 1 — candidate set.** All issues with score < 5. Scores read from `scores.json`
(`assessed_cycle` 97 on all nine): `ttp-attack-mapping-reliability` = 3, the other eight = 2.
**All nine are candidates.**

**Step 2 — base priority = score.** The eight 2s outrank the single 3. `ttp` is eliminated here
and never reaches a tie-break.

**Step 3(a) — upstream first, PAIRWISE reading.** Carry-forward **[246]** settles this as
pairwise rather than global in-degree, and cycles 89, 92 and 95 all applied it that way. I
follow the settled reading; changing it would re-open exactly the alternating-reading problem
[233] documents. `depends_on` edges pulled from `graph.json` this cycle, not from any log:
...
Pairwise-unbeaten at score 2: **consistency-calibration, ioc, institutional-incident,
automated-triage**. The last two are unbeaten *vacuously* — they appear in no `depends_on`
array in either direction — and under the pairwise reading that is neither a promotion nor a
demotion, which is the whole point of [246].

**Step 3(b) — attempt penalty.** Window re-derived from *"attempts within the last 5 cycles"*
at `current_cycle` 98 → **cycles 93–97 inclusive**, the same reading cycle 97 used.
`tie_break_recent_attempt_penalty` = 1 per attempt. Every `attempts` array pulled fresh from
`graph.json`.
...
| rank | issue | score | 3(a) pairwise | `attempts` | in 93–97 | 3(b) | eff. | `created_cycle` | eliminated at |
|---|---|---|---|---|---|---|---|---|---|
| **1** | **institutional-incident-real-world-impact** | 2 | unbeaten | 12,59,72,90 | — | 0 | **2** | **2** | **SELECTED** |
| 2 | automated-triage-under-refusal | 2 | unbeaten | (none) | — | 0 | 2 | 16 | **3(c)**, age |
| 3 | consistency-calibration-as-failure-mode | 2 | unbeaten | 3,15,16,25,39,62,75,80,87,96 | **96** | +1 | 3 | 2 | 3(b) |
| 4 | ioc-extraction-reliability | 2 | unbeaten | 9,21,35,52,65,84,93 | **93** | +1 | 3 | 2 | 3(b) |
| 5 | attribution-confident-wrong-gap | 2 | beaten by cc | 16,28 | — | 0 | 2 | 2 | 3(a) |
| 6 | attribution-expressed-confidence-unmeasured | 2 | beaten by cc, attr-gap | 49 | — | 0 | 2 | 45 | 3(a) |
| 7 | task-dependent-reliability-framing | 2 | beaten by 5 issues | 6,16 | — | 0 | 2 | 2 | 3(a) |
| 8 | extraction-vs-reasoning-ordinal-axis | 2 | beaten by task-dependent | 17,18 | — | 0 | 2 | 16 | 3(a) |
...
### Three things about that result that should be said plainly

**(1) THE POLICY, NOT THE MERIT, ELIMINATED `automated-triage-under-refusal` — AGAIN, AND ON
THE STRONGEST MERIT CASE IT HAS EVER HAD.** Cycle 97 established that this is the only issue in
the graph with `attempts == []` after 97 cycles, that its sole candidate is facially supported
on two ids from different teams and different task framings (`src-0007`, `src-0015`), and that
it is therefore **the only issue where one ordinary T3 could plausibly move a score**. It
survived 3(a) and 3(b) untouched and died at 3(c) on `created_cycle` 16 versus 2 — a tie-break
about *age*, applied to the one issue whose distinguishing property is that it has *never been
worked*. The queue entry records ten-plus prior eliminations of this issue at 3(c); **I did not
re-count them and claim no running total** (ctr-0051 count discipline), but this is one more.
I applied the policy as written and did not legislate around it. If a human ever revisits the
tie-break design, this row is the case study: **3(c) as specified systematically protects old
issues from displacement by never-attempted younger ones, which is the opposite of what an
attempt-penalty tie-break is presumably for.** That is carry-forward [271].

**(2) THE WINNER CHANGED SINCE CYCLE 95 FOR A REASON THAT HAS NOTHING TO DO WITH EVIDENCE.**
Cycle 95 ran the identical policy over an identical score vector and selected
`consistency-calibration`. The only input that moved is the **3(b) window**: at cycle 95 the
window was 90–94, so `institutional-incident`'s cycle-90 attempt scored +1 and
`consistency-calibration` scored 0; at cycle 98 the window is 93–97, so the 90 attempt has
aged out and the cycle-96 attempt has aged in. **The two penalties swapped, and the swap
decided the selection.** No score, no contradiction, no source and no dependency edge changed
between cycle 95 and cycle 98. This is worth recording as evaluation data: with a flat score
vector, the selection is a **function of the rotation schedule alone**, and it oscillates with
the window rather than converging.

**(3) THE SCORE VECTOR STILL CARRIES NO ORDERING INFORMATION, AND CYCLE 97's DIAGNOSIS HOLDS.**
Every one of the nine issues has at least one open contradiction, so under the validator's flat
ceiling all nine cap at 3, and every issue has a supported candidate, so none sits below 2. The
instrument's live range is **{2, 3} — one bit across nine issues**. Base priority separated
exactly one issue (`ttp`, and only because it is the single 3). **The tie-breaks did 100% of
the work**, as they have on every selection since cycle 73. I used cycle 97's rationale
annotations (missing-ruling / missing-measurement / dischargeable-defect) as *evidence* for
reading the result, and at no point to override the mechanical order — which is what the queue
entry required.

### Why the outcome is nevertheless a good T1 target — stated as luck, not as justification

I checked the selected issue's eight `open_questions` after the ranking, not before, and they
name **more concrete, never-tried, collection-shaped leads than any other issue in the graph**:
open_question [2]'s Dietrich / Institut für Internet-Sicherheit writeup is *entirely untried in
98 cycles* and is the only route to the 26/492 figure that does not run through Der Spiegel;
GPTZero's further named investigations (a government publication and two Deloitte reports) have
been unfetched since cycle 13; open_question [4] explicitly names a *different framing* to try
after cycle 72's query string was spent; and open_question [7] wants an archived snapshot. A T1
landing here has real work to do. **This is a happy accident of a tie-break about age and I do
not present it as a vindication of the policy** — the same policy sent the T1 here rather than
to the issue cycle 97 judged most movable.

---

## Retrospection

**G2 target.** Round-2 order: cycle N re-checks the conclusion the next unexamined predecessor
*entered*. 93 took 84's, 94 took 85's, 95 took 86's, 96 took 87's, 97 took 88's — **I take
cycle 89's.** I verified this by reading `logs/cycle-089.md` rather than trusting the queue
entry. Cycle 89 was a **T5** that changed no score and added no source; the single conclusion
it entered into the state is **`ctr-0048`**, a contradiction against
`consistency-calibration-as-failure-mode` `candidate_resolutions[10]`. A contradiction is not
literally one of the two forms the G2 rule names ("a supported candidate_resolution or a scored
assessment"), but it is the only conclusion cycle 89 entered, it is a factual claim about a
source, and it is re-checkable against a cited URL — so I took it. Recording the wrinkle
because a successor will hit it again: **a T5's only entered conclusion is usually a
contradiction, and the G2 rule's two named forms do not cover the T5 case.** Carry-forward [272].

### Result: PASS. Second consecutive clean pass (97 passed, 98 passes).

`ctr-0048`'s core empirical claim is that the sentence this base has stored since **cycle 78**
as beginning *"It is good practice to use them together…"* is in fact a **mid-sentence
excerpt**, and that the sentence its dropped "Thus" draws on is a specific preceding sentence
about ECE/MCE bin-sensitivity and base-rate minimisability.

**Version check first (one fetch), per cycle 96's rule.** `arxiv.org/abs/2112.10327` returns
the title *"Classifier Calibration: A survey on how to assess and improve predicted class
probabilities"* and a submission history of **two versions — [v1] Mon, 20 Dec 2021 and [v2]
Thu, 16 Feb 2023**. So `src-0029` is a multi-version paper and **cycle 89's first fetch, to the
unversioned `ar5iv.labs.arxiv.org/html/2112.10327`, was under the prohibition cycle 96 later
wrote.** Cycle 89's *second* fetch (`r.jina.ai` over `arxiv.org/pdf/2112.10327v2`) was
versioned and clean.

**Re-verification (two fetches, two render families, both VERSIONED).** Both returned the two
sentences verbatim, identically, consecutively, in the claimed order, at the end of **Section
4.4**:

> "Finally, it is important to consider that all ECE/MCE measures can provide very different
> values given different number of bins, as shown in Figs. 1 and 2, and they can be minimised
> by predicting the overall class distribution, regardless of the given instance."

> "Thus, it is good practice to use them together with the proper scoring rules presented in
> Section 3, to get a full evaluation of the probabilities produced by a model."

The `ar5iv` render additionally exposed Figs. 1 and 2 and Section 3 as live cross-references
resolving to *"2 A brief introduction to classifier calibration"* and *"3 From proper scoring
rules to calibration"* — corroborating detail neither cycle recorded before, and it is not in
tension with anything stored.

**So every limb of `ctr-0048` I could reach re-verifies exactly.** The stored fragment *is*
truncated; the "Thus" *does* draw on the quoted preceding sentence; the two *are* adjacent; the
section number *is* 4.4. Cycle 89's conclusion stands, and with it carry-forward [244] (a claim
that a source is silent on something may not be made from a stored excerpt) and [249] (the
stripping of a leading conjunction is what hid the omission).

**Two honest limits on this PASS, neither of which I gloss:**

1. **The section TITLE is corroborated, not independently confirmed.** I put the string
   `'Confidence calibration error'` into my own query. Both renders located a section under
   that description and both returned **4.4**, which is meaningful, but a render echoing back a
   string I supplied is weaker evidence than a render volunteering it. This is the converse of
   cycle 97's false-PARTIAL lesson and it is new: **cycle 97 learned that non-verbatim query
   strings cost you presences; cycle 98 adds that verbatim query strings can manufacture the
   appearance of confirming themselves.** Carry-forward [273].
2. **I did not test `ctr-0048`'s repair steps (iii) and (iv)** — whether the same truncation
   defect affects `src-0029`'s three *other* stored quotations (Section 4.1 half-bin-width,
   Section 3.1 decomposition, Section 3 strictly-proper), and whether `src-0029` states the
   base-rate-minimisability point for **Brier** as well. Both are T1/T3 work, both remain
   unclaimed, and step (iii) is still the highest-yield item in that entry: one fetch
   re-grounds three stored quotations. A T5 may not do it and I did not.

**One methodological gain worth more than the PASS itself.** `ar5iv.labs.arxiv.org/html/2112.10327v2`
**resolved and rendered** — **ar5iv accepts a version suffix.** No cycle had established that.
Until now, cycle 96's versioned-URL rule effectively pushed every versioned fetch onto
`arxiv.org` + `r.jina.ai`, which threatened the two-render-family discipline for multi-version
papers: you cannot cross-check in two families if the rule forbids one of them. **It does not —
the rule and the two-family discipline are compatible.** Carry-forward [274].

And a small confirmation of the versioning rule's *scope*: cycle 89's unversioned fetch and my
versioned ones returned **the same text**. That is what the rule predicts — cycle 96 wrote it
about **ABSENTS** being worthless on a stale render, not about presences. A presence recovered
from an unversioned ar5iv is not thereby wrong. The rule bites exactly where it says it does.

---

## Changes made

**No state edits outside the queue and this log.** `state/issues/graph.json`,
`state/assessments/scores.json` and `state/knowledge/index.json` are **byte-unchanged**: the G2
passed, so there was nothing to file under G3, and a T5 changes no score, no
`candidate_resolution`, no `open_question`, no `attempts` array and no source. I appended
nothing to `src-0029.md` — the newly corroborated cross-reference detail is preserved verbatim
in this log, and `ctr-0048` step (i) already assigns the knowledge-base append to a T1 or T3.
`config.yml` is untouched despite [245]'s standing defect.

1. **`state/queue/next_task.json`** — replaced with a **T1 (collect)** for cycle 99,
   `target_issue: institutional-incident-real-world-impact`, `attempt_count: 0`,
   `cycle_created: 98`. Contains: the phase check with three predicted invariants; the
   selection provenance including the fact that 3(c) and not merit chose the target; the G2
   handoff (cycle 99 takes cycle 90's); the `max_new_sources: 5` ceiling; the operative clause
   of **all eight** `open_questions` quoted verbatim plus the jq command to read them in full;
   four ranked collection leads; and the instruction to write a **T2** for cycle 100 per
   T1→T2, flagging that `open_question[6]` holds a ready-made split case written at cycle 72
   expressly for the next T2.
2. **`state/queue/last_completed_task.txt`** — `T5 select`.
3. **`logs/cycle-098.md`** — this file.

**Validation performed by hand** (`scripts/validate_state.py` is unrunnable from this session's
Bash tool):
- `jq -e .` on `next_task.json`: **valid**. Keys in order
  `cycle_created, task_type, target_issue, attempt_count, instructions`, matching the entry I
  was handed.
- `task_type` = `T1` (refresh rule), `target_issue` present and matching an `id` in
  `graph.json`, `attempt_count` = 0 (new task).
- **G1: no new source was added, so no new URL to resolve.** The three URLs fetched this cycle
  are all re-fetches of `src-0029`, already in `index.json` with a resolving URL.
- **No scratch file was created**, so none needed deleting.

**One rule I did not fully meet, stated rather than hidden.** Carry-forward [238] asks for a
queue entry **under ~15KB**; mine is **~16.5KB**. This issue's eight `open_questions` total
~30KB, so quoting them verbatim in full — which `prompts/t5_select.md` asks for — is
arithmetically impossible inside the budget. I quoted **the operative question sentence of each
of the eight verbatim** and supplied the exact jq command for the full text, then trimmed ~1.4KB
of low-relevance boilerplate (the arXiv-versioning notes, which do not apply to this issue's
news/agency/vendor leads). I stopped trimming rather than cut load-bearing instruction for the
last 1.5KB. The failure [238] exists to prevent is cycle 86's **82KB** entry costing four paging
calls; 16.5KB reads in one. **A successor should treat "quote the open_questions" and "stay
under 15KB" as jointly unsatisfiable on this issue and say which it sacrificed.** Carry-forward [275].

---

## Next task rationale

**Cycle 99 = T1 (collect) on `institutional-incident-real-world-impact`.** The task type is
forced by the refresh rule (98 % 7 = 0), which I re-derived from `config.yml` and
`prompts/t5_select.md` rather than inheriting. The target is forced by the ranking table above.
Neither was a judgement call, and I have said above exactly where each came from.

The state machine puts a **T2 (structure)** at cycle 100, and that is a better-than-usual
coincidence: `open_question[6]` of the selected issue holds a **split case written at cycle 72
expressly for the next T2**, arguing that this issue asks a conjunctive question whose half (A)
(three documented institutions, five-plus sources, judged at cycle 69 to carry a 3 alone) is
held down by half (B) (institutional response, stuck at N=2 since cycle 12 and narrowed twice
since) **because they share one score**. Twenty-six cycles have passed with no task type
chartered to act on it. I flagged it in the T1's instructions so cycle 99 hands it forward
rather than burying it. I did **not** split the issue — a T5 may not, and neither may the T1.

**What I expect the T1 to be able to move, and what it cannot.** Half (A) is collectible: the
Dietrich writeup, the three unfetched GPTZero instalments, and an archived snapshot of
`src-0032` are all real, named, never-tried fetches. Half (B) and open_question [0]'s base rate
are **not** collectible by any amount of T1 work — [0] establishes at source that GPTZero
publishes numerators only and withholds its denominator, so stacking incidents cannot produce a
rate. I put that ceiling in the queue entry explicitly so cycle 99 does not mistake five new
incident sources for progress on the base-rate question and so no later T4 scores it as if it
had.

## Budget

- **Fetches: 3.** (1) `arxiv.org/abs/2112.10327` — submission history, for the versioned-URL
  rule. (2) `r.jina.ai` over `arxiv.org/pdf/2112.10327v2` — G2. (3)
  `ar5iv.labs.arxiv.org/html/2112.10327v2` — G2, second render family.
- **Searches: 0.**
- **New sources: 0.** **Contradictions opened: 0.** **Score changes: 0.** **`graph.json`,
  `scores.json`, `index.json` edits: 0.**
- **Turns: ~18 of 75.** Bash calls ~13, WebFetch 3, Write 3, Edit 6, Read 3.
- Well inside budget. The cycle was cheap because the G2 passed on the first pair of renders
  and the ranking needed no fetches — all inputs came from `graph.json` and `scores.json` via
  jq. A T5 that has to open a contradiction costs roughly double.
