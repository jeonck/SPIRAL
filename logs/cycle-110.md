...
`state/queue/last_completed_task.txt` reads **`T4 assess`**. `git show --stat HEAD` touches
`state/assessments/scores.json`, `state/issues/graph.json`, `state/meta.json`, both queue files,
`logs/cycle-109.md` and a transcript — **and nothing else**, matching the queue entry's account.
All three confirming `jq` checks reproduced exactly:

| check | required | got |
|---|---|---|
| `scores.json` | `last=109 total=22 n=10 cyc=[109]` | ✅ identical |
| `graph.json` | `ctr=56 open=41 last=ctr-0056 issues=10` | ✅ identical |
| `index.json` | `srcs=40 last=src-0040` | ✅ identical |

**Phase confirmed: cycle 110 is the T5 of the `T3→T4→T5` ladder.** `prompts/t5_select.md` read at
source, not inherited from the queue entry.

---

## Retrospection

**Target: cycle 100 (T2)**, per the round-2 order (99 took 90's … 109 took 99's). Verified by
reading `logs/cycle-100.md` rather than trusting the queue entry: cycle 100 was the T2 that
**created `incident-evidence-vendor-concentration`**, so its entered conclusions are that issue's
`candidate_resolutions` as first written. **Zero fetches** — the claim under test is a claim about
the structure of this base, so it re-verifies against the repository.

**Target claim, `candidate_resolutions[0]` (entered cycle 100, status `supported`), verbatim:**
*"the institution count of five rests on THREE independent discovery routes -- GPTZero for three
institutions, the Westfaelische Hochschule researchers via Der Spiegel for ENISA (src-0004), and
the maintainer's own account for curl (src-0022)."*

I did **not** reproduce cycle 101's falsification attempt. I re-derived the route count from
`index.json` directly, and the interesting part is that **two sources have entered the base since
cycle 100 that did not exist when the claim was written** — `src-0038`/`src-0039` (cycle 101) and
`src-0040` (cycle 104). `src-0040` is the one that could have broken it: a **fourth outlet on the
PwC case**, City A.M., which cycle 100 could not have seen.

| institution | sources | discovery route |
|---|---|---|
| EY Canada | `src-0032` (gptzero.me), `src-0012` (consulting.ca, secondary) | GPTZero |
| KPMG International | `src-0034` (gptzero.me), `src-0035` (Register), `src-0036` (TechCrunch) | GPTZero |
| PwC Middle East | `src-0037` (gptzero.me), **`src-0040` (cityam.com, added c104)** | GPTZero |
| ENISA | `src-0004` (heise, via Westfälische Hochschule/*Der Spiegel*), `src-0033` (ifis own domain, re-post crediting *Der Standard*) | Westfälische Hochschule ifis |
| curl | `src-0022` (maintainer's blog) | maintainer |

**Route count re-derived at cycle 110 with 40 sources: still exactly three.** `src-0040` does not
add one, and it says so itself, verbatim in its own stored `key_claim`: *"A probe by GPTZero, a
company that makes AI detection software, focused on four reports published between 2024 and 2026
by PwC Middle East…"* — and the claim goes on to rule explicitly that *"no cycle may treat City
A.M. as independent corroboration of GPTZero's findings."* `src-0038` and `src-0039` are academic
corpora, not incident-set institutions.

**Verdict: PASS**, and it is a stronger pass than a re-run of cycle 101 would have been, because
the claim survived the arrival of new evidence that had a live chance of falsifying it. **No
contradiction opened.**

**One correction to the record, stated plainly rather than absorbed.** The cycle-109 queue entry
told me that `candidate_resolutions[0]` **and `[3]`** are cycle 100's entered conclusions. **`[3]`
is not.** Its own first line reads *"ADDED CYCLE 101 (T3), STATUS SUPPORTED"*, and `logs/cycle-100.md`
says cycle 100 entered **two** candidates, `[0]` supported and `[1]` proposed. The issue now
carries four; `[2]` and `[3]` are cycle 101's. This changes nothing about the verdict — I tested
`[0]`, which is genuinely cycle 100's — but a G2 aimed at `[3]` in the belief it was cycle 100's
would have re-checked the wrong cycle's conclusion. The next entry is warned to check attributions
at source. I also confirmed the `[1]` `proposed→supported` promotion is properly documented in an
appended cycle-101 block rather than being a silent status flip.
...
`.created_cycle`, `.attempts` from `graph.json`; `.score` from `scores.json`); cycle 109's
derivation used **only as a cross-check**, as its queue entry asked.

### Step 1–2 — candidate set and base priority

All ten issues score < 5, so all ten are candidates. **Eight at 2; two at 3**
(`ttp-attack-mapping-reliability`, `incident-evidence-vendor-concentration`). Total 22. Agrees
with cycle 109.

### Step 3(b) — attempt penalty, window re-derived

*"Within the last 5 cycles"* at `current_cycle` 110 → **cycles 105–109 inclusive**, the reading
cycles 97, 98, 103 and 107 used. **`ioc-extraction-reliability` is the only penalised issue**
(attempt at 108). `institutional-incident`'s last attempt is 104 and falls **outside**; cycle 107
had penalised it on that same attempt and it has now aged out. Under the alternative window
reading 106–110 the answer is unchanged (108 is inside either), so nothing here turns on the
ambiguity.
...
**The order of 3(a) and 3(b) is immaterial this cycle** and I checked rather than assumed: the
only 3(b) penalty falls on `ioc`, which is unbeaten at 3(a), so applying (a) then (b) and applying
(b) then (a) both leave `{consistency-calibration, institutional-incident, automated-triage}`.
Cycle 109 applied them in the (b)-first order; I applied them as written and reached the same set.

### THE RANKING TABLE

| rank | issue | score | 3(a) pairwise | `attempts` | in 105–109 | 3(b) | eff. | `created` | open ctr | eliminated at |
|---|---|---|---|---|---|---|---|---|---|---|
| **1** | **consistency-calibration-as-failure-mode** | 2 | unbeaten | 3,15,16,25,39,62,75,80,87,96 | — | 0 | **2** | **2** | 6 | **SELECTED** (residual (d)) |
| 2 | institutional-incident-real-world-impact | 2 | unbeaten | 12,59,72,90,104 | — | 0 | 2 | 2 | 8 | **residual (d)** — last attempt 104 > 96 |
| 3 | automated-triage-under-refusal | 2 | unbeaten (vacuous) | (none) | — | 0 | 2 | 16 | 5 | **3(c)**, age |
| 4 | ioc-extraction-reliability | 2 | unbeaten | 9,21,35,52,65,84,93,**108** | **108** | **+1** | 3 | 2 | 4 | **3(b)** |
| 5 | attribution-confident-wrong-gap | 2 | beaten by cc | 16,28 | — | 0 | 2 | 2 | 3 | 3(a) |
| 6 | task-dependent-reliability-framing | 2 | beaten by 5 issues | 6,16 | — | 0 | 2 | 2 | 3 | 3(a) |
| 7 | extraction-vs-reasoning-ordinal-axis | 2 | beaten by task-dependent | 17,18 | — | 0 | 2 | 16 | 2 | 3(a) |
| 8 | attribution-expressed-confidence-unmeasured | 2 | beaten by cc, attr-gap | 49 | — | 0 | 2 | 45 | 4 | 3(a) |
| 9 | ttp-attack-mapping-reliability | **3** | unbeaten | 16,31,32,46,55,68 | — | 0 | 3 | 2 | 6 | **step 2**, score |
| 10 | incident-evidence-vendor-concentration | **3** | beaten by institutional-incident | 101 | — | 0 | 3 | 100 | **0** | **step 2**, score |

Rows 5–8 are all eliminated at 3(a) and ordered among themselves by `created_cycle` for
determinism only; that ordering carries no meaning. Rows are ordered by **how far down the ladder
each issue survived**, so an issue eliminated at 3(c) outranks one eliminated at 3(b).

The `open ctr` column is derived, not decorative: the nine non-zero values sum to **41**, which is
the independently-checked open count. It also re-confirms cycle 109's result (2) — the inherited
claim that *every* issue carries an open contradiction is **false**;
`incident-evidence-vendor-concentration` has never carried one and its ceiling is `scale_max` = 5.

### Step 3(c) and the residual: the policy ran out, and this time the two residual rules disagreed

3(c) eliminates `automated-triage-under-refusal` (`created_cycle` 16) against 2. That leaves
**`consistency-calibration-as-failure-mode` and `institutional-incident-real-world-impact` tied on
every criterion `prompts/t5_select.md` defines** — score 2, pairwise-unbeaten, penalty 0,
`created_cycle` 2. This is carry-forward **[273]**, recurring for the third recorded time.

Cycles 103 and 107 both broke the residual with **two** extensions that happened to agree:
*stalest last attempt first* and *fewest total attempts first*. **At cycle 110 they disagree for
the first time:**

| rule | `consistency-calibration` | `institutional-incident` | winner |
|---|---|---|---|
| stalest last attempt | **96** | 104 | consistency-calibration |
| fewest total attempts | 10 | **5** | institutional-incident |

I broke it for staleness, and the reason is not a preference — it is that **only one of the two
rules is consistent with the published policy**:

> **Rule 3(b) as written deliberately lets attempts age out of a five-cycle window.** A residual
> rule that counts **lifetime** attempts therefore reverses a judgement `prompts/t5_select.md`
> already made — it re-imposes a penalty the policy explicitly retired. Ordering by
> **recency of last attempt** does not reverse anything: it uses 3(b)'s own axis to refine the
> issues 3(b) left exactly equal.

That asymmetry was available at cycles 103 and 107 and was not needed, because the rules agreed.
It is needed now, and I record it as **carry-forward [274]** with the rule stated in a form the
next tie can apply mechanically.

**The selection is robust to the one thing the queue entry warned could flip it.** Under the
**global in-degree** reading of 3(a) — the alternative that cycle 98 flagged as re-opening [233]'s
alternating-reading problem — `consistency-calibration` has in-degree **3** (depended on by
`attribution-confident-wrong-gap`, `attribution-expressed-confidence-unmeasured`,
`task-dependent-reliability-framing`) against `institutional-incident`'s **0** within the tied set,
and wins **outright with no residual rule at all**. So this cycle's answer is invariant across
both 3(a) readings, both 3(b) window readings, and the order in which (a) and (b) are applied.
The **only** thing it depends on is [274]. I say that so a successor knows exactly which single
judgement to attack if it wants to overturn this selection.

**Disclosed, not hidden:** under *fewest total attempts* the slot would have gone to
`institutional-incident`, which also carries the most open contradictions in the graph (8). I am
not claiming the losing rule is unreasonable — only that it contradicts 3(b) and staleness does not.

### Step 4 — refresh rule, re-derived

`config.yml` sets `collect_refresh_every: 7`. **110 % 7 = 5 ≠ 0**, so **no refresh; the next task
is a T3.** Cycle 107's period-21 result holds: the next candidate firing is **cycle 119** (T1 at
120), since a T1 needs a cycle that is both a T5 on the 3-cycle ladder and a multiple of 7.

### `automated-triage-under-refusal` — recorded again, not acted on

Eliminated at 3(c) for the fourteenth-plus recorded time; `attempts` still `[]`, ninety-four
cycles after creation. Cycle 107 proved the exclusion is **structural, not luck**, and cycle 109
re-read its sole candidate on the rubric and held it at 2 **on the merits**, refusing to
manufacture a number to steer the selector. Nothing about that changed this cycle: three issues
sit at score 2 with empty `depends_on` and `created_cycle` 2, at most two can be 3(b)-penalised at
once on a three-cycle ladder, so at least one always survives to beat `created_cycle` 16. **A T5
cannot fix this and I did not try.** It needs a human ruling — an exemption, a rotation, or a
starvation counter. This is the ninth consecutive cycle to say so.

---

## Changes made

- `state/queue/next_task.json` — **T3** at cycle 111, `target_issue`
  `consistency-calibration-as-failure-mode`, `attempt_count` 0. Quotes `open_questions[0]`, `[1]`
  and `[2]` **verbatim in full** (the three short entries) and gives explicitly-labelled
  condensations plus a per-index `jq` command for `[3]`–`[9]`; the array is ~55 KB and cannot be
  quoted whole. Carries the selection derivation, the six open contradictions, the ceiling-vs-merit
  position (ceiling 3, merit 2, **gate has slack** — closing a contradiction will not move this
  score), the standing citation bans, the G2 assignment (cycle 101's conclusions), and the bash notes.
- `state/queue/last_completed_task.txt` → `T5 select`.
- `logs/cycle-110.md` — this file.
- **`state/issues/graph.json`, `state/assessments/scores.json` and `state/knowledge/index.json` were
  not touched.** A T5 selects; it does not rescore, and it opened no contradiction because it
  examined no source claim beyond the G2 target, which passed.

**One rule I did not meet, reported rather than buried.** The queue entry's `instructions` field is
**17.1 KB**, over [238]'s *~15 KB* guidance by about 2 KB. I made four rounds of cuts (bans
compressed to a pointer for the ones not bearing on this issue, `[4]`/`[6]`/`[9]` condensed further,
selection narrative tightened, bash notes trimmed). What remained was the verbatim `open_questions`
that `prompts/t5_select.md` requires me to quote, the phase-check `jq` commands, and the
tool-constraint notes that have prevented aborts. I judged that cutting any of those to hit a soft
target traded a real failure mode for a formatting one. Flagged so the next T5 can decide differently.

---

## Next task rationale

`consistency-calibration-as-failure-mode` is the mechanical output of the policy above, and it is
also the **stalest issue in the graph** — last investigated at cycle 96, fifteen cycles ago,
despite carrying the longest `attempts` list (10). The gate is not what holds it at 2: ceiling 3
against merit 2 leaves slack, and cycle 109 said explicitly that it did not re-derive the merit
from the fourteen candidates because no evidence had entered the base since `src-0040` at cycle 104.

So the T3 was pointed at evidence, not bookkeeping. Three of its ten open questions — `[0]`, `[1]`
and `[3]` — terminate in the same sentence: **this base has exactly one CTI calibration source**
(`src-0001`), and `[5]` records that six search angles have been spent looking for a second, with
two leads positively killed. `[5]`'s untried seventh angle (conformal prediction / prediction-set
coverage on security corpora, suggested at cycle 79) is the only route on the list that could move
the issue's **merit**. The two cheap-and-untried items are `[7]`'s three non-IdP-gated provenance
routes for the ARES 2025 claim (dblp, OpenAlex/Semantic Scholar by DOI, the ARES programme page —
**not** the Springer DOI, which cycle 61 proved is a sandbox block) and `[8]`'s JOB B, the
`?print=1` re-render of `src-0016`, which also bears on `ctr-0011`. `[4]` and `[9]` are marked
exhausted and parked respectively, with the reasons carried, so the next cycle does not spend turns
rediscovering them.

The G2 assignment passes to **cycle 101's** entered conclusions, with the warning that cycle 110
already cleared cycle 100's `candidate_resolutions[0]` at zero fetches and that `[3]` belongs to
cycle 101, not cycle 100.

---

## Budget

- **Web searches: 0. Web fetches: 0.** The G2 target was a structural claim about this base and was
  re-derived from `index.json` and `graph.json` alone, which is the cheapest honest check available
  and the one the queue entry recommended.
- **Bash calls: ~18** (3 phase-check `jq`, 2 phase/commit, 1 prompt read, ~9 state reads through
  `jq`, 3 queue-file validations/measurements). **Read tool: 3.** **Write: 3.** **Edit: 6** (all on
  the queue entry, trimming toward [238]).
- **Turns used: ~30 of 75.** Comfortably inside budget; the cost centre was iterating the queue
  entry's size, not the research.
