...
**T4 — Assess.** Queue entry `state/queue/next_task.json` (created cycle 46) named
`task_type: "T4"`, `target_issue: "ttp-attack-mapping-reliability"`, `attempt_count: 0`.
...
- `git show --stat HEAD` = `aba325c cycle 46: T3 investigate`, touching `graph.json`,
  `index.json`, a new `src-0021.md`, and both queue files. I disbelieved the commit message
  and checked the state itself.
- `jq -r '.issues[] | select(.id=="ttp-attack-mapping-reliability") | "cr=\(.candidate_resolutions|length) attempts=\(.attempts|tostring)"'`
  returned **`cr=5 attempts=[16,31,32,46]`**, and `.contradictions|length` returned **15**.

Cycle 46 landed. I am the T4, not a re-run of the T3. I read `prompts/t4_assess.md` myself
rather than relying on the queue entry's account of it, and I read
`scripts/validate_state.py` lines 100–156 myself for the G1/S/G3 checks.

**The refresh rule does not apply to me and I checked the arithmetic rather than inheriting
it.** `prompts/t5_select.md` step 4 puts the rule *inside T5*; `config.yml` line 22 gives
`collect_refresh_every: 7`. I am a T4 and do not run the selection. Cycle 48 runs the next
T5 and `48 % 7 == 6`, so **cycle 48's T5 schedules a T3, not a T1** — unless it selects an
issue for which it has some other reason. Recomputed, not copied.

---

## Retrospection

**G2 target: `src-0020`** — *Evaluating Open-Source LLMs for Multi-Label ATT&CK Technique
Classification on CTI Reports*, `https://arxiv.org/abs/2606.18166`, added cycle 43,
**never previously G2'd**.

**Why this target over the stalest source.** The handoff recommended `src-0005` purely on
staleness (last checked cycle 26, twenty-one cycles). I chose `src-0020` instead because
**ctr-0015 step (iv)** asked a specific, cheap and consequential question *of this source* —
*are `src-0020`'s per-technique kappas themselves restricted in any way?* — and because
`src-0020` is one of three cycle-43 sources that has never been re-checked at all. A
targeted question beats an untargeted staleness sweep when both are available.
`src-0005` remains the stalest source in the base and is carried forward.

### VERDICT: FAILED. ctr-0016 opened.

**The answer to ctr-0015 step (iv) is yes.** `src-0020`'s κ=0.68 is computed **on a random
10% sample of 195 sentences**, at **phase (ii) of six**, before relabelling and dispute
resolution. The state stores neither restriction.

**What the state stored.** `src-0020` `key_claims[1]` renders the computation clause with an
ellipsis, ending at "chance agreement":
...
confidence interval". Both true as far as they go; neither records *which items* the values
were computed over. **ctr-0015 relied on that silence**, describing this estimand as "the
only one of the three measured over an unrestricted item set."

**What the source says.** Verification-mode fetch of `https://arxiv.org/html/2606.18166v1`
on 2026-08-01 returned the whole six-phase annotation paragraph, then a **second, separate
exact-string ask** on the literal strings `on a random 10% sample (195 sentences)` and
`which was collected from both annotators` returned PRESENT for both, same clause. Rule
(xiii) was complied with deliberately — a summarised read can manufacture a defect, and a
false contradiction is more expensive than a missed one (cycle 21's survived eighteen
cycles). The paragraph, verbatim:
...
(Bold added here only; the source has no emphasis.)

**Three consequences.**

1. **Precision.** 114 per-technique kappas each estimated from 195 sentences. The paper
   reports 2,028 technique occurrences over 2,076 sentences, so a 10% sample carries of the
   order of 200 occurrences across 114 techniques — **of the order of two positive instances
   per technique** — and the mean is reported **with no confidence interval**. *That
   per-technique figure is a cycle-47 derivation from two printed numbers and is labelled as
   such under rule (xii); the printed numbers are exact.*
2. **A tension inside the source, not a state defect.** The headline sentence — "Following
   the six-phase process, we reached a mean inter-annotator agreement of κ=0.68 across 114
   techniques (Landis and Koch: substantial agreement), with 65 techniques (57.0%) exceeding
   the κ≥0.7 quality threshold" — re-verified PRESENT **as one whole sentence**, so the
   base's stored quotation of it is accurate. But kappa is computed at phase (ii), with
   phases (iii)–(v) after it and phase (iv) described as a re-check "to verify κ
   improvement". "Following the six-phase process, we reached" and "computed at phase (ii)
   of six" cannot both be read strictly. No sentence read so far says the 0.68 was
   recomputed. **Nobody may resolve this by picking the reading that suits them.** That is
   ctr-0016 step (iv), open.
3. **A number this base had never recorded, and it is better evidence than the 0.68.**
   **981 of 2,076 sentences (47.3%, cycle-47 derivation from two printed numbers)** were
   "low-agreement (κ<0.7)" and required relabelling — measured over the **whole corpus**,
   not a sample. This is now the cleanest quantitative evidence in the base that parent-level
   ATT&CK sentence labelling is a contested annotation task.

**What re-verified clean.** Every stored numeral and every other stored quotation in
`key_claims[0]`, `[1]`, `[2]` returned PRESENT under exact-string ask: the 0.22 micro-F1
sentence; the full 2,076 / 114 / 2,028 / 211 dataset sentence, which the base stores
**complete** including its "totaling 2,028 technique occurrences across 211 potential
candidates" tail; the 211-parent-technique granularity sentence; the DFIR Report caveat.
The fetch reported the References section visible through reference [58], i.e. the document
was **complete, not truncated** (rule xvi).

**Version-staleness bolt-on, discharged for this source.** `arxiv.org/abs/2606.18166` on
2026-08-01 still lists `[v1] Tue, 16 Jun 2026 17:04:15 UTC (25 KB)` as its **only** version,
with **no** `Comments:` and **no** `Journal reference:` line (a `DOI:` line is present and is
only the auto-assigned `10.48550/arXiv.2606.18166`). `key_claims[2](a)`'s unreviewed-v1 limit
stands, re-confirmed rather than assumed.

**What is withdrawn.** ctr-0015's "the only one of the three measured over an unrestricted
item set" is **false and withdrawn**. All three pooled kappas are restricted or aggregated:

| estimand | value | computed over |
|---|---|---|
...
The "0.68–0.76 band" is **worse off than ctr-0015 left it**, and "two teams, two corpora, one
conclusion" must not be used. The qualitative point survives and now rests most strongly on
the 981/2,076 figure. The standing prohibition from `open_questions[1]` is undisturbed and
reinforced: **an agreement statistic is not an accuracy baseline.**

**Methodological notes for the taxonomy.** This is the **third** instance of the
omitted-clause shape (ctr-0012, ctr-0015, ctr-0016) and it is now the most frequent single
shape in the base. Rule (viii) is confirmed a **third** time: every stored string in
`src-0020` matched exactly and the defect was entirely in *what was not stored*. And a new
observation worth a rule: **ctr-0016 is the first time a repair step a predecessor marked
"NOT DONE AND OPTIONAL" has been discharged by a successor — and it found a defect.**
Optional repair steps are not low-yield; they are unpriced. Twenty-three source-checks have
now run and **fourteen** produced a defect.

---

## Changes made

1. **`state/issues/graph.json`** — appended **ctr-0016** (`issue_id`
   `ttp-attack-mapping-reliability`, `opened_cycle` 47, `resolved_cycle` null, 8,013 chars),
   recording the sample restriction, the phase-(ii) tension, the 981/2,076 figure, the
   withdrawal of ctr-0015's "unrestricted item set" wording, and a four-step resolution path
   of which (i) and (ii) are done this cycle. Validated with `jq -e`; open-contradiction map
   re-derived after the edit. **No issue, candidate_resolution or open_question was touched
   — that is T2/T3 work.**
2. **`state/knowledge/index.json`** — appended `src-0020` `key_claims[3]` (append-only
   respected; the source now has 4 claims, up from 3). ctr-0016 step (i).
3. **`state/knowledge/src-0020.md`** — appended the same as a cycle-47 G2 addendum.
   ctr-0016 step (ii). **Done in the same cycle as step (i), following cycle 46's example**
   — this is the step ctr-0011, ctr-0012, ctr-0013 and ctr-0014 each left undone on their
   own source files, and deferral is why all four are still undone.
4. **`state/assessments/scores.json`** — **all nine issues scored**, `assessed_cycle` set to
   47 on every entry, `last_assessed_cycle` raised 41 → 47. Every entry at ≥2 carries a
   non-empty `evidence` array of ids that exist in `index.json` (validator line 138). Each
   rationale gained an appended `[CYCLE 47 (T4) ...]` block; no prior rationale text was
   deleted, because `graph.json` candidate 1 of `ttp-attack-mapping-reliability` explicitly
   relies on the cycle-29 scores.json rationale as its audit record.

### The scores

| issue | 41 | **47** | ceiling | note |
|---|---|---|---|---|
| ttp-attack-mapping-reliability | 2 | **2** | 3 | +2 candidates, +1 contradiction; held |
| ioc-extraction-reliability | 2 | **2** | 3 | 4 open contradictions |
| consistency-calibration-as-failure-mode | 2 | **2** | 3 | calibration limb still single-source |
| attribution-confident-wrong-gap | 2 | **2** | 3 | error-rate leg only after cycle 45 split |
| **attribution-expressed-confidence-unmeasured** | **0** | **1** | 5 | **only move; placeholder corrected** |
| task-dependent-reliability-framing | 2 | **2** | 3 | closest to a real 3 |
| extraction-vs-reasoning-ordinal-axis | 2 | **2** | 3 | rests on unaudited derivations |
| institutional-incident-real-world-impact | 3 | **3** | 3 | **ceiling binds exactly** |
| automated-triage-under-refusal | 2 | **2** | 5 | pure merit; no gate pressure |

**Only one score moved, and it is a correction, not a finding.**
`attribution-expressed-confidence-unmeasured` was created by cycle 45's T2 with score 0 and
`assessed_cycle` 0 — a placeholder, since a T2 has no standing to assess. The rubric's line
for 0 is "unexamined: **no candidate resolutions**", and the issue has **one**. So 0 is
factually false of it whatever one thinks of the merits. 1 — "framed: open questions
articulated, no supported resolution" — describes it exactly. **I rejected 2**: the single
candidate nominally carries three source ids, but it is a *survey negative* (no source in
this base measures expressed confidence on attribution) and those ids are evidence **for an
absence** — the things surveyed, not sources supporting a positive claim. Counting them
would let any well-documented gap score 2 or 3 purely for being well documented.

### Policy question [97], decided explicitly

Cycle 46's candidate 5 forced this and it is no longer hypothetical: **may a
non-commensurability finding count as a resolution?** My reading, stated so a successor can
audit it and reverse it in writing:

> **A non-commensurability finding is a genuine resolution of a methodological question but
> not of an issue whose title asks for a magnitude.** Four sources agreeing that four numbers
> cannot be compared is not four sources supporting an answer; it is four sources
> establishing that the answer is not yet available.

Applied consistently to three issues this cycle: `ttp-attack-mapping-reliability` candidate 5
(4 sources, would have cleared the count for a 3 — did not), `automated-triage-under-refusal`
(2 sources agreeing on a direction while a standing prohibition forbids pooling them), and
`attribution-expressed-confidence-unmeasured` (survey negative). I record the contrary
reading as respectable: on the rubric's plain words, candidate 5 qualifies. **[97] still
needs a human.** I decided it only for this cycle's arithmetic.

### G3, re-derived not inherited

`jq -r '[.contradictions[] | select(.resolved_cycle==null)] | group_by(.issue_id)[] | "\(.[0].issue_id): \(length)"'`
**before** my edit returned exactly the map cycle 46 verified — ioc 4, consistency 1,
attribution-confident 1, task-dependent 1, institutional 1, extraction 1, ttp 1, and the two
other issues absent. The cycle-45 handoff's counts (ioc 5, consistency 3) are wrong;
ctr-0003, ctr-0005 and ctr-0007 all carry non-null `resolved_cycle`. Confirmed independently.
After adding ctr-0016, ttp has **2**.

`scripts/validate_state.py` lines 144–156 error only if such an issue scores **> 3**; lines
146–149 build the open set as a **set comprehension over `issue_id`**, so duplicates collapse
and **four open contradictions demote exactly as much as one**. I applied the **CEILING**
reading throughout and **refused the SUBTRACTION**, which would stamp issues holding five
supported candidates with the rubric label "no candidate resolutions". That conflict is
carry-forward **[4]**, now in its **thirty-eighth** cycle and still awaiting a human. The
collapse behaviour is itself worth a human's attention and I have written it into
`ioc-extraction-reliability`'s rationale: a gate meant to track evidential trouble cannot
sensibly treat four open contradictions the same as one.

---

## Next task rationale

State machine: **T4 → T5**. Cycle 48 runs the selection. I have **not** pre-selected a target
— a T5 selects its own, and pre-selecting would be doing the next task's work.

Two things the T5 will need and that I established this cycle rather than asserting:

- **`48 % 7 == 6`**, so the collect-refresh rule in `prompts/t5_select.md` step 4 does **not**
  fire at cycle 48; it schedules a **T3**. The next firing is cycle 49 *if* a T5 runs then,
  which under T5→T3→T4→T5 it will not — the T5 cycles are 48, 51, 54…, so the next
  refresh-eligible T5 is **cycle 56**. **Check that arithmetic yourself**; cycle 41's handoff
  got a projection of this wrong by fourteen cycles.
- **The weakest-link picture changed.** `attribution-expressed-confidence-unmeasured` is now
  the unique minimum at **1**, seven issues sit at 2, and `institutional-incident` at 3. That
  issue has `attempts []`, has **never** been a T1 target, and its primary open question is a
  **collection** question — so a T3 aimed at it would find nothing to investigate. That is a
  real tension between the state machine and the issue, and I have flagged it rather than
  resolved it, because resolving it is the T5's call.

---

## Budget

- **Fetches: 3.** Two against `arxiv.org/html/2606.18166v1` (discovery, then exact-string
  confirmation), one against `arxiv.org/abs/2606.18166` (version/provenance).
- **Searches: 0.**
- **Turns: ~26 of 75.** Well inside budget; the 50→75 raise was not needed this cycle but
  the margin is what let me do ctr-0016 steps (i) and (ii) properly instead of deferring them.
- **Edits: 13** (1 graph.json, 1 index.json, 1 heredoc append to src-0020.md, 10 scores.json)
  plus 3 file writes. Every JSON edit validated with `jq -e` and read back with `jq -r`.
- **Sandbox notes for a successor.** The `jq -r` tail-projection → short unique anchor → Edit
  → `jq -e` → `jq -r` read-back pattern worked without a single failed edit this cycle. New
  finding: **`scores.json` has one field per line**, so an anchor spanning a rationale's tail
  plus the following `evidence` and `assessed_cycle` lines updates all three in **one** edit —
  that halved the edit count. The 25,000-token Read cap still makes a bare `Read` of
  `scores.json` useless; `jq -r '.scores | to_entries[] | ...'` is the way in.
