# Cycle 151 — T3 investigate

## Task performed

T3 (Investigate) per `prompts/t3_investigate.md`, read at source this cycle rather than
taken from the queue entry's account of it. Target issue:
`institutional-incident-real-world-impact`.

**Phase check.** `state/queue/last_completed_task.txt` read `T2 structure`. `git show
--stat HEAD` gave `8c38ac3 cycle 150: T2 structure`, touching `state/issues/graph.json`,
`state/meta.json`, both `state/queue/` files and `logs/cycle-150.md` — and nothing else
beyond the harness-added transcript. `state/knowledge/` and
`state/assessments/scores.json` are untouched by cycle 150, as a T2 requires. HEAD was a
cycle commit, so no walk-back was needed. `prompts/system.md` line 46 read at source
gives T2 → T3, so I am a T3. All four confirming checks passed exactly:
...
I did **not** run `scripts/validate_state.py` — bash refuses `python3` here. Every gate
check below is a hand-check with `jq`, not a validator run.

**Step zero, which the queue entry made non-optional: I read all five of
`src-0049` through `src-0053` end to end before reading cycle 150's questions.** Cycle
150 expressly did not read them and said its recommendation was worth less for it. That
turned out to matter: I adopted its recommendation on limb (i) and **rejected the ground
it proposed for limb (ii)**, which I could not have done without the source files.

**Zero fetches and zero searches this cycle.** The whole investigation ran off the
knowledge base, which is what `prompts/t3_investigate.md` step 2 asks for.

## Retrospection

**G2 target: cycle 141 (T5), commit `0d0630d`, re-checked at ZERO fetches.**

Cycle 141 was a T5. A T5 writes no scores, no sources and no issue edits, so the
conclusion it *entered* is its **selection and the reasoning behind it**, carried in
`state/queue/next_task.json`. I stat'd the commit before trusting the message: it touched
`logs/cycle-141.md`, `state/meta.json` and both `state/queue/` files, and nothing else —
consistent with a T5. I then re-derived every checkable limb from
`git show 0d0630d:state/...` snapshots rather than from the log's tables.

**Result: PASSES on every checkable limb.** No contradiction opened.

| limb cycle 141 entered | re-derived at `0d0630d` | verdict |
|---|---|---|
| score vector: total 22, max 3, n 10, all `assessed_cycle` 140 | identical | ✅ |
| `ttp` 3, `incident-evidence` 3, other eight 2 | identical | ✅ |
| all ten `depends_on` lists | identical | ✅ |
| all ten `attempts` arrays | identical | ✅ |
| `created_cycle`: `ioc` 2, `automated-triage` 16, `extraction-vs-reasoning` 16, `attribution-expressed` 45 | identical | ✅ |
| rung 3(a) pairwise-within-tier excludes exactly four of eight | reproduced | ✅ |
| rung 3(b) window 136–140: `consistency` +1 (c136), `institutional` +1 (c139), `ioc` 0 (last 131), `automated-triage` 0 (empty) | reproduced | ✅ |
| rung 3(c): `ioc` `created_cycle` 2 beats `automated-triage` 16 | reproduced | ✅ |
| refresh arithmetic 141 % 7 = 1, so no T1 | 7×20 = 140, remainder 1 | ✅ |
| queue entry actually written: `T3`, target `ioc-extraction-reliability`, `attempt_count` 0 | identical | ✅ |

I also re-derived cycle 141's own honesty note — that under the **inclusive** 137–141
window `consistency-calibration` escapes its penalty and ties with `ioc` on score, penalty
*and* `created_cycle` 2, i.e. a deadlock. That is correct: cycle 141's list terminated
only because of the window boundary it declared. **Honesty item (B) is confirmed by an
independent third derivation.**

**Bonus: I closed the one soft spot cycle 150 declared in its own G2.** Cycle 150 said it
had not read the `broken` hit in cycle 140's `automated-triage-under-refusal` rationale
and had inferred it could not be the confound claim. I read it at `c730fdf`: the single
occurrence is inside *ONE POOLING PROHIBITION REPEATED BECAUSE IT KEEPS ALMOST BEING
BROKEN*, about the `src-0015` action-threshold construct. **Cycle 150's inference was
right, and it is now read rather than inferred.** (Note the snapshot discipline of [337]:
the current `scores.json` — cycle 146's rewrite — has *zero* occurrences of `broken` in
that rationale, so checking the wrong snapshot would have produced a spurious "the hit is
gone" result.)

**One bookkeeping error found, in a queue entry rather than in the state.** Cycle 150's
entry and log both state *eleven* open contradictions on
`institutional-incident-real-world-impact` after the `ctr-0064` closure, while telling its
successor to re-derive rather than inherit. I re-derived with [373] at HEAD: **twelve**.
The figure is **not** present anywhere in `state/issues/graph.json` (`grep -c` → 0), so
nothing in the state needed repairing and no contradiction is owed — it is a wrong number
in a log and a queue entry. Recorded as [401] and folded into work-owed (33). **The
instruction to re-derive is what caught it, for the second time in six cycles.**

## Changes made

A T3 investigates one issue and edits `state/issues/graph.json`. **I edited only that
file** (plus the mandatory bookkeeping files). No source was added, no score was moved,
`state/knowledge/` and `state/assessments/scores.json` are untouched.

All eight edits went in as one three-input `jq` splice ([237], [346]/[366]) from two
scratch JSONs written with the Write tool, and were proved append-only before `mv`:

- `cr3ext=true oq9ext=true oq10ext=true ctr69ext=true` (`startswith`, [377]/[383])
- `oq0to8same=true oq11same=true cands0to2same=true cands4to9same=true
  otherissues=true otherctr=true` (equality over every region that must not change)
- G1 hand-check: the ten evidence ids on the two new candidates
  (`src-0004, src-0012, src-0035, src-0040, src-0048, src-0049, src-0050, src-0051,
  src-0052, src-0053`) all exist in `state/knowledge/index.json` → `allknown=true`.
- Top-level keys unchanged: `_schema, contradictions, issues`.

### 1. The four admission rulings (`open_questions[9]`, posed by cycle 150)

**(i) South Africa (DCDT) — ADMITTED, stratum V.** Grounds stated for myself rather than
inherited: institution type is no bar (ENISA, an EU agency, is already in); the artefact
widening — a national AI policy gazetted for public comment — is *smaller* than the
widening this population already accepted at cycle 140 for `src-0048`, a bankruptcy
motion; and the case is forum-clean, so `ctr-0069`'s forum confound does not reach it.
Stratum V goes from four members to five. **This is the first exercise of the stratum
labelling cycle 150 built.**

**(ii) MAHA — NOT ADMITTED, and the ground cycle 150's question proposed is REJECTED as
the ground.** The question asked whether the population admits a case whose AI attribution
is *external and inferential rather than institutional*. **That test does not
discriminate: it would also expel EY Canada, KPMG and PwC**, all three of which decline to
state a cause and whose AI attribution is likewise a detection vendor's inference. The
population has never been defined as *incidents the institution attributed to AI*, and I
recorded that so no successor says it was. I decline MAHA on a narrower ground: **this
base holds no artefact of any kind linking the MAHA defect to AI.** `src-0051` is
two-render ABSENT for any AI attribution and for any named AI product; the only link is
`src-0052` relaying a Washington Post finding this base does not hold, has not fetched,
and has no method for. Since the topic is the reliability of AI-generated output, a case
with no held AI link is evidence about citation errors, not about AI. Two further reasons,
each sufficient to wait: furthest out of domain of any case here, and a **mixed forum** so
it cannot take a single stratum. **The declination carries a stated, testable lifting
condition** — a T1 fetches and enters the Washington Post reporting.

**(iii) The volunteer/decline axis — DECOMPOSED rather than extended.** Cycle 150 asked
for a fourth value (explicit refusal to confirm or deny) and possibly a fifth (hedged
volunteered cause), and suspected a five-valued axis was itself a symptom. It was. New
`candidate_resolutions[11]`, **status proposed**, splits it into **D1 — does the
institution state a cause?** (states a cause / states no cause / explicitly refuses to
confirm or deny, the third defined but *unoccupied* since its only candidate occupant is
the declined case) and **D2 — how strongly, applying only where D1 is "states a cause"**
(unhedged as recorded / hedged / control-failure attribution only). This absorbs all five
requested values with room left, and **does not disturb `ctr-0069` (7)(ii)**: control-
failure attribution stands with `src-0048` its sole member, relocated to D2. Left at
`proposed` because axis structure is a T2's.

The argument that decided it, and it is the useful part: **on a single flat axis ENISA and
DCDT fall in the same cell, so the structure itself strips the hedge** — the exact drift
this base documented at the `src-0049`/`src-0053` pair (work-owed (36)). A structure that
cannot hold a hedge will lose it.

**(iv) Spokesperson vs principal — YES in principle, deliberately NOT built.** The
distinction is real and evidenced (`src-0051` vs `src-0052`: one institution, two levels,
four weeks apart, materially different words, different forums). But its only occupant is
MAHA, which is not admitted, and cycle 150's order-of-operations rule — decide the
admission first, then the cell — is followed rather than overridden. **For the same reason
the third forum cell is not created**; I named the label a successor should use (stratum
L, legislative testimony) and recorded that it has no members and does not exist.

### 2. What the admission does to the sector-alignment observation

This was the point of admitting DCDT at all: **it is the first genuine test that
observation has ever had, rather than a restatement of it.** Within stratum V the count is
now five — EY, KPMG and PwC state no cause; ENISA and DCDT each state one. Both
cause-stating members are public-sector bodies and all three declining members are
commercial Big Four firms, so **the alignment holds on its first out-of-sample point.**

Three qualifications went in with it, all load-bearing:

- **Numeral-collision warning.** That yields a three-and-two split *within stratum V*
  whose second cause-stating member is **DCDT, not `src-0048`**. It is a different
  proposition from the restatement `ctr-0069`'s cycle-140 ruling forbids (which folds
  `src-0048` into the stratum-V tally). Any successor finding a three-and-two count on
  this issue must check which second member it names. This is the second numeral-collision
  hazard parked on this issue (`open_questions[7]` is the first).
- **The confound is not broken and is not even touched.** Commercial-exposure versus
  shared-boilerplate is separated only by a **commercial, non-Big-Four institution in
  stratum V**. DCDT is public sector, so it adds a point on the side never in dispute.
  That cell is empty. `src-0048` is commercial and non-Big-Four but stratum T; MAHA is
  public sector and declined.
- **The pass is weak, by my own arithmetic over the base's own tally and by no source.**
  With five stratum-V members of which exactly two are public-sector, a null process
  picking two cause-staters at random lands on exactly those two one time in ten. **A
  pattern that would arise by chance one time in ten is not a finding**, and I wrote into
  the graph that no cycle may score this issue as though the sector alignment were
  established.

### 3. The membership rule, written down for the first time

Five admissions had been made case by case with no written test. I recorded the rule **as
operated** (four limbs: institution published under its own name; fabricated citations
found in it; **this base holds an artefact linking the defect to AI**; the institution
made a public response that this base holds), explicitly labelled **a description of
practice and not a ruling that the practice was right**, and referred it to a T2. Limb (c)
is what MAHA fails. Work-owed (38).

### 4. `candidate_resolutions[10]` — the South Africa case record, status `supported`

Evidence `src-0049, src-0050, src-0053`. The `supported` status is **expressly scoped** to
the occurrence, date, stated ground, response shape and verbatim wording — each attested
either at two render families of `src-0049` or across carriers independent of government —
and **explicitly not** to the proposition that AI was in fact used, which is the minister's
hedged best explanation and not a finding. Every citation constraint travels with it: one
utterance and four carriers, never three independent findings; `src-0049` first-party;
`src-0050` the only page naming News24; the transcription divergence between carriers
recorded and unresolved; and the counts (`six of 67`, `86 pages`, the two suspensions, the
seven-member panel, the 10 April gazette date) named as *not claims of this base*.

### 5. `ctr-0069` — CERTIFIED AND CLOSED, `resolved_cycle` = 151

`open` fell 50 → 49. **On what authority, said rather than done silently:** paragraph (11)
of `ctr-0069` itself, written by cycle 150, expressly names *the next T2 or T3 on this
issue* as the role that should certify and close. I am a T3 on this issue and closed on
that naming. **I did not close on cycle 150's `ctr-0064` four-limbed convention, which
names a T2, and I am not widening that convention to T3s** — that stays with a human,
honesty item (E).

I certified by reading the graph, not by inheriting cycle 150's account: **(7)(i)** the
cycle-150 correction is present at the tail of `candidate_resolutions[3]`, names the
corrected sentence verbatim, fixes strata V and T, restricts the sector sentence to V,
declines the forbidden restatement, and leaves `candidate_resolutions[9]` at `proposed` —
verified directly at `[9]`. **(7)(ii)** paragraph (5) of the same append carries
control-failure attribution with `src-0048` sole member and the ABSENT findings attached.
**(7)(iii)** discharged as a search commission at cycle 148 and now carried beyond it,
since I have ruled the admission of both cases it surfaced. **(7)(iv)** is a precondition
and a flag, not a repair step: it forbids closure *until (i) is ruled*, and (i) was ruled
at 140, executed at 150 and certified here by a cycle that did not perform the execution —
which is exactly the independence [398] asks for.

Every ban the entry carries is restated as surviving the closure, and I recorded plainly
what the closure does **not** do: **the forum confound is not resolved.** No source in this
base tests forum, stratum T still has exactly one member, and any finding ranging over the
whole population remains untestable.

### 6. Open questions

`open_questions[9]` and `[10]` extended by append (nothing erased) with the rulings and
the reasons; new `open_questions[12]` added carrying the empty discriminating cell as a T1
target with the untried search axis named ([393]), plus the two open routes I declined.
`attempts` now `[12,59,72,90,104,114,128,139,151]`.

### What I did NOT do, and why

- **No fetch of the News24 investigation**, though the queue entry ranked it the
  highest-value remaining fetch on this issue. Reason: a T3 may not create a
  `src-*.md`, so the counts it would supply — the only counts absent from this entire
  base — could be recorded only as an open question and never cited. The same fetch spent
  by a T1 enters them properly under G1. Recorded as work-owed (39a), a T1 target. **Not
  dead, not attempted.**
- **No `ctr-0037` site (2) and no `ctr-0062` step (iv)**, both listed as cheap and owed.
  Both sit on *other* issues (`ttp` and `ioc`), and a T3 works one issue. Still owed,
  work-owed (19) and (20).
- **No update to the stale route ledger** at `open_questions[8]` (work-owed (26)); no
  ruling on work-owed (30); **none of the three pending standing checks adopted, reworded
  or dropped** — still reserved to a T2 (work-owed (18)); **no split** of the issue,
  though I have now met the precondition cycle 150 named for it (work-owed (37)).
- **I did not redirect myself to `automated-triage-under-refusal`** (work-owed (25),
  honesty item (A)), which the queue entry invited me to consider if the admission work
  proved cheaper than expected. It did not — reading five sources end to end, ruling four
  questions, and certifying a contradiction consumed the cycle. Forty-third consecutive
  cycle that issue goes uninvestigated.
- **I created no sixth instance of work-owed (30).** Every verbatim string I wrote into
  `graph.json` was copied from the corresponding `src-*.md` and established nothing new,
  so that count stays at five and stays honest.

## Next task rationale

`prompts/system.md` line 46, read at source: **T3 → T4**. Cycle 152 is a **T4 assess**,
`attempt_count` 0, target `ALL` (a T4 scores every issue). The refresh rule is not mine to
compute — it keys on a T5's own cycle number — but for the successor's planning the next
T5 is 153 and the next refresh falls at 154.

The entry gives the T4 the short list of what actually changed since cycle 146's full
rewrite, because everything else is unchanged:

1. **`institutional-incident-real-world-impact`** gained a supported candidate and a
   proposed one, and lost one open contradiction.
2. **`incident-evidence-vendor-concentration` is the issue to think hardest about.** Cycle
   146 recorded an expectation that a post-ruling revisit would press that score *down*
   from 3; the ruling is now executed and certified, so the expectation is due. The
   counter-consideration is decisive and I told the T4 it must appear in the rationale if
   it moves the score: **none of the three non-GPTZero discovery routes has been read at
   its own artefact.** Three routes attested by downstream carriers is not three routes
   read at source. This is also the one place in the base where merit meets the G3 ceiling
   exactly — honesty item (G), which is squarely a T4's.
3. Nothing else changed.

I also warned it off the two traps that would make its scores dishonest: **do not use the
score vector to reach an issue the selector cannot reach** (honesty item (C) — that would
be the same illegitimacy as rewiring an edge, in a different file), and **re-derive the
open-contradiction counts** rather than inheriting them, since [401] is exactly the error
that just happened.

The entry carries the ban set, all seven honesty items and all forty work-owed entries
forward whole, with the narrative compressed per [399].

## Budget

- **Fetches: 0. Searches: 0.** The whole investigation ran off the knowledge base, and the
  G2 ran off `git show` snapshots. Cycles 121–133, 135–148, 150 and 151 have now all done
  G2 at zero fetches.
- **Tool calls: ~55.** Roughly: 5 reading the 67,035-character queue entry and the prompts
  at source; 5 reading the five source files end to end; 8 on G2 re-derivation; 6 indexing
  and slicing the target issue's huge `open_questions` and `candidate_resolutions`;
  2 writing scratch JSON; 1 splice; 5 verification and append-only proofs; the remainder
  bookkeeping and cleanup.
- **Scratch files created and removed with `rm -f`:** `.instr.tmp.txt`, `c151scratch.json`,
  `c151scratch2.json`, `g2scores.json`, `g2graph.json`, `g2next.json`, `g2s140.json`,
  `oldg.json`, `nt_body.txt`, `nt_instr.json`.
- **Turn budget:** comfortable. No section was cut for turns.

## Carry-forward items — NEW AT CYCLE 151

**[400] Write a population's membership rule down *before* the next admission, not after
the sixth — and check any proposed exclusion test against the members already in.** Cycle
150 proposed excluding MAHA for having an *external and inferential* AI attribution. That
test would also have expelled EY, KPMG and PwC. The right ground was narrower and could
only be found by asking what the existing members actually have that the candidate lacks.
A test that expels sitting members is not a test, it is a redefinition — and a
redefinition should be argued for on its own terms.

**[401] A count in a queue entry is not a count in the state.** Cycle 150 wrote *eleven
open contradictions* on this issue and instructed its successor to re-derive; the
re-derivation gave **twelve** at HEAD. The figure was not in `graph.json`, so nothing
needed repairing — but the identical error inside a rationale or a candidate would have
needed a contradiction entry. Re-derive every count you report and say where you derived
it from. Second instance of work-owed (33) in six cycles; the "disbelieve this entry"
discipline has now paid off twice.

**[402] An axis that keeps needing new values is probably two axes.** The volunteer/decline
axis took a third value at cycle 150 and was asked for a fourth and a fifth at cycle 151.
Decomposing it into *does the institution state a cause* and *how strongly* absorbed all
five with room left. **The argument that decided it: on a single flat axis a hedged cause
and an unhedged one fall in the same cell, so the structure itself strips the hedge** —
the exact drift this base documented at the `src-0049`/`src-0053` pair. A structure that
cannot hold a hedge will lose it, and no internal consistency check will notice.

**A note on [399], for the human.** Compression alone will not fix queue-entry size. I
compressed every narrative section I could and this entry still came to ~67,000
characters against cycle 150's 67,035 — no saving at all, because four fifths of the entry
is the ban set, the honesty items, the work-owed ledger and the tool notes, all mandated
to be carried whole and all growing monotonically. The structural fix is to move them into
a versioned file under `state/` that each entry points at and each cycle updates in place,
leaving the queue entry to carry the phase check, the assigned work and the pointer.
**Creating a new state file is not a cycle's decision to make**, so it is recorded here and
in the cycle-152 entry as a question for a human.
