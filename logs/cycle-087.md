# Cycle 87 — T3 investigate (`consistency-calibration-as-failure-mode`)

## Task performed

**T3 (Investigate)** against `target_issue` `consistency-calibration-as-failure-mode`, per
`prompts/t3_investigate.md`, which I read at the start rather than trusting the queue entry's
account of it.

**Phase check, first thing.** `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed cycle 86 touching `graph.json`, `index.json`, `src-0002.md`,
both queue files and its log. The three confirming checks cycle 86 nominated all passed:
`.contradictions|length` = 45 with `.contradictions[-1].id` = `ctr-0045` on
`ttp-attack-mapping-reliability`, open count 34, `src-0002` `key_claims` = 8. Cycle 86
completed. I am the T3.

**Budget note on the entry itself, because it cost real turns.** Cycle 86 left an
**82,268-byte** `next_task.json`. That is over the Read tool's ~25,000-token limit, so I paged
through it with four `cut -c` calls before I could begin. Roughly a fifth of it re-spliced
material that is authoritative in `state/` (the issue's ten `open_questions`, verbatim) or in
`logs/cycle-086.md` (the carry-forward catalogue). My own handoff is **16,039 bytes** and the
rule I am proposing is carry-forward **[238]** below.

**What I did.** Executed steps **(i)**, **(ii)** and **(iv)** of `ctr-0044`'s repair path —
the work cycle 85 filed and cycle 86 recommended as the spine — and **resolved `ctr-0044` at
cycle 87**. Two fetches, zero searches, zero new sources. Step **(iii)** was not performed and
is carried forward, not silently dropped.

### `ctr-0044` step (ii) — the fetch, and it settles the limb cycle 85 left open

Cycle 85 established that `candidate_resolutions[10]`'s flat assertion *"Neither source names
ECE."* is **false for `src-0029`**, but explicitly charged itself with scope limit (a): it had
**not tested `src-0028`**, so what was shown false was the *conjunction*, not both limbs.

I put the all-occurrences question to `src-0028`'s full text through `r.jina.ai` over
`https://proceedings.mlr.press/v238/popordanoska24a/popordanoska24a.pdf`. The render
self-reported **COMPLETE**, last visible heading `F EXPERIMENTS`. It returns **three
occurrences**:

- Section 2: *"The first calibration error estimator for a multi-class model was given by
  Naeini et al. (2015) and is still the most commonly used measure to quantify calibration,
  known as expected calibration error (ECE)"*
- Appendix A: *"For p = 1, its binning based estimator is commonly referred to as expected
  calibration error (Naeini et al., 2015; Guo et al., 2017)."*
- Appendix F.4: *"The latter can be seen as the class-wise version of the commonly used ECE
  (Naeini et al., 2015; Guo et al., 2017)."*

**So the sentence fails on both limbs.** All three quotations are now in `src-0028`'s
`key_claims` and in `state/knowledge/src-0028.md`.

**One asymmetry survives and I recorded it rather than flattening it.** Asked directly for an
equation or numbered definition, the same render returned **ABSENT**: `src-0028` *names* ECE as
the *p* = 1 binned estimator of an already-defined calibration error but does **not define** it.
`src-0029` defines it three times over (binary-, classwise-, confidence-ECE). `names` is the
right verb for one source and `defines` for the other.

**A failed call, recorded so it is not repeated.** My first attempt fetched the PMLR PDF
*without* the `r.jina.ai` prefix and got unparseable binary — the same lesson cycle 85 recorded
for arXiv PDFs. Per carry-forward [202] a non-responsive render is not an ABSENT; I discarded it
and did not count it as evidence.

### `ctr-0044` step (i) — the correction, restated on ground that was tested

The sentence is marked **DO NOT CITE**. Paragraph (2)'s *labelling* of the step as an inference
was right; its stated *reason* was wrong. The true reason, and it is now supported by two ABSENT
results rather than asserted: **neither source states the ECE-versus-Brier non-interchangeability
as a conclusion in those words** — `src-0029` ABSENT on ar5iv at cycle 85, `src-0028` ABSENT on a
complete `r.jina.ai` PDF render at cycles 78 and 87. The prohibition on quoting the conclusion as
either source's words is unchanged. What is retired is only the false premise offered for it.

### `ctr-0044` step (iv) — the ruling cycle 85 asked a T3 to make

**Question.** Can the candidate's core methodological claim rest on `src-0029`'s Section 4.4
sentence — *"It is good practice to use them together with the proper scoring rules presented in
Section 3, to get a full evaluation"*, in the state since cycle 78 — instead of on the
Bregman-decomposition apparatus?

**Ruling: yes for the non-interchangeability claim, no for the mechanism, and the apparatus is
not retired.**

- **(A) The re-grounding works and is strictly shorter.** *"Them"* refers to the Section 4
  ECE-family metrics that `ctr-0044` confirmed `src-0029` actually defines. A prescription to
  use two instruments *together*, on the stated ground that either alone leaves the evaluation
  less than *full*, **entails** that they do not measure one and the same quantity — if they
  did, *together* and *full* would both be empty. That is one short entailment from one stored
  sentence, against paragraph (2)'s two premises plus a by-construction argument. **It is
  near-quotational and it is not quotational**, and I decline to call it quotational: the source
  prescribes complementary *use* and does not state non-interchangeability, which is exactly
  what the two ABSENT results record.
- **(B) Why the apparatus stays.** The Section 4.4 sentence establishes **that** the two are not
  interchangeable and says nothing about **why**. Paragraph (3)'s consequence — two systems with
  identical calibration error can carry different Brier scores, because Brier confounds
  calibration with refinement — is what makes this issue's Table 6 divergence *expected* rather
  than anomalous, and that mechanism is carried by `src-0028`'s decomposition and by nothing
  else in this base. Retiring the apparatus would retire the mechanism and leave paragraph (4)
  unsupported.
- **(C) Net effect, stated without inflation.** The claim now stands on **two routes of
  different character** — one near-quotational, one derived — rather than on the derived one
  alone. **This does not make it two-source in the sense that matters here.** Paragraph (7)'s
  scope limit is unchanged and binding: `src-0028` and `src-0029` measure no LLM and no security
  task and can never count toward CTI multi-source support on this issue.

### One thing no earlier cycle noticed, and it cost nothing

**The false sentence was refutable from inside the same candidate at the moment it was written.**
Paragraph (5), three paragraphs below paragraph (2), opens *"Both sources impeach the binned ECE
estimator."* A candidate that says in one paragraph that neither source names ECE and in another
that both impeach the binned ECE estimator is inconsistent on its face. The paragraph (5) phrase
is this base's gloss rather than a quotation, so the two are not *formally* contradictory — but
the tension sat there for six cycles at zero fetch cost, and cycles 79 through 84 all read past
it. General lesson, and it now has four instances behind it: **the cheapest check on a stated
absence is often the rest of the same paragraph.**

### Why `ctr-0044` is resolved rather than left open

The defect was a **false stated absence** inside `candidate_resolutions[10]`. That sentence is
now DO-NOT-CITE, paragraph (2) is restated on tested ground, and the untested limb has been
tested and also fails. The finding is now **over-determined**: `src-0028` alone falsifies the
conjunction, on a complete render in a family cycle 85 never used, so it no longer depends on
the one-form ar5iv result that scope limit (b) charged against it. Nothing about the defect
remains uncharacterised.

**What is *not* closed, and must not be read as closed.** Step (iii) was not performed: the
exact Section 4.2–4.4 **definition strings** of `src-0029` are **still not established and still
must not be quoted** (the ar5iv render carried visible noise — *"Roelofs Others."* for *"Roelofs
et al."*). That is a strengthening item, not a condition of the repair. Also still one-form:
`src-0029`'s ECE occurrences (ar5iv only) and `src-0028`'s ranking ABSENT.

**Ceiling note, derived at source.** The issue went from four open contradictions to three
(`ctr-0011`, `ctr-0027`, `ctr-0039` remain). `scripts/validate_state.py` lines 144–156 impose
the G3 demotion as a **ceiling** of `scale_max − g3_contradiction_demotion` = 3, computed from a
**set comprehension over `issue_id`**, not as a per-point subtraction. **The ceiling was 3
before and is 3 after. This resolution creates no headroom.** On a merit of 2 it does not bind
either way. I am a T3 and did not touch `state/assessments/scores.json`.

### `open_questions` — what genuinely remains open

I appended a `=== CYCLE 87:` update to `open_questions[6]` and left the other nine unchanged,
because nothing this cycle bears on them. Limb (b) is **re-tested and still narrowed, not
closed** — see Retrospection-adjacent honesty note below: my re-ask of `src-0028` returned
ABSENT again, but cycle 78's original ABSENT came from **the same render family** (its log
records the pull as *PMLR PDF via r.jina.ai*), so this is one form taken twice, not a two-form
confirmation. What *did* change cuts toward limb (b): `src-0028` **names** ECE and **declines to
rank it anyway**, which is stronger evidence for a structural rather than prescriptive answer
than a paper that simply never mentions the metric — which is what this base believed until
today.

## Retrospection

**G2 target: `candidate_resolutions[0]` of `attribution-confident-wrong-gap`** — the cycle-28
T3 rewrite carrying the misattribution-rate finding (GPT-4-turbo 14%, GPT-3.5-turbo 38%,
Gemini-1.5 26%, LLAMA3-70B 20%, LLAMA3-8B 64%, derived as 100 − Table 1's Plausible column).

**Why this target.** The round-2 G2 order that began at cycle 80 re-takes issues by least-recent
G2 attention: 80 took cycle 71's issue, 81 took 72's, 82 took 73's, 84 took 74's, 85 took 75's,
86 took 76's — so 87 takes **cycle 77's**, which was this issue. Within it,
`candidate_resolutions[0]` is the **only supported candidate no G2 has ever taken** (cycle 77
took `[1]`; `[2]` is merely *proposed*, and G2 takes supported conclusions).

**Result: PARTIAL FAIL. `ctr-0046` opened.**

**What passed, and it is the great majority.**

1. **The arithmetic is exact and triangulates three ways.** All five derived rates re-derive as
   100 minus the stored Plausible column (86, 62, 74, 80, 36), and are independently consistent
   with `src-0002` `key_claims[4]`'s separately derived plausible-but-not-correct differences
   (34, 18, 36, 28, 8) under *Plausible = Correct + difference*, on all five rows. No cell in
   error.
2. **The numbers are now two-render confirmed for the first time.** A pull through `r.jina.ai`
   over `https://arxiv.org/pdf/2406.07599` — **COMPLETE**, last heading `G Compute Cost` —
   reproduces the caption *"Results of different LLMs on the benchmark datasets (bold indicates
   the best performing model, lower is better for MAD)"*, the `CTI-TAA (Acc)` spanning header
   over `Correct`/`Plausible`, and **all ten CTI-TAA cells exactly**: 52/86, 44/62, 38/74,
   52/80, 28/36.
3. **The quotation layer holds and was already two-form** (cycle 74, two complete `r.jina.ai`
   PDF renders, character for character).
4. **The negative holds.** Asked directly, the complete render returned **ABSENT** to whether
   the paper prints any numeric incorrect-answer or misattribution rate for CTI-TAA — the first
   positive test this base has ever run behind a parenthetical it has carried since cycle 23.
5. **The interpretive discipline holds.** The candidate's closing paragraph — `src-0002`
   measures **wrongness, not confidence**, and supplies no support whatever for the *"confident"*
   half of this issue's title — is exactly right and is why the candidate is worth keeping.

**The failure, which is one clause.** The candidate guarantees that *"every cell they are derived
from was re-verified EXACT against Table 1 by **two independent fetches** at cycle 23"*. Cycle
23's own log records its budget verbatim as **"Web fetches: 2 (both `arxiv.org/html/2406.07599`,
different prompts and one version-pinned)"** — one host, one renderer, **one render family**,
differing only in prompt and version pinning. Cycle 28's three further pulls are the same family
again. So until today the ten CTI-TAA cells had been read **five times and never outside
`arxiv.org/html`**, while a supported candidate advertised them as independently confirmed. This
base has a working vocabulary for exactly that distinction (render family; one-form vs two-form;
carry-forward [205]), and the clause is false under it.

**Sharpest form of the point:** cycle 74 held **two COMPLETE `r.jina.ai` PDF renders of this very
paper** in hand and asked them only about the three-category definition. A second-family check on
the numbers was available at **zero marginal cost for thirteen cycles** and nobody took it.

**Charged, not credited.** (a) The defect is in a **provenance qualifier, not a number** — the
check that should have grounded the word *independent* was run this cycle and **passed**.
(b) Whether the same overstatement sits elsewhere in the graph **has not been swept**; that is
`ctr-0046` step (iii). (c) **Fifteen of the twenty-five cells** cycle 23 verified (MCQ, RCM, VSP,
ATE) are **still single-render-family** — my fetch was scoped to CTI-TAA.

**Direction, and this is what a successor is most likely to get backwards.** `ctr-0046` **does
not weaken the candidate**. Its finding — outright misattribution running from roughly one report
in seven for the best 2024-era model to nearly two in three for the weakest, over 50 real reports
scored with alias tolerance — is intact and **better grounded than it was this morning**.

**And the pattern is now four for four.** `ctr-0043` (84), `ctr-0044` (85), `ctr-0045` (86),
`ctr-0046` (87) — every G2 in the last four cycles found its defect in the **provenance layer**
of a claim whose **direction survived** the check. That recurrence is itself a finding, and it is
evaluation data for the paper: **this loop's supported candidates are reliable about the world
and unreliable about their own audit trail.**

## Changes made

**`state/issues/graph.json`** (~810KB, read and written through jq only):

1. **`ctr-0046` opened** against `attribution-confident-wrong-gap`, `opened_cycle` 87,
   `resolved_cycle` null. Five schema keys present.
2. **`ctr-0044` resolved**: `resolved_cycle` set to 87, and a `=== RESOLVED AT CYCLE 87 ===`
   paragraph **appended** to its description (the description's old text is a verified prefix of
   the new).
3. **`consistency-calibration-as-failure-mode` `candidate_resolutions[10].summary`** — a
   `=== CYCLE-87 CORRECTION ===` block appended. **Status unchanged at `supported`; evidence
   array unchanged.**
4. **`open_questions[6]`** — a `=== CYCLE 87:` update appended. The other nine untouched.
5. **`attempts`** — `87` appended (now `3,15,16,25,39,62,75,80,87`).

**`state/knowledge/index.json`**: one `key_claim` appended to **`src-0002`** (now 9) and one to
**`src-0028`** (now 4). **No source added; `.sources|length` still 31, last still `src-0031`.**

**`state/knowledge/src-0002.md`** and **`src-0028.md`**: `## Cycle 87 addendum` sections
appended via the Edit-tool idiom (carry-forward [234]). Nothing deleted or rewritten.

**Append-only proof (carry-forward [221]), twenty checks across three calls, all true.** Where I
**modified** an existing array element (`ctr-0044`) the whole-array prefix test **fails by
design**, so I proved instead that the contradictions array is identical after filtering that
element out, and that the element itself is identical after `del(.description, .resolved_cycle)`.
Other checks: all eight non-target issues byte-identical; the target issue identical after
`del(.candidate_resolutions, .open_questions, .attempts)`; its other twelve candidates identical;
`candidate_resolutions[10]` identical after `del(.summary)`; its other nine open_questions
identical; `attempts` old array equals the new array's `[0:8]` slice; both appended strings have
the old string as a `startswith` prefix; all 29 untouched sources byte-identical; both touched
sources identical after `del(.key_claims)`; both `_schema` blocks identical; source key set
`id,url,title,type,added_cycle,file,key_claims`; contradiction key set
`id,issue_id,description,opened_cycle,resolved_cycle`.

**Gate hand-check** (I cannot run `scripts/validate_state.py` — `python3` is refused in this
session). **G1:** no new source, no new URL; every new claim cites `src-0002`, `src-0028` or
`src-0029`, all of which exist in `index.json`. **G2:** performed and recorded above. **G3:**
`ctr-0046` opened; `attribution-confident-wrong-gap` scores **2**, well under the ceiling of 3,
so the lines 144–156 check does not fire. **Schema:** key sets checked as above.

**Queue:** `state/queue/next_task.json` written for a **T4 (assess)**, `target_issue` null,
`attempt_count` 0, 16,039 bytes — written to a scratch path and `mv`-ed onto the queue path
(carry-forward [235]). `state/queue/last_completed_task.txt` reads `T3 investigate`. Scratch
files `scratch-087.json` and `queue-087.json` removed.

## Next task rationale

**T3 → T4** per the state machine. The refresh rule (`collect_refresh_every: 7`) substitutes a
task *type* at a T5 only, so it does not touch cycle 88.

The T4 entry names the three decisions its cycle actually turns on rather than re-splicing state:
**(1)** carry-forward **[97]** — whether a non-commensurability or *negative* finding may itself
count as a resolution — is now in its **seventh** consecutive cycle of making this issue's best
work unscoreable, and cycle 87's entire output is of exactly that kind; the T4 must record where
it bites and state what the score would be under each reading, not resolve the policy. **(2)**
`ctr-0046` is **not** a reason to lower `attribution-confident-wrong-gap`. **(3)** `ctr-0044`'s
resolution **creates no headroom** — the ceiling was 3 before and is 3 after, because the set
comprehension counts *issues with any open entry*, not entries. Cycle 80 caught itself about to
make exactly that error.

I also flagged, for the T4 to price without acting on: `automated-triage-under-refusal` is still
**the only issue never investigated in eighty-seven cycles**, and the four-for-four
provenance-layer G2 pattern is a scoring question no cycle has ruled on.

## Budget

- **Web fetches: 2 usable, 3 calls.** (1) `r.jina.ai` over `arxiv.org/pdf/2406.07599` — G2,
  Table 1 CTI-TAA. (2) `r.jina.ai` over the PMLR PDF for `src-0028` — `ctr-0044` step (ii).
  (3) the bare PMLR PDF, **discarded** as unparseable binary, not counted as evidence.
- **Web searches: 0.** Everything else came from the existing knowledge base, as
  `prompts/t3_investigate.md` requires.
- **New sources: 0.** Both fetches were re-verifications of existing sources.
- **Turns: ~50 of 75.** Four of them spent paging through cycle 86's 82KB queue entry with
  `cut -c` before work could begin.
- **Cheapest thing that produced the most:** the `src-0028` ECE question. One fetch closed the
  exact scope limit cycle 85 had charged against itself, and turned a one-limb finding into an
  over-determined one — which is what made resolving `ctr-0044` defensible.
