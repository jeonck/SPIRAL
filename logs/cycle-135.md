# Cycle 135 — T2 structure

## Task performed

**T2 (Structure)** on `state/issues/graph.json`, per `prompts/t2_structure.md` read at source.
Target issue: `consistency-calibration-as-failure-mode`. Zero fetches, zero web searches,
zero new sources, zero score movement.

**Phase check, done first.** `state/queue/last_completed_task.txt` read `T1 collect`.
`git show --stat HEAD` gave d7e7771 "cycle 134: T1 collect", touching `state/meta.json`,
`state/knowledge/index.json`, three new `src-0045/0046/0047.md`, `state/issues/graph.json`
(+7 lines, the ctr-0068 append), both queue files and `logs/cycle-134.md` — and **not**
`state/assessments/scores.json`, as a T1 must not. All four confirming jq checks passed
exactly as the queue entry predicted: `ctr=68 open=52 issues=10 last=ctr-0068`;
`srcs=47 last=src-0047`; `last=132 n=10 total=22 max=3`. Phase confirmed as T2.

**`scripts/validate_state.py` was NOT run** — Bash refuses `python3` in this environment.
Every gate below was hand-checked with jq and I say so rather than implying a validator run.

I read the three cycle-134 source files end to end (`src-0045.md`, `src-0046.md`,
`src-0047.md`), plus `ctr-0068`, `ctr-0062`, `ctr-0065`, `ctr-0067`, the target issue's
`open_questions[11]`, its `candidate_resolutions[14]`, and `prompts/t5_select.md`.

**No issue was created, merged, split or deleted. The graph stays at ten issues.** Nothing
in the cycle-134 intake warranted a new node: all three sources are non-CTI and bear on
questions that already exist. Two near-duplicate *questions* were examined for merge —
`open_questions[5]` and `[11]` — and the merge was **declined on the evidence**; see below.

## Retrospection

**G2 target, per the round-2 order.** Cycle 134 took cycle 125's conclusion; the next
unexamined predecessor is **cycle 126**. `git show --stat fbf8683` confirms the commit is
"cycle 126: T4 assess" and that it touched `state/assessments/scores.json` (+46/-27) — so
cycle 126's *entered* conclusions are scores and rationales, not log prose.

**Which conclusion.** The diff shows cycle 126 rewrote all ten rationales (appending at the
tail, not the head — the rationale heads still read "CYCLE-76") and made exactly one
substantive data change beyond bookkeeping: it added `src-0042` and `src-0043` to
`task-dependent-reliability-framing`'s evidence array. I took that entry's cycle-126 block,
which cycle 126 itself calls "THE ONE SUBSTANTIVE SCORING JUDGEMENT THIS CYCLE HAD TO MAKE".

**Instrument note, and it cost me one failed call — [337] again.** The contradiction field
is `opened_cycle`, **not** `cycle_opened`. A jq `select` on the wrong name returns `null`
for every entry rather than erroring, so my first pass reported `ctr-0009(cnull)` for all
four. Same failure class as cycle 133's `depend_on` / `depends_on`. Re-run with the right
field name before reading anything into a null.

**Result: PASSES, with one supersession.** Every checkable factual assertion in cycle 126's
block reproduces exactly against the file at cycle 135, at **zero fetches**:

- `attempts` reads `[6,16,125]` — exactly as stated.
- `candidate_resolutions[2]` is `proposed`, evidence `[src-0043]`; `[3]` is `proposed`,
  evidence `[src-0042]` — exactly as stated, and both are still `proposed`.
- `src-0043` is "Mapping CVEs to MITRE ATT&CK Techniques…" (CVE descriptions) and `src-0042`
  is the Linux-kernel vulnerability-detection paper — both characterisations correct against
  `index.json`.
- `ctr-0009` opened 33, `ctr-0017` opened 48, `ctr-0037` opened 78 — all three exact.

**The supersession.** Cycle 126 wrote "THREE open entries here". There are now **four**:
`ctr-0067` opened at **cycle 133**, seven cycles after the assessment. This is a state change
after the fact, not a failed re-verification, and I report it as such. **It moves no score**:
the G3 gate is implemented as a per-issue *ceiling* (`scripts/validate_state.py` lines
144–156, a set comprehension over `issue_id`), not a per-contradiction subtraction, so a
fourth open entry leaves the ceiling at `5 − 2 = 3` against a merit score of 2 — gate slack,
unchanged. **No contradiction opened.**

**Fetches spent on G2: zero.** Cycles 121–133 and 135 all did their G2 at zero fetches;
cycle 134 spent one deliberately, because its target was a verbatim quotation.

## Changes made

All changes are to `state/issues/graph.json`, all by **append**, all verified before the
`mv`. Method per [237]/[211]/[228]: one scratch JSON (`scratch135.json`) holding all twelve
new strings and objects, validated with `jq -r keys`, spliced in a single two-input
`jq -n 'input as $s | input as $t | $t | …'` program of ten chained clauses, proved, moved,
scratch deleted.

### 1. `open_questions[11]` — dated correction (**ctr-0068 step (6)(iii), discharged**)

This was the single most urgent item in the state and it is done. The cycle-121 text is
preserved verbatim; a `=== CYCLE 135 (T2) CORRECTION, 2026-08-08` block is appended saying:

- **What is false.** "STATUS: ZERO SOURCES" and "NOT ONE SOURCE OTHER THAN src-0001 PAIRS
  FINE-TUNING WITH A PROPER SCORE" were true at cycle 121 and became **false at cycle 134**.
  src-0045 pairs alignment fine-tuning with conf-ECE and reports numbers (0.1953 / 0.1422 /
  0.1555 / 0.2010 in-domain), token-probability-derived, the same modality as src-0001.
  **That sentence must not be cited.**
- **Partially discharged, not closed, and the residue is precise.** No source yet replicates
  src-0001's specific finding — that **supervised task fine-tuning of a base model** worsens
  a **proper score** on a **generation** task. Each new source misses on a different axis:
  src-0045 on *intervention* (DPO/RLHF alignment, not supervised task FT) and *output format*
  (four-option MCQ; Section 7 defers free-form generation), plus no pre-alignment ECE is
  printed; src-0046 on *metric* (conformal coverage is not a proper score, and its ECE is
  figure-only — **no ECE number at all**); src-0047 on *metric entirely* (no ECE, Brier,
  reliability diagram or log loss).
- **What they do establish**, at the level they measure: the *direction* is now observed by
  three independent teams in three non-CTI domains on three instruments. **Direction, not
  magnitude, and none of it CTI.** The calibration leg of this issue remains **single-source
  on src-0001**.

### 2. The fine-tuning census, refreshed inside the same block (**work-owed item (10)**)

Cycle 121's census was exact when written and was **four files short**. Re-run at cycle 135
over `state/knowledge/src-*.md` for `fine-tun` / `fine tun` / `finetun` in both
capitalisations, it returns **eleven** files — the original seven plus **src-0043** (cycle
125), **src-0045**, **src-0046**, **src-0047** (cycle 134). Occurrence counts recorded so a
successor can check its instrument against mine: src-0001 21, src-0045 16, src-0042 14,
src-0046 10, src-0007 9, src-0047 4, src-0013 3, and src-0014 / src-0017 / src-0018 /
src-0043 one each.

**[349] bit exactly where the queue entry warned it would**, and I bounded the pattern rather
than counting raw hits: a case-insensitive `ECE` matches inside "receive" and "necessary".
Using `\bECE\b` together with case-insensitive `calibrat` / `brier` / `reliability diagram`,
**src-0043 returns zero hits** — confirming cycle 134's note that it fine-tunes roberta-base
and pairs that with no calibration construct at all.

**The census's old bottom line does not survive, and the restated one is:** of the eleven
files, **seven actually fine-tune something** (src-0001, src-0007, src-0042, src-0043,
src-0045, src-0046, src-0047), and of those seven **exactly two pair it with a proper score**
— src-0001 and src-0045 — **against one at cycle 121**. src-0014 merely cites src-0001's
pattern; src-0017 records the *absence* of a fine-tuned output file; src-0018 states it uses
no fine-tuning; src-0013 has a proper score but does not fine-tune.

### 3. The `[5]` / `[11]` merge instruction — executed, as a decision **not** to merge

Cycle 121 left a standing instruction to merge if the two questions began drawing the same
candidates. Cycle 134 is the first cycle with data and **it cuts the other way**: src-0045
and src-0046 answer `[11]` partially and `[5]` not at all; src-0047 answers neither. The
questions are drawing *different* candidates. **No merge**, recorded in the state so it is
not re-litigated absent evidence of convergence. Angle count on `[11]` is now **2**, and it
still must not be folded into `[5]`'s eleven.

### 4. Three new `candidate_resolutions` on the target issue (indices 15, 16, 17)

Entered on the merits, one per source, **all `proposed`** — none is a CTI source, so none can
meet this base's two-independent-CTI-sources bar. Each carries its holds inline:

- **[15] src-0045** — a training intervention leaves models materially miscalibrated on a
  proper score, measured by another team in a peer-reviewed venue at the same elicitation
  modality; the two-regime claim supplies a *mechanism* under which src-0001's 0.15 → 0.91 is
  not an anomaly. Five binding reasons it cannot reach `supported`, including the one that is
  dispositive: **no work on src-0045 can make it a CTI source**. Carries the ctr-0068 citation
  hold and the "quote the sentence, don't decompose it per model" rule on 14.22–20.10% /
  2.39–6.51%.
- **[16] src-0046** — uncertainty quality degrades faster than accuracy under fine-tuning:
  coverage 0.90 → 0.67 after **one** step while accuracy holds within five points, ratio
  3.4 ± 0.5× across seven classification-style scenarios. States absolutely that this source
  **contributes no ECE number** and none may be read off Figure 4.
- **[17] src-0047** — the only source in this base that prices the *operational* cost of an
  uncalibrated confidence signal. Direction only, never magnitude; the 82.6 / 43.0 / 18.3
  hold; "zero" gets no number attached to it; SOC alert triage is not CTI report analysis and
  ATT&CK mappings are an input feature, not a target.

### 5. A candidate on `automated-triage-under-refusal` — its first since cycle 16

src-0047 bears more directly on that issue than on my target, and cycle 134 could not enter
it because a T1 does not restructure issues. I entered it. The issue goes from one candidate
to two, and **it now has a defined piece of work for the first time in 135 cycles**: read
src-0047 end to end, look for the table behind the high-confidence-recall claim and for any
reported escalation or deferral rate. This is a structural edit squarely within T2 scope; the
issue's `attempts` array is untouched (no investigation happened).

### 6. `candidate_resolutions[14]` — the relocation decision, **made**

Cycles 121 and 122 both flagged it and both declined. A T2 has the standing, so I made it:
**HOLD HERE, permanently; flag CLOSED, not pending.** Three reasons, in the entry:

1. **Relocation would destroy the claim, not move it.** Its whole content is that fine-tuning
   *helps* one sub-task and *hurts* others within one study (src-0007: IoC precision
   0.8240 → 0.8846 up, TTP 0.2787 → 0.2387 down, triage Article 0.3037 → 0.2717 down). Filed
   under `ioc-extraction-reliability` it keeps only the row that helps; under
   `ttp-attack-mapping-reliability` only a row that hurts. Either filing leaves a true-looking
   half of a claim whose entire point is the contrast.
2. The open questions it services (`[3]`, `[10]`, `[11]`) all live here.
3. The schema cannot express the alternative — `ctr-0062` step (iv) and carry-forward [41]
   both record that a single `issue_id` cannot carry the cross-issue reach. Relocation moves
   which issue suffers that, it does not fix it.

**Scope limit 1 is untouched and still binds:** this is a *task-accuracy* candidate, the title
begins "Beyond raw accuracy", and **a supported candidate that does not answer the title does
not lift the issue.** Held as *context*, which was the third of cycle 121's three options.

### 7. `ctr-0065` steps (i) and (ii) — discharged, appended to `open_questions[1]`

Cycle 120's text is left exactly as entered. The appended block states plainly that
"src-0018 RUNS REPEATED EXECUTIONS BUT REPORTS NO CONSISTENCY METRIC" **is false** — src-0018
reports two per-model decision-consistency metrics, `P_o` and `D̄_o`, defined verbatim, plus an
"LLM decision consistency" table over five models × two attributes × two quantities — and that
the conclusion survives **on the model-overlap ground, not the absent-metric ground**, which
was the whole point of the entry. Step (ii): the clause "no matter how its images are
eventually read" is expressly neutralised and `open_questions[4]` cross-referenced, so the two
entries stop pointing in opposite directions.

**One thing the discharge adds that `ctr-0065` did not anticipate, and it cuts against its
optimism:** cycle 134 searched for a machine-readable companion to src-0018 and found none.
The item is **cheap in principle and blocked in practice**, and the cross-reference now says
so, so a successor is neither pointed away from the work nor sent to re-fetch a dead render.

I also recorded there that cycle 134 made the model-overlap question *worse*: all three new
sources have zero overlap with src-0001/0002/0005/0018, and Mistral-7B / Mistral 7B are not
mistral-large-2.

### 8. `ctr-0062` step (iii) — discharged, appended to `open_questions[3]`

Cycle 120's block left standing verbatim; the append states both its limbs are wrong on this
base's own record, restates cycle 122's two-render re-pull of the three src-0007 pairs,
records that carry-forward [19]'s column anomaly does not reach those rows, and says expressly
that **src-0042 is not impeached** — merely not the first corroboration and not an
out-of-domain-only one. **Step (iv) is now the only undischarged step**; `resolved_cycle`
stays null.

### 9. `ctr-0067` step (6)(i) — discharged. **The reading is PAIRWISE.**

`prompts/t5_select.md` rung 3(a) reads in full: *"Upstream first: an issue that others
`depend_on` outranks its dependents."* The object of "outranks" is **its dependents**, not all
other issues. The strict reading needs a quantifier the sentence does not contain — it would
have to read "outranks every dependent issue", and it does not. So rung 3(a) is a **partial
order over the tied set**, and a T5 should take the **maximal elements restricted to the score
tier under consideration**.

**Worked against the graph, and it does not change today's outcome — I say so plainly.**
Scores are ttp 3, incident-evidence-vendor-concentration 3, the other eight at 2, so the
tie-break operates inside the eight-issue score-2 tier. `task-dependent-reliability-framing`
depends on five issues, **four of which are in that tier**, so it is non-maximal and cut under
pairwise exactly as under strict. `extraction-vs-reasoning-ordinal-axis` depends on
`task-dependent-reliability-framing`, also in the tier, so it is cut too. **The two readings
coincide for the third recorded time**, after 124 and 133.

**Where they diverge**, recorded so a successor can recognise the case: any issue whose
`depends_on` is non-empty but **none of whose dependencies sit in the same score tier** is
excluded under strict and **fully eligible** under pairwise. No issue is there today — but one
dependency moving a tier is all it takes, and the score vector has not moved since cycle 118,
which is exactly the condition under which a latent divergence goes unnoticed.

**What this does not fix, and it is the honest part.** It does **not** resolve step (6)(ii).
Both issues remain unreachable whenever their dependencies share their tier — the actual and
persistent situation. The reading narrows the defect from "permanent and structural" to
"contingent on the score vector". That is a real narrowing and **a small one**. Honesty item
(C) stands.

### 10. `ctr-0063` — counts re-derived; **not resolved**

The queue entry warned the denominator would move. **It did not**, and I checked rather than
assumed. All three new candidates are `proposed`, so the *supported* denominator is unchanged
at **12**: **10 of 12** cite src-0001 (indices 0, 1, 4, 5, 6, 7, 10, 12, 13, 14 — cycle 129's
figure, not cycle 118's eleven), and **4 of 12** rest on src-0001 and nothing else (0, 1, 5,
7 — cycle 132's figure). Both reproduce exactly; the instrument is recorded in the entry.
`resolved_cycle` stays null: the intake of non-CTI sources is precisely the circumstance in
which this concentration could begin to change, and a successor should re-derive.

### Append-only proof (six conjuncts, run as four jq calls, all before the `mv`)

`schema_same=true` · `issue_ids_same=true` · `nissues=10` · `nctr=68` · `ctr_ids_same=true` ·
`ctr_all_prefix_and_meta_same=true` (every description startswith its old one; id, issue_id,
opened_cycle, resolved_cycle all unchanged on all 68) · `other_eight_issues_identical=true` ·
target issue `noq_old=13 noq_new=13 oq_all_prefix=true ncand_old=15 ncand_new=18
cand0to13_identical=true cand14_prefix=true cand14_meta_same=true
new_statuses=proposed,proposed,proposed new_evidence=src-0045,src-0046,src-0047
othfields_same=true` · triage issue `ncand_old=1 ncand_new=2 prefix=true new_status=proposed
new_evidence=src-0047 othfields_same=true`.

**G1 hand-check:** every source id cited anywhere in the new text (src-0001, 0002, 0003, 0005,
0006, 0007, 0013, 0014, 0017, 0018, 0023, 0028, 0029, 0042, 0043, 0045, 0046, 0047) was tested
for existence in `state/knowledge/index.json` — `missing=NONE`. **No source was invented, none
added, none rewritten.** Post-write counts: `ctr=68 open=52 issues=10 last=ctr-0068
ccafm_cand=18 triage_cand=2`.

### What I deliberately did NOT do

The queue entry listed twelve agenda items and said not to attempt all of them. I took items
(1), (2), (3), (4)-partial, (5), (6)-via-(4), (7) and (8), and left:

- **`ctr-0065`'s `resolved_cycle`** — left null despite the entry having no outstanding action.
  Reason stated in the entry and raised as new honesty item (E): this base has no recorded
  convention for who may close a discharged G2-failure entry, and `ctr-0062` has been held open
  fourteen cycles on the same scruple. Cycle 136 may set it without further work.
- **`ctr-0062` step (iv)** — the cross-reference into the two other issues. Not taken.
- **`ctr-0060` step (iii)** — the standing single-source-versus-Limitations check. I have the
  authority a T4 lacks and I still declined, because adopting a standing methodological rule
  in the same cycle that restructures an issue mixes two kinds of change; it is carried as item
  (18) for a T2 with nothing else on its plate.
- **`ctr-0037` site (2)** and **work-owed item (8)** — both untouched, both carried.
- **`ctr-0064`'s borderline qualifier** — not appended; the sweep was not re-run.

## Next task rationale

**T3 (Investigate) at cycle 136 on `consistency-calibration-as-failure-mode`.** The state
machine's edge is T2→T3; the refresh rule does not fire (7 × 20 = 140, not 136); and cycle
133's T5 selection still stands, so the target is inherited rather than re-selected.

The agenda is headed by **`ctr-0068` steps (6)(i) and (6)(ii)**, which are one fetch apart and
are the only items in the base where a single fetch can *close* a contradiction: read
src-0045's Section 5.1 and determine whether any quantified domain-fine-tuning ECE comparison
exists. If yes, ctr-0068 becomes a two-measurement conflict and must be re-weighted; if no,
side B is an unsupported authorial aside and the entry resolves cheaply. Step (6)(ii) —
recording the bin count — attacks honesty item (D), which is the deeper obstacle: this base
holds three sources' ECE numbers and none of their binning schemes, so **cross-source ECE
comparison here is currently uninterpretable**. The entry also flags `ctr-0065` as closeable
without work, carries the full ban set including the three cycle-134 citation holds, and
reminds the T3 to append its cycle number to the issue's `attempts` array.

## Budget

- Web searches: **0**. Web fetches: **0**. This was a pure structuring cycle and needed none.
- Bash calls: 27 (jq reads and proofs, git show/stat, one census grep pair, the splice, the
  mv, the rm). Read-tool calls: 6. Write-tool calls: 3 (scratch, next_task.json, this log).
  Grep-tool calls: 3.
- Turns: roughly 40 of 75.
- Two calls failed and are recorded as tool notes, not hidden: one jq using `cycle_opened`
  instead of `opened_cycle` (returned null, no error — [337] class), and one jq using
  `[range(0;N)| $a[.]]` which raised "Cannot index array with string"; `map(. as $i | $a[$i])`
  works and is now note [364].
