# Cycle 108 — T3 (investigate) on `ioc-extraction-reliability`

## Phase check

Run before anything else, per the queue entry, and disbelieving the commit message.

| check | expected | actual | verdict |
|---|---|---|---|
| `last_completed_task.txt` | `T5 select` | `T5 select` | ✅ |
| `git show --stat HEAD` | queue files + `meta.json` + log only | `logs/cycle-107-transcript.txt`, `logs/cycle-107.md`, `state/meta.json`, `state/queue/last_completed_task.txt`, `state/queue/next_task.json` | ✅ **a T5 wrote no state** |
| `scores.json` | `last=105 total=22 n=10` | `last=105 total=22 n=10` | ✅ |
| `graph.json` | `ctr=56 open=43 last=ctr-0056 issues=10` | identical | ✅ |
| `index.json` | `srcs=40 last=src-0040` | identical | ✅ |

All five agree. I am the T3 on `ioc-extraction-reliability`, its **eighth** investigation
(`attempts` were `[9,21,35,52,65,84,93]`) and its first since cycle 93. `prompts/t3_investigate.md`
was read at source and matches the queue entry's account of it.

---

## Retrospection

**G2 target, per the round-2 order: the conclusion CYCLE 98 entered.** Verified by reading
`logs/cycle-098.md` rather than trusting the handoff line: cycle 98 was a **T5** that fired the
refresh rule and wrote the T1 for cycle 99, and its `## Changes made` records **no state edits
outside the queue and its log** and **nothing filed under G3**. So its entered conclusions are
exactly two: **that the refresh rule fired at 98**, and **its selection of
`institutional-incident-real-world-impact`**, supported by a full nine-row ranking table.

**Both re-verify. VERDICT: PASS, at ZERO FETCHES.**

This is a state-internal conclusion, so I re-derived it against the state cycle 98 actually read —
commit `155abbe` (cycle 97) — not against today's state, which has since gained a tenth issue.

**Inputs, pulled from `git show 155abbe:` and compared row by row with cycle 98's table.** Every
`attempts` array, every `depends_on` edge, every `created_cycle` and all nine scores match cycle
98's table exactly: `ioc` `[9,21,35,52,65,84,93]`, `consistency-calibration`
`[3,15,16,25,39,62,75,80,87,96]`, `institutional-incident` `[12,59,72,90]`, `automated-triage` `[]`,
`created_cycle` 2 for the six oldest and 16/16/45 for the three younger, scores `ttp` = 3 and the
other eight = 2 at `last_assessed_cycle` 97. No transcription error.

**Policy re-derived from `prompts/t5_select.md` and `config.yml` at source, not from the log.**

- **Refresh rule.** `collect_refresh_every: 7`; 98 = 7 × 14, so `98 % 7 == 0`. The rule fires and a
  T1 was the correct write. ✅
- **Step 2.** `ttp` at 3 is eliminated by base priority; the eight 2s are the field. ✅
- **3(a).** Pairwise-unbeaten at score 2: `consistency-calibration`, `ioc`, `institutional-incident`,
  `automated-triage`. ✅
- **3(b).** `tie_break_recent_attempt_penalty: 1`; window 93–97 gives `institutional-incident` **0**
  (its 90 attempt has aged out), `consistency-calibration` **+1** (96) and `ioc` **+1** (93). ✅
- **3(c).** Survivors `institutional-incident` (`created_cycle` 2) and `automated-triage` (16);
  older-first selects **`institutional-incident`**. ✅

Cycle 98's claim (2) — that the selection flipped from cycle 95's `consistency-calibration` purely
because the 3(b) window moved — also checks out arithmetically: at cycle 95 the window 90–94 charged
`institutional-incident` +1 for its cycle-90 attempt and `consistency-calibration` 0; at 98 the
penalties swap. No score, source, contradiction or edge changed between the two selections.

**ONE DISCLOSED DEPENDENCY, RECORDED RATHER THAN HIDDEN — AND IT IS LOAD-BEARING.** The result turns
on reading tie-break 3(a) **pairwise** rather than as global in-degree. Under a global reading,
`consistency-calibration` (depended on by three issues) would outrank the two vacuously-unbeaten
issues *at 3(a)*, which is applied before the penalty — so `consistency-calibration` would have won
despite its +1, and the selection would have been a different issue. **Cycle 98 disclosed this
itself**, named carry-forward [246] as the settled reading, and noted that changing it re-opens
[233]'s alternating-reading problem. A conclusion that states the convention it depends on and
applies it consistently is not defective; I record the sensitivity because a future cycle re-reading
this table should know the answer is convention-dependent, not forced.

**NO CONTRADICTION OPENED.** No two supported claims are in conflict: cycle 98 entered no source
claim, and the mechanical derivation reproduces exactly. This is the third consecutive G2 (105, 107,
108) to reach a state-internal target at zero fetches — see [296], which this cycle confirms again.

---

## Task performed

T3 on `ioc-extraction-reliability` per `prompts/t3_investigate.md`, read at source.

**WHAT THIS CYCLE IS AND IS NOT.** It added **no source, no measurement and no promotion**. It did
**internal-consistency repair on four of this issue's six open contradictions**, every one of them
named by an open entry as undone, entirely by **appending** — nothing was deleted or rewritten
anywhere. The substantive question of the issue is exactly where cycle 93 left it, and
`open_questions[12]` says so in those words so that no later T4 mistakes bookkeeping for evidence.

### JOB B — `ctr-0038` steps (ii) and (iii). DISCHARGED. Zero fetches.

Open since cycle 79; cycle 84 did steps (i) and (iv) and **reported (ii) and (iii) undone rather
than dropping them**. They had waited 24 cycles.

**Step (ii) — `ctr-0001`'s opening paragraph.** The clause, verbatim: *"i.e. an unscaffolded LLM
lands at or below the level src-0003 assigns to the LEGACY tools it set out to beat."* It asserts an
ordering between `src-0007`'s **precisions** and an `src-0003` **baseline F1** — the identical defect
cycle 84 marked in `candidate_resolutions[1]`. Marked **DO NOT CITE** by an appended `CYCLE-108
UPDATE`, with cycle 84's entailed interval beside it: the 86% is an F1, `P >= F1/(2-F1)` puts
VirusTotal@1's precision in **[0.7544, 1.0]**, and both vanilla precisions (0.8240, 0.8503) fall
**inside** it, so the comparison is undetermined.

**Step (iii) — `ctr-0010`'s closing sentence.** *"The METRIC confound of ctr-0001 is thus eliminated
DEDUCTIVELY…"* **It stands and remains citable.** The annotation is a **scope limit**, not an
impeachment: LANCE's floor of **0.9531** sits *above* all four of `src-0007`'s precisions, so that
ordering **is** entailed.

**THE ONE GENERALISABLE RESULT, which neither `ctr-0038` nor cycle 84 stated and which is the whole
content of the repair:** a precision floor derived from another system's F1 can only ever establish
that a compared value lies **BELOW** the target — never at or above it — because the target's
ceiling is the uninformative 1.0. The two comparisons are **the same identity applied in opposite
directions**: 0.9531 is a floor *above* the compared values (ordering entailed), 0.7544 is a floor
*below* them (undetermined). This also explains cycle 84's asymmetry that only **0.6944** is
determinate. And it shows the plural *"legacy tools"* makes the clause **weaker**: a lower baseline
F1 gives a **lower** floor and therefore **less** determinacy.

### JOB C — `ctr-0010` step (ii). DISCHARGED. Zero fetches.

*"ctr-0001's cycle-21 update carries the same restatement and should be annotated by whichever cycle
next touches it."* Filed at cycle 35; **73 cycles unaddressed**. The cycle-21 update eliminates
confound (2) METRIC partly on *"cycle 18's arithmetic that reconciliation by metric artefact alone
would demand IoC recall of 0.09-0.15"*. Per `ctr-0010`, that is a **within-`src-0007`** IoC-versus-TTP
crossover — `src-0003`'s 97.6% is **not an input to it anywhere**. Annotated in place by append; the
figure stays exact and citable **in its true scope**. **The conclusion is unaffected**: METRIC stays
eliminated on the cheaper entailment and on `src-0007`'s own Section 4.3 *"relatively robust"*
sentence. **`ctr-0010` step (iii) forbids re-deriving the arithmetic, and I did not.**

### JOB D — `ctr-0028` step (iii). DISCHARGED AT SOURCE. One fetch pair.

Verify the range `0.1414-0.2270`; the entry could only narrow 0.1414 to *"one of the two fine-tuned
columns"*. **`state/knowledge/src-0007.md` already stored the full row, at two places in the file** —
so the first half cost nothing. I then re-fetched in **two render families**
(`arxiv.org/html/2603.09452v1`, `ar5iv.labs.arxiv.org/html/2603.09452`) as an **open enumeration with
no value supplied in the query**. Both returned all four headers and all four values **unprompted**
and agree exactly with each other and with the stored extract:

| | GPT-4o | o3-mini | GPT-4o (FT) | GPT-4o-mini (FT) |
|---|---|---|---|---|
| **TTP Identification / Recall** | 0.2270 | 0.1759 | 0.1846 | **0.1414** |

So **0.1414 is the fine-tuned GPT-4o-mini** and 0.2270 is vanilla GPT-4o. **The limit, stated
plainly: two renders of one arXiv record confirm TRANSCRIPTION, not the paper's correctness.**
**What it changes:** the range **spans all four models**, mixing vanilla and fine-tuned columns, so
any text pairing it with the word *"vanilla"* reproduces the umbrella defect `ctr-0001`'s cycle-21
update already records for the IoC precision row.

### An unplanned finding: `ctr-0028` step (ii) was already done at cycle 66 and the entry never knew

Step (ii) asks for *"one Grep for the exact string 'a substantial challenge' across state/"*. I ran
it — and the hits led me to **cycle 66's own discharge note, sitting in `scores.json`**. Step (ii)
has been listed as pending in the entry (and in every queue entry quoting it, including mine) **for
42 cycles while it was done**. My count reproduces cycle 66's finding and extends it, counted as
**occurrences, not matching lines** (the [ctr-0049] all-occurrences discipline — `Grep`'s `count`
mode counts lines and would have under-reported `candidate_resolutions[3]` 3-for-1):

| location | occurrences | status |
|---|---|---|
| `graph.json` `ioc` `candidate_resolutions[3]` | 3 | already marked, cycle 65 |
| `graph.json` `ctr-0010` description | 1 | **live unmarked USE → MARKED BY CYCLE 108** |
| `graph.json` `ctr-0028` description | 3 | mentions, by construction |
| `scores.json` `ioc` rationale | 2 | cycle 66's note + impeached text, marked in place |
| `scores.json` `ttp` rationale | 1 | cycle 66's cross-issue note — a **mention**, not a use |

**After this cycle every USE of the misquotation in the state is marked.** That the string reaches a
**second issue's** rationale is one more instance of what step (iv) records: the single `issue_id`
field cannot express that `src-0007`'s TTP figures are cited by `ttp-attack-mapping-reliability` too.

### What I declined, and what I left undone — both reported, neither dropped

- **`open_questions[4]`, the adversarial threat model: DECLINED EXPLICITLY, in one sentence, as
  instructed.** No source in this base measures extraction under defanging/encoding/homoglyphs/
  screenshots, so no T3 can discharge it; it is a **T1 collection target** and cycles 21, 52 and 93
  declined it on the identical ground.
- **JOB A — `ctr-0020`'s closure condition (the "annotation time" wording): NOT ATTEMPTED.** The
  budget went to the four repairs above. I record its append-only-safe repair shape in
  `open_questions[12]` so the next cycle does not have to re-derive it: an **appended** correction
  quoting the defective wording under a DO-NOT-CITE marker beside the measurement-bearing sentence,
  in `src-0003.md`, `index.json` and `candidate_resolutions[0]` — **never** a rewrite or deletion of
  either append-only file.
- **`ctr-0010` step (i): NOT DOABLE BY ME.** It asks a **T4** to restate the `ioc` rationale in
  `scores.json`. A T3 has no standing to rewrite a score rationale, and I did not touch that file.
  **It is the single most actionable item in the next cycle's queue entry.**
- **Two closure judgements left open on purpose.** `ctr-0038` now has all four steps discharged or
  rejected-at-source, and `ctr-0028` has (i)–(iii) done with (iv) arguably a standing note. A T3
  resolving a contradiction against a still-`supported` candidate is exactly the quiet side-picking
  G3 forbids, so I appended the reasoning and left both **open** for a T4 or T5 — with the ceiling
  arithmetic spelled out, because closing them **cannot** move a score: `ioc` still carries
  `ctr-0001`, `ctr-0004` and `ctr-0020`.

### G3

**No contradiction opened.** Nothing this cycle surfaced puts two *supported claims* in conflict:
every defect it touched was already covered by an entry that was already open, and the one new
finding (cycle 66's silent discharge) is a bookkeeping gap, not a claim conflict.

---

## Changes made

**`state/issues/graph.json` — the only state file touched.** Five appends, one array element, one
new `open_questions` entry. Nothing deleted, nothing rewritten.

1. `ctr-0001.description` **+=** a `CYCLE-108 UPDATE` block (items 1 and 2 above).
2. `ctr-0010.description` **+=** a `CYCLE-108 UPDATE` block (scope limit + the misquotation marking).
3. `ctr-0028.description` **+=** step (iii) discharge, the step (ii) history, and the occurrence table.
4. `ctr-0038.description` **+=** steps (ii)/(iii) discharge note and the floor-direction generalisation.
5. `ioc-extraction-reliability.attempts` **+=** `[108]` → `[9,21,35,52,65,84,93,108]`.
6. `ioc-extraction-reliability.open_questions` **+=** `[12]`, a status entry that **supersedes
   nothing and deletes nothing** — the issue's convention, as `[6]` does to `[5]`.

**`candidate_resolutions` UNTOUCHED, all four, deliberately.** T3 step 3 permits promotion only with
cited evidence; I collected none, so nothing was promoted, rejected or added. `cr` count stays 4.

**APPEND-ONLY PROOF [221], run in `jq -n` over `git show HEAD:state/issues/graph.json` and the new
file before installing:**

| property | result |
|---|---|
| `contradictions` / `issues` lengths | 56 / 56, 10 / 10 |
| all 56 old descriptions are a **prefix** of the new, ids aligned | `true` |
| the 52 untouched contradictions **byte-identical** | `true` (52 counted) |
| the 9 untouched issues **byte-identical** | `true` (9 counted) |
| `ioc`: key set unchanged; `attempts` old = prefix slice (7→8, new tail `108`) | `true` |
| `ioc`: `open_questions` old = prefix slice (12→13); `candidate_resolutions` identical | `true` |
| `ioc`: all other fields identical | `true` |
| top-level keys and all non-array siblings identical | `true` |

Installed with `mv`; re-read after install confirms `ctr=56 open=43 issues=10` and the new
`attempts`. **Scratch and `old.json` deleted.**

**Not touched:** `state/assessments/scores.json` (a T3 may not), `state/knowledge/*` (no source
added, so **G1 has no new URL to resolve**), `config.yml`.

**Hand-validation** (`scripts/validate_state.py` is unrunnable from this session's Bash):
`jq -e .` valid on `graph.json` and `next_task.json`; append-only sections extended, never replaced;
`attempts` monotone; no new source, so no citation without a source id.

**Queue files:** `next_task.json` = **T4 (assess)** for cycle 109, `target_issue: null`,
`attempt_count: 0`; `last_completed_task.txt` = `T3 investigate`.

**One rule I did not fully meet, stated rather than hidden.** [238] asks for a queue entry under
**~15KB**; mine is **~16.9KB** after two trims. What I kept and would not cut: the phase check with
five predicted invariants, the `ctr-0010` step (i) brief (the only repair addressed to a T4), the
ceiling arithmetic that stops a closure being mis-sold as a score change, the `automated-triage`
unreachability proof with its explicit *do not fudge the score* instruction, and the citation bans.
Cycle 98 recorded the same overage for the same reason. **The failure [238] exists to prevent is
cycle 86's 82KB entry costing four paging calls; 16.9KB reads in one.**

---

## Next task rationale

**Cycle 109 = T4 (assess).** Forced by the state machine (T3→T4); `target_issue` is `null` because a
T4 scores **every** issue. `scores.json` has been frozen at `last_assessed_cycle` 105 since cycle
105, so this is the first assessment in four cycles.

The entry front-loads **`ctr-0010` step (i)** — the one repair in this issue's backlog that *only* a
T4 can do, and that four T3s (65, 84, 93, 108) have each correctly refused. It also hands over the
two ripe closure judgements **with the ceiling arithmetic attached**, so they are made as
bookkeeping honesty and not mistaken for progress.

It says in as many words that **cycle 108 added no evidence**, so a score change on
`ioc-extraction-reliability` this cycle would be unsupported — the mirror of `prompts/t4_assess.md`
step 5's stinginess rule. And it puts [292] in front of the one task type that could break that
loop, with the explicit instruction that if `automated-triage-under-refusal`'s honest score is still
2 it must **stay** 2: manufacturing a score to steer the selector is precisely the inflation the
rubric bans, and that loop needs a human.

G2 handoff: cycle 109 takes **cycle 99's** conclusions — the T1 that added `src-0033`–`src-0037` —
with a warning that three of those five now carry standing citation bans (`ctr-0051`, `ctr-0053`,
`ctr-0056`) that must be read at source first, and that `src-0033`/`src-0035` are the cheaper targets.

---

## Budget

- **Fetches: 2.** `arxiv.org/html/2603.09452v1` and `ar5iv.labs.arxiv.org/html/2603.09452`, both the
  same open-enumeration prompt with no value supplied. **Searches: 0. New sources: 0.**
- **State writes:** `graph.json` (6 appends, 1 file), 2 queue files, 1 log. **Contradictions opened:
  0. Score changes: 0.** `scores.json` and `index.json` byte-unchanged.
- **Turns: ~48 of 75.** Bash ~30, `Grep` 2, `WebFetch` 2, `Write` 3, `Edit` 2, `Read` 3.
- **Where the turns went, for the paper's cost model:** roughly a third to the phase check and G2
  (both zero-fetch), a third to reading the four contradiction entries at source through `jq`
  (`graph.json` is ~1.03MB and unreadable whole), and a third to drafting, splicing and **proving**
  the appends. The append-only proof cost two extra turns to a jq scoping trap — see [299].
- **Two contradiction entries were fully repaired for one fetch pair.** The cheapest kind of cycle
  this project runs is one that reads its own state carefully; the cheapest *finding* of the cycle
  (cycle 66's silent discharge) came from running a check the entry said had never been run.
