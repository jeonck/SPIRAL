...
Phase check passed before any work: `state/queue/last_completed_task.txt` read `T4 assess`;
`git show --stat HEAD` showed `9c48513 cycle 129: T4 assess` touching exactly the files the
inherited queue entry predicted. All four confirming jq checks reproduced their stated values
exactly — `ctr=65 open=49 issues=10 last=ctr-0065`; `last=129 n=10 total=22 cyc=10`;
`srcs=44 last=src-0044`; and the score vector with `ttp-attack-mapping-reliability=3`,
`incident-evidence-vendor-concentration=3` and the other eight at 2. I read
`prompts/t5_select.md` at source rather than reconstructing the policy from the queue entry.

I did **not** run `scripts/validate_state.py` — Bash in this environment refuses `python3`.
Every gate below was hand-checked with `jq` and the Grep tool.

## Task performed

T5 — select the next research target and write the next task. No state file other than
`state/meta.json`, the two `state/queue/` files and this log was touched: a T5 selects, and it
adds no source, moves no score and edits no issue.

## Retrospection

**Target: cycle 121 (T2 Structure), commit `36a4324`.** Round-2 order is "cycle N re-checks the
conclusion the next unexamined predecessor entered"; 129 took 120's, so 121 is next in line.
**Zero fetches spent** — the whole re-check is `git show` plus local Grep.

### What cycle 121 actually entered

Per `git show 36a4324 --stat`, cycle 121's only substantive state write was
`state/issues/graph.json` (+23 lines). Reading the diff rather than the log, it entered seven
things, all on `consistency-calibration-as-failure-mode`: appends to three existing
open_questions; two **new** open_questions ([10] and [11]) split out of open_questions[3]; one
further new open_question routed from src-0042; one new `proposed` candidate_resolution
(evidence src-0001, src-0007, src-0042); and contradiction `ctr-0062`. The inherited queue entry
correctly warned that cycle 121's *declined* relocation decision on `candidate_resolutions[14]`
is not a conclusion entered — I took the split instead.

### The conclusion re-checked

Inside the new `open_questions[11]` (the calibration leg), cycle 121 wrote a **method census**
explicitly framed as a standing instruction — "METHOD, SO A SUCCESSOR NEED NOT REDO IT" — whose
load-bearing claim is:

> one Grep at cycle 121 established that the only src-\*.md files mentioning fine-tuning at all
> are src-0001, src-0007, src-0013, src-0014, src-0017, src-0018 and src-0042

This is the right kind of G2 target: a standing instruction that later cycles are invited to
reuse without redoing, resting on a reproducible instrument.

Per [337] I reproduced the instrument before judging it. **Field**: `src-*.md` under
`state/knowledge/`. **Case**: case-insensitive `fine-?tun`. **Snapshot**: cycle 121 —
`git show 36a4324:state/knowledge/index.json` gives `srcs=42 last=src-0042`, so the census was
taken over src-0001..src-0042.

**Result — CONFIRMED, exactly.** Restricted to src-0001..src-0042, the Grep returns exactly
`{0001, 0007, 0013, 0014, 0017, 0018, 0042}` — seven files, the same seven, no more and no
fewer. A widened pattern (`fine tun|finetun|LoRA`) adds nothing but src-0042 itself.

Two of the census's second-order claims re-verified at zero fetches as well:

- *"src-0007 reports NO ECE, NO Brier and NO reliability diagram … its single occurrence of the
  word calibration is LLM-as-Judge agreement calibration."* **CONFIRMED.** `src-0007.md` has
  exactly one `calibrat` hit and it reads "This calibration process yields an agreement rate
  between the LLM-as-Judge and human experts exceeding 95%" — an agreement construct, not a
  proper score. No ECE, Brier or reliability-diagram metric anywhere.
- *"src-0018 states it uses no fine-tuning."* **CONFIRMED.** Its single hit is inside a quoted
  passage: "used within the `Infrastructure`, `Executables`, and `Playbook Extractor` without any
  additional fine-tuning or task-specific adaptation."

### Supersession found — reported as a supersession, not a failure

The census is exact for its snapshot but is **no longer exhaustive at cycle 130**, and it is
written as a standing instruction that a successor is told not to redo. `src-0043` (added at
cycle 125, "Mapping CVEs to MITRE ATT&CK Techniques") is an **eighth** file mentioning
fine-tuning, and it does not merely mention it — line 46 reads "`roberta-base` fine-tuned
throughout". So cycle 121's follow-on clause "of those, only src-0001, src-0007 and src-0042
actually fine-tune anything" is now incomplete too. **This changed at cycle 125.**

**The census's bottom line nevertheless survives.** `src-0043` contains **zero** occurrences of
`calibrat`, `ECE`, `Brier` or `reliability diagram`, so the operative conclusion — that not one
source other than src-0001 pairs fine-tuning with a proper score, in any domain — holds
unchanged at cycle 130.

**No contradiction opened, and here is why.** The claim is explicitly timestamped in its own text
("one Grep *at cycle 121* established"), so as scoped it is true and there is no conflict between
two supported claims for G3 to record. What is defective is its use as a standing enumeration a
successor is told to trust. That is a documentation repair, not a factual conflict, and it lands
in the carry-forward as item (10) for a T2 or a T3 on that issue. I record it here rather than
editing the graph because a T5 does not write to `graph.json`.

Cheap trap worth recording, since it nearly produced a false mismatch — a case-insensitive grep
for `ECE` matches inside ordinary English words (`re**ce**ive`, `ne**ce**ssary`), so a raw hit
count for a metric acronym is not a count of the metric. Logged as **[349]**.

## Changes made

Selection run, then two queue files and this log. `state/knowledge/`,
`state/assessments/scores.json` and `state/issues/graph.json` are **untouched**.

### Step 1 — candidate set

All ten issues score < 5 (eight at 2, two at 3). Candidate set = all ten.

### Step 2 — base priority = score

The eight issues at score 2 lead; `ttp-attack-mapping-reliability` (3) and
`incident-evidence-vendor-concentration` (3) are eliminated here. Both are at their G3 ceiling
of `scale_max − g3_contradiction_demotion` = 5 − 2 = 3.

### Step 3 — tie-breaks. Full ranking table

All inputs re-derived with `jq` from `state/issues/graph.json` and
`state/assessments/scores.json` this cycle, not copied from the inherited queue entry.
Penalty window for 3(b) = cycles 125–129 (the five cycles preceding this one); the only issue
whose membership is window-sensitive is `institutional-incident-real-world-impact`, and it takes
the penalty under either the 125–129 or the 126–130 reading, so nothing here turns on that choice.

| # | issue | score | created | attempts | last attempt | 3(a) upstream | 3(b) penalty | eff. | outcome |
|---|-------|-------|---------|----------|--------------|---------------|--------------|------|---------|
| 1 | **ioc-extraction-reliability** | 2 | 2 | 9 | 117 | root (depends_on ∅; 1 dependent) | 0 | **2** | **SELECTED** (ad-hoc rung 4) |
| 2 | consistency-calibration-as-failure-mode | 2 | 2 | 12 | 122 | root (depends_on ∅; 3 dependents) | 0 | 2 | lost ad-hoc rung 4 |
| 3 | automated-triage-under-refusal | 2 | 16 | 0 | — | isolated (depends_on ∅; 0 dependents) | 0 | 2 | **elim. 3(c)** (created 16 > 2) |
| 4 | institutional-incident-real-world-impact | 2 | 2 | 7 | 128 | root (depends_on ∅; 1 dependent) | **+1** (cyc 128) | 3 | **elim. 3(b)** |
| 5 | attribution-confident-wrong-gap | 2 | 2 | 2 | 28 | dependent (→ consistency) | 0 | 2 | **elim. 3(a)** |
| 6 | task-dependent-reliability-framing | 2 | 2 | 3 | 125 | dependent (→ 5 issues) | +1 (cyc 125) | 3 | **elim. 3(a)** |
| 7 | attribution-expressed-confidence-unmeasured | 2 | 45 | 1 | 49 | dependent (→ 2 issues) | 0 | 2 | **elim. 3(a)** |
| 8 | extraction-vs-reasoning-ordinal-axis | 2 | 16 | 2 | 18 | dependent (→ task-dependent) | 0 | 2 | **elim. 3(a)** |
| 9 | ttp-attack-mapping-reliability | 3 | 2 | 6 | 68 | root | 0 | 3 | **elim. step 2** |
| 10 | incident-evidence-vendor-concentration | 3 | 100 | 1 | 101 | dependent (→ institutional) | 0 | 3 | **elim. step 2** |

Rung 3(a) is stated pairwise ("an issue that others `depend_on` outranks its dependents"). I
applied it as: an issue is eliminated if any issue it `depends_on` is still in contention. That
removes rows 5–8, all of which have a non-empty `depends_on` whose members are score-2 candidates.

One movement worth flagging against cycle 127, because it is the tie-break machinery working as
designed rather than drift: `consistency-calibration-as-failure-mode` was penalised out at 3(b)
at cycle 127 (its attempt at cycle 122 was then inside the window) and has now **aged back into
contention**, while `institutional-incident-real-world-impact` — which cycle 127 selected — has
taken cycle 128's attempt as a penalty and dropped out.

### Step 3, rung 4 — the invented rung. This is not policy

**The tie-break list in `prompts/t5_select.md` does not terminate.** After 3(a), 3(b) and 3(c)
I am left with `ioc-extraction-reliability` and `consistency-calibration-as-failure-mode`, both
score 2, both effective 2, both `created_cycle` 2. This is the second occurrence of a known,
twice-reported structural defect (cycle 127 hit it on a different pair).

I broke it with **least-recently-attempted**, the same ad-hoc rung cycle 127 invented:
ioc's last attempt is cycle 117, consistency's is cycle 122, so **ioc wins**. I chose to reuse
cycle 127's rung rather than invent a fresh one specifically because a selector that invents a
*different* fourth rung each time it deadlocks is not reproducible, and reproducibility of the
selection trace is the point of this table.

**I state plainly that this rung is not in the prompt and that I have no authority to adopt it.**
Alternative rungs, tested against this pair:

| candidate rung | ioc | consistency | picks |
|---|---|---|---|
| least-recently-attempted (cycle 127's precedent, **used**) | 117 | 122 | **ioc** |
| fewest total attempts | 9 | 12 | ioc |
| fewest supported candidates (weakest evidence base) | 3 | 12 | ioc |
| most open questions | 14 | 13 | ioc |
| most open contradictions | 3 | 10 | **consistency — dissents** |
| highest downstream fan-out (most dependents) | 1 | 3 | **consistency — dissents** |

Four agree, two dissent. The dissent from **highest-downstream-fan-out** is the one a human
should weigh hardest, because it is not an arbitrary alternative — it is a natural extension of
rung 3(a)'s own upstream-first logic, and it would have picked `consistency`. Note also that
"most open questions" *dissented* at cycle 127 and *agrees* here, which is evidence that these
rungs are not stably ordered and that picking among them cycle-by-cycle would be arbitrary.

**A human should rule on a fourth rung.** Repeated for the second time.

### Step 4 — refresh rule

`config.yml` sets `schedule.collect_refresh_every: 7`. `130 % 7 = 4 ≠ 0` (7 × 18 = 126;
130 − 126 = 4). **No refresh is due.** The edge is T5 → T3 and I wrote a T3.

### The standing honesty item, sharpened

`automated-triage-under-refusal` has still never been investigated in 130 cycles — `attempts` is
empty, the only such issue in the graph, created at cycle 16. It survived 3(a) and 3(b) again
this cycle and died at 3(c) on `created_cycle` 16 against 2, exactly as at cycle 127.

Previous cycles called it "unreachable by the stated policy". **That is slightly too strong and I
am correcting it, because the precise version gives a human a concrete lever.** Because the issue
is never attempted it never accrues a 3(b) penalty, so it *would* win in any cycle where every
score-2 `created_cycle`-2 root issue simultaneously carried a recent-attempt penalty. That
coincidence has not occurred in 130 cycles and there is no mechanism pushing toward it. So: not
strictly unreachable, but unreached and with no path to being reached that the loop can arrange
for itself. My reading of `prompts/t5_select.md` gives me **no discretion** to select it —
rung 3(c) is deterministic and it loses. A human should rule on whether the selector needs a
staleness or never-attempted term. Twenty-second consecutive cycle.

The same lack of discretion has a second cost this cycle, and it is worth naming: carry-forward
item (6) — src-0006's unfetched per-task scoring definitions — is described by cycles 125, 126
and 129 as the single highest-value unfetched item in the base and is load-bearing on two issues.
It sits on `task-dependent-reliability-framing`, which rung 3(a) eliminates automatically and
permanently, since that issue `depends_on` five others. **The selector cannot reach the item its
own investigators keep calling the most valuable one.** That is a third structural finding about
this policy, and it is evaluation data for the paper.

### Files written

- `state/queue/next_task.json` — T3 targeting `ioc-extraction-reliability`, `attempt_count` 0,
  self-contained, carrying the full ban set and the ten carry-forward items. Validated with jq.
- `state/queue/last_completed_task.txt` — `T5 select`.
- `logs/cycle-130.md` — this file.
- `state/meta.json` — cycle counter (by the runner).

## Next task rationale

T3 (Investigate) on `ioc-extraction-reliability`, per the selection above and the T5→T3 edge.

Within the issue, I pointed the T3 at **open_questions[2]** — the scaffolding-versus-LLM
decomposition — rather than letting it choose. The reason is that cycle 117, the last T3 here
(the issue's ninth), left an unusually explicit handoff in the tail of open_questions[13], and it
converges on this question from two directions: it found that src-0003's own
HTML-summarization retrieval caveat had never been carried by the state, and recorded that "the
'precise regex/LLM task split' half of the caveat is undischarged and is exactly what [2] asks
for" — and it noted that **no cycle has read src-0003 end to end** in ten investigations.
open_questions[2] currently rests on an *inference* from column headers (that src-0007's
undefined term "Vanilla" means unscaffolded), which cycle 21 flagged as weaker evidence than nine
cycles of notes had assumed. Moving that onto text is exactly what a T3 can do at one fetch pair
against an existing source.

I explicitly fenced off two things. **open_questions[4]** (the adversarial threat model) has been
declined five times — cycles 21, 52, 93, 108, 117 — on the correct ground that no source in this
base measures it, making it a T1 collection target that no T3 can discharge; the T3 is told not
to spend a search on it and to say it did not. **ctr-0010 step (i)** asks for a scores.json edit,
which a T3 does not write; cycle 117 already recorded this and it stays with a T4.

I also told the T3 to append cycle 131 to the issue's `attempts` array, since tie-break 3(b)
reads that array and a missing entry would silently corrupt future selections — a failure mode
this cycle's own derivation made visible.

## Budget

- Web searches: **0**
- Fetches: **0** (G2 done entirely from git history and local files, per the zero-fetch standard
  cycles 121–129 set; the selection needs no external evidence)
- Bash calls: 18 (one refused — see [348])
- Read/Grep tool calls: 8
- Write tool calls: 2
- Turns used: roughly 22 of 75
