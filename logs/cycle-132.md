...
T4 (Assess) per `prompts/t4_assess.md`, read at source. Scored **all ten** issues in
`state/issues/graph.json`, set `assessed_cycle = 132` on every entry and
`last_assessed_cycle = 132`.

**Phase check ran first and passed on all five probes.** `state/queue/last_completed_task.txt`
read `T3 investigate`; `git show --stat HEAD` showed `e605eca cycle 131: T3 investigate`
touching `state/meta.json`, `state/issues/graph.json`, both queue files and
`logs/cycle-131.md` — and **not** `state/knowledge/` or `state/assessments/scores.json`,
exactly as a T3 should. The four confirming jq calls returned `ctr=66 open=50 issues=10
last=ctr-0066`, `last=129 n=10 total=22`, `srcs=44 last=src-0044`, and
`attempts=[9,21,35,52,65,84,93,108,117,131] nq=15 ncand=5`. All four matched the queue
entry's stated expectations, so I did not need to re-derive my phase from
`last_completed_task.txt`.

### The result, stated plainly

**The score vector did not move.** Old and new, side by side:

| issue | old | new | open contradictions |
|---|---|---|---|
| ttp-attack-mapping-reliability | 3 | **3** | 6 |
| incident-evidence-vendor-concentration | 3 | **3** | 1 |
| task-dependent-reliability-framing | 2 | **2** | 3 |
| extraction-vs-reasoning-ordinal-axis | 2 | **2** | 2 |
| attribution-confident-wrong-gap | 2 | **2** | 3 |
| attribution-expressed-confidence-unmeasured | 2 | **2** | 4 |
| ioc-extraction-reliability | 2 | **2** | 4 |
| consistency-calibration-as-failure-mode | 2 | **2** | 10 |
| institutional-incident-real-world-impact | 2 | **2** | 12 |
| automated-triage-under-refusal | 2 | **2** | 5 |
| **total** | **22** | **22** | |

A T4 that moves nothing is a real result when nothing has entered the base, and I said so
in every entry rather than implying fresh derivation. **No source has entered the base
since cycle 128** (`srcs=44`, `last=src-0044`); cycle 130 was a T5 (selection only) and
cycle 131 was a T3 that added no source. So for eight of the ten issues there was
genuinely nothing new to weigh, and the honest entry is a re-derivation that says it is one.

### G3 demotion, re-derived at source

I re-read `scripts/validate_state.py` lines 144–156 rather than inheriting prior cycles'
account of it. It builds the open set as a **set comprehension over `issue_id`**, so N open
contradictions on one issue impose **one** ceiling of `scale_max - g3_contradiction_demotion
= 5 - 2 = 3`, not an N-fold subtraction. **Every one of the ten issues carries at least one
open contradiction**, so every entry in the vector is ceiling-capped at 3. The ceiling
**binds** on exactly two entries (ttp, vendor) and is **slack** on the other eight — meaning
those eight 2s are merit scores, not capped ones. I noted the demotion in every rationale as
step 3 requires.

Carry-forward **[4]**, the subtraction-versus-ceiling policy conflict, is still unresolved.
Read as a literal subtraction, `prompts/t4_assess.md` step 3 would put ttp and vendor at 1.
I recorded the score the validator will accept and flagged in both rationales that the two
rules disagree and only a human can settle it.

### Three places I declined a 3 the arithmetic would have allowed

Being stingy meant refusing id-counting three times, each on a stated ground:

1. **attribution-expressed-confidence-unmeasured** — the closest call in the vector. All
   three candidates are multi-source (3, 3, 2 ids) and none is merely proposed; on a
   numerical reading this is a 3. Held at 2 because the issue's central claim is a
   **negative existential**, and ctr-0034 is on record that such a claim is only as strong
   as the search that failed to falsify it. `attempts` is `[49]` — one investigation, 83
   cycles ago. Three ids that each failed to find a thing are not three independent
   confirmations the thing is absent.
2. **extraction-vs-reasoning-ordinal-axis** and **task-dependent-reliability-framing** —
   both held at 2 because a supporting limb is **src-0006, whose per-task scoring
   definitions have never been fetched** (ctr-0009 step (iii), still open). For the ordinal
   axis this goes to the heart of the claim, not its margin: the axis *is* an ordering of
   tasks by measured reliability, so an unread definition of how those tasks are scored is
   load-bearing.

### Ban-set arithmetic on the two widest issues

`institutional-incident-real-world-impact` and `incident-evidence-vendor-concentration` have
the widest id counts in the file (14 and 13 in their evidence arrays). Those counts are
**inflated by sources this state may not quote**. I re-derived the *quotable* support after
subtracting the ban set rather than counting ids: for vendor, candidate 0 retains src-0032,
src-0012, src-0035, src-0004 and candidate 1 retains src-0032, src-0038, src-0039 — both
still clear two independent quotable sources, so the 3 stands on surviving evidence and not
on raw count. I flagged that vendor entry as **the one score in the vector a single further
outright ban could break**, and told a successor T4 to re-run the check rather than inherit
my conclusion.

### automated-triage-under-refusal — looked at, not re-entered by inertia

The cycle-131 queue entry specifically asked the cycle that sets this score to examine it.
I read the issue and its sole candidate in full first. The candidate is `supported` with two
ids (src-0007, src-0015), which on arithmetic clears the level-3 bar. **It does not clear it,
and the candidate's own text says so**: it records a provenance asymmetry and states the
claim *rests* on src-0007, with src-0015 "worth little as independent quantification"
(single-author preprint, no stated venue or affiliation, n=40 synthetic episodes per model,
no confidence intervals). Separately and independently decisive: **src-0015 is under a
standing quotation ban** until ctr-0057 is read in full. A candidate that disclaims its own
second limb is a single-source candidate, which is the rubric's definition of level 2.

I recorded the honest characterisation in the rationale: **this 2 is a floor read off a
single never-tested candidate.** `attempts` is empty; the issue was created at cycle 16 and
has never been investigated in 132 cycles. Its score has therefore never once been informed
by an investigation, and no successor should read the stability of this 2 across many
assessments as corroboration — it is the same untested reading re-entered.

### ctr-0066 step (ii) — discharged, null result

ctr-0066 (opened cycle 131) charges that this state has described the system reaching
src-0003's headline F1 as one *including human validation*, when src-0003 places the
human-validation loop outside the measured path of that number. Its step (ii) asked for a
sweep of `scores.json` for the same conflation; cycle 131 did not run it and recorded its
size as unknown. **I ran it, because I was the cycle writing that file.** Four patterns,
using the [348] `scan()` window instrument:

| pattern | hits | verdict |
|---|---|---|
| `97\.6` (260/200-char window) | 18 | none conflating |
| `[Hh]uman.{0,3}validat` | **0** | — |
| `hybrid` | **0** | — |
| `HITL\|human.in.the.loop\|human review\|human-in` | **0** | — |

The eighteen `97.6` windows describe the figure as a closed-set classification score over a
regex-extracted candidate set, or as LANCE scored against the manually-labelled PRISM
reference — which places the analysts on the **reference** side, exactly as ctr-0066 says is
correct. **Zero corrections are owed in `scores.json`.** The conflation ctr-0066 charges
lives in `graph.json`'s candidate wording, not in this file. This mirrors cycle 129's
discharge of the analogous ctr-0064 step (ii), also null. Steps (i) and (iii) remain owed to
a T3 or T2 and I did not touch them — a T4 does not write `graph.json`.

### One count re-derived rather than inherited

Per [337] I reproduced the instrument before relying on a prior cycle's number. On
`consistency-calibration-as-failure-mode`: **12 supported candidates, 10 of which include
src-0001, and 4 of which rest on src-0001 and nothing else.** The ten-of-twelve figure
reproduces cycle 129's discharge of ctr-0063 step (i) exactly (against cycle 118's reported
"eleven"). The solo-src-0001 count of four is new and I add it as my own finding. This is why
that issue's fifteen candidates and eight-id widest candidate do not make it the
best-supported issue in the graph.

---

## Retrospection

**Target: cycle 123** — next in the round-2 order (131 took 122's, so 132 takes 123's).
Commit `58b5b33`, stat-verified before trusting the message: it touched
`state/assessments/scores.json`, `state/meta.json`, both queue files and `logs/cycle-123.md`
— a clean T4 commit.

Cycle 123 was itself a T4, so what it **entered into the state** is the set of rationale
appends in `scores.json`. Per the standing rule, I targeted what was entered, not what the
log said. Reading the tails of the cycle-123 snapshot, I picked its claim on
`task-dependent-reliability-framing`, which is quantified, comparative and therefore
falsifiable:

> "...and THREE open questions — the FEWEST open questions in the graph. attempts is [6,16]:
> last investigated at cycle 16, ONE HUNDRED AND SEVEN CYCLES AGO, the stalest of any issue
> that has been investigated at all."

Two limbs, both checkable at zero fetches. I reproduced the instrument as a ten-issue census
over the cycle-123 **input** snapshot (`git show 58b5b33^:state/issues/graph.json`) and the
current graph.

**Limb 1 — "three open questions, the fewest in the graph." VERIFIED at the snapshot.**
The census returned `nq=3` for task-dependent-reliability-framing, with the next lowest at 4
(attribution-confident-wrong-gap). Three, and genuinely the minimum.

**Limb 2 — "attempts [6,16], the stalest issue that has been investigated at all." VERIFIED
at the snapshot.** Last-attempt cycles across the nine investigated issues were 68, 117, 122,
28, 49, **16**, 18, 114, 101. Sixteen is the minimum; extraction-vs-reasoning-ordinal-axis at
18 was second. Also correct.

**Both limbs are now SUPERSEDED, and I report that as a supersession and not a failure.**
Currently task-dependent-reliability-framing shows `nq=6`, `attempts=[6,16,125]`. **The
change happened at cycle 125, commit `8f54b27` ("cycle 125: T3 investigate")** — I checked
that snapshot directly and it already reads `nq=6 attempts=[6,16,125] ncand=4`. So the
supersession is datable to one commit. The fewest-open-questions title passed to
attribution-confident-wrong-gap (nq=4), and the stalest-investigated title passed to
extraction-vs-reasoning-ordinal-axis (attempts `[17,18]`, last touched 114 cycles ago).

**The pleasing part, worth recording:** cycle 123's rationale did not merely state those two
facts, it *made a request on them* — "a T3 here is the cheapest way to find out whether this
2 is a 3 nobody has checked or a 2 that is correct... Cycle 109 said so, 115 repeated it, 118
repeated it, and NO SUCH INVESTIGATION HAS RUN IN THE FOURTEEN CYCLES SINCE." **That request
was answered two cycles later.** The conclusion was superseded precisely because the system
did the thing the conclusion asked for. I have carried both inherited titles into this
cycle's rationales for the issues that now hold them, re-derived rather than inherited.

**Verdict: PASSED as written, SUPERSEDED at cycle 125. No contradiction opened.**
**Fetches spent: ZERO** (four Bash calls: two censuses, one `git log` grep, one snapshot
probe). Cycles 121–132 have now all done G2 at zero fetches.

---

## Changes made

- **`state/assessments/scores.json`** — the only state file I touched.
  - `last_assessed_cycle` 129 → **132**.
  - `assessed_cycle` set to **132** on all ten entries (verified: `unique` → `[132]`).
  - A cycle-132 rationale **appended at the tail** of each of the ten existing rationales
    per [347]. Nothing rewritten, nothing deleted.
  - **Scores unchanged; evidence arrays unchanged.** Total 22, max 3.
- **`state/queue/next_task.json`** — T5 for cycle 133, `attempt_count` 0, `target_issue`
  null (selection is that task's whole job), full ban set and carry-forward list carried.
- **`state/queue/last_completed_task.txt`** — `T4 assess`.
- **`logs/cycle-132.md`** — this file.
- Not touched: `state/knowledge/` (no source added — correct for a T4),
  `state/issues/graph.json` (a T4 does not write it).

**Method.** Ten-key scratch JSON written with the Write tool, single quotes only for
verbatim material, validated with `jq -r keys` before use; one two-input splice per [237];
then the **five-conjunct append-only proof** before the `mv`, all five passing:
`keys_equal=true`, `score_keys_equal=true`, `rationale_prefix_all=true`,
`evidence_prefix_all=true`, `schema_unchanged=true`, plus the score vector reported old
against new (22 → 22). Scratch removed with `rm -f`.

**Gate status, stated honestly.** Bash refuses `python3`, so **I could not run
`scripts/validate_state.py`** and I am not implying otherwise. I hand-checked its three
relevant gates with jq: G1 — every entry scoring ≥ 2 has a non-empty evidence array
(`ev_nonempty=true`) and no evidence id was added, so the index-membership check is
unchanged from the state that already passed; G3 — max score is 3 and the ceiling is 3, so
no entry exceeds it; schema — `_schema` byte-identical and `scores` still an object keyed by
issue_id with the four required fields.

---

## Next task rationale

The state machine's edge is **T4 → T5**, so cycle 133 is a Select. `target_issue` is null
because choosing it is that task's entire job.

**The refresh rule fires on cycle 133, and I have flagged it as the most important line in
the queue entry.** `prompts/t5_select.md` step 4: *"if `current_cycle %
schedule.collect_refresh_every == 0`, the next task is `T1` targeting the selected issue
(bring in fresh evidence) instead of `T3`."* `collect_refresh_every` is 7 and 7 × 19 = 133,
so 133 % 7 == 0 and **cycle 133 must queue a T1, not a T3**. I checked the precedent: cycle
119 % 7 == 0 and the T1 that followed added src-0042 at cycle 120. I told cycle 133 to
re-derive both the modulus and `collect_refresh_every` at source rather than take them from
me.

This matters more than usual. Cycles 129–132 all added no source; the base has stood at 44
since cycle 128; and my own assessment just found nothing new bearing on *any* issue since
cycle 129 — ten entries, all re-derivations. **A collection cycle is what this state
actually needs**, and the calendar is about to supply one.

I pointed the T1 at the highest-value target while being explicit about the obstacle: ctr-0009
step (iii), src-0006's unfetched per-task scoring section, is now named by **five** cycles as
the single highest-value unread item in the base, and I have just made it the *stated* reason
two issues are held at 2 rather than 3. But its home issue has a non-empty `depends_on` and
tie-break 3(a) permanently eliminates such issues, so the selector cannot reach it by the
normal path. Rather than invent authority for a T5 to aim a T1 at a source, I noted that
**extraction-vs-reasoning-ordinal-axis has an empty `depends_on`, is reachable, and rests on
the same unread src-0006 rule** — so a T1 there could pick the item up legitimately. If that
does not work within the rules, cycle 133 is told to escalate rather than improvise.

I also warned cycle 133 that eight of ten issues sit at 2, so it lands in the tie-breaks
immediately; that only ioc-extraction-reliability (131) and
institutional-incident-real-world-impact (128) carry a recent-attempt penalty in the
cycles-128–132 window; and that if it reaches the known non-terminating tie it must say so
and escalate rather than present an invented fourth rung as if it were in the prompt.

## Budget

- **Web searches: 0. Web fetches: 0.** Neither the G2 nor the assessment needed one — no
  source claim was re-opened, and every check was a re-derivation over git snapshots and the
  current state.
- Bash calls: 24. Read calls: 3. Write calls: 3 (scratch, queue entry, this log).
- Turns used: roughly 33 of the 75 in `config.yml`. Bookkeeping was written before the log,
  in the mandated order, per the cycle-31 lesson.
- Cost concentration: the ten-key scratch and the splice/proof/`mv` sequence, plus the
  ctr-0066 four-pattern sweep. The G2 came in at four Bash calls.
