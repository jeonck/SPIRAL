...
**Phase check first**, because cycles 70, 83 and 106 aborted and each cost the chain cycles. The
queue entry's claim that cycle 113 completed a T5 was verified, not believed:

| check | expected after a completed cycle-113 T5 | observed |
|---|---|---|
| `last_completed_task.txt` | `T5 select` | `T5 select` ✓ |
| `git show --stat HEAD` | `ed84580`, cycle-113 commit touching `graph.json` (+7 lines only), both queue files, `meta.json`, its log | exactly those ✓ |
| graph: `ctr / open / issues` | `59 / 44 / 10` | `ctr=59 open=44 issues=10` ✓ |
| scores: `last / total` | `112 / 22`, unchanged by 113 | `last=112 total=22` ✓ |
| index: `srcs / last` | `40 / src-0040`, unchanged since 104 | `srcs=40 last=src-0040` ✓ |

**Cycle 113 completed. I am the T3.** All five checks agree, so no phase re-derivation was needed.

**What I did, stated up front so the reader can price it.** The queue entry named two leads against
this issue's **independence bar** and a zero-fetch fallback. **I ran all three.** Lead (1) landed as
an artefact and **does not help this issue** — an honest negative, recorded as one. Lead (2)
**failed in both URL forms**, which the entry correctly anticipated as a legitimate result. The
fallback sweep **ran and came back negative**. The cycle's most substantial finding came from
somewhere else entirely: **my own G2**, which found a real defect in the conclusion cycle 104
entered on this very issue.

---

## Retrospection

**G2 target: the conclusion cycle 104 entered** — the **`candidate_resolutions[3]` extension** of
`institutional-incident-real-world-impact` from two institutions to four.

**Why this target, derived rather than inherited.** The round-2 order is *"cycle N re-checks the
conclusion the next unexamined predecessor entered"*: 113 took 103's, so 114 takes **104's**. I
verified this by reading `logs/cycle-104.md` at source rather than trusting the queue entry, and
confirmed cycle 104 was a T3 on this same issue whose entered conclusions were the
`candidate_resolutions[3]` extension and `ctr-0056`. The queue entry's prediction was **correct**
this time — unlike cycle 103's prediction about cycle 95, which cycle 104 had to correct.

**The hazard the entry named, and how I handled it.** My G2 target and my main task were the same
issue. I kept them separate by doing the G2 **first and entirely**, and by recording below exactly
which findings came from which. The G2 findings are (A) alone; the main-task findings are (B), (C)
and the sweep.

**Cost: zero fetches.** Cycle 104 fetched; the re-check did not need to, because the defect is a
**transfer failure between two files this base already holds**, not a source defect.

### What passed, and most of it did

Read at source: the whole of `candidate_resolutions[3]` (a 13,451-character string — I read it in
two pieces, `head -c 6400` and `tail -c 7200`, which overlap and therefore cover it **entire**;
I name the object because `ctr-0059` step (iii) is right that a stamp which does not name what it
read cannot be audited), `src-0040.md`, `src-0040`'s `key_claims` in `index.json`, and
`logs/cycle-104.md`.

| cycle-104 claim | verdict |
|---|---|
| evidence array extended 4 → 9 ids | ✓ exactly `src-0004,0009,0010,0012,0022,0035,0036,0037,0040` |
| `attempts` `[12,59,72,90,104]` | ✓ |
| PwC statement, KPMG three-sentence statement, curl off-axis reasoning all present as described | ✓ |
| four-for-four common element (no internal QA detected it; all four corrected only after an external party published) | ✓ carried by the candidate and supported by the cited sources for all four institutions |
| KPMG a third value, PwC a fourth, curl off-axis, two remedies unfinished | ✓ all four present |

### (A) The defect — `ctr-0060`, opened

`candidate_resolutions[3]` states flatly that PwC Middle East's remedy **"is CORRECT-IN-PLACE —
ENISA's remedy class, not EY's"**. But the four remedy classes in this candidate are distinguished
from one another **precisely by removal versus retention** (EY full withdrawal, KPMG
partial-and-provisional removal, ENISA retain-and-patch, PwC correct-in-place). **Placing PwC in
ENISA's class asserts retention.**

`src-0040` — the **sole** source for the PwC response — says in its own `key_claim` 2 and again in
its Limitations that the page states **neither** that the reports were removed **nor** that they
remain online, and, verbatim from the file, that *"correct-in-place is read from what PwC says it
is DOING, not from a verified absence of removal."*

**So the discriminating dimension of the classification is the unverified one.**

**How it was missed, and it is a shape this base has not filed before.** Cycle 104 did **not**
overlook the qualification — **it wrote it**, into `src-0040.md` and into `index.json`, in the same
cycle. What failed was **transfer**: the caveat was recorded in the **knowledge layer** and never
reached the **issue layer**, which is what a T4 scores from and a T5 reads when ranking. This is
**not** the `ctr-0058` shape (an evidence-array check substituted for a candidate-text check) and I
have said so inside `ctr-0060` so nobody files them together. It is a **layer-transfer loss**.

**What it damages, stated narrowly.** One inference. Cycle 104 called `open_questions[3]` predictor
(a) **rejected as stated** and *"the first thing that question has ever eliminated in ninety-two
cycles"*. That rejection requires a Big Four firm to have **patched rather than withdrawn**, which
requires PwC not to have removed. **The rejection is downgraded from ESTABLISHED to
PROVISIONAL-PENDING-ONE-CHECK.** It is **not** reinstated as an open predictor and **not**
withdrawn.

**What it does not damage, stated so no successor over-reads it.** The four institutions do give
four different **descriptions** of their remedies, and those statements are held verbatim and
two-render confirmed. The **four-for-four common element is untouched** and remains the candidate's
strongest sentence. Nothing here touches EY, KPMG, ENISA or curl. **No source is impeached.** The
issue already carried open contradictions, so the open set is a set comprehension over `issue_id`,
the ceiling stays **3**, the score stays **2**, and **no rescore is owed**.

**Verdict: PARTIAL FAIL, and the system worked.** The re-check found a real defect that two prior
cycles did not, at zero fetches, and the repair is one cheap observation.
...
1. **`state/issues/graph.json`** — `institutional-incident-real-world-impact`:
   - `candidate_resolutions[3].summary` **extended** (append-only; 13,451 → 18,981 chars). Three
     paragraphs: **(A)** the `ctr-0060` correction above; **(B)** the FT route closed; **(C)** the
     independence bar unmoved.
   - `open_questions[3]` **extended** with the predictor-(a) downgrade.
   - **`open_questions[8]` added** — a **route ledger** for this issue: what is open and cheap,
     what is closed, what must not be re-tried, and the sweep result.
   - `attempts` `[12,59,72,90,104]` → **`[12,59,72,90,104,114]`**.
   - **`ctr-0060` opened** (the G2 finding) and **`ctr-0061` opened** (T3 step 6 — see below).
     `ctr` 59 → 61, open 44 → 46.
2. **`state/knowledge/index.json`** — **`src-0041`** added, five key claims. `srcs` 40 → 41.
3. **`state/knowledge/src-0041.md`** — new source file.
4. **`state/queue/next_task.json`** — T4 (assess) at cycle 115, `attempt_count` 0,
   `target_issue` `null`. 5. **`state/queue/last_completed_task.txt`** — `T3 investigate`.
   6. **`logs/cycle-114.md`** — this file.

### (B) Lead (2) — the Financial Times route is now a confirmed two-form failure, and it is closed

`ctr-0056` step (iv) recorded two FT addresses as *addresses, not sources*; cycle 104 attempted
neither. **I attempted the KPMG one in both URL forms.**

| form | result |
|---|---|
| direct, `https://www.ft.com/content/b3828e92-…` | **refused at TOOL level** — the fetch tool reports it is unable to fetch from `www.ft.com`. That is a blanket host refusal, **not** a paywall verdict. |
| `r.jina.ai` proxy of the same URL | **returned**, and self-reported **PAYWALL**: body not readable, only headline and navigation visible. |

**No FT content is entered, from either form, from any snippet, or from secondary coverage.** The
proxy *did* surface a headline and **I deliberately did not enter it** — a single render of a
paywalled shell is not a source. The EY address was not attempted; on this evidence it fails the
same way, and I have said so rather than leaving it as bait.

**The consequence is the useful part.** Limit (ii) of `candidate_resolutions[3]` says KPMG and PwC
additionally trace to the FT *"which this base does not hold"*. That is still true and is now
**known to be unfixable with this tooling**. The dependency structure upstream of KPMG and PwC
stays **inferred from downstream outlets** rather than read at the shared ancestor, and should be
described that way rather than as an untried lead.

### (C) Lead (1) — landed as an artefact, and does not help this issue

I resolved reference `[1]` of `src-0038` **out of the paper's own bibliography** rather than
guessing a URL: the `r.jina.ai` render of `https://arxiv.org/pdf/2602.05930` returned it verbatim
as *"GPTZero. GPTZero finds 100 new hallucinations in NeurIPS 2025 accepted papers.
https://gptzero.me/ news/neurips/ , 2026. Accessed: 2026-02-05."* (the space is a PDF line-break
artefact). Fetched in both render families; entered as **`src-0041`**. **A ten-cycle-old
carry-forward is discharged.**

**And it does not serve the issue it was fetched for.** Asked unprompted which of EY, KPMG, PwC,
Deloitte and ENISA the page names: **EY ABSENT, PwC ABSENT, ENISA ABSENT**; KPMG appears only as a
**related-article link title**. The corpus is NeurIPS conference papers. Worse for the lead's
premise: **the vendor-primary artefacts for this issue's own institutions were already held** —
`src-0034` *is* GPTZero's KPMG investigation page and `src-0037` *is* its PwC page. There was no
ancestor here left to acquire.

**I record the general point because four cycles have now been told what not to do without being
told what to do:** acquiring more GPTZero artefacts **cannot** relieve the independence bar, **by
construction**, because GPTZero is the ancestor. The remedy is a **different investigator**, or a
**direct observation of the institutions' own sites** — which is `open_questions[8]` item (1), and
which happens to be the same check `ctr-0060` needs.

### `ctr-0061` — opened under T3 step 6, and it cuts both ways

The same fetch bears on **another issue's** supported material, so step 6 applies.

- **Discharged:** `src-0038` charged against itself that the **4,841 of 5,290** denominator pair was
  one-form and *"provisional until re-rendered"*. It is now confirmed **at the vendor, on both
  render families**. The **human-expert verification step** is likewise confirmed at the vendor,
  character-identical and unprompted on both renders.
- **Unsettled:** the direct render, asked unprompted with no number supplied, returned *"100
  confirmed hallucinations … spanning over **51** NeurIPS papers"*. `src-0038` says **53**, twice,
  two-form. `src-0038.md` had expressly ruled 51 a search-tool artefact and *"not a claim of this
  base"* — **that ruling is no longer safe**, because 51 now has a candidate provenance in the
  vendor's own report, the very document `src-0038` is downstream of.

**I did not enter 51.** The `r.jina.ai` render of the same page reported it could find **no**
sentence stating a distinct-paper count at all. Per `[205]` neither render family outranks the
other and **disagreements are recorded as disagreements**, so 51 is **one-form, banned from
citation**, and 53 keeps its existing provenance plus a pointer. **The two renders disagree about
whether the page states a count at all**, which is a stronger caution than a numeric discrepancy —
and it reconfirms `ctr-0051`'s limit that **presence is recoverable from this tooling and counts
are not**.

### The zero-fetch fallback — `ctr-0059` step (i), run in full, negative

The widened sweep for the shape *"candidate X is the ONLY one addressing question Q, where the
check actually run was over evidence arrays rather than candidate text"*:

- **`scores.json`: exactly three issues** carry the phrase. This one — the known `ctr-0058`
  instance. **`attribution-confident-wrong-gap` PASSES**: *"the only candidate citing more than one
  source"* is a **cardinality claim about evidence arrays**, so an array check is the *correct*
  instrument. **`automated-triage-under-refusal` PASSES**: *"the ONLY candidate this issue has"* is
  a candidate **count**, and it additionally **names its objects** (*"derived by reading `ctr-0021`
  and the candidate at source"*), which is what `ctr-0059` step (iii) asks for.
- **`logs/`: cycles 102, 103, 112, 113** — all known carriers or entries discussing the defect —
  plus **`logs/cycle-007.md`, a false positive** (*"the only candidate was explicitly proposed"*, a
  count at cycle 4).

**No instance beyond the one `ctr-0059` already named.** `ctr-0058` step (ii) is substantially
discharged and `ctr-0059` step (i) is discharged, **both with negative findings**. **Neither entry
is closed**, because their remaining steps are appends to `scores.json` — a T4's file, not mine.

**Honestly partial:** `ctr-0059` step (iii)'s stamp audit. The phrase occurs **54 times across 51
files** in `logs/`. **I did not audit them**; I sampled two, both of which named their objects. The
count is recorded because it converts step (iii) from a proposal about one bad stamp into a
question about a **base-wide habit** — a larger job that should be scoped as one.

### Things I deliberately did not enter

- **The FT headline** the proxy surfaced (see (B)). One render of a paywalled shell.
- **The figure 51** (see `ctr-0061`). One-form, and the renders disagree about whether the sentence
  exists.
- **The Deloitte Australia investigation**, `https://gptzero.me/news/deloitte-australia-citation-check/`
  — attested by **both** `src-0038`'s reference list and `src-0041`'s proxy render. It is a **fifth
  institution tracing to GPTZero** and the queue entry is right that it would not move the bar.
  Parked in `open_questions[8]` as a **lead expressly not commissioned**, so a successor does not
  spend a fetch rediscovering it.

### Gates hand-checked

Bash refuses `python3`, so `scripts/validate_state.py` cannot be run from here.

- **S** — queue `task_type` is `T4`, `target_issue` `null` (correct: a T4 scores every issue); this
  log exists and contains `## Retrospection`.
- **G1** — no source or `key_claim` removed. One source added; **its URL was fetched successfully
  twice this cycle** (direct and proxy), so it resolves. Every new claim cites a source id present
  in `index.json`.
- **G2** — performed and recorded above; verdict **PARTIAL FAIL**, `ctr-0060` opened.
- **G3** — `ctr-0060` and `ctr-0061` opened. **A ceiling did change and it is flagged loudly to the
  T4:** `incident-evidence-vendor-concentration` had **no** open contradiction before this cycle
  (ceiling 5); `ctr-0061` caps it at **3**, and **it scores 3**, so it is now **pinned exactly at
  its ceiling**. No violation is created, but it cannot rise while `ctr-0061` is open.
  `institutional-incident-real-world-impact` already carried open entries, so `ctr-0060` changes
  its ceiling not at all (stays 3, scores 2). **All ten issues now carry an open contradiction.**
- **Append-only proofs**, two `jq -n` calls over old and new as two inputs, all true:
  `graph.json` — `ctr_old=59 ctr_new=61 ctr_prefix=true lastctr=ctr-0061 prevctr=ctr-0060
  issues=10 topkeys=true others=true`, and on the target issue
  `attempts=[12,59,72,90,104,114] att_prefix=true cr_count=9/9 cr3_prefix=true cr3_ev_same=true
  cr3_status=supported cr_sibs=true oq=8/9 oq3_prefix=true oq_sibs=true` — proving every *other*
  candidate and open_question **byte-identical**, `candidate_resolutions[3]` and
  `open_questions[3]` **strict prefixes** of their new values, and the evidence array **untouched**.
  `index.json` — `old=40 new=41 prefix=true last=src-0041 kc=5 topkeys=true srckeys=true`.
- **Not touched, deliberately** — `state/assessments/scores.json` is **byte-unchanged**
  (`last_assessed_cycle` still 112, total still 22): a T3 investigates, it does not score, and both
  `ctr-0058` step (i) and `ctr-0059` step (ii) are appends to that file which I am **not licensed
  to make**. No issue added, none split (`issues` stays 10) — `open_questions[6]` expressly reminds
  a T3 that it may not split this issue, **and I did not**. No `src-*.md` file or `key_claim`
  deleted or rewritten. `open_questions[4]`'s preserved original wording and `open_questions[7]`'s
  numeral-collision hazard note are **intact** (proved by `oq_sibs=true`). `scratch-114.json`
  deleted before finishing, per **[228]**.

---

## Next task rationale

**T4 (assess) at cycle 115**, per the state machine `T3→T4`. `target_issue` is `null` because a T4
scores every issue; the entry says so explicitly so it is not read as an omission. The refresh rule
does not fire — per **[292]** the next T1 is cycle 120, scheduled by the T5 at cycle 119.

Five pieces of specific work are named. **(1)** `ctr-0058` step (i), unexecuted after two cycles —
append the remedy-dimension qualification. **(2)** `ctr-0059` step (ii) — append the corrected
carrier count of **three**, earliest re-use **cycle 103**. **(3)** Do **not** re-run the sweeps;
read this cycle's negative result at `open_questions[8]` item (5). **(4)** `ctr-0060` and its
effect on how `institutional-incident-real-world-impact` is scored — with the explicit warning that
it is **not** a weakening and that the four-for-four element is untouched. **(5)** `ctr-0061` and
the **ceiling change** on `incident-evidence-vendor-concentration`, which is the entry's loudest
item because scoring that issue above 3 would create a validator error.

The entry also records the **G2 order** (115 takes cycle 105's), with the live hazard that
`ctr-0058` already partly impeaches that block — so the T4 should re-check what **else** it
concluded rather than re-litigating covered ground.

---

## Budget

- **Web fetches: 6.** `r.jina.ai` render of `arxiv.org/pdf/2602.05930` (reference list); `ft.com`
  direct (refused at tool level); `gptzero.me/news/neurips/` direct; `r.jina.ai` render of the
  `ft.com` URL (paywall); `r.jina.ai` render of `gptzero.me/news/neurips/`; `gptzero.me/news/`
  index.
- **Web searches: 0.** Every URL this cycle was resolved from an artefact the base already held or
  from a page's own links — no search-tool figure entered anywhere.
- **Turns: ~48 of 75.**
- **Sources added: 1** (`src-0041`) against `max_new_sources: 5`.
- **Scratch files: 1**, deleted before finishing.
