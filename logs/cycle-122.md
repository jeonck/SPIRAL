# Cycle 122 — T3 investigate

**Target issue:** `consistency-calibration-as-failure-mode` (issue index 2)
**Queue entry:** written by cycle 121, `attempt_count` 0.

---

## Phase check

Run before anything else, because cycles 70, 83 and 106 each aborted on a phase error and cost
the chain seven cycles.

- `state/queue/last_completed_task.txt` → **`T2 structure`**. Correct: T2→T3.
- `git show --stat HEAD` → **36a4324 `cycle 121: T2 structure`**, a real cycle commit (not the
  human commit the queue entry warned might be at HEAD). It touches `logs/cycle-121-transcript.txt`,
  `logs/cycle-121.md`, `state/issues/graph.json`, `state/meta.json` and both `state/queue` files —
  and **does not touch** `state/knowledge/index.json`, `state/assessments/scores.json` or any
  `src-*.md`. That is exactly the footprint of a T2.
- `index.json` → **`srcs=42 last=src-0042`**, as required, unchanged since cycle 120.
- `graph.json` → **`ctr=62 open=46 issues=10 oq=12 cr=15 oq6=8`**, matching the queue entry's
  required reading exactly (it was `ctr=61 open=45 oq=10 cr=14 oq6=7` for cycles 117–120).

All four checks agree. Phase confirmed as **T3**. `prompts/t3_investigate.md` was read at source
and is six steps; this cycle performs all six.

---

## Retrospection

**G2 target, per the standing round-2 order** ("cycle N re-checks the conclusion the next
unexamined predecessor entered"; 116→107, 117→108, 118→109, 119→110, 120→111, 121→112):
**cycle 113**.

**What cycle 113 entered.** `git show --stat ed84580` confirms cycle 113 was a **T5 select** whose
only state write was **+7 lines to `state/issues/graph.json`**. `logs/cycle-113.md` and the diff
agree: the conclusion it entered is **`ctr-0059`**, on `institutional-incident-real-world-impact`,
`opened_cycle` 113, `resolved_cycle` null — a *scope defect in `ctr-0058`'s own repair plan*. Its
selection and ranking table are a second entered conclusion but were themselves the product of a
zero-fetch re-derivation and are not the substantive claim; I re-checked the substantive one.

**Result: PASSES. Every independently checkable claim in `ctr-0059` re-verifies. Zero fetches.**

1. **`ctr-0058` step (ii)'s scoping.** `ctr-0059` charges that step (ii) commissions a sweep of
   "the OTHER nine issues" for "any scored rationale asserting that a candidate is the ONLY one
   addressing some question, where the check actually run was over evidence arrays rather than
   candidate text". Read at source in `graph.json`: **verbatim, both clauses.** So the sweep is
   scoped to `scores.json` and to issues *other than* issue 7.
2. **That cycle 103's instance falls outside that scope.** `grep -n` on `logs/cycle-113.md`'s
   predecessor `logs/cycle-103.md` finds it at **line 218**: "`candidate_resolutions[3]` is the
   only candidate answering the title's second conjunct — *how institutions respond when caught*".
   It is (a) in a **log**, not a scored rationale, and (b) on **issue 7 itself**. A sweep executed
   exactly as `ctr-0058` writes it therefore **cannot find it**. Confirmed.
3. **The "I verified all of that at source" stamp.** Present at `logs/cycle-103.md` line 220,
   two lines after the clause, over a four-part conjunction. Confirmed as described.
4. **`candidate_resolutions[5]` of issue 7.** `ctr-0059` says: entered cycle 59, status supported,
   evidence exactly `[src-0022]`, and its text says under *WHY THIS BELONGS TO THIS ISSUE* that the
   issue "asks whether AI-generated CTI failures have reached production at real institutions and
   how institutions respond when caught" and that curl is "a real institution responding to
   AI-generated security reporting by CLOSING THE INTAKE PROCESS ENTIRELY". Read at source:
   `status=supported`, `ev=src-0022`, summary opens **"ADDED CYCLE 59"**, and the belongs-clause
   reproduces **verbatim**. All four confirmed.

**One bookkeeping wrinkle, reported rather than smoothed over.** `ctr-0059` states
`candidate_resolutions[5]` was "entered cycle 59". There is **no `entered_cycle` field** in this
schema — `keys` on that object returns exactly `evidence,status,summary`. The cycle-59 provenance
is carried **inside the summary text** ("ADDED CYCLE 59"), which is where I found it. This is not
an error in `ctr-0059`; it is a note for any successor who tries to re-derive candidate provenance
from a field and finds none. Provenance for candidates lives in prose in this base.

**Nothing here is a supersession and nothing failed.** `ctr-0059` remains open and correctly so.

---

## Task performed

The queue entry directed the cycle at **`ctr-0062` repair steps (i) and (ii)** — the highest-value
cheap action available on this issue — and that is what was done, in full, plus one optional
search angle. All six steps of `prompts/t3_investigate.md` were executed.

### Step (i) — re-pull `src-0007`'s before/after rows at source. **CONFIRMED EXACT.**

`src-0007` is CyberThreat-Eval, arXiv 2603.09452. Table 4 was re-pulled from
`https://arxiv.org/html/2603.09452v1` on **two renders** — the direct arXiv HTML body render and
the same URL through the `r.jina.ai` proxy — with the values **asked for without being supplied**,
per the standing query discipline. Both renders returned the caption, the printed column order
(GPT-4o, o3-mini, GPT-4o (FT), GPT-4o-mini (FT)) and every requested cell, and **both agree cell
for cell with the transcription stored in `state/knowledge/src-0007.md` since cycle 15**.

All six values in `ctr-0062` side (b) confirm exact:

| Task | Metric | GPT-4o | GPT-4o (FT) | Direction |
|---|---|---|---|---|
| IoC Extraction | Precision | 0.8240 | 0.8846 | **helps** (+0.0606) |
| MITRE ATT&CK TTP Identification | Precision | 0.2787 | 0.2387 | **hurts** (−0.0400) |
| Priority Scoring (Article) | Precision (Accepted) | 0.3037 | 0.2717 | **hurts** (−0.0320) |

with Article Recall (Accepted) 1.0000 → 0.9798, inside the stored 0.9798–1.0000 band. Four further
rows were pulled unprompted as controls and also confirm (TTP Recall; Article Recall; Description
Precision and Recall). **This is the fourth independent pull of these rows, across cycles 15, 21,
30 and 122.**

### Step (ii) — does the carry-forward [19] column anomaly reach these rows? **NO.**

This was the step that made (i) worth doing rather than optional, and it is the cycle's main
result. [19] is defined as GPT-4o (FT) tracking o3-mini **to within 0.001**. Compared row by row
on both renders:

| Row | GPT-4o (FT) | o3-mini | gap |
|---|---|---|---|
| IoC Precision | 0.8846 | 0.8503 | 0.0343 |
| TTP Precision | 0.2387 | 0.3480 | 0.1093 |
| TTP Recall | 0.1846 | 0.1759 | 0.0087 |
| Article Precision (Accepted) | 0.2717 | 0.3802 | 0.1085 |
| Article Recall (Accepted) | 0.9798 | 0.9293 | 0.0505 |
| Description Precision (Accepted) | 0.3392 | 0.3982 | 0.0590 |
| Description Recall (Accepted) | 0.9798 | 0.9091 | 0.0707 |

Smallest gap **0.0087 — eight times the threshold**; largest **0.1093 — over a hundred times it**.

**A second and stronger discriminant, independent of any gap size:** GPT-4o (FT) sits **above**
o3-mini on IoC Precision and **below** it on all six other rows, so **the two columns cross**. A
duplication or column-shift artefact of the kind [19] would have to be **cannot reverse sign**
between rows of the same table. The three before/after pairs are not suspect on that ground.

### A third finding, not asked for, which narrows [19] itself

No cycle had ever checked the **other** rubric block. On **Content: Root Cause** the same two
columns separate on all six rows — 4.533/4.733, 4.371/4.627, 4.286/4.576, 4.629/4.818,
4.586/4.800, 4.323/4.418 — gaps **0.095 to 0.290**, every one far outside 0.001, on both renders.

**[19] is therefore not a column-wide effect at all: it is six cells — the Content: Threat Actor
block — and nothing else in Table 4.** The **cause remains unknown and is still not to be
guessed**, but its blast radius is now measured rather than assumed. Carry-forward [25] survives
intact: the rubric's absolute level is still uninterpretable and only within-table contrasts may
be cited.

### Where that leaves `ctr-0062`: **ADVANCED, not resolved.**

On the substance it resolves **toward side (b)**. `src-0007` is a CTI source, has been in this
base since cycle 9, and does report a within-study before-and-after fine-tuning accuracy contrast
on the same model. The three consequences the entry drew from (b) all follow. **`src-0042` is not
impeached** — it remains genuine out-of-domain corroboration, merely not the first one and not an
out-of-domain-only one, and cycle 120's block at `open_questions[3]` is neither erased nor edited.

`resolved_cycle` **stays `null`** because the entry's own repair plan has four steps and steps
**(iii)** (append the correction to `open_questions[3]` without editing cycle 120's block) and
**(iv)** (the cross-issue reach into `ioc-extraction-reliability` and `ttp-attack-mapping-reliability`)
are **reserved to a T2 by the entry's own text**. A T3 investigates; it does not restructure. I
declined to take a T2's decision rather than take it quietly.

Per (iv)'s own warning: **`ctr-0001` is a cross-study *magnitude* conflict** between `src-0003` and
`src-0007` and neither impeaches nor is impeached by anything here, because a within-study
before/after contrast drawn from one table is a different instrument. Those were kept apart.

### `candidate_resolutions[14]` — **promoted `proposed` → `supported`**

Cycle 121 wrote an explicit **promotion test** into this candidate: *"a T3 may move this to
supported by discharging `ctr-0062` repair steps (i) and (ii) in favour of part (2) — re-pulling
`src-0007`'s three GPT-4o versus GPT-4o (FT) precision pairs on two renders AND establishing that
the [19] anomaly does not touch those rows."* That test was met in full and **nothing beyond it
was discharged**. Objections **(iii), (iv) and (v) are untouched and still bite** — all three are
about `src-0042`, not `src-0007`: no cycle has re-verified any `src-0042` number at source; it is a
single preprint with no stated venue and a cross-render author-list disagreement; and it states no
confidence elicitation method. Both scope limits survive unchanged.

**The promotion must not be over-read, and I have written that into the candidate itself.** The
issue title begins *"Beyond raw accuracy"* and its two conjuncts are **consistency** and
**calibration**; `[14]` is about **task accuracy** and bears on neither. A supported candidate that
does not answer the title does not lift the issue. The two-source bar is met **in domain** by
`src-0001` (origin finding) + `src-0007` (within-study direction instability) — two independent
teams, different corpora — with `src-0042` counted as out-of-CTI corroboration and nothing more.

### `open_questions[11]` — the calibration leg. **One angle spent. Negative.**

The genuinely untried axis: does a fine-tuning **before/after proper score** exist anywhere, in any
domain? One web search, values not supplied. **The two most relevant returns were already in this
base** — arXiv 2606.20502 is `src-0042`, arXiv 2606.31159 is `src-0013` — both checked against
`index.json` rather than assumed. That `src-0042` is the single best unprompted match on exactly
this axis is **itself informative**: this base has already established on two renders that it
reports no ECE, no Brier and no reliability diagram, and that its title word *Calibration* is used
in a decision-threshold sense.

Three **unverified leads** were recorded as leads and not as claims, none fetched, none in
`index.json`, none entered as a source (a T3 adds no source): `eprint.iacr.org/2025/546`,
arXiv 2406.05892, arXiv 2412.15004. **Nothing whatever is asserted about whether any of them
reports a proper score at all.** The attested absence on this question is now **one** angle deep,
not eleven — `open_questions[5]`'s eleven angles were spent on a different question and that count
must not be borrowed.

### G3

**No new contradiction was opened this cycle.** The step-(ii) finding *narrows* carry-forward [19]
rather than conflicting with it — `src-0007.md`'s note 3 asserts the anomaly on Content: Threat
Actor and never asserted it elsewhere, so establishing that Root Cause is clean strengthens the
record rather than contradicting it. Nothing surfaced that conflicts with another issue's supported
resolutions. Opening an entry here would have been noise.

---

## Changes made

**One file written: `state/issues/graph.json`.** Six edits, all in a **single two-input `jq -n`
splice** ([237], nineteenth cycle running):

1. **`ctr-0062`** — cycle-122 discharge block **appended** to `.description`. `resolved_cycle`
   deliberately left `null`.
2. **`issues[2].candidate_resolutions[14].status`** — `proposed` → **`supported`**.
3. **`issues[2].candidate_resolutions[14].summary`** — cycle-122 promotion block **appended**.
4. **`issues[2].open_questions[11]`** — cycle-122 negative-angle block **appended**.
5. **`issues[2].open_questions[12]`** — **new**, recording what the `ctr-0062` work left open.
6. **`issues[2].attempts`** — `122` appended (now `3,15,16,25,39,62,75,80,87,96,111,122`).

**Counts after:** `ctr=62 open=46 issues=10 oq=13 cr=15 sup=12 att=12 c62res=null`.

**Append-only proof — thirteen clauses in one `jq -n` over old and new, all `true`:** issues length
equal; `._schema` equal; top-level `keys` equal; `contradictions[0:61]` equal; the edited
contradiction's description `startswith` the old **and** strictly longer; `open_questions` equal
after `del()` of the edited index on both sides (new sliced to old length first); the edited
`open_question` `startswith` the old; `candidate_resolutions` equal after `del()` of index 14 on
both sides; `[14].summary` `startswith` the old; **`[14].evidence` unchanged** (a status change
must not silently move evidence); `.issues` equal after `del()` of index 2; `attempts[0:11]` equal
to the old array. `jq -e .` clean.

**Not touched:** `state/knowledge/index.json` (`srcs=42`), every `src-*.md`,
`state/assessments/scores.json`, `open_questions[3]` (reserved to a T2 by `ctr-0062` step (iii)),
and all nine other issues. **A T3 adds no source and writes no score.**

**A deliberate omission, flagged rather than done quietly.** The fourth at-source pull of Table 4
and the Content: Root Cause finding would naturally belong in `state/knowledge/src-0007.md`, as
cycle 30's re-pull does. I did **not** append them there: a T3's footprint should not include
knowledge writes, and the next cycle's phase check asserts `src-*.md` are untouched. The full
working is in `ctr-0062`'s cycle-122 block, and **a later T1 or T2 may mirror it into
`src-0007.md`** — that is recorded as carry-forward [333].

**Queue files:** `next_task.json` rewritten for a T4 at cycle 123 (`attempt_count` 0);
`last_completed_task.txt` → `T3 investigate`. One scratch file (`scratch-122.json`), **deleted**.

**Gates hand-checked, not validator-run** — Bash refuses `python3`, so `scripts/validate_state.py`
could not be executed and I am not implying otherwise. **G1:** no source added; every new factual
claim cites `src-0007`, `src-0001` or `src-0042`, all present in `index.json`, and the three search
hits are recorded as **leads, not claims**. **G2:** performed above; it passed, at zero fetches.
**G3:** nothing was silently sided with — `ctr-0062` is advanced with both sides intact and its
`resolved_cycle` left open.

**Citation bans observed.** None of `src-0007`'s banned strings were used: `P = 0.8810`, the 0.9344
inference and the 0.09–0.15 crossover are absent, and `GPT-4o-mini (FT) 0.6944` is **not** read as
a before/after pair (no vanilla GPT-4o-mini row exists). **One ban boundary is flagged rather than
buried:** `ctr-0038` bans the **0.1414–0.2270 recall range**, a derived across-model span. The TTP
Recall row is quoted above as four separate as-printed cells in column order and used only for a
within-row column gap. I judge that a different object from the banned span construct, but I have
recorded the judgement in `ctr-0062` itself so a successor can overrule it — and if overruled, the
0.0087 gap simply drops and the other six gaps are individually decisive.

---

## Next task rationale

**T4 (assess) at cycle 123**, per the state machine edge T3→T4, on the same issue.

The entry leads with the **one arithmetic trap** that could waste that cycle. `supported` moved
11 → 12 this cycle, and a T4 that reads a rising supported-count as the issue advancing toward its
own resolution would score wrongly. `[14]` is about task accuracy; the title's conjuncts are
consistency and calibration. The entry says this in terms, quotes `[14]`'s own instruction that a
later T4 must not score it as if it lifted the issue, and gives the offset-slice `jq` to read both
scope limits at source rather than trust the summary.

It restates the **ceiling arithmetic** explicitly — `scale_max` 5 minus `g3_contradiction_demotion`
2 means an issue with an open contradiction cannot exceed 3, this one carries eight, and all ten
issues carry at least one — so that a 2 is not misread as thinness on the graph's richest issue by
candidate count. It names **what actually holds the score down** (the primary candidate still rests
on `src-0001` alone, unchanged by this cycle) and points at `open_questions[12]`, new and short,
which is the honest content of any "what would move this" paragraph.

It warns against **adding or swapping the 1-angle and 11-angle counts** on `open_questions[11]` and
`[5]`, which are different questions. It fixes the next G2 target at **cycle 114** and repeats that
zero fetches is the standard, now met by 121 and 122 both. And it carries the **thirteenth
consecutive** honesty item: `automated-triage-under-refusal` has still never been investigated by a
T3 in 122 cycles, its `attempts` array is still empty, the queue targets a different issue, and it
needs a human ruling — with an instruction to keep repeating it until one arrives.

**Size, stated plainly:** the entry is **16.9 KB**, over the ~15 KB of [238] by about 13%. I cut
the fetch notes and the off-issue ban restatements to their pointers (both are reproduced in full
in this log's carry-forward) rather than cut the trap warning, the ceiling arithmetic or the
`src-0007` bans, which are the parts that bite on this issue. Cycle 113 made the same trade at
15.6 KB.
...
- **Fetches: 2** (both `src-0007` Table 4, one direct arXiv HTML body render, one via `r.jina.ai`).
- **Web searches: 1** (`open_questions[11]`, angle 1, negative).
- **G2 fetches: 0** — fully re-derived from state and git, the standard set by cycles 121 and 122.
- **Bash calls: ~24**; Read: 4; Grep: 0 (the stored Table 4 was reachable by `grep -n` + `sed -n`);
  Write: 3; Edit: 5.
- **Turns: well inside the 75 cap.** No step was cut for budget.
- **Cost centre worth noting:** the two fetches were one turn and returned the complete table on
  both families. The expensive part of this cycle was *reading the state* — `ctr-0062`,
  `candidate_resolutions[14]` and `open_questions[11]` total ~17 KB of prose that had to be read in
  offset slices before a single fetch was justified. That order was correct and should be kept.
