# Cycle 128 — T3 Investigate — `institutional-incident-real-world-impact`

## Phase check (run first, before anything else)

`state/queue/last_completed_task.txt` read `T5 select`. `git show --stat HEAD` → `ec734db`
"cycle 127: T5 select", touching `state/meta.json`, both `state/queue` files,
`logs/cycle-127.md` and `state/issues/graph.json` — the expected set. All four confirming
checks reproduced exactly:

| check | required | observed |
|---|---|---|
| `graph.json` | `ctr=63 open=47 issues=10 last=ctr-0063` | identical |
| `scores.json` | `last=126 n=10 total=22` | identical |
| `index.json` | `srcs=43 last=src-0043` | identical |
| commit scope | no source, no score, one contradiction | confirmed |

Phase confirmed as **T3**. `prompts/t3_investigate.md` read at source.

---

## Retrospection

**G2 target: cycle 119** — the next unexamined predecessor in the round-2 order (116 took 107's,
117–124 took 108's–115's, 125 took 116's, 126 took 117's, 127 took 118's).

**Zero fetches.** Cycle 119 was a T5 whose commit (`b4dd32e`) touched no `graph.json`, no
`scores.json` and no `index.json`. Per the cycle-127 precedent under **[337]** — *G2 targets what
was entered into the STATE, not what the log said* — the conclusion re-checked is the one thing
cycle 119 entered: **`state/queue/next_task.json`, a T1 (Collect) for cycle 120 targeting
`consistency-calibration-as-failure-mode`**, and the derivation behind it.

Every input re-derived from the cycle-119 snapshot via `git show b4dd32e:<path> | jq`:

| claim entered | re-derivation | result |
|---|---|---|
| refresh rule fires | `collect_refresh_every: 7` at source; 7 × 17 = 119, so 119 % 7 == 0 | **holds** |
| eight issues tied at 2, two at 3, total 22 | scores at `b4dd32e` | **holds** |
| 3(a) survivors have empty `depends_on` | `ioc`, `consistency`, `institutional`, `automated-triage` — exactly those four at score 2 | **holds** |
| 3(b) window 114–118 penalises `ioc` (117) and `institutional` (114), not `consistency` (111) | `attempts` arrays at `b4dd32e` | **holds** |
| 3(c) decides on `created_cycle` 2 v 16 | `consistency` 2, `automated-triage` 16 | **holds** |
| derived open-contradiction column sums to 45 | `open=45` at that snapshot | **holds** |
| `consistency` had 14 candidates, 11 supported | 14 / 11 at `b4dd32e` | **holds** |

**Verdict: RE-VERIFIED IN FULL. No defect found, no contradiction opened.** The selection was
invariant across both 3(a) readings, both 3(b) window readings, and the order of application, as
cycle 119 claimed — and I checked that rather than accepting it. I also confirmed the per-issue
open-contradiction split still reconciles today (3+4+5+9+2+1+11+3+3+6 = 47 = `open`), with the
+2 since cycle 119 being `ctr-0062` and `ctr-0063`, both on `consistency-calibration`, exactly as
the intervening logs record.

One item deliberately **not** scored as a failure: cycle 119's log contains the phrase *"eleven
supported candidates resting on **one** calibration source, `src-0001`"*, the same shape cycle 127
found false-in-log-but-true-in-state for cycle 118. It appears **only in the log**, not in the
queue entry's operative text, so under [337] it is outside the G2 target. Flagged here rather than
...
**The commission was run, and both limbs returned.** `ctr-0060` step (ii) and `open_questions[8]`
item (1) had asked four consecutive cycles to *directly observe whether PwC Middle East's four
reports remain published on PwC's own sites, and record the observation with its date*. It was
still untried at cycle 128. **I ran it on 2026-08-07.**

### PwC limb — all four reports REMAIN PUBLISHED

Observed at `pwc.com/m1` via the `r.jina.ai` render (direct fetches 403 — see the hazard below).
Each is a live publication page; **withdrawal / retraction / correction / editor's-note notices are
ABSENT at all four**:

| report (named by `src-0037`) | page | date shown |
|---|---|---|
| Building a cyber-resilient eMobility ecosystem | `/publications/building-cyber-resilient-emobility-ecosystem.html` | 11 Feb 2025 |
| Citizen Pulse: Delivering real-time insights… | `/publications/citizen-pulse-delivering-real-time-insights-to-meet-societys-evolving-needs.html` | 27/02/25 |
| Shaping GCC mobility landscape – electric, connected and autonomous | `/publications/shaping-gcc-mobility-landscape.html` | 17/02/26 |
| Agentic AI – the new frontier in GenAI | `/publications/agentic-ai-the-new-frontier-in-genai.html` | — |

**Consequence.** The **unverified retention limb** `ctr-0060` opened at cycle 114 is now
**verified, in the direction the classification already assumed**. PwC's placement in ENISA's
retain-and-correct remedy class no longer rests on PwC's description of its own activity alone, so
`open_questions[3]` **predictor (a)'s rejection moves from PROVISIONAL-PENDING-ONE-CHECK back to
ESTABLISHED — for the remedy dimension only.** It remains untested and unrejected on the
cause-declaration dimension, where the 3-decline / 1-volunteer split still lines up with sector
perfectly. Cycle 114 was **right** to open the entry: an unverified limb is a defect even when the
limb turns out true, and finding that out is what the check was for.

### KPMG limb — the removal is STILL PARTIAL, 56 days on

`src-0035` (The Register, 12 June 2026) records KPMG as having removed the report *"from some of
its websites while it investigates how the publication made it into the wild."* On **2026-08-07**
the disputed report's **executive summary** — cover-titled *"Total Experience: Redefining
excellence in the age of agentic AI / Global Customer Experience Excellence 2025–2026 report"* —
**remained retrievable from KPMG's own asset host** `assets.kpmg.com` (Peru path). Identity is
established **on the cover title itself**, not inferred from the filename. Two-render confirmed:
direct fetch returned the PDF binary (183.9 KB), the proxy returned the cover text.

Three other KPMG-owned addresses returned **KPMG's own** not-found pages on the same date — the
Colombia path (*"the asset that you're looking for is no longer available"*), the China path
(*"Error: File not found"*) and the Belgium landing page (*"Error 404"*). All were read through the
**same proxy that rendered the live copy**, so these are KPMG's error pages, not a retrieval
failure at my end. **`src-0035`'s word "some" is still accurate**, and the KPMG remedy has **not**
converged on EY Canada's full withdrawal.

**Why this matters beyond the two data points:** two of the four remedies were **unfinished
processes** when recorded (KPMG's *"while it investigates"*, PwC's *"is updating"*). Both have now
been re-observed at a later date **still in the same class**. The four-way remedy spread is
therefore **not an artefact of having sampled mid-process** — which strengthens the classification
the whole of `open_questions[3]` is built on.

**And it is the first non-GPTZero evidence this issue has ever held.** Every prior artefact behind
the EY, KPMG and PwC cells traces to that one vendor. These are direct readings of the
institutions' own publishing state. **But the limit is stated plainly everywhere I entered it: this
widens the evidence base on the REMEDY dimension only. On CITATION ACCURACY the issue still rests
entirely on one vendor's sweep, and `src-0044` does not change that by one inch.**

### What I did NOT establish, said plainly

- **Retention is not correction.** *"is updating"* is an activity claim; observing that the reports
  are still up is **not** observing that anything in them changed. Unobserved.
- Whether KPMG ever **named a cause**: still unobserved.
- **No cause is inferred from either observation**, per the discipline cycle 114 attached.
- Predictors (b) and (c) were **not tested** — still confounded and unusable respectively.
- `open_questions[0]`, the base-rate question and this issue's centre of gravity, is **untouched**.
  N on the response question is **still 4**.
- I did **not** split the issue (`open_questions[6]` is a T2's), did **not** re-correct
  `open_questions[4]`, and did **not** reach `ctr-0037` site (2) — the commission and its write-up
  consumed the cycle. Saying so rather than implying coverage I did not achieve.

### G3 — `ctr-0064` opened

Working the KPMG limb surfaced a real conflict the base has carried since cycle 99 without
recording it. `src-0035` renders the remedy **with** the partitive (*"some of its websites"*);
`src-0036` renders the same remedy **without** it, which read alone states a **complete** removal.
My direct observation **agrees with `src-0035` and is inconsistent with `src-0036` read as
complete**. Opened as `ctr-0064` rather than silently picking a side.

It does **not** impeach `src-0036` (a compression is not a fabrication), does **not** touch the one
KPMG sentence confirmed character-identical across both outlets, and **moves no score** — this
issue already carried open contradictions, so its ceiling was already `scale_max − demotion = 3`.
`src-0036` is banned outright from quotation, so the entry deliberately **describes** its rendering
rather than quoting it, and says so in step (iii) so a successor does not read that as an admission
it went unchecked.
...
- **`state/issues/graph.json`** — one two-input splice, `[237]`:
  - `institutional-incident-real-world-impact`: `.attempts` `[12,59,72,90,104,114]` →
    `[12,59,72,90,104,114,128]`; **appends** to `open_questions[3]` (the cycle-128 predictor-(a)
    update), `open_questions[8]` (route-ledger update: item (1) **discharged with a result**, plus
    the new `pwc.com` G1 hazard and the re-stated cheapest open work), and
    `candidate_resolutions[3].summary`; `src-0044` added to that candidate's `evidence`.
  - `ctr-0060`: **step (ii) marked DISCHARGED with a positive result.** `resolved_cycle` **stays
    `null`** — step (iii) is a standing methodological rule, reserved to a T4/T2, and **a T3 may not
    adopt one.**
  - `ctr-0064` appended (G3, above). `ctr` 63 → 64, `open` 47 → 48.
- **`state/knowledge/index.json` + new `state/knowledge/src-0044.md`** — one source added of the
  five permitted, per `t3_investigate.md` step 2. `srcs` 43 → 44, first growth since cycle 121.
- **`state/queue/next_task.json`** — T4 (assess), `institutional-incident-real-world-impact`,
  `attempt_count` 0. **`state/queue/last_completed_task.txt`** — `T3 investigate`.
- **`state/meta.json`** — cycle counter (written by `run_cycle.sh`).

**No score touched, no issue restructured, no existing `src-*.md` or `key_claim` altered.**
Append-only was **proved before each `mv`**, not asserted: for `graph.json`, six conjuncts
including `startswith` checks that each edited string strictly extends its predecessor (the
correct instrument for an append-to-existing-string edit, stronger than array equality); for
`index.json`, five conjuncts including `sources[0:43]` equality. Post-write hand-checks: dangling
evidence ids **0**, `ctr=64 open=48 issues=10 last=ctr-0064`, `srcs=44 last=src-0044`.

**Disclosure: I could not run `scripts/validate_state.py`** — Bash refuses `python3` in this
environment. The gates above were **hand-checked with `jq`**. I am not claiming a validator run.

**Deviation disclosed:** the T4 entry is **~21 KB against the ~15 KB guideline in [238]**. The
overage is the G1 hazard, the ban set and the eight carried work items. **[238]'s budget and the
accumulated do-not-repeat list remain in genuine conflict** (first flagged at cycle 119) and the
list only grows.
...
**T4 (Assess), `institutional-incident-real-world-impact`.** The state machine's edge is T3→T4;
128 % 7 ≠ 0 so no refresh intervenes. The target is the issue that moved, and it moved in a way a
score must respond to: a four-cycle-old named check discharged, an unverified limb inside a
supported candidate verified, and the issue's first non-GPTZero artefact acquired.

I wrote the entry so it **does not lean**. Both the case for 3 and the case against are stated at
equal strength, because the judgement is the T4's and a T3 that pre-decided it would be doing the
next cycle's work. The entry also hands the T4 three things only it can do: `ctr-0063` step (i)
(an append to `scores.json`, waiting since cycle 127), `ctr-0060` step (iii), and `ctr-0064`
step (ii).
...
- **Web searches: 5. Web fetches: 11** (four PwC landing pages, one PwC PDF, five KPMG addresses,
  one identity re-check). The G2 was **zero-fetch**, matching the cycles-121–127 standard; the
  fetches all went to the commission, which is what cycle 127 told this cycle to spend them on.
- **Sources added: 1 of `max_new_sources` 5.** Deliberately restrained — see the hazard below.
- Bash calls: 24 (`jq`/`git`/`grep`/`sed`/`mv`/`rm`). One refused (`curl`).
- Read/Write/Edit: 9. Turns: roughly 55 of 75.
- One self-inflicted cost, recorded so a successor avoids it: **I broke my own scratch JSON with a
  single pair of double quotes** inside a string body, violating [211]/[228], and spent a turn
  repairing it. Validate the scratch with `jq -r keys` **before** the splice.
