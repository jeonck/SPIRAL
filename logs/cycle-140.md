...
Topic: `cti-ai-reliability`. Queue entry: `state/queue/next_task.json` written by cycle 139.
Task type T4, `target_issue` null (deliberate — a T4 scores all ten issues and selects nothing).

## Phase check (run before anything else)

Five confirming checks, all agreed, so I did not re-derive my phase from anything but the state:

| check | expected | observed |
|---|---|---|
| `state/queue/last_completed_task.txt` | `T3 investigate` | `T3 investigate` |
| `git show --stat HEAD` | cycle 139 commit, `state/` files only | `ed35672 cycle 139: T3 investigate` — meta.json, both queue files, logs/cycle-139.md, graph.json, index.json, **src-0048.md** (new), plus the harness transcript |
| `scores.json` summary | `last=137 n=10 total=22 max=3` | `last=137 n=10 total=22 max=3` |
| `graph.json` summary | `ctr=69 open=51 issues=10 last=ctr-0069` | identical |
| `index.json` summary | `srcs=48 last=src-0048` | identical |

`state/assessments/scores.json` was untouched by cycle 139, as a T3 requires. HEAD was a cycle
commit, not a human commit, so no walk-back was needed.

## Task performed

Scored all ten issues in `state/assessments/scores.json`, set `assessed_cycle` 140 on every
entry and `last_assessed_cycle` 140, and ruled on `ctr-0069` step (7)(i)–(ii), which that entry
addresses to a T4 or a T2.

**The vector does not move. Total 22, max 3, for the sixth consecutive T4 since the cycle-118
freeze.** `ttp-attack-mapping-reliability` 3, `incident-evidence-vendor-concentration` 3, the
other eight 2. I did not hold it by default: I re-derived every one of the ten merits from the
candidate `evidence` arrays in `state/issues/graph.json` against the ban set, and concurred with
cycle 137 on all ten for reasons I re-state per issue in the file. Only one issue had any new
input at all — cycle 138 was a T5 and entered no evidence, and cycle 139's `src-0048` bears on
`institutional-incident-real-world-impact` and on nothing else.

### G3 demotion, re-derived not inherited

One `jq` call over the graph gave the open-contradiction partition:
...
institutional-incident-real-world-impact=13  ioc-extraction-reliability=4
task-dependent-reliability-framing=4  ttp-attack-mapping-reliability=6
```

The ten counts sum to **51**, which equals the graph-wide open count, so the partition is
complete and no open contradiction is unattributed. Every issue carries at least one, so the
demotion fires on all ten and the ceiling is `scale_max - g3_contradiction_demotion = 5 - 2 = 3`
everywhere. The only movement since cycle 137 was `ctr-0069` (cycle 139), taking the graph-wide
total from 50 to 51 and `institutional-incident-real-world-impact` from twelve to thirteen. **No
issue changed demotion state.**

### Carry-forward [4] narrowed: I read the validator

Cycles 132–137 each recorded the subtraction-versus-ceiling policy conflict as unresolved, each
describing the ceiling as "the reading the validator will accept" — an inference, never checked.
**I read `scripts/validate_state.py` at source.** No cycle had, in 140 cycles.

- Its docstring: `G3. Contradiction gate: issues with open contradictions are not scored above
  the demotion ceiling implied by config.`
- Its implementation, under the comment `G3: contradiction demotion ceiling`, errors only when
  an entry's score is **greater than** `scale_max - demotion`.

So the code implements an **upper bound and nothing else**. I state the consequence exactly and
do not overclaim it: the validator **rejects** anything above 3 and **accepts** anything at or
below 3, so it *permits* the ceiling reading but does **not compel** it — a literal subtraction,
putting this vector at two 1s and eight 0s, would also pass. **The validator does not settle the
conflict.** `prompts/t4_assess.md` step 3 still says an issue "loses" the points.

I hold the ceiling reading, and this cycle with a reason beyond precedent: under a literal
subtraction the eight tier-2 issues would score 0, and rubric level 0 is *defined* as
"unexamined: no candidate resolutions", which is false of every one of them. A subtraction
assigns rubric levels whose own definitions are counterfactual, and it would flatten the
weakest-link selector by collapsing eight issues to an identical 0.

**The residual question for a human is now narrower**: not "which rule does the system
implement" — the code is a ceiling and is now read and quoted — but "should
`prompts/t4_assess.md` step 3 be reworded to match the code that validates it". I recommend the
rewording and did not make it; `prompts/` is not this loop's to edit.

I could **not run** the validator — bash refuses `python3` here — so every gate check this cycle
was hand-run with `jq`. I say that rather than implying a validator run.

### New honesty item (G): the g3 gate is currently inert

Under the ceiling reading the demotion only binds an issue that would otherwise score 4 or 5,
and nothing in this graph does. **So the demotion changed no score this cycle — including on
`institutional-incident-real-world-impact`, which carries thirteen open contradictions, a
quarter of all 51.** A gate inert across the entire vector is worth a human's attention, and
that observation is independent of the wording question above.

### Ruling on ctr-0069 (7)(i) and (7)(ii)

I read `ctr-0069` in full at source (all 5159 characters) before ruling, and I **ruled** rather
than declined. **Ruling: ADMIT, into a separately labelled stratum, and DO NOT PROMOTE.**

Reasons, in weight order:

1. **The criterion that would exclude `src-0048` is not the criterion that makes it evidentially
   different.** The proposed exclusion is "non-report artefact from a non-publishing
   profession". But this issue's population already contains three institutions the base itself
   records as consultancies and not CTI publishers proper, whose admission
   `candidate_resolutions[3]` calls a large dilution. "Publisher" is not doing principled work at
   that boundary — it has already been crossed for three of the four members. What genuinely
   distinguishes `src-0048` is **forum**: a filing before a judge carries a duty of candour and
   an adversary with a direct incentive to verify; a thought-leadership PDF carries neither.
   That is `ctr-0069` paragraph (5)'s own point. Excluding for a reason that is not the real one
   is the wrong move.
2. **Exclusion is the *less* stingy ruling, not the more stingy one.** If `src-0048` is out,
   side A's sector-alignment sentence stands unamended on four data points with only its old
   cycle-100 confound flagged. If it is in as a distinct stratum, that sentence must be
   corrected by appended text saying the alignment is observed **within the voluntary-publication
   stratum only and is untested outside it**. Admission *weakens* the standing generalisation,
   which is the direction the stinginess instruction points.
3. **Nothing in side A is impeached.** EY, KPMG and PwC did decline; ENISA did volunteer. It is
   side A's sector *generalisation* that is at issue, not its instances.

On **(7)(ii): yes** — the volunteer/decline axis gains a third value, **control-failure
attribution** (policies not followed, internal secondary review did not catch it), strictly more
than EY/KPMG/PwC withhold and strictly less than ENISA's stated cause. This limb is
**independent of the scope question**, exactly as paragraph (4) says: the binary does not fit
`src-0048` even before membership is settled.

**What the ruling does not do**, stated so nothing is over-read:

- It does **not** promote `candidate_resolutions[9]` to `supported`; I ruled against that limb
  explicitly. Admission to the population and promotion of the candidate are separate bars, and
  the second is independently unmet — `src-0048`'s two carrier outlets are **one Reuters wire**,
  this base holds **no primary artefact**, and the quotation/paraphrase boundary on its two
  admission fragments was unresolvable from cycle 139's renders.
- It does **not** license restating the tally as 3 decline / 2, anywhere.
- It does **not** report the commercial-versus-Big-Four confound as broken. A **forum confound is
  opened in its place** and no source in this base tests forum.
- **It is not executed.** The remedy is an edit to `state/issues/graph.json`, and a T4 does not
  touch that file. It is owed to a T2, or to a T3 on that issue. Until then
  `candidate_resolutions[9]` still reads `proposed` and side A still reads unamended — a
  successor must check the graph, not assume the ruling has landed. `ctr-0069` remains **open**;
  its (7)(iii) and (7)(iv) are untouched by me.

### The question my queue entry put to me about `automated-triage-under-refusal`

It asked whether a T4 could move that issue's score, noted this would change the selector's
arithmetic, and required me to say explicitly whether I scored on the evidence or to engineer an
outcome. **I scored on the evidence and the score does not move.**
`candidate_resolutions[1]` (cycle 135, from `src-0047`) is status **`proposed`**, and rubric
level 2 requires a candidate with single-source *support*; a proposed candidate is not support.
Its content does not carry it either — `src-0047`'s figures are under a hold with no cycle
having seen the table behind them, its two recall figures are *relative* improvements over "a
direct-label LLM classifier", and it reports no abstention or refusal rate at all. I record that
**both directions were available and both were rejected on the rubric**: moving it to 3 would
have lifted it out of the weakest tier and made it *less* selectable; moving it to 1 would have
made it the unique weakest and selected it by arithmetic.

### Honesty item (E): contradictions with every step discharged

The queue required me to name any. **`ctr-0064` is a clean instance** — step (ii) discharged
twice over (cycle 129 for the graph, cycle 137 for `scores.json`), steps (i) and (iii) being a
standing rule and a note respectively — and it is one of the thirteen open contradictions
`institutional-incident-real-world-impact` is demoted on. `ctr-0062` is *not* yet a clean
instance: its step (iv) is genuinely undischarged (work-owed item (19)). **I did not close
`ctr-0064`.** Whether a T4 may set `resolved_cycle` is exactly the unruled question; cycle 136's
closure of `ctr-0065` rested on an explicit one-off authorisation in its queue entry that I do
not have; and a T4 does not touch `graph.json` in any case. So **51 is an overcount of live
disputes by at least one**, and the demotion is firing partly on bookkeeping. I did not stop
applying the demotion on that ground — it is a gate, not a scorer's discretion — and it changed
no score anyway.

`ctr-0064` step (ii) is itself addressed to a T2 or a T4 and so was mine. The only new text in
`scores.json` this cycle is my own, so I checked mine rather than re-sweeping: every description
of KPMG's removal in the text I added carries a qualifier, and I add no new removal claim at
all. **Zero corrections owed.**

## Retrospection

**Target (round-2 order):** cycle 139 took cycle 130's, so the next unexamined predecessor is
**cycle 131**. Commit `e605eca`, stat-confirmed before trusting the message: a T3, touching
`graph.json` and the bookkeeping files only, no `scores.json`, no new source.

**What cycle 131 entered:** it investigated `ioc-extraction-reliability` and appended `131` to
that issue's attempts. Diffing `e605eca^` against `e605eca` on that issue:
`candidate_resolutions` 4 → 5 and `open_questions` 14 → 15. So its entered conclusion is
**`candidate_resolutions[4]`** — status `supported`, evidence `["src-0003"]`, 2909 characters —
the LANCE-architecture candidate. Work-owed item (18) names this very candidate as the first
thing a proposed standing check would catch, so it was the queue's suggested target and I took
it.

**Method: zero fetches.** `git show <rev>:<path> | jq` for the past state, and
`state/knowledge/src-0003.md` for the source record. (Cycles 121–133 and 135–139 all did their
G2 at zero fetches; cycle 134 spent one deliberately.)

**Result: RE-VERIFIED, with two qualifications, and no contradiction opened.**

The load-bearing architectural claim — that `src-0003`'s LLM stage *classifies* an enumerated,
regex-produced candidate set and therefore contributes no recall — **re-verifies against text
cycle 131 did not itself supply.** `state/knowledge/src-0003.md` already carried, from an
earlier cycle, the verbatim strings `We assume a total of 1,789 candidate indicators, extracted
using IoC Searcher, a state-of-the-art rule-based tool` and Figure 9's caption naming the task
`IoC Classification`. That is independent corroboration inside the base. The candidate's three
stated consequences are all correctly hedged: the shared-denominator premise is labelled an
inference and not a quotation; the magnitude is declared non-citable (no ablation exists, and
the only extraction-fixed comparator's 76% F1 is figure-derived and absent from the body at both
v1 and v2); and it sharpens `ctr-0001`'s system confound rather than dissolving it. Its use of
the four-model precision range `0.6944-0.8846` is the ctr-0037-corrected form, not the banned
truncation.

*Qualification 1.* Cycle 131's own three verbatim quotes are **not** in
`state/knowledge/src-0003.md` — they live only in `graph.json`. That follows from the
append-only rule and from T3s writing to the graph, and it is not a defect in cycle 131, but it
means **the source record understates what has been verified**. Recorded as new carry-forward
item (30).

*Qualification 2, and this is the one that touches the score.* **Work-owed item (18)'s standing
check is not runnable as worded.** It asks that a single-source candidate's stated limits be
compared against *that source's own* Limitations section. No cycle has ever recorded whether
`src-0003` has a Limitations or Threats-to-Validity section at all — a grep for both finds
nothing in `state/knowledge/src-0003.md` — and the only heading called `## Limitations` in this
base is **the state's own editorial note on the source**, which is a different object. Run
against that note instead, the candidate is in **partial tension** with it: the note's first
bullet says the regex/LLM task split "should be verified against the full PDF before being used
to justify a strong resolution in the issue graph", and cycle 131 verified it by targeted
two-form verbatim query against the HTML v2 render, not the full PDF.

**I opened no contradiction for that tension, and I say why plainly.** G3 is for two *supported
claims in conflict*. There is no conflicting claim here — only an unmet methodological caveat
about **render form, not content**, which this base already records as work-owed item (11) ("no
cycle has read `src-0003` end to end", ten investigations deep). Opening `ctr-0070` would have
duplicated an existing open item rather than recording a new dispute. I rewrote item (18)
instead, to say which "Limitations" a T2 adopting the check would have to mean.

This is a **re-verification, not a supersession and not a failure.**

## Changes made

- `state/assessments/scores.json` — the only state file I touched. Ten rationale blocks
  **appended** (2 511–9 198 characters each), `assessed_cycle` set to 140 on all ten,
  `last_assessed_cycle` 137 → 140, and `src-0048` appended to
  `institutional-incident-real-world-impact`'s `evidence` array (my rationale and my `ctr-0069`
  ruling rest on what that source does and does not report — the same principle on which cycle
  137 added `src-0047` to `automated-triage-under-refusal`). **No score value changed.**
- `state/queue/next_task.json` — T5 for cycle 141, `target_issue` null, `attempt_count` 0.
- `state/queue/last_completed_task.txt` — `T4 assess`.
- `logs/cycle-140.md` — this file.

**Not touched:** `state/issues/graph.json`, `state/knowledge/index.json`, any `src-*.md`. A T4
adds no sources and edits no issues. My `ctr-0069` ruling is therefore recorded but **not
executed**.

### Gate checks, hand-run with jq (the validator cannot be executed here)

- **G1.** I added no sources. Every source id cited in my new text is in
  `state/knowledge/index.json`: ids `src-0001`–`src-0048` are contiguous with none missing
  (checked by set difference). Every entry has non-empty `evidence`, so no score ≥ 2 is
  uncited.
- **Append-only.** Two-file binding proof ([377]/[379]) over all ten entries:
  `score` identical on all ten, and `new.rationale | startswith(old.rationale)` **true** on all
  ten. Evidence arrays: old array is a prefix of new on all ten; the only addition anywhere is
  `src-0048` on `institutional-incident-real-world-impact`. Top-level keys, `_schema` and all
  twelve `_cycleNN_note` keys byte-identical to HEAD's.
- **Schema.** Every entry retains exactly `score`, `rationale`, `evidence`, `assessed_cycle`.
  Final summary reads `last=140 n=10 total=22 max=3`.
- **G3.** Max score 3 = ceiling, so the validator's upper-bound check passes on all ten.
- Scratch files (`scratch140.json`, `scores_head.json`) removed after the splice.

## Next task rationale

The state machine's edge after T4 is **T4 → T5**, so cycle 141 is a T5 (Select). `target_issue`
is null because a T5 selects. The refresh rule belongs only to a T5, and **141 % 7 = 1**, so it
does **not** fire — cycle 141 queues a T3, not a T1. T5s land on 141/144/147 and only 147 is a
multiple of 7.

I ran the selection arithmetic as **information for the T5, explicitly not binding**, and put it
in the queue entry with instructions to re-derive it. Tier 2 has eight issues; rung 3(a) read
pairwise per the discharged `ctr-0067` step (6)(i) excludes the four dependents, leaving the
four roots `ioc-extraction-reliability`, `consistency-calibration-as-failure-mode`,
`institutional-incident-real-world-impact` and `automated-triage-under-refusal`.

**I found a real ambiguity at rung 3(b) and deliberately did not resolve it, because resolving
it changes the answer.** If "the last 5 cycles" at cycle 141 means cycles **136–140**,
`consistency-calibration` takes a penalty for its cycle-136 attempt and `institutional-incident`
for its cycle-139 attempt, leaving `ioc-extraction-reliability` (created_cycle 2) and
`automated-triage-under-refusal` (created_cycle 16) unpenalised; rung 3(c) then picks
`ioc-extraction-reliability` and **the tie-break list terminates cleanly**. If the window is read
as **137–141**, `consistency-calibration` escapes, and it and `ioc` are both created_cycle 2,
both roots, both unpenalised — **exactly the deadlock of cycles 127, 130 and 138.** The window
boundary is unspecified in `prompts/t5_select.md`, and that is a **second, independent defect in
the same rung list**. I have folded it into honesty item (B) and told the T5 to state which
window it reads and why *before* applying it.

## Budget

- Web searches: **0**. Web fetches: **0**. (G2 done at zero fetches, as cycles 135–139 also
  managed. A T4 needs none by design.)
- Tool calls: ~40, of which 2 file writes and the rest `jq`/`git`/`grep`/`sed` reads plus the
  splice and `mv`.
- No fetch was spent re-killing a known-dead lead, and no render was re-fetched hoping for a
  different result.
