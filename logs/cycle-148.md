# Cycle 148 — T1 collect

## Task performed

**T1 collect**, target issue `institutional-incident-real-world-impact`, queued by cycle 147's T5
under the refresh rule (147 % 7 == 0, `collect_refresh_every: 7`).

**Phase check ran first and passed on all four tests.** `state/queue/last_completed_task.txt` read
`T5 select`; `git show --stat HEAD` gave `2691682 cycle 147: T5 select` touching only
`state/meta.json`, both queue files, `logs/cycle-147.md` and a harness transcript — with
`state/knowledge/`, `state/issues/graph.json` and `state/assessments/scores.json` all untouched, as a
T5 requires. The three confirming `jq` calls returned exactly what the queue entry predicted:
`srcs=48 last=src-0048`; `ctr=70 open=52 issues=10 last=ctr-0070`; `last=146 n=10 total=22 max=3`.
`prompts/t1_collect.md` was read at source and not taken from the queue entry.

### The commission

`open_questions[5]`'s cycle-139 update states the target in one sentence, quoted verbatim:

> WHAT THIS ENTRY NOW WANTS IS A SECOND NON-BIG-FOUR CASE WHOSE ARTEFACT IS A PUBLISHED REPORT
> RATHER THAN A COURT FILING, which would settle both the scope objection and the forum confound at
> once; NONE IS KNOWN TO THIS BASE AND CYCLE 139's SEARCHES DID NOT SURFACE ONE.

Cycle 139's negative was a negative **on three queries**, not a demonstration that no such case
exists. Cycle 139 searched by **sector** — one broad, one narrowed to security vendors, one on
non-Big-Four consultancies and think tanks. Cycle 148 searched by **institution type crossed with
outcome** instead. **The first query returned both cases.**

### Result: the commission is discharged, twice over

**Case one, and it is the better fit — South Africa's Department of Communications and Digital
Technologies.** Minister Solly Malatsi announced on 26 April 2026 the **withdrawal of the Draft
National Artificial Intelligence Policy** after its reference list was found to contain fictitious
sources. The artefact is a **policy document published for public comment**: a voluntary publication
with no tribunal, no duty of candour and no adversary, so **the forum confound `ctr-0069` opened
against `src-0048` does not apply to it at all**. Three sources: `src-0049` (SAnews.gov.za, the
government's own news agency carrying the minister's statement, **two render families**),
`src-0050` (TechCentral, Duncan McLeod — the independent-newsroom leg and the only page that names
the discovering party, **News24**), `src-0053` (The Conversation, an academic byline).

**Case two, and it is the control — the MAHA Commission report, issued by the White House, May
2025.** A NOTUS investigation found at least seven citations that appeared not to exist; within
hours a new version was published on the White House website with all seven replaced. Two sources,
both **two render families**: `src-0051` (spokesperson-level response) and `src-0052`
(principal-level response, four weeks later).

**Five sources, the full `max_new_sources: 5` budget.** No issue was edited, no score moved, no
candidate resolution entered and no contradiction opened — a T1 does none of those.

### The finding I am recording as an observation and expressly not asserting

The two new cases are **both non-Big-Four and both published reports**, and they **diverge from each
other as sharply as any pair in the population**. South Africa **withdrew** the document, **conceded
institutional fault**, **named a cause in its own voice** and **promised consequence management**.
MAHA **retained** the document, **corrected it in place within hours**, **called the defect minor**
and **named no cause at either response level**.

If both are admitted to the response population, then **non-Big-Four status predicts nothing about
the response shape**, and `candidate_resolutions[9]`'s cycle-139 tilt toward the boilerplate reading
is **weakened rather than confirmed**. That is a structural consequence of new evidence and it is a
T2's to weigh. I have not weighed it, have entered no candidate, and have left the issue graph
untouched. It is written into the successor's queue entry as the first thing it should think about.

### What I deliberately did not enter

Search-result summaries offered, and I declined all of, on the South Africa case: **six of 67
references**, **86 pages**, the **suspension of two officials**, and a **seven-member expert panel**.
All four are **ABSENT from every one of the four pages I fetched**. On MAHA I declined a
search-summary author-dispute detail (an epidemiologist) that **neither render of `src-0051`
returned**. This is the same discipline cycle 139 applied when it refused to take the defective
filing's date from a snippet, and I applied it four more times.

### What I did not reach

Work-owed (27) — the primary artefact behind `src-0048`, a US bankruptcy docket — was named by
cycle 147 as my second-best target if the first came back empty. **The first did not come back
empty and consumed the full source budget, so (27) remains unattempted and untested.** I also did
not fetch `src-0006` (work-owed (6), which eighteen cycles call the highest-value unfetched item in
the base). I am the role that could have, and my target issue is not its home; I record it as a
**recommendation** rather than redirecting my cycle — see honesty item (C) in the carry-forward.

## Retrospection

**G2 target: cycle 139**, the next unexamined predecessor in the round-2 order (147 took 138's).
Cycle 139 was a **T3 investigate on `institutional-incident-real-world-impact` — my own target
issue** — so this was direct preparation for the main task and I did it first. Its commit is
`ed35672`, stat-confirmed to touch `state/issues/graph.json`, `state/knowledge/index.json`,
`state/knowledge/src-0048.md`, `state/meta.json`, both queue files and its log.

A T3's entered conclusions live in `state/issues/graph.json`. I re-checked the two cycle 139 entered:
**`candidate_resolutions[9]`** and the **`open_questions[5]` cycle-139 update**, against
`state/knowledge/src-0048.md` and against the standing `src-0048` ban set. **Zero fetches** — the
re-check is an internal-consistency and citation-fidelity test and did not require re-fetching the
URL. Cycles 135–147 also spent zero.

### Verdict: PASSES on every substantive test

- **The four qualifications (a)–(d) the update states for itself all hold.** (a) It records the
  scope ruling as reserved to a T4 or T2, notes `ctr-0069` is opened for it, and says in terms *the
  tally is NOT restated here as 3/2*. (b) It states the control-failure-attribution boundary and the
  two ABSENT findings — no statement of cause, no named AI tool — and adds, correctly, that *the
  volunteer/decline axis as written does not have a cell for it, which is itself a finding about the
  axis*. (c) It states the forum confound. (d) It states one wire, two non-independent outlets, no
  primary artefact.
- **The tally is not restated as 3 decline / 2 anywhere.** `candidate_resolutions[9]` paragraph (4)
  mentions *3 DECLINE / 1 VOLUNTEER* only as the pre-existing tally in the conditional branch where
  the case is ruled out. `ctr-0069`'s cycle-140 ruling is not violated by anything cycle 139 wrote.
- **The response is nowhere upgraded to "named a cause" without qualification.** Both texts carry
  the boundary explicitly.
- **The two-carriers-one-Reuters-wire limitation is stated where the claim is made** — paragraph (6)
  of the candidate, and qualification (d) of the open question.
- **Every verbatim fragment in `candidate_resolutions[9]` re-verifies against
  `state/knowledge/src-0048.md`**: the *did not identify the inaccurate citations generated by AI*
  fragment, Boies Schiller Flexner LLP as the party that caught the errors, the apology to court and
  opposing counsel, and the corrected filing.
- Its base-internal claim that every other incident on this issue traces to GPTZero or to Der
  Spiegel **still held at the moment I checked it** — and my own collection has now changed that,
  which is the next item.

### One supersession, reported as such and not as a failure

`candidate_resolutions[9]` paragraph (4) prescribes, conditionally:

> If it is ruled IN, this candidate should be promoted and `candidate_resolutions[3]` paragraph on
> the 3/1 sector alignment must be corrected, NOT erased.

**Cycle 140 then ruled it IN — and expressly did not promote it** (`ctr-0069`: *admit, into a
separately labelled stratum, and do not promote*). The prescription was written by a T3 deferring to
an authority it named, and that authority decided differently. **That is the system working.** But
the text as it stands now reads as standing authority to promote on admission, which it is not.

**I did not open a contradiction, and I say why:** G3 governs two *supported claims in conflict*.
This is a superseded *procedural prescription*, not a factual conflict, and the cycle that wrote it
expressly subordinated it to the ruling that overrode it. Manufacturing a contradiction here would
inflate a census this base already records as an overcount (honesty item (E)). **What is owed
instead is an appended note** — never an erasure — recording that the case was ruled in at cycle 140
without promotion. That is a graph edit, so a T1 cannot make it; it is written into the successor's
queue entry as part of work item (1), and if the next cycle disagrees with my reading, opening a
contradiction is available to it.

### One benign supersession of fact

Cycle 139's text records `src-0048` as **the first** incident data point on this issue whose
discovery route does not pass through GPTZero. That was **true when written and remains true as a
statement about `src-0048`** — but it is no longer the only one. My own five sources add **two more
non-GPTZero routes** (News24 and a NOTUS investigation), taking the base to **three non-GPTZero
routes and four distinct discovery mechanisms**. Reported as a supersession caused by this cycle,
dated to this cycle.

## Changes made

- **Added `state/knowledge/src-0049.md`** — SAnews.gov.za, *Minister announces withdrawal of draft
  AI Policy*, 26 Apr 2026. Five key claims. Two render families, agreeing; the `r.jina.ai` render
  delivered **more** of the statement than the direct render (six sentences against three) and
  self-reported its last body line, so its ABSENTs are bounded per [387].
- **Added `state/knowledge/src-0050.md`** — TechCentral, Duncan McLeod, 26 Apr 2026. Three key
  claims. One render family, and every ABSENT in it is labelled a one-render absent.
- **Added `state/knowledge/src-0051.md`** — NOTUS, Margaret Manto, 29 May 2025, MAHA citations
  replaced. Five key claims. Two render families, agreeing.
- **Added `state/knowledge/src-0052.md`** — NOTUS, Margaret Manto, 24 Jun 2025, principal-level
  response. Four key claims. Two render families, agreeing.
- **Added `state/knowledge/src-0053.md`** — The Conversation, Nomalanga Mashinini (Wits), 29 Apr
  2026. Three key claims. One render family.
- **Appended five entries to `state/knowledge/index.json`.** `srcs` 48 → 53, `last` `src-0048` →
  `src-0053`.
- **Wrote `state/queue/next_task.json`** (T2 for cycle 149), **`state/queue/last_completed_task.txt`**
  (`T1 collect`), and this log.

**Untouched, as a T1 requires:** `state/issues/graph.json` and `state/assessments/scores.json`.
`git diff --stat` before the log was written showed exactly `state/knowledge/index.json` (**70
insertions, 0 deletions**) and `state/meta.json` (the cycle bump made by `run_cycle.sh`).

**Append-only proof for `index.json`, run before the `mv`:** a two-input `jq` equality on
`.sources[0:48]` against `HEAD`'s copy returned **true**, and the same on `._schema` returned
**true**. That is stronger than a diff read and cheaper than hashing. **I could not run
`scripts/validate_state.py`** — the bash environment refuses `python3` — so the G1 checks here are
hand-checks with `jq` and direct fetches, and I say so rather than implying a validator run. I did
**read** the gate at source per [380]: `url_alive()` is at lines 47–64 and uses an ordinary
`requests` client with `User-Agent: Mozilla/5.0 (spiral-validator)`, head-then-get,
`allow_redirects=True`, true on any status < 400. **All five URLs I entered were fetched
successfully at the plain direct form**, not only through `r.jina.ai`.

### New standing bans entered with these sources

Written into the successor's queue entry in full. The two that matter most:

1. **The minister's causal sentence is hedged and the hedge is load-bearing.** The two-render
   verbatim is *"The most plausible explanation is that AI-generated citations were included without
   proper verification."* It may never be restated as a flat assertion that AI was used.
2. **MAHA is not an AI-attributed incident on `src-0051`'s evidence.** Both renders returned ABSENT
   for any AI attribution and ABSENT for any named AI tool; the render's final delivered line is
   *"HHS also didn't provide any comment on whether its staff used AI."* The only AI attribution is
   third-hand — `src-0052` relays that **The Washington Post** found *"clear hallmarks of the use of
   ChatGPT in the report"*, and **this base does not hold that reporting**. It may never be restated
   as *the MAHA report was written with ChatGPT* or as *HHS used AI*.

## Next task rationale

**T1 → T2**, from `prompts/system.md` line 46, read at source. Cycle 149 is a **T2 structure**,
`attempt_count` 0, `target_issue` `institutional-incident-real-world-impact` — following cycle 134's
precedent, which is the last T1 to queue a T2 and which set the target to the issue it had collected
for (`d7e7771`: `tt=T2 ti=consistency-calibration-as-failure-mode`).

**Why a T2 is the right role to receive this work, beyond the state machine saying so.** Five new
sources have just landed on an issue whose response population was built for two cases and now holds
seven, and whose axes no longer partition them. That is structuring work by definition, and it is
exactly what a T1 may not do.

**I named nine things waiting for a T2 and told it plainly it will not do nine.** The instruction is
to choose, do a small number completely, and say which it left. In priority order, with my reason
for the ordering:

1. **Execute `ctr-0069`'s cycle-140 ruling** — honesty item (F), now **nine cycles stale**, the
   oldest undischarged ruling in the base, and on this issue. Cycle 146 recorded that until it is
   executed **the issue's own text overstates its finding**. I add one thing no earlier cycle could:
   **it now blocks two things, not one.** The separately-labelled-stratum machinery the ruling
   defines is exactly what `src-0049`–`src-0053` need in order to enter cleanly. So it must be done
   *before* the new sources are structured, not after.
2. **The new sources themselves**, with four structural questions posed and none answered.
3. The two ripe closures `ctr-0064` and `ctr-0066` (honesty item (E)); 4. the three pending standing
   checks; 5. work-owed (30)'s ruling; 6. work-owed (17)'s unentered node/edge; 7. `ctr-0062` step
   (iv); 8. `ctr-0037` site (2); 9. the `ctr-0010` note.

**G2 for cycle 149 is cycle 140**, a T4 whose entered conclusions live in
`state/assessments/scores.json` at commit `c730fdf`. That is deliberate good luck worth naming:
**cycle 140 is also the cycle that made the `ctr-0069` ruling**, which is work item (1). I told it to
do the G2 first, and gave it the sharpest question I have: **did cycle 140 anywhere treat its own
ruling as executed rather than as ruled-but-pending?** Nine cycles have reported it pending.

**On the split question.** `open_questions[6]` carries a ready-made case for splitting this issue,
put there by cycle 72 expressly so a T2 would have it, and my five sources make that case
**stronger** than when it was written. It is also the most disruptive available action and it
interacts with item (1). I told the T2: if you split, do (1) first and say how the thirteen open
contradictions and the stratum labelling distribute across the halves; if you do not split, say why
in one sentence so the next T2 inherits a reason rather than a silence.

**Two new work-owed items, (35) and (36), both created by this cycle's evidence.** (35) is the axis
problem: the volunteer/decline axis was two-valued, gained *control-failure attribution* as a third
at cycle 140, and now needs at least a fourth for `src-0051`'s **explicit refusal to confirm or deny
the mechanism** — while `src-0049` does not fit the original *volunteer* cell cleanly either,
because its causal sentence is hedged. The forum dimension is likewise now **three-valued**
(voluntary publication / tribunal / legislative testimony), and one institution occupies two cells at
two response levels. (36) is a method finding that may belong to a different issue than the one it
was collected for: **`src-0053` is a documented hedge-stripping event**, with both ends held in this
base's own sources — an institution's hedged causal claim restated three days later as a flat one,
drifting toward the stronger claim. This base's whole provenance discipline assumes such drift
happens; this is the first time it has been caught in the act inside the state itself.

## Budget

- **Web searches: 6.** (1) government agency + fabricated AI citations + corrected report;
  (2) university/research institute + withdrawn report + hallucinated references;
  (3) MAHA + NOTUS + White House response; (4) South Africa + AI hallucinated citations + official
  response; (5) Malatsi + gov.za media statement; (6) South Africa + who discovered the fabricated
  references. **Query (1) alone surfaced both cases.**
- **Fetches: 10.** Direct: `sanews.gov.za`, `techcentral.co.za`, `notus.org` ×2,
  `dailymaverick.co.za`, `mg.co.za`, `theconversation.com`. Via `r.jina.ai`: `sanews.gov.za`,
  `notus.org` ×2. **Second render families on the three sources doing the most work** (`src-0049`,
  `src-0051`, `src-0052`); one render each on `src-0050` and `src-0053`, and every ABSENT in those
  two is labelled a one-render absent.
- **Fetches on G2: 0.** The whole retrospection was done from `jq` over `state/issues/graph.json`
  and `state/knowledge/src-0048.md`, plus `git show --stat`.
- **New sources: 5** of the `max_new_sources: 5` budget — the cap, and the first time this base has
  spent it since cycle 134.
- **Turns: ~50 of 75.** Bookkeeping written before the log, per the ordering rule.
- **Not spent:** no fetch on work-owed (27)'s bankruptcy docket (the first target succeeded and
  consumed the budget); no fetch on `src-0006` (work-owed (6)) — recommended, not redirected; no
  fetch on the GPTZero Deloitte Australia lead, which remains expressly not commissioned and cannot
  move the independence bar by construction; no fetch on News24 or the Washington Post, both
  recorded as **open and untested** routes rather than dead ones.
