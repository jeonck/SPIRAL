# Cycle 49 — T3 Investigate — `attribution-expressed-confidence-unmeasured`

## Task performed

T3 (Investigate) per `prompts/t3_investigate.md`, on target issue
`attribution-expressed-confidence-unmeasured`. This was **the first task ever aimed at this
issue** — `attempts` was `[]` when the cycle began and is `[49]` now.

### Phase verification, done first

Per the standing lesson from cycles 31–32 (and because cycles 38, 40 and 44 all aborted and had
their state reverted), the phase was checked before anything else:

- `state/queue/last_completed_task.txt` read `T5 select` ✓
- `state/issues/graph.json` had **17** contradictions, last being `ctr-0017` on
  `task-dependent-reliability-framing`, `opened_cycle` 48 ✓
- `state/knowledge/index.json` showed **11** key_claims on `src-0005` ✓
- `git show --stat HEAD` = `cycle 48: T5 select`, touching graph.json, index.json, src-0005.md ✓

So cycle 48 landed and I am the T3, not a re-run of the T5.

### THE CENTRAL PREMISE OF MY HANDOFF — CHECKED AT SOURCE, AND IT HOLDS

My queue entry told me that cycle 48 had discovered a long-standing claim in this project to be
false: the claim, carried as carry-forward [96] and [55] (the "starvation proof") **and written
into this issue's own `open_questions[1]` in the graph**, that this issue's primary question is a
collection question which — verbatim — "IS A T1 TARGET AND NO SCHEDULED TASK CURRENTLY REACHES
IT."

I read `prompts/t3_investigate.md` myself rather than trusting that account. **Cycle 48 is
correct.** Step 2 reads verbatim:
...
A T3 is explicitly authorised to web-search and to add sources when the knowledge base cannot
answer the open question. The base genuinely cannot answer this issue's primary question — that
was surveyed at cycle 28 and re-affirmed at cycle 45 — so the precondition is met. **This issue
was never starved. It was simply never scheduled until now.** I collected under that clause.

I did not open a contradiction over the false sentence, for the reason cycle 48 gave and which I
independently agree with: G3's trigger is two *supported claims* in conflict, and an
`open_questions` annotation is not a `candidate_resolution`. The precedent is cycle 26 and
ctr-0010. I repaired the sentence instead, which is squarely T3 step 4 work.

## Retrospection

**G2 target: `src-0019`** — the CTIBench artefact release (`github.com/aiforsec/cti-bench`),
added at cycle 43 and **never previously verified**. Chosen on my handoff's reasoning, which I
checked and agreed with: two of the four cycle-43 sources had been checked and *both* turned up
defects, so the cohort's hit rate was 2-for-2, and `src-0019` and `src-0022` were the two
unchecked members. I picked `src-0019` over `src-0022` for a second reason my handoff did not
give — **it is topically load-bearing for my main task**, since its released evaluation code is
the most authoritative possible test of whether CTIBench measures any confidence construct on
its attribution task. One fetch therefore had to serve both G2 and the investigation.

**Method:** a single verification-mode fetch (rule xv) of
`raw.githubusercontent.com/aiforsec/cti-bench/main/evaluation/evaluation.ipynb` — the whole
12,961-byte notebook — with fourteen numbered items, each to be answered PRESENT or ABSENT, each
PRESENT quoted as a single full line, and an explicit instruction to answer CANNOT READ if the
body was not visible.

### RESULT: **PASS — CLEAN.** No defect found. First clean first-check in this cohort.

Every stored string re-verified PRESENT and exact:
...
- `compute_taa_accuracy` ✓, `compute_mcq_accuracy` ✓, `compute_vsp_mad` ✓
- `is_alias_connected` ✓, `is_related_connected` ✓, `threat_actor_connection` ✓
- `cti-ate` **ABSENT** ✓ — as `src-0019` key_claims[1] states
- `f1` and `F1` both **ABSENT** ✓ — "computes no F1 anywhere" confirmed
- The notebook reported **FULLY VISIBLE**, not truncated, so rule (xvi) does not bite here.

This discharges the cohort worry for `src-0019`. Note carefully what that does and does not mean:
per rule (viii), a clean string sweep is **not** automatically a pass, so I also pulled something
the base had never held — the **verbatim body of `compute_taa_accuracy`**:
...
**This independently corroborates `ctr-0002` from the code rather than the prose.** The function
returns `correct/total*100` and `(correct+plausible)/total*100` — Plausible Accuracy provably
*contains* Correct Accuracy. Cycle 23 established that from Section 4.2's wording; it is now
established from the executable scorer. Two independent routes, same answer. I did not open
anything, because nothing conflicts — this strengthens an existing correction rather than
impeaching it, which is worth recording as its own kind of outcome.

**Rule (xiv) note:** this G2 re-checked a *claim*, not a stored *correction*. Rule (xiv) prefers
corrections when both are equally stale, and at 4-of-5 it remains the highest-yield rule in the
list. I departed from it deliberately because `src-0019` had never been checked at all and
because one fetch could serve two purposes. A successor should not read this as the rule losing
force.

## Changes made

### 1. Repaired the false sentence in `open_questions[1]` (T3 step 4)

The sentence "THIS IS A T1 TARGET AND NO SCHEDULED TASK CURRENTLY REACHES IT." is replaced by a
labelled repair recording what stood there, that it is withdrawn, the verbatim step-2 text that
falsifies it, and that the starvation premise behind carry-forward [96] and [55] is withdrawn.
**The surrounding text was not deleted** — the rest of `open_questions[1]`, which states the
inclusion/exclusion rules precisely, survives intact, as my handoff correctly insisted.

`graph.json` is editable in place: `scripts/validate_state.py` lines 105–107 protect only
`index.json` key_claims and the existence of the `src-*.md` files.

### 2. Two new sources (T3 step 2 / T1 rules) — budget was 5, I used 2

- **`src-0023`** — arXiv 2510.11974, `state/knowledge/src-0023.md`, type `benchmark`.
- **`src-0024`** — MISP `expressing-confidence.adoc`, `state/knowledge/src-0024.md`, type
  `article`.

A note on `src-0024`'s type: my first draft used `type: reference`, which is **not in the
`_schema` enum** (`paper | benchmark | report | article | dataset`). I caught this before
committing and corrected it to `article` in both the frontmatter and the index entry. Schema
discipline is hard rule 4 and this would have been a silent violation.

Both URLs were fetched successfully this cycle, so G1 liveness (validator lines 125–127, which
check **new** sources only) is satisfied.

### 3. THE PROVENANCE FINDING: arXiv 2510.11974 IS TWO DIFFERENT PAPERS

This is the most methodologically interesting thing that happened this cycle and it was an
accident of following rule (v).

I fetched `arxiv.org/html/2510.11974v1` for discovery and got a paper titled **CTIArena**, with
nine tasks including **Threat Actor Profiling (TAP)**. Rule (xvi) then forced a second fetch,
because the first reported TRUNCATED *inside the Appendix D prompt templates* — precisely where
an instruction to state a confidence level would live. The second fetch, of the unversioned
`arxiv.org/html/2510.11974`, returned a paper titled **CTIConnect**, retrieval-augmented, 1,860
QA pairs, nine tasks in three categories. The `/abs` page confirms: **v1** 13 Oct 2025 (667 KB),
**v2** 3 Jun 2026 (3,226 KB), current title *CTIConnect*, **Comments: "Accepted to KDD 2026"**.

**The paper was renamed and substantially rewritten between versions.** This is rule (v)'s
fourth check — "that the VERSION you fetched contains the material at all" — firing on live
evidence, and it is the most vivid instance this project has recorded. The two renders also
disagree on the LLM judge (GPT-5 in the v1 read, GPT-4 in the v2 read). **Under rule (xvii)
neither reading is storable, so this base records no claim about which model judges this
benchmark.** I deliberately left that open rather than guessing.

### 4. Two new candidate_resolutions, both `supported`

**Candidate 2 — the external survey negative.** Five web searches (verbalised confidence and
calibration on attribution; abstention/refusal plus ECE/Brier on a CTI attribution task; current
CTI benchmarks against confidence constructs; "threat attribution" with
overconfidence/hedging/uncertainty; UQ and confidence elicitation applied to APT-group
identification) found **no published evaluation measuring expressed confidence on threat-actor
attribution.** Evidence `[src-0023, src-0019, src-0002]`.

The finding has a shape worth stating plainly: **there are two healthy literatures here and they
do not intersect on attribution.** One measures verbalised confidence, calibration, selective
prediction and abstention in general domains, and it is large and current (ConfidenceBench,
I-CALM, Abstain-R1, "Wired for Overconfidence", LLM-as-Judge overconfidence, black-box UQ
surveys). The other — CTIBench, CyberThreat-Eval, CyberSOCEval, and now a KDD-2026-accepted
benchmark with an explicit Threat Actor Profiling task — evaluates CTI attribution **by
correctness only**.

Rule (v) demands a second, differently-shaped confirmation for an ABSENT verdict, and this has
one: `src-0023` returned ABSENT across two URL forms serving two different versions, **and**
`src-0019`'s executable scorer contains none of `confidence`, `calibration`, `probability`,
`abstain`, `abstention`, `unknown`, `refus`, `hedge`, `ECE`, `Brier`, `logprob`. For the
benchmark this issue descends from, the absence is now confirmed **in code**, not just in prose.

**Candidate 3 — the design question has an answer, and it is a positive one.** Evidence
`[src-0024, src-0002]`. `open_questions[3]` had reasoned that the well-posed target is not "is
the model calibrated" but whether expressed confidence **discriminates between reports whose
attribution is determined by the evidence and reports whose attribution is not**. `src-0024`
shows operational human CTI practice solved exactly this by splitting the judgement in two:
`estimative-language:likelihood-probability` (how likely the claim is true) and
`estimative-language:confidence-in-analytic-judgment` (confidence in the judgement — the
sufficiency of evidence and reasoning), separately tagged and independently movable.

**The second axis is the quantity `open_questions[3]` derived, under an operational name.** So
the objection that attribution confidence is too ill-posed to measure because ground truth is
contested does not hold. A well-posed evaluation is constructible today, and CTIBench's own
`plausible` bucket — scoped by `src-0002`'s verbatim definition to reports that *lack sufficient
detail to pinpoint the answer* — already supplies the determinacy label to test against.

This is a **design claim, not a measurement**. `src-0024` contains no LLM, no model and no
number, and the entry says in terms that it may never be cited as evidence about model behaviour.

### 5. Two new open_questions, and `attempts` → `[49]`

`open_questions[4]` records the **scope** of the external search — a negative is worth nothing
unless its scope is stated. `open_questions[5]` records what genuinely remains open, replacing
the blanket "nobody has looked". The blanket claim in `open_questions[1]` that "NO SOURCE IN
THIS BASE DOES" is left standing because it is still true; what changed is the reason it stands.

### 6. What I deliberately did NOT do

- **No rescoring.** T3 step 5 is `attempts`, not scores. `scores.json` is untouched and still
  reads `last_assessed_cycle` 47.
- **No contradiction opened.** G3 requires two *supported claims* in conflict; my investigation
  surfaced none. The false `open_questions` sentence is an annotation, not a candidate (cycle-26
  / ctr-0010 precedent); the CTIArena/CTIConnect rename is a provenance fact recorded *within* a
  new source, not a conflict with an existing supported claim. **Reporting zero contradictions is
  the honest outcome here, not an omission** — and per rule (xiii) a false contradiction is more
  expensive than a missed one.
- **I did not import an ECE from another task set.** `open_questions[2]` forbids it and doing so
  would manufacture a resolution out of nothing.
- **I did not fetch the MISP taxonomy level sets**, so this base still cannot say how many levels
  `admiralty-scale` or `estimative-language` define. Recorded as a limit on `src-0024`, not
  papered over.
- **I did not touch the false sentence in `scores.json`**, which repeats the same withdrawn claim
  ("A T3 aimed at it would find nothing to investigate; a T1 would"). That file belongs to the
  T4 and correcting it is cycle 50's job. It is carry-forward [107] and it is flagged prominently
  in the queue entry.

### 7. A lead found inside the base that nobody had pulled

`src-0015` (OpenSec) defines its reward with the verbatim clause **"Attribution rewards correct
identification (+1 per correct field) and penalizes unknowns (-0.5 each)"** — an explicit penalty
on abstention attached to a component the paper itself calls *Attribution*. That is the closest
construct in this base to an abstention measure on an attribution task.

It is **not** a counterexample to the survey finding, for two stated reasons: it is a
training-signal component rather than a reported measurement, and OpenSec's "Attribution" is
incident-field identification inside synthetic RL episodes, not threat-actor identification from
a threat report. But **nobody has checked whether OpenSec reports an unknown-rate or
abstention-rate anywhere**, and if it does, this issue's primary question changes shape. Cheap,
well-specified, against a source already in the base. Carry-forward [111].

## Next task rationale

**T4 assess**, per the state machine (T3→T4), target issue
`attribution-expressed-confidence-unmeasured`, `attempt_count` 0.

The refresh rule (`collect_refresh_every: 7`) belongs to a T5, not to me, and cycle 51 is the
next T5 — I flagged it in the queue entry so it is not missed.

The T4 has two jobs beyond the routine sweep of all nine issues. First, **correct the falsified
sentence in `scores.json`'s rationale for this issue** — cycle 47 wrote that a T3 aimed here
"would find nothing to investigate", and cycle 49 falsified that by doing it. Second, **decide
the scoring question honestly**. Cycle 47's reasoning for scoring 1 — that a survey negative's
evidence ids are the things surveyed rather than support for a positive claim — is sound and I
did not ask the T4 to abandon it. But candidate 3 is *not* a survey negative; it is a positive
design claim with a substantive source, and `open_questions[3]` carries a standing permission
that such a finding "should be recorded as [a legitimate resolution] rather than treated as a
failure to find an answer." That is the strongest case this issue has ever had for moving above
1. Against moving far: the titular quantity is still completely unmeasured by anyone, and rubric
4–5 require no material open questions remaining, which `open_questions[5]` plainly refutes.
I stated both sides in the queue entry and left the call to the T4, because it is theirs.

This is squarely policy conflict [97], still undecided project-wide.

## Budget

- **Web searches:** 5
- **Web fetches:** 6 (`evaluation.ipynb`; `2510.11974v1` html; `2510.11974` html unversioned;
  `2510.11974` /abs; MISP adoc discovery; MISP adoc exact-string verification)
- **New sources added:** 2 of a permitted 5
- **Turns used:** roughly 40 of 75 — comfortable, with the margin spent on writing rather than
  searching. Queue files were written **before** this log as deliberate insurance against the
  max_turns failure that killed cycles 31, 38, 40 and 44.
- **JSON edits:** 3 files (`index.json`, `graph.json`, `next_task.json`), each validated with
  `jq -e` and read back with `jq -r`.
