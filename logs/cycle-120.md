# Cycle 120 — T1 collect

Target issue: `consistency-calibration-as-failure-mode`.
Date: 2026-08-06. Model `claude-opus-5`, CLI 2.1.220.

Phase check passed before any other work, exactly as the queue entry demanded and without
trusting it: `state/queue/last_completed_task.txt` read `T5 select`; `git show --stat HEAD` showed
b4dd32e touching only the five bookkeeping paths a pure-selection T5 may touch; the two
confirming jq calls returned `srcs=41 last=src-0041` and `ctr=61 open=45 issues=10`. All four
agree with the entry, so the entry's account of the collect-refresh beat (119 % 7 == 0) stands and
I ran as a T1 rather than re-deriving a T3 from the T5→T3 edge.

## Task performed

A T1 collect against this issue's `open_questions`, with a budget of five new sources.
**One source was added.** That is the honest yield and it is written up as such below, not padded.

### The ninth angle — spent, negative for `open_questions[5]`, but negative in a new way

Cycle 111 named the one untried framing: **search by the CTI sub-task name rather than by the
calibration vocabulary**, on the reasoning that every angle so far had led with the statistics term
and had therefore been answered by the statistics literature. I spent it in three queries, varied
rather than copied from the entry's suggestions:

1. `malware family attribution large language model overconfident wrong reliability diagram evaluation`
2. `indicator of compromise extraction LLM confidence score threshold expected calibration error empirical study 2026 arxiv`
3. `security operations center alert triage LLM how often wrong when confident measurement study calibration`

**No source measuring ECE, Brier or a reliability diagram on a CTI task, other than src-0001, was
returned by any of them.** The gap is now searched-and-not-found on **eleven distinct angles across
five cycles** (75's four, 78's two, 111's two, 120's three).

**The reasoning matters more than the count here.** The ninth angle *worked as a search strategy*
— unlike angles 1–8 it returned security-domain rather than general-domain material — and its three
best returns were **all sources this base already holds**:

| what the search returned | already in base as |
|---|---|
| OpenSec: Measuring Incident Response Agent Calibration Under Adversarial Evidence (arXiv 2601.21083) | **src-0015** |
| An Empirical Study of Security Calibration in LLMs for Code (arXiv 2606.31159) | **src-0013** |
| From Narrative to Knowledge Graph (SentinelLabs) | **src-0018** |

I checked each identifier against `index.json` **before** proposing it, which is the only reason
none was entered twice. Two of the three were returned under titles or framings different enough
from the stored titles that a duplicate entry was a live risk — src-0015 in particular, which the
queue entry warned me about for an unrelated reason (ctr-0057) without either of us knowing it was
the same paper the ninth angle would surface.

I read that as **evidence of saturation** — the best security-domain calibration material a fresh,
deliberately-different search can surface is already held — and it raises the cost of dismissing
the eleven-angle absence as mere search failure. **It is not proof of non-existence and I have not
written it up as one.**

### Two leads fetched and dropped

Recorded so no successor re-spends them:

- **arXiv 2601.04486**, *Decision-Aware Trust Signal Alignment for SOC Alert Triage* (Chowdhury,
  Tanvir; Ontario Tech). The closest artefact yet found *on shape*: SOC alert triage, reliability
  diagrams **present**. Dropped because **the models are Logistic Regression and Random Forest, not
  any LLM**; ECE and Brier are absent; its confidence signal is 'the posterior probability...
  generated by the detection model'. A classical-classifier reliability diagram cannot corroborate
  an LLM calibration measurement.
- **arXiv 2606.22659**, *Confidently Wrong: Severity-Aware Calibration of Prompt-Injection
  Detectors under Attack Shift* (Md Anas Biswas). Prompt-injection detection is not a CTI task; the
  detectors are ProtectAI-v2 and Prompt-Guard-2 classical classifiers; ECE, Brier and reliability
  diagrams are all absent (it reports an unspecified 'pooled calibration error'); single author, no
  stated limitations. Weak on the T1 quality bar, so dropped rather than used as padding.

### The one source added — src-0042

Found via the **fine-tuning secondary brief** (`open_questions[3]`), not the primary one.

**src-0042** — *Calibration Without Comprehension: Diagnosing the Limits of Fine-Tuning LLMs for
Vulnerability Detection in Systems Software*, arXiv 2606.20502, Zibaeirad and Vieira, 2026-06-18.
<https://arxiv.org/abs/2606.20502> (verified to load).

**Its title is a trap and that is the first thing recorded about it.** 'Calibration' there means
*decision threshold*, not proper scoring: **ECE, Brier and reliability diagrams are all ABSENT**,
confirmed independently on two renders. It **does not answer `open_questions[5]`** and is on that
question's do-not-re-propose list. It is **not a CTI task** either (threat-intelligence,
IOC-extraction, ATT&CK and threat-report tasks each checked explicitly on both renders, all
absent), so the standing exclusion rule bars it from CTI multi-source support for any issue.

What it legitimately does: it **partially answers `open_questions[3]`, on one leg only**.
src-0001's fine-tuning-can-worsen pattern has an accuracy leg and a calibration leg. src-0042
corroborates the **accuracy** leg outside src-0001's setting, finding fine-tuning
**direction-unstable** across a 3-backbone × 5-dataset LoRA grid — Qwen3-4B 11.9% → 50.4% Overall
with LineVul-FT (+38.5 pp) against Llama3.1-8B 50.6% → 1.3% Overall with Devign-FT (−49.3 pp). It
says **nothing** about the calibration leg, which remains unreplicated anywhere in this base.

The claim entered is therefore the **weaker** one — *fine-tuning degrading accuracy is not peculiar
to src-0001's setup* — and not *src-0001's pattern generalises*. Disanalogies travel with it in the
key_claims: Linux-kernel C, LoRA only, and zero-shot prompts only by the authors' own admission,
where src-0001 tested zero-shot, few-shot **and** fine-tuning.

**Two defects charged, both of which became citation holds.** (i) The two renders **disagree on the
author list** (body: Zibaeirad and Vieira; abstract render: Zibaeirad only) — recorded unresolved.
(ii) The body render returns both `Best vanilla Top-1 on LFD is 14.71% (GPT-4.1-mini)` and `Exact
CWE ranking remains below 1.3% Top-1 accuracy`; these cannot both describe the same quantity and
only the second was confirmed on the second render. **No cycle may cite either** until the paper's
tables are read directly. Also charged: the paper states **no confidence-elicitation method at
all**, only `All inference runs use temperature = 0`, which is a defect on this base's own standard
since cycles 62/75 — and that same temperature-0 regime means it measures no run-to-run consistency.

### An addendum to src-0013, and a contradiction that dissolved

While checking whether arXiv 2606.31159 was new, I re-fetched it twice — once broadly, once asking
specifically for a per-elicitation-method ECE breakdown, in both cases without supplying any value.

The first fetch returned **per-benchmark aggregates only**, and on their face they read as though
the base had attributed a *benchmark* effect to an *elicitation* effect — i.e. as though the
load-bearing cycle-62 conclusion (which binds every future candidate on this issue) was wrong. The
second, targeted fetch **dissolved it**: the per-method table shows the low values are the
Verbalized rows and the high values are the Token Probability rows, on the same benchmark, for
exactly the two models cycle 62 named. **Cycle 62's conclusion survives independent
re-elicitation.** No contradiction was opened, and the episode is recorded in the addendum as a
cautionary example of a summary-level aggregate impersonating a contradiction.

Two further findings were entered **qualitatively, with no numbers**, respecting ctr-0026 rule (ix)
and carry-forward [20]: sampling-based consistency is the *best*-calibrated elicitation method in
that study and token probability the *worst* (`Token-probability estimation performs poorly
(ECE≈0.80), reflecting the mismatch between next-token likelihood and program-level security
outcomes`); and calibration degrades from the self-contained to the repository-level benchmark.

The first of those has a sharp edge: **src-0001's own modality is token-probability** (cycle 62,
quoted at source), and this source finds token-probability the worst-calibrated of four methods in
*its* setting. I entered that as a **question, not a finding**, and said so explicitly in the file.
A successor must not promote it.

### `open_questions[1]` — a narrowing, not an answer

Re-fetching src-0018 recovered from **prose** (not from the unreadable images `open_questions[4]`
concerns) this verbatim sentence: `All reported metric values were obtained through repeated
executions of each evaluation experiment until the point estimates reached a 95% confidence level
with a relative precision of less than 5%.`

That looks like a consistency measurement and is not one. **src-0018 repeats executions to
stabilise point estimates, which consumes run-to-run variance rather than measuring it.** It can
therefore never supply the per-model replication `open_questions[1]` asks for, however its images
are eventually read. The three model sets (src-0001, src-0002, src-0018) still have zero overlap
and there is still zero per-model replication. A dedicated search for one returned nothing beyond
sources already held.

Incidentally this re-fetch **independently re-confirmed cycle 80's finding** on
`open_questions[4]`: 'Charts presented but specific numerical values not quoted in text'. No fetch
was spent chasing it — the confirmation was a by-product.

## Retrospection

**G2 target, per the round-2 order (116→107, 117→108, 118→109, 119→110): cycle 111**, a T3
investigate on this same issue, commit e32717e. Its entered conclusions are the cycle-111 additions
to `graph.json`, the addendum to `src-0001.md`, the provenance corroboration in
`open_questions[7]`, and ctr-0057.

I checked `git show --stat e32717e` first and found the queue entry's account **incomplete but not
wrong**: cycle 111 also appended to `src-0014.md`, `src-0015.md` and `src-0016.md`, which the entry
did not mention. Noted, not treated as a defect.

**Re-checked conclusion: the `src-0001.md` cycle-111 provenance closure** — that the ARES 2025 /
Springer LNCS 343-364 publication is corroborated across three artefacts.

**Result: PASSES, on a re-fetch.** I re-fetched `https://api.openalex.org/works/doi:10.1007/978-3-032-00627-1_17`
asking for the fields without supplying any of them. It returned, unprompted: title *Large Language
Models Are Unreliable for Cyber Threat Intelligence*; publication_year **2025**; type
**conference-paper**; host venue **Lecture notes in computer science**; first page **343**; last
page **364**; authors in order **Emanuele Mezzi, Fabio Massacci, Katja Tuma**. Every value matches
what cycle 111 recorded, including the page-range fingerprint.

**One fetch was spent on the G2 and I say so plainly**, against the entry's preference for a
zero-fetch re-derivation. My reasoning: this conclusion is *about an external artefact*, so a
state-internal re-derivation would only confirm that cycle 111 copied its own numbers consistently,
which is not what G2 asks. The fetch cost nothing from the five-source budget, which is what the
entry identified as scarce.

**A fourth artefact surfaced by accident.** One of the main-task searches returned the arXiv `/html`
render carrying `pp. 343-364` and the same Springer DOI, unprompted and unrelated to this check.
Provenance now agrees across **four** artefacts. Per the standing rule I did **not** treat this as a
further upgrade: no cycle may upgrade it beyond registered-and-indexed without an artefact from the
ARES conference itself, and an arXiv citation block is the *original* artefact family, not a new one.

**Also re-verified at zero fetches:** cycle 111's seventh-angle negative is present in
`open_questions[5]` exactly as described, and ctr-0057 is still open (`resolved_cycle: null`).

**No failed re-verification and no supersession found this cycle.** Cycles 117, 118 and 119 each
found one supersession; 120 found none. Reported plainly rather than manufactured.

## Changes made

- **NEW** `state/knowledge/src-0042.md` — full write-up of arXiv 2606.20502, leading with what it is
  *not*, with limitations charged and two citation holds opened.
- `state/knowledge/index.json` — appended the src-0042 entry (3 key_claims). **41 → 42 sources**;
  the base had not grown since cycle 114.
- `state/knowledge/src-0013.md` — appended a cycle-120 addendum (append-only; nothing rewritten).
- `state/issues/graph.json` — appended text to issue index 2's `open_questions[5]` (11 837 →
  16 002 chars), `[1]` and `[3]`. **Nothing else touched.** No contradiction opened, no issue,
  candidate_resolution or open_question added or removed: `ctr=61 open=45 issues=10 oq=10 cr=14`,
  identical to cycles 117–119. No score changed — that is T4 work at cycle 123.
- `state/queue/next_task.json` — T2 structure at cycle 121, target unchanged, attempt_count 0.
- `state/queue/last_completed_task.txt` — `T1 collect`.

**Gate self-check, hand-run with jq — I did not run `scripts/validate_state.py`, because Bash in
this environment refuses `python3`.** G1: the one new URL loads (verified by direct fetch) and
every new claim cites src-0042, src-0013, src-0018 or src-0001, all of which exist in index.json.
Append-only: honoured; the two existing files touched were appended to at their tails via anchored
Edit. G3: no conflicting supported claims found — the one apparent conflict was chased to source
and dissolved, which is why no contradiction was opened. Schema: index.json's new entry uses
exactly the `_schema` fields (id, url, title, type, added_cycle, file, key_claims); both edited JSON
files were verified with a counts jq before `mv`, and the scratch file was deleted.

**One deviation declared:** the T2 queue entry is **16 768 bytes**, over the ~15 KB guideline in
[238]. I chose not to trim, because every candidate cut was binding guidance (citation bans,
do-not-re-propose lists, the render-depth note). Flagged rather than silently exceeded.

## Next task rationale

T1→T2 by the plain state-machine edge; no beat is in play (the next collect-refresh is cycle 140).
Target stays `consistency-calibration-as-failure-mode` because the new material lands on it.

The structural question left to cycle 121 is genuine and is posed as a suggestion, not an order:
`open_questions[3]` now has **one leg partially answered and one leg untouched**, so it may be
worth splitting so the answered half stops masking the unanswered half, and a candidate_resolution
for the accuracy leg citing src-0042 alongside src-0001 may be warranted — carrying the exclusion
rule in its own text. A reasoned refusal is a legitimate T2 outcome and the entry says so.

The entry also states in terms a T2 cannot misread that **src-0042 does not move
`open_questions[5]`**, so the issue's primary candidate still rests on src-0001 alone and the
score-2 ceiling logic is unchanged.

**Eleventh consecutive cycle to say it:** `automated-triage-under-refusal` has still never been
investigated by a T3 in 120 cycles — `attempts` is empty, the only such issue. A T1 cannot fix that
and did not try. It needs a human ruling.

## Budget

- Web searches: **5** (3 ninth-angle, 1 per-model-consistency, 1 fine-tuning).
- Fetches: **8** — 1 G2 (OpenAlex); 2 on arXiv 2606.31159 (src-0013, incl. the targeted
  elicitation-table pull); 1 src-0018; 1 arXiv 2601.21083 (src-0015); 1 arXiv 2601.04486 (dropped);
  2 attempts on arXiv 2606.22659 (first returned binary PDF, succeeded via r.jina.ai; dropped); 3 on
  arXiv 2606.20502 (body render, abstract render, URL-resolution check). *Counted as issued calls:
  12; distinct artefacts: 8.*
- Turns: ~62 of 75.
- Sources added: **1 of a permitted 5.** Four candidates were fetched and rejected — three as
  duplicates of src-0013/src-0015/src-0018, two on the merits. No padding.
