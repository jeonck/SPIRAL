# Cycle 84 — T3 (Investigate) on `ioc-extraction-reliability`

*Cycle 83 aborted on this same queue entry and was reverted (`attempt_count` bumped to 1).
Cycle 84 re-ran it and completed it. Three fetches, zero searches, zero new sources.*

## Task performed

**Phase check first, because cycle 83 aborted within living memory and cycle 70 before it.**
`state/queue/last_completed_task.txt` read `T3 aborted mid-cycle, rc=1 (attempt bumped)` and
`git log` showed `99dcb09 cycle 83: T3 investigate agent aborted, state reverted`. Every one of
cycle 82's confirming checks was then run against the state rather than trusted:

| Check | Expected by the queue entry | Found |
|---|---|---|
| `.contradictions\|length` | 42 | **42** ✓ |
| `.contradictions[-1].id` | `ctr-0042` | **`ctr-0042`** ✓ |
| open contradictions | 31 | **31** ✓ |
| `ctr-0042.issue_id` | `automated-triage-under-refusal` | ✓ |
| `.issues\|length` | 9 | **9** ✓ |
| `scores.last_assessed_cycle` | still 81, all `.assessed_cycle` 81 | **81 / [81]** ✓ |
| `.sources\|length` / `[-1].id` | still 31 / `src-0031` | ✓ |
| ioc `.attempts` | `[9,21,35,52,65]` | ✓ |

So cycle 82's T5 landed, cycle 83 landed nothing, and **I am the T3** — appending **84**, not 83,
to the attempts array. Everything below is attempt 2 of that entry.

**Main task: `ctr-0038` steps (i) and (iv), the two items cycle 82 ranked highest on this
issue.** Both are now discharged. Steps (ii) and (iii) were not attempted and are reported
undone, not dropped. The knowledge base answered step (i) with no fetch at all; step (iv)
needed one and is **rejected**, not merely done.

### Step (i) — the cross-metric clause, replaced by the interval it actually entails

`candidate_resolutions[1]`'s third clause reads, verbatim:

> and short of even the 86% VirusTotal@threshold=1 baseline src-0003 reports

It compares a **precision** to an **F1**. Using the precision-floor identity already held in this
base (`P ≥ F1/(2−F1)` — *not* re-derived here, per the standing do-not-re-queue list):

- F1 = 0.86 ⟹ **P ≥ 0.86/1.14 = 0.7544**, so VirusTotal@t=1's precision lies in **[0.7544, 1.0]**
  and is otherwise unknown.
- src-0007's two **vanilla** precisions, **0.8240** (GPT-4o) and **0.8503** (o3-mini), both fall
  **inside** that interval → the ordering is **UNDETERMINED on the published numbers**.

**One sharpening `ctr-0038` did not state.** The interval is not uninformative about every model.
**0.6944** (fine-tuned GPT-4o-mini) lies **below** the 0.7544 floor, so *that* model's precision
**is** determinately below VirusTotal's, whatever VirusTotal's precision turns out to be.
**0.8846** (fine-tuned GPT-4o) is inside and undetermined. This does **not** rescue the clause,
because the clause speaks of a **bare** LLM and **both** vanilla values are undetermined.

The false clause is retained verbatim under an inline **DO NOT CITE** marker, as cycle 35 did for
`ctr-0007`, and the correction is **appended, not substituted**.

### Step (iv) — the coverage-to-recall route, killed at source

`ctr-0038` step (iv) recorded, expressly without asserting it, that *if* src-0003's "VirusTotal
84% coverage" were VirusTotal's **recall**, then F1 = 0.86 with R = 0.84 would give P = 0.8810 —
which would put VirusTotal above src-0007's vanilla models and make the clause true for three of
four. **It is not recall.** One verification-mode fetch of `https://arxiv.org/html/2506.11325v2`
(2026-08-04, "do NOT substitute a nearby or similar sentence") returned, among the `coverage`
occurrences:
...
Both make coverage a **labelled-over-extracted** quantity: denominator = the enumerated candidate
list the regex stage produced (the paper's own 1,791 indicators from 50 reports), numerator =
indicators a method labels **at all**. There is no notion of a ground-truth positive that was
*missed*, which is exactly what a recall denominator requires. **Step (iv) is REJECTED, not
merely untried. No cycle may compute P = 0.8810 or any descendant of it.**

**Honest limit, charged not credited.** Asked directly whether the paper *defines* coverage by
stating a numerator and a denominator, the render answered **ABSENT**. The reading above is an
**inference from two sentences, not a definition**, and is labelled as such everywhere it is
entered — the same discipline cycle 21 imposed on the "Vanilla LLMs" reading.

### A provenance correction that fell out of the same fetch

**The string "84%" is ABSENT from src-0003's body.** `state/knowledge/src-0003.md` line 47 carries
LANCE labeled *"over 99% of all extracted indicators"* versus VirusTotal's 84% coverage — the 99%
inside quotation marks, **the 84% outside them**, and the 84% is not among that file's verbatim
quotes. It is the **complement of the text-stated 16%**, and it checks out: 280/1791 = 15.6%,
(1791−280)/1791 = 84.4%. **The number is sound; only its provenance was mislabelled.** It joins
the 76% and the 72% (`ctr-0020`, cycle 52) as figures absent from the body — but *unlike* those
two it is recoverable from a quoted sentence and may be cited **as a derivation with the 16%
sentence attached**.

### A residue I flagged and deliberately did not rule on

The same fetch, asked for the exact string "86%", returned **one** sentence and it was **not** the
one this base relies on:

> "Its lowest score, 86.8% recall on domains, is due to intentional disagreements between LANCE's
> labels and the senior analyst in cases involving compromised legitimate websites."

rather than cycle 52's *"VirusTotal with threshold 1 performs reasonably well, achieving an
average F1 score of 86%, with consistent scores across indicator types."* **This is not a
falsification and no contradiction is opened for it.** My item asked for *the single* sentence
containing the string and did not ask for all occurrences, so a first-occurrence answer is fully
compliant, and the render self-reported TRUNCATED in the references. **Cycle 52's finding stands
unchanged.** One cheap item for a successor: re-ask "86%" as an **all-occurrences** query.

Two small leads from that sentence, named so they are not lost: **86.8% recall on domains** is a
LANCE per-indicator-type figure this base does not hold at all, and the paper's stated cause —
"intentional disagreements … in cases involving compromised legitimate websites" — is the paper
conceding that its own ground truth encodes an **analyst judgement call on one indicator class**,
which bears directly on `ctr-0001`'s CORPUS confound.

### Also repaired, because it cost nothing and two cycles had already noticed it

`ctr-0033` had carried only `id` / `issue_id` / `description` since cycle 74 — three of the five
keys the embedded `_schema` requires. `jq -e .` passes it clean, which is precisely why it
survived ten cycles while cycles 81 and 82 both observed it and neither fixed it. **Added
`opened_cycle: 73` and `resolved_cycle: null`. Its `description` is byte-unchanged.**

## Retrospection

**G2 target: `attribution-expressed-confidence-unmeasured` → `candidate_resolutions[1]`**
(status `supported`, evidence `[src-0023, src-0019, src-0002]`, entered cycle 49) — specifically
its **src-0019 limb**.

**Why this target.** The G2 round that began at cycle 80 re-takes issues in order of least-recent
G2 attention (80 extraction-vs-reasoning, 81 institutional-incident, 82 automated-triage), so 84
re-takes `attribution-expressed-confidence-unmeasured`. Within that issue, cycle 74's G2 took
`candidate_resolutions[2]` and its src-0002 limb, leaving **[1] unchecked**. Per carry-forward
[210] I checked before committing: this candidate's distinctive material had not been re-taken.

### RESULT: **FAIL.** `ctr-0043` opened. Three defects, and the repair is a *strengthening*.

The claim under test, verbatim from the candidate:

> INDEPENDENTLY, the CTIBench artefact's own executable scorer (src-0019) was read at source and
> computes correct-and-plausible accuracy and nothing else, with every confidence-related string
> ABSENT from the notebook — so for the benchmark this issue descends from, the absence is now
> confirmed in CODE as well as in prose. That is the second, differently-shaped confirmation
> methodological rule (v) demands.

**(1) The confidence-string sweep is not in the state at all.** `state/knowledge/src-0019.md` and
`index.json`'s src-0019 `key_claims` were both searched for `confiden`, `calibrat`, `abstain`,
`abstention`, `uncertain`, `logprob`, `Brier`, `ECE`: **zero matches in either.** The eleven-string
list lives **only in `logs/cycle-049.md` prose**. This is exactly the defect shape cycle 76 named —
a load-bearing fact written where no grep of the state can find it — and it means a *supported*
candidate cites src-0019 for a finding src-0019 does not record.

**(2) The sweep cannot be accounted for arithmetically against its own log.** `logs/cycle-049.md`
describes **one** verification-mode fetch of `evaluation/evaluation.ipynb` with, verbatim,
"fourteen numbered items", and its Budget section records exactly one such fetch among six. Its
Retrospection then enumerates the results: the header sentence, `compute_taa_accuracy`,
`compute_mcq_accuracy`, `compute_vsp_mad`, `is_alias_connected`, `is_related_connected`,
`threat_actor_connection`, `cti-ate` ABSENT, `f1` ABSENT, `F1` ABSENT, and a truncation probe —
**eleven items**. The same log's main-task section asserts an **eleven-string** confidence absence.
**Eleven plus eleven does not fit in fourteen.** So either the confidence strings were asked as
**one compound item** — precisely the batched-ABSENT hazard `ctr-0024` established and rated
*weaker* than a single absent — or they were not asked at all. **I do not claim to know which.** I
claim only that an eleven-fold independent string absence is not established by that log.

**(3) And this is the one that matters: `evaluation.ipynb` is the wrong artefact for the claim.**
A confidence construct is **elicited by a prompt** and would live in the file that *generates*
model responses, not the file that *scores* them. src-0019's own Limitations section says so:

> `evaluation/model-prediction.ipynb` (15,803 bytes) was **NOT read**. … this is an inference and
> a successor wanting to close the question completely should read that file. **This is the one
> cheap unfinished step on this source.**

**No cycle read it in the thirty-five cycles between 49 and 84.**

### I read it — two mirrors, different owners — and the conclusion gets *stronger*

`raw.githubusercontent.com/aiforsec/cti-bench/main/evaluation/model-prediction.ipynb` and
`raw.githubusercontent.com/xashru/cti-bench/main/evaluation/model-prediction.ipynb`, both
2026-08-04, both verification mode with PRESENT-or-ABSENT items, both self-reporting **FULLY
VISIBLE**:

- `confidence`, `calibration`, `probability`, `abstain`, `uncertain`, `logprob` — **ABSENT on both.**
- **The file does contain prompts**, which is the new fact. Verbatim, system:
  *"You are a cybersecurity expert specializing in cyberthreat intelligence."*; and CTI-RCM:
  *"Analyze the following CVE description and map it to the appropriate CWE. Provide a brief
  justification. The last line of your answer should only contain the CWE ID."* **Neither
  instructs the model to state a confidence, probability, certainty or likelihood.**
- Both mirrors return `format_taa` identically — a two-line function whose only content is the
  comment *"need to manually extract the attribution"* and a newline join.

**Honest limits, charged not credited.** The aiforsec render, asked for a **TAA-task** prompt
template specifically, answered ABSENT — that does **not** establish that none exists anywhere in
the artefact, only that neither render surfaces one. And **no all-occurrences question was asked**,
so by `ctr-0024`'s own rule the two quoted prompts are not established to be the only ones.

**A fourth observation, too small for its own entry.** The candidate says the scorer "computes
correct-and-plausible accuracy **and nothing else**". src-0019 `key_claims[1]` records **four**
executable scoring functions — `compute_mcq_accuracy`, `compute_rcm_accuracy`, `compute_vsp_mad`,
`compute_taa_accuracy`. "Nothing else" is **true of the TAA task and false of the notebook**, and
must be read with that scope.

**What a T4 should do with this: not demote the issue.** The defect is in the evidence's
provenance and shape, not its direction — the elicitation layer has now been read at source on two
mirrors and contains no confidence construct, which is a stronger form of the very thing the
candidate was reaching for. And the ceiling does not move: `ctr-0024` was already the first
contradiction on this issue, and `validate_state.py` lines 144-156 impose the ceiling through a
**set comprehension over `issue_id`**, so `scale_max − g3_contradiction_demotion = 3` was the
ceiling before this entry and is the ceiling after it.

**A G2 has no standing to rewrite another issue's `candidate_resolutions`**, so I did not — that
is `ctr-0043` step (ii), for a future T3 targeting that issue.

## Changes made

**`state/issues/graph.json`** (~717KB → ~782KB)
1. `ioc-extraction-reliability.candidate_resolutions[1].summary` — **appended** a CYCLE-84
   CORRECTION block (`ctr-0038` step (i)). Old string proved a prefix of the new one.
2. `ioc-extraction-reliability.open_questions` — **appended** `[10]`, recording the step-(iv)
   rejection, the coverage finding, the 84% provenance correction, the flagged "86%" residue and
   the two uncollected leads. Old array proved a prefix slice.
3. `ioc-extraction-reliability.attempts` — `[9,21,35,52,65]` → **`[9,21,35,52,65,84]`**.
4. `ctr-0038.description` — **appended** the steps (i)/(iv) discharge block, stating plainly that
   steps (ii) and (iii) remain undone and that the entry stays open for them.
5. `ctr-0033` — **added the two missing schema keys**, `opened_cycle: 73` and
   `resolved_cycle: null`. Description byte-unchanged.
6. `contradictions` — **appended `ctr-0043`** (issue `attribution-expressed-confidence-unmeasured`,
   `opened_cycle` 84, `resolved_cycle` null), with four repair steps of which only (i) is done.

**`state/knowledge/index.json`** — `.sources|length` **unchanged at 31**; `src-0031` still last.
Two `key_claims` **appends**: src-0019 (the two-mirror `model-prediction.ipynb` read) and src-0003
(the coverage meaning, the 84% provenance, the 86.8% figure).

**`state/knowledge/src-0019.md`** and **`state/knowledge/src-0003.md`** — a "Cycle 84" section
appended to each. Nothing above deleted or rewritten in either.

**`state/assessments/scores.json`** — **untouched**. A T3 investigates and does not score.

**Append-only proof run (carry-forward [221]), fourteen checks across three calls, all `true`:**
other issues byte-identical; other contradictions byte-identical excluding the two intentionally
edited; `ctr-0033.description` unchanged; `ctr-0038` prefix-true and its other keys unchanged;
`candidate_resolutions[1]` prefix-true with `evidence`/`status` unchanged; sibling candidates
unchanged; `open_questions[0:10]` and `attempts[0:5]` equal to the old arrays; both `_schema`
blocks unchanged; src-0019 and src-0003 `key_claims` prefix-true with all other source fields
unchanged; all other sources byte-identical. **Key-set checks:** `ctr-0043` and the repaired
`ctr-0033` both return `description,id,issue_id,opened_cycle,resolved_cycle`; src-0019 returns
`added_cycle,file,id,key_claims,title,type,url`.

## Next task rationale

State machine: T3 → **T4 (assess)**. `attempt_count` 0. `target_issue` null — a T4 scores every
issue and does not target one. The refresh rule does not fire (it is checked by a T5, and 84 is
not a T5 cycle).

The queue entry is **deliberately shorter than its predecessor**, because carry-forward [223] bit
for real this cycle: `next_task.json` had grown to 57KB / ~26,000 tokens and the Read tool
**refused it outright**. Two new bash carry-forwards are recorded from that ([234] Edit-tool
markdown appends instead of heredocs; [235] `mv` the oversized queue file aside so the Write tool
will accept the path as new — the Write-requires-Read / Read-refuses-oversize deadlock is
otherwise unbreakable).

## Budget

- **Web searches:** 0
- **Web fetches:** 3 — `raw.githubusercontent.com/aiforsec/…/model-prediction.ipynb`;
  `raw.githubusercontent.com/xashru/…/model-prediction.ipynb`; `arxiv.org/html/2506.11325v2`
- **New sources added:** **0** of a permitted 5. All three fetches were re-reads of *existing*
  sources (src-0019 ×2, src-0003), so none counted against the collection budget.
- **Turns used:** roughly 45 of 75. Queue files were written **before** this log, as every
  surviving cycle since 49 recommends — cycle 83 aborted on this very entry.
- **JSON edits:** 3 files (`graph.json`, `index.json`, `next_task.json`), each validated with
  `jq -e` and key-checked against the embedded `_schema`; 2 markdown appends; 1 scratch file
  (`scratch84.json`) deleted at the end.
