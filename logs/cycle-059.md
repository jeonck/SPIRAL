# Cycle 59 — T3 Investigate: `institutional-incident-real-world-impact`

**Phase verification, done first.** `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed cycle 58 landing `logs/cycle-058.md`, `state/knowledge/index.json`,
`state/knowledge/src-0024.md` and the two queue files. Cycle 58 completed, so I am the T3 and not
the T5. `last_assessed_cycle` was 56 and `src-0024` carried four key_claims, both as the handoff
predicted.

**The handoff was wrong on one checkable fact, caught before any work was done.** It asserted
"TWENTY-THREE contradictions, of which NINETEEN are open". The graph held 23 contradictions of
which **16** were open — seven are resolved (ctr-0002, ctr-0003, ctr-0005, ctr-0006, ctr-0007,
ctr-0014, ctr-0019). This is the sixteenth handoff in this loop caught misstating something it was
summarising, and it is exactly the class of error the handoff's own preamble warns about. It did
not change the plan: the three contradictions filed against the target issue (ctr-0012, ctr-0018,
ctr-0022) were confirmed open by direct query.

---

## Task performed

A repair-heavy T3. The work list was almost entirely **acting on sources already in the base**,
not collecting: every contradiction against this issue was opened *after* its only investigation
at cycle 12, so no investigating cycle had ever seen any of them.

### ctr-0022 — steps (ii), (iii) and (iv) — DONE, entry RESOLVED

Opened cycle 54, unrepaired for five cycles, during which **the graph asserted in three named
places a claim that a G2 had refuted at source**.

Step (i) was checked rather than trusted (cycle 52's lesson) and *was* genuinely done —
`index.json` src-0012 carried the cycle-54 G2 narrative at `key_claims[5]`.

- **(ii)(a)** Appended `key_claims[6]` to src-0012 in `index.json`. This is *in addition to* the
  existing `[5]`, deliberately: `key_claims[1]` is short, quotable and wrong, `[5]` is three
  thousand characters of narrative, and a reader who stops at `[1]` met no warning. The new claim
  names `[1]` as wrong and states the replacement.
- **(ii)(b)** `open_questions[4]` amended in place, **original wording preserved inside the
  amendment** so the correction is auditable.
- **(ii)(c)** `candidate_resolutions[2]` amended in place, likewise preserving the original.

All three now carry the same defensible statement — both incidents caught **externally** rather
than by internal QA, but by materially **different** external routes, one unprompted and one an
instalment of a targeted automated sweep — and all three carry the explicit prohibition on writing
that the pipeline found EY, which is **not** established (the page returned ABSENT when asked
directly).

- **(iii)** The independence reassessment, applying rule (xxi) rather than restating it. The
  candidate's argument is **split**:
  - The **EXISTENCE** limb is undamaged. A case drawn from a targeted sweep is still a real,
    manually-verified case, and the institutional-type spread is a property of the *publishers*,
    not of the discovery route.
  - The **RECURRENCE-AS-INDEPENDENT-EVENT** limb is overstated and is **downgraded**. Two events
    are two independent draws only if found independently. The selection is maximally adverse:
    the vendor states it releases findings one at a time, so the observed *sequence* is an
    artefact of a release schedule, not of an incidence process.
  - Status held at `supported` (the headline sentence asserts the surviving limb); the demotion is
    written into the text, with an explicit instruction that a T4 score the recurrence limb lower
    than cycle 12 did.
  - **Answer to the question step (iii) poses: YES, a third incident is required, and it must come
    from an unprompted route.** A further draw from the same GPTZero sweep would add existence
    evidence and institutional spread but would **not** repair the independence defect. A T4 must
    not read incident count as independence.
- **(iv)** `open_questions[0]` amended: GPTZero's pipeline **is** the sweep option (a) has wanted
  since cycle 12, it **is** running, and it publishes **numerators without a denominator** —
  established at source, the page returning ABSENT when asked for any total scanned or any rate.
  So it is *structurally incapable* of yielding a base rate as published. Fetching its further
  named investigations is legitimate T1 work that **cannot** close the question.

Step (v) is a T1 collection option, not a repair, so it is carried forward rather than held open.
**ctr-0022 resolved at cycle 59.**

### ctr-0012 — all four steps — DONE, entry RESOLVED

Opened cycle 41. **Seventeen cycles unrepaired, and steps (i) and (ii) had never been performed.**
This was checked rather than trusted because cycle 52 caught ctr-0014's step recorded as DONE
while never having run: `index.json` src-0004 carried exactly **three** key_claims, and
`src-0004.md` carried no correction section. Both were proof the steps had never run.

- **(i)** `key_claims[3]` appended to src-0004, quoting the omitted sentence WHOLE.
- **(ii)** Matching `## Cycle 59` section appended to `state/knowledge/src-0004.md` — the step
  ctr-0011, ctr-0013 and ctr-0014 each left undone on their own source files.
- **(iii)** `candidate_resolutions[3]` amended in place. The divergence it asserts is narrowed to
  **REMEDY** and **CAUSAL ATTRIBUTION** only. The omitted material is a clause of the very
  sentence the state was quoting: *"Enisa, which has an annual budget of around 27 million euros,
  admitted the errors when asked by Der Spiegel magazine, speaking of "deficiencies" for which it
  takes responsibility."* **On the source's own words both institutions publicly accepted
  responsibility**, so a divergence over acceptance of responsibility is not established.
- **(iv)** `candidate_resolutions[0]`'s APT29 fingerprint amended from a prose naming error to a
  **fabricated hyperlink**: *"For example, a link to a Microsoft page about the Russian hacker
  group APT29 also contained this name - but Microsoft itself refers to the group as Midnight
  Blizzard."* The corrected reading is **stronger** evidence of LLM fabrication and makes the
  fingerprint an instance of the *same* defect class as the 26 broken footnotes. Added a forward
  note that cycle 28's finding about CTI-TAA's alias-tolerant scoring is unaffected and reinforced.

**ctr-0012 resolved at cycle 59.**

### ctr-0018 — steps (i), (ii) and (iv) DONE; step (iii) HALF done; entry STAYS OPEN

- **(i)** and **(ii)** Correction section appended to `src-0022.md` quoting the false "Limitations"
  fourth bullet in full before correcting it without deleting it, with the baseline sentence and
  its four non-optional caveats. A matching key_claim was also appended to `index.json` src-0022 —
  not asked for by step (i), but necessary, because `key_claims[1]` carried the same false "no
  before/after control" sentence and a reader working only from `index.json` would never have seen
  the correction.
- **(iv) — the one that mattered.** `src-0022` had been cited by **no issue and no assessment**
  since cycle 43; cycle 50 called that "the real reason eight cycles of calling it the
  highest-value uncollected source have produced no score movement". A **sixth candidate_resolution**
  was added to this issue with `evidence: ["src-0022"]`: the curl bug-bounty closure as a **third
  documented institution**, and the only one of the three that abandoned a *process* rather than
  correcting a *document*. It widens the institutional-type spread to public agency / Big Four
  commercial / major open-source project. All four of the source's limits travel with it, and the
  "~20%" figure is explicitly prohibited.
- **(iii) — half done, and the failed half is recorded rather than glossed.** One fetch of the
  monthly-archive form:
  - **Promoted:** no January 2026 windowed report count appears in the post — ABSENT on the
    archive form, corroborated by the exact string `seven reports` also ABSENT. Now confirmed on
    two forms.
  - **Bonus, not asked for:** the exact string `20%` returned ABSENT on the archive form too, so
    the prohibition is now confirmed on two forms by two independent probes (concept probe at
    cycle 50, exact-string probe here).
  - **Not promoted — a fetch defect, not a finding.** Asked whether the post states any count or
    percentage of AI-slop reports, the fetch answered PRESENT and quoted the *confirmed-vulnerability
    rate* sentence, which does not answer the question asked. Rule (xvii) substitution; verdict
    discarded in both directions. **This absence remains single-form and ctr-0018 stays open on
    it.** It needs one exact-string fetch.

**Priority item (4) — src-0011 / `candidate_resolutions[4]` — NOT done.** Budget went to the three
contradictions and the G2. Carried forward, and the next T4's handoff flags src-0011 as the actual
stalest source in the base.

---

## Retrospection (G2)

**Source chosen: src-0023** (`https://arxiv.org/abs/2510.11974`) — **the last source in this base
with no G2 behind it**. Not the stalest by date (src-0011, cycle 33, still holds that), but chosen
because a never-verified source is a strictly worse position than an old-verified one, and because
it simultaneously answered a question cycle 50 flagged nine cycles ago.

**Result: MIXED. ctr-0024 opened. The conclusion survives; one stored exact-string ABSENT does
not.**

Two new URL forms, neither used by any prior cycle, both in verification mode with an item-0 body
probe, a final truncation probe and "do NOT substitute a nearby or similar sentence":

| Form | Serves | Title returned |
|---|---|---|
| `ar5iv.labs.arxiv.org/html/2510.11974` | **v1** | CTIArena |
| `arxiv.org/html/2510.11974v2` | **v2** | CTIConnect |

### What holds

`key_claims[0]`'s conclusion — the benchmark measures no elicited confidence construct — survives
on both. `calibration` ABSENT on both. `abstain`/`abstention` ABSENT on ar5iv/v1. No prompt-template
instruction to state a confidence, probability, certainty or likelihood on either.

### What fails — and it is a new defect shape for this base

Every prior contradiction here concerned material the state **omitted, mischaracterised or
misderived**. This one concerns **a negative finding that was itself wrong**. `key_claims[0]`
asserts the v2 render was swept for thirteen exact strings over the entire document and returned
ABSENT for all of them, including `confidence`. The explicit-v2 render returns **PRESENT**:

> LLM-based judge (GPT-4) filters low-confidence samples via a structured rubric

The string is in v2, inside the compound "low-confidence". Both fetches address v2, and v2 has not
been superseded, so **this is not a version difference** — cycle 49's sweep verdict was simply
wrong on that string.

**The conclusion is undamaged**, and I decline to overstate this: "low-confidence samples" refers
to filtering benchmark-*construction* data by an LLM judge's rubric score, not to eliciting a
confidence from an evaluated model. That is precisely the distinction cycle 49 itself drew when it
recorded `probability` as its one PRESENT and correctly declined to count it. **A T4 must not
demote this issue on ctr-0024.**

What *is* damaged is the evidentiary standing of exhaustive-sweep verdicts. A thirteen-string sweep
reported in one fetch is a thirteen-fold ABSENT claim resting on one render's compliance, and one
of the thirteen is now known false.

### Two findings recovered in the same pass

**1. The judge-model discrepancy is explained and half-closed.** `key_claims[2](b)` records that
cycle 49's two fetches disagreed (v1 "GPT-5", v2 "GPT-4") and that under rule (xvii) *neither* was
storable. Asked for the whole paragraph per rule (iv), the explicit-v2 render returned:
...
GPT-4 on a second distinct URL form of v2, and `/abs` — the URL this source cites — serves v2. Per
cycle 58's refinement of rule (ix): **in v2 the multi-document-synthesis judge is GPT-4.** The v1
"GPT-5" reading is neither refuted nor resolved — ar5iv/v1 returns ABSENT for `automatic judge`
altogether, reporting "LLM judge" instead — so v1's judge stays unstorable and v1 is a different
document.

**2. Cycle 50's nine-cycle-old question is answered.** *Does src-0023 report an ATT&CK/TTP mapping
task, and under what scoring rule?* **YES, in both versions, and the rule is now verbatim.**
v2: *"CTI-ATA (Attack Technique Attribution, Report → ATT&CK entries) identifies all MITRE ATT&CK
techniques described in a threat report passage"*, scored by *"Precision, Recall, and F1-score
after regex-based identifier normalization"*. v1: *"Closed-Book Attack Technique Attribution
(ATA)"*. **No per-model score is recorded** because none was verified. This bears directly on
`ttp-attack-mapping-reliability` — and I deliberately did **not** wire it in, because a T3
investigates its target issue and nothing else. It is ctr-0024 step (iv) and carry-forward [171].

**Appendix D remains unread after a third and fourth fetch**, so `key_claims[2](a)` stands. The
explicit-v2 render reported no Appendix D visible, listing only A through A.4 — *identical* to
cycle 49's v2 result, which now suggests **v2 has no Appendix D at all** and the label is a
v1-only artefact. ar5iv/v1 claimed to see "Appendix D Prompt Template" and appendices A–E, but the
same fetch reported itself **truncated mid-body**, so under rule (xvi) its appendix listing is
unusable.

**Two fetch substitutions in three fetches this cycle** (ar5iv quoting a sentence not containing
the requested string; the curl archive answering a different question than the one asked). That
frequency is why rule (xxii) below is offered.

---

## Changes made

**`state/knowledge/index.json`** (append-only respected throughout — nothing deleted or rewritten):
- `src-0004` key_claims 3 → **4** (ctr-0012 step (i), eighteen cycles late)
- `src-0012` key_claims 6 → **7** (ctr-0022 step (ii)(a))
- `src-0022` key_claims 3 → **4** (ctr-0018 steps (i)/(ii))
- `src-0023` key_claims 4 → **5** (cycle-59 G2)

**`state/knowledge/*.md`**: correction sections appended to `src-0004.md` (ctr-0012 step (ii)),
`src-0022.md` (ctr-0018 steps (i)/(ii), plus a same-cycle addendum correcting my own earlier
statement that step (iii) was undone), and `src-0023.md` (G2 record).

**`state/issues/graph.json`**:
- `institutional-incident-real-world-impact`: `open_questions[0]` and `[4]` amended;
  `candidate_resolutions[0]`, `[2]` and `[3]` amended; **new `candidate_resolutions[5]`** citing
  src-0022; `attempts` → `[12, 59]`.
- **ctr-0024 opened** against `attribution-expressed-confidence-unmeasured` — the first
  contradiction ever filed against that issue.
- **ctr-0012 and ctr-0022 resolved** at cycle 59, each with a full resolution note recording which
  step was done where.
- **ctr-0018 updated**, stays open on one half of step (iii).
- Totals: 23 → **24** contradictions, 16 → **15** open. Target issue: **3 open → 1**.

**`state/queue/next_task.json`** → T4 assess. **`state/queue/last_completed_task.txt`** →
`T3 investigate`.

**Not touched:** `state/assessments/scores.json` (a T3 does not score).

---

## Next task rationale

T3 → T4 per the state machine. The T4 handoff leads with the trap this cycle created: resolving
ctr-0012 and ctr-0022 lifts this issue's **G3 ceiling from 3 to 5**, and *nothing in the repair is
new positive evidence*. The repairs narrowed two candidates and downgraded a third. A T4 that reads
the lifted ceiling as licence to raise the score would invert the meaning of the work. The genuinely
new material that may move a score is the src-0022 wiring (candidate_resolutions[5]) — and ctr-0024
is likewise not grounds to demote, since src-0023's conclusion survived.
...
- **Turns:** ~58 of 75.
- **WebFetch:** 3 (ar5iv/v1; arxiv /html v2; daniel.haxx.se monthly archive). **WebSearch: 0.**
- **New sources: 0** — correct for a T3; all work acted on sources already in the base.
- Every JSON edit validated with `jq -e` and the added fields read back with `jq -r`.
- One Edit failed on a trailing-comma assumption (last array element has none) and was retried —
  recorded as carry-forward [172] so a successor does not lose a turn the same way.
