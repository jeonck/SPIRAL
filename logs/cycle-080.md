# Cycle 080 — T3 Investigate

**Target issue:** `consistency-calibration-as-failure-mode`
**Queue entry:** written by cycle 79 (T2). Verified accurate — see *Retrospection*, final paragraph.
**Budget spent:** 2 fetches, 0 searches, 0 new sources.

**Headline results, in the order a successor should care about them:**

1. **G2 FAILED and opened `ctr-0040`** — the first propagation site of the `ctr-0036`
   equivocation on the word "attribution", on a *second* issue, inside a *supported* candidate,
   fifty cycles after cycle 30 had the correcting fact in hand. The claimed crossover
   **disappears entirely** when the correct cells are used.
2. **`ctr-0035` RESOLVED.** Its step (iii) — the single highest-value open repair in this graph
   on cycle 76's assessment, named by cycles 76, 77 and 78 and spent by none — was executed.
   The answer is negative **and the negative is deductive, not merely not-found**.
3. **`ctr-0039` steps (i), (ii) and (iii) discharged**, including the whole-state `Brier` sweep
   cycle 79 expressly declined to run. Four material sites found where cycle 79 named none.
4. **`ctr-0027` step (ii) discharged** after eighteen cycles of being named.

---

## Task performed

T3 per `prompts/t3_investigate.md`, read directly from the file and not from the queue entry's
account of it.

### Phase check

`state/queue/last_completed_task.txt` read `T2 structure`. `git show --stat HEAD` showed cycle
79 touching `state/issues/graph.json`, the queue files and its log. All four of cycle 79's
confirming checks passed exactly as written: contradictions 39, last id `ctr-0039`, open count
29; target issue at 10 open_questions and 12 candidate_resolutions. All three of its
*non*-abort checks also held: `sources` still 31, last id `src-0031`, `last_assessed_cycle`
still 76. **Cycle 79 completed as a T2; I am the T3.**

### Step 1 — read the target issue

Nine issues in the graph; the target carries 10 open_questions, 12 candidate_resolutions
(9 supported, 3 proposed), attempts `3,15,16,25,39,62,75`, and **four** open contradictions
(`ctr-0011`, `ctr-0027`, `ctr-0035`, `ctr-0039`). Merit score 2, assessed cycle 76, G3 ceiling 3.

### Step 2 — work the open questions from the knowledge base first

Cycle 79 ranked four ready items by value per fetch. I took all four, plus one item from cycle
62's backlog. **Two of the five needed no fetch at all**, and I did those first.

#### (A) `ctr-0039` step (i) — read `candidate_resolutions[0]` and `[1]` against the decomposition finding. ZERO FETCH.

Cycle 79 recorded that it had **not** read these two candidates against the src-0028/src-0029
decomposition finding and called that "an honest gap and not a clearance". I read them.

**`candidate_resolutions[0]` is UNAFFECTED, and the reason is that it is about *neither*
metric.** It cites no Table-6 cell, no ECE value and no Brier value. Its only calibration word
is `overconfident`, which the cycle-39 scoping note already established is src-0001's own
abstract term and is a qualitative characterisation, not a metric reading. Since the
decomposition finding bears only on what a *Brier value* may be taken to measure, and this
candidate reads no Brier value, **no repair is available at this site** — there is no metric
named here to repair. The caveat I recorded rather than suppressed: `overconfident` is
src-0001's own one-word summary of what Table 6 shows, so the candidate inherits at one remove
whatever Table 6 can bear.

**`candidate_resolutions[1]` IS affected and the defect is load-bearing.** The cycle-39 rewrite
retires the extraction-versus-generation calibration gradient, and its stated ground is that
the gradient "holds on ECE at every paradigm, vanishes on Brier at zero-shot, REVERSES on Brier
at few-shot". That inference carries a suppressed premise which the rewrite itself states
openly: *"the paper defines BOTH as calibration measures"*. **That premise is src-0001's
sentence, and it is the sentence `ctr-0039` impeaches.**

The invalidity is arithmetical, not verbal. A proper score decomposes as calibration loss plus
refinement loss, so **a Brier difference between two task families is the SUM of a calibration
difference and a refinement difference**. Two families can differ in calibration by any amount
whatever and still post equal Brier scores, provided the refinement difference cancels it. So
the Brier means failing to reproduce the ECE ordering is **not** evidence that the calibration
gradient is absent — it is equally consistent with a real calibration gradient offset by a
refinement gradient running the other way, which is precisely the shape
`candidate_resolutions[7]` and `open_questions[9]` independently isolate in the extraction
attack-vector row.

**The part a successor is most likely to get wrong: the retired gradient is NOT reinstated.**
Removing the ground for a retirement does not restore the claim that was retired. Three things
stand in the way, all recorded at source:

1. The **ECE column is independently impeached as an estimator** (src-0028's fixed-binning
   lower bound and no-optimal-default sentences; src-0029 §4.4, new at cycle 80). src-0001
   states **no bin count and no binning scheme** anywhere this base has read.
2. The twelve family means are **agent-derived**, printed nowhere in the paper, averaged over
   four and five heterogeneous sub-tasks, with no uncertainty estimate.
3. src-0001 prints no refinement term and releases no artefact, so the decomposition cannot be
   evaluated on its data at all.

**Corrected status: UNDETERMINED.** That is weaker than cycle 39's "NOT a robust property of
the data" *and* weaker than the cycle-3 framing cycle 39 retired. The base has less here than
it thought at either point. What survives intact: the consistency CI widths (0.02 vs 0.06,
a different measurement), and the finding that fine-tuning worsened ECE on 7 of 9 rows and BS
on the same 7 — a **within-metric** statement at each metric separately, needing no
cross-metric premise, and now the strongest thing that candidate carries.

#### (B) `ctr-0039` step (ii) — the whole-state `Brier` sweep. ZERO FETCH.

Cycle 79 wrote that it had not run this, "which is the same omission that made `ctr-0037` name
two sites when there were three". Run at cycle 80: **67 occurrences across 12 files** under
`state/`. Per carry-forward [230] — *an enumeration of a string is not an enumeration of a
defect* — I classified rather than collected, and listed the clean sites **as clean** so that
no successor edits them.

**TIER 1 — MATERIAL (an inference depends on reading a Brier value as a calibration quantity). FOUR SITES:**

| site | status |
|---|---|
| target `candidate_resolutions[1]`, the cycle-39 retirement | **repaired at cycle 80** |
| target `candidate_resolutions[5]` — argues a prior finding is "CORRECT BUT INCOMPLETE" *because* it "reports the mean gap on ECE ONLY", quoting the impeached sentence as warrant | **NOT repaired. Largest remaining piece of this step.** |
| target `candidate_resolutions[7]` — treats ECE/Brier *agreement* after dropping a row as confirming a hypothesis | **partly inoculated by its own text**: the replacement sentence it recommends names both metrics and asserts only that they "order the two families differently", which survives; the surrounding "THE HYPOTHESIS IS CONFIRMED" framing does not |
| `index.json` src-0001 `key_claims[8]` — "…REVERSES at few-shot, **generation being the better-calibrated family**" | **the only Tier-1 site outside the issue graph. Repaired at cycle 80.** |

On that last one I checked the obvious adjacent worry and **report plainly that it is not a
defect**: `key_claims[4]`'s older "markedly worse for information-generation" framing is
**already corrected in-file** by the appended `key_claims[7]` and `[8]`. It must not be
re-litigated. I had drafted it as a fifth Tier-1 site before checking.

**TIER 2 — already corrected at cycles 78–79, no action:** target `open_questions[5]`, `[6]`,
`[9]` and `candidate_resolutions[10]`.

**TIER 3 — mention-only or harmless as used; MUST NOT BE EDITED:** target
`candidate_resolutions[2]`, `[4]`, `[6]`; `attribution-confident-wrong-gap`
`candidate_resolutions[1]`; `attribution-expressed-confidence-unmeasured`
`candidate_resolutions[1]` and `open_questions[1]`, `[2]`, `[4]`;
`automated-triage-under-refusal` `candidate_resolutions[0]`; all three `scores.json`
rationales; `index.json` key_claims on src-0013, src-0014, src-0015, src-0018, src-0023,
src-0031.

Nearly every one uses the compound label `ECE/Brier` to name a family and enforce a
**non-pooling prohibition** against a third construct (src-0015's action-threshold rate,
src-0014's selective-prediction coverage, src-0018's FDR/FNR abstention). **The compound label
is technically wrong on both halves** — ECE is not a proper scoring rule, and Brier is not a
pure calibration measure — **but the prohibition is unaffected**, since whatever ECE and Brier
are, neither is an abstention rate; and every absence statement in this tier is unaffected for
the same reason, since no metric definition turns a not-found into a found. Editing these would
be over-collection of exactly the kind `ctr-0037` step (iii) invited. Two borderline sites are
**named without being repaired**: the phrase "ECE/Brier proper-scoring calibration"
(target `candidate_resolutions[4]` and the consistency rationale in `scores.json`) misdescribes
ECE; and `attribution-expressed-confidence-unmeasured` `open_questions[1]` writes "a
calibration statistic such as ECE or Brier". Both are immaterial because both sit inside
statements of an absence.

**TIER 4 — correction machinery, never to be edited:** `ctr-0003`, `ctr-0024`, `ctr-0026`,
`ctr-0027`, `ctr-0039`; `index.json` key_claims on src-0028 and src-0029.

#### (C) `ctr-0035` step (iii) — the one fetch three cycles named and none spent

**Question:** does src-0018 report *any* quantity measuring variation in the **correctness** of
its outputs across repeated runs — which would restore a like-for-like pairing with src-0001?

**Answer: NO, confirmed on TWO render families** (direct `sentinelone.com` and the `r.jina.ai`
proxy). Each fetch was told to write ABSENT rather than infer and told not to echo back a
supplied string it had not found.

- Both forms returned **the same two consistency metrics and no third**, both confined by their
  own printed definitions to alternation among values the **expert annotators accepted as
  valid**.
- `variance`, `standard deviation`, `error bar`, `stability` — **ABSENT on both**.
- The **number** of repeated executions — **disclosed on neither**.

**One new verbatim sentence, held nowhere in this base before**, recovered on the proxy render.
It settles the interpretation in the author team's own words rather than by this base's reading
of a definition:

> "higher percentages of `IOCs with Pₒ < 1` indicate greater fluctuation in how the LLM
> interprets ambiguous evidence"

Fluctuation in the interpretation of ambiguous evidence is not variation in correctness, and
that is SentinelLabs saying so.

**THE DEDUCTIVE POINT — new at cycle 80 and the reason this closes rather than merely fails.**
src-0018 **could not supply the missing quantity from its reported numbers even in principle.**
Its stopping rule —
...
— is a **variable-`n` rule that halts when the interval around the point estimate has become
narrow enough**. It therefore **pins the reported interval width at the threshold by
construction**, so that width carries no signal about the underlying run-to-run spread; and
because an interval half-width scales as spread ÷ √n, recovering the spread requires `n`, which
is undisclosed on both renders.
...
**Step (iv), answered as `ctr-0035` asked rather than left for a T4 to infer:** the honest
scoring position is that the consistency half of this issue is **two-source for the EXISTENCE**
of run-to-run output instability on CTI material at temperature 0, and **single-source
(src-0001 alone) for instability IN CORRECTNESS**. Written positively into the new
`candidate_resolutions[12]`.

**`ctr-0035` is RESOLVED at cycle 80.** All four steps done; both sides recorded;
`candidate_resolutions[4]` left standing unaltered per the append-only rule and now carrying a
pointer to the corrected position; nothing further recoverable from src-0018 without its
images, which remain under the standing three-strikes no-refetch rule. **Cycle 80 attempted no
image.**

#### (D) `open_questions[9]` route (ii) — the one cheap route, which failed as predicted

Fetched src-0029 on the **ar5iv** render (a different render family from cycle 78's 2.9MB PDF,
which truncated around §5.6) and asked directly for any inequality or bound relating a proper
scoring rule value to a calibration error in §6 onward and the appendices. **ABSENT.**

Recorded as a **one-form ABSENT**, which per rule (v) is *not* a confirmed absence. The route is
closed **for value rather than for certainty**: hardening it would cost a second fetch to firm
up a negative that changes nothing even if wrong. `open_questions[9]` is marked **PARKED**.

**What the same fetch did return, and it is worth more than the route was:** src-0029 §4.4,
verbatim and new to this base —

> "all ECE/MCE measures can provide very different values given different number of bins, as
> shown in Figs. 1 and 2, and they can be minimised by predicting the overall class
> distribution, regardless of the given instance."

**The second clause is the sharper one.** A binned calibration error is *minimised* by an
uninformative predictor that ignores the instance. So a **low ECE cell is not by itself evidence
of good confidence behaviour — it is also exactly what a base-rate predictor produces.**

This bears on `open_questions[9]` without answering it. That question asks whether the
extraction attack-vector row — zero-shot ECE 0.13, best in its family, against zero-shot Brier
0.46, worst — is the signature of a predictor that is well calibrated but **unsharp**. src-0029
now supplies a mechanism that would produce exactly that signature. **That is a qualitative fit
and I assert nothing stronger.** It is not a measurement, it does not distinguish that row from
any other low-ECE-high-Brier cell, and the quantitative question remains unanswerable from this
base for the original reason: src-0001 prints no refinement term, states no bin count, and
releases no artefact.

#### (E) `ctr-0027` step (ii) — cheap, on-target, named for eighteen cycles, never done

src-0001's token-log-probability elicitation passage is now in **`state/knowledge/src-0001.md`
and in `index.json` key_claims**, labelled **cycle-62-verified and NOT re-fetched at cycle 80**.
Until now this fact lived *nowhere* in the knowledge base — only inside `ctr-0027` in the issue
graph — so it was invisible to every grep-based technique this loop depends on. That is
carry-forward [217]'s exact failure mode, on this base's single most load-bearing source.

#### What I deliberately did NOT spend on

`open_questions[5]` (a second CTI-task ECE/Brier source). It is six angles deep in negative
searches across cycles 25, 36, 75 and 78, with CTIArena and src-0031 both recorded as confirmed
non-candidates. Cycle 79 advised against a seventh angle and I agree. **The drought is not
evidence that no such source exists; it is evidence that six angles have failed.**

### Steps 3–6

Candidate resolutions updated (three appends, one addition — see *Changes made*); open questions
rewritten to reflect what genuinely remains open (three appends); cycle 80 appended to
`attempts`; and one contradiction opened under G3 (`ctr-0040`, from the G2 pass).

---

## Retrospection

**G2 target:** `extraction-vs-reasoning-ordinal-axis` `candidate_resolutions[1]` — status
**supported**, evidence src-0006 and src-0007, entered cycle 18.

**Why this one.** It was the least recently G2-checked issue in the graph (last target at cycle
71), and the G2 window 71–79 has now touched **all nine issues exactly once**, so a new round
starts at the oldest. **Zero fetches were needed:** both sides of what follows are already
stored and both were verified at source by earlier cycles.

**VERDICT: FAILED. `ctr-0040` opened.** This is a success of the system, not a failure.

### What passed

Every number in the candidate's **route (2)** is arithmetically exact as stated. Re-derived
from the stored files at cycle 80:

- Stored Table 4 gives `Content: Threat Actor / Attribution` = **1.140** (GPT-4o) and **2.968**
  (o3-mini). ✓
- 1.140 / 5 = **0.228**; (1.140 − 1) / 4 = **0.035**. ✓
- 2.968 / 5 = **0.5936**; (2.968 − 1) / 4 = **0.492**. ✓
- TTP F1 re-derives exactly from the printed precision/recall pairs:
  GPT-4o 2(0.2787)(0.2270)/(0.2787+0.2270) = **0.2502**; o3-mini
  2(0.3480)(0.1759)/(0.3480+0.1759) = **0.2337**. ✓

**The arithmetic is not the defect and no number is impeached.** Route (1) — the src-0006 Table
2 nine-row stage means — was **not** re-derived and is **not** in dispute; those means are
closed per carry-forward (cycles 63 and 71), and src-0006's "Attribution" is that paper's own
**stage** name, a different object from src-0007's rubric **dimension**.

### The defect

Route (2) reads those cells as the **attribution rung** of the ordinal axis *"mechanical
extraction < classification < attribution < generation"*, where "attribution" means
**threat-actor attribution** — that is the entire subject of the axis and of the issue.

But cycle 30 established at source, from Appendix C.2, and cycles 77 and 78 re-confirmed, that
**in the `Content: Threat Actor` block the dimension named "Attribution" means SOURCE LINKING,
not threat-actor identification.** `state/knowledge/src-0007.md` says so in terms:

> The cell `Content: Threat Actor / Attribution = 1.140` is **not** a threat-actor-identification
> score; the cell `Content: Root Cause / Attribution = 3.612` **is**.

Its anchors run *"1: Information is unverified or unattributed. … 5: Fully attributable; all
details are clearly linked to the original article"*, and cycle 30 records expressly that **no
anchor in that block mentions identifying a threat actor**.

**Route (2) selects its evidence by the name of the row, and the name means something else.**

### The consequence — sharper than a labelling complaint

**The claimed reversal disappears entirely on the correct cells.** `Content: Root Cause /
Attribution` gives 3.612 (GPT-4o) and 4.418 (o3-mini). Normalised the same two ways:

| model | /5 | min-max | vs TTP F1 |
|---|---|---|---|
| GPT-4o | 3.612/5 = **0.7224** | (3.612−1)/4 = **0.653** | 0.2502 — both **above** |
| o3-mini | 4.418/5 = **0.8836** | (4.418−1)/4 = **0.8545** | 0.2337 — both **above** |

All four above. **No crossover at all.**

**The candidate's conclusion is unaffected and arguably strengthened**, which is why this is
filed as a defect in a *reason* and not a withdrawal of a *finding*. Its conclusion is that the
ordinal axis is **not supported**. Route (2) as written argued that from **one** of two models
violating the predicted ordering; on the correct cells **both** violate it. What is destroyed is
the stated **mechanism** — "reverses its own middle-to-third rung *between two models*" —
together with the "at or below" leg for GPT-4o, which is simply false once the right row is read.

**A robustness claim that also fails, and it is the part a successor would most likely miss.**
Route (2) says the reversal holds "under both defensible normalisations", and it does — **but
only for the `Attribution` row specifically.** Substituting the six-dimension **block mean**:

- GPT-4o (1.547+1.528+1.145+2.019+1.734+1.140)/6 = **1.51883** → 0.30377 (/5, **above**, no
  reversal) and 0.12971 (min-max, below)
- o3-mini (3.964+3.656+3.165+4.753+4.731+2.968)/6 = **3.87283** → 0.77457 and 0.71821 (both
  above)

So under `/5` **both are above and there is no reversal at all**; the reversal survives only
under min-max. The "both normalisations" robustness is an **artefact of the single row whose
name matched the issue's rung label** and does not survive on the block that row belongs to.
All eight quotients are **agent-derived at cycle 80** and labelled derived; a successor should
re-derive rather than copy.

**A caveat that cuts against my own repair, recorded so the substitution is not mistaken for a
clean fix.** Comparing a normalised 1–5 **LLM-judged** rubric mean against an F1 is a
cross-metric comparison of exactly the family `ctr-0008` and `ctr-0038` both concern, and the
substituted Root Cause cells inherit that weakness undiminished. Further, the Root Cause block's
Attribution dimension grades threat-actor identification **within root-cause content drafting**
rather than as a standalone attribution task, and cycle 30 records that the two blocks' anchors
are worded differently. **I therefore do not assert the substituted comparison as a new
finding.** I assert only that route (2) as written is wrong, and that its own obvious repair
does not rescue the crossover.

### Why this is not a duplicate of `ctr-0036`

`ctr-0036` is filed on `attribution-confident-wrong-gap` and scoped to that issue's
`candidate_resolutions[1]`. Its **step (ii)** says expressly that the sweep for other sites was
not run, that the `Content: Threat Actor` Grep returns **sixty files**, and that *"any other
supported entry that reads the Threat Actor block's Attribution dimension as threat-actor
attribution carries this same defect and is not yet identified"*.

**This is that entry, on a different issue, and it is the first one found.** `ctr-0036` step
(ii) is thereby **partly discharged** and remains open for the rest of the sixty files.

**One uncomfortable detail I recorded rather than smoothed:** `ctr-0036` names
`state/issues/graph.json` among the three files cycle 77 *did* examine — yet this site sits in
that file. **The sweep was conducted by FILE and not by CLAIM**, and a file-scoped sweep of a
672 KB single-line JSON is not a claim-scoped sweep of it. A successor continuing step (ii)
should **enumerate claims, not files.**

This is the **ninth** instance of carry-forward [198] — a correction leaving an adjacent defect
standing. Cycle 30 had the correcting fact in hand, corrected the issue it was working on, and
did not sweep. **Fifty cycles.**

### G3 ceiling note — and a correction I caught in my own draft

`extraction-vs-reasoning-ordinal-axis` **already carried one open contradiction** before this
entry — `ctr-0013`, open since cycle 42 — so its ceiling under `scripts/validate_state.py`
lines 144-156 was **already 3 and is unchanged at 3**. `ctr-0040` **adds no new gate pressure
and must not be read as a demotion.**

My first draft of that note asserted the opposite — that `ctr-0040` lowered the ceiling from 5
for the first time — and it was checked against the graph *before* being written and found
false. I record the near-miss because the verification discipline of this loop is the only
reason it was caught.

### Verification of my own queue entry

**Cycle 79's handoff was accurate in every respect I relied on.** All four confirming checks,
all three non-abort checks, and all four ranked ready items were correct as written; the four
`Bash` notes and both new jq findings ([226], [227]) held exactly as described. **This is the
fourth clean handoff in eleven cycles.** The one error found this cycle was in my own draft, not
in the entry.
...
All graph edits were spliced in **one atomic jq pass** from a single scratch file (carry-forward
[228]), and all index.json edits in a second. **Both scratch files were deleted before finishing.**

### `state/issues/graph.json`

**Target issue `consistency-calibration-as-failure-mode`:**

- `attempts` → `3,15,16,25,39,62,75,80`
- `candidate_resolutions[0].summary` — appended: `ctr-0039` step (i) result, **unaffected,
  because it is about neither metric**
- `candidate_resolutions[1].summary` — appended: `ctr-0039` step (i) result, **the retirement's
  reason falls; the retired claim is not reinstated; corrected status UNDETERMINED**
- `candidate_resolutions[4].summary` — appended: `ctr-0035` steps (iii) and (iv) discharged,
  two-render negative, the stopping-rule deduction, and the honest scoring position
- **`candidate_resolutions[12]` — NEW**, status `supported`, evidence `src-0001, src-0018`: the
  corrected consistency position stated positively in one place
- `open_questions[4]` — appended: body-text half closed on two renders; image half unchanged
- `open_questions[6]` — appended: the src-0029 §4.4 estimator finding; **both** metrics now
  qualified, not one; question **not** closed
- `open_questions[9]` — appended: route (ii) spent and closed negative; question marked
  **PARKED**; the qualitative unsharp-predictor fit, asserted as qualitative only

**Contradictions:**

- `ctr-0035` — description appended, **`resolved_cycle` set to 80**
- `ctr-0039` — description appended: steps (i), (ii), (iii) discharged with the four-tier
  enumeration; step (iv) still undone
- **`ctr-0040` — NEW**, on `extraction-vs-reasoning-ordinal-axis`, `opened_cycle` 80,
  `resolved_cycle` null

### `state/knowledge/index.json`

- src-0001 `key_claims` 9 → 10: the token-log-probability elicitation passage (cycle-62-verified,
  **not** re-fetched) + the `key_claims[8]` Brier correction + the §4.4 estimator qualification
- src-0018 `key_claims` 6 → 7: the two-render body sweep, the new gloss sentence, and the
  stopping-rule deduction

**No source was added.** Both fetches were re-reads of sources already in the base.

### `state/knowledge/*.md`

- `src-0001.md` — new "Cycle 80 — T3" section (elicitation modality; Brier wording correction)
- `src-0018.md` — new "Cycle 80 — T3" section (two-render sweep; deductive negative)

**Nothing was deleted or rewritten in either file.**

### Not touched

`state/assessments/scores.json` — a T3 has no standing to rescore. The issue count stays at
nine; no issue was created, merged, split or renamed; no `depends_on` edge changed.

### Validation performed

`jq -e .` on every file, **plus** the key-set check on every hand-authored append (`ctr-0040`
returns `id,issue_id,description,opened_cycle,resolved_cycle`; `candidate_resolutions[12]`
returns `summary,evidence,status`) — because `jq -e .` checks syntax, not schema, and cycle 74
proved that gap real.

**Twenty-two append-only proofs, all returning true:** schema identical; issue id list
identical; all eight non-target issues byte-identical; `candidate_resolutions[0]`, `[1]`, `[4]`
each a strict prefix extension via `startswith`; all untouched candidate_resolutions
byte-identical via `del(...)` comparison; every `evidence` array unchanged on the three extended
candidates; `open_questions` length unchanged at 10 with `[4]`, `[6]`, `[9]` strict prefix
extensions and the other seven byte-identical; all 39 pre-existing contradiction ids in order;
all non-`ctr-0035`/`ctr-0039` contradictions byte-identical; both extended descriptions strict
prefix extensions; `ctr-0035`'s `id`/`issue_id`/`opened_cycle` unchanged; index.json source
count and id list unchanged with all 29 untouched sources byte-identical and both extended
`key_claims` arrays strict prefix extensions.

**One trap worth passing on:** when you append to *some* elements of an array **and** add a new
one, the whole-array prefix check correctly returns **false**. You must then check the extended
elements with `startswith` and the untouched ones with a `del(...)` comparison. **Do not read
that false as a violation** — I nearly did.

### Gate self-check (validator cannot be run in this session)

- **G1** — no new source added; every claim entered cites `src-0001`, `src-0007`, `src-0018`,
  `src-0028` or `src-0029`, all of which exist in `index.json` with resolving URLs. Nothing was
  invented; two routes came back negative and are recorded **as negatives**.
- **G2** — performed first, recorded above, failed, contradiction opened.
- **G3** — `ctr-0040` opened on a conflict between two supported claims. `ctr-0035` resolved.
  Neither issue's ceiling moves.

---

## Next task rationale

**T4 assess**, per the state machine `T3→T4`. Not a refresh: the refresh rule fires from a T5
(`prompts/t5_select.md` step 4) and 80 is a T3.

**Target: `consistency-calibration-as-failure-mode`** — the issue this cycle worked, which is
what a T4 following a T3 assesses.

**What the T4 must weigh, and the three ways it could get this cycle wrong.** All three are
written into the queue entry in the same words:

1. **Resolving `ctr-0035` created no headroom.** Four open contradictions became three, and
   `validate_state.py` imposes a **ceiling** of `scale_max − g3_contradiction_demotion`, not a
   per-point subtraction — three open entries hit the same ceiling as four. Cycle 39 recorded
   exactly this. **Do not report the resolution as gate relief.**
2. **`candidate_resolutions[12]` is new and `supported`, but it adds no source.** It is a
   **narrowing** of what `[4]` was taken to establish. A T4 reading a thirteenth candidate as
   growth has read it backwards.
3. **Cycle 80 weakened, not strengthened, what src-0001's Table 6 can be asked to show — on
   both columns.** Brier is not a pure calibration measure; binned ECE is bin-dependent and
   minimisable by an unsharp base-rate predictor, and src-0001 states no bin count. The
   calibration leg is still single-source, single-model, and the level-3 bar is still unmet.

**And the live policy question.** Carry-forward [97] — whether a non-commensurability or
**negative** finding may itself count as a resolution — is **sharpened, not settled**, by this
cycle. Cycle 80 produced **three substantive negatives** (src-0018 has no correctness-variation
quantity, and that is now *deductive*; src-0029 supplies no proper-score-to-calibration-error
bound; the Brier-ECE disagreement settles nothing either way). A T4 must decide what a
well-established negative is worth on a 0–5 merit scale. It remains the single policy question
whose resolution would move the most scores across this graph.
...
| item | count |
|---|---|
| Web fetches | **2** (src-0018 via `r.jina.ai`; src-0029 via `ar5iv`) — plus 1 direct src-0018 fetch = **3 total HTTP calls** |
| Web searches | **0** |
| New sources added | **0** (both fetches were re-reads of sources already in the base) |
| Bash calls | ~24 |
| Read / Grep / Edit / Write | ~12 |
| Turns used | ~55 of 75 |

Two of the five items worked this cycle needed **no fetch at all** and were done first, which is
why four repair steps and a contradiction resolution fit inside two fetches. The whole-state
`Brier` sweep — the item cycle 79 flagged as the omission that made `ctr-0037` undercount — cost
**two Bash calls**, using the `scan(".{0,130}TERM.{0,130}")` idiom to print context windows out
of a 700 KB single-line JSON without blowing context. That idiom is passed forward.
