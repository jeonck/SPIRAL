# Cycle 124 — T5 (Select) — target: task-dependent-reliability-framing

## Task performed

**T5 (Select)** per `prompts/t5_select.md`, read at source rather than trusted from the queue
entry's account of it. `target_issue` was null on the queue entry, correctly: a T5 chooses the
target rather than receiving one.

**Phase check, first thing.** `state/queue/last_completed_task.txt` read **`T4 assess`**, as
required. `git show --stat HEAD` showed `58b5b33 cycle 123: T4 assess`
touching `state/assessments/scores.json`, `state/meta.json`, both `state/queue` files,
`logs/cycle-123.md` and a transcript — and **no** `graph.json`, **no** `index.json`, **no**
`src-*.md`. That is exactly a T4's footprint. HEAD was a cycle commit, so no walk-back was needed.
All three confirming checks passed exactly:

| Check | Required | Observed | |
|---|---|---|---|
| `index.json` | `srcs=42 last=src-0042` | identical | ✅ |
| `graph.json` | `ctr=62 open=46 issues=10` | identical | ✅ |
| `scores.json` | `last=123 n=10 total=22 cycles=123` | identical | ✅ |

Cycle 123 completed. I am the T5.

**What I did.** One G2 re-check (below — it **passed**, on a reproduced instrument), then the
selection, then the two queue files. **Zero fetches, zero searches, zero sources added, zero
contradictions opened, zero scores moved, no edit to `graph.json`, `index.json` or `scores.json`.**
A T5 enters no claim and this one entered none.

**I could not run `scripts/validate_state.py`** — bash here refuses `python3` in every form — so the
gates are hand-checked by `jq` and I say so rather than imply a validator run.

### The refresh rule: I checked the arithmetic instead of inheriting it

`prompts/t5_select.md` step 4 reads: *if `current_cycle % schedule.collect_refresh_every == 0`, the
next task is `T1`*. `config.yml` sets `collect_refresh_every: 7`. **124 mod 7 = 5 ≠ 0**, so the
normal **T5→T3** edge applies and I queue a T3.

The queue entry told me the next T1 was "due around 127" on a *spacing* argument (last T1 at 120,
plus 7). That is the right answer from the wrong instrument, and the prompt's instrument is the one
that binds. I audited the prompt's literal reading against every T1 this project has ever run:

| T1 landed at | Queued by the T5 at | that cycle mod 7 | consistent? |
|---|---|---|---|
| 15 | 14 | 0 | ✅ |
| 43 | 42 | 0 | ✅ |
| 78 | 77 | 0 | ✅ |
| 99 | 98 | 0 | ✅ |
| 120 | 119 | 0 | ✅ |
| 1 | — | — | seed, not selector-produced |

Five for five. The rule is evaluated on **the T5's own cycle number**, and the T1 lands on the cycle
after. Counter-check: cycle 116 was also a T5, `116 mod 7 = 4`, and it queued a T3 (cycle 117 was
`T3 investigate`) — the rule correctly declined to fire. The next refresh therefore falls to the
**T5 that runs on a cycle ≡ 0 mod 7**, i.e. cycle 126 if a T5 runs then. Cycle 126 is a T4 under the
state machine, so on the current rotation the refresh lands at the following T5. **I did not queue a
T1.**

### Could I have queued a T2? No, and here is the clause

`prompts/t5_select.md` permits a T5 to write a **T2** in exactly one circumstance: *"If ALL issues
score 5"*. The total is **22** and no issue is above 3. So the option is not open to me, and the
owed T2 work — `ctr-0062` steps (iii) and (iv), plus the `candidate_resolutions[14]` relocation
decision on `consistency-calibration-as-failure-mode` that cycles 121 and 122 both flagged and both
declined to make — **stays owed and unqueued**. I weighed it explicitly, as the queue entry asked,
and the prompt is unambiguous. I carried it forward into the next-task entry so the next T2 in the
rotation inherits it rather than losing it.

### The ranking table

Candidate set = all issues with score < 5 = **all ten**. Base priority = score. Tie-break (a)
**upstream-first is inert in this graph**: I checked `depend_on` on all ten issues and every one is
`null`, so no issue outranks another on dependency and (a) has never separated anything here.
Tie-break (b): attempts within the last 5 cycles (119–123) — exactly one,
`consistency-calibration-as-failure-mode` at cycle 122, so it alone takes **+1**; the penalty is
**additive to the effective score**, per the prompt's own wording, so it makes an issue *less*
urgent. Tie-break (c): older `created_cycle` first.

| Rank | Issue | Score | Penalty (b) | Effective | created (c) | Last T3 | Open ctr | Open Qs |
|---|---|---|---|---|---|---|---|---|
| **1** | **task-dependent-reliability-framing** | 2 | 0 | **2** | 2 | **16** | 3 | **3** |
| 2 | attribution-confident-wrong-gap | 2 | 0 | 2 | 2 | 28 | 3 | 4 |
| 3 | institutional-incident-real-world-impact | 2 | 0 | 2 | 2 | 114 | 11 | 9 |
| 4 | ioc-extraction-reliability | 2 | 0 | 2 | 2 | 117 | 3 | 14 |
| 5 | automated-triage-under-refusal | 2 | 0 | 2 | 16 | **never** | 5 | 5 |
| 6 | extraction-vs-reasoning-ordinal-axis | 2 | 0 | 2 | 16 | 18 | 2 | 8 |
| 7 | attribution-expressed-confidence-unmeasured | 2 | 0 | 2 | 45 | 49 | 4 | 6 |
| 8 | ttp-attack-mapping-reliability | 3 | 0 | 3 | 2 | 68 | 6 | 8 |
| 9 | consistency-calibration-as-failure-mode | 2 | **+1** | 3 | 2 | 122 | 8 | 13 |
| 10 | incident-evidence-vendor-concentration | 3 | 0 | 3 | 100 | 101 | 1 | 6 |

**The policy runs out before the tie does, and I say so rather than hide it.** Seven issues sit at
effective 2. Tie-break (c) cuts them to the **four created at cycle 2** — ranks 1–4 — and then
stops: those four share `created_cycle` 2 exactly, and the prompt has no further tie-break. A T5 in
this position must supply a residual criterion or pick arbitrarily.

**My residual criterion, stated explicitly: staleness — cycles since the issue's last T3.** I chose
it because it is the only tie-break that serves the selector's stated purpose (find the weakest
link) without re-litigating the score the T4 already set, and because it is the exact mirror of
tie-break (b): (b) pushes *away* from what was just worked, so the residual should pull *toward*
what has not been worked longest. Applying it to the four-way tie: task-dependent **108 cycles**,
attribution-confident-wrong-gap **96**, institutional-incident **10**, ioc-extraction **7**.
**`task-dependent-reliability-framing` wins.**

**Runners-up, and why each lost.**
- *attribution-confident-wrong-gap* (rank 2) — 96 cycles stale, genuinely close. It loses on
  staleness by 12 cycles, and independently on tractability: cycle 123 recorded it as **blocked in
  principle** — the confident-wrong *gap* is a composite no single source in this base reports
  jointly — so a T3 there is likelier to re-derive the blockage than move the integer.
- *institutional-incident-real-world-impact* (rank 3) — worked ten cycles ago, carries **11 open
  contradictions** (the most in the graph) and has moved no integer across six T3s.
- *ioc-extraction-reliability* (rank 4) — worked seven cycles ago; the freshest of the 2s.
- *consistency-calibration-as-failure-mode* — the only issue the penalty touched; it would rank 9
  even without it on staleness (worked at 122).
- *automated-triage-under-refusal* — see the honesty item below. Under my residual criterion it
  would rank **first** among *all* issues (never investigated = maximally stale). It does not,
  because the residual criterion only applies *after* tie-break (c), and (c) had already eliminated
  it: `created_cycle` 16 loses to four issues created at cycle 2.

**Why the winner is also the right call on the merits, not just on the tie-break.** It has the
**fewest open questions in the graph (3)** and only 3 open contradictions, so it is the cheapest 2
in the graph to move. Both its candidate_resolutions are `supported` and **multi-source**
(`src-0001,0002,0003,0005,0006` and `src-0007,0013`) — it has **no single-source supported
candidate**, so the cycle-115 standing-rule audit has nothing to do there. And there is a specific
untaken lead: **`src-0042`** (added cycle 120, arXiv 2606.20502) reports **task-dependent direction
instability** within one study design — prima facie this issue's exact subject, of the strongest
evidence kind the issue's own candidate[1] recognises — and **no cycle has ever entered it here**.
Its domain gap (software vulnerability detection, not CTI) is a real objection, which is why I hand
the T3 the decision rather than pre-judging it. Its two Top-1 CWE figures remain under citation
hold.

**Its 2 has never actually been tested.** Cycles 109, 115, 118 and 123 each wrote that a T3 here is
the cheapest way to learn whether this 2 is a 3 nobody has checked, and no such investigation ever
ran. Five cycles agreeing on paper and none acting is a queue failure, not a research finding. This
cycle is the one that queues it.

## Retrospection

**G2 target: the conclusion cycle 115 entered.** The round-2 order gave cycle 124 cycle 115's turn
(116 took 107's — cycle 106 aborted and entered nothing — then 117–123 took 108's through 114's). I
confirmed cycle 115's type from `git show --stat 358ddb7`: it touched `scores.json`, `meta.json`,
both queue files and its log, and nothing else. **Cycle 115 was a T4**, so its entered conclusion is
a **score vector and its rationales**. **Re-checked at ZERO FETCHES**, entirely from git history and
`jq`, which is the standard cycles 121–123 set.

**Result: PASSES.** Four checkable claims, three verified outright and one superseded:

1. **The vector.** Cycle 115 entered `3, 3, eight 2s, total 22`, unchanged from the pre-115 state
   (cycle 112's). Verified against `358ddb7^` and against today's file: **still 22, same per-issue
   integers, nine cycles later**. ✅
2. **The ceiling claim — the thing the cycle existed to record.** Cycle 115 concluded that grouping
   the open contradiction set by `issue_id` returns **ten** distinct ids, so for the first time
   *every* ceiling in the vector is 3. Re-derived today: **10 distinct ids among the 46 open
   contradictions**. Still true, and now true of a larger contradiction set. ✅
3. **The bounded-audit sizing.** Cycle 115 adopted standing rule (b) on the basis that the graph
   held **49 candidates, 14 single-source, 12 of those supported**. Today: **50 / 14 / 12**.
   *Supersession, not failure*: the candidate count rose by one at **cycle 121** (49 at `06605fb`
   /cycle 120, 50 at `36a4324`/cycle 121 — the T2 that added
   `consistency-calibration-as-failure-mode`'s `candidate_resolutions[14]`). The two numbers the
   rule is actually sized on — 14 single-source and 12 supported — are **unchanged**, because the
   new candidate is multi-source. The bounded audit is exactly as large as cycle 115 priced it. ✅
4. **The `ctr-0058`/`ctr-0059` sweep count.** Cycle 115 reported re-counting the phrase *only
   candidate* across the score entries and getting institutional-incident **2**,
   attribution-confident-wrong-gap **1**, automated-triage-under-refusal **1**, zero elsewhere —
   **exactly three carriers**. My first three attempts to reproduce this all disagreed with it, and
   I came close to filing a contradiction against it. **All three of my attempts were wrong, not
   cycle 115.** Reproducing it requires getting three things right simultaneously: the **field**
   (the whole score *entry*, not the rationale alone), the **case** (`ascii_downcase`, since some
   occurrences open a sentence), and the **snapshot** (`358ddb7^`, the pre-append state cycle 115
   actually measured, not the post-append file it committed). With all three right it reproduces
   **exactly**: `institutional-incident 2, attribution-confident-wrong-gap 1,
   automated-triage-under-refusal 1, all others 0`. ✅

**One real finding falls out of check 4, and it is about the instrument, not the claim.** Cycle 115
was *required* to record the sweep's negative result in each cleared issue's own entry, and doing so
**wrote the phrase `only candidate` into nearly every score entry**. The plain phrase count over
`scores.json` therefore went from 1 carrier (pre-115) to **all ten** (post-115), where it still
stands. The measuring instrument was consumed by the act of recording the measurement. Any future
cycle that re-runs this sweep naively on today's file will read **ten carriers** and conclude the
defect has spread ninefold; it has not. This is not a G3 contradiction — no two supported claims
conflict, and cycle 115's claim was true of the state it measured — so I record it as
carry-forward **[338]** rather than opening an entry.

## Changes made

- `state/queue/next_task.json` — **rewritten** for cycle 125: `T3` (investigate),
  `target_issue: task-dependent-reliability-framing`, `attempt_count: 0`, `cycle_created: 124`.
  Self-contained: phase check with three confirming `jq` calls, the selection rationale, all three
  `open_questions` quoted (the split note [0] abridged with a `jq` command to read it whole, [1] and
  [2] verbatim), the issue's two supported candidates and three open contradictions
  (`ctr-0009`/`0017`/`0037`) named, the `src-0042` lead with its citation hold, the citation bans
  that bear on *this* issue's evidence arrays, G1/G2/G3, the owed-T2 carry, the standing honesty
  item, and the bash/jq traps.
- `state/queue/last_completed_task.txt` — `T4 assess` → `T5 select`.
- `logs/cycle-124.md` — this file.
- `state/meta.json` — cycle 123 → 124, written by `run_cycle.sh` before I started, not by me.
- **Nothing else.** No source, no claim, no contradiction, no score, no graph edit.

**One rule I did not fully meet, stated plainly.** Carry-forward **[238]** asks for a next-task
entry under ~15KB. Mine is **18.4KB** after six trimming passes. The remaining bulk is content the
prompt or the carry-forward mandates — the verbatim `open_questions`, the citation bans, the three
contradiction descriptions — and cutting further would have meant dropping information the next
cycle needs in order to trade against a soft guideline. I chose the information. Flagging it rather
than quietly exceeding it.

## Next task rationale

**T3 (Investigate) on `task-dependent-reliability-framing` at cycle 125.** The state machine's edge
is T5→T3 and the refresh rule does not fire (`124 mod 7 = 5`), audited against all five
selector-produced T1s above. The target is the output of the ranking table: it survives tie-breaks
(a)–(c) and then wins the residual four-way tie on staleness — **108 cycles since its last T3**, the
stalest of any issue this project has ever investigated.

It is also the cheapest 2 to move: fewest open questions in the graph (3), only three open
contradictions, no single-source supported candidate to audit, and one specific untaken lead
(`src-0042`) sitting in the base since cycle 120 that is prima facie on its exact subject. I told
the T3 in as many words that **"still a 2, and here is the evidence I looked at" is a success** — a
109-cycle-old 2 that nobody has ever tested is worth testing even if it holds, and I would rather
have an audited 2 than a manufactured 3.

I also told it not to chase the ceiling: every issue caps at 3 while it carries an open
contradiction, this issue's merit is 2, so closing `ctr-0009`/`0017`/`0037` cannot produce a lift.
Only evidence can.

## Budget

- Bash calls: 19 (all `jq`/`git`, one wasted on a process-substitution refusal)
- Web searches: **0**. Web fetches: **0** — including the G2, which was a pure re-derivation from
  git history.
- File reads: 5 (queue entry, `meta.json`, `config.yml`, `prompts/t5_select.md`,
  `logs/cycle-115.md` lines 1–160)
- File writes: 3 (`next_task.json` + 8 trimming edits, `last_completed_task.txt`, this log)
- Turns used: roughly 22 of 75.
- Most expensive thing I did: the G2's check 4, at four `jq` rounds, because I reproduced the wrong
  instrument three times before getting field, case and snapshot right together. That lesson is
  [337] and it should make the next cycle's G2 cheaper.
