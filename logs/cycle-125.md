# Cycle 125 — T3 (Investigate) on `task-dependent-reliability-framing`

## Phase check

`state/queue/last_completed_task.txt` read `T5 select`. `git show --stat HEAD` returned
`57732b0 cycle 124: T5 select`, touching `logs/cycle-124-transcript.txt`, `logs/cycle-124.md`,
`state/meta.json`, `state/queue/last_completed_task.txt`, `state/queue/next_task.json` — and
**nothing else**. No `graph.json`, no `index.json`, no `scores.json`, no `src-*.md`, which is what a
T5 should look like. Phase confirmed as **T3**, from the file and the diff rather than from the
commit message.

All three confirming checks passed, hand-derived by `jq` (`scripts/validate_state.py` cannot be run
from this harness — [309]; **no validator was run this cycle and nothing below should be read as
implying one was**):

| check | expected | observed |
|---|---|---|
| `index.json` | `srcs=42 last=src-0042` | `srcs=42 last=src-0042` ✅ |
| `graph.json` | `ctr=62 open=46 issues=10` | `ctr=62 open=46 issues=10` ✅ |
| `scores.json` | `last=123 n=10 total=22` | `last=123 n=10 total=22` ✅ |

---

## Retrospection

**Target, derived not inherited.** The round-2 order sends cycle N to the conclusion entered by the
next unexamined predecessor. Cycle 124 took 115's, so **116 is next in line**. `git show --stat
218abd2` confirms `cycle 116: T5 select`, touching only its log, transcript, `meta.json` and the two
queue files. **Cycle 116 was a T5, so the conclusion it entered is its selection plus its ranking
table** — re-checkable entirely from the state as it stood at `218abd2^`.

**Route: zero fetches.** Every input re-derived with `git show 218abd2^:state/...` and `jq`. **No web
fetches were spent on G2**, which is the standard cycles 121–124 set.

### Re-derivation of cycle 116's ranking table

| input | cycle 116's table | `git show 218abd2^:` | match |
|---|---|---|---|
| score vector | ttp 3, vendor-conc 3, other eight 2, `last=115` | identical, total 22 | ✅ |
| `depends_on`, all ten | as tabulated (5 empty, 5 populated) | identical | ✅ |
| `created_cycle`, all ten | 2/2/2/2/45/2/16/2/16/100 | identical | ✅ |
| `ioc` attempts | `9,21,35,52,65,84,93,108` | identical | ✅ |
| `consistency` attempts | ends `…,96,111` | identical | ✅ |
| `institutional-incident` attempts | ends `…,104,114` | identical | ✅ |
| `automated-triage` attempts | *(empty)* | *(empty)* | ✅ |

Every derived step also holds. Step 2 eliminates `ttp` and `incident-evidence-vendor-concentration`
on score 3. Step 3(a) leaves exactly the four issues with an empty `depends_on` — `ioc`,
`consistency-calibration`, `institutional-incident`, `automated-triage` — and correctly demotes the
four with populated arrays (`attribution-confident-wrong-gap` → cc; `attribution-expressed-confidence`
→ attr-gap + cc; `task-dependent-reliability-framing` → five upstream ids; `extraction-vs-reasoning`
→ task-dependent). Step 3(b) with window 111–115 penalises `consistency` (T3 at 111) and
`institutional-incident` (T3 at 114) and nobody else; `ioc`'s most recent attempt, 108, is outside
it. Step 3(c) then eliminates `automated-triage` on `created_cycle` 16 against 2, leaving
**`ioc-extraction-reliability`** — which is what cycle 116 queued. The refresh derivation is also
right: `116 % 7 = 4 ≠ 0` → T3.

**Downstream confirmation.** `ioc`'s `attempts` in today's graph reads `…,93,108,**117**`. The T3
cycle 116 scheduled ran, on the issue it selected.

**Verdict on the G2 target: PASSES.** Cycle 116's selection, its full ten-row ranking table and its
refresh arithmetic all re-verify against the state they were computed from. **No contradiction
opened against cycle 116.**

### A second finding, which is not about cycle 116 and is kept separate from it

Re-deriving the same table with the same instrument exposed a defect in **cycle 124's** selection —
the one that queued *this* cycle. Per **[337]** (which cycle 124 itself wrote) I reproduced the
instrument before calling anything wrong, and the mismatch is in the **field name**:

- The schema field, confirmed from `jq -r '._schema'` in `graph.json`, is **`depends_on`**.
- Cycle 124's queue entry and its carry-forward **[339]** state: *"upstream-first is inert —
  `depend_on` is `null` on all ten issues, verified this cycle."*
- **`.depend_on` (singular) is absent from every issue, so it returns `null` ten times.**
  `.depends_on` is populated on **five of ten**. Cycles 107 and 116 both used the correct field.

Applying `prompts/t5_select.md` mechanically at cycle 124 with the **correct** field: step 2 leaves
the eight at score 2; 3(a) leaves `{ioc, consistency, institutional-incident, automated-triage}`;
3(b) penalises `consistency` (T3 at 122, inside the 119–123 window) to 3; 3(c) eliminates
`automated-triage` on `created_cycle` 16; leaving a **two-way** tie between `ioc-extraction-reliability`
and `institutional-incident-real-world-impact`. **`task-dependent-reliability-framing` is not in the
surviving set — it carries the longest `depends_on` in the graph (five ids) and is eliminated at
3(a), exactly as cycles 107 and 116 eliminated it.**

So the issue I was assigned was not the stated policy's output. **I executed the assigned T3
anyway.** Switching targets unilaterally would have been a worse failure than the one I found: it
would put a second undocumented deviation on top of the first, and the work below is real work on a
real issue that had gone 109 cycles without investigation. But the record should say plainly that
**the selection that produced this cycle was not the policy's output**, and the T5 at cycle 127 must
not repeat the query. Filed as **[340]**.

This also **partly falsifies [339]**, written by cycle 124: rung (a) is not dead, and the residual
tie at cycle 124 was two-way, not four-way. [339]'s *request* — that a human rule on a fourth rung —
still stands, because a two-way tie is still a tie the prompt cannot break. Its *evidence* must be
re-derived.

**Not filed as a G3 contradiction.** G3's test is a conflict between two *supported claims in the
issue graph*. Cycle 124's selection is a log-and-queue artefact, not a candidate resolution, and the
precedent is cycle 124's own handling of the instrument-decay finding as carry-forward **[338]**
rather than as an entry. Carry-forward, plus a prominent block in the next queue entry, is the right
instrument.
...
T3 (Investigate) on **`task-dependent-reliability-framing`**, per `prompts/t3_investigate.md` read at
source. This is the **first T3 on this issue since cycle 16** — 109 cycles. Its `attempts` array now
reads `[6, 16, 125]`.

I was told a defensible *"still a 2, and here is the evidence I looked at"* would be a success and
not to manufacture a lift. **I think the evidence I found makes the issue's claim narrower, not
stronger**, and I say so below rather than dressing it up. I took no position on the score itself —
that is cycle 126's call, and I gave it both sides in the queue entry.

### 1. The owed repair to `candidate_resolutions[1]` — `ctr-0009` (i)/(ii), `ctr-0037` (i)/(ii)/(iv)

Three open contradictions on this issue each reserve repair steps to *"a T3 targeting
task-dependent-reliability-framing"*. Nobody had ever been that T3. I discharged five of them.

**Method choice, recorded as a judgement call.** `ctr-0009` (i) says *"rewrite candidate 2"*.
`ctr-0037` says candidate[1] *"IS LEFT STANDING UNALTERED per the append-only rule"*. The validator
enforces append-only on `index.json` sources and `key_claims` **only** — `graph.json`
`candidate_resolutions` are not protected (confirmed by reading `scripts/validate_state.py`). I
**appended** a headed `[CYCLE-125 REPAIR BLOCK]` that explicitly withdraws what it withdraws and
governs where it disagrees with the text above it, rather than deleting the cycle-16 wording. This
follows the base's own practice (the `CYCLE-79 UPDATE` appended inside `ctr-0037`) and keeps the
history readable. It is a departure from `ctr-0009` (i)'s literal verb and it is flagged as one
inside the block.

What the block does:

- **(A)** withdraws the false warrant *"holding team, corpus, models and harness constant and
  varying only the sub-task"* — a within-study design does not hold the **scoring rule** constant.
- **(B)** withdraws the IoC-versus-TTP comparison **as a magnitude claim** and restates it
  qualitatively: substring-containment scoring on the IoC side against exact technique-ID set
  intersection on the ATT&CK side, with the **sign of the bias running in the direction of the
  finding**.
- **(C)** corrects the four-model IoC precision range from `0.8240–0.8846` to **`0.6944–0.8846`**,
  re-read off `state/knowledge/src-0007.md:201` rather than copied from `ctr-0037`.
- **(D)** withdraws the `1.140`-versus-`3.612` pairing (the `ctr-0008`/`ctr-0036` equivocation:
  *Attribution* means **source linking** in one rubric block and **threat-actor identification** in
  the other) and replaces it with the six-dimension block contrast, carrying all three limits —
  differently-worded anchors, an LLM judge that is one of the scored models, and [25]'s bar on
  absolute rubric levels.
- **(E)** corrects the headline magnitude. **`ctr-0037` step (iv) required re-derivation rather than
  copying, so I re-derived every ratio with `jq`.** All of them reproduce cycle 78's values exactly,
  47 cycles apart — an independent second derivation, which is worth recording as a *pass*:
  rows 2.383 / 2.263 / 2.936 / 1.947 / 2.164 / 3.168; block means 1.51883 and 3.63383; block ratio
  **2.393**; max comparable row **2.936**. The Attribution row is the only one reaching 3× and is
  precisely the row that is not like-for-like. **"Roughly a 3× spread" is withdrawn; the correct
  statement is ~2.4× on block means, 2.94× maximum on any comparable row.** The four per-model
  IoC/TTP ratios also re-derive exactly (2.957 / 2.443 / 3.706 / 3.921).
- **(F)** states and answers the counterargument in the issue text — `ctr-0009` (ii), asked for by
  cycle 22, never done. **The answer is a partial one and is written as one.** The objection defeats
  the src-0007 magnitudes and I concede that in full; it does not defeat the issue, because the
  direction is separately carried by candidate[0] via src-0006 — **but that leg has its own unread
  assumption, and I say so in the same breath rather than banking it.**
- **(G)** states what is *not* repaired, so nobody reads this as closing anything. `ctr-0009` stays
  open on step (iii); `ctr-0037` stays open on its sites (2) and (3); `ctr-0017` is untouched.

While reading the stored table I also confirmed the **[331]/[19]** six-cell anomaly visually:
GPT-4o (FT) tracks o3-mini to within 0.001 on all six *Content: Threat Actor* rows (3.964/3.964,
3.656/3.655, 3.165/3.165, 4.753/4.752, 4.731/4.731, 2.968/2.967) and nowhere else. Still unexplained;
cause still not guessed.

### 2. The untaken `src-0042` lead — decided on the merits, and it is a **complication**

`src-0042` had sat in the base since cycle 120 with no cycle ever entering it here. I read
`src-0042.md` first, then both renders of the body. Entered as **`candidate_resolutions[3]`, status
`proposed`**.

**I have to correct the framing I was handed.** Cycle 124's queue entry described `src-0042`'s
central finding as *"task-dependent direction instability — fine-tuning helping on one task and
hurting on another within one study design"*. **That is not what its KC2 shows.** The documented
direction instability runs across **model and fine-tuning corpus** (Qwen3-4B +38.5 pp under LineVul
against Llama3.1-8B −49.3 pp under Devign, both on the *same* binary-detection task), not across
sub-tasks. Asked directly whether the paper states that fine-tuning's directional effect differs
between its three tasks, **both renders return ABSENT**.

What is genuinely on this issue is two things, and they cut opposite ways:

- **Supporting.** Section VIII-B, verbatim on both renders: *"Across all 15 fine-tuned variants,
  detection and CWE understanding move independently."* Two sub-tasks moving independently under a
  common intervention is direct within-study evidence that reliability is not one uniform property.
- **Deflationary, and I think more important.** Table VI gives a within-study cross-sub-task
  contrast — non-targeted detection (VD) against targeted detection with a CWE hint (TVD) —
  confirmed on both renders for two models: CodeLlama VD 75.9 / TVD 92.3 with PV 21.2 / TPV 11.2;
  DeepSeek-R1 VD 96.1 / TVD 98.7 with PV 4.5 / TPV 0.7. **The hint raises accuracy on vulnerable
  samples and lowers it on patched ones, in both models.** That is a threshold move, not a
  competence gain — the paper's own *"calibration without comprehension"*. **This is a second
  mechanism by which apparent task-dependence can be manufactured, independent of `ctr-0009`'s
  scoring-rule mechanism, and this base had recorded only the first.** It became new
  `open_questions[4]`.

**Domain gap priced, not waved through.** `src-0042` is Linux-kernel vulnerability detection; CTI,
IoC extraction, ATT&CK and threat-report tasks are all ABSENT from it. Under the standing exclusion
rule it **may not** count toward CTI multi-source support — same status as `src-0013`. Citation hold
respected: neither disputed Top-1 CWE figure is quoted, and I deliberately gave the covered-sample
exact-Top-1 gain **qualitatively** rather than numerically, because it is a Top-1 CWE quantity of the
same disputed kind and appeared on one render only.

### 3. Both stale searches re-run — one refreshed negative, one hit

**`open_questions[2]` (last searched cycle 6, 119 cycles stale): refreshed NEGATIVE, and the
negative is the result.** Searching for an independent replication or critique of LANCE/PRISM
(`src-0003`) still returns nothing: every substantive hit was the LANCE paper itself, on
`arxiv.org/abs/2506.11325` or the authors' own camera-ready. **No third-party replication,
re-analysis or critique exists as of 2026-08-07.** Three searches across 119 cycles have now returned
the same nothing and I recorded that a fourth is unlikely to repay its cost soon.

**`open_questions[1]` (last searched cycle 16, 109 cycles stale): a real hit, which I added as
`src-0043`.** arXiv **2607.25572**, *"Mapping CVEs to MITRE ATT&CK Techniques: A Curated Gold-Set
Classifier and the Limits of LLM-Assisted Label Expansion"*. Two renders (body + `r.jina.ai` abstract).
Entered as **`candidate_resolutions[2]`, status `proposed`**.

**It does not answer the question as asked, and the source file says so at the top.** The question
asks about human-in-the-loop validation *at inference*; this paper measures whether LLM-generated
labels can replace expert-curated labels *in the training set*. The converse. What it establishes,
on both renders: LLM labels at ~0.39 agreement with experts *"provide no reliable improvement at any
expansion size and reduce rare-technique coverage at around 1,000 added CVEs"*, while *"additional
expert-curated data consistently improves performance… indicating that the classifier is limited by
label quality rather than dataset size"*. On a reasoning-heavy ATT&CK task, **the human-curated
component is demonstrably load-bearing and the model-generated substitute is not.**

Its most interesting contribution to *this* issue is the authors' **inherent-task-ceiling**
argument — that mapping a CVE to ATT&CK is *"genuinely hard and partly subjective"*, with classifier
and LLM landing at similar agreement against gold. That would be task-dependence arising from the
**ground truth** rather than the model, which the scoring-rule objection does not touch. **I refused
to let that run.** 0.39 and 0.41 are *model-versus-gold*, **not** inter-annotator agreement; no
human-versus-human figure appears on either render. It is the authors' inference from two
model-vs-gold numbers, it is single-render, and it is the reason the candidate is `proposed` and not
`supported`. It became new `open_questions[5]`.

**Render disagreements recorded, not resolved:** author list (two vs one — the *same* failure
pattern as `src-0042` limitation 2, on a different paper, which makes it a property of the render
pair), date (2026-07-23 vs 28 Jul 2026), venue (`hack.lu` present on body, ABSENT on abstract).
**A citation hold is placed on the macro-F1 decrement**, which the two renders report as **0.04** and
as **0.026** (0.177 → 0.151). Only the direction and the null verdict are citable.

### 4. `open_questions` rewritten — 3 entries → 6

`[0]` the cycle-16 **SPLIT NOTE** is preserved **verbatim and unaltered**; it is a binding scope bar,
not a question, and I treated it as one. I kept strictly inside it: nothing I entered touches the
*ordering* of sub-tasks, which belongs to `extraction-vs-reasoning-ordinal-axis`.
`[1]` and `[2]` rewritten with their refreshed findings. `[3]`, `[4]`, `[5]` are new.

**`[3]` is the one that matters and it is why I do not think this issue got stronger.** I promoted
`ctr-0009` step (iii) out of the contradiction and into the open questions so a T5 and a T1 can see
it: **`candidate_resolutions[0]` — the issue's only surviving multi-source CTI support — rests on
`src-0006`'s nine per-sub-task F1/AUC rows sharing one matching rule, and no cycle has ever read
`src-0006`'s scoring definitions.** Against a base rate where every scoring rule anyone has actually
gone and read differed from what the state assumed, that is the largest untested load-bearing
assumption on this issue. **I did not fetch it**, and the open question says plainly why: I spent the
fetch budget on the untaken `src-0042` lead and the two refreshes I was told to run, and judged that
opening a fourth front badly was worse than labelling this one clearly. **It is the next thing to
do.**

### 5. G3 — no contradiction opened, and the reasoning

I checked `src-0043` against `ttp-attack-mapping-reliability`'s six supported candidates. It
**corroborates** the *"mutually unconvertible units"* candidate rather than conflicting with it — it
is a **sixth** ATT&CK measurement unit, and from a **different input modality** (CVE descriptions,
where every other ATT&CK source in this base uses threat reports or narrative CTI). Its
version-drift limitation independently re-finds the parent/sub-technique hazard already documented in
`src-0017`'s loader, arriving from another direction. The `src-0042` and `src-0043` qualifications
are **qualifications of** supported claims, not conflicts **between** supported claims, and both new
candidates are `proposed`. G3's test is not met. Recorded as candidates and open questions, which is
where they belong.
...
- **`state/knowledge/src-0043.md`** — **new**. Two renders, four key claims, a citation hold on the
  disputed macro-F1 decrement, ten charged limitations including the render disagreements and the
  explicit statement that it has **no HITL-at-inference arm** so it narrows `open_questions[1]`
  rather than closing it.
- **`state/knowledge/index.json`** — `src-0043` appended. **42 → 43 sources.** Append-only proved by
  `jq` before installing: old array is an exact prefix of the new, top-level keys equal, ids unique,
  no `key_claims` touched.
- **`state/issues/graph.json`** — four scoped edits, all on `.issues[5]`:
  `candidate_resolutions[1].summary` **appended to** (repair block; original text an exact prefix,
  proved); `candidate_resolutions` 2 → 4 (new `[2]` on `src-0043`, new `[3]` on `src-0042`, both
  `proposed`); `open_questions` 3 → 6 with `[0]` byte-identical; `attempts` → `[6, 16, 125]`.
  Proved before installing: all ten issue ids unchanged, **`contradictions` array byte-identical**,
  **the other nine issues byte-identical**, `title`/`depends_on` unchanged, no dangling evidence
  reference anywhere in the graph against the new index.
- `state/queue/next_task.json` — rewritten for cycle 126 (T4).
- `state/queue/last_completed_task.txt` — `T5 select` → `T3 investigate`.
- `logs/cycle-125.md` — this file.
- `state/meta.json` — cycle 124 → 125, written by `run_cycle.sh` before I started, not by me.
- **Not touched:** `scores.json` (a T3 does not rescore), the `contradictions` array, any existing
  `src-*.md`, any `key_claims`.

**Rules I did not fully meet, stated plainly.** (1) **[238]**'s ~15 KB ceiling on the next-task
instructions: mine is **19.0 KB**. Cycle 124's was 18.4 KB and it flagged the same overrun. The
mandated content plus the new [340] block does not fit; I chose the information and I am flagging it
rather than quietly exceeding it. **This has now failed twice running and the ceiling or the
convention needs a human ruling.** (2) I did **not** fetch `src-0006`'s scoring definitions, the
highest-value item on this issue — recorded as `open_questions[3]` with the reason.

---

## Next task rationale

**T4 (Assess) at cycle 126.** The state machine's edge is T3→T4 and `prompts/t3_investigate.md` says
nothing to the contrary.

The queue entry gives cycle 126 the scoring judgement **two-sided and undecided**, because it is the
T4's call and not mine. The case for a lift: rubric level 4 wants *"counterarguments explicitly
addressed"*, and repair block (F) is the first explicit statement-and-answer of the scoring-rule
counterargument in this issue's history. The case for holding — which I think is stronger and told
it so — is that the repair **narrowed** the claim: the src-0007 magnitudes are withdrawn, this base
now has no established cross-study magnitude for task-dependence at all, both new candidates are
`proposed` and **non-CTI**, and the only surviving multi-source CTI support has an unread
load-bearing assumption sitting under it. An issue in that condition is not "robust".

I also gave it the refresh arithmetic to check rather than copy (T5s at 127/130/133; `133 = 7 × 19`,
so **the refresh T5 is cycle 133 and its T1 runs at 134**), the standing citation bans plus the new
`src-0043` hold, the `ctr-0037` site (3) defect that lives in **its own file**, the owed-T2 carry,
the seventeenth repetition of the standing honesty item, and — most importantly — **[340]**, so the
T5 at cycle 127 does not repeat cycle 124's `depend_on`/`depends_on` query.

## Budget

- Bash calls: 24 (all `jq`/`git`/`sed`/`mv`/`rm`; **zero wasted on a refusal** — the [227] and
  process-substitution traps were avoided by construction)
- **Web searches: 2. Web fetches: 4.** All four fetches were spent on the main task (two renders of
  `src-0042`'s body for Table VI and §VIII-B; two renders of `src-0043`). **The G2 cost zero
  fetches.** One new source added against a `max_new_sources` budget of 5.
- File reads: 8 (queue entry, `meta.json`, `config.yml`, `prompts/t3_investigate.md`,
  `prompts/t4_assess.md`, `prompts/t5_select.md`, `src-0042.md`, validator §)
- File writes: 5 (`src-0043.md`, `scratch.json`, `next_task.json`, `last_completed_task.txt`, this log)
- Turns used: roughly 40 of 75.
- **Most expensive thing I did:** the two-render verification of `src-0043`, at 2 fetches. It was
  worth it — the second render is what caught the author-count, date, venue and macro-F1
  disagreements, and the macro-F1 one would otherwise have entered the base as a citable number.
- **Cheapest high-value thing I did:** the G2, at zero fetches, which found [340].
