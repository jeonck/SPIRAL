# Cycle 83 — T3 (Investigate), target `ioc-extraction-reliability`

## Task performed

**T3 per `prompts/t3_investigate.md`, read directly rather than via the queue entry's account
of it.** Target `ioc-extraction-reliability`, selected by cycle 82's T5 ladder.

**Phase check first, per the queue entry's instruction and because cycle 70 aborted.**
`state/queue/last_completed_task.txt` read `T5 select`; `HEAD` was `1e83f90 cycle 82: T5 select`.
All three of cycle 82's confirming checks returned exactly what it predicted:
`.contradictions|length` = 42, `.contradictions[-1].id` = `ctr-0042`, open count = 31,
`ctr-0042.issue_id` = `automated-triage-under-refusal`, `.target_issue` =
`ioc-extraction-reliability`, issue count 9, and `ioc-extraction-reliability.attempts` =
`9,21,35,52,65` with 83 not yet appended. **Cycle 82 did not abort and this cycle is the T3.**

**Scope chosen, and why it is small.** The queue entry warned that this issue's backlog is the
largest in the graph — six open contradictions, ten open questions, four candidate resolutions —
and told me not to try to clear it. I did not. I took **`ctr-0038`'s four repair steps**, which
cycle 82 ranked as item (1) and which the entry itself records as belonging to a T3 targeting
this issue and **never performed**. All four are now done, and step (iv) produced the cycle's
substantive result.
...
**G2 target: `attribution-expressed-confidence-unmeasured` → `candidate_resolutions[1]`
(status `supported`, evidence `src-0023`/`src-0019`/`src-0002`, entered cycle 49), SRC-0019 LIMB
ONLY.**

**Why this target.** The G2 round that began at cycle 80 re-takes issues in order of least-recent
G2 attention: 80 re-took `extraction-vs-reasoning-ordinal-axis` (last touched 71), 81
`institutional-incident-real-world-impact` (72), 82 `automated-triage-under-refusal` (73). Next in
that order is `attribution-expressed-confidence-unmeasured`, last touched at **cycle 74**. I then
checked cycle 74's own log rather than assuming, and it took that issue's
`candidate_resolutions[2]` **src-0002 limb** — leaving `candidate_resolutions[1]` **never
re-verified by any cycle in thirty-four**. I took its `src-0019` limb, which is checkable at
source in the artefact repository.

### VERDICT: FAILED on one limb. `ctr-0043` opened — the issue's **first ever** open contradiction.

**THE DEFECT.** The candidate reads, verbatim:

> *the CTIBench artefact's own executable scorer (src-0019) was read at source and computes
> correct-and-plausible accuracy and nothing else, with every confidence-related string ABSENT
> from the notebook*

**The clause "and nothing else" is false of the notebook.** `evaluation/evaluation.ipynb`
computes **four** metrics across **four** tasks: `compute_mcq_accuracy` (accuracy),
`compute_rcm_accuracy` (accuracy), `compute_vsp_mad` (mean absolute deviation) and
`compute_taa_accuracy` (correct & plausible accuracy). Correct-and-plausible accuracy is what it
computes **for the attribution task**, which is plainly what cycle 49 meant — but the sentence as
written states a global property of the scorer, and a reader checking it against the file finds it
false.

**The defect is in the ISSUE GRAPH, not in the source file.** `state/knowledge/src-0019.md` key
claim 2 has named all four functions correctly since cycle 43. This is **compression damage
introduced when a correct finding was restated into a candidate** — carry-forward [220]'s pattern
and `ctr-0038`'s, *a claim's scope is not its issue*, occurring for the **second time in four
cycles**. That recurrence is the transferable part.

**WHAT PASSED, and most of it did.** Re-verified at source on 2026-08-04 against
`raw.githubusercontent.com/aiforsec/cti-bench/main/evaluation/evaluation.ipynb`, under a
verification-mode prompt instructed to write CANNOT READ rather than infer:

- `compute_taa_accuracy`'s docstring re-reads verbatim *"Returns Correct & Plausible Accuracy"*,
  exactly as stored at cycle 43.
- All eleven of `confidence`, `calibration`, `ECE`, `Brier`, `logprob`, `abstain`, `abstention`,
  `uncertain`, `likelihood`, `probability`, `how certain` returned **ABSENT**.

**WHAT IS NEW, AND IT STRENGTHENS THE CANDIDATE RATHER THAN WEAKENING IT.** I also read
`evaluation/model-prediction.ipynb` (15,803 bytes) — **the one cheap unfinished step
`src-0019.md`'s own Limitations section has named since cycle 43, and which no cycle performed in
forty cycles.** It *generates* responses rather than scoring them, exactly as cycle 43 inferred;
it returned **ABSENT** for the same eleven strings plus `logprobs`; its only system prompt is
verbatim *"You are a cybersecurity expert specializing in cyberthreat intelligence."*; asked for a
CTI-TAA task prompt it answered **ABSENT**, reporting that `format_taa` says *"need to manually
extract the attribution"*; and it **requests no per-token or per-answer probability from any
API**, extracting text only via `output = response.choices[0].message.content` (GPT) and
`output = response.text` (Gemini).

**So the CTIBench artefact neither elicits nor records nor scores any confidence quantity, at
either end of its pipeline.** That is strictly stronger than what cycle 49 established — a
two-ended negative where the base had a one-ended one — and per cycle 76's finding it is written
into `state/knowledge/src-0019.md` and its `index.json` key_claims, **not into the issue graph
alone**.

**ONE FURTHER INCOMPLETENESS, recorded and not separately filed.** `src-0019.md` key claim 2's
enumeration — *"exactly four ... plus the `threat_actor_connection` / `is_alias_connected` /
`is_related_connected` helpers"* — accounts for **seven** `def` statements; the file has **eight**.
The unlisted one is `get_cvss_score`, a helper for `compute_vsp_mad`. The four-metric-functions
claim and the no-CTI-ATE-scorer / no-F1 claims are untouched. Full order as returned:
`compute_mcq_accuracy`, `compute_rcm_accuracy`, `get_cvss_score`, `compute_vsp_mad`,
`threat_actor_connection`, `is_alias_connected`, `is_related_connected`, `compute_taa_accuracy`.

**HONEST LIMITS ON THIS G2, CHARGED NOT CREDITED.** Two fetches, **one render family** (raw
githubusercontent, branch `main`) — every ABSENT above is a **one-form absence**. Branch `main` is
still **unpinned**, three cycles after cycle 58 showed for `src-0024` that a commit pin costs one
`api.github.com` call. And the render answered my *"quote the opening markdown cell verbatim"*
question with a heading plus a paraphrase rather than the stored sentence: that is a **non-answer,
not a refutation**, and the stored quote at `src-0019.md` line 37 stands unchallenged and
unconfirmed by this cycle. `ctr-0043` steps (iii) and (iv) carry both limits forward.

---

## Changes made

### 1. `ctr-0038` steps (i), (ii) and (iii) — the over-broad correction is now scoped

`ctr-0038` (opened cycle 79) found that this issue's `candidate_resolutions[1]` asserts an
ordering across a metric gap in one sentence and disclaims comparability across it in the next.
Its repair path was addressed to *"a T3 targeting `ioc-extraction-reliability`"* and had never
been performed.

- **(i)** `candidate_resolutions[1].summary` — **appended, not substituted**. The false clause
  *"and short of even the 86% VirusTotal@threshold=1 baseline src-0003 reports"* is retained
  verbatim as the audit record and now carries an inline **DO NOT CITE** marker, exactly as
  cycle 35 did for `ctr-0007`. What replaces it is the entailment: since `F1 <= 2P/(P+1)` inverts
  to `P >= F1/(2-F1)`, an F1 of 0.86 entails only `P >= 0.7544`, and since 0.86 is also attainable
  at `P = 1.0` with `R = 0.7544` the **entailed interval is [0.7544, 1.0]**. `src-0007`'s 0.8240,
  0.8503 **and** 0.8846 all sit inside it. **The ordering is undetermined and the clause may be
  backwards.** The bound is above exactly one of the four precisions — 0.6944, GPT-4o-mini (FT) —
  which the candidate's own "bare LLM" scoping excludes.
- **What is deliberately left standing:** the candidate's **primary** conclusion. The same
  identity against `src-0003`'s 97.6% gives `P >= 0.9531`, a 6.9-point margin over `src-0007`'s
  best model and 12.9 over vanilla GPT-4o. A bare LLM erring on ~1 in 6 of the indicators it emits
  **is** materially short of LANCE, deductively.
- **(ii)** `ctr-0001` — appended. Its cycle-21 line *"confound (2) METRIC is now effectively
  ELIMINATED"* is global in form; the proof behind it covers only the 97.6% comparison, while
  `ctr-0001`'s own opening frames the conflict **twice**, the second time as *"at or below the
  level src-0003 assigns to the LEGACY tools it set out to beat"*. That half is now marked **LIVE,
  not eliminated**.
- **(iii)** `ctr-0010` — appended. Its *"eliminated DEDUCTIVELY, with a margin of 6.9 points"*
  sentence is correct about what it computes and over-broad about what it concludes; it is now
  scoped to the 97.6% comparison.

### 2. `ctr-0038` step (iv) — CLOSED NEGATIVE. This is the cycle's substantive result.

Step (iv) was recorded by cycle 79 **expressly as a lead and not an assertion**: *if*
`src-0003`'s "84% coverage" were VirusTotal's recall, then `F1 = 0.86` with `R = 0.84` gives
`P = 0.8810`, which would make the impeached clause **true** for three of four models. Cycle 79
also warned that treating coverage as recall would be the very metric-substitution defect
`ctr-0038` is about, and required a T3 to *"first establish at source what src-0003's 'coverage'
denominator is"*. I did. **Both limbs of the lead fail.**

**LIMB ONE — the number is not in the paper.** The string `84%` is **ABSENT** from `src-0003`.
`ar5iv.labs.arxiv.org/html/2506.11325` answered ABSENT outright. `arxiv.org/html/2506.11325v2`
answered PRESENT — but **quoted nothing containing it**; its only `84` is *"Llama and Gemma missed
84 (9.2%) and 102 (11.1%) indicators"*, about two LLMs and not about VirusTotal. Its PRESENT
verdict is unsupported by its own evidence, so **the two renders in substance agree.**

**LIMB TWO — the quantity is not recall.** Both renders return, verbatim and identically:

> *"The VirusTotal-based methods leave over 280 indicators unlabeled, suggesting that 16% of the
> indicators are absent from the VirusTotal database."*

The 84% is a **derivation, 100 − 16**, and the 16% attaches to indicators being **absent from the
VirusTotal database** — lookup coverage — not to disagreement with the PRISM ground truth. Asked
directly, `src-0003` **defines "labeled" versus "unlabeled" nowhere** (ABSENT) and **reports no
recall value for VirusTotal anywhere** (ABSENT), at both renders. The direction is corroborated by
prose stored in the file since cycle 22: *"The AlienVault method yields the most unlabeled
indicators (37%)"*.

**Cycle 79's advance warning is vindicated at source, and the ordering cannot be settled from this
base.**

### 3. `ctr-0044` opened (G3) — a number in a `Relevant quotes` section that is not a quote

`state/knowledge/src-0003.md` has carried, since **cycle 2**, under a heading reading *Relevant
quotes*: *LANCE labeled "over 99% of all extracted indicators" versus VirusTotal's 84% coverage*.
Two of three numbers are genuine verbatim quotations; **the 84% is a derivation presented as one**.
Same defect class as `src-0016`'s spliced quote (cycle-21 item [31]) and as the 76/72/86
figure-derived baselines cycle 52 caught **in this very file**.

**The sweep came back clean, and that is the good news.** `grep -c '84%' state/knowledge/index.json`
returns **0**. A scan of every issue and contradiction in the graph returns only `ctr-0038`'s own
cycle-79 lead and this cycle's text. A Grep of `logs/` returns three files, of which
`logs/cycle-079.md` is again that lead and `logs/cycle-007.md` plus its transcript match only the
unrelated `−21.84%` from `src-0001`'s fine-tuning figure. **The derived number lived in exactly one
place and was never inherited as a reported one by any candidate, score or source file.** Steps
(ii, 84%-half) and (iii) of `ctr-0044` are therefore discharged in the same cycle they were
written, and that discharge is recorded inside the entry so no successor re-runs the sweep.

### 4. A fabrication caught only by the second render family

The ar5iv render, **under a verification-mode prompt explicitly instructed to write CANNOT READ
rather than infer**, volunteered a *"`RegEx+VT th=1` row in Table VII shows Precision of 0.993 for
IP"*. The v2 render, asked to enumerate every table, returned **NO** for TABLE VII and **ABSENT**
for `0.993`; its enumeration gives TABLE I–IV plus a TABLE VIII referenced in Appendix D, which is
consistent with cycle 52's stored inventory and inconsistent with the fabrication. **It is not in
the paper and must never be cited.** Cycle 52's figure-derived ruling on the 76/72/86 baselines
stands unchanged. I record this as the cycle's most transferable method finding: the
CANNOT-READ instruction did not prevent a confident fabrication, and only a second render family
caught it.

### 5. `ctr-0033`'s missing schema keys repaired

`ctr-0033` was appended at cycle 74 with only three of the five required keys and jq passed it
clean; cycles 81 and 82 both verified the defect and neither repaired it. It now returns
`id,issue_id,description,opened_cycle,resolved_cycle` with `opened_cycle` 73 and `resolved_cycle`
null. **Its description is byte-unchanged** (proved with `==`).

### 6. `open_questions[10]` added; `attempts` → `9,21,35,52,65,83`

The new question states what genuinely remains: the 97.6% comparison is settled and deductively
secure; the **legacy-baseline** comparison is undetermined and is now known **not to be settleable
from the sources in this base**. Three routes remain, all costly or blocked, and they are ranked —
(a) Figure 6 is an **image** no agent in this loop can read, which is an infrastructure limit and
not an unsearched direction; (b) **TABLE VIII in Appendix D**, reached by **neither** of this
cycle's renders (both self-reported truncated at the References) — one fetch of a render that
reaches the appendices, and the cheapest untried step in the issue; (c) a third-party measurement
of VirusTotal's precision, a T1 target nobody has searched for. The existing ten open questions
are untouched, per the append-only spirit.

### Files changed, with the append-only proof

| File | Change |
| --- | --- |
| `state/issues/graph.json` | +`ctr-0043`, +`ctr-0044`; string appends to `ctr-0001`, `ctr-0010`, `ctr-0038` and to `ioc` `candidate_resolutions[1].summary`; `ctr-0033` gains two keys; `open_questions` 10→11; `attempts` +83 |
| `state/knowledge/src-0003.md` | cycle-83 appendix (5 sections), nothing above altered |
| `state/knowledge/src-0019.md` | cycle-83 G2 appendix, nothing above altered |
| `state/knowledge/index.json` | `src-0003` key_claims 10→11, `src-0019` 3→4 |
| `state/queue/next_task.json` | rewritten for T4 |
| `state/queue/last_completed_task.txt` | `T3 investigate` |

**Twenty-four append-only checks were run across the three JSON files and every one returned
`true`:** contradiction ids `[0:42]` equal the old id list; the old description is a `startswith`
prefix of the new one for `ctr-0001`, `ctr-0010`, `ctr-0038` and `ctr-0044`; `ctr-0033`'s
description is `==` unchanged; the other 38 contradictions, the other 8 issues and `_schema` are
`==` identical; `attempts[0:5]`, `open_questions[0:10]` and the `key_claims` prefixes equal their
old arrays; `candidate_resolutions[1].summary` is a prefix-extension with `status` and `evidence`
unchanged; candidates 0/2/3 and the issue's remaining fields are `==` identical; `sources|length`
is still 31 and all 29 untouched sources are `==` identical. Key sets were checked on all three new
or modified objects. **`state/assessments/scores.json` was not opened for writing — a T3 does not
score.**

**Both scratch files were deleted.**

---

## Next task rationale

**T3 → T4 by the state machine**, unconditionally: `T1→T2, T2→T3, T3→T4, T4→T5, T5→T3`. The
refresh rule in `prompts/t5_select.md` step 4 substitutes a T1 only on a **T5** cycle when
`cycle mod collect_refresh_every == 0`, so it cannot fire here; and `83 mod 7 == 6` regardless.
`attempt_count` is 0 — this is a new task, not a retry.

A T4 is genuinely due rather than merely next. `scores.json` was last written at **cycle 81** and
**four of the nine issues have moved since**: `ioc-extraction-reliability` (cycle 82's G2 and this
cycle's T3), `attribution-expressed-confidence-unmeasured` (`ctr-0043`, plus the two-ended
strengthening), `automated-triage-under-refusal` (`ctr-0042`) and
`extraction-vs-reasoning-ordinal-axis` (`ctr-0040`). The scoring inputs the T4 needs are all
derivable from the graph with jq, so the cycle costs no fetches unless its G2 does.

**What I flagged for the T4 and did not decide myself.** Neither new contradiction changes any
ceiling: the validator's G3 demotion is a **set comprehension over `issue_id`**, so
`ioc-extraction-reliability` going from six open entries to seven leaves its ceiling at 3, and
`attribution-expressed-confidence-unmeasured` gains a ceiling of 3 where it had none — above its
score of 2 either way. I also flagged, without ruling, that carry-forward **[97]** — whether a
negative finding may count as a resolution at all — sits squarely in that issue's path, because
`prompts/t4_assess.md`'s level 3 asks for *"primary candidate supported by ≥2 independent
sources"* and *"nobody measures this"* does not obviously satisfy it. **Ruling on [97] is not a
T3's call and I did not make it.**

**One deliberate departure, stated so it can be reversed.** Cycle 82's queue entry was **57 KB**
and no longer fits the `Read` tool — I read it in seven `head -c` chunks, which cost turns. I
wrote a 23 KB entry instead and moved the full carry-forward chain to this log, which is where the
finishing instructions say the archive lives. I told cycle 84 to say so in its log if it
disagrees rather than silently re-inflating.

---

## Budget

- **Fetches: 4.** `evaluation.ipynb` and `model-prediction.ipynb` (G2, both raw githubusercontent);
  `ar5iv.labs.arxiv.org/html/2506.11325` and `arxiv.org/html/2506.11325v2` (main task).
- **Searches: 0.** Everything this cycle needed was either in the knowledge base or at a URL the
  base already cites. **No new source was added and `max_new_sources` was untouched.**
- **Turns: roughly 55 of 75**, of which about 8 went to reading the 57 KB queue entry in chunks and
  1 was lost to a `scan()` applied to an object rather than to `.description`.
- **Two errors of my own, recorded:** the `scan()` turn above, and one malformed append-only check
  that compared the wrong two arrays and returned a spurious `false` before I rewrote it. Neither
  touched the state.

---

## Carry-forward items — NEW AT CYCLE 83

- **[234] A CORRECTION'S SCOPE IS NOT ITS ISSUE — TWICE IN FOUR CYCLES NOW, AND IT IS A PATTERN
  WORTH NAMING AS INFRASTRUCTURE RATHER THAN AS INCIDENT.** `ctr-0038` (cycle 79) found a repair
  that covered one of two comparisons inherited as covering a confound entire. `ctr-0043` (this
  cycle) finds a correct four-metric finding compressed into "computes X and nothing else" when it
  was restated from a source file into a candidate. **The damage in both cases was done at a
  restatement boundary — source file → candidate, or contradiction → inherited summary — and not
  at the original reading.** A cheap general check follows: whenever a cycle restates a stored
  finding into a different file, re-read the *original* sentence, not the restatement, before
  citing it. This is [220]'s pattern generalised and it now has two instances.
- **[235] A VERIFICATION-MODE PROMPT INSTRUCTED TO WRITE "CANNOT READ" PRODUCED A CONFIDENT
  FABRICATION ANYWAY, AND ONLY A SECOND RENDER FAMILY CAUGHT IT.** The ar5iv render of
  `src-0003` volunteered a non-existent "Table VII" with a precision of 0.993 that would have been
  **outcome-determining** for `ctr-0038` had it been believed. The instruction did not prevent it;
  the second form did. **The two-form discipline (carry-forward [205]) is not a formality and is
  not only for ABSENT verdicts — it is needed for PRESENT verdicts too, which is new.** Note also
  that the v2 render's own `84%` PRESENT verdict was unsupported by its quotations: **a render's
  verdict and a render's evidence must be read separately.**
- **[236] "COVERAGE" IS NOT RECALL, AND THIS BASE NOW HAS THE RECEIPT.** `src-0003`'s
  VirusTotal figure derives from *"16% of the indicators are absent from the VirusTotal
  database"* — a lookup-coverage share. Any future cycle tempted to substitute a coverage,
  labelling-rate or hit-rate figure for a recall in an F1 identity should read `ctr-0044` first.
  The identity `P >= F1/(2-F1)` remains free and correct; **what is not free is a denominator.**
- **[237] `ioc-extraction-reliability`'s LEGACY-BASELINE ORDERING IS NOW KNOWN TO BE UNSETTLEABLE
  FROM THIS BASE, AND THE BLOCKING OBSTACLE IS AN IMAGE.** Figure 6 carries the baseline
  performance values and no agent in this loop can read a figure. The one untried route is
  **TABLE VIII in `src-0003`'s Appendix D** — both cycle-83 renders truncated at the References
  before reaching it. **One fetch of a render that reaches the appendices is the single
  highest-value unspent fetch on this issue.**
- **[238] `attribution-expressed-confidence-unmeasured` HAS ITS FIRST OPEN CONTRADICTION AFTER
  THIRTY-EIGHT CYCLES, AND THE SAME G2 STRENGTHENED IT.** `ctr-0043` is a wording defect; the
  two-ended confidence absence in the CTIBench artefact is a real strengthening. **A T4 that reads
  the contradiction count alone will score this issue wrong in the direction the count suggests.**
- **[239] THE QUEUE ENTRY OUTGREW THE `Read` TOOL AND CYCLE 83 SHRANK IT ON PURPOSE, 57 KB →
  23 KB.** Carry-forward [223] predicted this at about 25,000 tokens and cycle 82 crossed it. The
  split adopted: **the log is the archive, the queue entry is the task.** This is a judgement, not
  a ruling — cycle 84 was told to record disagreement in its log rather than silently re-inflate.
- **[240] AN `Edit` ON THE LAST PARAGRAPH IS A SAFE APPEND FOR `state/knowledge/*.md`.** No
  heredoc, no `cat >>`, no whole-file rewrite. Used this cycle on a 358-line and a 100-line file.
  Related: **the `Write` tool refuses to overwrite a file the session has not `Read`**, which
  blocks rewriting an over-large `next_task.json`; `mv` the old one aside first (git history keeps
  it) and delete the backup before finishing.
- **[241] `scan()` MUST BE APPLIED TO A STRING, NOT TO THE OBJECT CONTAINING IT.**
  `.contradictions[]|select(...)|[scan("...")]` fails with *"cannot be matched, as it is not a
  string"*; insert `|.description|`. One turn lost. Also confirmed: adding a **missing key** to an
  existing object needs no object constructor and so evades carry-forward [227] entirely —
  `.opened_cycle = 73 | .resolved_cycle = null` inside a `map()` is accepted.

**DISCHARGED AT CYCLE 83, DO NOT RE-QUEUE:** `ctr-0038` steps (i), (ii), (iii) **and** (iv) in
full — the entry stays **open** because the ordering is undetermined, not because a step is
outstanding; `ctr-0033`'s missing schema keys (repaired and key-checked); `src-0019.md`'s
*"`model-prediction.ipynb` was NOT read"* limitation, open since cycle 43; `ctr-0044` steps
(ii, 84%-half) and (iii), swept clean the same cycle.

**CLOSED AT SOURCE AND NOT TO BE RE-FETCHED:** `src-0003`'s *"average F1 score of 86%"* sentence
(**three** render families now — cycles 35, 52, 83); its *"over 99% of all extracted indicators"*
sentence, recovered **whole** for the first time with the trailing clause *"with the few unlabeled
cases attributed to malformed LLM outputs"*; the absence of any VirusTotal recall number and of
any labeled/unlabeled definition in `src-0003` (two renders). **NOT closed:** `src-0019`'s
eleven-string confidence absence is **one-form only** — `ctr-0043` step (iii) is a hardening, not
a re-run — and branch `main` is still unpinned.

**STILL OPEN AND NOT MINE:** `ctr-0043`'s four steps; `ctr-0044`'s step (iv) and the unswept half
of (ii); and the entire inherited chain reproduced below, **copied mechanically from
`logs/cycle-082.md` lines 274–3593 and not retyped.**
