# Cycle 131 — T3 Investigate — `ioc-extraction-reliability`

Phase check passed before any work: `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed `de9b4a1 cycle 130: T5 select` touching only `state/meta.json`,
both `state/queue` files and the cycle-130 log and transcript — no `state/knowledge/`, no
`scores.json`, no `graph.json`, exactly as a T5 should. All four confirming jq checks matched the
queue entry's expected values: `ctr=65 open=49 issues=10 last=ctr-0065`, `last=129 n=10 total=22`,
`srcs=44 last=src-0044`, and the score vector with `ttp-attack-mapping-reliability=3`,
`incident-evidence-vendor-concentration=3` and every other issue at 2.

I could not run `scripts/validate_state.py` — this environment's Bash gate refuses `python3`. The
gates below were **hand-checked with jq**, not validator-run, and I say so rather than implying
otherwise.

## Task performed

T3 on `ioc-extraction-reliability` — this issue's **tenth** investigation (previous: cycles 9, 21,
35, 52, 65, 84, 93, 108, 117). **One fetch pair spent. No new source added** — a T3 adds none.

### Primary target: `open_questions[2]`, the scaffolding-versus-LLM decomposition

Cycle 117 handed this on with an explicit instruction: go to `src-0003`
(`https://arxiv.org/html/2506.11325v2`), pull the architecture/pipeline section, state which stage
does what, and find out whether the paper reports an ablation — and if it does not, record that as
an **attested absence** with the number of render forms asked. That is what I did, using the
mandated render pair (direct + `r.jina.ai`), asking for verbatim strings with an explicit ABSENT
option and an ALL-OCCURRENCES form where a count mattered.

**Four findings and one caution.**

**(A) No ablation exists in `src-0003`.** Both renders were asked directly for an ablation study,
for a component-wise LANCE number (LANCE with the LLM removed, or the LLM without the regex
stage), for any `w/o`/`without` variant table, and for **all occurrences** of the word `ablation`.
**Both answered ABSENT.** Stated limits, per this issue's own rule-(v) practice: two render forms
of **one** version (v2); both self-reported truncated but **in different places and both past the
body** — the direct render ends mid-References at `[32]`, the proxy ends mid-Appendix A.1 — so the
whole body was visible to both; no character-level search; v1 was not asked this cycle. **The
quantitative decomposition `open_questions[2]` asks for is not in the paper.**

**(B) The paper contains the right experiment shape under another name, and its number is banned.**
The **RegEx + Whitelist** baseline is the non-LLM labeller over the regex-produced candidate list,
so LANCE-versus-RegEx+Whitelist is an **implicit ablation of the LLM stage holding extraction
fixed**. The paper never calls it one, and its magnitude is uncitable: the RegEx+Whitelist F1 of
76% is **Figure-6-derived and absent from the body text at both v1 and v2** (cycle 52, two renders,
asked directly). The implicit ablation yields **a direction, not a number**.

**(C) The direction — and it inverts the question's own framing.** Two-form verbatim, identical in
both renders, and **never before held in this base**:

> For each segment, we provide the LLM with the segment text along with a list of all extracted
> indicators (of the target type) from that segment.

> We then prompt it to classify each indicator as IoC or nonIoC and provide a justification for its
> label.

The LLM is handed an **enumerated candidate list** and asked to **classify** it, so it contributes
**no recall whatever**; recall is wholly the regex stage's, which the paper states as *"Regular
expression-based tools are highly effective in achieving high recall for IoC extraction, but
struggle with precision."* (direct render; the proxy returned only the leading clause — its string
is a **prefix** of the direct's, not a disagreement). The architecture sentence, two-form
identical, describes LANCE as *"a tool that automatically extracts and labels Indicators of
Compromise (IoCs) from unstructured threat reports."*

So the answer is **not** "the scaffolding rather than the LLM." Within `src-0003` the scaffolding is
**common to LANCE and to the baselines it beats**, so it cannot be the source of the *gain*. What it
does is **redefine the task** — from recall-generation to precision-filtering over an enumerated
list — and on that redefined task the LLM is the only thing LANCE substitutes. This is entered as
**new `candidate_resolutions[4]`, supported, scoped strictly to `src-0003`'s own architecture**.

This **strengthens** rather than merely repeats cycle 22's classification finding: cycle 22 had the
*output*-side evidence (Figure 9's "IoC Classification" caption, the 1,789-candidate sentence);
cycle 131 has the *input*-side prompt sentence.

**Caution, and it is why (B) and (C) are not stronger than they are.** Asked whether Section
5.1.1's compared methods all label the **same** candidate set, the two renders **disagreed**: the
direct render said the 1,789-candidate sentence "establishes a shared baseline"; the **proxy
answered ABSENT**, correctly noting that no sentence states it and that AlienVault and VirusTotal
leave different fractions unlabelled. The shared-denominator reading is therefore an **inference**
from three sentences this base already held (the 1,789-candidate sentence, AlienVault's "most
unlabeled indicators (37%)", and VirusTotal's "over 280 indicators unlabeled … 16%") — an
unlabelled *fraction* is only countable against a shared enumerated denominator. **It is a strong
inference and it is not a quotation, and it is labelled as one everywhere it is used** — the same
discipline the "(Vanilla LLMs)" reading is under.

**(D) A second RegEx+Whitelist precision sentence, recorded and not adjudicated.** Cycle 22
recorded *"The RegEx + Whitelist method labels all extracted indicators but still suffers from poor
precision due to insufficient context awareness"*. Both of this cycle's renders instead returned
*"The RegEx + Whitelist method achieves almost perfect recall but low precision, particularly for
domains and URLs"*. These are two different sentences about the same method and both may well be in
the paper; neither carries a measurement and they agree in content. **No contradiction opened.** A
successor should ask ALL-OCCURRENCES of "RegEx + Whitelist" — per the cycle-93 method note, an
under-specified query yields an under-reporting answer and the defect is in the *query*.

### Secondary target: `open_questions[1]`, indicator-type coverage — taken at no extra fetch

`src-0003` now states its own scope **and its own reason**, verbatim, two-form identical:

> We limited our scope to the four most common and reliably verifiable indicator types, IP
> addresses, domain names, URLs, and file hashes.

Four-term attested absence, both renders, asked separately for each: `registry key` **ABSENT**,
`mutex` **ABSENT**, `YARA` **ABSENT**, `Sigma` **ABSENT**. This is the first time this base has held
`src-0003`'s scope statement verbatim rather than inferring the four types from result tables.
**Consequence: both sides of `ctr-0001` now carry an explicit four-type ceiling** — `src-0007`'s
ground truth is "file hashes, IP addresses, or domain names" (cycle 9), `src-0003`'s is the sentence
above — so **no source in this base measures registry keys, mutexes or YARA/Sigma rules at all**.
`open_questions[1]` therefore **joins `open_questions[4]` as a T1 collection target**; no T3 can
discharge it.

### G3 — `ctr-0066` opened

The investigation surfaced a conflict with a **supported** candidate of this issue.
`candidate_resolutions[0]` describes the system that reached the 97.6% F1 as a *"hybrid
regex+LLM+human-validation pipeline"*. **The human-validation loop is not in the measured path of
that number.** Section 5.1.1 compares LANCE against *"the most prominent existing automated ground
truth (GT) creation methods"* and scores it against the human-built PRISM — the analysts are on the
**reference** side of the measurement, not inside the system measured; the architecture sentence
says LANCE labels *automatically*; and the HITL configuration is evaluated **separately** in Section
5.2, which is where the 43% result lives. The 97.6% is an **automated-pipeline number**.

`ctr-0066` records three sites, of which the third is a **negative** finding worth stating plainly:
`state/knowledge/src-0003.md`'s Summary wording ("justifications **for** human reviewers") is
**correct as written** and needs no appended correction — it is listed only so a successor can see
the knowledge file is not at fault. What is **not** charged: the 97.6%, the 43%, the HITL design, or
any score.

### Explicitly not attempted, and said so as the queue required

- **`open_questions[4]`, the adversarial threat model — declined for the sixth time**, on the
  identical and correct ground cycles 21, 52, 93, 108 and 117 gave: no source in this base measures
  report-based extraction under an adversarial threat model, so it is a **T1 collection target** and
  no T3 can discharge it. **Not one search was spent on it and it was not re-litigated.**
- **`ctr-0010` step (i) — not touched.** It asks for the mis-scoped 0.09–0.15 restatement in
  `state/assessments/scores.json` to be replaced, and **a T3 does not write `scores.json`**. Still
  owed to a T4.
- I did **not** re-open the cycle-130 tie-break question, as the queue instructed.

## Retrospection

**G2 target: cycle 122** (the next unexamined predecessor in the round-2 order — 130 took 121's).
Cycle 122 was a T3 on `consistency-calibration-as-failure-mode`, commit `ed5860c`. **Zero fetches**,
matching the standard cycles 121–130 all met.

I confirmed what cycle 122 **entered into the state** from its own diff rather than from its log
(per [337] and the cycles-127/129/130 lesson). `git show --stat ed5860c` shows it touched
`state/issues/graph.json`, `state/meta.json`, both queue files and its log — **no
`state/knowledge/` files**, which independently confirms its own statement that it *"entered none as
a source because a T3 adds no source."* I then pulled the tail of the entry it appended to that
issue's `open_questions[11]`.

**Conclusion re-checked:** cycle 122's claim that the two most relevant returns from its one web
search *"were already in this base, checked against index.json rather than assumed"* — specifically
that **arXiv 2606.20502 is `src-0042`** and **arXiv 2606.31159 is `src-0013`** — together with its
claim that its **three recorded leads were none of them present in `index.json`**.

**Verdict: CONFIRMED, and not superseded.**

- Against the **cycle-122 snapshot** (`git show ed5860c:state/knowledge/index.json`, `srcs=42`):
  `src-0013 → arxiv.org/abs/2606.31159` and `src-0042 → arxiv.org/abs/2606.20502`. Both
  identifications exact. A pattern test for the three lead ids (`2025/546`, `2406.05892`,
  `2412.15004`) returned **no matching source** — the leads were genuinely absent, as claimed.
- Against the **current** index (`srcs=44`): both identifications still hold, and `leadhits=0` — the
  three leads are **still** unfetched and unsourced. The two sources added since (`src-0043` =
  arXiv 2607.25572, `src-0044` = the KPMG PDF) are unrelated to any of them, so **no supersession
  has occurred** on this conclusion.

No contradiction is owed and none is opened. I checked what cycle 122 *entered*, not what its log
said, and the two agreed here.

One thing I deliberately did **not** re-check: cycle 122's characterisation of *why* the `src-0042`
hit is "informative rather than merely disappointing." That rests on the cycle-121 fine-tuning
census, which **cycle 130 already re-verified** and found confirmed-as-scoped but superseded as a
standing instruction by `src-0043`. Re-running it would have duplicated cycle 130's G2.

## Changes made

All edits are to `state/issues/graph.json`, all **appends**, spliced in a single two-input jq call
from one scratch JSON ([237]/[211]/[228]), verified before `mv` and the scratch removed after.

| Change | Detail |
|---|---|
| `attempts` | `[9,21,35,52,65,84,93,108,117]` → `[9,21,35,52,65,84,93,108,117,131]` |
| `open_questions[1]` | cycle-131 block appended (scope sentence, four-term absence, T1 handoff) |
| `open_questions[2]` | cycle-131 block appended (findings A–D and the inference caution) |
| `open_questions[14]` | **new** status entry, cycle 131 (array 14 → 15) |
| `candidate_resolutions[0]` | `ctr-0066` DO-NOT-CITE marker appended; status/evidence untouched |
| `candidate_resolutions[1]` | `ctr-0066` marker appended; conclusion recorded as **strengthened** |
| `candidate_resolutions[2]` | cycle-131 note appended; **stays `proposed`**, deliberately not rejected |
| `candidate_resolutions[4]` | **new**, `supported`, evidence `["src-0003"]` (array 4 → 5) |
| `contradictions` | **`ctr-0066` appended** (65 → 66; open 49 → 50) |

**Append-only proof run before `mv`, nine conjuncts, all `true`:** other nine issues equal outright;
`_schema` equal; the new contradictions array's leading 65-element slice equal to the old array;
every edited `open_questions[i]` (i = 0…13) `startswith` its old text; `open_questions` 0 and 3–13
equal outright; every edited candidate summary `startswith` its old summary; candidates 0–3
`evidence` and `status` unchanged; candidate 3 untouched entirely; `title`/`depends_on`/
`created_cycle` unchanged. Counts after: `ctr=66 open=50 issues=10 last=ctr-0066`, `nq=15`,
`ncand=5`, new candidate `supported` on `src-0003`.

**Untouched, as a T3 requires:** `state/knowledge/` (no source added, no `src-*.md` or `key_claim`
altered) and `state/assessments/scores.json` (no score moved; none proposed).

**G1:** no new factual claim entered without a source id that exists in
`state/knowledge/index.json`. Every claim above cites `src-0003`, which is `index.json`'s existing
entry for `https://arxiv.org/html/2506.11325v2` — the URL I re-fetched. No new URL was recorded.

## Next task rationale

State machine edge **T3 → T4**, so cycle 132 is an Assess. `target_issue` is **null** by design:
`prompts/t4_assess.md` step 1 requires scoring **every** issue, not just the touched one.

Three things make this T4 non-routine and the queue entry says so:

1. **`ctr-0066` step (ii) is a `scores.json` sweep, and the T4 is the cycle that writes that file.**
   I did not run it and its size is unknown; the entry says so explicitly so no successor reads it as
   discharged. The instrument that worked for the analogous `ctr-0064` sweep at cycle 129 ([348]'s
   `scan()` window) is named in the queue.
2. **The G3 demotion arithmetic does not change.** `ioc-extraction-reliability` now carries four open
   contradictions instead of three, but `g3_contradiction_demotion` is a flat 2 points for having an
   open contradiction, not per-contradiction. I flagged this so cycle 132 neither double-deducts nor
   silently ignores the new entry.
3. **Do not inflate on the strength of this cycle.** A new *single-source* supported candidate does
   not lift an issue past rubric level 2 (which is *defined* as single-source support); level 3 needs
   two independent sources for the primary candidate. And an issue does not become better resolved
   because a cycle found a defect in it.

I also asked cycle 132 to look at `automated-triage-under-refusal` before re-entering 2 by inertia —
it is the cycle that sets that score, and the score is a floor read off a single untested candidate.

## Budget

- Web searches: **0**
- Fetches: **2** — one render pair of a single already-held URL (`arxiv.org/html/2506.11325v2`
  direct + through `r.jina.ai`). No new source added. G2 was done at **zero** fetches.
- Bash calls: 16
- Read tool calls: 5; Write tool calls: 4; ToolSearch: 1
- Turns used: roughly 30 of 75
