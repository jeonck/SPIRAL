...
**Target issue:** none on input (T5 selects); `institutional-incident-real-world-impact` on output
**Queue entry:** written by cycle 88, `task_type: T5`, `attempt_count: 0`
**Phase check:** PASSED. `state/queue/last_completed_task.txt` read `T4 assess`;
`git show --stat HEAD` showed cycle 88's commit touching `scores.json`, `graph.json`,
`meta.json` and both queue files. All four cheap confirming checks the queue entry named
came back as predicted: `last_assessed_cycle` = 88, `[.scores[].assessed_cycle]|unique`
= `[88]`, `contradictions|length` = 47 with `contradictions[-1].id` = `ctr-0047` on
`ttp-attack-mapping-reliability`, open contradictions = 35. The non-tests were also as
predicted: score total still 19, nine issues, 31 sources, `src-0031` last. Cycle 88 did
not abort and I am the T5.

**Budget headline: TWO fetches, ZERO searches, ZERO new sources, ZERO score changes,
ONE new contradiction.** Both fetches went to G2, and the second one existed only to
turn a one-form result into a two-form one before I filed on it.

---

## Task performed

T5 per `prompts/t5_select.md`, which I read myself rather than relying on the queue
entry's account of it. Three things had to be decided before the ranking could be run,
and the queue entry deliberately decided none of them. I take them in order.

### Decision 1 — the refresh rule. NO T1 THIS CYCLE, and the rule as written is defective.

`prompts/t5_select.md` step 4 states the rule as an explicit formula: *"if
`current_cycle % schedule.collect_refresh_every == 0`, the next task is `T1`"*.
`config.yml` sets `collect_refresh_every: 7` and glosses it *"every Nth cycle, T5
schedules a T1 for the weakest issue"*.

**89 % 7 = 5. Not 0. No T1 is due, and I schedule a T3.**

I adopt the written formula rather than the "at most 7 cycles between T1s" reading, for
one reason and not for convenience: a T5's job is to apply the policy mechanically, and
a T5 that overrides an explicit formula on intent grounds manufactures exactly the
alternating-reading problem that carry-forward [233] documents for tie-break 3(a). The
rule is written; I follow it and record the defect rather than legislating around it.

**The defect, which is arithmetic and which no cycle has recorded before.** T1 fires
only when a cycle is *both* a T5 *and* a multiple of 7. The steady-state ladder is
T5→T3→T4→T5, so **T5 cycles recur with period 3**. Multiples of 7 recur with period 7.
The two coincide with period **lcm(3, 7) = 21**. So the rule as written delivers a T1
**once every 21 cycles, not every 7** — it cannot honour its own config gloss, and the
error is a factor of three.

The history corroborates this rather than the intent reading. `grep -l "T1 collect"
logs/cycle-0*.md` returns cycles **014, 041, 078** and nothing later. Of those,
**14 % 7 = 0 ✓, 41 % 7 = 6 ✗, 78 % 7 = 1 ✗** — so **only one of the three T1s this loop
has ever run actually satisfied the written rule.** The loop has de facto used the
"overdue" reading twice and the written rule once, without ever noticing the two are
different rules. That is new, and it is carry-forward [245].

**When the next T1 falls under the rule I am applying:** the ladder from here is
90=T3, 91=T4, 92=T5, 93=T3, 94=T4, 95=T5, 96=T3, 97=T4, **98=T5**, and 98 % 7 = 0. So
**cycle 98**, twenty cycles after cycle 78. I state that plainly because it is a cost.

**Why I am nevertheless comfortable not forcing a T1 on merit.** The binding constraints
on this graph right now are not evidentiary. Eight of nine issues are pinned at 2 by
*policy* questions (carry-forward [4]'s validator-versus-prompt conflict, [97], [242]'s
both-conjuncts reading) and by *undone repairs* — `ctr-0008` step (i), `ctr-0018` step
(iii), `ctr-0043` steps (iii)+(iv), `ctr-0047` steps (i)+(ii), and now `ctr-0048`'s four
steps — none of which a new source would discharge. Six named, unclaimed, one-fetch
repairs is more dischargeable work than a T1 could create. **If that stops being true —
if the repair queue empties and the issues are still at 2 — a future T5 should read this
paragraph as the argument for fixing the rule in `config.yml` rather than for obeying
it.** Changing `config.yml` is outside a T5's remit and I did not touch it.

### Decision 2 — tie-break 3(a). I adopt the PAIRWISE reading, and here is why.

The text at source: *"a. Upstream first: an issue that others `depend_on` outranks its
dependents."*

Carry-forward [233] records that this loop has read this line in alternating ways. The
two readings are:

- **Pairwise (adopted).** The clause defines a relation between an issue and *its own*
  dependents. It orders only pairs actually joined by a `depends_on` path. An issue with
  no dependents is neither raised nor lowered by 3(a); it simply falls through to 3(b).
- **Global in-degree (rejected).** "Is depended upon" is a sort key over the whole
  candidate set, so an issue with three dependents outranks one with one, which outranks
  one with none.

**I adopt the pairwise reading on the plain text.** The object of "outranks" is
***its* dependents** — a possessive, which restricts the comparison to the issue's own
dependents. The global reading requires the sentence to say something it does not say
(e.g. "issues with more dependents outrank those with fewer").

There is also a consequential argument, which I record as secondary because the textual
one is sufficient. The global reading makes the ranking a **fixed function of graph
topology**: `consistency-calibration-as-failure-mode` (3 dependents) would win every
single cycle regardless of research progress, and the three issues with zero dependents
— including the never-investigated one — would rank last **permanently**. A weakest-link
selector that cannot respond to the state of the research is not a selector. That is
carry-forward [246].

### Decision 3 — the 3(b) window. I take cycles 84–88.

"Within the last 5 cycles" at cycle 89 = **84, 85, 86, 87, 88**. Cycle 89 is the current
cycle, not one of the last ones.

**This matters, and the alternative exposes a real gap.** Under an 85–89 window,
`ioc-extraction-reliability`'s cycle-84 attempt drops out, its penalty falls to +0, and
it joins `institutional-incident-real-world-impact` at effective 2 — whereupon 3(c)
compares `created_cycle` **2 against 2** and **the stated tie-breaks run out with the
selection unresolved.** Six of the nine issues share `created_cycle: 2`, so 3(c) is a
much weaker discriminator than it looks. The policy has no step (d). Carry-forward [247].

### The ranking table

Candidate set = all issues with score < 5 = **all nine**. Penalty = +1 per attempt in
cycles 84–88 (`tie_break_recent_attempt_penalty: 1`).

| Rank | Issue | Score | 3(a) | 3(b) | Eff. | `created_cycle` | Decided by |
|---|---|---|---|---|---|---|---|
| **1** | **institutional-incident-real-world-impact** | 2 | maximal | +0 | **2** | 2 | **3(c): older than automated-triage (2 < 16)** |
| 2 | automated-triage-under-refusal | 2 | maximal | +0 | 2 | 16 | lost 3(c) on `created_cycle` |
| 3= | consistency-calibration-as-failure-mode | 2 | maximal | **+1** (attempt c87) | 3 | 2 | lost 3(b) |
| 3= | ioc-extraction-reliability | 2 | maximal | **+1** (attempt c84) | 3 | 2 | lost 3(b); tied with row 3 and **3(c) cannot separate them** |
| 5 | attribution-confident-wrong-gap | 2 | non-maximal (⊂ consistency-calibration) | +0 | 2 | 2 | lost 3(a) |
| 6 | attribution-expressed-confidence-unmeasured | 2 | non-maximal (⊂ attribution-confident-wrong-gap) | +0 | 2 | 45 | lost 3(a) |
| 7 | task-dependent-reliability-framing | 2 | non-maximal (⊂ 5 issues) | +0 | 2 | 2 | lost 3(a) |
| 8 | extraction-vs-reasoning-ordinal-axis | 2 | non-maximal (⊂ task-dependent) | +0 | 2 | 16 | lost 3(a) |
| 9 | ttp-attack-mapping-reliability | **3** | maximal | +0 | 3 | 2 | lost on **base score** (step 2) |

**Tie-breaks are applied in the stated order a → b → c**, so 3(a) partitions before 3(b)
charges penalties: a non-maximal issue at effective 2 (rows 5–8) still ranks below a
maximal issue at effective 3 (rows 3=). The four **maximal elements** of the `depends_on`
partial order within the score-2 band are rows 1, 2, 3= and 3=.

**Working for 3(a), from `depends_on` as stored:** `consistency-calibration-as-failure-mode`
→ depended on by `attribution-confident-wrong-gap`, `attribution-expressed-confidence-unmeasured`,
`task-dependent-reliability-framing`. `attribution-confident-wrong-gap` → depended on by
`attribution-expressed-confidence-unmeasured`, `task-dependent-reliability-framing`.
`ioc-extraction-reliability` and `ttp-attack-mapping-reliability` → depended on by
`task-dependent-reliability-framing`. `task-dependent-reliability-framing` → depended on
by `extraction-vs-reasoning-ordinal-axis`. `institutional-incident-real-world-impact` and
`automated-triage-under-refusal` → **depended on by nothing, and depending on nothing;
they are isolated vertices**, which under the pairwise reading makes them maximal by
default rather than by strength.

**Attempt arrays as stored** (for audit): ttp `[16,31,32,46,55,68]`, ioc
`[9,21,35,52,65,84]`, consistency `[3,15,16,25,39,62,75,80,87]`, attribution-confident
`[16,28]`, attribution-expressed `[49]`, task-dependent `[6,16]`, extraction-vs-reasoning
`[17,18]`, institutional `[12,59,72]`, automated-triage `[]`.

### THE INSTRUMENT CHOSE THIS TARGET, NOT MY JUDGEMENT. I record the divergence.

The queue entry asked me to say which. **The instrument did**, and I applied it without
thumb on the scale. But cycle 88 asked the question for a reason, so here is the honest
answer to it:

**On merit I would have chosen `automated-triage-under-refusal`, and the instrument put
it second.** It is the only issue **never investigated in 89 cycles** (`attempts` is
`[]`), it has **one** candidate_resolution against a graph median of three, and it
carries **three open contradictions opened by three separate G2 passes that all found
something**. Cycles 76, 81, 85 and 88 all called it the weakest issue in the graph on
merit. Carry-forward [30] records the `created_cycle` tie-break eliminating it; **this
is the eleventh time**, and it lost by a margin of fourteen cycles of birth order to an
issue that has been investigated three times.

**The instrument-level finding, which is the evaluation datum here.** At the top of the
table, `ioc-extraction-reliability`'s 2 is the **best-audited** 2 in the graph (six
investigations, six open contradictions, eleven open questions) and
`automated-triage-under-refusal`'s 2 is the **least-audited** (zero investigations, one
candidate, five open questions). **The score cannot tell them apart, and neither can any
of the three tie-breaks — 3(a) makes them both maximal, 3(b) is a recency proxy that
charges the audited issue and spares the unaudited one, and 3(c) is birth order.** No
step of the policy measures how much is *known*. Carry-forward [248]: the selector's
inputs contain no term for evidential depth, which is why an issue can go 89 cycles
untouched while scoring identically to the most worked issue in the base.

I did not act on that. Fixing it means changing `prompts/t5_select.md`, which is not a
T5's to change, and I would rather the record show the policy running honestly to a
result I disagree with than show a T5 quietly correcting it.

### Does the selected issue have work a T3 can actually do?

Yes, and I checked before writing the queue entry, because carry-forward [242] records
`institutional-incident-real-world-impact` as **structurally pinned** at 2 by cycle 76's
both-conjuncts reading, and sending a cycle at an unmovable issue would be a waste.

`ctr-0018` (opened cycle 50, still open) has steps (i), (ii) and (iv) **done** at cycle
59 and step (iii) **half** done. Its own closing sentence names precisely what remains:
*"only step (iii)'s second absence. It needs ONE verification-mode fetch on the
permalink or the archive asking, as a bare exact-string probe rather than a conceptual
question, for any numeral adjacent to 'slop'."* Cycle 59's attempt at it failed as a
**rule (xvii) substitution** — the fetch answered a different question and quoted the
confirmed-vulnerability rate instead — so the failure mode is known and avoidable.

**One fetch could resolve `ctr-0018`**, taking the issue from five open contradictions
to four and closing the oldest undone repair on it. That would be the first contradiction
resolved on this issue since cycle 59. The score will not move — G3 caps it at 3 and the
both-conjuncts reading pins it at 2 — and I told the T3 not to spend the cycle fighting
that, but to strengthen half (B) and feed `open_questions[6]`'s split case instead.

---

## Retrospection

**G2 target.** Per the round-2 order (cycle 87 took cycle 77's issue, 88 took 78's), I
take **cycle 79's**. I verified this by reading `logs/cycle-079.md` rather than trusting
the queue entry: cycle 79 was a **T2 (Structure)** on
`consistency-calibration-as-failure-mode`.

Cycle 79 entered exactly one **supported** candidate_resolution —
`candidate_resolutions[10]`, the ECE/Brier non-interchangeability finding, evidence
`src-0001` / `src-0028` / `src-0029` — so the G2 rule ("a supported candidate_resolution
or a scored assessment") selects it without discretion.

**How I chose which limb to test, since the candidate is now ~14KB.** It has already
been audited twice (cycle 85, and cycle 87 executing `ctr-0044`). Re-running those limbs
would be cheap and worthless. I went instead at the limb **cycle 87 newly made
load-bearing** and did *not* re-fetch: its step (iv) ruling rests the candidate's core
claim on `src-0029`'s Section 4.4 sentence *"It is good practice to use them together
with the proper scoring rules presented in Section 3, to get a full evaluation"* — a
sentence **stored since cycle 78 and never re-fetched by cycles 79, 85 or 87**.
Carry-forward [241], written by cycle 88 one cycle earlier, predicts that a claim
asserting more than it checked will fail. **It did.**

### Result: FAILED. `ctr-0048` opened.

**Fetch 1** — `ar5iv.labs.arxiv.org/html/2112.10327`, asking for Section 4.4's title,
the target sentence verbatim, and the sentences immediately preceding it so the referent
of "them" could be fixed.
**Fetch 2** — `r.jina.ai` over `arxiv.org/pdf/2112.10327v2`, a **different render
family**, asking for the same two sentences. **The two renders agree verbatim and
identically.** I spent the second fetch before filing precisely because this base keeps
filing one-form results and then having to caveat them (`ctr-0044`, `ctr-0046`, and
paragraph (6) of this very candidate).

**Three limbs PASSED, recorded so no successor redoes them:**

1. The guidance sentence **exists** in `src-0029` Section 4.4, on both renders.
2. Section 4.4's title is **"Confidence calibration error"** and the metrics under
   discussion there are **confidence-ECE and confidence-MCE**. Neither fact had ever
   been recorded in this base.
3. **Cycle 87's referent ruling is CORRECT**, and is now pinned tighter than cycle 87
   could pin it. Cycle 87 said "them" refers to "the Section 4 ECE-family metrics"; the
   antecedent is in fact the noun phrase **"all ECE/MCE measures"** in the immediately
   preceding sentence, so the referent is the ECE *and* MCE families specifically.

**The limb that FAILED.** The stored sentence is a **mid-sentence excerpt**. In the
original it opens **"Thus, it is good practice…"** and closes **"…to get a full
evaluation of the probabilities produced by a model."** The sentence its "Thus" draws
on — **never fetched by any cycle** — reads, verbatim on both renders:

> "Finally, it is important to consider that all ECE/MCE measures can provide very
> different values given different number of bins, as shown in Figs. 1 and 2, and they
> can be minimised by predicting the overall class distribution, regardless of the given
> instance."

Cycle 87's step (iv)(B) asserts, verbatim: *"The Section 4.4 sentence establishes THAT
the two are not interchangeable and says NOTHING ABOUT WHY."* **That preceding sentence
is the why**, and it is joined to the stored sentence by the very conjunction the stored
excerpt drops.

**Why it is material and not a quibble.** The second clause — *a measure minimised by
predicting the overall class distribution regardless of the given instance* — **is the
refinement/sharpness-insensitivity point**, stated by `src-0029` in its own words and
about the ECE family by name. Cycle 87 declined to retire paragraph (2)'s Bregman
apparatus on the express ground that the mechanism is *"carried by `src-0028`'s
decomposition and by nothing else in this base"*. `src-0029` carries a mechanism too,
and a nearer-to-quotational one: a metric minimisable by predicting the base rate
irrespective of the instance is **by construction blind to refinement**, which is exactly
what makes a low-ECE/high-Brier cell possible and this issue's Table 6 divergence
*expected* rather than anomalous.

**What is NOT in dispute, so this is not over-read.** `candidate_resolutions[10]` keeps
its `supported` status and **its conclusion is unchanged** — the finding is not weakened,
it gains a second mechanism. **Cycle 87's conclusion also survives**: keeping the Bregman
apparatus was right, because `src-0029`'s clause states blindness-to-refinement but not
a *decomposition*, so `src-0028` still supplies something `src-0029` does not. **Only
cycle 87's stated reason is falsified.** Paragraph (7)'s scope limit is untouched and
binding: both methodology sources measure no LLM and no security task, **this cycle
added no CTI source and no CTI measurement**, and nothing here makes the calibration leg
two-source. **No score moved.**

### The transferable part, which is the more valuable half

**Cycle 87's sentence was TRUE OF THE BASE AS STORED AND FALSE OF THE SOURCE.** That is
the precise shape, and it is worth stating carefully because the looser version ("cycle
87 was sloppy") is wrong — cycle 87 read every byte the base held.

**Carry-forward [244], the rule: A CLAIM THAT A SOURCE IS SILENT ON SOMETHING MAY NOT BE
MADE FROM A STORED EXCERPT. IT REQUIRES A FETCH OF THE EXCERPT'S NEIGHBOURHOOD.**
A stored excerpt is evidence of what a source *says*; it is evidence of nothing about
what a source *does not say*.

**Carry-forward [249], the mechanism, which is new and is a storage defect rather than a
reasoning one: THE STRIPPING OF A LEADING CONJUNCTION IS WHAT HID THE OMISSION.** Once
"Thus," is dropped and "it" capitalised to "It", a mid-sentence excerpt is
typographically indistinguishable from a whole sentence, and every downstream cycle
inherits a fragment that *looks* complete. Cycle 78 did this; cycles 79, 85 and 87 could
not have detected it from the file. **Store the full sentence, or mark the truncation.**

This extends [241] rather than merely confirming it. Cycle 88 found that candidates
**hedging** their provenance survive and ones **overstating** their checking fail. Add:
the failure here is a **bare negative about a source stated from a stored fragment**,
which is the sharpest instance of the overstatement pattern yet, because paragraph (2)'s
own explicit `INFERENCE` label — written at cycle 79 and left standing — did **not**
fail, and neither did paragraph (7)'s scope banner. Within one candidate, the hedged
parts held and the unhedged negative broke.

One further note, recorded because it is uncomfortable and therefore useful: cycle 87
wrote the near-miss version of [244] **at the foot of the very block in which it broke
it** — *"THE CHEAPEST CHECK ON A STATED ABSENCE IS OFTEN THE REST OF THE SAME
PARAGRAPH"* — three paragraphs after step (iv)(B). Stating a methodological rule and
applying it are separate acts, and this base has now produced direct evidence of the gap.

`ctr-0048` names **four repair steps**, none of which a T4 or T5 may perform. Step (iii)
is the highest-yield: the Section 4.1 half-bin-width quotation, the Section 3.1
decomposition quotation and the Section 3 strictly-proper quotation **all came from the
same truncated cycle-78 fetch and none has had its neighbourhood read** — one fetch
re-grounds three stored quotations.

---

## Changes made

**One state edit outside the queue and the log.**

1. **`state/issues/graph.json` — `ctr-0048` APPENDED** to `contradictions`, against
   `consistency-calibration-as-failure-mode`, `opened_cycle: 89`, `resolved_cycle: null`.
   Contents as described above: three passing limbs recorded, the falsified sentence
   quoted verbatim from both the candidate and the source, the scope limits restated as
   binding, and four repair steps assigned to a T1 or T3.
   Contradictions **47 → 48**; open **35 → 36**.

**What I deliberately did NOT do.** I did **not** append the newly verified sentence to
`src-0029.md` or to `index.json` `key_claims`. It is new evidence from an existing
source, and adding key_claims is T1/T3 work; a T5 that starts editing the knowledge base
is doing the next cycle's job. The sentence is preserved **verbatim inside `ctr-0048`**
so nothing is lost, and `ctr-0048` step (i) assigns the append explicitly. I also
changed **no score** (`scores.json` is byte-unchanged, `last_assessed_cycle` still 88),
**no candidate_resolution**, **no open_question**, **no `attempts` array**, and **no
source**. `config.yml` is untouched despite the refresh-rule defect above.

**Validation performed, by hand, since `scripts/validate_state.py` cannot be run from
this session's Bash tool.**
- `jq -e .` on `graph.json` after the splice: valid.
- Key check on the new entry: `id,issue_id,description,opened_cycle,resolved_cycle`, in
  that order, matching `contradictions[-1]` before the edit. (My first draft of the
  scratch file had the order wrong — `opened_cycle` before `description` — and I
  re-authored it rather than shipping a key-order mismatch.)
- **Append-only proof (carry-forward [221]), eight checks in one call, all true, run
  BEFORE the `mv`:** new `contradictions|length` = 48; `new.contradictions[0:47] ==
  old.contradictions` → **true** (old array is an exact prefix); `new[-1].id` =
  `ctr-0048`; keys correct; open count 36; `(new|del(.contradictions)) ==
  (old|del(.contradictions))` → **true** (every untouched sibling byte-identical);
  `issues|length` = 9.
- G1: no new source was added, so no new URL to resolve. The two URLs fetched this cycle
  are re-fetches of `src-0029`, already in `index.json` with a resolving URL.
- Scratch file `scratch-089.json` **deleted** before finishing.

---

## Next task rationale

**`state/queue/next_task.json` → T3 (Investigate) on
`institutional-incident-real-world-impact`, `attempt_count: 0`.** Selected by the policy
as ranked above; no T1, per Decision 1.

The entry gives the phase check with its **single** confirming test (contradictions = 48,
`ctr-0048` last, open = 36) and is explicit that **nothing else changed**, so a
successor cannot mistake a held value for an abort. It quotes all seven `open_questions`
by their opening words and tells the T3 to read them in full from the graph rather than
work from the stubs. It names `ctr-0018` step (iii) as the one thing most worth its
fetch, reproduces the exact probe `ctr-0018` itself specifies, and warns about the rule
(xvii) substitution that defeated cycle 59. It warns against spending the cycle fighting
the [242] structural pin, which a T3 has no power to lift. It flags `ctr-0048` step (iii)
as a high-yield off-target option and requires the T3 to say so in its log if it takes it.

**Size: 15.9KB, marginally over the ~15KB guideline of carry-forward [238].** I trimmed
it twice (16.3 → 16.0 → 15.9) and stopped; further cuts would have removed the
`ctr-0018` probe text or the phase check, which are the two things most likely to save
the next cycle a wasted turn.

Cycle 90's G2 target under the round-2 order is **cycle 80's issue**; I told it to verify
that against `logs/cycle-080.md` rather than trust my line.

---

## Budget

- **Fetches: 2.** Both to `src-0029` for G2, deliberately in two different render
  families (`ar5iv`, then `r.jina.ai` over the arXiv PDF) so the finding filed as
  `ctr-0048` is two-form rather than one-form.
- **Searches: 0.**
- **New sources: 0.** Source count stays at 31, unchanged since cycle 78.
- **Turns: ~22 of 75.** Well inside budget; the T5 task is cheap and the cost was in G2.
- **Files written: 4** — `state/issues/graph.json` (one appended array element),
  `state/queue/next_task.json`, `state/queue/last_completed_task.txt`, `logs/cycle-089.md`.
  Plus `scratch-089.json`, created and deleted.
