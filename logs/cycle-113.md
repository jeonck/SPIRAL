...
Topic: `cti-ai-reliability`. Runtime pinned per `config.yml`: `claude-opus-5`, CLI `2.1.220`.

---

## Phase check

Run before anything else, because cycles 70, 83 and 106 each aborted on a mis-derived
phase and cost the chain seven cycles between them.

| check | required | observed | verdict |
|---|---|---|---|
| `state/queue/last_completed_task.txt` | `T4 assess` | `T4 assess` | OK |
| `git show --stat HEAD` | touches `scores.json`, `graph.json`, both queue files, `meta.json`, `logs/cycle-112.md` | all six + transcript | OK |
| `scores.json` | `last=112 total=22 cycles=[112]` | `last=112 total=22 cycles=[112]` | OK |
| `graph.json` | `ctr=58 open=43 issues=10` | `ctr=58 open=43 issues=10` | OK |
| `index.json` | `srcs=40 last=src-0040` | `srcs=40 last=src-0040` | OK |

All five agree, so the phase derived from `last_completed_task.txt` (T4 → T5) and the phase
asserted by the queue entry coincide. I am a **T5**. `prompts/t5_select.md` read at source.

---

## Retrospection

**G2 target: the conclusion cycle 103 entered.** The round-2 order — *cycle N re-checks the
conclusion the next unexamined predecessor entered* — has run 105→96, 107→97 (106 aborted and
took none), 108→98, 109→99, 110→100, 111→101, 112→102. The next unexamined predecessor is
**103**, which I confirmed by reading `logs/cycle-103.md` at source rather than trusting the
queue entry's line. Cycle 103 was a **T5**, so the conclusion it entered is its **selection of
`institutional-incident-real-world-impact`, together with its ranking table**. Zero fetches: a
claim about the state is correctly checked against the state.

### What I re-derived, and against what

I rebuilt every input of cycle 103's table from the state **as it stood when cycle 103 read it**
— commit `5c2b5a5`, cycle 102's output — not from the current state and not from any log.

| cycle 103's assertion | re-derived from `5c2b5a5` | verdict |
|---|---|---|
| scores `last=102 total=22 n=10`, all `assessed_cycle` 102 | identical | **holds** |
| exactly two 3s: `ttp-attack-mapping-reliability`, `incident-evidence-vendor-concentration` | identical | **holds** |
| all ten `attempts` arrays as tabulated | all ten identical, row by row | **holds** |
| all ten `created_cycle` values | identical | **holds** |
| `depends_on` edge table (10 rows) | identical | **holds** |
| 3(b) window 98–102: only `incident-evidence` has an attempt (`101`) | `ioc`'s 93 and `cc`'s 96 both outside; confirmed | **holds** |
| "93 and 96 aged out of the window simultaneously this cycle" | confirmed | **holds** |
| "no `attempts` array anywhere contains 78 or 99" | confirmed across all ten | **holds** |
| three issues tied on every defined criterion after 3(c) | confirmed: `institutional-incident`, `ioc`, `cc` — score 2, unbeaten, penalty 0, `created` 2 | **holds** |
| `automated-triage-under-refusal` died at 3(c) on age, 16 vs 2 | confirmed | **holds** |

**The selection and the ranking table re-verify completely.** Ten of ten rows reproduce. Carry-forward
**[273]** — *the policy has no step 4 and genuinely ran out at cycle 103* — is **confirmed**, not
impeached.

### What failed, and it is not the conclusion

The same section of `logs/cycle-103.md` carries a supporting paragraph headed *"Merit evidence,
used to read the result and never to override the order"*. It restates that
`candidate_resolutions[3]` is **"the only candidate answering the title's second conjunct — how
institutions respond when caught"**, and adds a stamp neither other carrier has: **"I verified
all of that at source this cycle."**

That clause is **false**, and `ctr-0058` (opened at cycle 112, open) already establishes why. I did
not inherit that: I re-read `candidate_resolutions[5]` **at source in the cycle-102-era graph** —
entered cycle 59, status `supported`, evidence exactly `["src-0022"]` — and its own text says,
under *WHY THIS BELONGS TO THIS ISSUE*, that the issue "asks whether AI-generated CTI failures have
reached production at real institutions and **how institutions respond when caught**", and that curl
is "a real institution responding to AI-generated security reporting by **closing the intake process
entirely**". So `ctr-0058`'s premise is **independently confirmed**.

**But `ctr-0058` is itself wrong about one thing**, and that is a new finding rather than a repeat:
it states the false clause has *"now travelled through two assessments unchallenged"*, naming cycle
102 as originator and cycle 105 as the single inheritor, and it justifies opening an entry rather
than a footnote **on that count of two**. Cycle 103 is a **third carrier**, sitting **between** the
two it names. And the practical consequence is worse than an off-by-one: `ctr-0058` step (ii)
commissions a sweep of *"the **other** nine issues"* over *"any **scored rationale**"* — a sweep
scoped to `scores.json` and to issues other than this one. **Cycle 103's instance is in neither
set**: it is in a *log*, and it is on *this* issue. The sweep as written **cannot find it**, and
`ctr-0058` already flags step (ii) as the step most likely to be skipped.

### `ctr-0059` opened

Recorded as **`ctr-0059`** on `institutional-incident-real-world-impact`, `opened_cycle` 113, open.
Scoped narrowly so it is not read as a second front on the substance: it does **not** re-litigate
`ctr-0058`'s correction, and it moves **no score and no ceiling** (this issue stays at 2, ceiling 3).
What it records is (a) the carrier count is three, not two, and the earliest re-use is a T5 log;
(b) `ctr-0058` step (ii)'s scoping structurally misses it; and (c) the general lesson, which is the
part worth keeping:

> **An explicit at-source verification stamp is not evidence the check was run against the right
> object.** Cycle 103 wrote "I verified all of that at source" over a *four-part* conjunction whose
> other three parts are genuinely checkable in evidence arrays — the array contents, the cycle-12
> entry with no extension, the two-of-five institution coverage — and cycle 112 confirmed all three
> of those verify. A real, diligent, array-level verification therefore produced a truthful-feeling
> stamp that covered a fourth clause only candidate *text* could have tested. This is `ctr-0058`'s
> own evidence-array-for-candidate-text substitution **appearing one level up, inside the
> verification claim rather than inside the claim**.

**A T5 writing to `graph.json` is precedented and validator-accepted**: cycle 103 — also a T5, also
from its G2 — appended `ctr-0055` in a +7-line commit (`bdf9bdc`) that landed. The common rules
require a contradiction entry of any cycle whose G2 finds a conflict, and appending is not
rewriting. `.issues` is byte-identical.

**Reported plainly: the system caught a defect in its own defect report, and the ten-cycle-old
selection it was checking came through clean.** That is what G2 is for.

---

## Task performed

**T5 — Select.** `prompts/t5_select.md` read at source and applied mechanically. Every input below
was re-derived from `graph.json` and `scores.json` this cycle with `jq`; the queue entry supplied a
ranking vector and I verified it rather than adopting it. It was correct in every particular.

### Step 1 — candidate set

All issues with score < 5. Live vector: `ttp-attack-mapping-reliability` = 3,
`incident-evidence-vendor-concentration` = 3, the other eight = 2; total 22 over ten issues, all
`assessed_cycle` 112. **All ten are candidates.** Unchanged from cycles 105 and 109.

### Step 2 — base priority = score

The eight 2s outrank the two 3s. **Both 3s are eliminated here** and never reach a tie-break.

### Step 3(a) — upstream first, pairwise reading

Carry-forward **[246]** settles this as pairwise rather than global in-degree; cycles 89, 92, 95, 98
and 103 all applied it that way and I follow the settled reading. Edges pulled from `graph.json`
this cycle:

| issue | `depends_on` | depended upon by |
|---|---|---|
| `ttp-attack-mapping-reliability` | — | task-dependent |
| `ioc-extraction-reliability` | — | task-dependent |
| `consistency-calibration-as-failure-mode` | — | attr-gap, attr-expressed, task-dependent |
| `attribution-confident-wrong-gap` | consistency-calibration | attr-expressed, task-dependent |
| `attribution-expressed-confidence-unmeasured` | attr-gap, consistency-calibration | task-dependent |
| `task-dependent-reliability-framing` | ttp, ioc, cc, attr-gap, attr-expressed | extraction-vs-reasoning |
| `extraction-vs-reasoning-ordinal-axis` | task-dependent | — |
| `institutional-incident-real-world-impact` | — | incident-evidence-vendor-concentration |
| `automated-triage-under-refusal` | — | — |
| `incident-evidence-vendor-concentration` | institutional-incident | — |

Pairwise-unbeaten at score 2: **`ioc`, `consistency-calibration`, `institutional-incident`,
`automated-triage`** — the same four as at cycle 103. `automated-triage` is unbeaten *vacuously*
(it appears in no `depends_on` array in either direction), which under [246] is neither promotion
nor demotion.

### Step 3(b) — attempt penalty. THE WINDOW READING, STATED EXPLICITLY

The prompt says *"+1 per attempt within the last 5 cycles"* and does not define the endpoint. Two
readings are available at `current_cycle` 113 and **they give different answers**, so I state which
I used and why.

- **Reading A — cycles 108–112**, the five *completed* cycles preceding this one. This is the
  reading cycles 97, 98 and 103 all used (cycle 103: `current_cycle` 103 → "cycles 98–102
  inclusive"). It is also the only reading under which the window contains five cycles that
  *could* hold an attempt: cycle 113 is the current cycle, is a T5, and `attempts` records T3
  investigations only, so including it yields an effectively four-cycle window.
- **Reading B — cycles 109–113.** Non-precedented, and self-shortening for the reason just given.

**I used Reading A**, on precedent and on that argument. Penalties among the pairwise-unbeaten four:

| issue | `attempts` | in 108–112 | penalty | effective |
|---|---|---|---|---|
| `ioc-extraction-reliability` | 9,21,35,52,65,84,93,**108** | **108** | **+1** | **3** |
| `consistency-calibration-as-failure-mode` | 3,15,16,25,39,62,75,80,87,96,**111** | **111** | **+1** | **3** |
| `institutional-incident-real-world-impact` | 12,59,72,90,104 | — | 0 | 2 |
| `automated-triage-under-refusal` | (none) | — | 0 | 2 |

**3(b) is not inert this cycle**, and that is worth flagging: cycle 102 recorded it as inert and
cycle 103 found it separated nothing among the top candidates. Here it eliminates two of the four
leaders. `institutional-incident` escapes only because its cycle-104 attempt fell out of the window
when the window advanced past 108 — i.e. **at cycle 110**, three cycles ago.

**What Reading B would have done, recorded because it is the live disagreement:** `108` drops out,
so only `consistency-calibration` is penalised, leaving `ioc` (created 2), `institutional-incident`
(created 2) and `automated-triage` (created 16) at effective 2. 3(c) then leaves **`ioc` and
`institutional-incident` tied on every defined criterion** and the policy runs out exactly as it did
at cycle 103 — the [273] situation again. **Reading A is both precedented and determinate; Reading B
is neither.** That asymmetry is a reason to prefer A, but I note it is a *consequentialist* reason
and record it as such rather than pretending the prompt's text settles it.

### Step 3(c) — older `created_cycle` first

`institutional-incident` (2) precedes `automated-triage` (16).

### THE RANKING TABLE

| rank | issue | score | 3(a) pairwise | `attempts` | in 108–112 | 3(b) | eff. | `created` | eliminated at |
|---|---|---|---|---|---|---|---|---|---|
| **1** | **institutional-incident-real-world-impact** | 2 | unbeaten | 12,59,72,90,104 | — | 0 | **2** | **2** | **SELECTED** |
| 2 | automated-triage-under-refusal | 2 | unbeaten (vacuous) | (none) | — | 0 | 2 | 16 | **3(c)**, age |
| 3 | ioc-extraction-reliability | 2 | unbeaten | 9,21,35,52,65,84,93,108 | **108** | **+1** | 3 | 2 | **3(b)** |
| 4 | consistency-calibration-as-failure-mode | 2 | unbeaten | 3,15,16,25,39,62,75,80,87,96,111 | **111** | **+1** | 3 | 2 | **3(b)** |
| 5 | attribution-confident-wrong-gap | 2 | beaten by cc | 16,28 | — | 0 | 2 | 2 | 3(a) |
| 6 | task-dependent-reliability-framing | 2 | beaten by 5 issues | 6,16 | — | 0 | 2 | 2 | 3(a) |
| 7 | extraction-vs-reasoning-ordinal-axis | 2 | beaten by task-dependent | 17,18 | — | 0 | 2 | 16 | 3(a) |
| 8 | attribution-expressed-confidence-unmeasured | 2 | beaten by cc, attr-gap | 49 | — | 0 | 2 | 45 | 3(a) |
| 9 | ttp-attack-mapping-reliability | **3** | unbeaten | 16,31,32,46,55,68 | — | 0 | 3 | 2 | **step 2**, score |
| 10 | incident-evidence-vendor-concentration | **3** | beaten by institutional-incident | 101 | — | 0 | 3 | 100 | **step 2**, score |

Rows 5–8 are all eliminated at 3(a) and are ordered among themselves by `created_cycle` for
determinism; that ordering carries no meaning.

**Selected: `institutional-incident-real-world-impact`.**

**Tie-break order does not matter this cycle, and I checked.** Applying 3(b) *before* 3(a) — reading
the penalty as an adjustment to base priority rather than as a tie-break — puts `ioc` and `cc` at
effective 3 and leaves six issues at 2; 3(a) then leaves `institutional-incident` and
`automated-triage`; 3(c) picks `institutional-incident`. **Same winner by both orders.** Recorded so
no successor has to re-derive it.

**The policy did NOT run out this cycle.** Unlike cycle 103, the selection is fully determined by
the three specified tie-breaks and needed no unspecified fourth step. Carry-forward **[273]** stands
as a latent defect rather than an active one — it will bite again the next time the penalised
attempts age out together.

### `automated-triage-under-refusal`: the fourteenth elimination

It survived 3(a) untouched and **died at 3(c) on age, 16 against 2.** I record this plainly, as the
queue entry asked and as nine consecutive assessments have asked.

**This was the closest it has come, and it still lost.** Cycle 107 established the precise condition
for it to win: the 3(b) penalty must displace **all three** score-2 issues with `created_cycle` 2
that outrank it — `ioc`, `consistency-calibration`, `institutional-incident` — **simultaneously**,
and a three-cycle T3→T4→T5 ladder cannot put three T3s inside one five-cycle window. This cycle the
penalty displaced **two of the three**. The third, `institutional-incident`, escaped by three
cycles: its attempt at 104 left the window at cycle 110.

So the structural diagnosis is now empirically confirmed rather than merely predicted. The issue
has **`attempts == []` after 113 cycles**, is the only such issue in the graph, and is the one the
last several assessments have called the single place where one ordinary T3 could plausibly move a
score — and the selector has now eliminated it fourteen times without a single cycle of research.

**I did not steer the ranking to reach it.** A T4 declined to move its integer to buy reachability;
a T5 must not buy it by fudging a tie-break either, and every arithmetic step above is reproducible
from `graph.json`. **The defect needs a human ruling** — either a step-4 rule that breaks
never-attempted ties in favour of the never-attempted issue, or an explicit decision that
`created_cycle` age is the intended terminal criterion and starvation is acceptable. Carry-forward
[271]/[273] both point at the same missing legislation.

### The T2 job that is outstanding and that I cannot schedule

Cycle 112 ruled `candidate_resolutions[2]` of `consistency-calibration-as-failure-mode` **for
scoring** but expressly declined the **structural** act, because editing candidate text or status is
a T2's charter. The job is small and well-specified: split it into an under-refusal candidate
(promote to `supported`; three behavioural sources `src-0014`, `src-0015`, `src-0016`; cross-domain
label attached and load-bearing) and an overconfidence candidate (leave `proposed`; single-source
`src-0013`).

**`prompts/t5_select.md` provides no way to schedule a T2 outside the all-issues-at-5 case.** Its
only T2 branch is "if ALL issues score 5". Nine issues score 2 and one scores 3, so that branch is
unavailable, and inventing a task type is not a T5's licence. **I therefore scheduled the T3 the
prompt requires and record the outstanding T2 job here** so it is not lost. It has now been
outstanding one cycle. `ctr-0058` step (i) and `ctr-0059` step (ii) are *also* T4-or-T2 jobs, so the
T2 backlog is three items deep.

### The refresh rule, derived rather than inherited

`prompts/t5_select.md` step 4 keys on `current_cycle % schedule.collect_refresh_every == 0`;
`config.yml` gives `collect_refresh_every: 7`; `current_cycle` is 113. **113 % 7 = 1 ≠ 0, so the
next task is a T3 and not a T1.** Cross-checked against run history rather than asserted: T1s fell
at cycles 1, 15, 43, 78, 99, and the T5s that scheduled them ran at 14, 42, 77, 98 — every one
divisible by 7.

**Worth recording because it is new this rotation:** cycle 112 was itself divisible by 7 (7 × 16)
but was a **T4**, so the refresh was **skipped** — the rule fires only when a multiple of 7 lands on
a T5. The ladder is T3→T4→T5, so the T5 cycles ahead are 113, 116, 119, and **119 = 7 × 17**. **The
next T1 therefore falls to the T5 at cycle 119, scheduling a T1 at cycle 120.** Since 3 and 7 are
coprime the refresh rotates through all three phases and is skipped **two rotations in three** — a
design fact worth one sentence for whoever reads this run as data: the effective collect interval is
not 7 cycles but 21.

### The instrument is still one bit wide

Live range {2, 3} across ten issues; base priority separated only the two 3s, and even they were not
separated from each other. **The tie-breaks did 100% of the work**, as on every selection since
cycle 73.
...
1. **`state/issues/graph.json`** — **`ctr-0059` appended** (issue
   `institutional-incident-real-world-impact`, `opened_cycle` 113, `resolved_cycle` null): the G2
   finding above. Spliced **by reference** in one two-input `jq -n` ([237], eighteenth cycle
   running). Contradictions 58 → 59, open 43 → 44, issues unchanged at 10.
   **[221] append-only proof, one call, all seven true:** `.issues` identical, `._schema` identical,
   top-level `keys` unchanged, old contradictions array equals a prefix slice of the new, growth
   exactly 1, new entry open, new entry's `issue_id` resolves to a real issue. Key order
   `id,issue_id,description,opened_cycle,resolved_cycle`, matching `._schema` and the prior entry.
2. **`state/queue/next_task.json`** — rewritten for a **T3** at cycle 114, `attempt_count` 0,
   `target_issue` `institutional-incident-real-world-impact`. **15,587 bytes** — marginally over
   15 KB and within the "~15 KB" of [238]; I trimmed the off-issue ban restatements (which are
   reproduced in full below) rather than cut the four bans that bite on this issue.
3. **`state/queue/last_completed_task.txt`** — `T5 select`.
4. **`logs/cycle-113.md`** — this file.
5. One scratch file (`scratch-113.json`), **deleted** before finishing.

**Not touched:** `state/assessments/scores.json` (byte-unchanged, `last=112 total=22`),
`state/knowledge/index.json` (`srcs=40 last=src-0040`, unchanged since cycle 104), every `src-*.md`,
and `graph.json`'s `.issues` array — no candidate text, status, `open_questions` or `attempts` entry
was altered. **A T5 selects; it does not assess, collect, or restructure.**

**Gates hand-checked** (the validator cannot be run — Bash refuses `python3`): **G1** — no source was
added, so no URL check applies; the only new text is `ctr-0059`, whose every factual assertion either
cites `src-0022` (present in `index.json`), quotes existing state verbatim, or is a statement about
this repository's own files and commits, each of which I read at source this cycle. **G2** —
performed and recorded above; it failed, and the failure was opened as a contradiction rather than
absorbed. **G3** — the conflict found (`ctr-0058`'s carrier count against `logs/cycle-103.md`) was
opened rather than silently resolved, and no side was picked on the substance. `jq -e .` clean on
both edited JSON files.

---

## Next task rationale

**T3 (investigate) at cycle 114**, per the state machine T5→T3, targeting the selected issue. The
refresh rule does not fire (113 % 7 = 1), so this is a T3 and not a T1; the next T1 is cycle 120.

The entry leads with **what actually blocks the score**, because breadth is not the problem and a
successor could easily waste a cycle adding breadth. Cycle 112's diagnosis is quoted directly: nine
candidates, eight supported, thirteen source ids — **the widest level-2 margin in the graph** — held
at 2 solely by **the independence bar**, since EY, KPMG and PwC all trace to GPTZero and, upstream of
two of them, to the Financial Times, which this base does not hold. The entry therefore says in
terms: **do not add a sixth institution that also traces to GPTZero.**

Two named leads attack that bar directly: **GPTZero's own public NeurIPS report** (reference [1] in
`src-0038`, carried forward by cycle 103 expressly so it would not be lost, still uncollected ten
cycles later) and **the FT reporting upstream of KPMG/PwC**. The entry is honest that ft.com is
paywalled and may simply fail, and pre-authorises a confirmed two-form retrieval failure as a
legitimate result — so the cycle is not pushed into entering FT content from a search snippet.

A **zero-fetch fallback** is supplied that is a full cycle's work on its own: `ctr-0059` step (i),
the widened sweep, which a T3 may *run* even though it may not *append the correction* (that is
marked for a T4 or T2 in both `ctr-0058` and `ctr-0059`).

All eight `open_questions` are quoted by index, length and opening clause with a `jq` command to read
each in full; they total **~48 KB**, which cannot fit inside the ~15 KB cap, and the entry **says so
rather than truncating silently**. `open_questions[6]` is flagged as a standing T2 request and not
work for the T3. Step 5 — appending 114 to the `attempts` array — is called out explicitly with the
current array, because cycles have forgotten it before.

Two hazards are flagged: the G2 target for cycle 114 (cycle 104's entered conclusion) is a T3 **on
this same issue**, which is efficient but invites conflation, so the entry demands the two write-ups
be kept separate; and the four citation bans that bite on this issue — `src-0034`, `src-0036`,
`src-0037`, `src-0022` — are restated in full, including that **curl is off the remedy axis
entirely**.
...
| Web fetches | **0** |
| Web searches | **0** |
| Bash calls | ~20 (jq/git/wc reads, plus one `>` redirect, one `mv`, one `rm -f`, one `echo >`) |
| Read/Write/Edit | 5 reads, 3 writes, 1 edit, 2 Grep |
| Assistant turns | ~24 |

Well inside `max_turns: 75`. **Zero fetches this cycle** — correctly so: a T5 is a reading task, and
this cycle's G2 was a claim *about the state*, which is checked against the state and against git
history, not against the web. The one place a fetch could have been spent (re-reading `src-0022` to
confirm `candidate_resolutions[5]`'s quotation) was unnecessary because the candidate text itself is
in `graph.json` and it was the *candidate text*, not the source, that the check was about — which is
precisely the distinction `ctr-0058` and `ctr-0059` are both about.
