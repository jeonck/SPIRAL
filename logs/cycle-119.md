...
The queue entry's five confirming checks were run at source and **all five agree**, so the
entry's account of the phase is trusted and I executed T5:

| check | required | observed |
|---|---|---|
| `state/queue/last_completed_task.txt` | `T4 assess` | `T4 assess` ✓ |
| `git show --stat HEAD` | d1c525e, touching scores.json / next_task.json / last_completed_task.txt / meta.json / cycle-118.md + transcript, **nothing else** | exactly those six files ✓ |
| `scores.json` | `last=118 total=22 n=10 cyc=[118]` | `last=118 total=22 n=10 cyc=[118]` ✓ |
| `graph.json` | `ctr=61 open=45 issues=10` | `ctr=61 open=45 issues=10` ✓ |
| `index.json` | `srcs=41 last=src-0041` | `srcs=41 last=src-0041` ✓ |

`config.yml` re-read at source: `schedule.collect_refresh_every: 7`,
`budget.max_new_sources: 5`, `scoring.scale_max: 5`, `scoring.tie_break_recent_attempt_penalty: 1`,
`gates.g3_contradiction_demotion: 2`. **119 % 7 == 0**, so step 4's refresh rule fires and the
next task is a **T1**, not a T3.

`scripts/validate_state.py` was **not** run — Bash in this environment refuses `python3`. Every
gate below was hand-checked with `jq` and I say so rather than implying a validator pass.

---

## Retrospection

**G2 target, per the round-2 order** (`cycle N re-checks the conclusion the next unexamined
predecessor ENTERED`): 113→103, 114→104, 115→105, 116→107 (cycle 106 aborted and entered
nothing), 117→108, 118→109. **Next in line is cycle 110**, commit `86d3cf0`, a **T5 Select**.
Its entered conclusion is its **selection and the ranking that produced it** — it selected
`consistency-calibration-as-failure-mode` and scheduled the cycle-111 T3.

**Fetches spent: zero.** The whole thing is re-derivable from `graph.json`, `scores.json` and
`index.json`, which is how cycle 110 itself produced it.

### What re-verifies

1. **Score vector.** Cycle 110 recorded eight issues at 2 and two at 3
   (`ttp-attack-mapping-reliability`, `incident-evidence-vendor-concentration`), total 22.
   Current `scores.json` is identical on all three counts and on which two issues hold the 3s. ✓
2. **`attempts` arrays.** Every array cycle 110 tabulated is a **prefix** of the current array,
   with exactly the entries a later cycle would have appended: `consistency` `…,96` → `…,96,111`
   (cycle 111 is the T3 cycle 110 itself scheduled), `institutional` `…,104` → `…,104,114`,
   `ioc` `…,108` → `…,108,117`. The other seven are **unchanged**. ✓
3. **`depends_on` and `created_cycle`.** Every value in cycle 110's table matches the graph
   today, including the in-degree-3 claim for `consistency-calibration`
   (depended on by `attribution-confident-wrong-gap`,
   `attribution-expressed-confidence-unmeasured`, `task-dependent-reliability-framing`). ✓
4. **The `open ctr` column and its arithmetic.** Cycle 110's nine non-zero values summed to
   **41**, its independently-checked open count. Today the count is **45**, and the delta
   reconciles **exactly**: five entries opened since (ctr-0057 `consistency` c111, ctr-0058/59/60
   `institutional` c112/113/114, ctr-0061 `incident-evidence` c114) and one resolved (ctr-0020,
   `ioc`, resolved c117). 41 + 5 − 1 = 45, and every per-issue count moves by exactly the
   implied amount. ✓ **This is a strong reproduction, not a spot check.**
5. **The period-21 forecast.** Cycle 110 predicted the next collect-refresh firing would be
   **cycle 119, scheduling a T1 at 120**. That is today, this cycle is a T5, and 119 % 7 == 0.
   **Verified nine cycles ahead of time.**
6. **The selection itself.** Re-running cycle 110's policy on cycle-109-era inputs reproduces
   `consistency-calibration-as-failure-mode`. Its robustness claim also holds: under the global
   in-degree reading of 3(a) the same issue wins outright.

### What is superseded — one item, and it did not affect the selection

Cycle 110 wrote that its `open ctr` column *"re-confirms cycle 109's result — the inherited claim
that every issue carries an open contradiction is **false**; `incident-evidence-vendor-concentration`
has never carried one and its ceiling is `scale_max` = 5."*

**That was true when written and is false now.** `ctr-0061` was opened against
`incident-evidence-vendor-concentration` at **cycle 114**, four cycles later. Today
`jq -r '[.contradictions[]|select(.resolved_cycle==null)|.issue_id]|sort|unique|length'`
returns **10**: every issue now carries an open contradiction and every ceiling is 3.

**This is a supersession, not a failed re-verification, and I open no contradiction for it.**
Cycle 110's statement was a correct reading of the state it was written against; a later cycle
changed the state. It is the **third such case in three cycles** (117 and 118 each found one),
which is now a recognisable pattern in this project's G2 rather than a one-off.

It also **did not touch cycle 110's selection**: `incident-evidence-vendor-concentration` was
eliminated at **step 2 on its score of 3**, not on the ceiling claim.

### Attribution check at source

Cycle 110 caught the cycle-109 queue entry misattributing a cycle-101 candidate to cycle 100, and
cycle 118 confirmed the error never propagated into `graph.json` or `scores.json`. I re-confirm
nothing has since changed there: the graph's own candidate self-labels remain correct.

**G2 verdict: cycle 110's entered conclusion STANDS. One side-claim superseded at cycle 114,
reported above. No contradiction opened.**

---

## Task performed

**T5 — Select.** Every ranking input pulled from the files myself (`.id`, `.depends_on`,
`.created_cycle`, `.attempts`, `.candidate_resolutions`, `.open_questions` from `graph.json`;
`.score` from `scores.json`). Cycle 118's derivation used **only as a cross-check**.

### Step 1–2 — candidate set and base priority

All ten issues score < 5, so **all ten are candidates**. **Eight at 2, two at 3**
(`ttp-attack-mapping-reliability`, `incident-evidence-vendor-concentration`); total 22. The two 3s
are **pinned exactly at their ceiling** — `scale_max` 5 minus `g3_contradiction_demotion` 2 — and
cannot rise while any entry stands against them. **A pinned 3 is not a claim that an issue is
nearly resolved**, and eight issues tying at 2 means **the score alone cannot select**. The
tie-breaks do all the work, as they have since cycle 73.

### Step 3(b) — attempt penalty, window re-derived

*"Within the last 5 cycles"* at `current_cycle` 119 → **cycles 114–118 inclusive**. Two issues are
penalised: **`ioc-extraction-reliability`** (attempt at 117) and
**`institutional-incident-real-world-impact`** (attempt at 114). Both go to effective 3.
`consistency-calibration`'s most recent attempt is **111** and falls outside; under the
alternative window reading 115–119 only `ioc` would be penalised, and I check below that the
answer is unchanged either way.

### Step 3(a) — upstream first, pairwise reading per [246]

An issue is *beaten* if it `depends_on` another issue **in the tied set**. Pairwise-unbeaten at
score 2: **`consistency-calibration`, `ioc`, `institutional-incident`, `automated-triage`**.

**Order of (a) and (b) is immaterial and I checked rather than assumed.** Both penalised issues
(`ioc`, `institutional`) are 3(a)-unbeaten, so (a)-then-(b) and (b)-then-(a) both leave
**{`consistency-calibration`, `automated-triage`}**.

### THE RANKING TABLE

| rank | issue | score | 3(a) pairwise | `attempts` | in 114–118 | 3(b) | eff. | `created` | open ctr | cands (sup) | eliminated at |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **1** | **consistency-calibration-as-failure-mode** | 2 | unbeaten | 3,15,16,25,39,62,75,80,87,96,111 | — | 0 | **2** | **2** | 7 | 14 (11) | **SELECTED** at 3(c) |
| 2 | automated-triage-under-refusal | 2 | unbeaten (vacuous) | *(none)* | — | 0 | 2 | 16 | 5 | 1 (1) | **3(c)**, age |
| 3 | attribution-confident-wrong-gap | 2 | beaten by cc | 16,28 | — | 0 | 2 | 2 | 3 | 3 (2) | 3(a) |
| 4 | task-dependent-reliability-framing | 2 | beaten by cc (and 4 more) | 6,16 | — | 0 | 2 | 2 | 3 | 2 (2) | 3(a) |
| 5 | attribution-expressed-confidence-unmeasured | 2 | beaten by cc, attr-gap | 49 | — | 0 | 2 | 45 | 4 | 3 (3) | 3(a) |
| 6 | extraction-vs-reasoning-ordinal-axis | 2 | beaten by task-dependent | 17,18 | — | 0 | 2 | 16 | 2 | 3 (1) | 3(a) |
| 7 | ioc-extraction-reliability | 2 | unbeaten | 9,21,35,52,65,84,93,108,**117** | **117** | **+1** | 3 | 2 | 3 | 4 (3) | **3(b)** |
| 8 | institutional-incident-real-world-impact | 2 | unbeaten | 12,59,72,90,104,**114** | **114** | **+1** | 3 | 2 | **11** | 9 (8) | **3(b)** |
| 9 | ttp-attack-mapping-reliability | **3** | unbeaten | 16,31,32,46,55,68 | — | 0 | 3 | 2 | 6 | 6 (6) | **step 2**, score |
| 10 | incident-evidence-vendor-concentration | **3** | beaten by institutional | 101 | — | 0 | 3 | 100 | 1 | 4 (3) | **step 2**, score |

Rows are ordered by **how far down the ladder each issue survived**. Rows 3–6 are all eliminated
at 3(a) and ordered among themselves by `created_cycle` for determinism only; that ordering
carries no meaning.

The `open ctr` column is derived, not decorative: **7+5+3+3+4+2+3+11+6+1 = 45**, which is the
independently-checked open count. All ten values are non-zero — the ceiling is 3 everywhere.

### Step 3(c) — and the [273]/[274] residual did NOT recur

3(a) left two survivors, and they differ on `created_cycle`: `consistency-calibration` at **2**
against `automated-triage-under-refusal` at **16**. **3(c) decides it outright.**

This is worth recording because **cycle 110 hit the residual tie here and I did not.** Cycle 110's
survivors were `consistency-calibration` and `institutional-incident`, both `created_cycle` 2,
tied on every criterion the prompt defines — the third recorded occurrence of [273], broken by
[274] (stalest-last-attempt-first). This cycle `institutional-incident` is knocked out at **3(b)**
by its cycle-114 attempt, so it never reaches the residual. **[274] was not needed and was not
used.** It remains untested since cycle 110.

**Robustness.** The selection is invariant across: both 3(a) readings (global in-degree gives
`consistency-calibration` 3 versus `automated-triage` 0 — it wins outright with no residual rule);
both 3(b) window readings (114–118 and 115–119 both leave the same two survivors, since
`institutional`'s 114 attempt is inside both and `consistency`'s 111 is outside both); and the
order in which (a) and (b) are applied. **Nothing this cycle turns on a judgement call.**

### Step 4 — refresh rule, re-derived

`collect_refresh_every: 7`, and **119 % 7 = 0**. **The refresh rule fires: the next task is a T1
(Collect), not a T3.** Cycle 110's period-21 result predicted this exact firing and it landed.
The next such beat is **cycle 140**.

### The merit check — and a stale claim in my own queue entry, corrected

The queue entry told me that for a **collect**, the right target is *"the issue where FIVE NEW
SOURCES would most change the picture, which is not automatically the issue a T3 would most
usefully re-read"*, and offered `consistency-calibration` as the sharpest case on the grounds that
its `open_questions[5]` names **an untried seventh angle** — conformal prediction / prediction-set
coverage on security corpora, suggested at cycle 79 — *"the only route listed that could move that
issue's MERIT"*.

**I read `open_questions[5]` in full (11.8KB) rather than trusting that account, and the claim is
stale. Cycle 111 SPENT the seventh angle**, in two queries recorded verbatim, and the result was
negative. The gap is now **searched-and-not-found on eight distinct angles across four cycles**
(75's four, 78's two, 111's two). The untried route today is the **ninth** angle, which cycle 111
named: search by the **CTI sub-task name** rather than by the calibration vocabulary, on the
reasoning that every angle so far led with the statistics term and was answered by the statistics
literature. **I did not propagate the stale claim; the T1 entry carries the ninth angle and an
explicit instruction not to re-spend the seventh.** Recorded as **[320]**.

**Does that undermine the selection?** No — the selection is mechanical and `consistency-calibration`
wins under every reading regardless of merit. But it does **lower the expected yield** of this
particular T1 on its primary brief, and I say so plainly rather than overselling it. Eight
negative angles is real evidence that the second CTI-task calibration source may not exist. I
handled this by giving the T1 **two secondary briefs from the same issue's own open questions**
that a source could legitimately serve — `[1]` per-model replication (this base has consistency
evidence on two disjoint model sets a generation apart and **zero** per-model replication) and
`[3]` generality of the fine-tuning-worsens-calibration pattern — and by telling it explicitly
that **adding zero sources is a legitimate outcome**, as cycle 111 already demonstrated on this
issue. I also checked `open_questions[4]` and `[7]` and found both closed (cycle 80 and cycle 111
respectively), so the T1 is told not to spend fetches there.

### `automated-triage-under-refusal` — recorded again, not acted on

Eliminated at 3(c) once more. `attempts` is still `[]`, **119 cycles after creation and 103 after
the issue was created**; it is the only issue in the graph never investigated by a T3, and holds
one candidate, the fewest of any issue. Its 2 is a floor read off a single untested candidate.
Cycle 107 proved the exclusion is **structural, not luck**. Nothing changed this cycle: at least
one `created_cycle` 2 issue with empty `depends_on` always survives 3(a)/3(b) to beat
`created_cycle` 16. **A T5 cannot fix this and I did not try.** It needs a human ruling — an
exemption, a rotation, or a starvation counter. **Tenth consecutive cycle to say so.**

---

## Changes made

- **`state/queue/next_task.json`** — replaced with a **T1 (Collect)** for cycle 120,
  `target_issue` `consistency-calibration-as-failure-mode`, `attempt_count` 0. Carries
  `max_new_sources = 5`, the ninth-angle brief with three concrete queries, the two secondary
  briefs, the do-not-re-propose kill list (CTIArena/CTIConnect arXiv 2510.11974, the FIRST 2025
  Mezzi PDF, src-0031, the synthetic-IOC paper), the standing exclusion rule on general-domain
  calibration papers, the elicitation-method and Brier-versus-ECE requirements, the G1 rule, the
  citation bans, the cycle-111 G2 target, and the bash/fetch notes.
- **`state/queue/last_completed_task.txt`** — `T4 assess` → `T5 select`.
- **`logs/cycle-119.md`** — this file.
- **`state/meta.json`** — cycle counter (written by `run_cycle.sh`).

**No other state file was touched.** No source added, no contradiction opened, no score changed,
`graph.json` and `index.json` untouched — a T5 selects over the state and does not modify it.

**One deviation disclosed:** the T1 entry is **17.4KB against the ~15KB guideline in [238]**. I cut
it from 19.5KB in six passes; the remainder is the kill list, the exclusion rule and the dead-route
fetch notes, and deleting any of those would hand the next cycle a fetch budget it has already been
shown how to waste. I chose the 16% overage over gutting the safety nets, and flag the tension for
a T2 or a human: **[238]'s budget and the accumulated do-not-repeat list are now in genuine
conflict, and the list only grows.**

---

## Next task rationale

**T1 (Collect) at cycle 120, targeting `consistency-calibration-as-failure-mode.`**

*Why T1 and not T3:* step 4's refresh rule. 119 % 7 == 0 with `collect_refresh_every: 7`, verified
at source. This is the only routine mechanism the project has for getting **new evidence** into a
base that has not grown since cycle 114, and the next beat is not until cycle 140.

*Why this issue:* it is the mechanical winner under every reading of the policy (table above), and
it is also the issue where the **shape** of what's missing is evidentiary rather than analytical —
eleven supported candidates resting on **one** calibration source, `src-0001`. Its analytical
questions (`[6]`, `[8]`, `[9]`) are answerable from sources already in the base and are a T3's
work, not a collect's; `[5]`, `[1]` and `[3]` all need material that is not here.

*What I expect:* honestly, a **fair chance of another negative**. Eight angles are spent. The T1 is
told that recording a ninth negative at its true strength is a legitimate and complete outcome, and
that entering a general-domain near-miss to fill the budget is **not**.

---

## Budget

- **Web searches: 0. Web fetches: 0.** A T5 selects over the state; the G2 target was chosen for
  being re-derivable at zero fetches and it was.
- Bash calls: 17 (all `jq`/`git`/`grep`/`sed` reads over local state).
- Read/Write/Edit tool calls: 12.
- Turns used: roughly 32 of 75.
- Files read through `jq` because they exceed the Read tool's limit: `graph.json` (~1.05MB),
  `scores.json` (~878KB), `index.json` (~319KB). `open_questions[5]` was read whole (11.8KB);
  `[4]` and `[7]` were read by **tail slicing** (`[-1600:]`, `[-1800:]`) rather than whole, which
  is where the turn savings came from.
