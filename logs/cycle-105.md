# Cycle 105 — T4 (Assess) — every issue scored

## Task performed

**T4 (Assess)** per `prompts/t4_assess.md`, which I read at source rather than trusting the
queue entry's account of it. `target_issue` is null because a T4 scores every issue.

**Phase check, first thing.** `state/queue/last_completed_task.txt` read `T3 investigate`;
`git show --stat HEAD` showed cycle 104 touching `graph.json`, `index.json`, `src-0037.md`,
`src-0040.md`, both queue files, `meta.json` and its log — no `scores.json`, consistent with a
T3. All four confirming checks the queue entry nominated passed exactly: `ctr=56 open=43
last=ctr-0056 issues=10`; `srcs=40 last=src-0040`; `last=102 total=22 n=10`;
`attempts=[12,59,72,90,104]` with `ev3` the nine expected ids. Cycle 104 completed. I am the T4.

**Refresh rule, re-derived rather than inherited.** `prompts/t5_select.md` step 4 evaluates
`current_cycle % collect_refresh_every == 0` at the T5's own cycle; `collect_refresh_every` is 7.
The next T5 is at 106 and 106 % 7 = 1, so it schedules a **T3**, not a T1. On an unbroken ladder
the next T5 on a multiple of 7 is **112**, so **the next T1 falls at cycle 113**. This confirms
the correction the queue entry made to cycle 103's arithmetic.

**What I did.** One G2 re-check (below), then scored all ten issues, and discharged the
**uncensored-merit debt** this file has carried since cycle 97. **Two fetches, zero searches,
zero new sources, zero contradictions opened.**

### The state was verified, not assumed

Rather than trust the queue entry's claim that only one issue moved, I extracted
`graph.json` at the cycle-102 commit (`git show 5c2b5a5:state/issues/graph.json`) and compared
**whole issue objects** with `==` in one `jq -n` pass over both files. Result:
`institutional-incident-real-world-impact=CHANGED`, and **SAME for the other nine**.
Contradictions went 54 → 56 with the old array an exact prefix of the new. This is a cheap,
reusable check and it is now in the carry-forward.

### The owed work (b): ttp-attack-mapping-reliability's uncensored merit — **it is 3**

Cycles 97, 102 and 103 each asked a later T4 for this figure and none supplied it. Three
consecutive assessments recorded it as unknown; this is the fourth queue entry to carry it. It
needed **no fetches**, only a read of `ctr-0045` and `ctr-0047` against the level-4 bar.

**Level 3 is met comfortably** and was never what was in doubt: six of six candidates are
`supported` — still the only issue in the graph where that holds — and the primary,
`candidate_resolutions[5]`, rests on five ids (src-0023, src-0017, src-0007, src-0020, src-0021).

**Level 4 fails, and the reason is specific.** Level 4 asks that counterarguments be *explicitly
addressed*. This issue raises them with unusual precision and addresses only some.

- **Addressed, and credited** — this is why it is not lower. `ctr-0006` and cycle 31 withdrew
  `candidate_resolutions[1]` claim (b), the within-table control, **in the candidate text
  itself**, on the ground that src-0002's IOC-side rule is one-directional substring containment
  while the ATT&CK side is exact technique-ID set intersection; and the surviving claim (a) was
  narrowed in place to the *direction* of the finding. That is a model level-4 treatment.
- **Not addressed, and decisive.** `ctr-0045` step (i) is a **no-fetch** narrowing of claim (a)
  from *institutional* non-affiliation to *disjoint authorship*, and it has never been applied. I
  checked the candidate text rather than assuming: `candidate_resolutions[1]` still reads,
  verbatim, `src-0007's authors are genuinely unaffiliated with CTIBench's` — precisely the
  institutional wording `ctr-0045` bans from citation, unamended **nineteen cycles** on. A
  counterargument whose repair is named, costless and unapplied is **raised, not addressed**.
- `ctr-0047` compounds it: it expressly declines to adjudicate `ctr-0045` step (iii) and records
  that an institutional disjointness check on the PRISM/LANCE limb is **still not groundable**
  from the state — src-0003 has stored author *names* and no affiliations, src-0007 has stored
  *affiliations* and no author names. The independence premise is confirmed on names and
  unconfirmed on affiliations, **on both limbs**.
- The primary carries its own undischarged limit: `candidate_resolutions[5]` leg (2) self-declares
  `THAT ABSENT RESTS ON ONE URL FORM AND IS NOT A CONFIRMED ABSENCE UNDER RULE (v)`.

**Merit is 3.** Level 5 is not in question — eight open questions stand.

### The consequence, and it is an instrument finding that changes how the vector reads

**No issue in the graph is currently held down by the G3 ceiling.** Nine sit at 2 under a ceiling
of 3 (slack 1); ttp sits at 3 on a *merit* of 3 — the gate coincides, it does not bind; and
`incident-evidence-vendor-concentration` carries zero open contradictions and has no cap at all.

So **carry-forward [242]'s order-blindness complaint is a *merit* flatness, not a ceiling
artefact.** Cycle 102 floated uncapping `extraction-vs-reasoning-ordinal-axis` as "the cheapest to
uncap" at two open entries. On this finding, **uncapping any issue would buy nothing at any
price**. Only evidence moves these scores now. That is a materially different diagnosis from the
one this file has carried for thirty cycles, and it is recorded in the ttp rationale, in the
ordinal-axis rationale (against cycle 102's remark), and in `_cycle105_note`.

### The owed work (a): institutional-incident-real-world-impact — held at 2, reason **replaced**

Cycle 102 held this at 2 on one precise ground: the response conjunct rested on a **single**
candidate, entered at cycle 12, never extended, reporting an *absence of pattern* over **2 of 5**
institutions. Cycle 104 did exactly the work cycle 102 named as dischargeable and extended
`candidate_resolutions[3]` to KPMG, PwC and curl — evidence array four ids → nine. **The coverage
objection is gone.** I record that plainly, because holding a score for a reason that has been
answered is inertia, not stinginess.

**What replaces it is the candidate's own self-charged limit (ii), not anything I invented.**
Level 3 asks for a primary supported by ≥2 **independent** sources. The candidate states that EY,
KPMG and PwC **all trace to GPTZero**, that KPMG and PwC additionally trace upstream to the
**Financial Times, which this base does not hold**, and that outlet count is not independence.
**The extension multiplied institutions, not routes.** That is precisely the structural defect
`incident-evidence-vendor-concentration` was created at cycle 100 to track — the two issues now
meet on it, which is the strongest practical vindication that issue's route-count claim has had.

Three further reasons not to move: the extension's headline is **negative** (four distinct
response values over four cases, plus curl off the axis — variance, not a pattern); two of the
four remedies are **unfinished processes** whose outcomes this base has not checked; and EY, KPMG
and PwC are all **consultancies, not CTI publishers proper**, so the cycle-12 scope caveat now
dilutes three of four responses where it once diluted one of two.

**What did get stronger, recorded so it is not lost:** no institution's internal QA detected the
problem and every one was corrected only after an *external* party published the finding — now
**four for four** (ENISA via src-0004/0009/0010/0012; EY; KPMG via src-0035/0036; PwC via
src-0040). ENISA is a leg genuinely independent of the GPTZero cluster. But it is a *sub-finding*
within the candidate, not its primary claim, so it raises confidence without buying the point.

**G3: I did not double-demote.** `ctr-0056` was opened at cycle 104 against this issue, but the
validator builds the open set as a **set comprehension over `issue_id`** (`scripts/validate_state.py`
lines 144–156), so an eighth open entry imposes exactly the ceiling of 3 the first imposed.

**ctr-0052 status, re-read at source.** Step (i) remains unruled and I did not rule it — step (v)
forbids a T4 acting on it. The title, read this cycle, is *"Have AI-generated CTI failures already
reached production/publication at real institutions, and how do institutions respond when
caught?"* — **no impact conjunct**, which is reading one. Cycle 102 recorded that readings one and
three both give 2. After cycle 104 that is still the position, but under reading one **for a
changed reason**: coverage no longer blocks it, independence does.

## Retrospection

**G2 target, derived not inherited.** The round-2 order is "cycle N re-checks the conclusion the
next unexamined predecessor entered": 99←90, 100←91, 101←92, 102←93, 103←94, 104←95, so **105
takes cycle 96's**. I verified by reading `logs/cycle-096.md` rather than trusting the queue line.
Cycle 96 was a T3 on `consistency-calibration-as-failure-mode` and entered
**`candidate_resolutions[13]`** (status `supported`, eight ids) as the positive answer to
`open_questions[8]`. That is the target.

**What I checked, and why that limb.** I picked the candidate's **most falsifiable** limb —
cycle 96's own named **JOB A**: src-0014's render self-reported TRUNCATED at "6. Conclusion", so
its appendices never rendered and *every src-0014 ABSENT in this base is body-only*. If an
appendix elicits confidence, "six for six" and "exactly two" both break.

**Method: the working pattern.** Two render families (direct `arxiv.org/html/2605.24171` and the
`r.jina.ai` proxy), asked **unprompted** — no candidate values supplied, so neither render had a
string to echo back.

**Verdict: PASS.**

- Both returned **ABSENT** for any model-expressed confidence or token-probability value across
  the whole document.
- Both **independently reproduced the stored abstention sentence verbatim**: `When no label
  receives a true majority across the requested samples, the final output is treated as an
  abstention (UNKNOWN)`.

**JOB A is only partly advanced and I do not claim it.** Both renders now **enumerate five
appendix headings** (A Prompt Templates and Output Protocol, B Label Parsing Logic, C Generation
Settings and Configurations, D Evaluation Metrics, E Supplementary Result Tables) which cycle 96's
render did not surface at all — but **both still self-report truncation and neither rendered any
appendix body**. Appendix D is *Evaluation Metrics*, exactly where a confidence metric would sit
if one existed. **src-0014's ABSENTs remain body-only** and need a *different artefact*, not a
fourth render of the same URL.

**One render disagreement, recorded as a disagreement and not as a contradiction**, because it
concerns render *extent* and not a stored claim: the direct render reports truncation
mid-Conclusion, the proxy mid-References, and the proxy lists a sixth heading "Appendix E.1
Additional Raw Results". **No contradiction opened** — nothing here conflicts with a supported
claim.

## Changes made

**`state/assessments/scores.json`** (~600KB, read and written through `jq` only), built with the
two-input splice [237] from one scratch file [211], then `mv`d into place:

1. **A `=== CYCLE 105 (T4) ===` block appended to all ten rationales.** Nothing rewritten.
2. **`assessed_cycle` set to 105 on every entry**, via `.scores |= map_values(.assessed_cycle = 105)`.
3. **`last_assessed_cycle` 102 → 105.**
4. **`institutional-incident-real-world-impact.evidence`** extended by `src-0035`, `src-0036`,
   `src-0040` (append only) — the sources cycle 104 brought into `candidate_resolutions[3]`.
   All three exist in `index.json`. **No other evidence array touched.**
5. **`_cycle105_note`** added at top level.

**Every score integer is unchanged. Total 22 across ten, identical to cycle 102's vector** — and
every entry was nevertheless re-derived, with the nine untouched issues *proved* byte-identical
rather than assumed.

**Append-only proof (carry-forward [221]), 13 checks in one `jq -n` call, all true.** Key sets
identical; all ten score integers unchanged; total 22; n=10; `assessed_cycle` uniquely `[105]`;
every old rationale a `startswith` prefix of its new one and every one strictly longer; every old
evidence array equal to a prefix slice of the new; exactly one evidence array changed
(`institutional-incident-real-world-impact`); per-entry key sets identical; `_schema` identical;
all eight prior `_cycleNN_note` values byte-identical.

**Gate hand-check** (I cannot run `scripts/validate_state.py` — `python3` is refused). **G1:** no
new source, no new URL; every score ≥2 cites source ids present in `index.json`. **G2:** performed
and recorded above. **G3:** none opened; none needed. **Ceiling:** applied as a **flat ceiling**
of `scale_max − g3_contradiction_demotion` = 3 per the validator's set comprehension, **not** as a
per-point subtraction — `prompts/t4_assess.md` step 3 reads it as a subtraction and the two
disagree. **Carry-forward [4], now in its ninety-second cycle, awaiting a human. I followed the
validator**, as cycles 76/81/85/88/91/94/96/97/102 did, and say so in every rationale.

**Not changed:** `state/issues/graph.json`, `state/knowledge/index.json`, every `src-*.md` —
byte-untouched, as a T4 must leave them.

**Queue:** `state/queue/next_task.json` written for a **T5 (select)** at cycle 106,
`target_issue` null, `attempt_count` 0, **15,379-byte `instructions` field**. `state/queue/last_completed_task.txt`
reads `T4 assess`. Scratch files `scratch-105.json` and `old-graph-102.json` removed.

## Next task rationale

T4 → T5 per the state machine. The T5 at 106 is not a refresh cycle (106 % 7 = 1), so it schedules
a **T3**; the next T1 falls at **113**. The entry hands the T5 three things it cannot cheaply
re-derive: the **G3-is-inert finding**, which tells it not to spend a cycle uncapping anything;
the **replaced blocker** on institutional-incident, so it does not re-price a discharged objection;
and the standing pricing that **`automated-triage-under-refusal` is the only issue blocked by
dischargeable defects** — never investigated in 105 cycles, empty `attempts` array so no
tie-break penalty, and now carrying `ctr-0055`'s **named, chartered, no-fetch first step** which it
did not have at cycle 102. Six consecutive assessments have priced it that way and none has been
acted on. I state it as pricing and do not override the T5's tie-break.

## Budget

- **Fetches: 2** (both renders of `arxiv.org/html/2605.24171`, for G2).
- **Searches: 0.** **New sources: 0.** **Contradictions opened: 0.**
- **Turns: ~34 of 75.** Comfortably inside budget; both owed items discharged with room left,
  which is why neither had to be dropped for a fourth time.
- Bookkeeping written before the log, per the standing order.
