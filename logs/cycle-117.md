# Cycle 117 — T3 Investigate — `ioc-extraction-reliability`
...
- `git show --stat HEAD` → `218abd2 cycle 116: T5 select`, touching exactly
  `logs/cycle-116-transcript.txt`, `logs/cycle-116.md`, `state/meta.json`,
  `state/queue/last_completed_task.txt`, `state/queue/next_task.json` — **and nothing else**.
  No `scores.json`, no `graph.json`, no `index.json`, no new `src-*.md`. ✅
- `scores.json` → `last=115 total=22` ✅ (unchanged by cycle 116)
- `graph.json` → `ctr=61 open=46 issues=10` ✅ (unchanged by cycle 116)
- `index.json` → `srcs=41 last=src-0041` ✅

All five agree. Phase confirmed: **T3 investigate on `ioc-extraction-reliability`**.
`prompts/t3_investigate.md` read at source, not taken from the queue entry's account of it.

## Retrospection

**G2 target, per the round-2 order: cycle 108.** The order is *cycle N re-checks the conclusion
the next unexamined predecessor entered* — 115 took 105's, 116 took 107's (cycle 106 aborted and
entered nothing), so 117 takes **108's**. Cycle 108 was a T3 on this same issue; its conclusion is
the set of discharges recorded at `open_questions[12]` plus the generalisable precision-floor
result it states.

**Route: zero fetches.** Re-derived from git history against the state at cycle 107's commit
(`92b7f57`) and cycle 108's own commit (`d2c7c49`). No fetch was spent on this G2.

**Verdict: PASSES.** Every limb re-verified.

| Cycle 108's claim | Check | Result |
|---|---|---|
| (a) ctr-0038 (ii): ctr-0001's opening clause now marked DO NOT CITE with `[0.7544, 1.0]` beside it | `ctr-0001` description 10900 → **15426** chars at `d2c7c49`, unchanged since | ✅ present, and added at 108 |
| (a) ctr-0038 (iii): ctr-0010's `eliminated DEDUCTIVELY` sentence now carries a scope limit | text found verbatim: *"IT STANDS AND REMAINS CITABLE. Cycle 108 CONFIRMS it and adds a SCOPE LIMIT; this is not an impeachment."* | ✅ |
| (b) ctr-0010 (ii): the 0.09–0.15 figure recorded as a within-src-0007 IoC-vs-TTP crossover | found verbatim, incl. *"src-0003's 97.6% IS NOT AN INPUT TO IT ANYWHERE"* | ✅ |
| (d) ctr-0028 (ii): the last unmarked `a substantial challenge` use, sitting inside ctr-0010, now marked | found: *"THAT STRING IS THE MISQUOTATION ctr-0028 IMPEACHES AND IT IS DO NOT CITE"*, with src-0007's actual wording beside it | ✅ |
| ctr-0010 grew at cycle 108 | 4730 → **8412** at `d2c7c49` (→ 11773 today, from later cycles) | ✅ |

**The generalisable result re-derived from first principles, not inherited.** Cycle 108 states that
a precision floor from another system's F1 *can only ever establish that a compared precision lies
below that system, never at or above it*. Checking the algebra independently: from
F1 = 2PR/(P+R) with R ≤ 1, P = F1·R/(2R − F1), which is minimised at R = 1, giving the floor
**P ≥ F1/(2 − F1)**. Then:

- F1 = 0.976 → 0.976/1.024 = **0.953125** → the stored 0.9531 ✅
- F1 = 0.86 → 0.86/1.14 = **0.754386** → the stored 0.7544 ✅
- All four of src-0007's precisions (0.8240, 0.8503, 0.8846, 0.6944) lie below 0.9531, so the
  LANCE-above-src-0007 ordering is entailed ✅. Margins: 0.9531 − 0.8846 = 0.0685 ≈ **6.9 points**,
  0.9531 − 0.8240 = 0.1291 ≈ **12.9 points** — both match the stored values ✅
- Against the 0.7544 floor: 0.6944 falls below (established), but 0.8240 and 0.8503 fall *inside*
  [0.7544, 1.0] and are therefore **undetermined** ✅

The asymmetry is sound: a floor gives a lower bound, and the target's ceiling is the uninformative
1.0, so only *below* verdicts are ever licensed. Two comparisons of identical form, opposite
verdicts — cycle 108's characterisation is correct.

**Attributions checked at source**, because the cycle-109 entry misattributed a cycle-101 candidate
to cycle 100 and cycle 110 caught it. Cycle 108 calls itself this issue's *eighth* investigation
and its *first since cycle 93*: `attempts` at `d2c7c49` reads `9,21,35,52,65,84,93,108` — 108 is
indeed the eighth entry and the predecessor is indeed 93. ✅ No misattribution.

**One limb NOT re-verified at source, stated plainly rather than implied.** Cycle 108's (c) — that
src-0007 Table 4's TTP-recall row reads `0.2270 | 0.1759 | 0.1846 | 0.1414` — is a *source-level*
fact that only a fetch could re-check, and this G2 spent none. What I could check, I did: the
derived ban is intact, and a `Grep` for `vanilla-only` across `state/` returns exactly two hits,
both of them *statements of the ban* (in `graph.json` and the inherited `next_task.json`), not
violations of it. ✅ No contradiction opened; cycle 108's conclusion stands.

## Task performed

T3 on `ioc-extraction-reliability`, at **zero fetches and no new source**. The queue's two
commissions both landed on the same object — `candidate_resolutions[0]` — and were done in one pass.

### Commission 1 — ctr-0020's closure condition, open since cycle 52

`ctr-0020` was read in full first (9,870 chars, in three `tail -c`/`head -c` windows), and
`open_questions[12]` — cycle 108's status entry, which reported this *undone rather than dropped* —
was read in full before acting.

**The defect.** This state has said since cycle 2 that src-0003's 43% figure is a reduction in
*annotation time* (or *parsing/annotation time*). src-0003 states the figure three times in three
wordings naming three different quantities, and **`annotation time` is not one of them** — it is
this state's own word for a quantity the paper never names. The measurement-bearing sentence is:

> From the results of those measurements, we see a 43% drop in the average and median time the
> analysts spent on a report.

The population half of the original defect was already withdrawn at cycle 65 in the state's favour
(`junior` follows the paper's only method sentence, §5.2.2). Only the **quantity** limb survived,
and that is what cycle 117 repaired.

**The repair shape** is the one cycles 65 and 108 settled and is not negotiable: an **appended**
correction quoting the defective wording under a **DO-NOT-CITE** marker with the measurement-bearing
sentence beside it. Never a rewrite, never a deletion — hard rule 2 binds `index.json` key_claims
and `src-0003.md` alike.

**All three locations repaired**, in the priority order the queue set (candidate first, since that
is what a T4 scores from and a T5 ranks on):

1. `graph.json` → `issues[1].candidate_resolutions[0].summary`, appended block **(j)**
2. `index.json` → src-0003 `key_claims`, new element **[11]**
3. `state/knowledge/src-0003.md` → new appended section *"Cycle 117 (T3)"*

**The fourth location needed no repair, and I checked rather than assumed.** ctr-0020's closure
condition also names *"any scores.json rationale carrying it"*. `scores.json` carries the phrase
**only inside a warning against it** — *"the 43 percent figure never came from src-0003's abstract
and annotation time is not that source's wording (ctr-0020)"* — i.e. it carries the correction, not
the defect. Nothing there needed changing and nothing there was changed.

**ctr-0020 is now CLOSED**, `resolved_cycle` 117, sixty-five cycles after it was opened. What
closure does *not* mean is stated explicitly in the entry, in `open_questions[13]` and in the
candidate itself: **the defective strings are still physically present** in all three files,
because hard rule 2 forbids removing them. What changed is that each now carries a DO-NOT-CITE
marker and sits beside the measurement-bearing sentence, so a cycle that quotes the old wording
bare is now in breach of a standing ban rather than merely repeating an unflagged error.

**The standing ban survives the closure**: the 43% must never be cited from src-0003's abstract,
which attributes to *six* analysts a measurement §5.2.2 takes over *five*. That inconsistency is
the paper's and is not charged against this state; closing the entry does not repair the paper.

**Provenance: nothing was spent.** Every string quoted in all three repairs was already held
verbatim in this state, recorded by cycles 52 and 65 from three render families
(`arxiv.org/html/2506.11325v1`, `.../v2`, `ar5iv.labs.arxiv.org/html/2506.11325`). Cycle 117 added
no measurement, no source and no fetch — G1 is satisfied trivially, since every claim entered cites
src-0003, which exists in `index.json` with a resolving URL.

### Commission 2 — standing rule [304] single-source audit, bounded to this issue

Scope **re-derived, not inherited**: `[0] supported ev=src-0003` · `[1] supported
ev=src-0007,src-0003` · `[2] proposed ev=src-0003,src-0007,src-0008` · `[3] supported
ev=src-0003,src-0007,src-0017`. So the bounded audit is **exactly one candidate, [0]** — the same
one ctr-0020's repair touches, as cycle 116's carry-forward [313] predicted.

src-0003's `Limitations of this source` lists three. Result:

- **Already carried, needing no addition (2 of 3).** The conflict-of-interest limit is carried by
  the candidate's own *"a single-study result from the benchmark's own authors, not yet
  independently replicated"*; the scope limit is carried by its opening clause *"For narrow,
  mechanical IoC extraction (as opposed to open-ended CTI reasoning)"*.
- **Not carried, now recorded (1 of 3).** The HTML-summarization retrieval caveat. It is **largely
  superseded rather than live** — cycles 22, 52, 65, 84 and 93 re-fetched across three render
  families and text-verified the 97.6%, the 86%, the three 43% wordings, the 86.8% per-type recall
  floor and the provenance, and impeached the 76%/72% as figure-derived *because* of that scrutiny.
  What genuinely survives: **no cycle has read this paper end to end**, and the *"precise regex/LLM
  task split"* half of the caveat is undischarged — which is exactly what `open_questions[2]` asks.

Per [304](c), **no score change is proposed**. An unstated limit is a documentation defect; a T4
prices the source's actual limits, and a T3 does not score.

### What did not move — stated so no later cycle mistakes bookkeeping for evidence

No new source, no new measurement, no promotion, no demotion. The substantive question sits exactly
where cycles 93 and 108 left it: `[0]`'s SYSTEM-versus-CORPUS gap untouched and **no head-to-head
exists**; `[1]` generalisation beyond the four indicator types untouched; `[2]` the
scaffolding-versus-LLM decomposition untouched; `[3]` unrecoverable from the released artefact;
`[11]`'s 0.9344 floor still **an inference**, and ctr-0001's CORPUS confound still narrowed but not
settled.

`[4]`, the adversarial threat model, is **declined again — the fifth time**, on the identical ground
cycles 21, 52, 93 and 108 gave, with no attempt to re-litigate it: no source in this base measures
report-based extraction under an adversarial threat model, so no T3 can discharge it and only a T1
can. `ctr-0010` step (i) remains undone and is **not doable by a T3** — it asks a T4 to replace the
mis-scoped 0.09–0.15 restatement in `scores.json`; it is now commissioned explicitly to cycle 118.

**G3:** the investigation surfaced no claim conflicting with any other issue's supported
resolutions — it entered no new factual claims at all, only corrections to existing ones. No new
contradiction was owed and none was opened.

## Changes made

- `state/issues/graph.json`
  - `issues[1].candidate_resolutions[0].summary` += block **(j)**/(k)/(l) (4,409 chars)
  - `issues[1].open_questions` += `[13]`, the cycle-117 status entry (5,259 chars) — 13 → **14**
  - `issues[1].attempts` += `117` → `9,21,35,52,65,84,93,108,117`
  - `contradictions[19]` (ctr-0020): description += cycle-117 update (3,515 chars);
    `resolved_cycle` `null` → **117**
- `state/knowledge/index.json` — src-0003 `key_claims` += `[11]` (2,195 chars). 11 → **12**.
  `.sources|length` **still 41**; no source added.
- `state/knowledge/src-0003.md` — new appended section. `git diff --numstat`: **60 insertions,
  0 deletions**.
- `state/queue/next_task.json` — T4 assess for cycle 118 (14,399 chars, under the ~15KB [238] cap)
- `state/queue/last_completed_task.txt` — `T3 investigate`
- `logs/cycle-117.md` — this file
- Scratch file `state/scratch-117.json` created for the [237] two-input splice and **deleted**
  before finishing.

**Gate hand-checks.** `scripts/validate_state.py` cannot be run — Bash refuses both `python3
script.py` and `python3 -c` — so the gates were hand-checked with `jq` and I say so rather than
implying a validator run:

- **Append-only proof on `graph.json`**, ten conjuncts, all `true`: every issue `id` and `title`
  unchanged and in order; every pre-existing `open_question` a `startswith`-prefix of its new value;
  every pre-existing `candidate_resolutions[].summary` a prefix of its new value; no array shrank;
  every contradiction `id` unchanged; every contradiction `description` a prefix of its new value;
  both top-level array lengths unchanged.
- **Append-only proof on `index.json`**, six conjuncts, all `true`, and *stronger than a prefix
  test*: every pre-existing `key_claim` is **byte-identical** (`==`), every source `id` and `url`
  unchanged, `.sources|length` unchanged, and src-0003's `key_claims` grew by **exactly one**.
- Post-change invariants: `ctr=61 open=45 issues=10`, `ctr-0020 resolved=117`,
  `oq=14`, and — the one that matters for cycle 118 —
  `[.contradictions[]|select(.resolved_cycle==null)|.issue_id]|sort|unique|length` **still 10**, so
  closing ctr-0020 relieved no gate pressure anywhere and every issue stays capped at 3.

## Next task rationale

State machine: **T3 → T4**. Cycle 118 is a T4 assess over all ten issues. Not a T1: `118 % 7 = 6`,
and per `config.yml` `schedule.collect_refresh_every: 7` the next collect falls at **cycle 119's
selection** (`119 % 7 = 0`), i.e. a T1 running at cycle 120 — the queue entry says so explicitly so
that T5 does not have to rediscover it.

The entry carries, as commissioned: the **ceiling fact** (all ten capped at 3 = `scale_max` 5 −
`g3_contradiction_demotion` 2, with the `jq` to verify the ten-of-ten open-contradiction spread
rather than trust it), and that `incident-evidence-vendor-concentration` is **pinned exactly at its
ceiling at 3** — as is `ttp-attack-mapping-reliability`, which I add because it is the same trap.
It also warns that the `open` count legitimately fell 46 → 45 this cycle, so the T4 does not read
its own phase check as a violation.

Two repairs are commissioned to cycle 118 because only a T4 can do them: **ctr-0010 step (i)**
(open since cycle 35, reported as a T4's job by both cycle 108 and cycle 117, with the corrected
scope of the 0.09–0.15 figure supplied so it need not re-derive it), and the surviving half of
**ctr-0020 step (iii)** (candidate [0] still carries the two figure-derived 76%/72% baselines).
[304](c) is restated as a guard *against* a demotion the T4 might otherwise make from this cycle's
documentation finding.

G2 for cycle 118 is **cycle 109** — a T4, so its conclusion is the scores and rationales it wrote,
and it is a cycle with a *known* attribution error that cycle 110 caught. The entry flags that and
supplies the `git show d2c7c49:...` route so it can be done at zero fetches.

## Budget

- Web searches: **0**
- Fetches: **0** — both the G2 and the main task were discharged entirely from the existing state
  and git history. This was possible because ctr-0020's repair was a *wording* defect: every string
  it needed was already recorded verbatim by cycles 52 and 65.
- Bash calls: ~29. Read/Write/Edit/Grep tool calls: ~8. Turns used: well inside `max_turns: 75`.
- New sources added: **0** (a T3 is not a T1; `open_questions[4]` remains T1-only).
