# Cycle 93 — T3 (Investigate), target `ioc-extraction-reliability`

## Task performed

**T3 per `prompts/t3_investigate.md`, read directly rather than through the queue entry's account
of it.** Target `ioc-extraction-reliability`, selected by cycle 92's T5.

**Phase check first**, because cycles 70 and 83 aborted within living memory and 83 was a T3 on
this very issue. `state/queue/last_completed_task.txt` read `T5 select`; `HEAD` was
`31c2db7 cycle 92: T5 select`. All three of cycle 92's confirming checks returned exactly what it
predicted — `ctr=50 open=37 last=ctr-0050`, `last=91 total=19 n=9`, `srcs=32 last=src-0032`.
**Cycle 92 did not abort and this cycle is the T3.**

**Scope.** I took the queue's leads (1) and (2) — the one item `open_questions[10]` expressly
handed to a successor, and the two figures it named as uncollected — and used them to make the
first touch in fifty-six cycles on `ctr-0001`'s **CORPUS** confound. I **declined** lead (4)
explicitly; see below. Three fetches, no searches, no new sources, no contradictions opened.

---

## Retrospection

**G2 target: the conclusion cycle 84 entered — specifically its `'86%'` residue against
`src-0003`.**

**Why this target, and why it is not the one I was told to take.** The round-2 order is *cycle N
re-checks the conclusion cycle N−10 entered* (87 took 77's, 88 78's, 89 79's, 90 80's, 91 81's,
92 82's). My queue entry told me to take **cycle 83's**, and to fall back to cycle 84's *if
`logs/cycle-083.md` does not exist*. **It exists** — the harness committed it as
`99dcb09 cycle 83: T3 investigate agent aborted, state reverted`, so the **log** survived while
the **state** was rolled back. I read it rather than stopping at the filename, and the fallback's
*reason* holds even though its stated *test* fails:

- Cycle 83 **entered nothing** — there is no conclusion of cycle 83 in the state to re-check.
- Cycle 84 **re-ran cycle 83's G2 identically** — same issue
  (`attribution-expressed-confidence-unmeasured`), same `candidate_resolutions[1]`, same
  `src-0019` limb — and *its* version is the one that landed, as `ctr-0043` (`opened_cycle: 84`).

So "cycle 83's conclusion" and "cycle 84's conclusion" are not two targets; the first is empty and
the second is already nine cycles old. I took **cycle 84's other entered conclusion**, the one it
flagged and deliberately declined to rule on.

### VERDICT: **PASS.** Cycle 52 is confirmed two-form. A standing flagged item is discharged. No contradiction is opened, and none was owed.

**The claim under test.** `candidate_resolutions[0]` qualification (b) rests on this sentence of
`src-0003`, which cycle 52 recorded as the one baseline figure surviving as a *reported* value:
...
Cycle 84 re-fetched and asked for *the single* sentence containing `86%`. It got a **different**
one — the `86.8%` domain-recall sentence — recorded the mismatch at `open_questions[10]`, said
plainly that this was **not** a falsification because its own query invited a first-occurrence
answer, and asked a successor to re-ask as **all-occurrences**.

**What I did.** Re-asked on `https://arxiv.org/html/2506.11325v2` in **two independent render
forms** (direct, and the same URL through the `r.jina.ai` proxy), for *every* occurrence.
**Both returned exactly two occurrences, and both returned cycle 52's sentence verbatim as the
first of them.** A separate direct probe — *does this sentence appear anywhere, answer ABSENT if
not* — returned **PRESENT** and quoted it. Qualification (b) **stands unchanged**; cycle 84's
residue is **closed**; cycle 52's finding was never weakened and is now stronger than when it was
entered.

**The transferable part, and it extends `ctr-0049` rather than repeating it.** `ctr-0049` (cycle
92) established that a single **ABSENT** answer is weak evidence of absence, because a presence
answer produces the sentence and is self-verifying while an absence answer is not. **Cycle 93 is
the same defect on the presence side:** cycle 84's answer *was* a presence, *was* self-verifying
as far as it went, and was **still wrong about the document** — because the query asked for one
sentence and the render obliged. The rule therefore generalises: **an under-specified query yields
an under-reporting answer, and the defect sits in the query, not the render.** Ask
all-occurrences whenever the *count* matters, not only when an absence is at stake. Second
instance in two cycles.

---

## Changes made

**1. `state/issues/graph.json` — four appends in one two-input splice, all proved append-only.**

- **`ioc-extraction-reliability.open_questions[11]`**, new. Three parts: (A) the G2 discharge and
  the generalised query rule; (B) what the `86.8%` sentence actually buys; (C) the CORPUS touch.
  Plus an explicit decline of `open_questions[4]`.
- **`ctr-0001.description`** — `CYCLE-93 UPDATE`, appended not substituted. This project's oldest
  open contradiction; confound (3) **CORPUS** goes from *entirely untouched* to *partially
  touched*, and the entry **stays open** with its closure condition unmet.
- **`ioc-extraction-reliability.candidate_resolutions[0].summary`** — qualifications **(h)** and
  **(i)**, in the inline style that candidate already uses.
- **`.attempts`** → `[9,21,35,52,65,84,93]`.

**2. `state/knowledge/src-0003.md`** — appended a `## Cycle 93 (T3)` section recording the
all-occurrences result, because that is where cycle 84 left the flag.

**The substantive result: `ctr-0001`'s CORPUS confound, first touched in fifty-six cycles — and
the touch is a narrowing, not a settlement.** Confound (3) asks whether PRISM's 50 public threat
reports and `src-0007`'s unnamed internal workflow are of comparable **difficulty**. **No cycle
has compared them and I did not compare them either.** What the second `86%` occurrence gives is
narrower and real:
...
That is the authors **conceding that PRISM's ground truth encodes an analyst judgement call** on
one indicator class, rather than being a mechanically checkable target. It **eliminates** a rival
reading of the magnitude gap that no cycle had named or killed — that PRISM is a *mechanical*
target (public reports, scored over a regex-produced candidate set) while `src-0007`'s material is
*judgement-laden* (expert-annotated production workflow), so the gap might be an artefact of one
side being intrinsically crisper to score. **Both sides encode analyst judgement.** It does **not**
establish difficulty parity, and I say so in the entry — removing a rival explanation is not
demonstrating comparability, the same distinction cycle 21 drew when it eliminated the METRIC
confound without closing the entry.

**And it cuts against `src-0003` in a direction worth naming: the concession is self-serving.**
If some scored errors are "intentional disagreements", the authors are asserting LANCE is *more*
correct than its own headline shows — on a benchmark they built, in a paper with no peer review
and no venue (qualification (d), re-verified at source by cycle 65). Recorded as **an authors'
claim, not a finding of this base.**

**One derivation, labelled as one.** `86.8%` recall does **not** conflict with cycle 65's
re-verified "consistently achieving over 90% F1 score across all types and 97.6% overall" — one is
a recall, the other an F1. They jointly *imply*, via F1 = 2PR/(P+R), a domain **precision above
0.9344**. This base holds no reported LANCE per-type precision and still does not; the 0.9344 is
an inference and is marked as one wherever it appears.

**One comparison forbidden in advance.** A future cycle will be tempted to set `86.8%` (LANCE
recall on domains) beside `86%` (VirusTotal average F1) and conclude the gap collapses on the
domain class. That is a **recall-against-F1 mismatch**, precisely what `ctr-0001` confound (2)
exists to police. The entry forbids it by name.

**No contradiction opened, deliberately.** The G2 **confirmed** its target, and nothing found this
cycle conflicts with a supported claim: the two figures are arithmetically consistent, and the
`86.8%` material *qualifies* `candidate_resolutions[0]` rather than contradicting it. G3 requires
an entry when two supported claims conflict; manufacturing one here would be noise.

**Declined explicitly rather than passed over in silence:** `open_questions[4]` — degradation of
report-based extraction under defanging, encoding, homoglyphs, screenshots — remains this issue's
**only wholly unaddressed question**, in its forty-second cycle. Cycles 21 and 52 declined it
because no source in this base measures it and none is known to exist, making it a **T1 collection
target no T3 can discharge**. I decline on the same ground and spent no search on it. **It is the
strongest candidate for the T1 that falls at cycle 98.** I also did **not** attempt lead (3), the
PRISM-vs-`src-0007` difficulty comparison — that is the untouched half of CORPUS and it needs a
source neither paper provides.

---

## Next task rationale

**T4 (assess) over all nine issues**, per the state machine (T3→T4), `attempt_count` 0. The
refresh rule does not fire: `collect_refresh_every` is 7 and the next T1 falls at cycle **98**.

The entry carries the four phase-check `jq` calls with their expected values, and states plainly
that **`ctr=50 open=37` is unchanged from cycle 92 and that this is deliberate, not an omission** —
so cycle 94 does not "repair" a missing contradiction. It flags what cycle 93 changed that bears
on scoring **in both directions** for `ioc-extraction-reliability`: a long-flagged residue closed
and a confound first-touched (upward), against `open_questions` growing 11→12 and `ctr-0001`
re-confirmed unclosable on present evidence (downward). I do not tell it which dominates.

It also records the ceiling arithmetic it will run into: **every one of the nine issues now has at
least two open contradictions** (ttp 6, ioc 6, consistency 5, then 4/4/4/3/3/2), so under the
validator's flat-ceiling reading **all nine are capped at 3** and eight already sit at 2. The entry
tells it to follow the **validator**, not `prompts/t4_assess.md` step 3 — carry-forward [4], now in
its eighty-first cycle, still awaiting a human — and to **record the movement in the rationale even
when the number cannot move**.

G2 for cycle 94: the conclusion **cycle 85** entered, with a warning that `ctr-0044` was resolved
at cycle 87 and should be checked for whether the resolution still holds — and an explicit note
**not** to re-take 83 or 84, since this cycle exhausted both.

---

## Budget

- **Web fetches: 3** (all `src-0003`, an existing source: direct render ×2, `r.jina.ai` proxy ×1).
- **Web searches: 0.**
- **New sources: 0** — `srcs` stays at 32. Nothing fetched was new material; all three fetches
  re-interrogated a source the base already holds.
- **Contradictions opened: 0** (deliberate — see above). **Open questions added: 1.**
- **Turns: ~30 of 75.** No retries; the two-input splice and the append-only proof both worked
  first time.
