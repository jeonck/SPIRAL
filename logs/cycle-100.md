# Cycle 100 — T2 Structure

## Phase check (run first, before anything else)

`state/queue/last_completed_task.txt` read **`T1 collect`**. `git show --stat HEAD` showed
`cycle 99: T1 collect` touching `index.json`, five new `src-00{33..37}.md`, `meta.json` and the
queue files — and **not** `graph.json`, as cycle 99's own handoff predicted. All three
confirming counts matched exactly:

| check | required | actual |
|---|---|---|
| `state/knowledge/index.json` | `srcs=37 last=src-0037` | `srcs=37 last=src-0037` |
| `state/issues/graph.json` | `ctr=51 open=38 issues=9` | `ctr=51 open=38 issues=9` |
| `state/assessments/scores.json` | `last=97 total=19` | `last=97 total=19` |

I am the T2. `prompts/t2_structure.md` was read at source before acting.

---

## Task performed

**T2 — Structure**, with one headline item: **discharge `open_questions[6]` of
`institutional-incident-real-world-impact`**, the split proposal written at cycle 72, carrying a
case cycle 69 asked to be parked, and endorsed six times by T4s at cycles 81, 85, 88, 91, 94 and
97 without any of them being chartered to act. No T2 had run since cycle 79 — twenty-one cycles.

### The decision: DO NOT SPLIT

I was ordered to decide, not to split, and a reasoned refusal is a complete outcome. I refuse on
five grounds, **four of which were not available to cycle 72**. All five are written into the
graph at the end of `open_questions[6]` so the reasoning survives me.

**(1) The precedent the case rests on did not work, and the case does not know that.**
`open_questions[6]` cites `attribution-confident-wrong-gap`, split at cycle 45, as the precedent
for exactly this situation. I checked the **outcome** rather than the intent:

| | pre-split | first post-split (c47) | today (c97) |
|---|---|---|---|
| strong half (`attribution-confident-wrong-gap`) | **2** (c41) | **2** | **2** |
| new half (`attribution-expressed-confidence-unmeasured`) | — | 0 → **1** (placeholder repair) | **2**, 4 open contradictions, capped at 3 |

The strong half **has never risen in the fifty-two cycles since the split.** The new half
accumulated `ctr-0024`, `ctr-0029`, `ctr-0034`, `ctr-0043` and is capped exactly as its parent
was. And this is not the only data point: cycle 45's *own* carry-forward `[1]` records that the
cycle-16 split of `task-dependent-reliability-framing` left both halves "at 2 and fell for the
same root defect — **weak evidence that the split was along the right seam**."

**This loop has split an issue twice, both times to free a strong half from a weak one, and
neither split ever moved a score.** That is the single most decision-relevant fact available on
the question and `open_questions[6]` was written without it. A third split proposed on the same
reasoning with the same predicted mechanism owes an explanation of why the outcome would differ.
The case offers none.

**(2) The case's central premise is falsified by cycle 99.** It asserts half (B) "cannot be
strengthened by any amount of work on half (A)". Cycle 99 strengthened half (B) **as a
by-product** of collecting for half (A): the same GPTZero sweep that produced the KPMG incident
(`src-0034`) produced the outlets carrying KPMG's response (`src-0035`, `src-0036`), taking
recorded responses from N=2 to N=3 — half (B)'s first movement since cycle 12.

The queue entry required me to weigh this in both directions and I did. The opposite reading —
that half (B) being *movable at all* is the thing `[6]` said it was not — is equally available.
**Both readings point away from the split**, which is why I state this as falsification rather
than as a close call. The halves are coupled *in collection practice*, and that is the coupling
that determines whether work on one advances the other.

**(3) The contradiction load has migrated to half (A), which inverts the case's arithmetic.**
`[6]` names `ctr-0012`, `ctr-0018`, `ctr-0030`, `ctr-0032` and assigns one to (A) and two to (B).
`ctr-0012` was resolved at cycle 59 and `ctr-0018` at cycle 90. I re-derived the assignment from
each open entry's own description rather than copying the stale list:

| contradiction | what it is about | half |
|---|---|---|
| `ctr-0025` | unconfirmable GhostCite definition quotation on `src-0011`, sole evidence for `candidate_resolutions[4]`, the **base-rate** candidate | **(A)** |
| `ctr-0030` | one-sided-bound arithmetic on `src-0022`, the curl case | **(A)** |
| `ctr-0041` | ENISA version-premise defect under `candidate_resolutions[7]` | **(A)** |
| `ctr-0032` | institutional-response divergence construct error under `candidate_resolutions[3]` | **(B)** |

**Three of four land on half (A), not one.** Under the flat G3 ceiling a split leaves half (A)
carrying three open entries and capped at 3 — precisely where the undivided issue sits today —
while half (B) drops to one and is capped at 3 as well. **The split buys half (A) no ceiling
relief whatsoever.** Since the entire practical case is that half (A)'s evidence cannot lift a
shared score, and half (A) turns out to carry most of the gate pressure itself, the case does not
survive its own corrected bookkeeping.

**(4) The actual blocker is a ruling, and splitting would destroy the evidence for making it.**
Cycle 97 states it without hedging: on this issue carry-forward `[240]` is *the sole blocker* and
`[97]` does not arise at all, so a human "settling `[240]` alone would move this issue." A
permissive settlement of `[240]` moves this issue **and** `consistency-calibration-as-failure-mode`
with no split and no new evidence — two scores from one ruling. A split does not settle `[240]`;
it **evades** it by dissolving the conjunction, and removes one of only two test cases a human
would use. Cycle 88 required a T2 to settle `[240]` **first**; 91, 94 and 97 re-endorsed that.
I cannot settle it — no task type is chartered to make a cross-issue rubric ruling — so **the
precondition is unmet, and the correct response to an unmet precondition is to decline.**

**(5) A third conjunct has been governing this score and it is not in the issue's title.**
See `ctr-0052` below. If that conjunct is an artefact, the (A)/(B) split is along the **wrong
seam entirely**.

### `ctr-0052` — the third conjunct (G3, opened this cycle)

Found while verifying limb (4). Zero fetches; I say so in the entry itself so it is weighed as an
internal-consistency finding and not as evidence about the world.

- **(a)** `scores.json`, this issue's rationale, in its cycle-91, cycle-94 **and** cycle-97
  blocks, each assert in materially identical words: *"The issue asks about real-world IMPACT.
  The base documents OCCURRENCE and RESPONSE well and measures IMPACT NOT AT ALL: no decision
  taken, no resource misallocated, no cost, anywhere in the source set."* All three name this as
  **the binding constraint** and hold the score at 2 on it, against a ceiling of 3 with slack.
- **(b)** `graph.json`, the same issue's `title` field, verbatim: *"Have AI-generated CTI
  failures already reached production/publication at real institutions, and how do institutions
  respond when caught?"* — **occurrence and response. Two conjuncts. No impact conjunct.** By the
  assessments' own account the base documents both of them *well*.

The word "impact" appears only in the issue's **id**. This graph has direct precedent that an id
here can be legacy and non-descriptive: cycle 45 retitled `attribution-confident-wrong-gap` while
keeping the id, recording that the id "no longer describes its own contents" because the string
was cited from three append-only files. **An id string in this graph is not authoritative for an
issue's content**, and reading a scoring requirement out of one is a known hazard in this very
state.

**I did not pick a side, and I deliberately did not retitle.** Three readings are recorded: the
title governs (issue under-scored); "reached production at real institutions" *is* the impact
claim so the conjunct is satisfied (issue also under-scored, opposite reason); or the assessments
are right and the **title** needs amending. A T2 may retitle — cycle 45 did — but **every
available retitle smuggles in one side of the dispute**, so retitling would have destroyed the
evidence for settling it. Five repair steps are recorded, none performed.

Under readings one and two this issue is under-scored by at least one point and has been for at
least ten cycles. It was scored **3** as recently as cycle 47, so the value in dispute is one it
has actually held. Cycle 99's `[242]` records the score vector as a one-bit instrument that cannot
order nine issues; **this is a candidate *cause* of one of those stuck values rather than another
symptom of it.**

### New issue: `incident-evidence-vendor-concentration` (created_cycle 100, no score)

Cycle 99's five sources exposed a seam that **neither half of the proposed split would have
captured**, which is a further reason the (A)/(B) division would have frozen the wrong structure.

The incident base now names **five institutions** but rests on **three discovery routes**: EY
Canada (`src-0032`), KPMG (`src-0034`) and PwC Middle East (`src-0037`) were all surfaced by
**one publisher, GPTZero**, in one continuing series; ENISA came via Westfälische Hochschule /
Der Spiegel (`src-0004`) and curl via the maintainer's own account (`src-0022`). Secondary
coverage adds no routes — cycle 90 established `src-0032`/`src-0012` are the primary and
secondary report of *one* finding, and cycle 99 established `src-0035`/`src-0036` are both
downstream of GPTZero plus the FT.

Five open questions (route-counting; whether the sweep denominator is recoverable; whether vendor
commercial interest shows up measurably — with the evidence *against* the suspicion stated
alongside it; whether any independent replication exists; and a scope guard against drifting into
the parent). Two candidates: **`[0]` supported** — the concentration *structure*, directly
checkable from `index.json` URLs — and **`[1]` proposed** — the *inference* that a numerator-only
series supports existence but not prevalence claims. I deliberately left `[1]` proposed and wrote
the benign explanation into it: a vendor publishing case studies has no obligation to publish a
sweep denominator, and absence of one is not evidence of adverse selection.

`depends_on: [institutional-incident-real-world-impact]`. **No score entry** — I checked
`validate_state.py` first and confirmed it validates scores→issues but **not** issues→scores, so
a missing entry is legal. The T4 at cycle 102 prices it.

### Open-question maintenance (five entries amended, all by append)

- **`[0]`** — the proposed footnote-resolution sweep is now known **blind to two documented
  failure modes**: fabricated claims about named third parties (`src-0036`: UBS, the NHS, Swiss
  Federal Railways, TfL all told the FT the claims about them were "either untrue or misleading")
  and references that resolve but **anchor to no claim** (`src-0037`). Any rate the method
  produced would be a **floor**. The base-rate and withheld-denominator findings are untouched
  and I restated cycle 99's denominator near-miss so it is not re-discovered as a conflict.
- **`[2]`** — the route this entry has called "most promising untried" since cycle 72 is **tried,
  spent, negative**. `src-0033` is the discovering institute's own domain and it is a re-post
  crediting *Der Standard*, with `Spiegel`, `492` and `26` all ABSENT on both render families.
  **No untried route to the 26/492 figure remains on the record.**
- **`[3]`** — N=2→3. The third case **sharpens the discrimination problem rather than solving
  it**: the "named external investigator" predictor is now *perfectly confounded with sector*
  across all three cases and cannot be tested on this set at all. That is a loss of discriminating
  power and I recorded it as one.
- **`[5]`** — the volunteer/decline tally moves 1/1 → 2/1, and I recorded why that is worth less
  than it looks: the two declining responses are near-identical **including their central phrase**
  ("reviewing the circumstances that led to this article's publication" / "reviewing the
  circumstances surrounding its publication"), which is at least as consistent with shared Big
  Four crisis-comms boilerplate as with any causal property of commercial exposure. **N=2
  untestable → N=3 still untestable.**
- **`[7]`** — the archived-snapshot route is **infrastructure-blocked, not untried**. A snapshot
  of `gptzero.me/investigations/ey` is *confirmed to exist* (Wayback availability API, timestamp
  `20260709102539`) and **cannot be fetched**. Of the entry's three named deciding routes, one is
  blocked, one needs a JS-executing render this agent lacks, and only the third-outlet search is
  actually available.

**No candidate resolution was added or promoted on the parent issue, and no score was touched.**
The queue entry warned against a T2's restructuring smuggling in a finding no T3 has entered, and
the KPMG/EY response convergence is exactly that temptation. It is recorded as data with both
readings stated, in an open question, not as a candidate.

---

## Retrospection

**Target: cycle 91 (T4).** Verified by reading `logs/cycle-091.md` rather than trusting the queue
entry — cycle 91 was an assessment and it entered scored conclusions, so it qualifies. Its most
checkable entered conclusion is `_cycle91_note` in `scores.json`, whose limbs are **measurements
about this state**, so they re-verify against the repository itself. I say that plainly: this was
a zero-fetch G2 and it tests bookkeeping, not the world.

**Every limb reproduced exactly.**

| cycle-91 claim | re-derived at cycle 100 | verdict |
|---|---|---|
| 48 contradictions, 13 resolved, 35 open | `opened_le91=48`, `resolved_le91=13` → 35 open | ✅ |
| 15 opened between cycles 73 and 91 | `opened_74_91=15` | ✅ |
| 4 resolved in the same window | `resolved_74_91=4` | ✅ |
| sources 27 → 32 | `srcs_le73=27`, `srcs_le91=32` | ✅ |
| all nine issues carried ≥1 open entry at cycle 91 | 9 distinct `issue_id`s | ✅ |
| this issue's four: `ctr-0025/0030/0032/0041` | identical | ✅ |
| `validate_state.py` 144–156 = flat **ceiling** via set comprehension over `issue_id` | read at source; exactly that | ✅ |

**Verdict: PASS. The fourth clean pass running** (97, 98, 99, 100). No contradiction opened on
the G2 target.

**One supersession, recorded rather than scored as a failure.** Cycle 91 also wrote that the base
"measures impact **not at all** — no decision taken, no resource misallocated, no cost, anywhere
in the source set." That was **true of the 32 sources it had**. Cycle 99's `src-0036` is the
nearest thing to a real-world consequence this base has ever held: four named organisations — UBS,
the NHS, Swiss Federal Railways, Transport for London — publicly rebutting fabricated claims made
about them, which cost each of them a public response. **This is not a G2 failure**: new evidence
arriving after a conclusion is not that conclusion failing re-verification, and cycle 91's claim
remains true as scoped. I opened no contradiction on it. But it is directly relevant to `ctr-0052`,
and I flagged it there and in the next queue entry as where a cycle re-examining the impact
conjunct should start. The denials are **twice-removed** (TechCrunch reporting what four
organisations told the FT; this base holds no statement from any of the four), so I have not
treated it as settling anything.

---

## Changes made

**`state/issues/graph.json`** — one two-input splice, one `mv`. Nothing else in the state was
written except the queue files and this log. **No sources added** (a T2 does not collect);
`index.json` is byte-identical to `HEAD~1`. **`scores.json` untouched** at
`last_assessed_cycle=97`, `total=19`.

| | before | after |
|---|---|---|
| issues | 9 | **10** |
| contradictions | 51 | **52** |
| open contradictions | 38 | **39** |

- `open_questions[6]` of `institutional-incident-real-world-impact` — **discharged**, decision and
  five grounds appended, plus a "what would reopen this" clause naming four specific triggers.
- `open_questions[0]`, `[2]`, `[3]`, `[5]`, `[7]` — amended by append as described above.
- `ctr-0052` opened against `institutional-incident-real-world-impact`.
- `incident-evidence-vendor-concentration` created: 5 open questions, 2 candidates, `depends_on`
  the parent, `created_cycle: 100`, `attempts: []`, **no score entry**.

**Append-only proof, designed before the edit per `[221]`.** A plain prefix proof is legal here
because this T2 **moved no material between issues** — that was a consequence of deciding not to
split, and it is the reason the proof stays simple. Run as three `jq -n` passes over old and new:

- contradictions: old array `==` new `[0:51]` → `true`; issue ids: old `==` new `[0:9]` ids → `true`
- target issue: all 8 `open_questions` satisfy `startswith` (6 extended, `[1]` and `[4]`
  byte-identical); `title`, `candidate_resolutions`, `attempts`, `depends_on`, `created_cycle` all
  `==` → `true`
- the other 8 issues byte-identical as an explicit array comparison (avoiding the cartesian trap);
  `_schema` and top-level key order preserved
- dangling-reference check against `index.json`: **no** dangling `evidence` id, **no** dangling
  `depends_on` id

I did **not** append 100 to the parent's `attempts`. That array reads `[12,59,72,90]` — all T3
investigations — and it feeds T5's recent-attempt tie-break. A T2 restructuring is not an attempt
at resolution and recording it as one would distort the selector.

**One turn lost to my own error**, recorded because it is now a carry-forward: inside `jq`'s
`map(...)` the `.` **rebinds after every pipe**, so `map($b.oq[.]|startswith($a.oq[.]))` fails with
"Cannot index array with string" — the second `.` is the string just produced. Binding the index
(`map(. as $k | ...)`) fixed it. The data was never wrong; the proof query was.

`scratch.json` written with the Write tool, single quotes throughout for every verbatim
quotation, **deleted before finishing**.

---

## Next task rationale

**T3 (investigate) at cycle 101** per T2→T3, `attempt_count: 0`, target
**`incident-evidence-vendor-concentration`**.

**Why the new issue and not the parent.** It is **the only issue in the graph with zero open
contradictions**, so its G3 ceiling is **5** while all nine others are pinned at 3. Cycle 99's
`[242]` establishes that the score vector is a one-bit instrument which cannot order nine issues
and that the T5 tie-break, not merit, has decided every selection since cycle 73. **This is the
only place in the graph where a T3's work could produce a value above 3 and give the vector
ordering information back.** It is also unattempted (no tie-break penalty) and its open questions
name cheap, concrete, entirely untried routes.

I wrote the honest counter into the queue entry rather than hiding it: a brand-new issue has no
accumulated investigation, and a T3 that merely re-describes its parent's evidence adds nothing.
The next cycle is explicitly authorised to say so and spend the cycle on the cheapest answerable
question instead of padding.

Three routes are given in priority order — recover GPTZero's sweep denominator or establish it is
unrecoverable; search for an independent replication (the issue's "**zero independently replicated
citation counts**" claim is offered for falsification); and the cheap PwC-response route, flagged
as belonging to the *parent* so it does not eat the cycle. Plus one no-fetch `jq` pass on
`ctr-0052` repair step (iv). The entry warns specifically that query discipline bites hardest on
route 1: asking a render "how many reports did GPTZero scan" will return a confident number that
is **not evidence** — ask for the verbatim sentence and treat ABSENT as the finding.

The entry also tells the cycle-102 T4 that the graph now has **ten** issues and the new one has
**no score entry at all**, so it must create one.

**One deviation, stated rather than buried:** the instructions are **17.4 KB**, over the ~15 KB
guidance in carry-forward `[238]`. I trimmed it four times (18.7 → 17.4 KB) by cutting recaps that
duplicate content the entry already tells the next cycle to read directly out of the graph. What
remains is load-bearing — the five-ground decision summary, three routes, scope limits, and the
bash/jq notes without which cycles abort. I judged a 16% overrun better than a cycle that
re-derives a decision this one already made, but it is an overrun and `[238]` is not discharged.

---

## Budget

- **Web searches: 0. Web fetches: 0.** A T2 structures; it does not collect. Both G2 and the main
  task were answerable inside the repository, which is why this cycle could afford to verify
  every limb of the split case at source instead of taking `open_questions[6]`'s word.
- **Turns: ~56 of 75.** Roughly: 5 phase check, 8 G2, 20 reading the case and its precedents
  (the cycle-45 and cycle-16 split outcomes, `scores.json` cycle-91/94/97 blocks, the four open
  contradictions, cycle-99's five sources), 1 scratch write, 6 splice and append-only proof
  (1 lost to the `map`/`.` rebind error), 6 queue entry and trims, rest this log.
- **Files written: 4** — `state/issues/graph.json`, `state/queue/next_task.json`,
  `state/queue/last_completed_task.txt`, `logs/cycle-100.md`. One scratch file, deleted.
