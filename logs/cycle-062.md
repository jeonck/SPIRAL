# Cycle 62 — T3 Investigate — `consistency-calibration-as-failure-mode`

Phase verified before acting, per the standing lesson from cycles 31–32 and the four aborted
cycles (38, 40, 44, 57). `state/queue/last_completed_task.txt` read `T5 select`; the last commit
was `a41928f cycle 61: T5 select`; `ctr-0026` was present as the 26th contradiction with 17 open;
`last_assessed_cycle` was 60; `index.json` held 24 sources. **All five checks passed, so cycle 61
landed and I am the T3.** The inherited queue entry's account of the phase was accurate.

**Refresh rule, re-derived at source and not inherited.** `config.yml` line 22 sets
`collect_refresh_every: 7`. 62 % 7 = 6, so it would not fire even if it were mine — and it is
not: `prompts/t5_select.md` step 4 is the only place it is written, so it fires only inside a T5.
The inherited claim that "the next cycle at which the rule fires is 63" is **true of the
modulus and false of the loop**: 63 = 9 × 7, but cycle 63 is a **T4**, and the rule is not
evaluated in a T4. T5s fall on 64, 67, 70, 73…; 64 % 7 = 1, 67 % 7 = 4, **70 = 10 × 7**. So the
next *actual* firing is **cycle 70**, not 63. See new carry-forward **[185]** — this is a real
structural finding, not a bookkeeping quibble: because the test is evaluated only every third
cycle, the modulus can and does skip firings entirely.

---

## Task performed

`T3` per `prompts/t3_investigate.md`, which I read at source rather than relying on the queue
entry's account of it. The prompt's six steps map onto this cycle as: (1)(2) worked the target
issue's open questions against the existing base first; (3) added one `candidate_resolution`,
promoted to `supported`; (4) rewrote `open_questions`; (5) appended 62 to `attempts`; (6) opened
`ctr-0027`. **No new source was collected** — `index.json` is unchanged at 24 sources — so no
`max_new_sources` budget was consumed.

I took the lead `ctr-0026` named as "the substantive prize": **step (iii), pull the Token
Probability block of `src-0013`'s Table I.** It paid, and step **(iv)** then paid considerably
more than `ctr-0026` anticipated.

### Finding 1 — the elicitation-method effect (three-URL-form confirmed)

`src-0013`'s Table I carries **four** confidence-elicitation blocks — **Verbalized, Token
Probability, Sampling-based Consistency, Self-Consistency** — each with columns
`TT | ECE | Brier | OC Gap | Sec% | FT%`, three models × six temperatures. The base had stored
**only the Verbalized block, for forty-seven cycles.**

Token Probability, **ECE column, confirmed cell-for-cell on three URL forms**
(`arxiv.org/html/2606.31159v1`, `ar5iv.labs.arxiv.org/html/2606.31159`, unversioned
`arxiv.org/html/2606.31159`), each asked in a *different* way, all three agreeing exactly:
...
**The headline:** measured calibration error is not a property of the model. On the *same*
benchmark, *same* model, *same* temperature, Gemini-2.0-Flash goes **0.25 verbalized → 0.81
token-probability (Δ 0.56)** and GPT-4o-mini **0.46 → 0.80 (Δ 0.34)**. The model *ordering* also
stops meaning anything: Gemini is the best-calibrated of the three under Verbalized and
indistinguishable from GPT-4o-mini under Token Probability. **That swing is larger than any
ECE-versus-Brier gap anywhere in this base, and larger than most between-model gaps this base
treats as findings.**

The paper's own reading, PRESENT on two forms, verbatim: *"Token-probability estimation performs
poorly (ECE≈0.80), reflecting the mismatch between next-token likelihood and program-level
security outcomes."* And Qwen's blank row is a **tooling limit, not a null result**: *"…the
Hugging Face inference provider does not expose raw logits or logit access, so the confidence
estimation for the Qwen3-Coder-Next model uses indirect measures…"*

**Deliberately weaker, and labelled so.** The Brier column of the same block agreed on two
fetches — but `src-0013` has exactly **one** arXiv version, so `/html/<id>` and `/html/<id>v1`
serve *the same document*. Those are independent **fetches**, not independent **renders**, and
`ar5iv` declined to transcribe Brier at all. Recorded as provisional. New rule caveat **[184]**.

**Why I trust the fetch at all**, which matters given five fetch-compliance failures in three
cycles: the same fetch returned the **Verbalized** block whole, and it reproduces this base's
stored figures *exactly* — GPT-4o-mini ECE `0.46/0.46/0.46/0.46/0.47/0.48` against stored
`[0.456,0.481]`, Gemini `0.25/0.25/0.26/0.25/0.26/0.26` against `[0.247,0.263]`, Qwen
`0.41/0.41/0.41/0.41/0.42/0.42` against `[0.408,0.421]`, and all three Brier ranges likewise.
The table prints two decimals; the stored three-decimal ranges come from body prose. Consistent,
not conflicting. **No stored number in this base is impeached by this cycle.**

### Finding 2 — `ctr-0027`: the two calibration sources are in OPPOSITE modalities

`ctr-0026` step (iv) asked whether the impeached bullet's comparability claim with `src-0001`
survives. **It does not, on a limb `ctr-0026` did not examine.** The bullet reads *"…which is the
same modality as src-0001's, so the two are methodologically comparable…"*.

**`src-0001` does not elicit verbalized confidence. It derives confidence from token log
probabilities.** Verbatim, `arxiv.org/html/2503.23175v2` §5 *Evaluation Metrics*:
...
So the base has held two studies "methodologically comparable" **across exactly the degree of
freedom that one of them measures as the single largest source of variation in the quantity being
compared.** That is rule **(vi)** — both sources clean, the state's account of them not — and it
is the **fifteenth** instance. It is also the *second* defect found in a *single bullet* in two
consecutive cycles: `ctr-0026` caught the last clause and stopped; the false premise sat one
clause earlier in the same sentence. **Rule (iv)'s whole-sentence discipline was applied to the
source and not to the state's own prose.** That is a genuinely new lesson.

**Evidential status, stated rather than banked.** This stands on a **PRESENT** verdict from one
URL form, which rule (v) licenses, and it survives the rule (xvii) post-check because the passage
contains the very strings the fetch was separately asked for (`log probability` PRESENT,
`token probability` PRESENT). It does **not** stand on the accompanying ABSENTs (`verbalized`,
`self-reported`), which are single-form and therefore corroborating only. It does not need them:
a paper that computes confidence by multiplying token probabilities is not eliciting verbalized
confidence whether or not it uses the word. `ctr-0027` step (iii) names the one fetch that would
close that gap.

**This is a gain, not a loss, and I want that on the record.** `src-0013`'s **Token Probability**
block — not its Verbalized block — is the modality-matched counterpart to `src-0001`. The base
now holds its **first modality-valid cross-study calibration comparison on a security task**,
where it previously held a mismatched one nobody had noticed. Matched: `src-0001` (gpt4o, CTI
extraction/generation) ECE 0.13–0.47 zero-shot rising to 0.91 fine-tuned, against `src-0013`
(GPT-4o-mini, Gemini-2.0-Flash, security code generation) ECE 0.79–0.81 flat. **Direction
corroborates; magnitudes diverge sharply — and that divergence must NOT be read as a conflict
between the studies, because the tasks differ.** I say so explicitly in both the candidate and
the contradiction so no successor overreads it.

### What I did NOT establish — stated plainly

- **No CTI claim gained a second source.** `src-0013` is secure code generation on SALLM, not
  CTI. The cycle-16 scope ruling in `candidate_resolutions[3]` is **untouched and still holds**.
  `open_questions[0]`, `[1]` and `[5]` are unchanged in substance.
- **`open_questions[5]` went unsearched for a twenty-third cycle.** I spent the whole budget on
  `ctr-0026`/`ctr-0027` and ran no new search for a second CTI-task ECE/Brier source. I judged
  the modality finding worth more than another speculative search — but the drought is real and I
  have recorded it rather than glossed it. What I *did* do is raise the bar that search must
  clear: any candidate must have its **elicitation method recorded before being proposed**.
- **No per-cell FT% value was entered, from any block.** My `/html/v1` read puts Verbalized
  GPT-4o-mini FT% 33.50 at T=0.00 and **33.90 at T=0.40**; cycle 61's `ar5iv` read put 33.9 at
  **T=0.20**. `ctr-0026`'s rule-(ix) prohibition **stands, reinforced by a third data point.**
- **The Sampling-based Consistency and Self-Consistency blocks are one-form only** and no cell
  from them is in the state. Their shape is recorded as a lead: Sampling-based Consistency
  returned ECE `0.00` everywhere; Self-Consistency showed strong temperature dependence
  (GPT-4o-mini 0.55 → 0.07). **If confirmed, they make the effect larger still.**
- **`ctr-0026` is not resolved.** All three of its outstanding steps are discharged, but I left it
  open so that step (iv)'s spillover (`ctr-0027`, and the issue's new `open_questions[8]`) is
  recorded against it first. A cycle with standing should resolve it.

### On the selection — my honest verdict, as the queue entry invited

The queue entry was right that this issue "won on two of three unratified conventions" and asked
me to say if my work argued for a different target. **It does not. The out-degree patch picked
correctly here** — this issue had a named, costed, high-value lead sitting in a contradiction
opened one cycle earlier, and it paid out twice. But that is **luck, not vindication of the
patch**: the lead came from `ctr-0026`, which any selector could have seen, and *"has a fresh
contradiction with a named substantive step"* would have picked the same issue on a criterion
that is actually about expected yield. Carry-forward **[75]** stands entirely undiminished.

---

## Retrospection (G2)

**Target chosen:** `consistency-calibration-as-failure-mode` `candidate_resolutions[5]`,
`supported`, evidence `["src-0001"]` — specifically its **consequence (iii)**, the claim that
relative to zero-shot, fine-tuning *"WORSENED ECE on 7 of the 9 rows and WORSENED BS on the same
7 … improving only extraction CVE and generation goals"*. Chosen because it is (a) a **supported**
conclusion, (b) **AGENT-DERIVED arithmetic**, which rule (xii) says must be audited against the
log that derived it and which no string check can reach, and (c) the entry's own self-description
calls it *"the robust finding"* — the load-bearing survivor of `ctr-0003`. If it were wrong, a lot
would move.

**Method:** rule (xii) audit (re-derive from the stored cells myself) **plus** a source re-check
on a **fourth** URL form never used on this source — `arxiv.org/html/2503.23175v4` — under a
verification-mode prompt (rule xv) with a visibility probe and a truncation probe.

**RESULT: PASSES, CLEANLY, ON BOTH LEGS.**

**Leg 1 — all 54 cells of Table 6 re-verified EXACT on v4.** This is now the *third* independent
confirmation (cycle 25 on the then-current render, cycle 39 on v1, cycle 62 on v4). Caption
re-confirmed verbatim: *"ECE and BS with gpt4o for information generation and extraction."*

**Leg 2 — the arithmetic reproduces exactly.** ECE zero-shot → fine-tuning: campaign 0.25→0.48 ✗,
APT 0.16→0.25 ✗, ext-CVE 0.28→**0.18 ✓improved**, ext-attack-vector 0.13→0.27 ✗, goals
0.13→**0.08 ✓improved**, labels 0.45→0.48 ✗, country 0.19→0.35 ✗, gen-CVE 0.15→0.91 ✗,
gen-attack-vector 0.47→0.87 ✗ = **7 worse, 2 better**. BS: 0.26→0.48 ✗, 0.15→0.23 ✗,
0.32→**0.21 ✓**, 0.46→0.58 ✗, 0.14→**0.05 ✓**, 0.44→0.49 ✗, 0.22→0.29 ✗, 0.29→0.98 ✗,
1.00 from 0.43 ✗ = **7 worse, 2 better, the SAME 7.** Exactly as claimed, including *which* two
rows improve.

**I also re-derived all twelve family means** carried by that entry since cycle 39. Every one
reproduces: ECE extraction 0.205 / 0.2425 / 0.295, generation 0.278 / 0.320 / 0.538; BS
extraction 0.2975 / 0.3225 / 0.375, generation 0.304 / 0.294 / 0.562. **Twelve for twelve.**

**Confirmed at source:** the paper *never counts rows* — asked directly whether it states in words
how many rows got better or worse after fine-tuning, the fetch returned **ABSENT**. So the 7-of-9
count is correctly labelled AGENT-DERIVED wherever it appears.

**One ABSENT that is a FALSE NEGATIVE, and it is the third on this one source.** The v4 fetch
returned the metric-definition sentence *"The ECE and BS, are two measures of calibration that
quantify the deviation from perfect calibration."* as **ABSENT**. The v2 fetch returned it
**PRESENT and verbatim**. **No contradiction opened** — this is precisely the failure rule (v)
exists to catch, and it is the third recorded false-negative ABSENT on `src-0001` (cycle 25's
*"inconsistent and overconfident"*, cycle 39's *"dataset of 350 threat intelligence reports"*, and
now this). Three on one source is itself the finding: **for `src-0001` specifically, treat a
single ABSENT as no evidence at all.** Recorded in `ctr-0027`.

**Recovered in the same pass and never previously held by this base** — `src-0001`'s ECE
definition paragraph, attributing the formalism to Jiang et al. and defining the model's
confidence as *"the calculated probability of its prediction"*. That **independently corroborates
the token-probability reading from a second passage in the same paper**, which is why `ctr-0027`
does not rest on a single sentence. A G2 and a main task landing on the same artefact bought two
results for one fetch — cycle 49's unnumbered observation, applied deliberately.

**Verdict: the audited conclusion survives unchanged.** No contradiction arises from the
retrospection itself. `ctr-0027` arose from the *main task*, not from this G2.
...
1. `attempts` on the target issue → `[3, 15, 16, 25, 39, 62]`.
2. **New `candidate_resolutions[6]`**, `status: supported`, evidence `["src-0013","src-0001"]` —
   eleven numbered paragraphs recording the elicitation-method finding, its three-form
   confirmation, the weaker Brier evidence, the step-(iv) adjudication, the modality-matched
   repair, the explicit scope limit, what it does to `[2]`, and what was deliberately **not**
   entered. `candidate_resolutions[0]`–`[5]` untouched (append-only).
3. **`open_questions` rewritten from 8 to 9.** `[5]` amended (still open, still unsearched, but
   the bar it must clear is now higher — record elicitation method before proposing a source).
   `[6]` **substantially narrowed** — its premise that this base "has no source that discusses
   ECE-versus-Brier behaviour at all" is **no longer true**, and in `src-0013` the two metrics
   **do not diverge**, which localises `src-0001`'s divergence to a fragile agent-derived family
   mean rather than a general property. `[7]` amended to carry the withdrawal of the dead Springer
   route in the question itself rather than only in a queue entry that gets overwritten. **New
   `[8]`** — the elicitation modality of every other confidence-bearing source in the base
   (`src-0018`, `src-0014`, `src-0015`, `src-0016`, `src-0002`) is **unrecorded**, which under
   rule (xxi) directly weakens `candidate_resolutions[2]`'s four-source aggregate.
4. **`ctr-0027` opened** (27 contradictions, 18 open).

**`state/knowledge/src-0013.md`** — new `## Cycle 62 — T3` section. Nothing deleted or rewritten;
**the impeached bullet is left standing** so the correction stays auditable.

**`state/knowledge/index.json`** — a fifth `key_claim` on `src-0013` (`ctr-0026` step (ii) plus
(iii) and (iv)). **Still 24 sources; no source added.**

**`state/assessments/scores.json` — NOT TOUCHED.** A T3 has no standing to rescore.
`last_assessed_cycle` remains 60, all nine issues remain at 2.

**G3 ceiling.** This issue now carries **three** open contradictions (`ctr-0011`, `ctr-0026`,
`ctr-0027`). `scripts/validate_state.py` lines 144–156 build the open set as a **set
comprehension** over `issue_id` and enforce a **ceiling** of `scale_max − g3_contradiction_demotion
= 3`, not a per-point subtraction. **The ceiling was already 3 with one and is unchanged with
three.** `ctr-0027` adds **no** gate pressure and **is not a demotion**. Carry-forward **[4]**,
the conflict between that code and `prompts/t4_assess.md` step 3, is now in its **fifty-first**
cycle awaiting a human.

**Every JSON edit validated with `jq -e .` and the added fields read back.**

---

## Next task rationale

**T4** per the state machine (T3→T4), `target_issue: null`, re-scoring **all nine** issues, with
`last_assessed_cycle` to be set to 63. Cycle 63 is the T4; the refresh rule does not fire in a T4
and would not fire at 64 either (see **[185]**).

The T4 is handed three things it needs and cannot safely infer: (a) **the direction of
`ctr-0027` cuts both ways** and must not be read as a reason to lower this issue — the corrected
picture is *better understood and no better supported*, which on this base's rubric is still a 2,
though the T4 has standing to overrule that and I do not; (b) **the G3 ceiling is a set
membership test**, so three open contradictions exert exactly the pressure one did; (c) the
**`scores.json` layered-rationale hazard** ([172]), now known on three issues, is squarely the
T4's to fix and it is the cycle with standing to do it.

---

## Budget

- **Fetches: 4.** `arxiv.org/html/2606.31159v1`, `ar5iv.labs.arxiv.org/html/2606.31159`,
  `arxiv.org/html/2606.31159` (all `src-0013` Table I, asked three different ways);
  `arxiv.org/html/2503.23175v4` (G2) and `arxiv.org/html/2503.23175v2` (the false-negative
  re-check and the modality question) — **5 fetches total**, two of them to `src-0001`.
- **Searches: 0.** Everything came from sources already in the base, which is what
  `prompts/t3_investigate.md` step 2 asks for.
- **New sources: 0**, so no `max_new_sources` budget consumed.
- **Turns: roughly 33 of 75.** Well inside budget; state edits and both queue files were written
  **before** this log, as cycles 49–56 and 58–61 all recommend.
- **Yield:** 2 findings, 1 contradiction opened, 3 contradiction steps discharged, 1 clean G2 with
  a 54-cell and 12-mean arithmetic audit, 1 false-negative ABSENT caught, 1 new open question,
  1 numbering error caught in my own handoff before it propagated.
