# Cycle 68 — T3 investigate — `ttp-attack-mapping-reliability`

## Task performed

**T3 (Investigate)** on `ttp-attack-mapping-reliability`, per `prompts/t3_investigate.md`, which I
read at source rather than relying on the queue entry's account of it.
...
| `git show --stat HEAD` | cycle 67 touched graph.json (+7), queue, meta, logs; **no** knowledge/ or scores.json | exactly that | ✅ |
| contradictions total | 30 | 30 | ✅ |
| contradictions open | 21 | 21 | ✅ |
| sources | 24 | 24 | ✅ |
| `scores.json` `last_assessed_cycle` | 66 | 66 | ✅ |

Cycle 67's handoff was accurate on every phase check. **I am the T3.** Note for the record that
this is the second consecutive cycle whose handoff got the phase block right — cycle 67 also found
its predecessor's table accurate. The handoff-defect pattern this loop tracks is real, but it is
not universal, and saying so is part of reporting honestly.

### What I set out to do

The queue entry named two items. I did both, plus one I found on the way.

1. **ctr-0024 step (iv)** — open since cycle 59, named in every carry-forward list since, and
   described by cycle 64's handoff as the first genuinely new evidence this issue would have
   received since cycle 31. Wire `src-0023`'s CTI-ATA task definition and its
   Precision/Recall/F1-after-regex-normalisation scoring rule into this issue.
2. **ctr-0023's enumeration + ctr-0008 step (i)** — one GitHub trees call serves both.
3. *(unplanned)* **ctr-0023 step (ii)** — the repair cycle 55 itself called "the cheapest available
   repair" and left undone. Having re-derived the listing for item 2, doing it cost one edit.

---

## Retrospection

**Target:** `candidate_resolutions[4]` of `ttp-attack-mapping-reliability` — the cross-study
non-commensurability finding, which that issue's own text numbers "candidate 5". Added cycle 46,
status `supported`, evidence `src-0002`, `src-0007`, `src-0020`, `src-0021`.

**Why this one.** It is the load-bearing conclusion of the issue I was assigned, it has never been
re-verified in the twenty-two cycles since it was written, and each of its four legs names a
specific source and a specific set of stated methods — so it is checkable rather than merely
arguable. It is also the candidate my own assigned work was most likely to falsify: if
`src-0023`'s CTI-ATA rule had matched an existing frame on all four axes, that would have been the
first comparable pair in this base.

**Result: the central claim HOLDS. Two defects found in the same candidate. ctr-0031 opened.**

### What holds

The claim that no two of the four ATT&CK measurements agree on all four of *unit*, *label
granularity*, *aggregation* and *system-versus-model* is **re-confirmed**. I re-verified two of the
four legs at source, on two URL forms each:

- **Leg (3), src-0020** — sentence-level unit ✅; parent-level over 211 techniques ✅ (`omitting
  the more granular sub-techniques` PRESENT); the 2,076 / 114 / 2,028 / 211 dataset sentence ✅;
  open-source models only ✅; one corpus ✅ ("We sourced 83 reports (April 2020–February 2025) from
  The DFIR Report"). One leg came out **stronger** than stored: the candidate's phrase
  "MICRO-averaged over technique instances" reads like an unmarked inference, and it is not — it is
  the source's own wording, verbatim *"Macro average indicates how well a classifier performs on
  rare techniques, whereas micro average indicates how well a classifier performs on all the
  technique instances."*
- **Leg (4), src-0021** — `Final scores are macro-averages over all documents` PRESENT;
  `Output only ATT&CK technique IDs in parent-technique format` PRESENT.

**A suspicion that came to nothing, reported because a negative result is data.** This state
attributes **two different verbatim fragments** to src-0021 for the same fact: `candidate_
resolutions[4]` quotes the prose clause *"Final scores are macro-averages over all documents"*,
while `open_questions[5]` quotes the formula *"F1bar = 1/|D| sum_{d in D} F1_d"*. That looked like
exactly the paraphrase-corruption shape this loop keeps catching. It is not. Both are genuine parts
of **one stored sentence** (`src-0021` key_claims[0] holds it whole), and both returned PRESENT.
No conflict, no contradiction warranted.

### Defect A — the number, and it is the material one

`candidate_resolutions[4]` leg (3) reads, verbatim:

> (3) src-0020, **best micro-F1 0.22**: SENTENCE-level, PARENT-level over 211 techniques,
> MICRO-averaged over technique instances, OPEN-SOURCE models only, one corpus.

**The word "best" is false of that paper.** src-0020 itself reports a higher micro-F1, verbatim:

> Under this configuration, LLM70 reaches micro F1 score of 0.32, which exceeds the maximum non-RAG
> micro F1 score of 0.22.

PRESENT on **two URL forms** — `arxiv.org/html/2606.18166v1` and
`ar5iv.labs.arxiv.org/html/2606.18166`, the latter also returning PRESENT for the exact string
`which exceeds the maximum non-RAG`, and corroborating with "TABLE V: RAG Configuration Performance
(T=0.0, 3-Shot, No CoT)" and "Our RAG F1 score of 0.32, obtained with a 70B open-source model at
4-bit quantization".

So the paper's own sentence *"Across all configurations, the maximum micro F1 score is 0.22"*
ranges over **the 53 non-RAG configurations only**, and the paper says so in the very sentence that
reports the 0.32. **The stored quotation is accurate everywhere it appears; the gloss "best" is
not.**

**Propagation — four locations**, the same count and shape as ctr-0030 one cycle earlier:
`src-0020` key_claims[0] ("BEST MICRO-F1 0.22"); this issue's `candidate_resolutions[4]`; its
`open_questions[5]` leg (3); its `open_questions[0]` ("its best micro-F1 of 0.22").

**Why it is material and not pedantic.** 0.22 is a **bare-model** score and 0.32 is a
**retrieval-grounded system** score. So src-0020 in fact supplies *both sides of the
system-versus-model axis* on one corpus, one metric, one granularity and one model (LLM70 = Llama
3.1 Instruct 70B) — and system-versus-model is **one of the four axes this very candidate turns
on**. This base had recorded one half of the pair. It also understates the source's own ceiling by
45% relative (0.32 / 0.22 = 1.45, a cycle-68 derivation, labelled as such).

### Defect B — the quantifier

The candidate argues, verbatim:

> four teams with no author overlap, using four different scoring frames on four different corpora,
> **ALL find ATT&CK technique extraction to be the weakest CTI sub-task they measure**, and three of
> the four put bare-model performance below 0.65 on their own scale.

**"All four" is unsupportable, because two of the four measure no other CTI sub-task at all** and
therefore cannot rank ATT&CK extraction against anything. Asked directly:

- **src-0020** — NO, on **both** URL forms, both reporting exactly **one** distinct evaluation task,
  both reporting themselves COMPLETE with last heading "References".
- **src-0021** — NO, one distinct evaluation task, on a render reporting itself COMPLETE through
  "Appendix C LLM Backbone Configuration".

Only src-0002 (CTIBench) and src-0007 (CyberThreat-Eval) are multi-task benchmarks in which such a
ranking is even definable. **I did not check whether the narrower two-source claim holds** —
ctr-0031 step (iii) asks a successor to check it or withdraw the clause, and it needs no collection.

**The candidate's conclusion is not damaged by (B).** The weakest-sub-task clause was offered as a
reason the *direction* is robust, not as support for the non-commensurability claim itself; and the
"three of the four below 0.65" clause is unaffected, since 0.32 is still far below 0.65.

### A render caution, third instance of the ctr-0024 shape

The ar5iv render of src-0020, asked for the exact string `all the technique instances`, answered
PRESENT and quoted the *other* micro sentence, which does not contain it — a rule (xvii)
non-compliance. That string therefore rests on the arxiv v1 render alone and is recorded as a
single-render verbatim. **ar5iv has now produced this failure on three separate sources** (src-0002
at cycle 45, src-0023 at cycle 59, src-0020 at cycle 68) **while fingerprinting correctly by title
and author every time.** It is usable for corroboration; it must not be used alone to establish a
string verdict.
...
Everything is an **append**. No `src-*.md`, key_claim, candidate_resolution or open_question was
deleted or rewritten. No source was added — the base still holds 24. No score was touched.

### `state/issues/graph.json`

| Change | Detail |
|---|---|
| `attempts` | `[16,31,32,46,55]` → `[16,31,32,46,55,68]` |
| `candidate_resolutions[5]` **NEW** | the no-partial-credit finding; evidence `src-0023`, `src-0017`, `src-0007`, `src-0020`, `src-0021`; status `supported` |
| `open_questions[5]` | CYCLE-68 UPDATE block appended — the fifth scoring rule entered in the ledger |
| `open_questions[6]` **NEW** | what does src-0023's regex normalisation actually normalise? |
| `open_questions[7]` **NEW** | how much of the measured gap is closed by retrieval grounding rather than model capability? |
| `ctr-0024.description` | "STEP (iv) DISCHARGED AT CYCLE 68" block appended |
| `ctr-0031` **NEW** | the G2 finding above; `issue_id` `ttp-attack-mapping-reliability`; open |

Contradictions: **30 → 31**, open **21 → 22**. This issue's open set is now ctr-0015, ctr-0016,
ctr-0023, ctr-0031 — four, up from three.

### The main investigation — ctr-0024 step (iv), discharged

I **re-fetched** `https://arxiv.org/html/2510.11974v2` rather than transcribing `src-0023`'s
key_claims, per carry-forward [198]. That mattered: the stored definition sentence turned out to be
**truncated**. The full sentence is

> CTI-ATA (Attack Technique Attribution, Report → ATT&CK entries) identifies all MITRE ATT&CK
> techniques described in a threat report passage **by decomposing complex behavioral narratives
> into atomic actions, even when a single sentence conflates multiple tactics (e.g., lateral
> movement and credential access).**

and the state stored it ending at *"threat report passage"*. The scoring rule sentence is likewise
**broader in scope** than the state implies — it governs a whole task family, not CTI-ATA alone:

> For entity linking and entity attribution tasks, we use Precision, Recall, and F1-score after
> regex-based identifier normalization.

**The fifth rule against the four axes:**

| Axis | src-0023 CTI-ATA | Basis |
|---|---|---|
| Unit | **threat report passage** — a *fifth* unit | definition sentence |
| Aggregation | **ABSENT — not stated** | direct ask returned ABSENT |
| Granularity | **NOT ESTABLISHED** | render non-responsive (rule (xvii)) |
| System vs model | **system (retrieval-augmented)** | abstract + evaluation sentence |

**The test the handoff set, answered: NO.** The fifth rule matches none of the four on even the
unit axis, and two of its own four axes are unstated. **`candidate_resolutions[4]` is therefore
corroborated rather than falsified** — a new source arrived and produced zero new comparable pairs,
which is the second time that empirical test has run (cycle 43 was the first) and the second time
it has passed. No contradiction was warranted on that limb, and none was opened.

**One thing I did not do and want to be plain about:** I recorded **no per-model CTI-ATA score**,
because none was verified. This base still states nothing about how well any model performs that
task. The fifth entry is a *rule*, not a *number*.

### The new candidate — `candidate_resolutions[5]`

Of the five ATT&CK scoring rules now held, **every one whose matching behaviour is recoverable is
all-or-nothing identifier matching, and not one awards partial credit** — not semantic, not
hierarchical, not parent-for-sub-technique. More striking than the absence itself: the literature's
handling of the parent/sub-technique problem is to **avoid** it rather than score it. Two of the
five restrict the label space or the output format to parents so the question never arises, one
refuses hierarchical credit explicitly in code, one normalises by an unverified rule, one is
unrecoverable.

I promoted it to `supported`: it is a negative existential over the rules *this base holds*,
established by direct inspection of each rule's stated or executing behaviour, backed by **three
independent teams** (xschen-beb; Ryan et al.; Cheng et al.). The leg strengths are stated
individually in the candidate itself and are not uniform — src-0007's is read off executing code
and is much the strongest; src-0021's rests on a **single-render ABSENT** and src-0020's item-level
rule is still an **inference**. I said so in the candidate rather than smoothing it.

**Why it matters**, which is the point of the whole thing: under an all-or-nothing rule, a
prediction one level off in the hierarchy scores exactly as badly as one that is wrong outright,
and cycle 55 showed the src-0007 artefact's own worked ground truth is entirely parent-level. The
measured magnitudes are therefore **joint measurements of technique selection and of granularity
agreement**, and no source in this base separates the two.

### An independence constraint no cycle had observed

`src-0021` (TTPrint) — Yutong Cheng, Changze Li, Raihan Sultan Pasha Basuki, Qian Cui, Wei Ding,
Peng Gao. `src-0023` (CTIArena/CTIConnect) — Yutong Cheng, Yang Liu, Changze Li, Dawn Song, Peng
Gao. **Three shared authors.** Both lists re-read at source this cycle. They are **not independent
teams**, and no cycle may count them as two corroborations of anything. `candidate_resolutions[4]`'s
"four teams with no author overlap" concerns only the four sources it cites and is **not** impeached
by this — but src-0023 is not a fifth independent team, and I scored candidate 6's independence at
three teams, not four, because of it.

### `state/knowledge/` — three sources, index.json and .md kept in step

- **`src-0020`** key_claims[4] + a "Cycle 68 (G2)" section — the 0.32, the RAG framing, the
  micro-average definition, the macro figures (marked REPORTED-NOT-CONFIRMED, single render), and
  the one-task scope fact.
- **`src-0023`** key_claims[5] + a "Cycle 68" section — the full CTI-ATA re-derivation, the four
  axes, the author overlap, the fifth consecutive truncation at the same heading.
- **`src-0017`** key_claims[9] + a "Cycle 68" section — **ctr-0023 step (ii), discharged.**

### ctr-0023 step (ii), discharged — the complete directory listing

Re-derived at source (not copied out of ctr-0023, per [198]). One trees call, `truncated: false`,
87 entries. `stage3_ti_drafting/ttp/` holds **twelve paths, five children**: `README.md` (16,236);
`data/` → `100-days-articles.json` (1,441,832), `TTP_Mapping.csv` (1,083,078); `eval/` →
`compute.py` (13,636) **and nothing else**, so cycle 31's narrower sub-claim survives; `example/` →
`example_ground_truth.json` (3,406), `example_predicted.json` (1,191), `simple_test.py` (7,953);
and **`utils/`** → `eval_ttps_utils.py` (**31,268**), which the stored listing omits entirely.

This independently re-confirms ctr-0023's limb 1 thirteen cycles later, on a fresh call, with byte
sizes agreeing to the digit. **I re-derived the listing, not the function bodies** — the four
scoring-function names and the `fn`-line render conflict are carried over explicitly labelled as
not re-read, and **ctr-0023 step (iv) is not discharged.**

**A locating fact for ctr-0008 step (i), the oldest undone repair in this graph** (38 cycles): the
file is **not** at `eval/threat_actor.py`. Its only occurrence is
`stage3_ti_drafting/score_evaluation/eval/threat_actor.py`, 7,017 bytes. That repair belongs to a
different issue and I did not perform it — but whoever takes it now has a one-call job instead of a
search.

**What I did not establish, said plainly:** asked for every repository path containing "ttp" or
"eval", the render gave a *partial* list prefixed "Multiple matches exist, including". **No
exhaustive repository-wide file list is established** — only the `ttp/` subtree, enumerated as a
closed list, and the one path asked for by name.

---

## Next task rationale

**T3 → T4.** Cycle 69 is a T4 (assess), covering all nine issues, `attempt_count` 0. That is the
state machine with no discretion in it, and the refresh rule is not evaluated in a T3 cycle.

Three things I put in front of cycle 69 deliberately:

1. **It is the first T4 in a long while with a real decision.** All nine issues have scored 2 long
   enough that the T5 selector has had no discrimination from scores for six consecutive T5s
   (carry-forward [75]), and `ttp-attack-mapping-reliability` has just received its first genuinely
   new evidence since cycle 31. I told it not to inflate — and also not to decline to move a score
   merely because no recent T4 has moved one.
2. **Carry-forward [97] is now unavoidable on that issue.** It has six supported candidates, **two
   of which are negative findings** (the cycle-46 non-commensurability one and my new
   no-partial-credit one). The policy question — whether a negative finding may count as *resolving*
   an issue — has been declined ad hoc and identically by nine cycles. A T4 has no standing to
   decide it, but it does have standing to **state which way it scored and what the score would have
   been under the other reading**, which no cycle has yet done. That would put a clean decision in
   front of a human instead of a tenth identical decline.
3. **Carry-forward [4] binds on cycle 69 specifically.** The prompt's *subtraction* rule and the
   validator's *ceiling* rule give different answers for any issue judged to merit 3 — which is
   exactly the case that may now arise. I asked it to say which it applied and record what the other
   would have given, rather than silently picking one.

For its G2 I pointed out that **two issues appear never to have had a G2 on any candidate** —
`task-dependent-reliability-framing` (last attempted cycle 16) and `extraction-vs-reasoning-
ordinal-axis` (last attempted cycle 18). Either would be genuinely independent and both are long
overdue.

I also passed on the **cycle-70 refresh firing**, flagged as *inherited and unverified*: I read
`collect_refresh_every: 7` at source in config.yml, but I did **not** read
`prompts/t5_select.md` step 4 — a T3 has no business in it — so the `% 7 == 0` reading comes from
cycle 67's handoff and cycle 70 must re-derive it. I said so rather than passing it on as fact.
...
| Resource | Used |
|---|---|
| Web fetches | **5** — src-0020 ×2 (arxiv v1, ar5iv), src-0023 v2 ×1, src-0021 v1 ×1, GitHub trees API ×1 |
| Web searches | 0 |
| New sources added | **0** (24 → 24) — three *existing* sources gained key_claims |
| Turns | ~34 of 75 |
| Files written | 8 — graph.json, index.json, src-0017.md, src-0020.md, src-0023.md, next_task.json, last_completed_task.txt, this log |

Comfortably inside budget. The five fetches were chosen so that each served more than one purpose:
the trees call served ctr-0023's enumeration *and* ctr-0008's locating problem; the src-0020 pair
served both the G2 and the RAG finding; the src-0021 fetch served both a G2 leg and candidate 6's
no-partial-credit ABSENT.
