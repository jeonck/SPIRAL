...
`state/queue/last_completed_task.txt` read `T4 assess`. `git show --stat HEAD` returned
`358ddb7 cycle 115: T4 assess`, touching `logs/cycle-115-transcript.txt`, `logs/cycle-115.md`,
`state/assessments/scores.json`, `state/meta.json`, `state/queue/last_completed_task.txt`,
`state/queue/next_task.json` — and **nothing else**. No `graph.json`, no `index.json`, no new
`src-*.md`, which is what a T4 whose G2 passed and which opened nothing should look like. Phase
confirmed as **T5**, from the file and the diff rather than from the commit message.

All three confirming checks passed, hand-derived by `jq` (`scripts/validate_state.py` cannot be run
from this harness — [309]):

| check | expected | observed |
|---|---|---|
| `scores.json` | `last=115 total=22 cycles=115` | `last=115 total=22 cycles=115` ✅ |
| `graph.json` | `ctr=61 open=46 issues=10` | `ctr=61 open=46 issues=10` ✅ |
| `index.json` | `srcs=41 last=src-0041` | `srcs=41 last=src-0041` ✅ |

The ceiling fact was re-derived, not inherited:
`jq -r '[.contradictions[]|select(.resolved_cycle==null)|.issue_id]|sort|unique|length'` returns
**10**. Every issue in the vector carries an open contradiction, so under
`gates.g3_contradiction_demotion: 2` and `scoring.scale_max: 5` **all ten are capped at 3**.
...
next unexamined predecessor. 115 took 105's, so 106 is next in line — but `ls logs/` shows
`cycle-106-transcript.txt` with **no `cycle-106.md`**, and `git log --oneline` shows commit
`886a433` titled *"cycle 106: T5 select run failed, no state change"*. Both facts verified
first-hand this cycle, as instructed. Cycle 106 entered nothing, so **I re-checked cycle 107's**
conclusion.

**What cycle 107 concluded.** Cycle 107 was a T5, so the conclusion it entered is its **selection —
`ioc-extraction-reliability` — together with its ranking table** and the two instrument findings it
recorded.

**Route: zero fetches.** Cycle 107 ranked against the state as it stood at cycle 106's commit, and
because 106 changed nothing that state is `886a433`. I re-derived every input with
`git show 886a433:state/...`, exactly as cycle 113 did against cycle 103. **No web fetches were
spent on G2.**

### Input re-derivation — cycle 107's table against the state it ranked

| input | cycle 107's table | `git show 886a433:` | match |
|---|---|---|---|
| scores | ttp 3, vendor-conc 3, other eight 2 | `last=105`; ttp=3, vendor-conc=3, other eight=2 | ✅ |
| `ioc` attempts | `9,21,35,52,65,84,93` | `9,21,35,52,65,84,93` | ✅ |
| `consistency` attempts | `3,15,16,25,39,62,75,80,87,96` | identical | ✅ |
| `institutional-incident` attempts | `12,59,72,90,104` | identical | ✅ |
| `vendor-concentration` attempts | `101` | `101` | ✅ |
| `automated-triage` attempts | *(none)* | *(empty)* | ✅ |
| `created_cycle` (all ten) | 2/2/2/2/45/2/16/2/16/100 | identical | ✅ |
| `depends_on` (all ten) | as tabulated | identical | ✅ |

Every row of cycle 107's ten-row table reproduces from the committed state. The derived steps also
hold: `107 % 7 = 2 ≠ 0` → T3 (correct); 3(a) leaves exactly the four score-2 issues with empty
`depends_on` (correct — `ttp` also has empty `depends_on` but is eliminated one step earlier at
step 2 on score); the 3(b) window 102–106 catches `institutional-incident`'s attempt at 104 and
nothing else, and `vendor-concentration`'s 101 falls outside under either candidate window reading
(correct); 3(c) then eliminates `automated-triage` on `created_cycle` 16 against 2 (correct). The
residual `ioc`-vs-`consistency` tie was broken on stalest-last-attempt (93 < 96) and fewest-total
(7 < 10) — both arithmetic, both check out.

**Downstream confirmation.** `ioc`'s `attempts` array in the *current* graph reads
`9,21,35,52,65,84,93,**108**`. The T3 cycle 107 scheduled did run, on the issue it selected. The
selection was not merely recorded, it was executed.

**The structural finding re-checked on its own terms.** Cycle 107's unreachability argument for
`automated-triage-under-refusal` turns on a counting claim: on a `T5→T3→T4` ladder, the window
`[C−5, C−1]` at a T5 contains T3s at `C−2` and `C−5` and **no others**, so at most two of the three
empty-`depends_on`, `created_cycle`-2, score-2 blockers can be penalised at once, leaving at least
one to beat it at 3(c). Walking the ladder back from a T5 at C — T4 at C−1, T3 at C−2, T5 at C−3,
T4 at C−4, T3 at C−5 — reproduces exactly two T3s in the window. The argument holds.

**Verdict: PASSES.** Cycle 107's selection, its full ranking table and its unreachability finding
all re-verify against the state they were computed from. **No contradiction opened.** One note for
the record rather than a defect: cycle 107 wrote that the next refresh firing would be "cycle 119,
T1 at 120", and that remains right — see the refresh derivation below, which I computed
independently before reading it.

---

## Task performed

T5 (Select), applied mechanically per `prompts/t5_select.md`, read at source this cycle.
**Selected: `ioc-extraction-reliability`.** Next task is a **T3** at cycle 117.

### Refresh rule, re-derived and not inherited

`prompts/t5_select.md` step 4 evaluates `current_cycle % schedule.collect_refresh_every == 0` at the
T5's own cycle. `collect_refresh_every` is **7** in `config.yml`. **`116 % 7 = 4 ≠ 0`, so no
refresh: the next task is a T3.** On an unbroken ladder the next T5 sits at **119**, and
`119 % 7 = 0`, so **the next T1 is scheduled by cycle 119's selection and runs at cycle 120.** I
record this so my successor does not re-derive it wrong.

### Step 1 — candidate set

All ten issues score < 5 (the vector is 3/3/2×8, total 22). **All ten are candidates.**

### Step 2 — base priority

`ttp-attack-mapping-reliability` (3) and `incident-evidence-vendor-concentration` (3) are eliminated
here. The **eight-way tie at 2 is the whole selection problem**, and the tie-breaks decide it.

### Step 3(a) — upstream first

`depends_on` read from the graph. An issue is *beaten* if it depends on another issue in the tied
set; *unbeaten (vacuous)* means it appears in no `depends_on` array in either direction, which under
[246] is neither promotion nor demotion. Unbeaten at score 2: **`ioc`,
`consistency-calibration`, `institutional-incident`, `automated-triage`** — the same four as at
cycle 107.

### Step 3(b) — attempt penalty

Window = "within the last 5 cycles" at `current_cycle` 116 → **cycles 111–115 inclusive**, the same
reading cycles 97, 98, 103 and 107 used. Penalty **1 per attempt** (`config.yml`
`tie_break_recent_attempt_penalty: 1`). Two issues are penalised: **`consistency-calibration` (T3 at
111)** and **`institutional-incident` (T3 at 114)**. `ioc`'s most recent attempt is **108**, which
is *outside* the window — it carries **no penalty**.

### THE RANKING TABLE

| rank | issue | score | 3(a) pairwise | `attempts` | in 111–115 | 3(b) | eff. | `created` | eliminated at |
|---|---|---|---|---|---|---|---|---|---|
| **1** | **ioc-extraction-reliability** | 2 | unbeaten | 9,21,35,52,65,84,93,108 | — | 0 | **2** | **2** | **SELECTED** (3(c)) |
| 2 | automated-triage-under-refusal | 2 | unbeaten (vacuous) | *(none)* | — | 0 | 2 | 16 | **3(c)**, age |
| 3 | consistency-calibration-as-failure-mode | 2 | unbeaten | 3,15,16,25,39,62,75,80,87,96,**111** | **111** | **+1** | 3 | 2 | **3(b)** |
| 4 | institutional-incident-real-world-impact | 2 | unbeaten | 12,59,72,90,104,**114** | **114** | **+1** | 3 | 2 | **3(b)** |
...
### Step 3(c) — and the policy did NOT run out this cycle

After 3(b), two issues remain at effective 2: `ioc` (`created_cycle` **2**) and `automated-triage`
(`created_cycle` **16**). **3(c) selects the older: `ioc-extraction-reliability`.**

This is worth flagging as evaluation data. **Carry-forward [273] — "the policy has no step 4 and
runs out" — did NOT recur.** It fired at cycles 103, 107 and 113. This is the first selection since
cycle 98 where the policy as written reached a unique answer on its own steps, and it did so only
because 3(b) happened to penalise two of the three long-standing blockers in the same window.
**The defect is dormant, not fixed.**

### The selection is robust to the one genuine ambiguity in the policy

`prompts/t5_select.md` does not say whether 3(a) *partitions* (lexicographic: unbeaten always
outranks beaten, whatever the 3(b) penalty) or whether 3(b) is an *effective-score modifier applied
across the whole tied set*. Prior cycles used the lexicographic reading and so did I. It makes no
difference here: **`ioc` is unbeaten at 3(a), carries penalty 0 at 3(b), and is `created_cycle` 2 —
it dominates every other candidate on every criterion the policy defines.** The reading only
matters for the ordering of rows 3–8, which affects nothing. I record the ambiguity rather than
pretend the policy is tighter than it is.

### `automated-triage-under-refusal` came the closest it ever has, and lost exactly as predicted

It reached the final pair and was eliminated at 3(c) on age. **This is the cleanest empirical
confirmation of cycle 107's structural argument yet produced**, and it happened by the mechanism
that argument names: two of the three empty-`depends_on`, `created_cycle`-2, score-2 blockers
(`consistency` at 111, `institutional-incident` at 114) were penalised inside one window, and the
third — `ioc` — survived to beat it. Cycle 107 proved at most two can ever be penalised at once.
Cycle 116 is the case where exactly two were, and the issue *still* lost.

It has now gone **100 cycles since creation with an empty `attempts` array**, while seven
consecutive assessments have called it the only issue where one ordinary T3 could plausibly move a
score. **I did not hand-pick it.** A T5 selects under the policy; three consecutive T4s have refused
to move its integer to buy reachability because that would falsify the instrument, and the same
discipline binds a T5. I applied the policy as written, and I record the result plainly: the
exclusion is **structural, not unlucky**, and it needs a human to amend
`prompts/t5_select.md`. That is the second standing defect in this instrument, alongside
carry-forward [4].

### What the T3 at cycle 117 is commissioned to do

`ioc`'s live agenda is unusually well-specified, so the queue entry is a narrow, zero-fetch
commission rather than an open investigation:

1. **Discharge `ctr-0020`'s closure condition**, open since cycle 52 and *reported undone rather
   than dropped* by cycle 108 for want of budget. The state says src-0003's 43% figure is a
   reduction in *"annotation time"*; src-0003 states the figure three times in three wordings naming
   three different quantities, and the measurement-bearing sentence says *"a 43% drop in the average
   and median time the analysts spent on a report"*. **"Annotation time" is this state's word for a
   quantity src-0003 never names.** Cycle 108 settled the append-only-safe repair shape and left it
   waiting: an *appended* DO-NOT-CITE correction quoting the defective wording beside the
   measurement-bearing sentence, in `src-0003.md`, `index.json` and `candidate_resolutions[0]` —
   never a rewrite, never a deletion.
2. **Apply standing rule [304]** — both limbs. I pre-derived the bounded retrospective scope at zero
   cost and told the successor to re-derive it: of `ioc`'s four candidates, **exactly one is
   single-source and supported — `candidate_resolutions[0]`, resting on src-0003 alone** ([1] is
   two-source, [3] three-source, [2] is `proposed`). **That is the same candidate `ctr-0020`'s
   repair touches**, so both commissions land in one pass. This is the first cycle to apply [304](b)
   at all, and the bounded form costs one candidate.
3. **Decline `open_questions[4]` again** on the ground cycles 21, 52, 93 and 108 gave: no source in
   this base measures adversarial IoC concealment, so no T3 can discharge it. It is T1-only, and the
   next T1 is cycle 120.
4. **Leave `ctr-0010` step (i)** — it asks a T4 to fix a mis-scoped restatement in `scores.json` and
   is not a T3's to do.

### What I did not do

I did not touch `scores.json`. I did not add a source, split, merge or create an issue, or edit any
`candidate_resolutions` or `open_questions`. I opened **no contradiction** — my G2 passed and no two
supported claims are in conflict this cycle. I did not revive the "cheapest issue to uncap" route,
which [302] records is dead now that all ten issues are capped. **Zero web fetches were spent this
cycle**, on G2 or on anything else.

---

## Changes made

- `state/queue/next_task.json` — replaced with a **T3 (Investigate)** for cycle 117 targeting
  `ioc-extraction-reliability`, `attempt_count` 0, self-contained, **15.9 KB** (comparable to cycle
  113's 15.6 KB; [238]'s ~15 KB guideline). Carries the phase check with post-116 expected values,
  the selection rationale, the `ctr-0020` commission with its settled repair shape, [304](a)+(b)
  with the pre-derived single-source scope, the operative heads of all thirteen `open_questions`
  plus the `jq` to read each in full, the four open contradictions of the issue, the standing
  citation bans, the G2 hand-off (cycle 108, re-derivable from `92b7f57`), and the bash/tool notes.
- `state/queue/last_completed_task.txt` — `T5 select`.
- `logs/cycle-116.md` — this file.
- `state/meta.json` — cycle counter (written by `run_cycle.sh`).

Nothing else in `state/` was touched, which is the full and correct footprint of a T5 whose G2
passed.
...
The policy chose `ioc-extraction-reliability` and reached it through 3(c) without needing an
extra-policy residual — the first time since cycle 98. It is stale by the policy's own measure (last
investigated at 108, outside the 111–115 window), it is upstream (`task-dependent-reliability-framing`
depends on it), and it is `created_cycle` 2.

The commission is narrow on purpose. Cycle 108 spent its budget on internal-consistency repairs and
explicitly handed forward one unfinished, fully-specified item — `ctr-0020`'s closure condition, open
since cycle 52 — together with the append-only-safe shape for fixing it. That is a rare thing in this
base: a contradiction whose remedy needs **no fetches, no new source and no judgement call**, only
execution. Pairing it with [304](b)'s bounded audit costs nothing extra because both land on the same
single-source candidate, `candidate_resolutions[0]`. A T3 that discharges `ctr-0020` reduces the open
set on this issue from four to three and removes a wording defect that has been propagating through
three files since cycle 2.

I chose **not** to send the T3 at `open_questions[0]`'s system-versus-corpus gap, which is the
substantively bigger question. Cycles 9, 21, 52, 93 and 108 all report that **no head-to-head
exists** and that forward-citation sweeps are unavailable infrastructure ([6]) — that is a T1
problem, and the next T1 is 120. Spending cycle 117 re-searching a route five cycles have found
closed would be the thrashing 3(b) exists to prevent.
...
- Web searches: **0**
- Web fetches: **0** (G2 was re-derived entirely from `git show 886a433:state/...`, as the queue
  entry recommended and as cycle 113 did against cycle 103)
- Bash calls: 20 (all `jq`/`git`/`sed`/`grep` reads, one command each)
- Read/Write/Edit tool calls: 8
- Turns: well inside `max_turns: 75`

A T5 is a zero-fetch step by construction; every input was already in the repository.
