# Cycle 145 — T3 investigate — consistency-calibration-as-failure-mode

Date: 2026-08-08. Model `claude-opus-5`, CLI `2.1.220` (pinned, per `config.yml`).
Phase confirmed before any work: `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed `7cb403e cycle 144: T5 select` touching `state/meta.json`,
both `state/queue/` files and `logs/cycle-144.md` and **nothing in `state/knowledge/`,
`state/issues/graph.json` or `state/assessments/scores.json`** — exactly what a T5 may touch.
All three confirming `jq` checks reproduced the queue entry's expected values on the nose:
`srcs=48 last=src-0048`; `ctr=70 open=52 issues=10 last=ctr-0070`; `last=143 n=10 total=22 max=3`.
`prompts/t3_investigate.md` was read at source and not taken from the queue entry's account of it.

## Task performed

Two commissioned items, both landed. **Three fetches, zero searches.** No source added, no score
moved, no contradiction opened, no candidate's status changed.

### ITEM 1 — src-0013's ECE bin count and binning scheme. DISCHARGED, TWO RENDER FAMILIES.

The question was put **without supplying a candidate number** (the standing query discipline,
`[361]`) and asked for the metric definition only, never for a results table (`[20]` binds).

Both `https://arxiv.org/html/2606.31159v1` and `https://ar5iv.labs.arxiv.org/html/2606.31159`
returned the same bin-count sentence, verbatim:

> 'We use B=10 following standard practice'

— the ar5iv render carrying the trailing citation marker, `'We use B=10 following standard
practice [28].'` Both also returned the scheme inside the definition sentence, verbatim:

> 'is calculated as the average absolute difference between a model's expressed confidence and
> its empirical accuracy across B equal-width confidence bins'

**So src-0013 computes ECE over ten equal-width bins.** Per `[382]` each render was asked
*separately* where its received text ended; both reported reaching past the References into the
page footer, so neither ABSENT-by-truncation risk applies. The ar5iv form is legitimate here and
the cycle-96 version-staleness rule does **not** bite: `state/knowledge/src-0013.md` records
src-0013 as having exactly one arXiv version, and that was re-checked *before* the fetch.

The mathematics arrives from both renders as flattened LaTeX. It is the ordinary
bin-population-weighted mean absolute gap between per-bin empirical accuracy and per-bin average
confidence, and it is **not entered as a character-level verbatim string** — only the two prose
sentences above are.

**What this discharges:** paragraph (2) of `open_questions[13]`; work-owed (23); and the last
recoverable leg of the standing ECE-binning honesty item, which is now closed. This base knows
the binning of **two of the three** sources that report an ECE — src-0045 ten equal-width
(cycle 136), src-0013 ten equal-width (this cycle) — and the third, src-0001, is confirmed absent
from its published text on two renders (cycle 136) and is irrecoverable without its code or its
authors.

**What this does not do, and the limit is the point.** It does **not** lift the cross-source ECE
comparison ban. src-0001's leg is unknown, and per src-0028 and src-0029 a fixed-binning ECE is
bin-count-dependent and a lower bound on the true calibration error, so any difference involving
src-0001 still cannot even be signed. src-0028's own words, held in
`state/knowledge/src-0028.md` and re-read this cycle: *'using a fixed binning scheme represents a
lower bound of the respective calibration error'* and *'the number of bins and the binning scheme
can significantly influence the estimated calibration error'*.

A **narrower question replaces the old one** and is recorded rather than answered. src-0045 and
src-0013 are now the one pair in this base whose binning is known and matched — so binning is no
longer what blocks that pair. Two other blocks stand and both are independent of binning:
(a) **modality**, ctr-0027 — src-0045 is token-probability-derived (verbatim at source in
`state/knowledge/src-0045.md`), src-0013 is verbalized, so the two numbers are not readings of one
construct; and (b) **domain** — src-0045 is not a CTI source and src-0013 is a calibration study
of code security, so neither confers CTI support here whatever the arithmetic would say. **No
cycle may read the matched binning as permission to compare those two numbers**, and that
sentence is written into the graph rather than left to a successor's judgement.

### ITEM 2 — JOB A, the src-0014 (PromptAudit) appendices. PARTIALLY DISCHARGED; NOW EXHAUSTED ON ALL KNOWN ROUTES.

The prohibited `/html` render was **not** re-fetched. Both untried forms were run.

**Fifth attempt — `https://r.jina.ai/https://arxiv.org/pdf/2605.24171` — PARTIAL SUCCESS, and it
is the first render in this base's history to deliver src-0014 appendix body text.** It returned
content beneath A.1 (Zero-Shot Prompt), A.2 (Few-Shot Prompt), A.3 (Chain-of-Thought Prompt),
A.4 (Adaptive Chain-of-Thought) and A.5 (Self-Consistency Strategy), then truncated mid-sentence
inside the following subsection at `'Full : structured parsing plus w'`. Per `[382]` it was asked
separately where its text ended and it reported that truncation itself rather than asserting
completeness.

Sweep over the delivered region: `confidence` ABSENT, `logit` ABSENT, `log probabilit` ABSENT,
`token probabilit` ABSENT, `verbaliz` ABSENT, `calibrat` ABSENT. `self-consistency` **PRESENT**,
returning *'The self-consistency strategy uses the same adaptive CoT template as A-CoT, but
relies on the experiment runner to issue multiple completions under the active output protocol.'*

**That PRESENT is a positive control, not a new finding, and no G3 contradiction is opened.**
`state/knowledge/src-0014.md` already records `self-consistency` as PRESENT throughout that
paper's body, so the hit *reproduces* a known presence instead of conflicting with the body-only
ABSENTs. Its evidential value is that it demonstrates the render was reading real appendix prose
rather than reporting an empty region — which is exactly what licenses reading the other six
ABSENTs as real. That generalises, and is filed below as `[388]`.

**Sixth attempt — `https://ar5iv.labs.arxiv.org/html/2605.24171` — CLEAN FAILURE.** The
cycle-96 version-staleness rule in `state/knowledge/src-0015.md` was read at source *before* this
fetch and does not bite: `state/knowledge/src-0014.md` records src-0014 as **v1 only**, submitted
22 May 2026, so an unversioned ar5iv URL cannot serve a stale version of it. The render
nevertheless truncated **mid-bibliography** — last received line a reference entry beginning
*'Brittany Johnson, Yoonki Song, Emerson Murphy-Hill, and Robert Bowdidge. 2013.'* — and delivered
no appendix body at all.

It returned ABSENT for all six modality strings 'in appendices B–E'. **That ABSENT is worthless
and is cited nowhere.** The render never received appendices B–E, so the ABSENT is an artefact of
truncation and not an attested absence. This is recorded explicitly in the graph so no successor
mistakes it for a result, and it is filed below as `[387]`.

**What the sixth attempt did buy, one-render and labelled as such:** ar5iv returned the top-level
appendix titles from the document front matter — A *'Prompt Templates and Output Protocol'*,
B *'Label Parsing Logic'*, C *'Generation Settings and Configurations'*, D *'Evaluation Metrics'*,
E *'Supplementary Result Tables'*. **This reprioritises the residue.** Appendix E has been JOB A's
named target since cycle 36, and two independent renders now *indicate* E is a results-table
appendix and not a method one: ar5iv titles it 'Supplementary Result Tables', and the r.jina.ai
render returned the body sentence pointing at it, *'the full standard-F1 and effective-F1 table is
provided in Appendix E (Table 4)'*. Both are **indirect evidence about E's content rather than a
reading of E** and are labelled that way. If the indication holds, Appendix D 'Evaluation Metrics'
is the higher-value target for a modality or calibration string — and no cycle has ever named it.
Recorded as a lead, not commissioned.

**The two renders numbered the appendices differently** — r.jina.ai placed 'Label Parsing Logic'
at A.6, ar5iv places it at top-level B. Per `[378]` that is a rendering/format difference, not a
two-render disagreement about the document, and **both renders truncated**, so neither is
authoritative on this source's appendix structure. No appendix numbering was entered as a fact.

**Net position.** Scope limit (i) of `candidate_resolutions[13]` is **partially discharged and not
closed**. The standing label 'EVERY src-0014 ABSENT IN THIS BASE IS BODY-ONLY' is wrong as worded
from this cycle onward and is replaced by: *every src-0014 modality ABSENT covers the body and
Appendix A, and Appendices B through E are untested because no render has ever delivered them.*
JOB A is marked **exhausted on all known routes** — `/html` four times (cycles 36, 96, 111),
r.jina.ai PDF once, ar5iv once — and a successor wanting B–E needs a route this base does not
have. Do not spend a seventh fetch on any of the three known forms.

### ITEM 3 — write-up

Both items landed early, so ITEM 3 was performed in full. See **Changes made**.

The bans held. `candidate_resolutions[2]` was not promoted (its cycle-96 block stands);
`[15]`, `[16]` and `[17]` were not promoted (the cycle-134 source hold stands); no ECE value was
compared across sources; src-0013's results tables were not pulled.

## Retrospection

**G2 target: cycle 136**, per the round-2 order (144 took 135's; the next unexamined predecessor
is 136). Commit `a162864`, stat-checked before its message was trusted: it touched
`state/issues/graph.json` (26 lines), `state/meta.json`, both queue files and its log — and
**nothing in `state/knowledge/` or `state/assessments/scores.json`**, correct for a T3.
Cycle 136 was a T3 on *this same issue*, so the G2 doubled as preparation.

**Zero fetches.** Method: `git show a162864^:state/issues/graph.json > pre136.json` and
`git show a162864:state/issues/graph.json > post136.json`, then two-input `jq` diffs.
Cycles 121–133 all did their G2 at zero fetches, 134 spent one deliberately, 135–144 spent zero;
this cycle continues that.

**Result: PASSES IN FULL. No contradiction opened.** Every conclusion cycle 136 entered is
present, correct and unchanged today:

| claim | verified |
|---|---|
| `candidate_resolutions[18]` added, status `supported`, evidence src-0045/src-0001/src-0028/src-0029 | pre 18 → post 19 candidates; header identical in post136 and today (`len=2922`) |
| a new `open_questions[13]` | pre 13 → post 14 open_questions |
| `open_questions[11]` extended, not rewritten | 11105 → 12610 chars, `startswith` true; today byte-identical to post136 |
| `ctr-0068` closed | `resolved_cycle` null → 136; reads 136 today, `opened_cycle` 134 |
| cycle appended to `attempts` | `[…,122]` → `[…,122,136]` |

I also re-checked cr[18]'s **substance** against its cited sources at zero fetch, which is the
part that matters. Its paragraph (2) rests on src-0028 and src-0029, and
`state/knowledge/src-0028.md` key_claim 2 carries the two verbatim sentences that back it exactly
(quoted above). Its src-0001 limb — 'states no bin count and no binning scheme anywhere' — is
recorded verbatim at `state/knowledge/src-0001.md` line 270. **Both citations check out.**

**One benign coverage note, reported as a supersession and not a failure.**
`state/knowledge/src-0045.md` still says that source's *'bin count and binning scheme were not
captured by either render this cycle'*. That was **true when written at cycle 134**, and cycle 136
then established the ten-equal-width-bin quotes by fetch — recording them, correctly for a T3,
only in `graph.json`. The source file's sentence is explicitly scoped 'this cycle', so it is not
wrong; the source record simply understates what a later cycle verified. That is work-owed (30),
and ITEM 1 has now created a fifth instance of it on src-0013. It is not a defect in cycle 136's
conclusion and no contradiction is opened for it.

## Changes made

`state/issues/graph.json` only. Five edits, all on `consistency-calibration-as-failure-mode`,
all append-only, all proved before the `mv`:

1. `open_questions[13]` — cycle-145 block appended: ITEM 1's result, both verbatim sentences, the
   two-render attestation, what it discharges, and the explicit statement that the cross-source
   ECE ban survives and that the matched src-0045/src-0013 binning is **not** permission to
   compare.
2. `open_questions[8]` — cycle-145 block appended: JOB A's fifth and sixth attempts in full, the
   positive control, the worthless-ABSENT warning, the Appendix E → Appendix D reprioritisation
   (labelled one-render), the numbering non-dispute, and the exhausted-on-all-routes verdict.
3. `candidate_resolutions[13].summary` — cycle-145 block appended: scope limit (i) partially
   discharged, the standing body-only label retired and replaced. Status stays `supported`.
4. `candidate_resolutions[18].summary` — cycle-145 block appended: scope limit 1 superseded
   (two of three, not one of three), the prohibition restated as still binding in full because
   scope limit 3 is the load-bearing one. Status stays `supported`.
5. `attempts` — `145` appended: `[3,15,16,25,39,62,75,80,87,96,111,122,136,145]`.

**Append-only proof, run before the `mv`, two `jq` calls:**

- `startswith` true for all four extended string fields against `git show HEAD:…`.
- `.contradictions`, `._schema`, the issue-id vector and **the other nine issues** byte-identical.
- Within the edited issue: the candidate **status** vector, the candidate **evidence** vector, the
  twelve unedited `open_questions` (via `del(.[8],.[13])`) and the key set all byte-identical.
- `cr=19 oq=14` after the edit — no entry added or removed.

Scratch files (`scratch145.json`, `old_graph.json`, `pre136.json`, `post136.json`) removed;
`git status --short` shows only `state/issues/graph.json` (mine) and `state/meta.json` (the
harness's cycle increment).

**Re-derived, not inherited** (work-owed (21), re-run *after* my edits): nineteen candidates,
thirteen `supported`, eleven of those citing src-0001, and the only two supported candidates not
citing src-0001 are **index 8 (src-0018 alone)** and **index 9 (src-0013 alone)**, both
single-source. Unchanged from cycles 140, 141, 143 and 144. **ctr-0063 still cannot be resolved.**
Open contradictions per issue also re-derived: institutional-incident 13, consistency-calibration
9, ttp 6, automated-triage 5, ioc 5, attribution-expressed 4, task-dependent 4,
attribution-confident 3, extraction-vs-reasoning 2, incident-evidence-vendor-concentration 1 —
**total 52**, unchanged.

**Not run:** `scripts/validate_state.py` cannot be executed in this sandbox (`python3` is
refused). The gates were hand-checked with `jq` as described. I am not implying a validator run.

## Next task rationale

State machine edge is **T3 → T4**, so cycle 146 is a **T4 assess**, target
`consistency-calibration-as-failure-mode` — the issue this cycle investigated, which is the
issue a T4 should now re-score.

The queue entry says plainly what a T4 should expect, so it does not manufacture movement:
**cycle 145 changed no candidate's status.** Its two results are a *methodological* fact about
this base's own numbers (src-0013's binning, which narrows why the ECE ban exists without lifting
it) and a *coverage* fact (src-0014's Appendix A is now swept, hardening
`candidate_resolutions[13]` without widening it). Neither adds a CTI source, and the issue's
central weakness is unchanged: **the calibration leg remains single-source on src-0001.** If the
rubric does not move the score from 2, the T4 should re-affirm 2 and say why.

The entry carries forward the whole ban set and work-owed list, updates (22) and (23) for what
this cycle discharged, closes honesty item (D), retires the src-0014 body-only label, and adds
`[387]` and `[388]`. It also flags that honesty item (G) — the inert g3 ceiling — is *directly*
the T4's business this cycle and must be stated, not assumed; and it notes that because cycle 145
attempted this issue, the next T5 will see it carrying a 3(b) penalty while
`automated-triage-under-refusal` will not (honesty item (A)).

G2 for cycle 146 is **cycle 137** (`e37d7ef`, a T4 assess — again same-role preparation), with the
`git show <rev>:<path>` diff recipe spelled out and the `[384]` warning that `scores.json` is over
1 MB and must go through `jq`.

`146 % 7 = 6`, so cycle 147 is not a refresh cycle — but the entry tells the T5 to re-read
`config.yml` and `prompts/t5_select.md` at source and recompute rather than trust that line.

## Budget

- **Fetches: 4.** Two for ITEM 1 (`arxiv.org/html/2606.31159v1`,
  `ar5iv.labs.arxiv.org/html/2606.31159`) and two for ITEM 2
  (`r.jina.ai/https://arxiv.org/pdf/2605.24171`, `ar5iv.labs.arxiv.org/html/2605.24171`).
- **Searches: 0.**
- **G2 fetches: 0.**
- **Turns: roughly 35 of 75**, comfortably inside budget. No abort risk; queue files were written
  before this log, per the finish order.
- Both commissioned items landed, so no attempt was retried and `attempt_count` for cycle 146 is 0.
