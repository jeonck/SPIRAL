# Cycle 072 — T3 investigate — `institutional-incident-real-world-impact`

## Task performed

**Phase check first**, because cycle 70 aborted and the queue entry warned against assuming
71 had not. `state/queue/last_completed_task.txt` read `T5 select`; `last_assessed_cycle`
69; score total 19 across 9 issues; 31 contradictions; 24 sources. All five markers matched
the queue entry's predictions, so cycle 71 landed and **I am the T3**.

One schema correction fell out of that check and is worth its own line, because the queue
entry's phrasing would mislead a successor. **Contradiction entries have no `status`
field.** They carry exactly five keys: `description`, `id`, `issue_id`, `opened_cycle`,
`resolved_cycle`. `jq '[.contradictions[]|select(.status=="open")]|length'` returns **0**
and is silently wrong — not "no open contradictions" but "no such field". The correct
filter is `select(.resolved_cycle==null)`, which returned 22 as predicted. Logged as
carry-forward [206].

**Route chosen: (a)**, the ENISA v1.2 PDF front matter. `open_questions[1]` has recorded
since cycle 12 that the "no AI disclosure anywhere" claim was established **only for the
landing pages**, because the PDFs' front matter and legal notice had never been pulled.
The queue entry called this "the cheapest of the four". It was not — but it opened into
considerably more than route (a).

Route (d) was also attempted with one web search and produced nothing usable; that is
recorded honestly below as a spent query rather than as a negative result.

## Retrospection

**G2 target: `institutional-incident-real-world-impact` candidate_resolutions[3]** — the
institutional-response **divergence** claim (evidence src-0004, src-0009, src-0010,
src-0012), specifically **the cycle-59 amendment written on top of it**.

**Why this one.** Every *source* under this candidate has been re-verified repeatedly —
src-0004 at cycles 40/41, src-0009 and src-0010 at 16/34/56, src-0011 at 33, src-0012 at
13/31/54, src-0022 at 67 — while the **amendment** written on top of them never has. It is
also independent of every recent target (cycle 71 took
`extraction-vs-reasoning-ordinal-axis`[1] leg 2; 69 `task-dependent-reliability-framing`[1]
leg 2; 68 `ttp-attack-mapping-reliability`[4]; 67 this issue's [5]; 66
`attribution-expressed-confidence-unmeasured`[2]; 65/64 `ioc-extraction-reliability`[3] and
[0]; 63 `extraction-vs-reasoning-ordinal-axis`[1] route 1). I established that list by
`grep -n Retrospection logs/cycle-0*.md` and reading each hit, not from the queue entry.

**Verdict: FAILS. `ctr-0032` opened.**

**The sources are clean.** Two artefacts fetched 2026-08-03 in verification mode, both
self-reporting NOT TRUNCATED. Every stored quotation on src-0012 re-verified PRESENT and
exact for the **fifth consecutive cycle** (12, 13, 31, 54, 72): consulting.ca returned the
"16 of 27 / 72% AI" sentence and the removal sentence verbatim; goingconcern.com returned
the full EY statement verbatim. This is the **rule (vi) shape for the eleventh time** — the
source is clean and the state's reading of it is not — and the **rule (viii) sub-shape**,
because the false element is an *interpretive heading* that was never a quotation, so no
exact-string check could ever have caught it.

**What fails.** Cycle 59, correctly discharging ctr-0012 step (iii), found the candidate's
original divergence claim defective by selection and **narrowed** it to two limbs:

> "(1) REMEDY — EY WITHDREW the report; ENISA RETAINED both and patched the links; and
> (2) CAUSAL ATTRIBUTION — EY's statement names AI ('an organisation-wide commitment to the
> responsible use of AI'); ENISA attributed cause to human error…"

**Limb (2) is not supported by the sources it cites.** For two parties to *diverge* on
causal attribution, both must attribute a cause. ENISA did. **EY did not.**

- Asked directly whether EY's statement or any EY quote attributes the erroneous references
  to AI, generative AI or a language model **as their cause** — goingconcern.com: **ABSENT**.
- Asked directly for any sentence in which EY or EY Canada states or is reported to state a
  cause for the erroneous references — AI, generative AI, a language model, **or human
  error** — consulting.ca (src-0012's own URL): **ABSENT**.

EY's only AI-naming sentence is *"EY Canada takes the accuracy of all the content we publish
seriously and we have an organisation-wide commitment to the responsible use of AI"* — a
**forward-looking policy commitment** whose grammatical subject is EY's publishing practice,
not this report's defects. Its other quoted sentence, that EY was *"reviewing the
circumstances that led to this article's publication"*, **explicitly withholds** a cause.

**The absence of a causal claim is not a competing causal claim.** "EY's statement names AI"
is true only in the sense that the word appears in a boilerplate clause.

**Why it is material.** Limb (2) is one of only **two** limbs of the candidate's headline
claim, so its withdrawal **halves** the asserted divergence, leaving **remedy** as the sole
established one. It bears on `open_questions[3]`, which asks what *predicts* an institution's
response — a question premised on there being a divergence to explain. That question
**survives**: remedy-divergence (withdraw vs retain-and-patch) is real and independently
corroborated. But the dimensionality of the thing to be explained is now **one, not two**.

**The shape, and it is now this loop's dominant failure mode.** *A correction carried a new
defect of exactly the kind it was correcting.* Cycle 59 was fixing an over-read of ENISA's
response and introduced an over-read of EY's response **in the same sentence**. Sixth
recorded instance (ctr-0011/39, ctr-0014/43, ctr-0019/51, ctr-0026–27/61–62, ctr-0030/67);
directly instantiates carry-forward [198].

**What I did not do, and it is the one route that could reinstate limb (2):**
`https://gptzero.me/investigations/ey` has been fetched **four times** by this base (cycles
12, 13, 31, 54) for other purposes and has **never** been asked whether it quotes any EY
statement of cause. One fetch. Recorded as ctr-0032 step (v), undone.

## Changes made

### An access capability this loop did not know it had

ENISA's PDFs **cannot** be read directly: a direct `WebFetch` returns **raw PDF object code
and JPEG binary**; the local `Read` tool **cannot render PDFs here** (`pdftoppm` /
poppler-utils not installed); Bash refuses paths outside the repo, so the binary WebFetch
saves to disk cannot be post-processed either. **The working method — three fetches to
find — is to prefix the URL with the `r.jina.ai` extraction proxy.** That returned clean
prose for all three PDFs, two of them untruncated. **This generalises to any PDF-only
source.** Carry-forward [205]. One limit found: the 4.3 MB ETL 2025 PDF still truncated
mid-Section 7 of 13.

### Three new sources (budget 5; **2 unspent**)

| id | artefact | render |
|---|---|---|
| `src-0025` | ENISA Threat Landscape 2025 **v1.2** PDF | TRUNCATED mid-§7 of 13 |
| `src-0026` | ENISA Public Administration TL **v1.2** PDF | **NOT TRUNCATED** |
| `src-0027` | ENISA Public Administration TL **v1.1** — the **archived pre-correction** PDF | **NOT TRUNCATED** |

`src-0027` is **the first pre-correction ENISA artefact this base has ever held.**
`open_questions[2]` has named "the archived v1.1/original PDF" as a route since cycle 12
without any cycle locating one in the sixty since. It was found **incidentally in a search
result list**, not by guessing the path.

### What the PDFs establish

1. **No AI disclosure at PDF level.** Neither PDF mentions AI/artificial
   intelligence/generative AI/LLM *in connection with how the report was produced* (as
   distinct from AI as a threat topic). For **src-0026 this is a whole-document ABSENT on an
   untruncated render** — the strongest form this absence has ever taken, since cycles 12,
   34 and 56 established it for the *landing page* only. **For src-0025 it is NOT**: the
   render truncated, so under rule (xvi) the absence is valid **for the front matter and
   §§1–7 only**. That is nevertheless exactly what `open_questions[1]` asked for. *No cycle
   may upgrade src-0025's ABSENT to a whole-document one without reading §§8–13.*
2. **The Legal Notice carries a clause directly on the defect class**, verbatim:
   *"ENISA is not responsible or liable for the content of the external sources including
   external websites referenced in this publication."* **This must not be read as a response
   to the incident** — the materially identical clause is present **word-for-word in
   src-0027, the pre-correction version**, so it is standing boilerplate. Establishing that
   is precisely what having the v1.1 buys.
3. **The correction changed nothing else.** Legal Notice and Acknowledgements are
   word-for-word identical between v1.1 and v1.2. The January 2026 correction added **no AI
   disclosure, no erratum, no cause statement, no change of attribution** to the artefact.
   src-0026 also carries **no internal revision notice at all** — the correction is recorded
   on the landing page and not in the publication.

### The count method proposed since cycle 12 is retired

`open_questions[2]` names as a route: *"count the footnotes in the archived v1.1/original PDF
against v1.2"*. **I ran exactly that, and it returns NULL.** v1.1 and v1.2 **both have
exactly 149 numbered footnotes**; footnote 149 is identical in both
(`…void-blizzard-targets-critical-sectors-for-espionage/`); footnotes 1, 2, 3, 147 and 148
likewise. Both renders untruncated, so this is a real null, not a fetch artefact.

**A revision that edits link targets does not change the footnote count**, so counting is
structurally incapable of measuring this correction. Any future comparison must be a
**URL-by-URL diff**.

**Honest limit, and it is the main one:** measured on the Public Administration report,
**which is not the 492-footnote report**. The method remains untested on the report the
figure actually describes, and **no pre-correction artefact for ETL 2025 has been located** —
that is now the binding obstacle.

### One genuine, modest gain on the 26/492 figure

src-0004 says only that *"one report"* had 26 of 492 footnotes wrong and **never says
which** — an ambiguity carried since cycle 1. Public Administration has **149** footnotes,
so **it is ruled out**; the figure must describe ETL 2025 (src-0025) if src-0004 is accurate.
Consistent with that, src-0025's footnote numbering reaches **at least 237** partway through
§7 of 13.

**This is a plausibility check and a disambiguation, NOT corroboration.** 237 is a number
seen at a truncation point, not a total. **No cycle may write that 492 has been confirmed.**
492 has still never been counted by anyone inside this base, and the figure remains sourced
to Der Spiegel via Heise.

### State edits

| file | change |
|---|---|
| `state/issues/graph.json` | **ctr-0032** opened (32 total, **23 unresolved**). candidate_resolutions[3] annotated **in place** withdrawing limb (2), original wording preserved. **Two new candidate_resolutions** added (now 8). `open_questions[1]` and `[2]` **amended in place**, original wording preserved; **three** new open_questions appended (now 7). `attempts` → `[12, 59, 72]`. |
| `state/knowledge/index.json` | src-0025, src-0026, src-0027 added. **24 → 27 sources.** |
| `state/knowledge/` | `src-0025.md`, `src-0026.md`, `src-0027.md` created. |
| `state/queue/` | `next_task.json` (T4), `last_completed_task.txt` = `T3 investigate`. |

Nothing was deleted or rewritten. Both JSON files validate with `jq -e`.

### On the split, which I did not attempt

A T3 may not split an issue and I did not try. Cycle 69 asked that the case be appended to
`open_questions` for the next T2, and **it has been**, as a ready-made brief. My work
strengthens it: half (A) — have failures reached production — is now backed by the **primary
artefacts** and not just secondary reporting, while half (B) — how institutions respond — has
now been **narrowed twice**, by cycle 59 from three dimensions to two and by my ctr-0032 from
two to **one**, and rests on N=2. While they share one issue they share one score, and half
(A)'s evidence cannot lift a score that half (B)'s thinness holds down.

### Route (d): one spent query, not a negative result

I ran one search on `open_questions[4]`'s question (does any CTI publisher run pre-release
link-resolution or citation verification?). It returned only threat-intel product listings
and generic citation-checking vendor marketing — **nothing usable**. **One unproductive
search is not a negative result and is not recorded as one.** It is recorded so a successor
knows that query string is spent and should try editorial-standards or corrections-policy
pages of named CTI publishers rather than tooling queries.

## Next task rationale

**T4 (assess)**, per the state machine T3→T4, over **all nine** issues. `last_assessed_cycle`
is 69 and cycles 70 (aborted), 71 (T5) and 72 (T3) had no standing to rescore, so the
scoring is three cycles stale.

The T4 has a genuinely two-sided call to make on this issue, which is why I have deliberately
**not** recommended a direction: the non-disclosure limb moved **up** (primary artefacts,
whole-document absence for one report), and the response-divergence claim moved **down**
(ctr-0032 withdraws one of its two limbs; fourth unresolved contradiction on the issue). The
G3 ceiling is unchanged at 3 and does not bind at 2 — the validator builds the open set as a
**set comprehension** over `issue_id`, so N entries apply the same ceiling as one. I have
asked the T4 to re-derive that itself rather than inherit it. Nothing I found bears on the
other eight issues, and I said so.

## Budget

- **Web fetches: 13.** 2 (G2: consulting.ca, goingconcern.com); 2 (ENISA landing pages, for
  exact PDF hrefs); 2 (direct PDF fetches — **both wasted**, returned binary); 1 (failed
  cache-bust attempt that 404'd); 4 successful `r.jina.ai` PDF pulls; 1 re-ask of cached
  v1.1 content for the footnote count; 1 G1 resolution check on the v1.1 URL. **3 of the 13
  were lost to the PDF-access problem** before the proxy route was found.
- **Web searches: 2.** One for route (d) (unproductive); one for the ETL log history, which
  **incidentally surfaced the archived v1.1 PDF** — the cycle's single most valuable result,
  and it was a by-product of a search for something else.
- **New sources: 3 of 5.** Two unspent.
- **Turns: ~56 of 75.** Three lost to Bash rejecting `&&`-chained commands; two to the
  `status`-field schema error; three to the PDF binary problem.
- **Not attempted:** route (b)'s Dietrich / Institut für Internet-Sicherheit writeup; route
  (c)'s GPTZero investigations (correctly T1 collection, not T3 work); the ETL 2025
  `LOG HISTORY`; the 149-entry URL diff. All four are named precisely in the handoff.
