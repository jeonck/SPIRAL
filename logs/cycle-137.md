...
Topic: `cti-ai-reliability` — Reliability assessment of AI-generated Cyber Threat Intelligence.
Queue entry executed: `state/queue/next_task.json` as written by cycle 136.

## Phase check

Run before anything else, because cycles 70, 83 and 106 each aborted on a mis-derived phase.

- `state/queue/last_completed_task.txt` reads `T3 investigate`. Correct predecessor for a T4.
- `git show --stat HEAD` is `a162864 cycle 136: T3 investigate`, touching `logs/cycle-136-transcript.txt`,
  `logs/cycle-136.md`, `state/issues/graph.json`, `state/meta.json` and both `state/queue` files.
  **`state/knowledge/` untouched** (a T3 adds no source) and **`state/assessments/scores.json` untouched**
  (a T3 moves no score). HEAD is a cycle commit, not a human commit, so no walk-back was needed.

All four confirming checks from the queue entry reproduce exactly:

| check | required | observed |
|---|---|---|
| `graph.json` | `ctr=68 open=50 issues=10 last=ctr-0068` | identical |
| ccafm counts | `ccafm_cand=19 ccafm_oq=14 att=136` | identical |
| `index.json` | `srcs=47 last=src-0047` | identical |
| `scores.json` | `last=132 n=10 total=22 max=3` | identical |

I am the T4 for cycle 137, and the first cycle since 132 permitted to write `scores.json`.

`scripts/validate_state.py` was **not** run — Bash in this environment refuses `python3 script.py`
and `python3 -c`. Every gate below was hand-checked with `jq` and I say so rather than implying
a validator pass.

## Retrospection

**Target, per the round-2 order.** 136 took cycle 127's conclusion, so the next unexamined
predecessor is **cycle 128**. `git log --oneline | grep -n "cycle 128"` gives `c28f38f`, and
`git show --stat c28f38f` confirms it is a cycle commit touching `state/issues/graph.json`,
`state/knowledge/index.json` and `state/knowledge/src-0044.md`.

**Locating what cycle 128 actually entered, not what its log said** (carry-forward [337]).
I diffed the graph either side of the commit rather than reading the log:

- `git show c28f38f^:… | jq` vs `git show c28f38f:… | jq` on per-issue counts: the **only**
  change is `institutional-incident-real-world-impact` attempts 6 → 7. No candidate_resolution
  and no open_question was added anywhere.
- On contradiction counts: `ctr=63 open=47 last=ctr-0063` → `ctr=64 open=48 last=ctr-0064`.

So cycle 128's entered conclusion is **`ctr-0064`**, and that is what I re-checked.

**The claim under test.** ctr-0064 records a scope conflict between two outlets reporting the same
remedy: src-0035 quotes KPMG's removal of the report `'from some of its websites while it
investigates how the publication made it into the wild'` — explicitly partial and provisional —
while src-0036 renders the same act without the partitive, which read alone states a *complete*
removal. Cycle 128 backed the partitive reading with a direct observation entered as src-0044.

**Result: RE-VERIFIES IN FULL, at ZERO FETCHES.** All three limbs check out against the stored
sources:

1. `grep -c "some of its websites" state/knowledge/src-0035.md` → **3**. The partitive is there.
2. `state/knowledge/src-0036.md` lines 45 and 79 render the remedy as removal `from its websites
   while conducting its own investigation` — **no partitive**. The divergence is real and is not a
   paraphrase introduced by this base. (src-0036 is under an outright quotation ban; that string is
   the *disputed rendering itself*, which ctr-0064 describes rather than quotes, and I reproduce it
   only to show the absence. No content claim is drawn from it.)
3. `state/knowledge/src-0044.md` records the URL on `assets.kpmg.com`, the retrieval date
   **2026-08-07**, the two-render confirmation, and the three *other* KPMG-owned addresses that
   returned KPMG's own not-found pages. Its own wording — *"THE REMOVAL IS REAL BUT STILL PARTIAL"* —
   is exactly what ctr-0064 relies on.

No contradiction opened. Nothing failed re-verification and nothing was superseded.

**A live finding fell out of it, and it is mine to act on.** ctr-0064 step (ii) is addressed
*"FOR A T2 OR T4, NOT A T3"* — a sweep of `scores.json` and `graph.json` for unqualified renderings
of the KPMG removal inherited from src-0036. Cycle 129 ran it and found zero corrections owed in
`scores.json` plus one borderline item in `graph.json`. **I re-ran the `scores.json` half rather
than inheriting the null**, using the [348] `scan()` instrument with a ±300-character window on
`KPMG`: every KPMG-removal mention in the file already carries a qualifier — the two live forms are
`'removal from some websites WHILE IT INVESTIGATES'` and `'KPMG partial-and-provisional removal'`.
**Zero corrections owed. Cycle 129's count reproduces.**

Which means: step (ii) discharged on both files, step (i) is a standing citation rule, step (iii) is
an explicit note-not-action. **ctr-0064 appears to have no outstanding work and is nonetheless being
counted as an open dispute in the g3 demotion I am obliged to apply.** That is a second instance of
standing honesty item (E), and I have widened (E) in the next queue entry to name it. I did **not**
close it: a T4 does not touch `graph.json`, and cycle 136's closure of ctr-0065 rested on a one-off
authorisation I do not have.

Fetches spent on G2: **zero**, matching cycles 135 and 136.

## Task performed

Scored **all ten** issues per `prompts/t4_assess.md`, stingily, each with a rationale citing source
ids, with the g3 demotion derived per issue rather than assumed.

**The demotion, derived first.** One grouped `jq` over the contradictions, before any scoring:
68 entries, **50 with `resolved_cycle` null**, distributed —

| issue | open ctrs | score |
|---|---|---|
| institutional-incident-real-world-impact | 12 | 2 |
| consistency-calibration-as-failure-mode | 9 | 2 |
| ttp-attack-mapping-reliability | 6 | **3** |
| automated-triage-under-refusal | 5 | 2 |
| ioc-extraction-reliability | 4 | 2 |
| attribution-expressed-confidence-unmeasured | 4 | 2 |
| task-dependent-reliability-framing | 4 | 2 |
| attribution-confident-wrong-gap | 3 | 2 |
| extraction-vs-reasoning-ordinal-axis | 2 | 2 |
| incident-evidence-vendor-concentration | 1 | **3** |

**Every one of the ten carries at least one open contradiction**, so the demotion of 2 fires on the
whole vector and the ceiling is `scale_max - 2 = 3` everywhere. I derived this rather than assuming
it, because cycle 136 closed ctr-0065 and ctr-0068 (52 → 50) and both sat on
consistency-calibration-as-failure-mode — which still has nine open. **No issue changed demotion
state.** Two closures out of eleven changed nothing arithmetically.

### Does any score move? No — and here is the honest working.

The vector has been frozen at 22 since cycle 118 and this is the fifth T4 to hold it. I did not
want to hold it by default, so I worked each entry.

- **consistency-calibration-as-failure-mode (2, held).** The only entry whose *inputs* moved. Cycle
  136's `candidate_resolutions[18]` is supported and multi-source (src-0045, src-0001, src-0028,
  src-0029) — but it raises nothing, on its **own** scope limits: it is a methodological finding
  about this base's numbers rather than a CTI finding, src-0045 is not a CTI source, and its scope
  limit 2 says in terms that the calibration leg **remains single-source on src-0001**. A candidate
  that disclaims the support it would have to supply cannot supply it. It also moves nothing *down*:
  it impeaches no number, and src-0001's 7-of-9 within-source within-metric finding is untouched.
  **Which leg drove the number:** the *consistency* leg reaches level-3 arithmetic —
  `candidate_resolutions[4]` and `[12]` both carry src-0001 **and** src-0018, two independent CTI
  sources. The *calibration* leg does not and cannot at present: src-0013 (code security, not CTI,
  and verbalized where src-0001 is token-probability-derived), src-0018 (elicits no model-expressed
  confidence at all), src-0045, src-0046 (no ECE number at all) and src-0047 (no calibration metric,
  ABSENT asked directly) each fail to be a second CTI calibration source. I score the issue, not its
  better half. An issue named for two failure modes, one carried by a single source, is a 2.
- **automated-triage-under-refusal (2, held).** Its second candidate is new since cycle 132 but is
  `proposed`, rests on src-0047 alone, and disqualifies itself in terms — vendor-authored preprint,
  not peer-reviewed, not reproducible, single deployment, CoT and RLVR never ablated, and *no cycle
  has read that source's body for this issue*. Meanwhile `candidate_resolutions[0]` is effectively
  single-source on src-0007, because src-0015 is under a standing quotation ban until ctr-0057 is
  read in full. Single-source support is the rubric's own definition of level 2. Scored on the
  rubric alone, neither punished nor rewarded for the selector never reaching it.
- **The other eight.** Nothing bearing on them changed. The three sources added at cycle 134 are all
  calibration/triage work; I added none of them to eight of the ten evidence arrays and said so in
  each rationale rather than leaving the omission silent.

### Re-derivations run rather than inherited

Per carry-forward [337], I reproduced every count I relied on:

- **ctr-0063 / item (21):** 19 candidates on ccafm, **13** supported (indices 0,1,4,5,6,7,8,9,10,12,
  13,14,18), **11** citing src-0001, **4** resting on src-0001 alone (0,1,5,7). Matches the queue's
  13/11/4. The only two supported candidates *not* citing src-0001 are index 8 (src-0018 alone) and
  index 9 (src-0013 alone) — both themselves single-source.
- **incident-evidence-vendor-concentration's ban-adjusted support**, which cycle 132 expressly asked
  its successor to re-run: `candidate_resolutions[0]` retains src-0032, src-0012, src-0035, src-0004
  after the outright bans; `[1]` retains src-0032, src-0038, src-0039. Both still clear two quotable
  independent sources. Cycle 132's result reproduces, and no new outright ban landed on any source of
  this issue at cycles 133–136 (cycle 136's new rule is the cross-source ECE prohibition, which
  touches src-0001, src-0013 and src-0045 and no source here). The ban density is re-recorded as a
  live risk to that 3.
- **Every candidate evidence array** for the other issues, read out of the graph rather than copied
  from the previous rationale.
- **attempts arrays.** One correction to my own draft: I had written ioc-extraction-reliability's as
  `[16,17,29,42,56,70,84,97,117,131]`; the graph says `[9,21,35,52,65,84,93,108,117,131]`. Fixed
  before the splice. Ten investigations, last at 131, as previously reported.

### Honesty item (C): I ran the contingency and declined to game it

ctr-0067 settled at cycle 135 that rung 3(a) is read **pairwise**, restricted to the score tier under
consideration. That narrowed the defect from structural to contingent, so I ran the contingency on
the vector I am setting. The weakest tier is score 2 with eight members.
`task-dependent-reliability-framing` is still excluded because **four** of its dependencies
(ioc-extraction, consistency-calibration, attribution-confident-wrong-gap,
attribution-expressed-confidence-unmeasured) sit in that same tier; `extraction-vs-reasoning-ordinal-axis`
is excluded because its sole dependency `task-dependent-reliability-framing` does too. **Both remain
unreachable**, and with them ctr-0009 step (iii) — src-0006's unfetched per-task scoring definitions,
which eight separate cycles call the highest-value unfetched item in the base.

I am the one role that could change that. Unblocking it would require all four dependencies to leave
tier 2 simultaneously, and **not one of them has an honest case for moving this cycle**. I state that
plainly and score honestly anyway. I did not score to engineer a selection and no successor should.

## Changes made

`state/assessments/scores.json` only. No source added, no `src-*.md` touched, `state/issues/graph.json`
untouched, and I appended myself to **no** `attempts` array (that is a T3's act and would corrupt the
selector's 3(b) penalty).

- Ten rationale appendices, spliced in one two-input `jq` pass ([237]) from a single scratch JSON
  ([211]/[228]), validated with `jq -r keys` before the splice and `rm -f`'d after.
- `assessed_cycle = 137` on all ten (via `(.scores[].assessed_cycle) = 137`, cheaper than ten
  clauses — recorded as note [372]); `last_assessed_cycle = 137`.
- **Two evidence-array appends**, both because this cycle's rationale rests on those sources:
  consistency-calibration-as-failure-mode gains src-0028, src-0029, src-0045, src-0046, src-0047
  (8 → 13); automated-triage-under-refusal gains src-0047 (3 → 4). No other array changed.

**The five-conjunct append-only proof, run and reported** (carry-forward [347]):

| conjunct | result |
|---|---|
| `.scores` keys equal | `true` |
| top-level keys equal (`_schema`, the `_cycleNNN_note`s, `last_assessed_cycle`, `scores`) | `true` |
| `_schema` unchanged | `true` |
| every new rationale `startswith` its old rationale | `true` |
| every new evidence array has the old one as a **prefix** | `true` |

**Score vector, old and new side by side** — `attribution-confident-wrong-gap 2→2`,
`attribution-expressed-confidence-unmeasured 2→2`, `automated-triage-under-refusal 2→2`,
`consistency-calibration-as-failure-mode 2→2`, `extraction-vs-reasoning-ordinal-axis 2→2`,
`incident-evidence-vendor-concentration 3→3`, `institutional-incident-real-world-impact 2→2`,
`ioc-extraction-reliability 2→2`, `task-dependent-reliability-framing 2→2`,
`ttp-attack-mapping-reliability 3→3`. **Total 22 → 22, max 3 → 3, n=10, `last_assessed_cycle` 132 → 137.**

**G1 hand-check:** every id in every evidence array exists in `state/knowledge/index.json` —
`unknown_evidence_ids=0`. No source was invented and none was added.

Also written: `state/queue/next_task.json` (T5 for cycle 138) and `state/queue/last_completed_task.txt`
(`T4 assess`).

## Next task rationale

The state machine's edge is **T4 → T5**, so cycle 138 is a **T5 (Select)**, `target_issue` null,
`attempt_count` 0. `collect_refresh_every` is 7 and 7 × 20 = 140, so the refresh rule does not fire
at 138 or 139 — it fires at 140.

The entry hands the T5 everything its ranking needs and tells it to re-derive all of it: the score
vector, the open-contradiction census by issue, every `attempts` array (flagging that
consistency-calibration took an attempt at 136 and so carries the only live 3(b) penalty at 138), and
every `depends_on` with the field-name trap spelled out. It carries the pairwise ctr-0067 ruling and
my own working of it, the non-terminating tie-break defect with the instruction to declare any
invented rung as unauthorised, and the full ban set and carry-forward list.

Honesty item (A) is put to it directly rather than left as a footnote. automated-triage-under-refusal
is a root, has never taken a 3(b) penalty, and at cycle 138 only **one** of the three score-2
`created_cycle`-2 roots has a recent attempt — so the condition under which it can finally win is
closer than usual. The entry tells the T5 to check that rather than assume it, to work the rungs
honestly in order in **either** direction, and to report which rung eliminated it if it loses a
fourth time. New carry-forward item (25) records that this issue now has the first defined piece of
work in its 137-cycle existence, so a T3 selecting it would not be starting cold.

## Budget

Rough estimate: **0 web searches, 0 fetches**, ~22 Bash/jq calls, 4 file reads, 3 writes, ~30 turns
of the 75 available. The whole cycle ran off the stored state and git history — G2 included, matching
cycles 135 and 136 at zero fetches.
