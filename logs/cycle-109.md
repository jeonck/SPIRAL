...
Topic: `cti-ai-reliability`. Model `claude-opus-5`, CLI `2.1.220`, per `config.yml`.

## Phase check

Run before anything else, because cycles 70, 83 and 106 each aborted and cost the chain
seven cycles by shifting T5 parity.

- `state/queue/last_completed_task.txt` → `T3 investigate`. So this cycle is a **T4**.
- `git show --stat HEAD` → cycle 108 touched `logs/cycle-108.md`, its transcript,
  `state/issues/graph.json`, `state/meta.json` and the two queue files. **It did not touch
  `state/assessments/scores.json`**, as a T3 may not.
- `scores.json` → `last=105 total=22 n=10`. Unchanged since cycle 105.
- `graph.json` → `ctr=56 open=43 last=ctr-0056 issues=10`.
- `index.json` → `srcs=40 last=src-0040`. Unchanged since cycle 104.
- `ioc-extraction-reliability` → `att=9,21,35,52,65,84,93,108 oq=13 cr=4`.

All four confirming checks matched the queue entry. Phase confirmed; I proceeded.

I also re-read `prompts/t4_assess.md` at source and `scripts/validate_state.py` lines
95–156 at source rather than trusting any account of them.

## Retrospection

**Target.** The round-2 G2 order is *"cycle N re-checks the conclusion the next unexamined
predecessor entered"*; 108 took cycle 98's, so I take **cycle 99's**. Verified by reading
`logs/cycle-099.md`: cycle 99 was the T1 that added `src-0033`–`src-0037`, so its entered
conclusions are those sources' `key_claims`.

`src-0034`, `src-0036` and `src-0037` carry standing citation bans (`ctr-0051`, `ctr-0053`,
`ctr-0056`), which I read at source before choosing. `src-0035` was already pair-rendered
and fully confirmed at cycle 104, so re-checking it would reproduce known work. I took
**`src-0033`** — the `if(is)` writeup — and specifically its **load-bearing negative
claim**, that the discovering institute's own page is a *re-post* and therefore **not** an
independent route to the ENISA footnote figure. That claim closed off `open_questions[2]`
of `institutional-incident-real-world-impact` after it had stood as "the most promising
untried route" for 98 cycles, and it is an **absence** claim — the exact class `ctr-0049`
found this tooling unreliable on. Good target.

**Method.** Two render families (direct and `r.jina.ai`), one query each, **no value
supplied in the query** per the standing discipline: list every numeral with its containing
sentence; quote verbatim any line attributing the story to another publication; list every
news outlet named anywhere; list every outbound URL.

**Result — PASS.** Both renders returned, unprompted and in agreement:

- the attribution line `Quelle: Der Standard (8. Januar 2026)` (the proxy render also
  returned the trailing `, Bild: tungnguyen0905`);
- **Der Standard as the only news outlet named anywhere on the page**;
- the `rund fünf Prozent` sentence;
- **no footnote count** and **no mention of Der Spiegel**.

`derStandard` is already held by this base as *non-independent* because it cites Der
Spiegel. So the institute's own domain relays the same single-outlet chain, the route
stays spent, and the 26/492 figure stays exactly where cycle 72 left it. **No contradiction
opened.**

**One scoping correction, recorded rather than hidden.** `src-0033`'s key claim reports the
two-digit numerator as *"ABSENT from the whole page"*. Read as a **literal string** check
that is **false** — the page carries the date `26. Januar 2026`. The claim is true on the
only reading it was ever meant to bear (the page states no footnote **count**), and both
renders confirm that reading. I did **not** open a contradiction, and I give my reason so a
successor may disagree with it: G3 asks for **two supported claims in conflict**, and there
is no second claim here — only a shorthand whose literal reading is looser than its intent.
A successor running a literal string check must not mistake the date for the figure.

**One methodological confirmation, free.** The two renders **agreed on every quotation** and
**disagreed on the enumeration total** (25-plus vs 6). That is `ctr-0051`'s rule holding
again on a fresh page: **presence is recoverable from this tooling, counts are not.**

Cost: 2 fetches.

## Task performed

T4. Scored **every** issue in the graph, set `assessed_cycle` to 109 on all ten entries and
`last_assessed_cycle` from 105 to 109.

### The vector did not move, and that is the honest outcome

**Eight issues at 2, two at 3** (`ttp-attack-mapping-reliability`,
`incident-evidence-vendor-concentration`), **total 22 over ten issues** — identical to cycle
105. Enumerated, not inherited; cycle 105's queue entry claimed "nine issues at 2", which is
arithmetically impossible at n=10, total=22, and I did not propagate it.

The reason is stated once and applies to all ten: the only cycles since the last assessment
were 106 (aborted, no state change), 107 (T5) and 108 (T3). **Cycle 108 added no source, no
measurement and no promotion** — `index.json` still ends at `src-0040`, unchanged since
cycle 104. There is no new evidence in the base on which any integer could honestly move.
Cycle 108's repairs make the existing record honest; they do not raise any issue's merit.

I did not invent separation. The live range is one bit across ten issues and the T5
tie-breaks have done 100% of selection work since cycle 73, but `prompts/t4_assess.md`
step 5 forbids inflating to fix that, and a fabricated integer is indistinguishable from an
honest one to every later cycle.

### The substantive finding: the repair I was sent to perform was done 73 cycles ago

The queue entry made `ctr-0010` **step (i)** its headline instruction — *"THE ONE REPAIR
ADDRESSED TO YOU SPECIFICALLY, AND NO OTHER TASK TYPE CAN DO IT"* — on the authority of
`ctr-0010`'s own CYCLE-108 UPDATE, which states that the step *"IS NOT"* discharged and that
the `ioc-extraction-reliability` rationale *"STILL CARRIES the mis-scoped 0.09–0.15
restatement"*.

**That is false. Cycle 36 discharged step (i) in full.** I read the rationale at source
rather than trusting the account of it, and it contains a marked block opening

> `[RESTATED AT CYCLE 36, DISCHARGING ctr-0010 STEP (i). The sentence replaced here read:`

which does all three things step (i) asks:

1. it quotes the superseded sentence verbatim and **preserves** it so the correction is
   auditable;
2. it states the scope was FALSE and why — `logs/cycle-018.md` derives the range under a
   heading about the recall at which each model's IoC F1 falls to its **own** derived TTP
   F1, so `src-0003`'s 97.6% is not an input to it anywhere;
3. it re-derives the entailment argument from scratch (`F1 = 2PR/(P+R)`, `dF1/dR > 0`, so
   `F1 ≤ 2P/(P+1)`; `2P/(P+1) ≥ 0.976` gives `1.024P ≥ 0.976`, i.e. **`P ≥ 0.953125`**) and
   states verbatim that **THE 0.09-0.15 FIGURE IS RETAINED AND REMAINS CITABLE IN ITS TRUE
   SCOPE**, listing all four crossovers against all four actual TTP recalls.

I obeyed step (iii) and **did not re-derive** the 0.09–0.15 arithmetic — I read it.

**So I made no substantive edit to the pre-cycle-109 text of that rationale.** There was
nothing left to replace, and writing a second correction over the first would have produced
two competing repairs of a sentence that no longer asserts anything. I recorded the finding
instead, as a CYCLE-109 UPDATE on `ctr-0010` and in the score rationale.

**The generalisable cause, and it is the second instance in two cycles running in opposite
directions.** Cycle 108 itself found that `ctr-0028` step (ii) had been *done* at cycle 66
and never propagated back, sitting listed-as-pending for 42 cycles. Cycle 108 then asserted
that *this* step, done at cycle 36, was outstanding. **The common cause is that a T3 may not
write `scores.json` and so does not read it** — a repair step addressed to a file the
reporting cycle cannot touch is a step nobody verifies. The rule this suggests (*no cycle may
report a repair step as UNDONE without reading the file the step names; reporting it as
UNVERIFIED is always available and costs nothing*) is **offered and not adopted** — adopting
a standing methodological rule is not a T4's charter. It is flagged for a T2 or a human,
alongside `ctr-0053` step (iii) and `ctr-0056` step (iii), unruled since cycles 101 and 104.

I left the impeached TTP string in the cycle-35-derived text exactly where cycle 66 marked
it. The correction is auditable only while the wording it corrects stays visible.

### A false generalisation in the queue entry, corrected

The queue entry states *"every issue has at least one open contradiction, so all cap at 3."*
**False.** A `jq` pass over `.contradictions[]` with `resolved_cycle == null` returns **nine**
distinct `issue_id`s. `incident-evidence-vendor-concentration` is absent — and absent from
the **unfiltered** list too, so it has never carried an entry at all. **Its ceiling is
`scale_max` = 5.** Cycles 102 and 105 recorded this correctly inside that issue's own entry;
it is the generalisation, repeated in queue entries, that is wrong. I again **left the
ceiling unused** and scored that issue 3 on merit, for cycle 102's reason: buying vector
ordering with an inflated integer would corrupt the instrument.

### Two closures

`ctr-0028` (opened 65) and `ctr-0038` (opened 79), both on `ioc-extraction-reliability`,
**closed at cycle 109** with `resolved_cycle = 109`. Each was closed at the **written
invitation of its own final paragraph**, which delegates the decision to a later T4 or T5.

- **`ctr-0038`** — all four steps discharged (i, ii, iii) or rejected at source (iv); its
  defective clause is marked in every location it names. I verified step (i)'s CYCLE-84
  CORRECTION block on `candidate_resolutions[1]` and steps (ii)/(iii) on `ctr-0001` and
  `ctr-0010` before closing.
- **`ctr-0028`** — steps (i)–(iii) discharged at cycles 65, 66/108 and 108. **I judged step
  (iv) a standing note, not an open step**, and gave the reason rather than asserting it: it
  observes that the single `issue_id` field cannot express a defect spanning two issues,
  which is a **schema limitation** already recorded as carry-forward [41]. No cycle is
  permitted to discharge it, and an entry cannot be held open forever by a step nobody may
  complete. I **re-flagged [41] as live** inside the closure note, because closing this entry
  removes the last place that limitation was tracked in the open set.

**I checked the ceiling arithmetic before closing, not after.** Closure changes a score only
if it **empties** an issue's open set. `ioc-extraction-reliability` still carries `ctr-0001`,
`ctr-0004`, `ctr-0010` and `ctr-0020`, so its ceiling is **unmoved at 3** and its merit of 2
sits under it with slack. **This is bookkeeping honesty, not a score change, and I do not
present it as one.** Graph-wide the open count falls **43 → 41**.

The **citation bans carried by both closed entries survive closure** and are restated in
every score rationale and in the next queue entry, because a cycle reading only *open*
contradictions would otherwise lose them.

### `automated-triage-under-refusal` — re-read on the rubric, held at 2

This is the one issue where a score change would have changed the selector's behaviour, so I
re-read its sole `candidate_resolution` at source before holding it.

Level 3 requires the primary candidate to be supported by ≥2 **independent** sources. The
candidate **disqualifies itself in its own words**: *"The claim therefore RESTS on src-0007;
src-0015 corroborates its direction … and is worth little as independent quantification."*
It records `src-0015` as the weakest-provenance source in this base — single-author preprint
filed `cs.AI`, no venue, no peer review, no affiliation, n=40 synthetic episodes per model,
no confidence intervals. And `ctr-0033` (open since cycle 73) shows one limb of the
independence argument is simply **false**: the two sources are not from "different years",
they are six weeks apart in the same year. Applying `ctr-0033`'s own rule — *score an
independence claim on its weakest verified limb, not on the number of ids in the evidence
array* — gives the same answer.

Five contradictions stand open against it (`ctr-0021`, `ctr-0033`, `ctr-0042`, `ctr-0049`,
`ctr-0055`), one of them filed against a **score rationale this file entered at cycle 94**.
And `attempts` is **still `[]`** — 93 cycles after creation, this issue has never once been
investigated by a T3.

**The honest score is 2. I left it at 2.** Cycle 107 proved the issue is unreachable by the
T5 selector until a score changes; a T4 is the only task type that could break that loop and
the integer is its only lever. **I refuse to pull it.** Writing an unsupported 1 or 3 would
buy reachability by falsifying the instrument — exactly the inflation step 5 bans — and
would leave a number no later cycle could distinguish from an honest one. **This is a
selector defect that needs a human ruling** (an exemption, a rotation rule, or a starvation
counter in the tie-break), not a fudge from me. This is the **eighth consecutive assessment**
to say so. What would move it honestly: a second well-provenanced measurement of
action-threshold permissiveness, replacing `src-0015` as the corroborating limb.

### G3 gate

**Ceiling applied, subtraction refused** — carry-forward [4], now in its **57th cycle**
awaiting a human. `scripts/validate_state.py` lines 144–156 build the open set as a **set
comprehension over `issue_id`** and enforce a **flat ceiling** of `scale_max − 
g3_contradiction_demotion` = 3; `prompts/t4_assess.md` step 3 states a **per-point
subtraction**. On this vector the readings diverge absurdly (`consistency-calibration` at six
open entries would read `5 − 12` floored to 0). I followed the validator, as every T4 since
cycle 33 has.

**No issue's recorded integer is below the merit I judge it to have, so the ceiling censors
nothing this cycle.** One place it *could* bite, recorded before it happens:
`ttp-attack-mapping-reliability` is the only issue whose **merit equals its ceiling** (3 and
3). A future T4 that judges its merit to be 4 **must report the censoring explicitly**, because
a silent 3 would be indistinguishable from this cycle's honest 3.

## Changes made

- `state/assessments/scores.json` — `last_assessed_cycle` 105 → **109**; `assessed_cycle` =
  109 on all ten entries; a `CYCLE 109 (T4) ASSESSMENT` block appended to all ten rationales
  (a shared header carrying the corrections, closures, surviving bans and G3 reasoning, plus
  a per-issue block); new top-level `_cycle109_note` following the existing
  `_cycleNNN_note` convention. **No score changed. No evidence array changed. No prior text
  deleted** — prefix-proved against `HEAD` per [221]: `keys_same=true scores_unchanged=true
  evidence_unchanged=true all_rationale_prefix=true old_topkeys_preserved=true`.
- `state/issues/graph.json` — `ctr-0028` and `ctr-0038` given `resolved_cycle = 109` with
  appended closure notes; `ctr-0010` given a CYCLE-109 UPDATE recording step (i) as
  discharged at cycle 36. Append-only proof passed: `issues_identical=true topkeys_same=true
  ctrlen_same=true all_desc_prefix=true ids_unchanged=true untouched_byte_identical=true`.
- `state/queue/next_task.json` — T5 (select) for cycle 110.
- `state/queue/last_completed_task.txt` — `T4 assess`.
- `logs/cycle-109.md` — this file.
- Scratch files (`scratch109.json`, `ioc_rat.txt`, `old_graph.json`, `old_scores.json`)
  deleted before finishing.

**No source was added and none was needed:** a T4 does not collect.

## Next task rationale

**T5 (select) at cycle 110**, per the state machine `T4 → T5`. `attempt_count` 0,
`target_issue` null — a T5 chooses its own target.

**Refresh rule, re-derived rather than inherited.** `collect_refresh_every: 7`, and step 4
fires only when the current cycle is divisible by 7. `110 % 7 == 5`, so **it does not fire**
and cycle 110's next task is a T3. On a three-cycle ladder a T5 lands on a multiple of 7 with
period 21; cycle 98 fired it and the **next candidate is cycle 119** (which is a T5 slot:
110, 113, 116, **119**).

I supplied my own ranking derivation **as a cross-check only**, with an instruction to
re-derive it. Under the settled pairwise reading of tie-break 3(a) (carry-forward [246]) the
eight issues at 2 reduce to three unbeaten survivors — `consistency-calibration-as-failure-mode`,
`institutional-incident-real-world-impact`, `automated-triage-under-refusal` — after
`ioc-extraction-reliability` takes a 3(b) penalty for its cycle-108 attempt (the only attempt
in the 105–109 window; `institutional-incident`'s cycle-104 attempt falls **outside** it). At
3(c) `automated-triage` (created 16) loses to the other two, **which are both `created_cycle`
2, and the policy then runs out.** I flagged that unbroken tie explicitly and left it for the
T5 to break, on the record, as a new carry-forward — deciding it here would be doing the next
cycle's work.

The entry is **15.9KB**, marginally over the ~15KB guideline of [238]; I record the overage
rather than trimming a phase check or a ban restatement to hide it.

## Budget

- Web fetches: **2** (both `internet-sicherheit.de`, direct + `r.jina.ai`, for G2).
- Web searches: **0**.
- Bash/tool turns: ~40 of `max_turns: 75`.
- Files written: 6 (2 state, 2 queue, 1 log, 1 scratch — scratch deleted).
- Wasted turns: **0**. `[227]`, `[237]`, `[211]/[228]` and `[299]` all held first time; the
  one non-obvious cost was that a plain `grep -n` on a dumped rationale returned a single
  unusable 45KB line, solved with `grep -o -n` and a fixed context width.
