# Cycle 115 — T4 (Assess) — every issue scored

## Task performed

**T4 (Assess)** per `prompts/t4_assess.md`, read at source rather than trusted from the queue
entry's account. `target_issue` is null because a T4 scores every issue.

**Phase check, first thing.** `state/queue/last_completed_task.txt` read `T3 investigate`;
`git show --stat HEAD` showed cycle 114 touching `graph.json`, `index.json`, the new
`src-0041.md`, both queue files, `meta.json` and its log — no `scores.json`, consistent with a T3.
All four confirming checks the queue entry nominated passed exactly:

| Check | Required | Observed | |
|---|---|---|---|
| `graph.json` | `ctr=61 open=46 issues=10` | identical | ✅ |
| `index.json` | `srcs=41 last=src-0041` | identical | ✅ |
| `scores.json` | `last=112 total=22` | identical | ✅ |
| `institutional-incident` attempts | `[12,59,72,90,104,114]` | identical | ✅ |
| open contradictions by `issue_id` | 10 distinct | 10 | ✅ |

Cycle 114 completed. I am the T4.

**What I did.** One G2 re-check (below, and it **passed**), then scored all ten issues, discharged
four pieces of named work owed to a T4, and adopted one bounded standing rule. **Zero fetches,
zero searches, zero sources added, zero contradictions opened or closed.**

### The result: no integer moved, but a ceiling did

The vector holds at **3, 3 and eight 2s, total 22** — unmoved since cycle 105. What changed is the
**ceiling vector**, and it is the one thing this cycle exists to record.

`ctr-0061`, opened at cycle 114, is the **first contradiction ever filed against
`incident-evidence-vendor-concentration`** — not merely the first open one; it had never carried an
entry at all. Its ceiling therefore falls from `scale_max` 5 to `5 − 2 = 3`. It scores 3, so it is
now **pinned exactly at its ceiling** and cannot rise while `ctr-0061` is open. Grouping the open
set by `issue_id` now returns **ten** distinct ids, so **for the first time in this project every
ceiling in the vector is 3** and the vector's maximum possible total is 30, down from 32.

**I could not run `scripts/validate_state.py`** — bash here refuses `python3` in every form — so the
gate is hand-checked by `jq` and I say so rather than imply a validator run.

### Cycle 105's instrument finding survives the ceiling change, on a substitute ground

Cycle 105 concluded that **no issue in the graph is held down by the G3 ceiling**. One of its three
grounds — vendor-concentration's uncapped 5 — is now obsolete. The conclusion nonetheless holds,
for a reason I derived rather than inherited: vendor-concentration's **merit** is 3 on the
unchanged why-not-4 reasoning, so its new ceiling **coincides** with its merit and does not bind,
exactly as `ttp-attack-mapping-reliability`'s has since cycle 105. **No issue's recorded integer is
below the merit I judge it to have.** Uncapping still buys nothing at any price; only evidence
moves these scores. Consequently the "cheapest issue to uncap" route that cycle 102 floated is now
**formally dead**, not merely unpersuasive — there is no longer a comparatively cheapest issue to
uncap, because they are all capped.

### Four pieces of named work, discharged

1. **`ctr-0058` step (i)** — appended to `institutional-incident-real-world-impact`'s rationale the
   corrected wording of the defective cycle-102 premise: `candidate_resolutions[3]` was the only
   candidate addressing institutional response **on the remedy dimension for published-document
   incidents**. Append only; not one character of the cycle-102 or cycle-105 text was edited.
2. **`ctr-0059` step (ii)** — appended that the carrier count is **three, not two**, and the
   earliest re-use is **cycle 103** (a T5 selection log), not cycle 105.
3. **`ctr-0058` step (ii) / `ctr-0059` step (i)** — recorded as swept with a negative finding.
   I did not re-run cycle 114's sweep, but I **did not take it on trust either**: I re-counted the
   phrase *only candidate* across all ten rationales by `jq`, and it returns institutional-incident
   2, attribution-confident-wrong-gap 1, automated-triage-under-refusal 1, zero elsewhere. Exactly
   three carriers; the other two **pass**, because both are *cardinality* claims for which an
   array-level check is the correct instrument. The negative result is recorded **in each cleared
   issue's own entry**, so it is findable from the issue rather than only from the contradiction.
   Neither entry is closed — their remaining steps are carry-forward adoptions.
4. **`ctr-0060` step (iii)** — the standing methodological question it reserved to a T4 or T2.

### The standing rule adopted (`ctr-0060` step (iii))

Should each single-source candidate's stated limits be checked against its source's Limitations
section? **Adopted, in bounded form only.**

- **(a) Prospective and mandatory** — any cycle that enters or extends a candidate resting on
  exactly one source must carry that source's Limitations into the candidate text, or say why it
  need not. Cheapest possible form: the cycle writing the candidate already has the source open.
- **(b) Retrospective and bounded** — the graph holds 49 candidates, **14 single-source, 12 of
  those supported** (`jq`, this cycle). A wholesale audit is a T3-sized job and I do **not**
  commission it; a T3 investigating an issue audits the single-source supported candidates **of
  that issue only**.
- **(c) What it is not** — not a licence to demote. Unstated limits are a *documentation* defect;
  a T4 prices the source's **actual** limits, not the candidate's silence about them.

### `ctr-0060`, priced honestly

`candidate_resolutions[3]` classifies PwC Middle East as **correct-in-place**, which asserts
**retention**, because the four remedy classes are distinguished from one another precisely by
removal versus retention. `src-0040` — the sole source — says in its own key_claim 2 and its
Limitations that the page states **neither** removal **nor** non-removal. The discriminating
dimension of the classification is the unverified one.

Consequence, stated narrowly: cycle 104's **rejection** of `open_questions[3]` predictor (a) is
**downgraded from established to provisional-pending-one-check**. It is *not* reversed and the
predictor is *not* reinstated. The **four-for-four common element** — no institution's internal QA
detected the problem, all four corrected only after an external party published — is **untouched**
and remains the candidate's strongest sentence. **I did not price `ctr-0060` as a demotion.**

This is a **new failure shape** and should not be filed with `ctr-0058`. `ctr-0058` is an
evidence-array check substituted for a candidate-text check. `ctr-0060` is a **layer-transfer
loss**: cycle 104 did not overlook the caveat, it *wrote* the caveat — into `src-0040.md` and
`index.json` — in the same cycle it omitted it from the candidate resting on that source. The
knowledge layer carried the limit and the issue layer did not, and the issue layer is what a T4
scores from and a T5 ranks from. The rule above is the response.

### `ctr-0061`, which cuts both ways, and my rationale says so

- **Against**: `src-0038.md`'s stored ruling that **51** is a mere search-tool artefact is **no
  longer safe** — 51 now has a candidate provenance in the vendor's own page, precisely the
  document `src-0038` is downstream of. No successor may cite that ruling as settling anything.
- **For**: the same fetch **discharged** `src-0038`'s self-charged provisionality on the
  4,841-of-5,290 denominator (now confirmed at the vendor on **both** render families) and
  confirmed the human-expert verification step at the vendor on both renders.

One fetch strengthened two of `src-0038`'s claims and unsettled one. **Net effect on the integer:
none** — nothing there touches `candidate_resolutions[0]` or `[3]`, which carry the 3.

**New ban: the figure 51 is not citable.** It is one-form (direct render has it; the `r.jina.ai`
render of the same page, asked unprompted, reported no such sentence), and neither render family
outranks the other. **53 remains citable** with its two-form provenance plus a pointer to
`ctr-0061`.

**`src-0041` added to one evidence array — and it is not a new route.** I added `src-0041` to
`incident-evidence-vendor-concentration`'s score-entry evidence (11 → 12 ids) because it is now a
document this base holds that bears directly on the issue's claims. It is **GPTZero**. Adding the
vendor's own report to an issue about vendor concentration **cannot relieve the independence bar by
construction**. The route count is still **three**. I did **not** add it to
`institutional-incident-real-world-impact`, where cycle 114 found it does not bear (EY, PwC and
ENISA absent; KPMG only as a related-article link title).

### The scores

| Issue | Score | Ceiling | Gate |
|---|---|---|---|
| ttp-attack-mapping-reliability | **3** | 3 | coincides |
| incident-evidence-vendor-concentration | **3** | 3 (**new**) | coincides |
| institutional-incident-real-world-impact | 2 | 3 | slack |
| ioc-extraction-reliability | 2 | 3 | slack |
| consistency-calibration-as-failure-mode | 2 | 3 | slack |
| task-dependent-reliability-framing | 2 | 3 | slack |
| attribution-confident-wrong-gap | 2 | 3 | slack |
| attribution-expressed-confidence-unmeasured | 2 | 3 | slack |
| extraction-vs-reasoning-ordinal-axis | 2 | 3 | slack |
| automated-triage-under-refusal | 2 | 3 | slack |

**Total 22.** `automated-triage-under-refusal` remains the only issue never investigated (`attempts`
empty, 99 cycles after creation) and remains **unreachable by the selector**. For the **third
consecutive assessment I refuse to move its integer to buy reachability** — that would falsify the
instrument, which is exactly the inflation `prompts/t4_assess.md` step 5 bans. **It needs a human
ruling**, not a fudge from me.

## Retrospection

**G2 target: the conclusion cycle 105 entered.** The round-2 order gave cycle 115 cycle 105's turn
(105 took 96's, 107 took 97's, … 114 took 104's). I verified by reading `logs/cycle-105.md` at
source rather than trusting the queue entry. Cycle 105 was a **T4**, so its entered conclusion is a
scored rationale block; its headline was **`ttp-attack-mapping-reliability`'s uncensored merit** —
a figure cycles 97, 102 and 103 each asked a later T4 for and none supplied.

**The live hazard, handled explicitly.** `ctr-0058` already impeaches the cycle-105 block on
*institutional-incident* (it inherited cycle 102's defective premise while declaring the objection
discharged). So my target was **already partly impeached**. I deliberately re-checked a
**different** conclusion of the same cycle so the pass would be informative rather than
duplicative, and I say plainly which is which.

**Re-checked at zero fetches. Result: PASSES, in full.**

| Limb cycle 105 asserted | Observed at cycle 115 | |
|---|---|---|
| six of six candidates `supported` | six of six | ✅ |
| primary `candidate_resolutions[5]` rests on five ids | `src-0023, src-0017, src-0007, src-0020, src-0021` | ✅ |
| `ctr-0045` step (i) never applied; candidate still reads *src-0007's authors are genuinely unaffiliated with CTIBench's* | sentence still there character-for-character; case-insensitive search of the whole candidate for `disjoint` returns **nothing** | ✅ |
| `ctr-0045` and `ctr-0047` both open | both `resolved_cycle: null` | ✅ |
| level 5 not in question — eight open questions | eight | ✅ |

Cycle 105 called the unapplied repair **nineteen cycles** old. At cycle 115 it is **twenty-nine**
(`ctr-0045` opened at cycle 86). **The defect has grown rather than closed** — which strengthens
cycle 105's level-4 refusal rather than weakening it. **No contradiction opened.**

**One limb of cycle 105's wider instrument finding is obsolete, and that is a *state change*, not a
re-verification failure.** Cycle 105 partly grounded "no issue is held down by the G3 ceiling" on
vendor-concentration carrying zero open contradictions. `ctr-0061`, opened **nine cycles later**,
capped it. **The ground was true when written**; I do not impeach it, and the conclusion survives on
the substitute ground given above. A successor must not read this as a failed G2.

**Honest summary of cycle 105:** *partly impeached* (its institutional-incident premise, by
`ctr-0058`, before I arrived) and *partly re-verified* (this issue's merit ruling, by me).

**Fetches spent on G2: zero.** Everything was derivable from `graph.json` and `logs/cycle-105.md`.

## Changes made

- **`state/assessments/scores.json`** — the only state file written. All ten entries got a cycle-115
  block **appended** (common block + issue-specific block) and `assessed_cycle: 115`;
  `last_assessed_cycle` → 115; new top-level `_cycle115_note`; `src-0041` appended to
  `incident-evidence-vendor-concentration`'s `evidence`. **No score changed.**
- **Append-only proof, run in one `jq` pass** against the pre-`mv` original: top-level keys gained
  only `_cycle115_note` and lost none; every score equal; every `evidence` array equal except the
  one deliberately extended; **every old rationale a `startswith` prefix of its new value**. Output:
  `newkey=_cycle115_note lostkey= last=115 total=22 cycles=115 prefix_ok=true scores_same=true ev_same=true`.
- **`state/queue/next_task.json`** — T5 (Select) for cycle 116, `attempt_count` 0, ~15 KB.
- **`state/queue/last_completed_task.txt`** — `T4 assess`.
- **`logs/cycle-115.md`** — this file.
- Scratch JSON written and **deleted** before finishing.
- **Not touched:** `graph.json`, `index.json`, `src-*.md`. My G2 passed, so a T4 that opens nothing
  writes only its own file.

## Next task rationale

The state machine is T4→T5, so cycle 116 is a **Select**. `target_issue` is null because a T5
chooses it. **116 % 7 = 4**, so it schedules a **T3**, not a T1; on an unbroken ladder the next T5
is at **119**, and **119 % 7 = 0**, so **the next T1 falls at cycle 119's selection**.

The entry carries forward, self-contained: the phase check with three `jq` confirmations; the full
score vector and every ranking input (`created_cycle`, `depends_on`, `attempts`, and the fact that
the 3(b) window at 116 is **111–115**, catching consistency-calibration and institutional-incident);
the **two ceiling traps** (the uncapping route is dead; the two 3s are still not equivalent but the
old distinguisher is wrong); the **selector defect** with an explicit instruction to run the policy
as written and **not** hand-pick automated-triage out of sympathy; its **G2 target** — cycle 106
aborted and entered nothing (no `logs/cycle-106.md`, verified by `ls`), so **116 takes cycle
107's**, re-derivable at zero fetches via `git show <commit>:state/...` exactly as cycle 113 did;
the standing citation bans including the new one on **51**; the new single-source-Limitations rule
to be carried into whatever T3 it schedules; and the bash/jq notes, including the `startswith`
rebinding trap I hit this cycle and its fix.

## Budget

- **Fetches: 0. Searches: 0.** Everything was derivable from the state and the logs.
- **Bash calls: ~26** (all `jq`/`sed`/`grep`/`mv`/`rm`, one command each). **Write calls: 3**
  (scratch, queue entry, this log). **Read calls: 3.**
- **Turns: well inside `max_turns` 75.** No turn was spent re-reading a file I had already read
  through `jq`.
- Cheapest wins this cycle: reading huge rationales with `jq … | tail -c N` instead of whole, and
  splicing **all ten** rationale appends in a **single** two-input `jq` call.
