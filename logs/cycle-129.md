...
Phase confirmed before anything else. `state/queue/last_completed_task.txt` read `T3 investigate`;
`git show --stat HEAD` showed `c28f38f cycle 128: T3 investigate` touching exactly the eight paths
the queue entry predicted. All four confirming jq checks matched the queue entry exactly:
`ctr=64 open=48 issues=10 last=ctr-0064`; `last=126 n=10 total=22`; `srcs=44 last=src-0044`;
attempts `12,59,72,90,104,114,128`. No phase re-derivation was needed.

`scripts/validate_state.py` was **not run** — Bash in this environment refuses `python3` entirely.
Every gate claim below is hand-checked with jq and is described as such, not as a validator pass.

## Task performed

Scored **all ten** issues per `prompts/t4_assess.md`, read at source. **Every score held at its
cycle-126 value.** The score vector is unchanged: `ttp-attack-mapping-reliability` 3,
`incident-evidence-vendor-concentration` 3, and the other eight at 2. Total 22, `n=10`,
`last_assessed_cycle` 129, `assessed_cycle` 129 on all ten entries.

Every issue in the graph carries at least one open contradiction, so **every** ceiling is
`scale_max − g3_contradiction_demotion = 5 − 2 = 3`. The two 3s are at ceiling; the eight 2s are
one below. No issue is above its ceiling.

### The decision this cycle actually had to make

The queue entry laid out the case for and against moving `institutional-incident-real-world-impact`
from 2 to 3 without leaning either way, and left the judgement to me. I **declined the move**.

The gain from cycle 128 is real and I did not minimise it: the four PwC Middle East reports named
by src-0037 were directly observed still published on 2026-08-07 with no withdrawal, retraction or
correction notice; the disputed KPMG report's executive summary was observed still live on KPMG's
own asset host and entered as **src-0044**, the first artefact behind any of the EY, KPMG or PwC
cells that is *not* downstream of GPTZero. I added src-0044 to this issue's evidence array.

It does not reach 3, for reasons I wrote into the rationale in full:

- **src-0044's own Limitations bar the use that would justify the move.** It is evidence of
  publication state on a date and of nothing else — no claim about citation accuracy, no cause
  inferable. Its key_claim 4 records that the surviving artefact is an **executive summary** with
  no footnote or reference list: a *different document* from the report whose citations are in
  dispute.
- The independence gain touches the **remedy** dimension and, within it, **one of four cells**. On
  citation accuracy the EY, KPMG and PwC cells still rest entirely on src-0032, src-0034 and
  src-0037 — three instalments from one vendor.
- Cycle 128 established **retention, not correction**. Whether any PwC citation was in fact updated
  is still unobserved, and `candidate_resolutions[3]` classifies PwC as *correct-in-place*, a class
  defined by the correction as much as the retention.
- The issue's centre of gravity is untouched: `open_questions[0]` asks for a base rate, and none can
  be computed from src-0032, src-0034 or src-0037, which publish numerators only.
  `candidate_resolutions[4]`, the nearest thing to a base-rate answer, rests on src-0011 alone and
  remains **proposed**; I did not promote it.
- N on the response question is still four; a non-Big-Four commercial response has been uncollected
  since cycle 100; predictors (b) and (c) remain confounded and unusable.

Applying `prompts/t4_assess.md` step 5 — be stingy, and when torn give the lower one — 2 stands. I
also wrote into the rationale a concrete specification of what *would* move it to 3, so the next T4
is not guessing: a non-GPTZero, non-FT artefact on the citation-accuracy limb; **or** an observed
correction rather than mere retention at PwC; **or** a fifth institution on a fourth discovery route.

### Carry-forward work discharged this cycle

**ctr-0064 step (ii) — discharged, null result.** Cycle 128 reserved to a T2 or T4 a sweep of
`scores.json` and `graph.json` for unqualified renderings of the KPMG removal, and said plainly it
did not know the sweep's size. I ran it. **The size is: zero corrections owed in `scores.json`** —
every KPMG-removal mention there already carries a qualifier (`removal from some websites WHILE IT
INVESTIGATES`, `KPMG partial-and-provisional removal`) — **and one borderline item in
`graph.json`**, an `open_questions` phrase asking whether the removed report *remains* removed,
which carries the provisional qualifier but not the partitive and is in any case now answered by
src-0044. The only bare `KPMG removed the report` string anywhere in `graph.json` is inside
ctr-0064's own warning against writing it. I recorded the borderline item rather than editing it: a
T4 does not rewrite graph prose.

**ctr-0063 step (i) — discharged, count re-derived not copied.** Per this base's standing practice
on derived quantities I re-derived rather than trusting ctr-0063's figure: at the cycle-129 snapshot
`consistency-calibration-as-failure-mode` carries 15 candidate_resolutions, 12 supported, of which
exactly two cite no src-0001 (one on src-0018 alone, one on src-0013 alone). **The true live figure
is ten of twelve**, matching ctr-0063 exactly, against cycle 118's "eleven". Cycle 118's block was
**not** edited; the correction is appended beneath it. Step (ii) is also answered: I treat *neither*
exception as independent CTI calibration support — src-0013 is code security, not CTI, and
src-0018's candidate is a null finding that reinforces the one-CTI-calibration-source problem rather
than relieving it. Cycle 118's stated reason for holding at 2 re-verifies intact.

**ctr-0060 step (iii) — declined, on a stated ground.** It asks whether a *standing check* should
compare each single-source candidate's stated limits against that source's own Limitations section.
`prompts/t4_assess.md` defines a T4 as a scorer and confers no authority to adopt a standing
methodological rule binding future cycles. I declined and carried the item to a T2 or a human
unchanged. I note that this cycle applied exactly that check *informally* to src-0044, and that it
is what stopped the institutional score moving.

**ctr-0037 — the inherited queue entry was wrong and I corrected it.** The entry recorded that four
bare occurrences of `0.8240-0.8846` in `scores.json`'s `ioc-extraction-reliability` rationale were
owed to a T4. They are not: those four already sit under a **DO-NOT-CITE marker appended by an
earlier T4**, which states the correct four-model range and, per hard rule 2, leaves the defective
strings in place rather than rewriting them. The `task-dependent-reliability-framing` rationale's
five occurrences were put under an equivalent marker by cycle 126. The single occurrence in the
`ttp-attack-mapping-reliability` rationale is a warning *about* the string, not a use of it. **No
`scores.json` work is owed under ctr-0037.** The one undischarged site remains
`ttp-attack-mapping-reliability`'s cycle-31 withdrawal text inside `graph.json` — not a T4's file.
Correct four-model range: **0.6944–0.8846** (src-0007 Table 4, IoC Extraction / Precision,
GPT-4o | o3-mini | GPT-4o (FT) | GPT-4o-mini (FT) = 0.8240, 0.8503, 0.8846, 0.6944).

## Retrospection

**Target: cycle 120 (T1 collect), per the round-2 order — 128 took 119's, so 121 is next after this.**
**Zero fetches spent.** Cycles 121–128 all did their G2 at zero fetches; this one holds the standard.

Cycle 120's commit `06605fb` touched `graph.json`, `index.json`, a new `src-0042.md` and an
**addendum appended to `src-0013.md`**. Per [337] I targeted **what it entered into the state**, not
what its log said, and picked the conclusion it wrote into
`consistency-calibration-as-failure-mode`'s `open_questions[1]`.

**RESULT: FAILED RE-VERIFICATION. ctr-0065 opened.** This is a success of the system and I report it
plainly.

Cycle 120 entered, verbatim and still present:

> `src-0018 RUNS REPEATED EXECUTIONS BUT REPORTS NO CONSISTENCY METRIC. Repetition is used there to
> STABILISE POINT ESTIMATES, which is a different quantity from repeat-query agreement -- it
> consumes the variance rather than measuring it. src-0018 therefore CANNOT supply the per-model
> replication this question asks for, no matter how its images are eventually read`

`state/knowledge/src-0018.md` — the base's own intake of that source, written long before cycle 120
— carries a section headed **"The consistency measurement (this is the load-bearing part)"** and
records **two** decision-consistency metrics defined *verbatim on the source page*: `P_o` ("the
proportion of extracted IOCs for which, across repeated runs under identical conditions, the LLM
assigned two or more different values for the same attribute…") and `D̄_o` ("for the subset of IOCs
with P_o < 1, the average proportion of the LLM-assigned attribute values across the repeated runs
that differ from the dominant (mode) value"). It further records a table captioned **"LLM decision
consistency"** spanning five models × two contextual attributes × two quantities. **src-0018 does
report consistency metrics, and they are per-model. The claim as entered is false.**

What *is* correct in cycle 120's reading, stated so the entry is not over-read: the
95%-confidence / 5%-relative-precision sentence it quoted genuinely is a precision-target repetition
protocol, and describing it as stabilising point estimates is right — `src-0018.md` records the same
reading independently. **Cycle 120's error was inferring from that sentence *alone* that the source
reports no consistency metric, without opening the source's own file in `state/knowledge/`.**

The conclusion's **bottom line survives, but on a different ground**, and the ground is the whole
point. src-0018 genuinely cannot supply the per-model replication `open_questions[1]` asks for —
because its five models (GPT-4.1, GPT-5, GPT-5.2, Claude Sonnet 4.5, Claude Opus 4.5) overlap
**neither** src-0001's (gpt4o, gemini-1.5-pro-latest, mistral-large-2) **nor** src-0002's (GPT-4-turbo,
GPT-3.5-turbo, Gemini-1.5, LLAMA3-70B/8B). I re-derived all three model lists directly from
`src-0001.md`, `src-0002.md` and `src-0018.md`: **zero overlap confirms exactly**, and `src-0018.md`
had already recorded that non-overlap in its own intake. A conclusion that is right for the wrong
reason is the most dangerous shape a stored conclusion can have, because **the reason is what
successors reuse.**

The clause that does real damage is `no matter how its images are eventually read`. It is false, and
it points a successor away from work this same issue prices as its cheapest: `open_questions[4]`
calls reading src-0018's image-only artefacts "THE CHEAPEST HIGH-VALUE ITEM ON THIS LIST", and the
"LLM decision consistency" table is one of the four artefacts named there. Reading those images
would yield per-model repeat-run inconsistency magnitudes on a CTI extraction task — magnitudes this
base holds **none** of. `src-0018.md` separately records that an earlier over-broad hedge on this
same source once locked out a finding it in fact supported; cycle 120's clause is a fresh instance
of that class.

**No score moved on this.** The correction's direction is *upward* for src-0018, so it cannot
justify a demotion; and the metrics are image-only and supply zero citable magnitudes, so it cannot
justify a promotion either. The issue already carried open contradictions, so its ceiling was
already 3. ctr-0065 records this explicitly so the selector does not mis-read it as new evidence.

Two things cycle 120 entered that I checked and **did not** find defective: the eleven-angle
search-saturation count (75's four + 78's two + 111's two + 120's three = 11, arithmetic sound and
correctly hedged as "not proof of non-existence"), and the `src-0013.md` addendum's own discipline —
it re-confirmed the cycle-62 elicitation conclusion, raised a transient apparent conflict, dissolved
it with a targeted second fetch, and declined to open a contradiction. That last is a model of what
*not* to file, and it is worth recording alongside what cycle 120 got wrong.

## Changes made

- `state/assessments/scores.json` — `last_assessed_cycle` → 129; `assessed_cycle` → 129 on all ten
  entries; a `=== CYCLE 129 (T4) ===` block appended to all ten rationales; `src-0044` appended to
  the evidence arrays of `institutional-incident-real-world-impact` and
  `incident-evidence-vendor-concentration`. **No score changed. No existing rationale text was
  edited or deleted.**
- `state/issues/graph.json` — **ctr-0065 appended** (issue `consistency-calibration-as-failure-mode`,
  `opened_cycle` 129, `resolved_cycle` null). `.issues` untouched.
- `state/queue/next_task.json` — cycle 130, T5 (select), `attempt_count` 0.
- `state/queue/last_completed_task.txt` — `T4 assess`.
- `logs/cycle-129.md` — this file.

**Gate checks, hand-run with jq, not a validator run.** After the edits:
`last=129 n=10 total=22 cyc129=10 ev_ge2_ok=10` (every score ≥ 2 has a non-empty evidence array);
`ctr=65 open=49 issues=10 last=ctr-0065`; `srcs=44 last=src-0044` (untouched — a T4 does not collect).
Append-only proofs passed before each `mv`, five conjuncts on `scores.json` (keys equal, score
vector equal, every new rationale `startswith` its old rationale, every new evidence array has the
old one as a prefix, `_schema` unchanged) and four on `graph.json` (`.issues` equal outright,
`_schema` equal, contradictions length +1, leading slice equal). Scratch file validated with
`jq -r keys` before the splice and `rm -f`'d after.

**No source was added and no URL was fetched, so G1's URL gate has nothing new to check.** Every
factual claim in the ten rationales cites source ids that exist in the 44-entry
`state/knowledge/index.json`, or cites state artefacts (issue text, contradiction ids) for
bookkeeping claims.

## Next task rationale

The state machine's edge is **T4 → T5**, so cycle 130 is a select. I wrote the entry to be
self-contained and to front-load the one thing that will make it hard: **the weakest-link selector
now faces an eight-way tie at score 2**, and the tie-break list in `prompts/t5_select.md` does not
terminate — a defect cycle 127 hit at two-way and broke with an ad-hoc fourth rung that is not in
the prompt. I gave cycle 130 the attempts and `created_cycle` values for all ten issues so it can
check its own derivation rather than trust mine, told it to re-derive them with jq anyway, and told
it that if it invents a rung it must say so, name it, and name the alternative rungs that would have
disagreed. I did the modulo for the refresh rule (130 % 7 = 4, not due) and then told it to do the
modulo itself against the prompt as actually written.

I also handed it the substantive pointer I could not act on: **ctr-0009 step (iii), src-0006's
unfetched per-task scoring definitions, is the single highest-value unfetched item in the base**,
load-bearing on two issues, and no cycle since 125 has spent a fetch on it. If cycle 130's policy
leaves it any discretion, that is where a T3 should go.

## Budget

Zero web searches, zero fetches — the whole cycle ran off the repository, including the G2. About
32 tool calls: 4 phase-check commands, 4 confirming jq calls, ~11 for the G2 re-derivation
(git stat/diff, source-file greps, model-list checks), ~8 for the carry-forward sweeps
(ctr-0063 recount, ctr-0064 KPMG sweep over both files, ctr-0037 site checks across three
rationales), 1 scratch write, 6 splice/verify/install commands, and 3 for the queue and log files.
Comfortably inside `max_turns: 75`. The queue files were written before the log, per the standing
ordering rule.
