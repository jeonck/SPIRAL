# Cycle 150 — T2 structure

Queue entry: `state/queue/next_task.json`, `task_type: T2`, `cycle_created: 148`,
`attempt_count: 1`, `target_issue: institutional-incident-real-world-impact`.
The entry was addressed to cycle 149; **cycle 149 aborted mid-cycle and its state was reverted**
(commit `ecd7f92`, `last_completed_task.txt` read `T2 aborted mid-cycle, rc=1 (attempt bumped)`).
Cycle 150 is the second attempt at the same entry.

## Phase check (run first, per the entry, and confirmed at source)

- `prompts/system.md` lines 45–46 read at source: `T1→T2, T2→T3, T3→T4, T4→T5, T5→T3`.
  Cycle 148 was a T1, so cycle 150 is a **T2** and the successor is a **T3**. The refresh rule
  keys on a T5's own cycle number and is not mine to compute.
- `git show --stat 8c1e465` (cycle 148): touches `state/knowledge/index.json`, the five new
  `src-0049.md`–`src-0053.md`, `state/meta.json`, both queue files and `logs/cycle-148.md`, plus the
  harness transcript. **`state/issues/graph.json` and `state/assessments/scores.json` untouched** —
  correct for a T1. `index.json` diff is `70 +` insertions and no deletions.
- `srcs=53 last=src-0053` ✔ · `ctr=70 open=52 issues=10 last=ctr-0070` ✔ ·
  `last=146 n=10 total=22 max=3` ✔. All three confirming checks matched the entry, so cycle 149's
  revert was clean and nothing of its work survived.

## Task performed

A T2 structure pass on `state/issues/graph.json`. The entry named **nine** candidate items and said
plainly that no cycle would do nine. I did **four completely** and left five, named below.

### (1) Executed ctr-0069's cycle-140 ruling — honesty item (F), ten cycles stale

This was the entry's highest priority and cycle 148's recommendation, and it was blocking two things
rather than one: the correction it was written for, and the stratum machinery the five new sources
need in order to enter cleanly. Cycles 141–149 could none of them execute it.

Appended to `institutional-incident-real-world-impact` `candidate_resolutions[3]` (nothing erased;
the corrected sentence is named verbatim so the correction cannot drift onto something else):

- **The stratum labels are now fixed in the graph.** **Stratum V, voluntary publication** — no
  tribunal, no duty of candour, no adversary: ENISA (`src-0004`, `src-0009`, `src-0010`, `src-0025`,
  `src-0026`, `src-0027`), EY Canada (`src-0012`, `src-0032`), KPMG (`src-0035`, `src-0044`), PwC
  (`src-0040`). **Stratum T, tribunal**: Sullivan and Cromwell LLP (`src-0048`), sole member.
- The sector-alignment sentence — verbatim `THE SPLIT IS 3 DECLINE / 1 VOLUNTEER AND IT LINES UP
  WITH SECTOR.` — is **observed within stratum V only and untested outside it**.
- **The tally is not restated as 3 decline / 2**, here or anywhere; the cycle-140 ruling forbids it.
- `candidate_resolutions[9]` is **not promoted** and still reads `proposed` (verified in the written
  file: `c9_status=proposed`).
- The commercial-exposure-versus-Big-Four-boilerplate confound is **not broken**; a **forum**
  confound is opened in its place and no source in this base tests forum.
- **ctr-0069 (7)(ii) executed in the same append**: the volunteer/decline axis gains a third value,
  **control-failure attribution**, `src-0048` its only member, carried with the ABSENT-for-cause and
  ABSENT-for-named-AI-tool findings so it can never be read as a stated root cause.

Also appended, on cycle 148's G2 recommendation, a note to `candidate_resolutions[9]` recording that
its paragraph (4) prescription — promote on admission — was **superseded**: cycle 140 ruled the case
in and expressly did not promote. No contradiction opened; G3 governs two supported claims in
factual conflict, and this is a superseded procedural prescription. I agree with cycle 148 on that
and recorded the disagreement route in case a successor takes the other view.

An execution record was appended to `ctr-0069` itself, stating what is and is not done. **I did not
close ctr-0069.** Under my own proposed convention (below) it now looks ripe, since (i) is ruled and
executed — but the cycle that executes a remedy should not also certify the execution. The next T2
or T3 on this issue should read the `candidate_resolutions[3]` append and, if it agrees, set
`resolved_cycle`. That is a one-step job and the cheapest closure on this issue.

I also recorded **ctr-0069 (7)(iii) as discharged *as a search commission and only as that***: cycle
148's four queries did surface two non-Big-Four published-report cases. I did **not** accept
(7)(iii)'s further claim that one such find settles (i) and (5) together — it does not, because the
two cases diverge as sharply as any pair in the population and MAHA's forum is mixed, not clean.

### (3) Closed the two ripe contradictions — honesty item (E), open since cycle 135

`ctr-0064` and `ctr-0066` both set `resolved_cycle: 150`. `open` falls **52 → 50**, which is the
overcount cycle 146 recorded as corrupting the census `scores.json` itself reports.

**I ran the one step in each that was genuinely undischarged, rather than inheriting it.**

- `ctr-0064` step (ii) (a sweep reserved to a T2 or T4; cycle 128 said plainly it had not run it and
  did not know its size): swept every `candidate_resolutions` summary, every `open_questions` entry
  and every contradiction description in `graph.json`, plus all ten `scores.json` rationales, for
  `removed the report from its websites`, `removed the report from all`, `removed it from its
  websites`. **Zero hits outside ctr-0064's own description of `src-0036`'s rendering.**
- `ctr-0066` step (ii) (cycle 131 likewise said its size was unknown): all ten rationales tested for
  `human-validation pipeline`, `human validation pipeline`, `regex+LLM+human` — **zero hits**. The
  three rationales mentioning both `97.6` and `human` were located; the `ioc-extraction-reliability`
  rationale itself records an earlier eighteen-hit sweep of its own `97.6` windows finding none that
  attribute the figure to a human-validation pipeline.

**Both sweeps are negatives on those patterns and not proofs of absence, and both entries say so.**

**The closure convention, proposed in writing inside ctr-0064 so a successor inherits a rule and not
a precedent:** a T2 may set `resolved_cycle` when and only when (a) every numbered repair step is
discharged, (b) the closing cycle names each step and says where its discharge is recorded, (c) any
step the closing cycle discharges itself is actually **run** and its result **reported, including
the limits of the check**, and (d) any standing citation rule or ban the entry carries is restated
as surviving the closure. A T3 or T4 may not close, because neither writes this file — the same
structural defect work-owed (32) records on `ctr-0010`. **A human may overrule this and should be
asked to.** I stated the asymmetry explicitly in the entry: a wrongly closed entry hides a live
dispute, which is worse than an overcount, so the convention is four-limbed and conjunctive and I
applied it to exactly the two entries honesty item (E) names as clean and to nothing else.

### (9) ctr-0010 note — work-owed (32), discharged

Appended a note recording that step (i) has been discharged since cycle 36, verified at cycle 109
(in `scores.json`), again by a later T4, and a third time by character offset at cycle 143. The
cycle-108 text asserting it undone is **not erased**. The repair was **not** re-done, and I make no
claim about the entry's remaining steps — it stays open.

### (17) work-owed — entered at last, unentered since cycle 126 (24 cycles)

Added one open question to `task-dependent-reliability-framing` and one to
`extraction-vs-reasoning-ordinal-axis` recording that **both rest on the unread `src-0006` matching
rule** — ctr-0009 step (iii). The edge already existed; what was missing was the record of what it
carries. The point worth having: the dependency is **shared**, so one fetch bears on both issues,
which makes it the highest-**leverage** fetch in the base and not merely the highest-value one.

**I changed no `depends_on` edge.** Honesty item (C) notes a T2 is the one role that can change the
structure rung 3(a) reads. I considered it and declined: rewiring the graph to engineer a selector
outcome would be illegitimate, and I found no merits objection to the existing edges. Per [386] the
coincidence that makes rung 3(a) computable as "is `depends_on` empty" still holds, because I did
not disturb it.

### (2)/(35)/(36) — posed as open questions, not ruled

Three open questions added to `institutional-incident-real-world-impact` (9 → 12) and one to
`incident-evidence-vendor-concentration`:

- **The four admission rulings the five new sources need**, kept separate rather than merged: does
  the population admit a **government department publishing a national AI policy** (`src-0049`,
  `src-0050`, `src-0053`)? Does it admit a case whose AI attribution is **external and inferential**
  (MAHA — and it must not enter any population defined as institution-attributed-to-AI, since both
  renders of `src-0051` returned ABSENT for that)? Does the axis need a **fourth** value for an
  explicit refusal to confirm or deny, and possibly a fifth for a **hedged** volunteered cause, since
  `src-0049`'s causal sentence is hedged and the hedge is load-bearing? Must the population separate
  **spokesperson-level from principal-level** responses, which `src-0051`/`src-0052` show can differ
  within one institution and across two forum cells? My recommendation is recorded as a
  recommendation — admit (i) to stratum V, do not admit MAHA to any AI-attributed population — and
  the full citation-constraint set travels with the question.
- **The forum dimension is at least three-valued** and I did not create the legislative-testimony
  cell, because building a stratum for an unadmitted case inverts the order. The sharper point,
  recorded because it survives either decision: with the sector-alignment observation now formally
  restricted to stratum V, **any finding ranging over the whole population is untestable until some
  stratum other than V has more than one member**, and stratum T has exactly one.
- **Work-owed (36) placement ruled**: the hedge-stripping event belongs to
  `incident-evidence-vendor-concentration`, recorded there with the pointer held here. It is about
  how evidence about incidents travels and degrades, not about publication or institutional
  response. Not a new issue — one observation on one pair of sources, and the graph is at ten with a
  5–12 healthy range.
- On `incident-evidence-vendor-concentration`: work-owed (29)'s route count, with the qualification
  that cycles 140/143/146 relied on — `src-0048`'s two carriers being one Reuters wire — **not**
  applying to `src-0051`, a newsroom doing its own checking. Counter-consideration stated: none of
  the three non-GPTZero routes has been read at its own artefact, so what has been added is three
  routes **attested by downstream carriers**, not three read at source. A T4 should take the score
  question up; a T2 may not, and I did not.

### On splitting institutional-incident-real-world-impact — declined, with a reason

`open_questions[6]` carries a ready-made split case, and cycle 148's five sources strengthen it.
**I did not split.** The reason, in one sentence for the next T2: the split would have to distribute
thirteen open contradictions and the stratum labelling across the halves, and doing that in the same
cycle that first created the stratum labelling would mean partitioning machinery whose own load has
never once been exercised — split it after at least one successor has used the labels.

### Left undone, named so nothing is silently dropped

(2) as a **ruling** (posed, not ruled); (4) the three pending standing checks — ctr-0060 step (iii),
cycle 136's `candidate_resolutions[18]` paragraph (5), ctr-0070 step (iii) — **none adopted,
reworded or dropped**; (5) work-owed (30), the T3-verbatim-strings-into-`src-*.md` rule, still
unruled at five instances and still the clearest-cut pending T2 item; (7) ctr-0062 step (iv), the
cross-reference into the two extraction issues, untaken by cycles 136–150; (8) ctr-0037 site (2),
the truncated `0.8240-0.8846` range still standing in `ttp-attack-mapping-reliability`'s cycle-31
withdrawal text; and work-owed (31)'s `state/knowledge/` half. **(5) and (8) are the cheapest of
these and either would fit a cycle that has other work to do.**

## Retrospection

**G2 target: cycle 140's entered conclusions** — the next unexamined predecessor in the round-2
order (147 took 138's, 148 took 139's). Cycle 140 was a T4, so its entered conclusions are the ten
rationales in `state/assessments/scores.json` at commit `c730fdf`, read with
`git show c730fdf:state/assessments/scores.json > old140.json`. **Zero fetches.** Scratch file
removed.

`last=140 n=10 total=22`, all ten `assessed_cycle: 140`. Verdict: **PASSES on every sharp question
the entry raised, and the one it most wanted answered comes back clean.**

- **Do the ten scores still re-derive?** Strong corroboration rather than a re-derivation from
  scratch: cycle 146 independently rewrote all ten rationales six cycles later and reached
  **identical scores on every issue** (3/2/2/2/2/2/2/2/2/3, total 22). An independent re-derivation
  landing on the same ten numbers is the best evidence available at zero fetches.
- **Does any rationale restate the tally as 3 decline / 2?** No. Two matches in the
  `institutional-incident-real-world-impact` rationale, both read in context: offset 148037 is
  `3 decline / 1 volunteer`, side A's own correct tally; offset 149604 is inside the **prohibition**
  — "the tally is not restated as 3/2 anywhere in the state and must not be."
- **Does any rationale call the commercial-versus-Big-Four confound broken without recording the
  forum confound?** No. Cycle 140's limb (c) reads: "IT DOES NOT REPORT THE
  COMMERCIAL-EXPOSURE-VERSUS-BIG-FOUR-BOILERPLATE CONFOUND AS BROKEN. It is not broken; a FORUM
  confound is opened in its place and no source in this base tests forum." The other two `broken`
  hits are unrelated: the `extraction-vs-reasoning-ordinal-axis` one, read in context, is about
  ctr-0013's construct ("THE ARITHMETIC IS CLEAN AND THE CONSTRUCT IS STILL BROKEN"). **I did not
  read the `automated-triage-under-refusal` hit**; its rationale tests negative for both `ctr-0069`
  and `forum`, and the confound claim cannot appear without one of them. That is an inference, and
  it is the one soft spot in this G2.
- **Did cycle 140 anywhere treat its own ruling as EXECUTED rather than ruled-but-pending?** **No —
  it is explicit in the opposite direction**, and this is the answer cycle 148 most wanted. Its limb
  (d) reads: "IT IS NOT EXECUTED… A T4 DOES NOT TOUCH graph.json. THE RULING IS RECORDED HERE AND IN
  `logs/cycle-140.md` AND IS OWED TO A T2, OR TO A T3 ON THIS ISSUE, TO EXECUTE. UNTIL IT IS
  EXECUTED, `candidate_resolutions[9]` STILL READS proposed IN THE GRAPH AND SIDE A STILL READS
  UNAMENDED, and a successor must check the graph rather than assume this ruling has landed."
  Cycle 140 wrote its own pending-ness, named the role owed, and told successors to check the graph
  rather than trust the ruling. Nine cycles then reported it pending, and every one of them was
  right. **No contradiction opened. No supersession found.**

That G2 was direct preparation for work item (1), exactly as the entry predicted, and I executed the
ruling in the same cycle that re-verified it.

## Changes made

- `state/issues/graph.json` — the **only** state file I edited, which is correct for a T2. I added
  no source and moved no score.
  - `institutional-incident-real-world-impact`: `candidate_resolutions[3].summary` extended;
    `candidate_resolutions[9].summary` extended; `open_questions` 9 → 12.
  - `incident-evidence-vendor-concentration`: `open_questions` 6 → 7.
  - `task-dependent-reliability-framing`: `open_questions` 6 → 7.
  - `extraction-vs-reasoning-ordinal-axis`: `open_questions` 8 → 9.
  - `ctr-0069`, `ctr-0010`: `description` extended, `resolved_cycle` untouched (`null`).
  - `ctr-0064`, `ctr-0066`: `description` extended, `resolved_cycle` `null` → `150`.
  - Counts after: `ctr=70 open=50 issues=10 last=ctr-0070`. No issue created, merged or renamed; ten
    issues, inside the 5–12 healthy range.
- **Append-only proof, run with two-input `jq` against the working tree copy before `mv`** (per
  [377]/[379]): `_schema` identical; `keys` identical; issue id order and contradiction id order
  identical; the six untouched issues byte-identical; the sixty-six untouched contradictions
  byte-identical; the four touched descriptions each `startswith` their old value; the two touched
  candidate summaries each `startswith` their old value; the other nine candidates on that issue
  byte-identical; every extended `open_questions` array equal to the old array on its prefix; and
  `id`/`issue_id`/`opened_cycle` unchanged on all four touched contradictions.
- **G1 hand-checked with `jq`, not with the validator.** `scripts/validate_state.py` cannot be run
  here (Bash refuses `python3`), and I am not implying a validator run. Every source id I cite —
  `src-0003`, `src-0004`, `src-0006`, `src-0007`, `src-0009`, `src-0010`, `src-0012`, `src-0025`,
  `src-0026`, `src-0027`, `src-0032`, `src-0035`, `src-0036`, `src-0040`, `src-0044`, `src-0048`,
  `src-0049`, `src-0050`, `src-0051`, `src-0052`, `src-0053` — lies inside the contiguous
  `src-0001`–`src-0053` range confirmed present in `index.json` this cycle (`srcs=53`). **I added no
  URL**, so `gates.g1_check_urls` has nothing new to resolve on my account.
- Every ban-set limb touching the text I wrote was honoured: the `src-0049` hedge is carried
  verbatim and never flattened; `src-0053`'s stripped version is labelled as the commentator's and
  never as the minister's; MAHA is recorded as **not** an AI-attributed incident; `src-0051`/
  `src-0052` are recorded as one outlet and one lead reporter; `src-0049`/`src-0050`/`src-0053` are
  recorded as one ministerial utterance; the `src-0048` control-failure-not-root-cause boundary is
  restated; the KPMG partitive rule, the `src-0036` ban and the `src-0003` 97.6%/Section-5.2 rule are
  restated as surviving their entries' closure.

## Next task rationale

**T3 investigate on `institutional-incident-real-world-impact`**, per the state machine read at
source (`T2→T3`), `attempt_count: 0`.

Why that issue rather than another: cycle 148 put five sources on it, cycle 150 built the stratum
machinery they need, and **the admission decision is now the single thing standing between a
T1's full source budget and any use of it**. Five sources have sat unentered for two cycles. The
successor inherits four separated rulings, a recommendation on two of them, the full citation
constraint set, and a one-step closure available on `ctr-0069` — a T3 on this issue may take it,
since ctr-0069's own (7)(i) names "a T2 **or a T3 on this issue**" as the roles owed the execution,
and the execution is now done and only needs certifying by a role that did not do it.

**On the size of the successor entry — I set out to shrink it and did not.** The entry I executed
was 68,612 characters; cycle 149 aborted on this same entry, and reading it cost me four tool calls
before any work began. I compressed the inherited narrative substantially and carried the ban set,
the work-owed ledger and the honesty items forward whole as required — **and the entry I wrote is
67,035 characters, a 2% reduction, which is not a reduction.** The compression was consumed by what
I added: a fourth admission ruling, the closure convention, work-owed (37), three new tool notes.
So [399] is recorded as a finding **against my own output as much as my predecessor's**: the ledger
grows monotonically because every cycle adds to it and none is authorised to prune it, and no
cycle-level discipline fixes that. **A human should rule on whether the ban set and work-owed list
can live in a versioned file the queue entry points at, instead of being copied into every entry.**
That is the structural fix; compressing prose is not.

## Budget

- **Searches: 0. Fetches: 0.** A T2 needs neither, and the G2 was done at zero fetches (cycles
  135–150 have now all managed that).
- **Turns: ~62 of 75.** Four of those went to paging the 68 KB queue entry before work could start.
- Reads: `prompts/t2_structure.md` and `prompts/system.md` at source, `config.yml`, `state/meta.json`,
  the queue files, targeted `jq` slices of `graph.json`, `scores.json` and `old140.json`. I did **not**
  read the five new `src-*.md` bodies — work item (2) is posed from the queue entry's own account of
  them plus their ban sets, and a successor ruling on admission **must read the five files
  themselves**. That is the honest limit on the admission recommendation I recorded.
- Not spent: no split of the largest issue; no adoption of the three pending standing checks; no
  ruling on work-owed (30); ctr-0062 (iv) and ctr-0037 site (2) untaken.
