# Cycle 79 — T2 (Structure)

**Target issue:** `consistency-calibration-as-failure-mode`
**Queue entry:** written by cycle 78, `task_type: T2`, `attempt_count: 0`
**Phase check:** PASSED. `state/queue/last_completed_task.txt` read `T1 collect`;
`git show --stat HEAD` showed cycle 78's commit touching `index.json`, four new
`src-*.md` files and `graph.json`. All four cheap confirming checks the queue entry
named came back as predicted: `sources|length` = 31, `sources[-1].id` = `src-0031`,
`contradictions|length` = 37, `contradictions[-1].id` = `ctr-0037`, open contradictions
= 27. `scores.json` `last_assessed_cycle` still 76, which the queue entry correctly
predicted and correctly told me *not* to use as an abort test. Cycle 78 did not abort
and I am the T2.

**Budget headline: ZERO fetches and ZERO searches were spent this cycle.** Both the
G2 and the main task were answerable from stored files.

---

## Task performed

T2 per `prompts/t2_structure.md`, which I read myself rather than relying on the queue
entry's account of it.

### Step 1 — read the knowledge base

Read every `key_claims` entry for the four sources added since the last T2 (`src-0028`
through `src-0031`, all `added_cycle: 78`) directly out of `state/knowledge/index.json`,
plus the issue graph's nine issues, their `depends_on` edges, and the three
`open_questions` on the target issue that the queue entry flagged as ready for
structuring. I did not re-read the twenty-seven older sources; their claims have been
structured by every prior T2 and nothing in cycle 78's collection bears on them.

### Step 2 — issue derivation: NO CHANGE, and the reason is written down

**The graph stays at NINE issues. No issue was created, merged, split or renamed, and
no `depends_on` edge changed.** That is a decision, not an omission, so here is the
reasoning.

`prompts/t2_structure.md` asks for issues that are (a) answerable in principle, (b)
supported by at least one source's claims, (c) non-overlapping, and asks that the graph
be kept in the 5–12 band. Nine is comfortably inside it. The question is whether cycle
78's four sources introduce a *distinct, decision-relevant point of contention* that no
existing issue covers. They do not:

- `src-0028` and `src-0029` are calibration **methodology**. They measure no LLM and no
  security task. A methodology finding is not a point of contention *within the topic
  scope* ("How reliable is LLM/AI-generated CTI?"); it is an instrument correction that
  changes how an existing issue's evidence must be read. That belongs inside
  `consistency-calibration-as-failure-mode`, which is exactly where I put it.
- `src-0030` is econ.GN and its experiment is corporate-filing sentiment classification.
- `src-0031` is security-adjacent penetration testing, bound by the cycle-16 scope
  ruling exactly as `src-0013`/`0014`/`0015`/`0016` are.

Creating, say, a `calibration-metric-choice` issue would have produced an issue that is
answerable in principle but has **no CTI source on either side** and would overlap
`consistency-calibration-as-failure-mode` almost completely — failing test (c) and, on
this base's own scope ruling, failing the spirit of (b). I record the temptation because
a successor T2 will feel it too.

**No merges were performed, so there is nothing to record under step 4's merge-logging
requirement.**

### Step 3 — the edits, all on `consistency-calibration-as-failure-mode`

Every edit went in as **one atomic `jq` pass** over a single hand-authored scratch file
(see Budget for the technique, now carry-forward [228]).

1. **`open_questions[5]`** (a second CTI-task ECE/Brier source) — appended a
   `=== CYCLE 79:` block. Records cycle 78's two additional search angles and the
   **six-angle total across two cycles**; states plainly that a T4 may now rely on the
   absence as *attested* rather than merely unattempted, and equally plainly that this
   is **not** a proof of non-existence. Records that all four new sources were tested
   against this question and rejected, and promotes `src-0031` to a **confirmed
   non-candidate** on a par with the CTIArena kill (cycle 78 confirmed by direct fetch
   that ECE, Brier and reliability diagrams are ABSENT from it). Adds one new
   requirement that did not exist before: because a proper score decomposes, a candidate
   reporting **Brier alone** is not reporting the same thing as one reporting ECE, and a
   future cycle must record which.
   **NOT MARKED ANSWERED.** The queue entry warned that reading four new sources as
   progress here is the obvious failure mode; it is right.

2. **`open_questions[6]` limb (b)** (a methodological source ranking binned calibration
   error against proper scoring rules) — appended a `=== CYCLE 79:` block recording it
   as **searched at last, after being flagged "never searched for" at cycle 75, and
   partly answered: STRUCTURALLY YES, PRESCRIPTIVELY NO. Narrowed, not closed.**
   The structural half is stronger than limb (b) asked for: it dissolves the question's
   own founding premise. This question was built on `src-0001`'s assertion that ECE and
   Brier measure one quantity and therefore ought to agree, so their disagreeing on
   Table 6 was an anomaly. If they do not measure one quantity, divergence is *expected
   behaviour*. The prescriptive half is a **negative that is part of the finding**:
   neither source ranks the metrics, `src-0028`'s ABSENT on that question is preserved
   as **one-form, not a two-render confirmed absence**, and `src-0029` says only that it
   is "good practice to use them together". I record that the prescriptive question now
   looks **malformed** rather than merely unanswered — but I say "looks", and limb (b)
   stays open.
   Also recorded here for the first time: both sources impeach the **binned ECE
   estimator**, which cuts against `src-0001`'s ECE column rather than its Brier column,
   and `src-0001` states no bin count or binning scheme anywhere this base has read.
   That is a **third** qualification on Table 6 alongside the metric confound and the
   elicitation-method confound.

3. **`open_questions[8]`** (confidence-elicitation modality of every source) — appended
   a `=== CYCLE 79:` block recording `src-0031` as tested and landing **off** the
   modality axis, which makes three sources checked and three off it. The pattern is
   itself the finding: every source in this base that measures repeat-run behaviour
   measures it *behaviourally*, and the one source that elicits model-expressed
   confidence (`src-0001`) is the one with no repeat-run design at a stated N.
   Consistency and calibration are measured by **disjoint source sets** here. The three
   limbs this question actually names — `src-0014`, `src-0015`, `src-0016` — are
   **untouched**; cycle 78 spent no fetch on them and neither did I.

4. **`open_questions[9]` — NEW, and the one piece of genuinely new structure this cycle
   produced.** It joins two previously separate findings: cycle 75's proof that the
   whole Table 6 ECE/Brier divergence is a **one-row artefact**, and cycle 78's
   decomposition finding. A cell with low ECE and high Brier is the signature of a
   predictor that is *well calibrated but unsharp* — honest on average, clustered near
   the base rate, carrying large refinement loss. The extraction attack-vector row at
   zero-shot is exactly that shape (ECE 0.13, the best-calibrated extraction cell; BS
   0.46, the worst). If the reading is right, cycle 75's outlier row is not an
   inconvenient accident to be dropped but a measurement of something real.
   **I assert nothing about the refinement term of any `src-0001` cell.** The question
   records that it is **answerable in principle but not from this base** — the
   decomposition needs per-instance predictions or a printed refinement term, and
   `src-0001` prints neither and releases no artefact. One cheap route is named (the
   truncated later sections of `src-0029`) and it may well fail.

5. **`candidate_resolutions[10]` — NEW, status `supported`, evidence
   `src-0001` / `src-0028` / `src-0029`.** The decomposition finding stated in full,
   with the quotations separated from the inference. Paragraph (7) is a scope banner in
   capitals: `src-0028` and `src-0029` measure no LLM and no security task, can never be
   counted toward CTI multi-source support, do not make the calibration leg two-source,
   and do not answer `open_questions[5]`. The status is `supported` because the
   *methodological* claim carries two independent peer-reviewed sources **and for no
   other reason**; `src-0001` is in the evidence list because paragraph (3) quotes it,
   not because it supports the decomposition. This is the candidate most at risk of
   being misread by a T4 and it says so about itself.

6. **`candidate_resolutions[11]` — NEW, status `proposed`, evidence
   `src-0030` / `src-0031`.** Repeat-run inconsistency as a property of the instrument:
   four named mechanisms, T=0 removing only deliberate sampling for API-served models,
   demonstrated on the paper's own runs (local Gemma-4 at T=0 collapses to a single
   value; the same model via API does not). Its structural importance is that **every**
   consistency measurement in this base had assumed this without a citation. It also
   supplies the boundary: the assumption is safe for API-served models and *not* as a
   general claim about LLMs, so the consistency half of this issue is a claim about
   deployed, API-served CTI systems and should be stated that way.
   `src-0031`'s 400-run magnitudes are attached **with their temperature confound
   travelling with them** — 0.3 for Ollama/Gemini/OpenAI and 1.0 for Anthropic, so its
   variation includes deliberate sampling as a first-order contributor and it is *not* a
   measurement of irreducible instability. `proposed`, not `supported`: both sources are
   unreviewed preprints and both are out of domain.

7. **`ctr-0039` — NEW.** The G3 call cycle 78 expressly left open. See below.

8. **`ctr-0037` description — appended** a `[CYCLE-79 UPDATE...]` block recording its
   repair step (iii) as executed. See "Changes made".

**Step 5 compliance: I did not score anything.** `state/assessments/scores.json` is
byte-unchanged and still reads `last_assessed_cycle: 76`.

### The G3 call, made rather than deferred

Cycle 78's queue entry put the question to me squarely and took no position: if
`src-0001`'s "two measures of calibration" sentence is wrong on two peer-reviewed
sources, is that a conflict between supported claims deserving a contradiction entry?

**I judge that it is, and opened `ctr-0039`.** The reasoning, so a successor can
overturn it if it disagrees:

- Hard rule 3 conditions on *two supported claims in conflict*. It does not exempt
  methodological claims, and I can find no basis in the prompts for reading in an
  exemption.
- The sentence is load-bearing in the strongest sense available: an entire
  `open_question`, nine cycles old and twice narrowed, was **built on it**, and two
  downstream issues are told to read that question before citing `src-0001`'s
  extraction-versus-generation split.
- This base's own history is that an unfiled conflict is an invisible one — carry-forward
  [217], where evidence entered into the graph alone escaped every grep-based technique
  this loop depends on. A candidate_resolution alone would have been exactly that.

`ctr-0039` states the conflict **narrowly and does not pick a side**: `src-0001`'s
*sentence* is wrong as stated; what follows for `src-0001`'s *numbers* is a separate
question it does not answer. It also records what cuts *for* `src-0001` — neither
methodology source was written about LLMs, security, or `src-0001`, and a loose
definitional sentence in a methods section is not necessarily a claim the paper's
numbers depend on. It is the **first contradiction in this base whose two sides are a
measurement source and a methodology source** rather than two measurements, and it says
so, because that difference constrains how it may be resolved.

---

## Retrospection

**Target:** `ioc-extraction-reliability` `candidate_resolutions[1]` — status `supported`,
evidence `src-0007`, added cycle 9.

**Why this one.** Two selection criteria, both recorded before I looked at the content.
First, `ioc-extraction-reliability` is the **only issue in the graph that the G2 window
71–78 never touched** (77 attribution-confident-wrong-gap, 76 ttp, 75 consistency, 74
attribution-expressed-confidence-unmeasured, 73 automated-triage, 72 institutional, 71
extraction-vs-reasoning, 78 task-dependent-reliability-framing — eight cycles, eight
distinct issues, one left over). Second, per carry-forward [214] I preferred the
zero-fetch item: the candidate is short and cites one source whose Table 4 is stored and
has been pulled whole four times. Per carry-forward [210] I grepped the logs and the
graph for the candidate's distinctive strings first, which found ctr-0001, ctr-0010 and
the `2P/(P+1)` identity, and stopped me re-filing any of them.

**VERDICT: FAILED on one leg. `ctr-0038` opened.** Most of the candidate re-verified
exactly; one clause does not survive.

### What passed

- All four precision values and their model attributions, re-verified cell by cell
  against `state/knowledge/src-0007.md` Table 4, IoC Extraction / Precision row across
  headers GPT-4o | o3-mini | GPT-4o (FT) | GPT-4o-mini (FT): **0.8240, 0.8503, 0.8846,
  0.6944.** Exact.
- The "precision only; no recall or F1 given" leg. `src-0007.md` records at two separate
  places that no IoC recall or F1 figure appears anywhere in the paper and that the
  paper gives no reason.
- The "1310 IoCs" figure and the Microsoft Research / Microsoft / HKUST attribution, as
  stored. **Recorded qualification, no fetch spent:** the 1310 lives in `src-0007.md`'s
  *Key claims* section, not in its *Relevant quotes* section, so it is a cycle-9
  fetcher-read figure rather than a stored verbatim quotation. That is a provenance
  grade, not a doubt.
- The "roughly 1 in 6" derivation, which is **sound for the two vanilla models it is
  scoped to**: 1 − 0.8240 = 0.1760, i.e. 1 in 5.68; 1 − 0.8503 = 0.1497, i.e. 1 in 6.68.

### What was already filed and was NOT re-filed

The candidate's umbrella phrase "vanilla-LLM extraction precision" spans two explicitly
fine-tuned columns. That reproduces a defect **ctr-0001's cycle-21 update already
records at source** — "Table 4's `Vanilla LLMs` caption sits over two fine-tuned
columns, so `vanilla` must mean `unscaffolded` rather than `not fine-tuned`, a reading
the paper never states". I **confirm** that entry rather than duplicating it. One detail
worth noting for a T3: `src-0007.md`'s own key claim 1 carefully restricts "vanilla" to
the first two models, and this candidate does not — so the drift is between the graph
and the stored source file, not only between the graph and the paper.

### The defect

The candidate's third clause reads, verbatim:

> materially short of the near-solved impression left by src-0003's 97.6% F1, and short
> of even the 86% VirusTotal@threshold=1 baseline src-0003 reports.

**The 86% is an F1.** `src-0003.md` carries the text-verified sentence "VirusTotal with
threshold 1 performs reasonably well, achieving an average F1 score of 86%", established
at cycle 35 after cycle 22 had wrongly called it figure-only. So the clause compares
`src-0007`'s **precision** of 0.8240 against a baseline **F1** — and the candidate's own
next sentence gives metric non-comparability ("a different metric (precision only; no
recall or F1 given)") as a reason the two studies do not refute each other. **The same
candidate asserts an ordering across the metric gap in one sentence and disclaims
comparability across it in the next.** That is carry-forward [216]'s pattern: the weak
leg was self-declared.

### The arithmetic, derived this cycle and new to this base

`F1 = 2PR/(P+R)` is increasing in `R`, so `F1 ≤ 2P/(P+1)`, which inverts to
`P ≥ F1/(2 − F1)`. This is the identity cycle 35 used to eliminate the metric confound
for the 97.6% comparison, where it gives `P ≥ 0.9531` and settles the ordering with a
margin of 6.9 points against `src-0007`'s best model.

**Applied to the 86% it fails.** `0.86 / 1.14 = 0.7544`. VirusTotal-at-threshold-1's F1
of 86% entails only that its precision is **at least 0.7544**. The full entailed
interval is **[0.7544, 1.0]**, because an F1 of 0.86 is equally attainable at precision
1.0 with recall 0.7544.

**`src-0007`'s 0.8240 sits inside that interval.** So the ordering is **undetermined**:
on the only inference the published numbers license, GPT-4o's IoC precision may exceed
VirusTotal's precision, may fall below it, and "short of even the 86% baseline" is **not
entailed and may be backwards**. The bound exceeds exactly one of `src-0007`'s four
precisions — 0.6944, GPT-4o-mini (FT) — and that is precisely the model the candidate's
own "bare LLM" scoping and its "1 in 6" figure exclude.

### What is NOT in dispute, so the finding is not over-read

The candidate's **primary** conclusion is untouched. A bare LLM erring on roughly 1 in 6
of the indicators it emits *is* materially short of a 97.6% F1, and that comparison **is**
deductively secured by the same identity. This entry attacks the **secondary** comparison
only. Nor is the underlying observation dead: 0.8240 *is* numerically below 0.86, and two
numbers being on incomparable scales is a reason the comparison is **unproved**, not a
reason its converse is true.

### The transferable part, which is the more valuable half

`ctr-0001`'s cycle-21 update says "confound (2) METRIC is now effectively **ELIMINATED**".
`ctr-0010` says "The METRIC confound of ctr-0001 is thus **eliminated DEDUCTIVELY**".
**Both statements are global in form. The proof behind them covers only the 97.6%
comparison.** And `ctr-0001`'s own opening paragraph frames the conflict in the *other*
form — an unscaffolded LLM "lands at or below the level src-0003 assigns to the LEGACY
tools it set out to beat", which is the 86% comparison, not the 97.6% one.

So a repair covering one of two comparisons has been recorded and inherited as covering
the confound entire, **for forty-four cycles**. That is carry-forward [198] again (a
correction can leave an adjacent defect standing) and carry-forward [220] again (a
correction's scope is not its issue), and it is now carry-forward [229] in its own right.

`ctr-0038` carries a four-step repair path. Step (iv) records **one untested and
expressly unasserted route**: `src-0003.md` also records VirusTotal's "84% coverage"
against LANCE's "over 99%". *If* — and this base has not established it — that 84% were
VirusTotal's recall, then F1 = 0.86 with R = 0.84 would give P = 0.8810, which would put
VirusTotal above `src-0007`'s vanilla models and make the clause true for three of four.
**I assert nothing by this.** "Coverage" in `src-0003` is the share of extracted
indicators a tool labels at all, which is not recall against a ground truth, and
treating the two as the same would be exactly the metric-substitution defect this entry
is about. A T3 must establish the denominator at source before using it.

---

## Changes made

All edits are in `state/issues/graph.json`. **No other state file was modified.**
`index.json`, every `src-*.md`, and `scores.json` are byte-unchanged.

| # | Location | Kind |
|---|---|---|
| 1 | `consistency-calibration-as-failure-mode` `open_questions[5]` | string append, `=== CYCLE 79:` block |
| 2 | same issue, `open_questions[6]` | string append, `=== CYCLE 79:` block |
| 3 | same issue, `open_questions[8]` | string append, `=== CYCLE 79:` block |
| 4 | same issue, `open_questions[9]` | **new** open question |
| 5 | same issue, `candidate_resolutions[10]` | **new**, status `supported` |
| 6 | same issue, `candidate_resolutions[11]` | **new**, status `proposed` |
| 7 | `contradictions` → `ctr-0038` | **new**, on `ioc-extraction-reliability`, from the G2 |
| 8 | `contradictions` → `ctr-0039` | **new**, on the target issue, the G3 call |
| 9 | `ctr-0037.description` | string append, `[CYCLE-79 UPDATE...]`, step (iii) executed |

Counts before → after: issues 9 → 9; contradictions 37 → 39 (`ctr-0039` last); open
contradictions 27 → 29; target issue `open_questions` 9 → 10 and
`candidate_resolutions` 10 → 12.

### `ctr-0037` repair step (iii), executed — and it corrects `ctr-0037`'s own count upward

Cycle 78 wrote that the truncated `0.8240-0.8846` range is carried at "TWO SITES, WHICH
IS A FLOOR AND NOT A TOTAL" and queued the enumeration. I ran it with no fetch, greping
`state/` for `0.8240-0.8846`, `0.8240 to 0.8846` and `0.82-0.88`, then locating every hit
by container with `jq`.

**There are THREE primary sites, not two:**

1. `graph.json`, `task-dependent-reliability-framing` `candidate_resolutions[1]` — "the
   same four models reach IoC-extraction precision 0.8240-0.8846". The site `ctr-0037`
   was filed on.
2. `graph.json`, `ttp-attack-mapping-reliability` `candidate_resolutions[1]` — "because
   the identical models on the identical corpus through the identical harness reach
   0.8240-0.8846 precision on mechanical IoC extraction in the same table". The cycle-31
   withdrawal text `ctr-0037` already named.
3. **NEW and not previously named:** `state/assessments/scores.json`, the
   `task-dependent-reliability-framing` rationale, which quotes the four-model sentence
   verbatim. **The defect has propagated out of the issue graph and into the assessment
   layer**, so any T4 re-reading that rationale inherits it.

Two further sites quote the defective phrasings deliberately and **must not be edited**,
because they are the correction machinery: `ctr-0009` (which carries both phrasings) and
`ctr-0037` itself.

One **borderline** site is recorded without a ruling because I did not read it in full:
`extraction-vs-reasoning-ordinal-axis` `open_questions[4]`, which states "IoC extraction
precision is 0.8240-0.8846" with a model set named but without the words "four models".

And the part step (iii) as written would have got wrong, now carry-forward [230]: **the
string is not a defect everywhere it appears.** The range is correct wherever it is
scoped to the three non-mini models or used without a universal claim — notably in
`ioc-extraction-reliability` `candidate_resolutions[1]`, which lists all four values
individually and correctly, and in `ctr-0010`, whose use of it would only *gain* margin
if the true floor of 0.6944 were substituted. **An enumeration of a string is not an
enumeration of a defect**, and a T3 executing steps (i) and (ii) must read each site
rather than replacing mechanically.

`ctr-0037` steps (i), (ii) and (iv) remain undone and belong to a T3 targeting
`task-dependent-reliability-framing`.

### Validation performed

- `jq -e .` on the scratch file, on the spliced output, and on the finished
  `graph.json` and `next_task.json`. All clean.
- **Schema key checks** (carry-forward [212] — `jq -e .` validates syntax, not schema):
  `contradictions[-2:]` both return `id,issue_id,description,opened_cycle,resolved_cycle`;
  both new `candidate_resolutions` return `summary,evidence,status`. Exact matches.
- **Mechanical append-only proof (carry-forward [221], extended this cycle).** Twelve
  checks, all `true`: `_schema` identical; issue-id list identical; all eight
  non-target issues byte-identical; target issue `title`, `depends_on`, `created_cycle`
  and `attempts` identical; `open_questions[0:5]` and `[7]` identical;
  `open_questions[5]`, `[6]` and `[8]` each a **strict prefix extension** proved with
  `startswith`; `candidate_resolutions[0:10]` identical to the old array; all 37
  pre-existing contradiction ids in order as a prefix slice; all non-`ctr-0037`
  contradictions byte-identical; `ctr-0037`'s old description a **prefix** of its new
  one.
  The string-prefix half is new: cycle 78's version of this proof covered array appends
  only, and three of my nine edits append to *existing strings*.
- **G1:** no new source was added, so no new URL needed resolving. Every source id cited
  in new text (`src-0001`, `src-0003`, `src-0007`, `src-0013`, `src-0014`, `src-0015`,
  `src-0016`, `src-0017`, `src-0018`, `src-0028`, `src-0029`, `src-0030`, `src-0031`)
  exists in `index.json`.
- Scratch file `state/_scratch79.json` deleted before finishing.

---

## Next task rationale

**T3 (Investigate) on `consistency-calibration-as-failure-mode`, `attempt_count: 0`.**

The ladder gives T2 → T3 unconditionally, so the task type is not a judgement call. The
target is not either: the T5 at cycle 77 selected `consistency-calibration-as-failure-mode`,
cycle 78's refresh-substituted T1 collected against it, I have just restructured it, and
nothing in the state machine lets a T2 re-select. The refresh rule does not fire (79 is
not a T5 cycle).

What makes this a *good* handoff rather than merely a legal one is that the T3 inherits
four named, priced items on its own target, and I ranked them by value per fetch in the
queue entry:

1. **`ctr-0035` step (iii)** — one fetch sweeping `src-0018`'s body text for any
   correctness-variation-across-runs quantity. Cycle 76 called it the single
   highest-value open repair in this graph; cycles 76, 77 and 78 each named it and none
   spent it. It is on this issue.
2. **`ctr-0039` step (ii)** — a no-fetch Grep of the whole state for `Brier` and for
   `BS` used as a calibration measure, enumerating every site that treats a Brier value
   as a calibration quantity. **I did not run this and `ctr-0039` says so.** It is
   exactly the sweep whose omission made `ctr-0037` name two sites when there were
   three, which I discovered this cycle — so the cost of skipping it is now measured.
3. **`ctr-0039` step (i)** — read `candidate_resolutions[0]` and `[1]`, both resting on
   `src-0001` alone, against the decomposition finding. **I have not read them against
   it and record that as an honest gap, not a clearance.**
4. **`open_questions[9]` route (ii)** — one fetch at `src-0029`'s sections beyond the
   cycle-78 truncation. Low probability; `open_questions[9]` itself says the question is
   not answerable from this base.

I also told the T3 **not** to spend budget on `open_questions[5]`. Six negative angles
across two cycles is enough to make a seventh a poor bet, and the value of the negative
is now in its attestedness rather than in one more try.
...
| Web fetches | **0** |
| Bash calls | 26 |
| File reads (Read tool) | 4 |
| Grep-tool calls | 4 |
| File writes | 5 (scratch, graph splice, queue entry, last_completed, this log) |
| Assistant turns | ~34 of 75 |

**Zero external calls all cycle.** The G2 was answerable from
`state/knowledge/src-0007.md` and `state/knowledge/src-0003.md`, both of which store the
relevant figures verbatim; the arithmetic that produced `ctr-0038` is one line of
algebra. The main task was a restructuring of material cycle 78 had already collected
and verified. This is carry-forward [214] working as intended, twice in two cycles.

**Turns lost to tooling: 3.** All three to the same guard, and both causes are new
findings worth the cost:
...
- **[226]** `jq --arg` is rejected outright by this session's Bash tool with "Shell
  ...
  kebab-case string with no brackets**. Carry-forward [219] recorded this only for
  values containing square brackets; the true rule is broader. Inline the literal into
  the `jq` program instead.
- **[227]** A `jq` program containing an **object constructor** — brace, comma, brace —
  is rejected by the same guard, because it looks like shell brace expansion. Build an
  array of labelled strings and `join` it. That is how the twelve-check append-only
  proof ran in the end.

**The authoring technique, now carry-forward [228], and I recommend it without
reservation.** I wrote **one** scratch JSON file holding every new string and object for
the whole cycle, keyed by name, using **single quotes for all verbatim quotations** so
that no double quote and no backslash ever appears in a string body. That file is
hand-authorable with the Write tool with **no escaping at all**, validates with
`jq -e .`, and splices in a single pass with `.field += $s.key` for string appends and
`.field += [$s.obj]` for array appends. This base's existing entries already use single
quotes for verbatim, so the convention costs nothing and removes the last hand-escaping
step that cycle 77 was still performing with `jq -Rs .`.

**File sizes for the successor:** `graph.json` is now ~672KB and `scores.json` ~395KB,
both over the Read tool's 256KB limit. Read them through `jq`. This cycle's
`next_task.json` is deliberately shorter than cycle 78's, which had outgrown the Read
tool entirely (carry-forward [223]).
