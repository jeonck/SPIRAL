# Cycle 136 — T3 Investigate — `consistency-calibration-as-failure-mode`

Phase confirmed before anything else. `state/queue/last_completed_task.txt` read `T2 structure`.
`git show --stat HEAD` gave `3630426 cycle 135: T2 structure` touching `state/meta.json`,
`state/issues/graph.json`, both `state/queue` files, `logs/cycle-135.md` and its transcript —
**no `state/knowledge/` file and no `state/assessments/scores.json`**, exactly as a T2 must.
All four confirming jq checks reproduced their required values: `ctr=68 open=52 issues=10
last=ctr-0068`; `ccafm=18 triage=2`; `srcs=47 last=src-0047`; `last=132 n=10 total=22 max=3`.
The queue entry's account of the phase and the state agree, so I proceeded as T3.

I read `prompts/t3_investigate.md` at source rather than trusting the queue entry's summary of it.

---

## Retrospection

**Target: cycle 127, per the round-2 order.** 116 took 107's, 117–124 took 108's–115's, 125–135
took 116's–126's, so cycle 127 is the next unexamined predecessor. Cycle 127 was a **T5**
(commit `ec734db`, stat confirms `state/queue/next_task.json`, `state/issues/graph.json`,
`state/meta.json`, `logs/cycle-127.md` — a T5 shape), so the conclusion it *entered into the
state* is a **selection**: `state/queue/next_task.json` went from `T5`/`null` to
**`T3` targeting `institutional-incident-real-world-impact`**.

**Zero fetches.** I re-derived the ranking from the graph and scores as they stood at `ec734db`
(`git show ec734db:state/...`), applying `prompts/t5_select.md` mechanically, *before* reading
cycle 127's own table — so the reproduction is independent, not a transcription.

| step | my re-derivation from `ec734db` |
|---|---|
| 1. candidates | all 10 issues (all score < 5) |
| 2. base priority | eight issues tie at score 2; `ttp-attack-mapping-reliability` and `incident-evidence-vendor-concentration` are 3 and drop out |
| 3(a) upstream-first | within the score-2 tier, four have empty `depends_on` and are maximal: `ioc-extraction-reliability`, `consistency-calibration-as-failure-mode`, `institutional-incident-real-world-impact`, `automated-triage-under-refusal`. The other four each list an upstream inside the tier |
| 3(b) attempt penalty, window 122–126 | fires on `consistency-calibration-as-failure-mode` only (attempt at 122 → effective 3). `task-dependent-reliability-framing` also takes +1 (attempt at 125) but was already out at 3(a) |
| 3(c) older `created_cycle` | `automated-triage-under-refusal` out on 16 against 2 |
| residue | **two-way tie**: `institutional-incident-real-world-impact` vs `ioc-extraction-reliability`, both score 2, both maximal, both unpenalised, both `created_cycle` 2 |

**Verdict: VERIFIED, with an already-known defect reproduced.** My table matches cycle 127's
line for line, including the 122–126 window choice and the identification of exactly one
outcome-relevant 3(b) penalty. The written policy **terminates without deciding**, and cycle 127
broke the tie with a declared ad-hoc fourth rung (least-recently-attempted: last attempt 114 vs
117 → `institutional-incident-real-world-impact`). That fourth rung has **no authority in
`prompts/t5_select.md`**. Cycle 127 said so itself and flagged it for a human.

This is the **already-known** defect carried by `ctr-0067` and standing honesty item (B). Per the
queue entry's instruction I did **not** open a duplicate contradiction. What I add is that the
defect is now *reproduced by an independent cycle* rather than merely self-reported: the tie is
genuinely unbreakable under the written rules, not an artefact of cycle 127's arithmetic.

Two further findings worth recording:

- **Robustness of the outcome.** I tested the other plausible 3(b) window (123–127, which would
  drop `consistency-calibration-as-failure-mode`'s attempt at 122 out of scope). That yields a
  *three*-way residual tie instead of two-way, but the same ad-hoc rung still selects
  `institutional-incident-real-world-impact` (last attempts 122 / 117 / 114). **The selection is
  invariant to the window ambiguity**; only the size of the tie changes.
- **No past selection is impeached by `ctr-0067`'s cycle-135 ruling.** Cycle 127 applied 3(a) in
  the stricter "empty `depends_on` outranks any dependent" form; cycle 135 ruled the correct
  reading is pairwise-maximal within the tier. On cycle 127's data **both readings give the same
  four survivors**, so the ruling changes nothing retrospectively here. Carried as item (14).

Not a failed re-verification, not a supersession: the conclusion stands, and the process gap
under it was correctly self-reported at the time.

---

## Task performed

The queue's agenda item (1) — `ctr-0068` steps (6)(i) and (6)(ii) — was the reason this cycle
existed, and it consumed the cycle. Four fetches, two sources, two render families each, every
query phrased to **ask for values without supplying them** so a render could not echo a number
back at me.

### Step (6)(i) — is there a quantified domain-fine-tuning ECE comparison in src-0045?

Fetched `arxiv.org/html/2505.01997v2` **and** `r.jina.ai/https://arxiv.org/pdf/2505.01997`.
**The two families agreed on every limb.**

Section 5.1 is titled **"Approximating the Accuracy Loss for LLMs"**. It diagnoses preference
collapse under RLHF/DPO and *proposes* fine-tuning on a domain-specific question-answering
dataset via two supervised losses. **It contains no numbers.** Both renders returned **ABSENT**
for any quantified before/after calibration comparison attributable to domain-knowledge
fine-tuning as a standalone intervention — not only in Section 5.1 but anywhere in the paper.

So the branch of step (6)(i) that would have forced a rewrite and re-weighting **did not fire**:
`ctr-0068` was never a two-measurement conflict.

**But I did not stop at the queue entry's suggested wording of "unsupported authorial aside",
because that would be an over-read.** The abstract's domain-knowledge limb *is* operationalised
in the paper — as the authors' own CFT/RCFT methods, whose recovery **is** tabulated (the html
render names Table 2 as comparing DPO/RLHF, Temperature Scaling, Label Smoothing, CFT and RCFT;
the 2.39%–6.51% figures this base already holds come from that family). What is genuinely absent
is an **ablation isolating plain domain-knowledge SFT from the calibration-specific machinery
bundled with it**.

That changes the closure argument in a way I think is more honest and more durable:

> The two sides are **not the same intervention**. src-0001 is plain supervised task fine-tuning
> of a **base** model on **free-form generation**. src-0045's domain-knowledge limb is
> calibration-targeted fine-tuning of an **already-alignment-degraded** model on **four-option
> multiple choice**. A *direction* conflict does not survive that.

`ctr-0068` **resolved, `resolved_cycle` 136.** Scope difference (b) of its paragraph (4) is
thereby confirmed rather than conjectured; (a) and (c) stand untouched. The reconciling
hypothesis in paragraph (4) **remains a hypothesis** and I have re-stated in two places that no
cycle may assert it. No score moves on the closure. **The citation hold on src-0045's
domain-fine-tuning sentence is not lifted — it is hardened**: that sentence has now been checked
for quantified backing at both render families and none exists.

### Step (6)(ii) — the binning, which is standing honesty item (D)

This is the deeper obstacle and it moved.

**src-0045: ten equal-width bins.** Bin count verbatim, *"We use their discrete versions by
stratifying all the samples into ten bins according to the prediction probability"*; scheme
verbatim, *"we discretize q in [0,1] into M equal-width bins: [0,1/M],…,[(M−1)/M,1]"*. The
r.jina.ai PDF render independently returned the same ten-bins sentence and the same interval
form.

I then judged a src-0001 fetch cheap enough to take (the queue entry invited it) and spent two
more on `arxiv.org/html/2503.23175v1` and `r.jina.ai/https://arxiv.org/pdf/2503.23175`.

**src-0001: no bin count, no binning scheme, no library — ABSENT at both renders.** The html
render adds that src-0001 cites Naeini et al. on Bayesian binning *without stating its own
implementation parameters*. A citation is not a parameter and must not be read as one.

**src-0001's binning is therefore not merely unrecorded in this base but irrecoverable from the
published text.** That is a real result, and it is a *negative* one confirmed two-render. I have
written "do not spend another fetch on it" into the graph, the queue and the carry-forward.

**Unsought corroboration, picked up in the same pass:** both renders reproduced src-0001's
confidence construction verbatim — *"we implement calibration analysis by extracting the log
probabilities that the LLM assigns to the tokens composing the generated response"* — with the
per-entity procedure of isolating a JSON section and multiplying its token probabilities. This
independently re-confirms the **token-probability** elicitation modality this base has recorded
for src-0001 since cycle 62, at a render family and a cycle with no stake in the answer.

### What this does to honesty item (D)

**The obstacle is now one-sided and located, not symmetric and unknown.** src-0045's ECE is a
ten-bin equal-width estimate and is therefore, per src-0028 and src-0029, bin-count-dependent and
a *lower bound* on the true calibration error. src-0001's has no recoverable binning at all. The
gap between the two **cannot even be signed** — I cannot say which understates more.
Cross-source ECE comparison for the src-0001/src-0045 pair is now known to be **permanently**
uninterpretable from the published texts. src-0013 is the last of the three whose binning a fetch
could still supply.

I want to be exact about the limit, because it would be easy to inflate this into something it is
not: **it impeaches no number.** src-0001's Table 6 values are as good as they ever were *within*
src-0001, and its 7-of-9 fine-tuning-worsens-ECE finding is a within-source, within-metric
comparison — cycle 80 identified that property and it is precisely what makes that finding immune
here. The prohibition is on cross-source ECE arithmetic and on nothing else.

### `ctr-0065` closed

Agenda item (4). Steps (i) and (ii) were discharged at cycle 135, step (iii) is a standing
caution rather than an action, so the entry had no outstanding work; cycle 135 left it open only
on the convention scruple in honesty item (E). **My queue entry explicitly authorised the
closure and I took it on that authority, not on a self-asserted one.** `resolved_cycle` 136.
Two things do not follow, and I wrote both into the entry: step (iii)'s caution **survives** the
closure and remains binding wherever src-0018 is cited; and **one authorised closure is not a
convention** — honesty item (E) is unsettled and `ctr-0062` is still held open on the same
scruple across fifteen cycles.

### What I did not do, and why

- **Agenda item (2), read src-0003 end to end.** Item (1) did not finish cheaply — it took four
  fetches — so item (2) was conditional and I did not take it. Eleventh cycle it has been owed.
  Carried, and I have said plainly in the next queue entry that cycle 136 had budget and spent it
  elsewhere on purpose.
- **Agenda item (3), `ctr-0062` step (iv).** Not taken. Carried as item (19) with its
  keep-`ctr-0001`-apart warning intact.
- **Agenda item (6), open_questions[8].** I checked it before deciding, and **the queue entry's
  premise is stale in one direction and right in another.** `candidate_resolutions[13]` answered
  it at cycle 96, but reading the question's own tail shows **scope limit (i) stands and Job A is
  not discharged**: src-0014's modality strings have returned ABSENT at the /html render *four
  times*, the untried forms are the r.jina.ai PDF and ar5iv, and `ctr-0057` must be read before
  quoting anything from cycle 96's list. So it is neither "closed" nor "a zero-fetch synthesis
  available now" — it needs fetches at untried render families. Recorded as carry-forward (22) so
  the next cycle inherits the corrected picture rather than the queue entry's version.
- **Items (7), (9) — the T2-authority items.** Carried, not taken. A T3 has no authority to adopt
  a standing methodological rule. My new `candidate_resolutions[18]` *proposes* one (record
  binning whenever an ECE source is fetched) and says explicitly that adoption is a T2's.
- **Item (8), `ctr-0037` site (2)** and **item (10)**: untouched, carried.

---

## Changes made

`state/issues/graph.json` only. No source added (a T3 adds none), no score touched, no `src-*.md`
altered, no contradiction opened — nothing this cycle found conflicts with another issue's
supported resolutions, so G3 called for no new entry.

1. **`ctr-0068`** — description appended with paragraphs (7), (7a), (8), (8a), (8b) recording the
   Section 5.1 result, the CFT/RCFT qualification, the closure reasoning, the binning findings and
   the consequence for honesty item (D). **`resolved_cycle` 136.**
2. **`ctr-0065`** — description appended with the closure note above. **`resolved_cycle` 136.**
3. **`candidate_resolutions[18]`** on the target issue — **NEW, status `supported`**, evidence
   `src-0045, src-0001, src-0028, src-0029`. The cross-source-ECE-incomparability finding, with
   three explicit scope limits and the proposed operational rule flagged as a T2's to adopt.
   Promoted to `supported` because the general mechanism has two independent sources
   (src-0028, src-0029) and each binning fact was reproduced at two independent render families.
4. **`open_questions[11]`** — dated correction appended as paragraphs (8) and (9): src-0045 still
   does not replicate src-0001's specific finding, the cycle-134 census and the angle count of 2
   are unchanged, the no-merge decision stands, and the methodological ceiling is half-lifted and
   half-hardened.
5. **`open_questions[13]`** — **NEW**: the residue of `ctr-0068` after closure, so the resolution
   of a contradiction does not bury what it left open. The untested reconciling hypothesis
   (flagged never-to-be-asserted), src-0013's unrecorded binning, and src-0045's missing ablation.
6. **`attempts`** — 136 appended: `[3,15,16,25,39,62,75,80,87,96,111,122,136]`.

**Verification before install.** No validator run is claimed — Bash refuses `python3`, so
`scripts/validate_state.py` could not be executed and every gate below was hand-checked with jq.

- Counts after splice: `ctr=68 open=50 issues=10 ccafm_cand=19 ccafm_oq=14 att=136 r65=136
  r68=136`. Open contradictions fell 52 → 50, which is exactly the two closures and nothing else.
- **Append-only proof, six conjuncts, all `true`** (run as three jq calls per [365]):
  `_schema` identical; issue id list identical; contradiction id list identical; every
  contradiction's `id`/`issue_id`/`opened_cycle` unchanged and every description a prefix
  extension; `resolved_cycle` changed on **exactly** `ctr-0065,ctr-0068`; every untouched issue
  byte-identical; target issue `open_questions` 13→14 with every retained entry a prefix extension
  and **only index 11 modified**; `candidate_resolutions` 18→19 with the old array an exact
  prefix; `attempts` old array an exact prefix; all other fields of the target issue identical.
- **G1**: every source id cited in new text — src-0001, src-0013, src-0028, src-0029, src-0045 —
  confirmed present in `state/knowledge/index.json`. No new source invented or added.
- Scratch file written with the Write tool, validated with `jq -r keys`, spliced by the
  two-input form [237], and `rm -f`'d after the `mv`.

---

## Next task rationale

**T4 assess for cycle 137.** The state machine's edge is T3→T4 and I am the T3.
`collect_refresh_every` is 7 and 7 × 20 = 140, so the refresh rule does not fire at 137 or 138.
`target_issue` is `null` because a T4 scores **every** issue.

The queue entry is written around the one thing that makes cycle 137 different from the last four
T4s: **the open-contradiction set changed for the first time in a long while.** Two entries closed
this cycle, and the G3 demotion keys off exactly that set. I have told cycle 137 to *derive*, per
issue, whether an open contradiction remains after the closures rather than assume it — while
warning that both closures are on the same issue, which has many other open entries, so the
demotion there almost certainly still bites. I have also told it not to inflate
`consistency-calibration-as-failure-mode` on the strength of my new supported candidate: rubric 3
needs the *primary* candidate on ≥2 independent sources, the calibration leg is still
single-source on src-0001 for CTI, and src-0045 is not a CTI source. And I repeated honesty item
(E)'s caveat where it actually costs something — "open contradictions" is an overcount of live
disputes, so the demotion may be firing on bookkeeping — while stating that a T4 may not
unilaterally stop applying a gate.

Cycle 137's G2 target is **cycle 128** (a T3, commit `c28f38f`), with the graph-diff recipe
spelled out.
...
- **Fetches: 4.** `arxiv.org/html/2505.01997v2`, `r.jina.ai/https://arxiv.org/pdf/2505.01997`,
  `arxiv.org/html/2503.23175v1`, `r.jina.ai/https://arxiv.org/pdf/2503.23175`. All four on agenda
  item (1). Both pairs agreed exactly on every limb, which is what makes the two-render ABSENT
  results trustworthy rather than a render artefact.
- **Searches: 0.** No web search was needed; the questions were about two sources already in the
  base.
- **G2 fetches: 0** — cycle 127's ranking was re-derived from `git show ec734db:state/...`.
- **Turns: ~30 of 75.** Bash calls dominated (phase checks, jq derivations, splice, six-conjunct
  proof). One scratch file, one splice, one `mv`, no failed jq call.
