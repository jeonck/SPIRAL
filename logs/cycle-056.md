...
**Date:** 2026-08-01 · **Model:** claude-opus-5 · **CLI:** 2.1.220

---

## Phase verification (done first, before anything else)

The queue entry told me to disbelieve it and check the phase at source. I did.

| check | expected by handoff | actual | verdict |
|---|---|---|---|
| `last_completed_task.txt` | `T3 investigate` | `T3 investigate` | ✅ |
| contradiction count | 23 | 23 | ✅ |
| `ttp-attack-mapping-reliability.attempts` | `[16,31,32,46,55]` | `[16,31,32,46,55]` | ✅ |
| `scores.json.last_assessed_cycle` | 53 | 53 | ✅ |
| sources in index | 24, unchanged | 24 | ✅ |

Cycle 55 landed. **I am the T4.** The handoff's phase account was correct in every
particular — the first cycle in a while where it was.

I also read `prompts/t4_assess.md` myself rather than taking the queue entry's account of
it, and `config.yml` myself for `gates.g3_contradiction_demotion` (**2**, as claimed) and
`scoring.tie_break_recent_attempt_penalty` (**1**). I read
`scripts/validate_state.py` lines 128–158 directly: lines 135–142 enforce the
evidence requirement for scores ≥ 2, lines 144–156 implement the **G3 ceiling**
(`score > scale_max − demotion` is the error), *not* the subtraction the prompt's step 3
describes. That is carry-forward **[4]**, forty-fifth cycle awaiting a human.

**Open-contradiction map re-derived myself with jq, not inherited from any log** (cycle 46
re-derived it and found its own queue entry's map wrong on two issues):

```
jq -r '.contradictions[]|select(.resolved_cycle==null)|.issue_id' state/issues/graph.json | sort | uniq -c
```

| issue | open contradictions |
|---|---|
| `ioc-extraction-reliability` | 4 — ctr-0001, ctr-0004, ctr-0010, ctr-0020 |
| `institutional-incident-real-world-impact` | 3 — ctr-0012, ctr-0018, ctr-0022 |
| `ttp-attack-mapping-reliability` | 3 — ctr-0015, ctr-0016, ctr-0023 |
| `task-dependent-reliability-framing` | 2 — ctr-0009, ctr-0017 |
| `attribution-confident-wrong-gap` | 1 — ctr-0008 |
| `consistency-calibration-as-failure-mode` | 1 — ctr-0011 |
| `extraction-vs-reasoning-ordinal-axis` | 1 — ctr-0013 |
| `automated-triage-under-refusal` | 1 — ctr-0021 |
| `attribution-expressed-confidence-unmeasured` | **0** |

23 entries total, 18 open, 5 closed (ctr-0002, ctr-0003, ctr-0005, ctr-0006, ctr-0007,
ctr-0014, ctr-0019 — that is 7; the counts reconcile because ctr-0007/0014/0019 all sit on
`ioc-extraction-reliability`, which still has 4 open). Under the validator's **set
comprehension over `issue_id`**, multiple entries demote exactly as much as one, so every
issue except `attribution-expressed-confidence-unmeasured` carries a ceiling of 3.

---

## Retrospection (G2)

**Chosen conclusion:** `institutional-incident-real-world-impact` →
`candidate_resolutions[1]` (status *supported*, evidence `src-0009`, `src-0010`,
`src-0004`), which asserts that ENISA's own publication pages independently corroborate
the incident's verifiable structure.

**Why this one rather than the stalest.** The queue entry named `src-0011` (cycle 33) as
the stalest source and `src-0009`/`src-0010` (cycle 34) as joint second. I verified that
ordering against the inherited list myself and it holds. I chose the cycle-34 pair anyway,
for a stated reason: `src-0011` supports only a **proposed** candidate and could not move
any score, whereas `src-0009`/`src-0010` underwrite a **supported** candidate on the one
issue this cycle was explicitly asked to reconsider. One cycle of extra staleness bought
direct leverage on the decision I owed. A successor should take `src-0011` next.

**Method.** Both pages fetched 2026-08-01 in **verification mode** (rule xv): numbered
items, PRESENT/ABSENT each, whole-sentence quotation only, explicit "do NOT substitute a
nearby or similar sentence", an **ITEM 0** body-visibility probe and a **final truncation
probe**. Both pages self-reported the body visible and **not truncated**, so rule (xvi)
voids nothing. I added one new item type — a **version-staleness probe** — which had never
been run on either source.

**Verdict: CLEAN PASS. No contradiction opened.** Every stored string re-verifies.

| item | result |
|---|---|
| src-0009 revision notice | PRESENT verbatim: `Revision Notice – Version 1.2. (09 January 2026): This publication has been updated to edit some links.` |
| src-0009 publication date | `Publication date:October 1, 2025` |
| src-0009 PDF | `ENISA Threat Landscape 2025_v1.2.pdf`, path `/sites/default/files/2026-01/` |
| src-0010 revision notice | PRESENT verbatim: `Revision Notice – Version 1.2 (09 January 2026): This publication has been updated to correct some broken links and typos.` |
| src-0010 revision date / publication date | `09 January 2026` / `Publication date:November 6, 2025` |
| **version staleness, both pages** | any revision notice with a version **other than 1.2** → **ABSENT**. Neither report has been revised again in 22 cycles. |
| AI / artificial intelligence / generative AI / LLM, both pages | **ABSENT** |
| any count of incorrect or corrected footnotes/links, both pages | **ABSENT** |
| any statement of the **cause** of the defects, both pages | **ABSENT** |

**Honest limit on my own G2, rule (v).** The three ABSENT verdicts were taken on **one URL
form each**. They agree with same-form ABSENTs recorded at cycles 12 and 34, so this is a
*third consecutive* confirmation — but it is **not** confirmation on an independent second
URL form, and I do not claim it as one. These are short publication pages with no
alternative rendering I could find that is not `web.archive.org` (blocked).

**What the pass nonetheless establishes, and it cuts against the state.** A clean pass on
*what is stored* re-confirms that ENISA's primary artefacts corroborate only the
**structure** of the incident — two reports, one revision date, link defects — and say
**nothing** about AI, about any footnote count, or about cause. So the **AI-attribution
limb**, which is what the issue is actually about, remains **single-sourced to `src-0004`**
(heise, secondhand from Der Spiegel), which itself carries `ctr-0012` unrepaired. That is
the rule (vi) shape once more: the sources are clean; the *aggregation* the state built
over them was not. It became the second of my two grounds for the demotion below.

**Recorded in state:** a `CYCLE-56 G2 RE-VERIFICATION` key_claim appended to `src-0009`
and `src-0010` in `index.json`, and a matching `## Cycle 56` section appended to
`src-0009.md` and `src-0010.md`. Nothing deleted or rewritten. This discharges, for these
two sources, the step that `ctr-0011`, `ctr-0012`, `ctr-0013`, `ctr-0014` and now
`ctr-0023` each left undone on theirs.

---

## Task performed

**T4 — scored all nine issues** in `state/issues/graph.json`, set `assessed_cycle: 56` on
every entry and `last_assessed_cycle: 56`.

### Ranking

| issue | c53 | **c56** | open ctr | ceiling | bound? |
|---|---|---|---|---|---|
| `institutional-incident-real-world-impact` | 3 | **2** ⬇ | 3 | 3 | no (merit demotion) |
| `ttp-attack-mapping-reliability` | 2 | 2 | 3 | 3 | no |
| `ioc-extraction-reliability` | 2 | 2 | 4 | 3 | no |
| `consistency-calibration-as-failure-mode` | 2 | 2 | 1 | 3 | no |
| `attribution-confident-wrong-gap` | 2 | 2 | 1 | 3 | no |
| `attribution-expressed-confidence-unmeasured` | 2 | 2 | **0** | none | n/a |
| `task-dependent-reliability-framing` | 2 | 2 | 2 | 3 | no |
| `extraction-vs-reasoning-ordinal-axis` | 2 | 2 | 1 | 3 | no |
| `automated-triage-under-refusal` | 2 | 2 | 1 | 3 | no |

**One score changed. Every issue in the graph now scores 2.**

### The demotion: `institutional-incident-real-world-impact`, 3 → 2

This was the only score above 2 anywhere in the graph and had been excluded from
weakest-link contention on that basis for many cycles. **The demotion is a merit
judgement, not a gate effect** — its ceiling was already 3 and did not cause it. Three
grounds, in order of weight:

1. **`ctr-0022` impeaches the aggregation the 3 rested on.** No single candidate here is
   supported by two independent sources for the issue's titular claim. The 3 came from
   *aggregating two incidents* — ENISA (`src-0004`) and EY Canada (`src-0012`) — each
   carried by one secondary source, into `candidate_resolutions[2]`'s
   existence-and-recurrence argument: *"Two independent institutions, one public and one
   commercial, is materially stronger than one."* Cycle 54 pulled GPTZero's own page and
   found, verbatim, `Over the past few months we have set up an automated pipeline to
   search for vibe citations by finding and scanning public reports from major consulting
   firms.` and `Instead of releasing our results all at once, we are going to focus on one
   report at a time.` The state's claim in **three places** that EY was found by "the same
   route as ENISA (an outside party clicking through footnotes)" is refuted at the
   publisher's page. Two incidents are two independent draws only if found independently;
   ENISA was unprompted, EY is the first serial instalment of a targeted commercial sweep
   over a population selected for exactly this failure mode. **The aggregation is what
   broke.**
2. **My own G2 closes the other route** — the AI-attribution limb has no primary
   corroboration on either incident (above).
3. **The rubric read literally.** Level 3 needs the *primary* candidate supported by ≥2
   *independent* sources; per incident there is exactly one secondary source. Level 2 is
   "≥1 candidate with single-source support", which is precisely the position. Step 5:
   torn between 3 and 2, give the lower.

**What I am not saying.** `ctr-0022` itself warns against over-reading, and I respect it:
both incidents are real, both were caught **externally** rather than by internal QA, and
that shared property survives. `src-0012`'s four key_claims have now passed **four**
consecutive re-verifications and `src-0009`/`src-0010` a third. No source is impeached
here.

**What restores a 3:** (a) a third documented incident surfaced by an *unprompted* route;
(b) any source independent of `src-0004` corroborating the AI attribution for ENISA, or of
`src-0012` for EY; (c) a link-resolution sweep publishing a **denominator** —
`open_questions[0]`, which `ctr-0022` shows GPTZero's pipeline structurally cannot supply
while it publishes numerators only. Route (a) has named, unfetched leads sitting in
`src-0012.md` since cycle 13 — "two different Deloitte reports" and "a government
publication", now **43 cycles** old.

**I considered whether the consequence for the selector should stay my hand, and it must
not.** A nine-way tie is harder for the T5 than an eight-way one. Scoring an issue 3 to
keep the selector's job easy is exactly the inflation `prompts/t4_assess.md` step 5
forbids. An honest tie is a better input than a dishonest rank. I have warned the T5
loudly instead.

### `ttp-attack-mapping-reliability` — held at 2, and the hold is the judgement

`ctr-0023` **cuts both ways** and I read it at source rather than through the queue's
precis. **Good limb:** the central strictness finding is now *independently replicated in a
second file* — `tp = len(article_ttps_set.intersection(validated_ttps_set))` PRESENT at all
three renders of `utils/eval_ttps_utils.py`, and both the raw and sha-pinned renders
answered **ABSENT** when asked directly about dot-splitting or parent-technique credit. So
exact-ID set intersection with no partial credit is a property of *both* modules read so
far. **Bad limb, and larger:** the stored directory listing omitted `utils/` entirely since
cycle 31; that module is 31,268 bytes against `compute.py`'s 13,636 and holds **four**
distinct precision/recall functions, two "raw" and two "mapping". `candidate_resolutions[2]`
opens with a definite article — "**THE** ATT&CK/TTP SCORING RULE … IS NOW KNOWN" — and the
singularity does not survive. **The inference gap in `open_questions[4]` is wider, not
narrower**: previously the paper merely failed to say `compute.py` produced Table 4; now
there are ≥4 candidates it also fails to choose between, and the three renders conflict on
the `fn` assignments so that under rule (ix) no `fn` line is usable at all. A strictness
property better corroborated but attached to a scorer we can no longer name is not progress
toward the **magnitude** the issue's title asks for. **2 holds.**

### `automated-triage-under-refusal` — held at 2, now close to the 2/1 boundary

The queue asked what `ctr-0021` does to merit three cycles on. Answer, and no prior
rationale has said it in terms: **`ctr-0021`'s Defect 1 strikes the candidate's own
headline sentence.** This issue has exactly **one** candidate, and it asserts that "the
system accepts or acts on most of what a human analyst would reject" — `ctr-0021`'s
location (e). The `accepts` limb is the invalid conversion: precision (Accepted) 0.27–0.40
fixes the share of *accepted* items ground truth had rejected, not `FP/(FP+TN)`, which is
not derivable without a base rate the paper never reports. The `acts on` limb survives
intact via `src-0015` (82.5%/65%/57.5%/45% false-positive rates; evidence-gated action
0.375–0.542) — directly reported rates, not conversions. So **2, not 1** (single-source
support still exists twice over) but **3 is further away than before**, because the two
sources no longer support the *same sentence*. Compounding, and endorsed after checking at
source: this issue's `attempts` array is **empty** — in 56 cycles no T3 has ever
investigated it, so its score rests entirely on evidence no investigating cycle has read.
That is a reason to score low, not to score generously. It was eliminated at cycle 54 only
by `created_cycle` at tie-break (c), the unratified rule (carry-forward **[30]**).

### The six inherited holds

`ioc-extraction-reliability`, `consistency-calibration-as-failure-mode`,
`attribution-confident-wrong-gap`, `attribution-expressed-confidence-unmeasured`,
`task-dependent-reliability-framing`, `extraction-vs-reasoning-ordinal-axis` all **hold at
2**, and I say plainly in each rationale that **the hold is inherited** rather than
manufacturing fresh reasoning. Nothing touched them since cycle 53: cycle 54 was a T5 that
altered no issue state, cycle 55 a T3 confined to `ttp-attack-mapping-reliability`. Each
still received a cycle-56 block with a *specific* new observation:

- **`ioc-extraction-reliability`** — `src-0017` is in *its* evidence list too, and nobody
  has re-derived which files in that repository implement **IoC** matching from a complete
  listing. Rule (xx) applies here and it is new this cycle.
- **`attribution-confident-wrong-gap`** — `ctr-0008` step (i), the re-fetch of
  `eval/threat_actor.py`, is now **25 cycles** undone and is the oldest live repair in the
  base. Whoever runs it should pull the **complete trees-API listing first**; a 25-cycle-old
  step executed against an incomplete enumeration would fail in exactly the way `ctr-0023`
  describes.
- **`task-dependent-reliability-framing`** — its second candidate's whole warrant is that
  everything except the sub-task is held constant. If the sub-tasks were scored by
  different variants of the four now known to exist, **the harness was not held constant
  either**, and nobody in this base can currently say that it was. Rule (xix) in its purest
  form: "same team, same corpus, same models, same harness" is a *similarity claim*, is
  nowhere quoted from `src-0007`, and is invisible to every string and number check run on
  that table.
- **`extraction-vs-reasoning-ordinal-axis`** — its formal **withdrawal** of the
  cross-sub-task inference was *more* justified than the cycle that made it could know.
  Exactly one of its three candidates is supported (one REJECTED, one PROPOSED), which is
  the rubric's own wording for 2.
- **`consistency-calibration-as-failure-mode`** — `src-0015` sits in *this* evidence list
  and in `automated-triage-under-refusal`'s, and its "calibration" is an **action-threshold**
  construct, not the ECE/Brier calibration of `src-0001`/`src-0013`. An issue titled for
  calibration holds both senses of the word in one evidence list. **No contradiction
  opened** — it is a caution already recorded inside a candidate summary, not a conflict
  between two supported claims, which is G3's test; per the `ctr-0010` precedent a T4
  restates such a caution in place. A T3 should check whether any candidate crosses the two
  senses.
- **`attribution-expressed-confidence-unmeasured`** — the jq re-derivation surfaced a fact
  worth stating outright: **it is the only issue in the graph with no open contradiction at
  all.** No ceiling applies; 3, 4 and 5 are all *available* to it and it still scores 2 on
  merit. **A successor must not read that clean record as strength.** Two of its six
  sources — `src-0023`, `src-0024` — have **never been independently verified by any
  cycle**, both added by cycle 49 itself, and `src-0023` carries the unresolved GPT-5/GPT-4
  judge discrepancy. **An absence of contradictions against unverified sources is an absence
  of scrutiny, not an absence of defects** — 11 of the 23 entries in this base were opened
  by a G2, and cycles 47–55 opened one each. Cycle 53 asked a successor whether it agreed
  that this alone justifies 2 without reaching **[97]**. **I agree, and I recorded it as an
  independent ground** so the score no longer rests on [97] alone.

### Policy conflicts I refused to resolve unilaterally

- **[4] — subtraction vs ceiling.** `prompts/t4_assess.md` step 3 says *subtract 2*;
  `validate_state.py` lines 144–156 enforce a *ceiling of 3*. I applied the **ceiling**, as
  every T4 since cycle 16 has, and refused the subtraction — flipping it now would produce a
  graph-wide discontinuity on an unratified reading. **Forty-fifth cycle.** The illustration
  is starker than ever: under subtraction, `institutional-incident-real-world-impact` and
  `automated-triage-under-refusal` would both now score **0** — the rubric label
  "unexamined" — one of them while holding four supported candidates and five sources.
- **[97] — may a non-commensurability or negative finding count as a resolution?** Live on
  three issues now (`ttp-attack-mapping-reliability` candidate [4] with four independent
  sources; `extraction-vs-reasoning-ordinal-axis`; `attribution-expressed-confidence-unmeasured`).
  **Sixth ad-hoc decision, and identical to the previous five.** I took the conservative
  side per step 5 and declared the dependence in each rationale rather than deciding it. If
  [97] is ever answered *yes*, two issues gain a live case for 3 on the same day.
- **[41] — single `issue_id` per contradiction.** Under-counts (four open entries on
  `ioc-extraction-reliability` demote exactly as much as one) and mis-attaches (`ctr-0018`
  ceilings an issue via a source that issue does not cite). Not fixable by editing the
  schema; awaiting a human.
...
| file | change |
|---|---|
| `state/assessments/scores.json` | `last_assessed_cycle` 53 → **56**; `assessed_cycle` → 56 on all nine entries; `institutional-incident-real-world-impact` score **3 → 2**; a `[CYCLE 56 (T4) …]` block **appended** to all nine rationales. Nothing deleted or rewritten. |
| `state/knowledge/index.json` | `CYCLE-56 G2 RE-VERIFICATION` key_claim appended to **src-0009** and **src-0010** (each now 4 claims). **No source added**; count unchanged at 24. |
| `state/knowledge/src-0009.md`, `src-0010.md` | `## Cycle 56` G2 section appended to each. |
| `state/issues/graph.json` | **untouched** — the G2 passed, so no contradiction was opened. Still 23 entries. |
| `state/queue/next_task.json` | T5 (select) for cycle 57. |
| `state/queue/last_completed_task.txt` | `T4 assess`. |

All four JSON edits validated with `jq -e .` and read back with `jq -r` projections on the
fields added.
...
**T4 → T5** per the state machine. Cycle 57 selects.

**Refresh rule, derived myself:** `config.yml` sets `collect_refresh_every: 7`;
`prompts/t5_select.md` step 4 tests `current_cycle % collect_refresh_every == 0` *inside*
the T5 cycle. **57 mod 7 = 1 ≠ 0**, so on that arithmetic cycle 57 is **not** a refresh
cycle and the next task is a T3. I have told the T5 to re-derive it anyway and to
cross-check against the actual T1 history in git, because cycle 41 found a handoff's
schedule projection wrong by **fourteen** cycles and cycle 50 found its own wrong too.

**What the T5 is walking into.** A **nine-way tie at 2**, up from eight, because of my
demotion. The ladder has failed outright in **two consecutive** T5 cycles (51, 54) and
cycle 51's ad-hoc patch *splits* on cycle 54's tie. I read the `depends_on` graph with jq
while scoring and passed forward a **candidate clean path, explicitly labelled a hypothesis
to be checked and not a conclusion** — I was a T4 and had no standing to select:

- Empty `depends_on`: `ttp-attack-mapping-reliability`, `ioc-extraction-reliability`,
  `consistency-calibration-as-failure-mode`, `institutional-incident-real-world-impact`,
  `automated-triage-under-refusal`.
- Both upstream *and* depended upon: `consistency-calibration` (by 3),
  `ttp-attack-mapping` (by 1), `ioc-extraction` (by 1).
- Attempt penalty at 57: `ttp` attempts `[…,55]` → ≥ +1. `ioc` attempts `[…,52]` → 57−52 =
  **exactly 5**, so the answer depends on whether the window is 52–56 or 53–57 and the T5
  must state its convention. `consistency-calibration` attempts `[…,39]` → **0 under either
  convention**.
- **If** the topological reading of 3(a) is taken, `consistency-calibration-as-failure-mode`
  is the only upstream issue with a zero penalty and the ladder **terminates at 3(b)**
  without reaching the unratified `created_cycle` rule. That would be materially better
  than cycles 51 and 54 managed — which is why I wrote it down, and why I also flagged the
  gap: **3(a) as written relates an issue only to its own dependents**, so it does not rank
  `institutional-incident-real-world-impact` or `automated-triage-under-refusal` against
  anything at all. Whether it licenses a global topological order is precisely **[75]**.
...
| item | count |
|---|---|
| Web fetches | **2** (both ENISA publication pages, verification mode) |
| Web searches | **0** — a T4 does not collect |
| Assistant turns | ~34 of 75 |
| Files read | `next_task.json`, `meta.json`, `config.yml`, `prompts/t4_assess.md`, `prompts/t5_select.md`, `validate_state.py` (128–158), `graph.json` (projections + 3 full contradiction pulls), `index.json` (projections + one window), `scores.json` (projections + 6-line window), `logs/cycle-053.md` (Budget), `logs/cycle-055.md` (carry-forward) |
| Files written | `scores.json`, `index.json`, `src-0009.md`, `src-0010.md`, `next_task.json`, `last_completed_task.txt`, this log |
| JSON validations | 4 (`jq -e .` after every JSON edit, each followed by a read-back projection) |
| Sources added | **0** — count unchanged at 24 |
| Contradictions opened | **0** — the G2 passed clean |

**Efficiency notes for successors.**

- **The `scores.json` editing method works and I confirm cycle 53's version of it.** Ten
  Edits for a nine-issue rescore: `Read` only the first 6 lines to satisfy the Edit tool,
  one `replace_all` on `"assessed_cycle": 53` (which does **not** collide with
  `"last_assessed_cycle": 53` because the character before `assessed` differs), then one
  targeted Edit per issue anchored on the **last ~150 characters** of its existing
  rationale, appending a new block rather than rewriting. The anchors must be long enough
  to be unique — several rationales end with the identical
  `Carry-forward [4], FORTY-SECOND cycle.]`, so I extended each anchor backwards until it
  contained a distinguishing `ctr-` id or phrase.
- **New and cheap:** `jq -r '.scores|to_entries[]|"\(.key) \(.value.rationale[-900:])"'`
  prints the tail of every rationale at once, which is where the current cycle's reasoning
  lives. That one call replaced nine separate reads.
- **The version-staleness probe costs nothing.** One extra numbered item — "is there any
  revision notice with a version *other* than X?" — turned a routine re-verification into a
  re-verification *plus* a first-ever currency check on two sources. It should be a standing
  item on every verification fetch. `src-0019` and `src-0022` have still never had one.
- The **ITEM 0 / final-truncation** pair again cost nothing and again made the ABSENT
  verdicts trustworthy. Fifth consecutive cycle in which the verification-mode shape earned
  its keep. Note the honest converse, though: it did **not** give me a second URL form, and
  I did not pretend it did.

---

## Methodological rule, in twenty parts — reproduced in full

Reproduced from `logs/cycle-055.md` and extended. Cycle 56 **accepts (xx)** and offers a
candidate **(xxi)**.

- **(i)** Read the prompt file yourself; never trust a handoff's summary of a rule.
- **(ii)** Re-derive maps, counts and schedules from the files with `jq`, not from prose.
- **(iii) PULL THE VERBATIM DEFINITION OF THE METRIC, not only its value.** Eleven
  contradictions now.
- **(iv) PULL THE WHOLE TABLE AND QUOTE THE WHOLE SENTENCE INCLUDING ITS PARENTHETICALS,
  AND SINCE CYCLE 50 THE WHOLE PARAGRAPH FROM ITS FIRST WORD.** The omitted-material shape
  is the most frequent single defect in this base at **eight** contradictions.
- **(v)** A **PRESENT** verdict may be trusted from one fetch; an **ABSENT** verdict must be
  confirmed against a **second URL form**, checking four things: the abstract, a different
  URL rendering, that you fetched the file the claim actually cites, and that the version
  you fetched contains the material at all. **Eighteen instances.** *Cycle 56's own limit is
  a live example of the rule biting its author: its three ABSENTs are one-form only and are
  recorded as such.*
- **(vi) A SOURCE CAN BE CLEAN WHILE THE STATE'S ACCOUNT OF IT IS NOT.** `ctr-0023` is the
  eleventh instance; cycle 56's demotion applies the shape to an **aggregation** rather than
  to a single claim.
- **(vii)** Prefer `/html` and `/abs`; there is no PDF text extraction in this sandbox.
- **(viii) STRING-MATCHING A CLAIM'S QUOTATIONS AND NUMBERS DOES NOT TEST ITS QUANTIFIER,
  ITS SCOPE, ITS ARITHMETIC DERIVATION, OR THE CHARACTERISATION WRAPPED AROUND IT.**
  `ctr-0023` is a pure case: every stored string re-verified PRESENT and the defect was an
  **omission from a directory listing** plus a **definite article**.
- **(ix)** A fetch that returns a summary instead of the requested verbatim text is as
  untrustworthy as a bare ABSENT; and **when two or three renders of the same file disagree
  on a line, all of them are unusable for that line** and nothing may be stored from any.
- **(x)** Record dead ends and refusals; they are data.
- **(xi)** Append, never rewrite. Corrections go in a contradiction entry.
- **(xii) A DERIVED NUMBER MUST BE AUDITED AGAINST THE LOG THAT DERIVED IT.** `src-0006`'s
  derived stage means are now in their **ninth** cycle undone.
- **(xiii)** Before opening a contradiction on the strength of a summary, go back and get
  the artefact verbatim.
- **(xiv)** An aborted cycle's findings do not exist in the state; re-derive them.
- **(xv)** Some fetches refuse verbatim reproduction and return a summary. Use a
  **verification-mode** prompt: numbered items, each answered PRESENT or ABSENT, quote only
  the single full sentence for PRESENT, an explicit "answer CANNOT READ if the body is not
  visible", plus "reproduce to its very end, do not stop early", plus "do NOT substitute a
  nearby or similar sentence; if you cannot find the exact string answer ABSENT even if
  something similar exists", plus on at least one item ask for the **entire paragraph**
  "from its first word to its last word", plus make **ITEM 0** "can you see the body, YES or
  NO, and if YES quote its first sentence" and the **last** item "was your rendering
  truncated, YES or NO, and where does it stop". Cycle 55 used this on all five of its
  fetches and it surfaced `ctr-0023`; cycle 56 used it on both of its fetches and it is what
  made a clean pass trustworthy. **It is the default shape for any verification fetch.**
- **(xvi)** Where a page is long enough to truncate, an ABSENT verdict about any part of it
  is worthless unless you asked the fetch directly whether it could see the section your
  absence concerns.
- **(xvii)** The word "verbatim" in a prompt does not bind the fetch.
- **(xviii)** When the state converts a reported metric into a different quantity,
  re-derive the conversion from the definitions before trusting it.
- **(xix)** When the state asserts that two cases are "the same" — same route, same
  mechanism, same population, same design — **check the similarity claim itself at source**,
  because a similarity claim is an assertion about a *relation*, is never quoted from either
  source, and is invisible to every string and number check. **Cycle 56 records that (xix)
  is what carried its demotion.**
- **(xx) ACCEPTED AT CYCLE 56.** When a conclusion rests on having read "**the**" artefact,
  **re-enumerate the artefact before trusting the definite article**. A directory listing, a
  table of contents or a file manifest is cheap, is almost never re-checked, and determines
  whether the file you read is the only file there is. Cycle 55 spent **one** fetch on a
  trees-API listing and overturned a four-cycle-old enumeration. **Cycle 56 accepts it as
  distinct from (v)'s fourth limb**: (v) asks whether the *version* you fetched contains the
  material, a question about **one file**; (xx) asks whether that file is the **only** file,
  a question about the **set**. It also flagged a live application — `ctr-0008`'s
  25-cycle-old step (i) targets `eval/threat_actor.py` **by name** and should be run against
  a complete listing first.

**Candidate twenty-first rule, offered not imposed:**

- **(xxi)** **A score built by aggregating several weak cases is only as good as the
  independence of the cases, and independence of *discovery* is a separate question from
  independence of *source*.** Two incidents reported by two unrelated outlets look like two
  independent sources and may still be **one draw**, if one of them was surfaced by a
  process that was hunting for exactly that failure mode. **Ask how each case came to be
  known before counting it.** A successor should decide whether this is genuinely distinct
  from (xix) or a special case of it.
