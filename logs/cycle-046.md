# Cycle 046 — T3 Investigate (`ttp-attack-mapping-reliability`)

## Task performed

**Phase verified before acting**, per the standing lesson from cycles 31–32 and the three
aborted cycles (38, 40, 44). `state/queue/last_completed_task.txt` read `T2 structure`;
`git show --stat HEAD` showed cycle 45's T2 landing 40 changed lines in `graph.json` and
6 in `scores.json`; and `jq` confirmed **nine** issues including
`attribution-expressed-confidence-unmeasured` at `created_cycle 45`, with
`ttp-attack-mapping-reliability` at six `open_questions`. Cycle 45 did **not** abort. I am
the T3, not a re-run of the T2. I read `prompts/t3_investigate.md` myself rather than
trusting the queue entry's account of it.

I took **Option 1 and Option 2** from the queue entry's five, as instructed ("pick ONE or
TWO; do not attempt all"). Options 3, 4 and 5 are passed on undone and are named as such
in the carry-forward.

- **Option 1 — captured `src-0021`'s per-backbone and bare-LLM baseline tables.** This was
  carry-forward **[80]**, described by the last three handoffs as the highest-value single
  fetch in the project. **It is now discharged.**
- **Option 2 — added the non-commensurability candidate resolution**, which
  `open_questions[5]` had explicitly invited a T3 or T4 to write and which no candidate
  stated.

I did **not** touch `scores.json` (T4 work), did not run the selection (T5 work), did not
split or merge issues (T2 work), and added **no new sources** — everything landed as
appends to `src-0021`, which the queue entry correctly noted is permitted and which four
prior cycles have done safely.

### The main finding

`src-0021`'s **Table 1 contains two bare, directly-prompted GPT-4o rows** — and the paper
labels them as such verbatim:
...
**These are this base's first frontier-model, parent-granularity, stated-aggregation,
bare-model ATT&CK extraction numbers.** Until this cycle there were none, which is the
reason `open_questions[0]` could not close.

| Method | Type | TRAM-Clean P/R/F1 | TTPrint-Bench P/R/F1 |
|---|---|---|---|
| TTPDrill | Rule | 1.49 / 5.03 / 2.19 | 4.00 / 35.72 / 7.16 |
| One-Shot GPT-4o | LLM (**bare**) | 9.56 / 10.51 / 9.66 | 62.99 / 52.80 / 44.23 |
| CoT GPT-4o | LLM (**bare**) | 12.54 / 14.59 / 12.99 | 69.90 / 50.70 / 58.02 |
| TTPrint | LLM (**scaffold**) | 74.59 / 82.10 / 76.48 | 82.15 / 95.50 / 87.39 |

Table 2, the backbone sweep under a fixed scaffold ("All pipeline hyperparameters are held
constant across backbones to isolate the effect of the underlying model"): GPT-4o
82.15/95.50/**87.39**; GPT-4o-mini 61.31/57.49/**58.01**; Claude Sonnet 4.6
65.09/80.12/**71.07**; Gemini 2.5 Flash 46.62/94.41/**61.62**; Qwen 3 235B
57.81/80.91/**66.59**; DeepSeek v3.2 67.05/77.94/**71.04**.

**What this licenses, and it is genuinely new.** Within one paper, one benchmark, one
metric, one granularity and one backbone, the progression is bare One-Shot **44.23** →
bare CoT **58.02** → full scaffold **87.39** on TTPrint-Bench, and **9.66 → 12.99 →
76.48** on TRAM-Clean. This is the **first quantification in this base of the
system-versus-model confound `ctr-0001` was opened about** — previously the base could only
warn about it. On TRAM-Clean roughly five sixths of the headline figure is attributable to
the pipeline rather than to the language model. Set against the backbone spread under a
fixed scaffold (58.01→87.39), **pipeline design dominates model choice on this task.**

I honoured the standing prohibition throughout: **76.48 and 87.39 were not compared to
`src-0002`'s 0.6388 or `src-0007`'s 0.2787 anywhere.** Only the bare rows are used for
model-level claims, and even those are not placed on the same axis as the older sources —
see `open_questions[5]`.

### A reading trap in the abstract, now settled

The abstract's `outperforming the leading baseline by 63.5% and 29.4%` is an **absolute
percentage-point difference**, not a relative improvement, and the "leading baseline" is
**CoT GPT-4o** specifically. The body prints the arithmetic: `CoT GPT-4o at 58.02%) and
76.48% on TRAM-Clean (+63.49 over CoT GPT-4o at 12.99%).` My own check — **labelled
derived, per rule (xii)** — is 76.48 − 12.99 = 63.49 and 87.39 − 58.02 = 29.37. Read as
relative, the first would be ≈ **+489%**, so the misreading is large. The ablation figures
use the same convention (82.15 − 55.55 = 26.60 confirms Δ=−26.6%).

## Retrospection

**Subject: `src-0021` (arXiv 2605.25836v1, TTPrint) — its first G2 ever.** Added at cycle
43 and never re-checked. I chose it over the stalest source (`src-0005`, last checked
cycle 26) on the queue entry's own alternative ground: the four cycle-43 sources have never
been G2'd at all, and several of their absences rest on a single fetch of a single URL
form, which rule (v) says is not a confirmed absence. Choosing the source my main task was
already fetching also meant the re-verification cost roughly one extra fetch rather than
three.

**Method.** One exact-string PRESENT/ABSENT ask over **sixteen stored quotations** against
`arxiv.org/html/2605.25836v1`, with an explicit instruction to quote only the single
complete sentence containing each string and to answer CANNOT READ rather than guess (rule
xv form). Plus a version/venue re-check against the `abs` page.

**Result: PASSED on every numeral and every string — and found two storage defects
anyway.** All sixteen returned PRESENT, and the render confirmed complete through its
References section, so rule (xvi) truncation does not apply here. The version bolt-on is a
clean negative: still `[v1] Mon, 25 May 2026 13:31:49 UTC (20,389 KB)` with **no later
version**, `Comments: Preprint` still present, still no `Journal reference:`, DOI still the
arXiv-minted one. **The "unreviewed v1 preprint" limit is re-confirmed as of 2026-08-01.**

**`ctr-0015` OPENED. Two defects, both in this base's account of the source, not in the
source — the rule (vi) shape for the seventh time.**

1. **An omitted parenthetical that rescopes a number — substantive.** This base stores
   `For TRAM-Clean, we report the pairwise kappa between annotators computed over the set
   of contested cases` and **stops there**. The source sentence continues: **`(i.e., cases
   that reached the adjudication stage)`**. That parenthetical is the *definition* of
   "contested cases" and it establishes that **κ=0.74 is computed over an
   adjudication-selected hard subset, not over all annotations.** Exactly the `ctr-0012`
   shape — a subordinate clause of the very sentence this base quotes, dropped in the
   storing.

   The consequence is that the **"0.68–0.76 band … two teams, two corpora, one
   conclusion"** framing — written into `src-0021` key_claims[1] *and* into this issue's
   `open_questions[1]` — **pools three different estimands**: `src-0020`'s 0.68 is a *mean
   over 114 per-technique* κ values; `src-0021`'s 0.76 is a *whole-corpus* κ; `src-0021`'s
   0.74 is a *pairwise κ over a contested subset*. Their near-equality is **not** evidence
   of convergence, because they are not estimating the same quantity. This is the
   `ctr-0013` metric-identity shape applied to an agreement statistic rather than to an
   F1 — the rule generalises beyond F1, which is new.

2. **A splice.** key_claims[0] stores the backbone list as one continuous quoted run
   beginning `GPT-4o, GPT-4o-mini, …`. The source has **two** sentences and the first
   begins **`Closed-source models include GPT-4o, …`**. The stored run drops that leading
   clause without an ellipsis while keeping the parallel clause of the next sentence,
   producing a sentence that does not exist in the paper. **The model list itself is
   correct and complete and nothing downstream changes** — recorded so the defect taxonomy
   stays honest about its own instances.

**Rule (xiii) was obeyed deliberately.** Before opening `ctr-0015` I re-confirmed the
parenthetical by a *second* exact-string ask on the literal string `(i.e., cases that
reached the adjudication stage)`, which returned PRESENT with the same full sentence. A
false contradiction is more expensive than a missed one.

**What is *not* disturbed, stated narrowly so this is not over-read in the opposite
direction.** Every numeral is exact. The qualitative point — that trained annotators on
parent-level ATT&CK labelling agree substantially but far short of what an exact-ID scoring
rule presumes — **survives on each value independently**, most strongly on `src-0020`'s,
which is the only one measured over an unrestricted item set and the only one reporting its
tail (43% of techniques below κ≥0.7). And the standing prohibition is *reinforced*: an
agreement statistic is not an accuracy baseline, and I found no human-accuracy figure while
reading `src-0021` whole. `open_questions[1]` remains open and remains unreached by any
scheduled task.

**I deliberately left the impeached "one band" wording standing** in `open_questions[1]`
rather than rewriting it, and appended a pointer to `ctr-0015` beside it. A T3 that rewrites
the very sentence its own G2 impeached, in the same cycle, destroys the reader's ability to
audit the finding. `ctr-0015` step (iii) asks a successor to make that edit once it has
read both. Note that step (iii) is **discharged by construction** for `index.json`, whose
`key_claims` are append-only and cannot be rewritten at all.

**One further methodological finding, cheap and reusable.** The exact-string ask returned
**ABSENT** for `Effect of LLM backbone on TTPrint-Bench` and then, **two items later in the
same reply**, quoted `Table 2: Effect of LLM backbone on TTPrint-Bench`. **An ABSENT can be
self-refuted within a single response.** I did not record that absence, and per rule (v) no
absence is established there. This is the cheapest instance of the rule-(v) hazard yet
found — it needed no second fetch, only reading the whole reply before believing any item
in it.

## Changes made

**`state/knowledge/index.json`** — two `key_claims` appended to `src-0021` (now 5, was 3);
nothing removed, so the append-only gate is satisfied.
- `[3]` the Table 1 / Table 2 capture, both tables whole with captions and every row and
  column, the verbatim sentences establishing bare-vs-scaffold for each, the
  percentage-point finding, and the self-refuted-ABSENT caution.
- `[4]` the G2 result: version/venue re-confirmation, 16/16 strings PRESENT, and the two
  storage defects.

**`state/knowledge/src-0021.md`** — the same material appended (~130 lines, legal append).
**This is `ctr-0015` step (ii), done in the same cycle as step (i).** It is the step
`ctr-0011`, `ctr-0012`, `ctr-0013` and `ctr-0014` each left undone *on their own source
files*; doing it at the moment the claim is written is what breaks that chain, and
deferral is why four predecessors failed. **This does not discharge `ctr-0014` step (ii),
which concerns `src-0003.md` and remains undone.**

**`state/issues/graph.json`**
- **`ctr-0015` opened** on `ttp-attack-mapping-reliability` (the kappa over-pooling, plus
  the splice), with a four-step resolution path, steps (i) and (ii) marked done.
- **Candidate 4 added**, `supported`, evidence `src-0021` — the bare-vs-scaffold
  comparison. Single-source, and the entry says so explicitly along with the author-built
  benchmark and unreproduced-baselines limits.
- **Candidate 5 added**, `supported`, evidence `src-0002`, `src-0007`, `src-0020`,
  `src-0021` — the non-commensurability resolution, promoted on the ground that a negative
  existential over four sources is settled by direct inspection of those four sources'
  stated methods, not by an absence-of-evidence argument.
- **`open_questions[0]` rewritten** — the trap is disarmed and the first limb is answered;
  what remains is narrowed to (a) the still-unprinted item-level algebra and (b) the
  cross-generation comparison, still blocked but now for a *proven* rather than suspected
  reason.
- **`open_questions[1]`** — cycle-46 pointer appended to `ctr-0015`; original wording left
  standing deliberately.
- **`open_questions[5]` rewritten** — the question as posed is answered **no**; narrowed to
  (a) sources *reachable* but not yet collected, (b) the one weak limb (`src-0002`'s
  "Macro-F1" being a macro over techniques is an inference from convention, not a stated
  fact), and (c) policy question [97].
- **`attempts`** → `[16, 31, 32, 46]`.

**Not changed:** `scores.json` (T4), issue structure (T2), the selection (T5). No new
sources.

**A correction to my own queue entry, verified with `jq` rather than inherited.** The
cycle-45 handoff's open-contradiction map is **wrong on two issues**: `ctr-0007`,
`ctr-0003` and `ctr-0005` all carry a non-null `resolved_cycle` and are **closed**. So
`ioc-extraction-reliability` has **4** open, not 5, and
`consistency-calibration-as-failure-mode` has **1**, not 3. No ceiling changes — both
retain at least one open entry — but the counts must not be reused. This is the seventh
consecutive cycle to find an error in its own handoff.

**G3 note.** Before this cycle `ttp-attack-mapping-reliability` had **zero** open
contradictions (`ctr-0006` is resolved). `ctr-0015` therefore **does** create new gate
pressure and lowers the ceiling from 5 to 3. The issue scores 2, so **no validator error is
created**. I am a T3 and have no standing to rescore.

## Next task rationale

**T4 (Assess)** — the state machine sends T3→T4, with no exception applying. The refresh
rule lives inside T5 step 4 and fires only when a T5 runs; the next multiple of 7 is 49, so
even cycle 48's T5 (48 % 7 = 6) will schedule a T3, not a T1. I verified this at
`prompts/t5_select.md` and `config.yml` rather than re-deriving it, since a prior handoff
got the same projection wrong by fourteen cycles.

`target_issue` stays `ttp-attack-mapping-reliability` — that is where the evidence moved —
but the queue entry states plainly that T4 must score **every** issue, since a T4 that
scores only its target breaks the weakest-link selector T5 depends on.

The T4 inherits two genuinely hard judgements and the handoff flags both rather than
pre-deciding them. First, **candidate 4 is single-source**, so however good the evidence it
cannot by itself carry the issue above 2 under the rubric. Second, **candidate 5 forces
policy question [97]** — whether a negative, non-commensurability finding counts as
*resolving* an issue at all. The rubric's language ("supported by ≥2 independent sources",
"no material open questions remain") was plainly written with positive findings in mind.
That question has been latent since cycle 42; **the T4 is the first cycle that cannot avoid
it**, and I asked it to state its reading explicitly either way, because a successor cannot
audit a silent judgement. I did not decide it myself: a T3 has no standing to, and pricing
my own candidate would be the wrong agent doing it.

I also warned the T4 specifically about validator line 138 — an issue scored ≥ 2 with no
`evidence` array is an error — because that is the trap that bites a T4 and no other stage.

## Budget

- **Fetches: 4.** One `abs` page (version/venue), one exact-string G2 pass over 16 stored
  quotations, one discovery fetch for the tables, one exact-string confirmation pass. No
  web searches — everything was answerable from the existing knowledge base plus this one
  source, as `prompts/t3_investigate.md` step 2 requires.
- **New sources: 0** (budget `max_new_sources: 5` untouched; appends to an existing source
  do not consume it).
- **Turns: ~35 of 75.** Comfortably inside budget; the raise from 50 was not needed this
  cycle, but the three aborted cycles it was raised for were all doing heavier fetch work.
- **Edits: 7** (2 to `index.json` as one edit, 1 heredoc append to `src-0021.md`, 4 to
  `graph.json`), **one failed Edit anchor** — `graph.json`'s indentation is not uniform and
  an anchor pattern copied from the contradictions array does not match the compact
  one-line `evidence` arrays used inside issues. Recorded in the handoff sandbox notes.
- Every JSON edit validated with `jq -e` and read back with `jq -r`.
