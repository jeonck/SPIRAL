# Cycle 96 — T3 (Investigate) — `consistency-calibration-as-failure-mode`

## Task performed

**T3 (Investigate)** against `target_issue` `consistency-calibration-as-failure-mode`, per
`prompts/t3_investigate.md`, which I read at source rather than trusting the queue entry's
account of it.

**Phase check, first thing.** `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed cycle 95 touching `index.json`, `src-0002.md`, both queue files,
`meta.json` and its log — no `graph.json`, consistent with a T5 that opened nothing. All three
confirming checks cycle 95 nominated passed exactly: `srcs=32 kc2=10`, `ctr=50 open=37
last=ctr-0050`, `last=94 total=19 n=9`. Cycle 95 completed. I am the T3.

**Refresh rule, re-derived rather than inherited.** `config.yml` `schedule.collect_refresh_every`
is 7 and the rule substitutes a task *type* at a **T5 only**. The ladder from here is 97=T4,
98=T5. 98 = 7 × 14, so the T5 at 98 fires the refresh and schedules a T1 at 99. Cycle 95's
arithmetic checks out. Nothing about it touches cycle 96 or 97.

**Budget note on the entry itself:** cycle 95's `next_task.json` was well-sized and cost me no
paging calls. Mine is 15,138 bytes.

**What I did.** One G2 re-check (below) and then the spine cycle 95 recommended:
**`open_questions[8]`**, opened at cycle 62 and open for thirty-four cycles. I did not do part of
it — I closed **all three** of its remaining limbs, because each turned out to be a single fetch.
**Five fetches, zero searches, zero new sources, one contradiction opened.**

### The modality sweep — `open_questions[8]` is CLOSED, and all five limbs answer in the negative

`open_questions[8]` asked what confidence-elicitation method every confidence-bearing or
abstention-bearing source in this base uses, and named five unknowns. Two were closed earlier
(src-0002 at cycle 74 by `ctr-0034`; src-0018 at cycle 75). The three it called "still the whole
of this question" — **src-0014, src-0015, src-0016** — were untouched since cycle 75 and are
closed here, by one **five-exact-string sweep** each (`confidence`, `logit`, `log probabilit`,
`verbaliz`, `self-consistency`), in verbatim mode with an explicit ABSENT option plus a direct
truncation question. That is exactly the method the question itself prescribed and which had
already worked three times.

**All three elicit no model-expressed confidence of any kind.**

- **src-0014 (PromptAudit).** `confidence` / `logit` / `log probabilit` / `verbaliz` all
  **ABSENT**. The construct is majority-vote disagreement over resampled generations, verbatim:
  "The model produces multiple independent predictions using the A-CoT template under the active
  output protocol" and "When no label receives a true majority across the requested samples, the
  final output is treated as an abstention (UNKNOWN)", with coverage = one minus abstention rate.
  So the 24.04–60.85% coverage collapse is a *behavioural* selective-prediction quantity, not a
  confidence number. ECE / Brier / reliability diagram: **ABSENT**, re-confirming at source by
  direct question what `key_claims[3]` has asserted since cycle 15.
- **src-0015 (OpenSec).** **All five strings ABSENT** on a COMPLETE v3 render. The construct is
  an action threshold — false-positive rate and EGAR. This **confirms at source, by direct
  question and for the first time**, the scope note `key_claims[3]` has asserted since cycle 20:
  this source's "calibration" is not ECE/Brier confidence calibration.
- **src-0016 (Snyk VulnBench).** **All five strings ABSENT.** The decisive question — *is any
  recurrence count anywhere interpreted as a confidence or a probability?* — returned **ABSENT**.
  So `open_questions[8]`'s own phrase "a sampling-based confidence signal **in all but name**"
  turns out to have been doing real work: the vendor draws no probabilistic reading from the
  recurrence counts at all. Any cycle treating a 5-of-5 recurrence as a confidence estimate is
  making *this base's* inference, not Snyk's.

**The aggregate result, entered as `candidate_resolutions[13]` (supported).** In this knowledge
base, **exactly two** sources elicit a model-expressed confidence value, and they use **different
modalities on different domains**: src-0001 (token-probability, CTI) and src-0013 (verbalized,
code security) — that difference is `ctr-0027`, still open. **Six** others have now been checked
one by one at source and all six are **behavioural** (src-0002, src-0018, src-0031, and the three
new ones). Consequences: src-0001 is not merely the only CTI source reporting ECE and Brier, it
is the **only CTI source in this base that elicits model-expressed confidence in any form**, and
**nothing in this base replicates it** — not on modality, not on domain, not on model. The
pattern cycle 79 called "not a coincidence" on three sources now holds at **six for six**. The
issue's two conjuncts, consistency and calibration, are supported by **non-overlapping evidence**
and cannot presently be resolved together.

**The consequence for `candidate_resolutions[2]`, and it is a split, not a promotion.** That
candidate aggregates src-0013/0014/0015/0016 into one corroboration of "overconfidence and
under-refusal", and `open_questions[8]` forbade promoting it until these modalities were known.
Now that they are: the four cases are **one** measurement of model-expressed confidence
(src-0013) plus **three** measurements of behaviour. Per rule (xxi) the conjunction comes apart —
**under-refusal is the *stronger* half** (a behavioural property triangulated three different
behavioural ways) and **overconfidence is effectively single-source** (a property of *expressed*
confidence, which only src-0013 measures). I appended that ruling and **left status at
`proposed`**: splitting or rewriting a proposed candidate is a T2 structuring act, not a T3's,
and I declined to do it by fiat.

**Two one-form limits, spun out as named jobs rather than buried.** `ctr-0044` step (iii) sat
undone for seven cycles because nobody named it. **JOB A:** src-0014's render self-reported
TRUNCATED at "6. Conclusion", so its appendices did not render — the third such failure on this
source — and every src-0014 ABSENT in this base is **body-only**. **JOB B:** src-0016's sweep is
one-form on a page with a documented false-ABSENT history (`ctr-0011`); it should be re-run
against `?print=1`. Neither blocks `candidate_resolutions[13]`, which rests on affirmative
mechanism quotations as well as on the ABSENTs.

### A fetching rule worth more than the finding: unversioned ar5iv is version-stale

My first src-0015 pull used `ar5iv.labs.arxiv.org/html/2601.21083` and returned **ABSENT for the
restraint sentence this base has quoted since cycle 20** — a stored, cycle-37-verified quotation.
Rather than record a failed re-verification, I re-fetched the pinned `arxiv.org/html/2601.21083v3`,
which returned it **PRESENT and exact**, along with the full EGAR definition.

A discriminating pull settles which render is wrong: **the ar5iv abstract is the v1 abstract word
for word** (90–97% FP rates, Sonnet at 85%/72%, metric set "TTFC, blast radius, injection
violation rates" with **no EGAR**). **ar5iv served v1 for a paper whose latest version is v3.**
The rule: for any multi-version arXiv paper, an **unversioned ar5iv URL must not be used**, and
any ABSENT from one is worthless. **This is not a false-ABSENT of the `ctr-0011` kind** — the
render was faithful, to the wrong document — and I did not file it as one.

**A named uncollected job discharged as a by-product.** `key_claims[5]` recorded the v1 Table 1
as *summarised, untrustworthy and not entered*, saying a cycle wanting it must re-fetch v1
verbatim. The ar5iv pull **is** that verbatim v1 re-fetch and confirms the table exactly (caption
and all twenty cells). Note v1 carries a `Corr.` column v3 lacks and **no EGAR column**. The
companion half is **not** closed: the alleged v1 "95% Wilson CIs for rates" sentence is still
unconfirmed — `confidence` returned ABSENT on the v1 render, which is evidence against it but not
decisive, since a Wilson-interval sentence need not contain the word *confidence*.

## Retrospection

**G2 target, derived not inherited.** The round-2 order is *cycle N re-checks the conclusion the
next unexamined predecessor entered*: 93 took 84's, 94 took 85's, 95 took 86's, so 96 takes
**cycle 87's**. I verified this by reading `logs/cycle-087.md` rather than trusting the queue
entry. Cycle 87 entered several conclusions; `candidate_resolutions[10]`'s cycle-87 ruling has
already been audited (`ctr-0048`, cycle 89), so I took the one with the most riding on it:
**`src-0028` `key_claims[3]`**, the all-occurrences ECE result that cycle 87 used to execute
step (ii) of `ctr-0044` and, on its strength, to **resolve `ctr-0044`**.

**Method.** Same question, same URL (`r.jina.ai` over the PMLR PDF), as an independent second
pass, with the query written to **full `ctr-0049` discipline** — exhaustive enumeration demanded,
*several / multiple / various* forbidden, an explicit PARTIAL-decline option offered, a total
count demanded. Render self-reported COMPLETE.

**Verdict: PARTIAL FAIL. The conclusion survives and is strengthened; the count fails.**
`ctr-0051` is the record of it.

The second pass disagreed with the first **in both directions**. It reproduced cycle 87's
Section 2 sentence verbatim (and showed it continues with a citation cycle 87 cut). It returned a
**fourth naming sentence no cycle has ever recorded** — "The ECE is a special case of top-label
confidence calibration…". It **failed to enumerate** cycle 87's Appendix A and Appendix F.4
sentences — yet **quoted the Appendix F.4 one back inside its answer to a different question**,
which proves the sentence is in the render and that the *enumerator*, not the render, dropped it.
And **three of the five** sentences it did enumerate contain neither literal string at all. It
nonetheless declared "TOTAL COUNT: 5 sentences" with **no PARTIAL flag**.

- **Survives, over-determined:** "src-0028 **names** ECE" — two independent passes, four distinct
  naming sentences between them. So `candidate_resolutions[10]`'s "Neither source names ECE" is
  false on that limb beyond doubt, and **`ctr-0044`'s resolution at cycle 87 stands and is not
  reopened**, because it rested on that limb being true.
- **Fails:** the **count**. At least four; true number **not established**. No cycle may cite a
  count of ECE occurrences in src-0028 — cite presence, cite the quotations.
- **Not upgraded:** the "names but does not define" ABSENT and the ECE-vs-Brier ranking ABSENT are
  both **the same render family**, now taken twice and three times respectively. Neither is a
  two-form confirmation and I did not record either as one.

**The methodological result, and it revises four cycles of doctrine.** Cycles 93, 94 and 95
established that an *under-specified* query yields an under-reporting answer, locating the defect
in the **query**. This query was fully specified to that rule and **still** produced a
confidently-wrong enumeration — false positives, omissions, an unflagged total. **Query
discipline is necessary and not sufficient.** The operational rule I propose: an enumerated
**count** from this tooling is not evidence of a count in either direction, whereas an individual
**verbatim quotation** returned by a render remains good evidence of **presence**. *Presence is
recoverable; counts are not.* I applied this to my own work in the same cycle: the three modality
sweeps are entered as **presence/absence findings with their counts explicitly discarded**, and
src-0014's returned count of 14 (two of which contain the string nowhere) is not entered.

**A G2 that failed is a success of the system, and this one paid twice** — it caught a defect in
a cycle-87 claim *and* it produced the rule that made the same cycle's own sweeps honest.

## Changes made

**`state/issues/graph.json`** (~900KB, read and written through `jq` only):

1. **`ctr-0051` opened** against `consistency-calibration-as-failure-mode`, `opened_cycle` 96,
   `resolved_cycle` null. Five schema keys, key order matched to `ctr-0050`.
2. **`candidate_resolutions[13]` added** — status `supported`, evidence `src-0001, src-0002,
   src-0013, src-0014, src-0015, src-0016, src-0018, src-0031`, key order `summary, evidence,
   status` matched to the preceding entries.
3. **`candidate_resolutions[2].summary`** — a `=== CYCLE 96 ===` block **appended**. **Status
   unchanged at `proposed`; evidence array unchanged.**
4. **`open_questions[8]`** — a `=== CYCLE 96: CLOSED ===` block appended. The other nine
   untouched.
5. **`attempts`** — `96` appended (now `3,15,16,25,39,62,75,80,87,96`).

**`state/knowledge/index.json`**: one `key_claim` appended to each of **`src-0014`** (5→6),
**`src-0015`** (7→8), **`src-0016`** (6→7) and **`src-0028`** (4→5). **No source added;
`.sources|length` still 32.**

**`state/knowledge/src-0014.md`, `src-0015.md`, `src-0016.md`, `src-0028.md`**: `## Cycle 96
addendum` sections appended via the Edit-tool idiom (carry-forward [234]). Nothing deleted or
rewritten. Cheap trick worth carrying: `wc -l` first, then `Read` with `offset` near the end,
instead of reading whole files just to satisfy Edit's read requirement.

**Append-only proof (carry-forward [221]), 25 checks across two `jq -n` calls, all true.**
`index.json`: source count 32→32; the four touched sources' old `key_claims` each equal a prefix
slice of the new; the 28 untouched sources byte-identical; all 32 sources identical after
`del(.key_claims)`; `_schema` identical. `graph.json`: contradictions old array equals new
`[0:50]`; the eight non-target issues byte-identical; the target issue identical after
`del(.attempts, .candidate_resolutions, .open_questions)`; `attempts` old equals new `[0:9]`;
`candidate_resolutions` old equals new `[0:13]` after `del(.summary)`; the twelve *other*
summaries byte-identical; `candidate_resolutions[2].summary` old is a `startswith` prefix of new;
`open_questions` length 10→10 with the nine others byte-identical and `[8]` a `startswith`
prefix; `_schema` identical. Key sets confirmed on the new objects.

**Gate hand-check** (I cannot run `scripts/validate_state.py` — `python3` is refused in this
session). **G1:** no new source, no new URL; every new claim cites `src-0014`, `src-0015`,
`src-0016` or `src-0028`, all of which exist in `index.json` with resolving URLs I fetched this
cycle. **G2:** performed and recorded above. **G3:** `ctr-0051` opened. **Ceiling:**
`scripts/validate_state.py` lines 144–156 builds the open set as a **set comprehension over
`issue_id`**, so the demotion is a **flat ceiling** of `scale_max − g3_contradiction_demotion`
= 5 − 2 = **3**, not a per-contradiction subtraction. This issue already carried five open
entries and scores 2, so the sixth imposes exactly what the first did and **breaks no gate**.
`prompts/t4_assess.md` step 3 reads the same rule as a per-point subtraction and **the two
disagree — carry-forward [4], eighty-third cycle, awaiting a human. I followed the validator**,
as cycles 76/81/85/88/91/94 did. **Schema:** key sets and orders checked as above.

**Queue:** `state/queue/next_task.json` written for a **T4 (assess)**, `target_issue` null,
`attempt_count` 0, **15,138 bytes**. `state/queue/last_completed_task.txt` reads
`T3 investigate`. Scratch file `scratch-096.json` removed.

**Not changed:** `state/assessments/scores.json` — byte-untouched, as a T3 should leave it.

## Next task rationale

**T3 → T4** per the state machine. The refresh rule substitutes a task *type* at a T5 only, so it
does not touch cycle 97; 98 = 7 × 14 puts it at the T5 two cycles out, and I told that cycle to
re-derive it rather than inherit my arithmetic.

The T4 entry names the four decisions its cycle actually turns on rather than re-splicing state.
**(1)** Carry-forward **[97]** — whether a *negative* or non-commensurability finding may itself
count as a resolution — bites harder here than it ever has, because
`candidate_resolutions[13]` is precisely that kind of finding and is the largest single piece of
work this issue has received in twenty cycles. The T4 must **state the score under each reading
and record which it applied**, not resolve the policy. **(2)** `ctr-0051` is **not** a reason to
lower the issue: the ceiling was 3 before and is 3 after. Cycle 80 caught itself about to make
the opposite error. **(3)** Carry-forward **[4]**: follow the validator, say so, patch nothing
from inside the loop. **(4)** Carry-forward **[30]**: `automated-triage-under-refusal` is still
the only issue never investigated in 96 cycles — price it, don't redesign the policy.

I also flagged, for the T4 to price without acting on: the total has been **19 for eight
consecutive assessments** with no individual score moving in **twenty-two cycles**, so the score
vector carries no ordering information and the tie-break has decided every selection since cycle
73. If cycle 97 leaves it at 19 again it should say **explicitly** whether that is a finding about
the research or about the scoring instrument. That is evaluation data for the paper. And a note
for the T5 at 98, not for the T4: appending 96 to this issue's `attempts` means it now carries a
3(b) attempt penalty it did not carry at cycle 95.

## Budget

- **Web fetches: 5, all usable, no discards.** (1) `r.jina.ai` over the PMLR PDF — G2, the ECE
  all-occurrences re-run. (2) `arxiv.org/html/2605.24171` — src-0014 five-string sweep.
  (3) `ar5iv.labs.arxiv.org/html/2601.21083` — src-0015 sweep, which turned out to be **v1**.
  (4) `arxiv.org/html/2601.21083v3` — src-0015 sweep on the pinned version, which caught (3).
  (5) `ar5iv…/2601.21083` again with a discriminating query — established (3) was v1 and
  verbatim-confirmed the v1 Table 1.
- **Web searches: 0.** Everything came from sources already in the base, as
  `prompts/t3_investigate.md` requires.
- **New sources: 0.** All five fetches were re-verifications or characterisations of existing
  sources, so nothing counted against `max_new_sources`.
- **Turns: ~34 of 75.** The cheapest cycle in a while: no paging calls (cycle 95's queue entry was
  well-sized), and the four `index.json` edits went in as a single four-way `if/elif` inside one
  `map(. as $x | …)`.
- **Cheapest thing that produced the most:** fetch (4). One extra fetch, spent on *disbelieving my
  own ABSENT*, converted what would have been a false "stored quotation fails re-verification"
  report into a reusable fetching rule (unversioned ar5iv is version-stale) **and** discharged a
  named uncollected job from cycle 37. The instinct worth carrying: **when a render contradicts a
  stored, previously-verified quotation, suspect the render before the record.**
- **Most expensive near-miss:** had I trusted fetch (3), I would have opened a spurious
  contradiction against `src-0015` `key_claims[1]` and impeached the single most-quoted sentence
  in this issue's evidence base.
