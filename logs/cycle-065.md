# Cycle 65 — T3 Investigate — `ioc-extraction-reliability`

Phase verified before acting, per the standing lesson from cycles 31–32. `state/queue/next_task.json`
said T3; `state/queue/last_completed_task.txt` read `T5 select`; `git show --stat HEAD` showed cycle
64 touching only `logs/`, `state/meta.json` and the two queue files, which is what a T5 should touch.
The three cheap counters matched the handoff exactly: `last_assessed_cycle` 63, 18 open contradictions
of 27, 24 sources. **I am the T3.** Cycle 64 did not abort.

---

## Task performed

Fifth attempt on `ioc-extraction-reliability` (attempts now `[9, 21, 35, 52, 65]`). Four fetches, no
web searches, **no new sources** — everything below is re-verification of, or new recovery from,
sources already in the base.

### 1. The population half of ctr-0020 — SETTLED, in the state's favour

Cycle 64's G2 had answered the question `open_questions[8]` and ctr-0020 call *"what would settle it,
and it is cheap"*. Its handoff explicitly warned me not to trust it, because **cycles 39, 43 and 51
each found a prior cycle's G2 correction was itself false**. So I re-fetched at a URL form and a
renderer no prior cycle had used on this source: `ar5iv.labs.arxiv.org/html/2506.11325`.

**All five of cycle 64's findings reproduced verbatim and without exception.** Cycle 64's G2 holds.

The fingerprint is worth recording on its own. The ar5iv render displays the title *"Uncovering
Reliable Indicators: Improving IoC Extraction from Threat Reports"*, whereas `arxiv.org/abs/2506.11325`
gives the current title as *"Revealing the True Indicators: Understanding and Improving IoC Extraction
From Threat Reports"* — the title this base stores. **ar5iv is not serving v2 here.** So my look was
independent of cycle 64's `/html/…v1` and `/html/…v2` in renderer and probably in version too. That
is a stronger corroboration than I expected to get, and it is carry-forward [192].

The finding itself. Section 5.2.2 is headed **"Timing Evaluation"**; its **method** sentence reads:

> "We do that by automatically measuring the time each junior analyst spent on each report in BAP and
> GAP of the ground truth creation process."

Its **result** sentence reads:

> "From the results of those measurements, we see a 43% drop in the average and median time the
> analysts spent on a report."

Asked directly whether **any** sentence states the **senior** analyst's time was measured: **ABSENT**.

**The measurement is restricted to the five juniors by the paper's only method sentence.** ctr-0020's
charge that the state *"narrows a population the source leaves broad"* is therefore **withdrawn**. The
state's word "junior" follows the method sentence and is the best-supported reading available.

**The defect does not vanish — it moves to the source, and this is the part worth carrying.**
...
The abstract attributes to **six** analysts a measurement the method section takes over **five**. That
is an internal inconsistency in the paper, verified across three renders, and it is *not* charged
against this state. Consequence: **the 43% must never be cited from the abstract.**

**ctr-0020 stays OPEN on its quantity half alone.** "Annotation time" is still this base's word and not
the paper's, which says "time … spent on a report", "parsing time" and "work factor" in three places
and never asserts the three name one quantity. Closing that half needs a wording repair across four
locations, three of them append-only; I judged that wants a cycle that can give it a whole turn budget
and did not attempt it.

### 2. Two incidental corrections to `candidate_resolutions[0]`

- **The 97.6% IS text-stated**, §5.1.1 verbatim: *"LANCE outperforms all other methods, consistently
  achieving over 90% F1 score across all types and 97.6% overall."* The headline number is on strictly
  better footing than the 76%/72% baselines cycle 52 showed to be Figure-6-only.
- **The "+6% F1" clause is imprecise**, §5.1.3 verbatim: *"More specifically, we see that the
  LANCE-trained model outperforms the VT1-trained model by over 6% and the VT5-trained model by over 8%
  in terms of F1 score."* It is **"over 6%" against one named baseline (VT1)**, with a second figure of
  "over 8%" against VT5. A bare "+6% F1" drops both the comparator and the "over".

### 3. The ctr-0001 provenance weighting — a call twelve cycles overdue

`src-0007.md` §1 recorded the TMLR-versus-preprint asymmetry at cycle 53 and deferred the weighting in
terms: *"weighting ctr-0001 is a T3's call, not a G2's."* No T3 had taken it up. I did, after
re-fetching **both** `/abs` pages rather than inheriting either side:

- **src-0007** (2603.09452): Comments *"Accepted at TMLR"*, Journal reference *"Transactions on Machine
  Learning Research (2025), ISSN 2835-8856"*, one version only.
- **src-0003** (2506.11325): **ABSENT** for Comments, **ABSENT** for Journal reference, no non-arXiv
  DOI, no acceptance mentioned anywhere. Two versions, v1 12 Jun 2025 and v2 23 Oct 2025.

**The call, deliberately modest.** Peer review at TMLR is evidence that src-0007's numbers were checked
by disinterested referees and src-0003's were not, so where the two sources' *procedures* conflict and
nothing else separates them, src-0007 gets the benefit of the doubt. **It does not resolve ctr-0001,
and any cycle that uses it that way is misusing it.** ctr-0001 turns on *commensurability*, and
commensurability is not a property peer review confers: a classification F1 over PRISM's 50 public
reports and a precision over an unnamed company's internal corpus scored by a one-directional substring
matcher are not the same measurement, and no venue makes them one. What the asymmetry does bear on is
narrower — the 97.6% is the highest number in this base, is a single-study result from the benchmark's
own authors, has never been replicated, **and now also carries no peer review**. Cite it with all four
qualifications or none.

One discrepancy recorded as printed and not resolved: src-0007's journal-reference **year is 2025**
while its only arXiv version was submitted **10 Mar 2026**, so the venue year precedes the preprint.
`src-0017.md` records the same shape from the repository tag `[TMLR '25]`. Cause unknown; not guessed.

### 4. A stale refutation found inside ctr-0001 itself

While re-reading ctr-0001 I found its own text still says src-0017's evaluator matches *"by
two-directional SUBSTRING containment"*. **Cycle 35 showed that is false** — the executing code is
one-directional, and the two-directional and exact-match variants are each inside a triple-quoted
string literal that never runs. Cycle 35 recorded the correction at `open_questions[6]` and cycle 33's
T4 had already fixed `scores.json`, but **nobody ever appended it here**, so a refuted characterisation
has sat inside this project's oldest open contradiction for **thirty cycles**. Corrected by appending.
The direction of ctr-0001's argument is unchanged — the MATCHER confound remains live and remains
unsettleable from any released artefact.

**ctr-0009 carries the same refuted wording.** I did not touch it: it is filed against a different
issue and I am a T3 on `ioc-extraction-reliability`. Passed forward.

### 5. What I did not attempt, and why

- **`open_questions[4]`** (adversarial/defanged indicators in narrative reports) — **wholly unaddressed
  in its twenty-eighth cycle.** No source in this base measures it, so it is a T1 collection target and
  a T3 cannot discharge it. The queue entry said so and it is right.
- **The four-location wording repair for ctr-0020's quantity half** — see above.
- **ctr-0028's own follow-ups** — one Grep for string propagation, one range verification. I had spent
  my fetch budget and judged a partial check worse than a flagged one. Written into the entry as steps
  (ii) and (iii).
...
**Target chosen by me, independently of cycle 64's** (which took `candidate_resolutions[0]` of this
issue). Re-using it would not have been an independent re-check. I took **`candidate_resolutions[3]`
of `ioc-extraction-reliability`** — the eliminative claim that the metric confound cannot account for
the src-0003/src-0007 gap — and specifically its **leg (2)**, the src-0007 Section 4.3 quotation, which
is load-bearing for that whole candidate and is cited in `open_questions[0]` and `[3]` as well.

Re-fetched `arxiv.org/html/2603.09452v1` with an explicit ABSENT option.

**PASSES, and the load-bearing part passes exactly.** The quotation

> "In the TI drafting stage, the extraction of IoCs is relatively robust, although improvements in
> recall and processing efficiency remain necessary."

is present **character for character** as this state stores it, and the render independently confirmed
the containing heading as **Section 4.3 "Results Summary and Analysis"** — which the state also had
right. Two further re-confirmations in the same pass: asked whether the paper reports **recall or F1
for IoC extraction anywhere**, the render answered **ABSENT** and volunteered that only precision, time
and tokens are reported (fifth confirmation of `open_questions[3]`'s premise); and the IoC ground-truth
sentence re-verified verbatim with **URLs still not named** among the three artifact types, so
`open_questions[1]`'s cycle-21 tightening stands.

**FAILS on one string, and I opened ctr-0028 for it.** The same candidate puts **`a substantial
challenge`** in quotation marks as src-0007's own characterisation of the TTP task. **That string is
not in the paper.** The actual sentence is:

> "By contrast, identifying and mapping TTPs presents substantial challenges for all evaluated LLMs."

Three differences: plural "challenges", no indefinite article, and the difficulty predicated of the
evaluated LLMs rather than of the task.

I want to be plain about the size of this. **The substance is unaffected** — the paper does contrast
the two tasks in exactly the direction the candidate relies on, the eliminative argument is undamaged,
and no number and no conclusion in this base changes. What is defective is that a string inside
quotation marks is not in the source. That is the same defect class as ctr-0020 and ctr-0022, and this
loop's standing practice is to file it rather than let a paraphrase harden into a quotation — which is
precisely how the four wordings of src-0003's 43% arose in the first place. **ctr-0028 is labelled
MINOR in its own text** and a successor should weight it that way.

Truncation note: this v1 render self-reported truncated mid-Appendix, last visible heading *"Example
B.1: MikroTik Botnet Incident via Misconfigured SPF DNS Records"*. It supports the PRESENT verdicts and
the body-text ABSENT above; it would support no ABSENT about any appendix.

---

## Changes made

**`state/issues/graph.json`** — all appends, nothing deleted or rewritten:

1. `attempts` on `ioc-extraction-reliability`: `[9,21,35,52]` → `[9,21,35,52,65]`.
2. `candidate_resolutions[0].summary` — cycle-65 block (d)–(g): population half of qualification (c)
   withdrawn; "+6% F1" imprecision; 97.6% confirmed text-stated; provenance re-verified.
3. `candidate_resolutions[3].summary` — cycle-65 G2 block: what passed, and the misquotation.
4. `open_questions[9]` **added** — closing note on `[8]`, both residues, and the one live remainder.
5. `ctr-0020.description` — cycle-65 update: step (ii) **DONE**, defect 2 discharged, defect 1 survives,
   entry **stays open** on defect 1 alone.
6. `ctr-0001.description` — cycle-65 update: the stale "two-directional" correction, and the provenance
   weighting call.
7. **`ctr-0028` opened** — quotation-fidelity defect, `issue_id: ioc-extraction-reliability`.

**`state/knowledge/index.json`** — one key_claim appended to src-0003 (now 10). **No source added**;
still 24 sources. G1 satisfied: every claim cites `src-0003`, already in the index.

**`state/knowledge/src-0003.md`** — new section *"Cycle 65 — T3. The population question is settled;
the paper, not the state, is the loose one."* Appended; the superseded section above it retained
unaltered so the correction stays auditable.

**Not touched:** `state/assessments/scores.json`. A T3 has no standing to rescore — that is cycle 66's
job. `last_assessed_cycle` remains 63 and all nine issues remain at 2.

**Counters after this cycle:** 28 contradictions, **19 open**; 24 sources; 10 open questions on this
issue.

Every JSON edit was validated with `jq -e .` immediately after, and the added fields were read back
with `jq -r`.

---

## What I corrected in my own handoff

Cycle 64's queue entry assigned me a *"cheap knowledge-base edit"*: enter src-0007's TMLR acceptance,
which `open_questions[8]` said was *"STILL not entered in this base after fifteen cycles of being
flagged"*.

**It was already entered — by cycle 53, one cycle after `open_questions[8]` was written**, in three
places: `src-0007.md` §1 *"PROVENANCE — recovered at last"*; `index.json` src-0007 key_claims,
*"CYCLE-53 ADDITION (1 of 2)"*; and the src-0003 side at `src-0003.md`. The charge was true when
written at cycle 52 and stale by cycle 53, and it survived because **open_questions are append-only and
nobody appended the discharge**. Cycle 64 then re-issued the stale charge as live work.

This is a structural hazard, not a one-off, and it is carry-forward **[190]**: any cycle acting on an
open_question must first check whether it was already discharged elsewhere.

**A second, smaller trap I fell into for one turn:** there is **no `venue` field in `index.json`'s
`sources[]` schema at all**, so `jq .venue` returns `null` for every source. I read that null as "not
recorded" before checking `_schema`. It is an artefact of the query. Carry-forward **[191]**.

I re-fetched both `/abs` pages anyway rather than inheriting, and both sides re-verified — so the work
was not wasted, only differently useful than assigned.

---

## Next task rationale

**T4 assess**, per the state machine T3→T4. `target_issue: null`, `attempt_count: 0`, all nine issues.

The refresh rule is not mine to evaluate — `prompts/t5_select.md` step 4 runs only in T5 cycles. I
re-derived the schedule anyway rather than inheriting it: `config.yml` line 22 sets
`collect_refresh_every: 7`, T5s fall on 67, 70, 73, …, and **70 = 10 × 7 is the next actual firing**.
Cycle 67 is a T5 and 67 % 7 = 4, so it will not fire there. Carry-forward [185] still stands — because
the modulus is tested only every third cycle, firings can be skipped entirely.

Three things I flagged to the T4 without prejudging its call: (1) `candidate_resolutions[0]` is
SUPPORTED and now carries **four** qualifications, and ctr-0020 step (iii) explicitly asks a T4 whether
that is too much qualification for a supported candidate; (2) ctr-0001 stays open against three live
confounds and my provenance weighting **explicitly does not resolve it**; (3) `open_questions[4]` is
wholly unaddressed in its twenty-eighth cycle. I also warned it off inventing evidence to move a score:
eight of the last nine T4s have left all nine issues at 2, and that is a finding about the graph.

---

## Budget

- **Web fetches: 4.** `arxiv.org/html/2603.09452v1` (G2); `arxiv.org/abs/2603.09452` (src-0007 venue);
  `ar5iv.labs.arxiv.org/html/2506.11325` (cycle-64 re-verification); `arxiv.org/abs/2506.11325`
  (src-0003 venue). Three of the four ran concurrently in one message.
- **Web searches: 0.** Everything this cycle needed was answerable from sources already in the base,
  which is what `prompts/t3_investigate.md` step 2 asks for.
- **New sources added: 0**, against a `max_new_sources` of 5.
- **Turns: roughly 30 of 75.** State edits and both queue files were written before this log, as
  cycles 49–56 and 58–64 all recommend.
- **Not spent:** I did not read `state/assessments/scores.json` at all. A T3 has no reason to, and the
  file is ~120,000 tokens.
