# Cycle 149 — T2 structure

## Task performed

T2 structure on `state/issues/graph.json`, target issue `institutional-incident-real-world-impact`.
Zero searches, zero fetches. No source added, no score moved, no issue created, nothing deleted,
no issue split, no `depends_on` edge touched.

Phase confirmed at source before acting: `state/queue/last_completed_task.txt` read `T1 collect`;
`git show --stat HEAD` (8c1e465) touched `state/knowledge/index.json`, five new `src-0049`–`src-0053`
files, `state/meta.json`, both queue files and `logs/cycle-148.md`, and neither `graph.json` nor
`scores.json` — exactly a T1's footprint. `prompts/system.md:45-46` gives T1→T2, so this cycle is a
T2. The three confirming counts all matched the queue entry: `srcs=53 last=src-0053`,
`ctr=70 open=52 issues=10 last=ctr-0070`, `last=146 n=10 total=22 max=3`.

The queue entry listed nine candidate work items and said plainly that no cycle would do nine.
Five were done completely. Four plus the split were left, and they are named under
**What I left** below.

### (1) Executed ctr-0069's cycle-140 ruling — honesty item (F), discharged after nine cycles

This was the oldest undischarged ruling in the base. The remedy is an edit to `graph.json`, which
the ruling T4 could not make; cycles 141–148 could none of them take it.

Appended to `institutional-incident-real-world-impact` `candidate_resolutions[3]`: the
sector-alignment sentence `THE SPLIT IS 3 DECLINE / 1 VOLUNTEER AND IT LINES UP WITH SECTOR.`
**stands unerased** and is now scope-restricted — the alignment is observed within the
voluntary-publication stratum only and is untested outside it. The stratum labelling the ruling
requires is recorded there:

- **Stratum V, voluntary publication** — no tribunal, no duty of candour, no adversary with an
  incentive to verify: ENISA (src-0004), EY Canada (src-0012), KPMG (src-0035, src-0036, src-0044),
  PwC Middle East (src-0040, src-0037). The 3 decline / 1 volunteer split and its sector alignment
  are statements about this stratum and nothing else.
- **Stratum T, tribunal** — src-0048, and src-0048 alone.

Cycle 140's four prohibitions are carried verbatim into the append: no promotion of
`candidate_resolutions[9]` (it stays `proposed`), no restatement of the tally as 3 decline / 2, no
report of the commercial-versus-Big-Four confound as broken (a forum confound replaces it and no
source in this base tests forum), and no new factual claim or source. ctr-0069 is **not** closed:
its (7)(iii) and (7)(iv) are untouched.

### (2) The candidate_resolutions[9] supersession note cycle 148's G2 asked for

Paragraph (4) of that candidate prescribes promotion *if* the case is ruled in. Cycle 140 ruled it
in and expressly did not promote, on independent grounds (one Reuters wire behind two carriers, no
primary artefact, unresolvable quotation/paraphrase boundary). The prescription was a T3 deferring
to a named authority that decided differently — the system working, not a defect — but the text as
written reads as standing authority to promote on admission. The append records the supersession
and leaves the original in place. No contradiction opened: G3 governs two *supported* claims and
this candidate is `proposed`.

A second, smaller caution went in with it. Paragraph (5) calls the commercial/Big-Four confound
`broken`; cycle 140 says it is **not** broken. The two agree on the forum replacement and diverge
only on the word. Cycle 140's wording governs — the scope ruling was its to make, and `broken`
unqualified would licence dropping the predictor on a single out-of-stratum case. Binding form:
no state text may report that confound as broken without also recording that a forum confound
replaced it and that no source tests forum.

### (3) Closed three contradictions and wrote the convention honesty item (E) has wanted since cycle 135

`ctr-0064` (KPMG partitive), `ctr-0066` (src-0003 pipeline conflation) and `ctr-0060` (PwC
unverified retention limb) all now carry `resolved_cycle = 149`. Open contradictions: **52 → 49.**

I did not inherit the "all steps discharged" claim. For each entry I re-ran at first hand the step
addressed to a file the original reporting role could not write:

- **ctr-0064 step (ii)** — sweep `graph.json` and `scores.json` for unqualified renderings of the
  KPMG removal. Clean negative in both. In `graph.json` the only bare `KPMG removed the report`
  string is inside ctr-0064's own warning against writing it; every other mention carries the
  partitive. In `scores.json` every mention carries a qualifier.
- **ctr-0066 step (ii)** — sweep all ten rationales for the human-validation-pipeline and
  `regex+LLM+human` phrasings. Zero instances.
- ctr-0066 steps (i) and (iii) I read at first hand in `ioc-extraction-reliability`
  `candidate_resolutions[0]` and `[1]`; both cycle-142 appends are present and complete.
- ctr-0060 steps (i) (cycle 114) and (ii) (cycle 128) are recorded in the entry itself; step (iii)
  I discharged this cycle — see (4).

**The proposed convention**, written out in full in ctr-0064's cycle-149 update: a T2 may set
`resolved_cycle` on a contradiction whose every repair step is discharged, provided it (a) names
each step and says where and when it was discharged, (b) re-runs or re-reads at first hand at least
those steps addressed to a file the original reporting role could not write, and (c) restates in the
closing note any standing rule the entry carries, because a closed entry is still read but is no
longer counted. Ground for (a)/(b): the failure mode ctr-0010 records. Ground for vesting it in a
T2: `graph.json` is a T2's file, closure is a statement about the graph and not about the world, and
no role can close an entry it cannot edit. **This is a proposal by one cycle and a human may
overturn it** — and one cycle proposing a rule and applying it in the same cycle is not the same
thing as the base having a rule. If it is overturned, the three closures should be reversed by
setting `resolved_cycle` back to `null`, not by deleting the cycle-149 updates. It does **not** rest
on cycle 136's closure of ctr-0065, which was a one-off queue authorisation.

ctr-0062 was **not** closed: its step (iv) is genuinely undischarged, so it is not a clean instance.

### (4) Discharged ctr-0060 step (iii) — one of the three pending standing checks, work-owed (18)

Ruling: **adopt, but reworded**, because the check as worded is not runnable. Cycle 142 established
two-render that src-0003 has no Limitations, Threats-to-Validity or Discussion section, so a check
keyed to a section heading returns a null on one of the most relied-on sources in the base, and that
null is indistinguishable from "the source states no limits".

Adopted wording: whenever a cycle enters or re-affirms a candidate resting on a single source, it
must compare the limits the candidate states for itself against the limits the *published source*
states for itself wherever those appear — a named section, an abstract hedge, a scope sentence, a
future-work sentence — and must record a null as a null, saying which kind of null it is.

One disambiguation the check needed, because the object count changed under it at cycle 148: the
only heading literally called `Limitations` anywhere in this base is the **editorial** note this
state's own T1s write into `state/knowledge/src-*.md`, and cycle 148's five new files each carry
one, so there are now six. The check means the published source and never the editorial heading. A
cycle that reads the editorial heading and calls it the source's limitations has checked this base
against itself. The check is not a fetch commission.

### (5) Discharged ctr-0037 site (2) — work-owed (20)

Correction appended to `ttp-attack-mapping-reliability` `candidate_resolutions[1]`. The truncated
range `0.8240-0.8846` appears **twice** in that candidate — once in the original cycle-15 claim (b)
and once in the cycle-31 withdrawal text that restates claim (b) in order to withdraw it. ctr-0037
names the second as site (2); the append covers both, since they carry the same defect in the same
words. Correct four-model range from src-0007 Table 4: **0.6944-0.8846** (GPT-4o 0.8240, o3-mini
0.8503, GPT-4o (FT) 0.8846, GPT-4o-mini (FT) 0.6944), with the standing rule that 0.6944 is never
read as a before/after pair.

Claim (b) stays withdrawn. The correction makes the withdrawn within-table control *weaker*, not
stronger: the range it appealed to spans a model materially worse at mechanical IoC extraction than
the truncated form implied. Recorded for whoever next scores that issue — cycles 140, 143 and 146
all gave this site as a reason `ttp` cannot reach level 4 even if the G3 ceiling were lifted, and
that reason is now gone. Six open contradictions still sit on the issue. A T2 does not score and I
did not.

Two observations recorded in ctr-0037's update without acting on them: site (1)
(`task-dependent-reliability-framing` `candidate_resolutions[1]`) already carries a ctr-0037
correction naming 0.6944; site (3), the task-dependent rationale in `scores.json`, also carries a
correction block naming 0.6944, although the truncated string still stands uncorrected at five
earlier points in that same rationale. That file is a T4's and I did not touch it.

### (6) The ctr-0010 note — work-owed (32), and the item was partly wrong as written

Work-owed (32) has asked since cycle 126 for "a note appended to ctr-0010 recording the discharge"
of step (i). **That note has existed since cycle 109.** ctr-0010's cycle-109 update already states
that step (i) was discharged at cycle 36, that the cycle-108 update is wrong where it says
otherwise, that cycle 109 read the `scores.json` block at first hand, and that all three steps are
discharged. I read it directly before writing anything. So the remedy (32) commissions was not
owed; what a reader inherits is the cycle-108 error, which the append-only rule requires be
retained, and the only available remedy for that is a second pointer — which is what I appended.
No repair was re-done.

What *was* actually missing is the thing cycle 109 could not do. Cycle 109 offered a standing
methodological rule, expressly declined to adopt it because adopting one is not a T4's charter, and
flagged it for a T2 or a human. **I adopted it, in cycle 109's own words:** no cycle may report a
repair step as undone without reading the file the step names; reporting it as unverified is always
available and costs nothing. It binds every role, including the one that writes the queue entry.
Three instances in this base's own history support it: ctr-0028 step (ii) (done cycle 66, listed
pending 42 cycles), ctr-0010 step (i) (done cycle 36, reported undone at cycle 108), and work-owed
(32) itself. ctr-0010 stays open — ctr-0001's metric confound is still live.

I did **not** read ctr-0053 step (iii) or ctr-0056 step (iii) and rule on nothing there.

### (7) One new open_questions entry on the target issue

`institutional-incident-real-world-impact` `open_questions[9]`, recording six structural questions
raised by cycle 148's five sources and **ruling on none of them**: whether the population admits a
government department; whether it admits a case whose AI attribution is external and inferential
(MAHA is not an AI-attributed incident on src-0051's evidence, and entering it into an
AI-attributed population would be a G1 failure, not a judgement call); whether the volunteer/decline
axis needs a fourth value for src-0051's explicit refusal to confirm or deny, and whether src-0049's
hedged causal sentence fits the volunteer cell; whether spokesperson-level and principal-level
responses must be separated; how many strata forum has, given that src-0052's congressional
testimony is neither V nor T; and where the src-0049/src-0053 hedge-stripping event belongs. The
entry carries the standing cautions that travel with any answer.

Deliberate: the stratum machinery those questions would be answered with was defined only this
cycle, and a scope ruling made in the same cycle that built the machinery would be untested by
anyone.

### What I left, and why

- **work-owed (30)**, the T3-verbatim-strings ruling — the cycle-148 entry called it the
  clearest-cut pending T2 item and I did not do it. Reason: the graph schema has no conventions
  container, so the ruling would have to be buried in an entry whose topic it does not match, and I
  had already used ctr-0060 and ctr-0064 for two other conventions this cycle. It is carried forward
  with the asymmetry a successor needs to rule well: the role that *establishes* such a string is a
  T3, which may not write `src-*.md`, so any adopted rule binds a future T1 at its next touch of
  that source. Still five instances.
- **work-owed (17)/(6)**, the src-0006 node or edge — twenty-four cycles unentered. Left for turn
  budget; it is the smallest of the remaining items and should be the next T2's first act.
- **ctr-0062 step (iv)**, work-owed (19) — the cross-reference into two issues. Left; it needs a
  careful reading of ctr-0001's cross-study/within-study boundary that I did not have turns for.
- **The other two pending standing checks** — `candidate_resolutions[18]` paragraph (5) and ctr-0070
  step (iii). One of three adopted this cycle; two remain.
- **The issue split** (open_questions[6]) — declined, with the reason recorded in the graph as the
  T2 prompt asks: the split case is stronger than when cycle 72 wrote it, but splitting in the same
  cycle that first executed the ctr-0069 stratum ruling would distribute eleven open contradictions
  and a brand-new stratum labelling across two halves with no cycle having yet used either, and the
  honest order is to let a successor apply the machinery once before the graph is cut around it.
- **incident-evidence-vendor-concentration**, work-owed (29) — the second blocker on revisiting it
  (the unexecuted ctr-0069 ruling) is gone as of this cycle, but restructuring it is a separate
  piece of work and I did not start it.
- **work-owed (36)**, placing the hedge-stripping finding — recorded as a question, not placed.

## Retrospection

**G2 target: cycle 140's entered conclusions.** The round-2 order took 143–148 through cycles
134–139, so 140 is next in line. Cycle 140 was a T4 assess; a T4's entered conclusions live in
`state/assessments/scores.json`, and c730fdf touched that file and nothing else in `state/` except
meta and the queue — confirmed by `git show --stat c730fdf` rather than by its message.
`git show c730fdf:state/assessments/scores.json > old140.json`. **Zero fetches**; scratch removed.

**Result: PASSES on every test, and it settles a question nine cycles have asked.**

1. *Do the ten scores still stand?* At c730fdf: `last=140 n=10 total=22`, all ten `assessed_cycle=140`.
   Compared entry-by-entry against the current file: **all ten identical** (ttp 3, vendor-concentration 3,
   the other eight 2; total 22). Nothing has silently drifted in nine cycles.
2. *Does any rationale restate the tally as 3 decline / 2?* **No.** A permissive regex over all ten
   rationales returned three hits, all in the institutional rationale, and reading each at its offset:
   one is the correct `3 decline / 1 volunteer`, and the other two are cycle 140's own prohibition —
   verbatim, `IT DOES NOT LICENSE RESTATING THE TALLY AS 3 DECLINE / 2, IN THIS FILE OR ANYWHERE ELSE
   ... the tally is not restated as 3/2 anywhere in the state and must not be.`
3. *Does any rationale call the commercial-versus-Big-Four confound broken without recording the forum
   replacement?* **No.** Four `broken` hits in the institutional rationale; the two in the ruling region
   are the sentence `IT DOES NOT REPORT THE COMMERCIAL-EXPOSURE-VERSUS-BIG-FOUR-BOILERPLATE CONFOUND AS
   BROKEN. It is not broken; a FORUM confound is opened in its place and no source in this base tests
   forum.` The two other file-wide hits (extraction-vs-reasoning, automated-triage) are unrelated usages.
4. **The one cycle 148 most wanted answered — did cycle 140 anywhere treat its ctr-0069 ruling as
   executed? No.** Its own list of what the ruling does not do has a limb (d) reading, verbatim,
   `IT IS NOT EXECUTED`, naming the remedy as a `graph.json` edit a T4 does not make, and adding
   `UNTIL IT IS EXECUTED, candidate_resolutions[9] STILL READS proposed IN THE GRAPH AND SIDE A STILL
   READS UNAMENDED, and a successor must check the graph rather than assume this ruling has landed.`
   The nine-cycle report of *pending* was accurate throughout, and cycle 140 is the source of its own
   accuracy. **No contradiction is opened and none is owed.**

This G2 was direct preparation for work item (1), exactly as the queue entry predicted, and the
execution above follows cycle 140's own limb list rather than any later account of it.

## Changes made

`state/issues/graph.json` only (plus `state/meta.json`, the two queue files, and this log).
`state/knowledge/` and `state/assessments/scores.json` are untouched — a T2 adds no source and moves
no score.

Nine edits in one two-input jq splice, verified before `mv`:

| Container | Edit |
|---|---|
| `institutional-incident-real-world-impact` `candidate_resolutions[3].summary` | append (+3.6k), ctr-0069 execution |
| `institutional-incident-real-world-impact` `candidate_resolutions[9].summary` | append (+2.6k), supersession note |
| `institutional-incident-real-world-impact` `open_questions` | one new entry, index 9 |
| `ttp-attack-mapping-reliability` `candidate_resolutions[1].summary` | append (+1.9k), ctr-0037 site (2) |
| `ctr-0069.description` | append +1293 |
| `ctr-0064.description` | append +3231, `resolved_cycle = 149` |
| `ctr-0066.description` | append +2079, `resolved_cycle = 149` |
| `ctr-0060.description` | append +2668, `resolved_cycle = 149` |
| `ctr-0037.description` | append +1507 |
| `ctr-0010.description` | append +2749 |

**Append-only proof, run before `mv` in four jq calls** (this is carry-forward item [399]):
`_schema` unchanged; contradiction id list identical; all 70 descriptions `startswith` their old
value; all 70 contradictions equal under `del(.description, .resolved_cycle)`; issue id list
identical; every issue equal under `del(.candidate_resolutions, .open_questions)`; every candidate
`summary` `startswith` its old value with `status` and `evidence` unchanged and the candidate count
unchanged in all ten issues; `open_questions` prefix-equal in all ten with delta 0 everywhere except
`institutional-incident-real-world-impact` at +1. Nothing was erased, reordered or rewritten.

**Post-edit counts, hand-checked with jq** (I cannot run `scripts/validate_state.py` here — the bash
sandbox refuses `python3`, so this is a hand-check and not a validator run):
`ctr=70 open=49 issues=10 last=ctr-0070`. Per-issue open contradictions re-derived, summing to 49:
attribution-confident-wrong-gap 3, attribution-expressed-confidence-unmeasured 4,
automated-triage-under-refusal 5, consistency-calibration-as-failure-mode 9,
extraction-vs-reasoning-ordinal-axis 2, incident-evidence-vendor-concentration 1,
institutional-incident-real-world-impact 11, ioc-extraction-reliability 4,
task-dependent-reliability-framing 4, ttp-attack-mapping-reliability 6.

**G1**: every source id named in new text (src-0003, src-0004, src-0007, src-0012, src-0035,
src-0036, src-0037, src-0040, src-0044, src-0048, src-0049, src-0050, src-0051, src-0052, src-0053)
exists in `state/knowledge/index.json`, whose ids are contiguous src-0001–src-0053. No new source,
no new URL, so nothing new for the URL gate to resolve.

Issue count stays 10 — inside the 5–12 healthy range. No merges, so nothing to record under the
merge rule.

## Next task rationale

State machine at source (`prompts/system.md:45-46`): T2 → T3. Cycle 150 is a **T3 investigate**,
`attempt_count` 0. The refresh rule is not a T2's to compute; per [390] it keys on a T5's own cycle
number and the next refresh falls at cycle 154.

**Target issue: `institutional-incident-real-world-impact`** — the issue T5 selected at cycle 147
and the one cycles 148 and 149 both worked. Three reasons: it holds eleven open contradictions, the
largest block in the graph; five sources entered it at cycle 148 and nothing has been structured
from them beyond the questions I opened this cycle; and the stratum machinery any ruling on them
needs was defined only this cycle and **has never been applied by any cycle**. The queue entry names
five ranked pieces of work on it — read the five `src-*.md` bodies and answer what `open_questions[9]`
asks; enter a candidate for the South Africa case if the evidence supports one; ctr-0069 (7)(iii)
and (7)(iv); the stale route ledger at `open_questions[8]`; and work-owed (30). It also says plainly
that `prompts/t3_investigate.md` governs and that a T3 judging another issue riper — the obvious
alternative is `automated-triage-under-refusal`, whose work-owed (25) is addressed to exactly that
role and which has never been investigated in 149 cycles — should say so and say why.

The entry carries the whole ban set and work-owed list forward, updated: (13), (20) and (32) marked
discharged with what replaced them; (18) reduced from three pending standing checks to two; honesty
item (E) rewritten as answered-in-part-by-proposal rather than unruled; honesty item (F) marked
discharged with a warning not to re-execute; and a new item (37) telling whoever next scores
`ttp-attack-mapping-reliability` that the ctr-0037 reason for its level ceiling is gone and must be
re-derived rather than inherited. G2 for cycle 150 falls on cycle 141, a T5, so the re-check is a
re-derivation of the selector run from `prompts/t5_select.md` at source against the state at
0d0630d — which bears directly on honesty item (B).

## Budget

Zero web searches, zero fetches — the whole cycle ran off files already in the repo, as a T2 should.
Roughly 45 assistant turns: 5 reading the 68KB queue entry (via the Grep tool at 400-character
slices, [392]'s cheaper cousin), 6 on phase and count checks, 5 on G2, 14 reading graph regions and
re-running the two closure sweeps at first hand, 4 on the splice and its append-only proof, 4 on the
queue files, and the rest on this log. Comfortably inside `max_turns: 75`.

## Carry-forward items — NEW AT CYCLE 149

- **[397] A carried-forward work item can outlive its own remedy.** Work-owed (32) commissioned a
  note that had already been written forty cycles earlier, and this base has two earlier instances of
  the same failure running in both directions (ctr-0028 step (ii) reported pending after being done;
  ctr-0010 step (i) reported undone after being done). Before executing a carried-forward remedy,
  read the file the remedy names and check it is not already there. Now a standing rule of the base,
  adopted this cycle in cycle 109's words: *no cycle may report a repair step as undone without
  reading the file the step names; reporting it as unverified is always available and costs nothing.*
- **[398] `transpose` plus `startswith` is [383] in a new costume.**
  `[$a,$b]|transpose|map(.[1].summary|startswith(.[0].summary))` fails with `Cannot index string with
  number`, because the argument of `startswith` is evaluated with `.` set to that filter's own input —
  the string, not the pair. Bind first: `map(.[0] as $x|.[1] as $y|$y.summary|startswith($x.summary))`.
- **[399] The three-part whole-file append-only proof.** Cheaper than hashing and stronger than a spot
  check. Bind old and new with the two-input form and assert, per container: (a) the id list is
  identical, (b) every string field in the new file `startswith` its old value, (c) every field you did
  not intend to change is equal under `del()` of the ones you did. For arrays that may grow, add
  (d) `new[0:len(old)] == old` and report the length delta explicitly. Ran over 70 contradictions and
  10 issues in four calls.
- **Reading a huge single-line JSON field without a shell call.** The Grep tool with pattern
  `.{1,400}`, `output_mode: content`, `-o: true` and `head_limit`/`offset` paging reads a 68KB
  single-line field in four calls. 400 characters is under the tool's long-line omission threshold;
  800 is over it and every match comes back as `[Omitted long matching line]`. This is a second route
  to [392] and costs no Bash calls.
- **A convention proposed and applied in the same cycle is not yet a convention.** The `resolved_cycle`
  authority rule in ctr-0064's cycle-149 update is a proposal by one cycle. It is recorded as such in
  honesty item (E), with the reversal path written down (set `resolved_cycle` back to `null`; do not
  delete the updates), precisely so a human overturning it does not have to unpick prose.
