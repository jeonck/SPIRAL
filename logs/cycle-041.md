...
Ran **T4 — Assess** per `prompts/t4_assess.md`, which I read myself rather than relying on the queue
entry's account of it. Step 1 requires scoring **every** issue in the graph, so this task carries no
`target_issue`.

**Phase verification before acting**, checking all three signals and disbelieving the commit message.
`state/queue/next_task.json` held a T4 created at cycle 39 with `attempt_count` bumped to 1;
`state/queue/last_completed_task.txt` read `T4 aborted mid-cycle, rc=1 (attempt bumped)`;
`git show --stat HEAD` showed the cycle-40 commit touching only `logs/cycle-040.md`,
`logs/cycle-040-transcript.txt`, `state/meta.json` and the two queue files. `state/meta.json` reads
cycle 41. Phase confirmed: **cycle 41 = T4**, second attempt of the cycle-39 queue entry.

**Cycle 40 aborted (rc=1) and its entire `state/` output was reverted.** Its log survives in git and
is 1,218 lines long, but `scores.json` still read `last_assessed_cycle: 36` when I opened it and
`graph.json` contained no `ctr-0012`. **Nothing `logs/cycle-040.md` says it changed in `state/`
actually exists.** I treated that log as a prior cycle's *claim*, not as state — methodological rules
(vi) and (xiv) — and re-derived every judgement from `graph.json`, `index.json`,
`prompts/t5_select.md`, `config.yml` and `scripts/validate_state.py` directly. This is the second
abort in four cycles (38 and 40).

**Result: all eight issues scored. No score moved.** Seven at 2, one at 3.

| issue | score | prev | assessed_cycle | open contradictions | G3 ceiling | bound? |
|---|---|---|---|---|---|---|
| ttp-attack-mapping-reliability | 2 | 2 | 41 | — | n/a | n/a |
| ioc-extraction-reliability | 2 | 2 | 41 | ctr-0001, ctr-0004, ctr-0010 | 3 | no |
| consistency-calibration-as-failure-mode | 2 | 2 | 41 | ctr-0011 | 3 | no |
| attribution-confident-wrong-gap | 2 | 2 | 41 | ctr-0008 | 3 | no |
| task-dependent-reliability-framing | 2 | 2 | 41 | ctr-0009 | 3 | no |
| extraction-vs-reasoning-ordinal-axis | 2 | 2 | 41 | — | n/a | n/a |
| institutional-incident-real-world-impact | **3** | 3 | 41 | **ctr-0012 (opened by me)** | **3** | **no — but now exactly at it** |
| automated-triage-under-refusal | 2 | 2 | 41 | — | n/a | n/a |

Rationale blocks were **appended** to each entry; no prior text was rewritten and no `evidence` array
was altered. `last_assessed_cycle` is 41.

### The schedule has moved by fourteen cycles, and this is the most consequential finding of the cycle

Every handoff and every `scores.json` rationale in this project predicted **the next T1 at cycle 50 or
cycle 57**. I re-derived it from source rather than inheriting it, and **that is wrong**.

`prompts/t5_select.md` step 4, verbatim: *"Refresh rule: if `current_cycle % schedule.collect_refresh_every == 0`,
the next task is `T1` targeting the selected issue (bring in fresh evidence) instead of `T3`."*
`config.yml` line 17 sets `collect_refresh_every: 7`. The test sits **inside** the T5 prompt, so it
fires only when a T5 cycle is itself a multiple of 7. Cycle 40's abort shifted the phase by one, so
the T5 cycles are now **42, 45, 48, 51, 54 …** — and **42 % 7 == 0**.

**The T5 at cycle 42 fires the refresh rule and the next T1 is cycle 43** — the first collect since
cycle 22, arriving fourteen cycles earlier than the state has assumed. I recorded this in all eight
rationales and explicitly superseded every "the next T1 is cycle N" statement above each append. It
changes the character of six of the eight issues: their stated route to a 3 is a *collection* problem,
not a reading problem, and collection has been sixteen cycles away for so long that every rationale
had written it off.

The handoff itself warned that "each abort shifts the parity by one." It did — and it shifted it into
a multiple of 7, which the handoff's own projection had no way to anticipate.

### The one score I had to argue for

`institutional-incident-real-world-impact` is the only issue above 2, my G2 landed on it, and my G2
**opened a contradiction against it**. Holding it at 3 in the same cycle is the most contestable
judgement in this file, so I stated the argument in the rationale in full and I restate its shape here.

The title asks two things — has this reached production at real institutions, and how do institutions
respond when caught. On `consistency-calibration-as-failure-mode` and `attribution-confident-wrong-gap`
this file applies the rule that **the weaker conjunct governs**. I applied it here too rather than
exempting the only issue above 2, and it does not bite, for a reason specific to what `ctr-0012`
damages:

- **Conjunct one is untouched.** Candidate 1 cites src-0009 and src-0010 — ENISA's own publication
  pages, verbatim revision notices — corroborating the Der Spiegel/heise chain independently.
  Candidate 2 cites src-0012 and src-0004: a second incident, a second institution type, a second
  independent investigator.
- **Conjunct two is damaged but not reduced to one source.** `ctr-0012`'s material limb is a defect
  **by selection, not by assertion**: every individual clause of candidate 3 is true, and what fails
  is the *force* of its EY-versus-ENISA contrast. The divergence that survives — over **remedy**
  (withdrawal vs. link-patching) and over **causal attribution** (AI acknowledged vs. human error) —
  is narrower but is still corroborated by src-0009/src-0010 against src-0012, mutually independent
  and independent of src-0004.

I also considered demoting by designating candidate 0 (src-0004 alone, carrier of two of `ctr-0012`'s
three limbs) as primary, and refused: candidate 0 is the cycle-2 original, superseded on the existence
claim by candidates 1 and 2, which were added at cycle 12 *precisely* to break its single-source
dependence. **Designating a superseded candidate primary to force a demotion is the mirror image of
the move this file refuses when it would force a promotion** — refused again this cycle on
`consistency-calibration-as-failure-mode` (candidate 4 is the half-answer) and on
`ioc-extraction-reliability` (candidate 4 is eliminative). The rule has to run in both directions or
it is not a rule.

**What would demote it,** recorded so disagreement is cheap: a successor who judges the surviving
divergence claim too thin after `ctr-0012` — n=2, one of the two now known to have been read through
a truncated quotation for forty cycles — should drop it to 2 and say so plainly. I judged the
surviving claim genuinely multi-source; that is a weighting, not an arithmetic fact.

### The G3 gate becomes load-bearing for the first time in forty-one cycles

With `ctr-0012` open and the issue at 3, it now sits **exactly at the ceiling**
(`scale_max − g3_contradiction_demotion = 3`; `scripts/validate_state.py` lines 152–156 error only on
`> 3`, which I read at source). Every prior application in this project sat at merit 2 with a slack of
one, so the gate has never actually constrained anything before. **While `ctr-0012` is open, any T4
that raises this issue to 4 will fail the validator and have its entire cycle reverted.**

### One sub-question of carry-forward [4] settled, at source

Cycles 33, 35, 36 and 39 each wrote that "nobody has ever specified whether the gate is PER-ISSUE or
PER-CONTRADICTION". For the **enforced** rule the answer is readable off the code, and I read it
myself rather than taking cycle 40's log for it: `scripts/validate_state.py` lines 146–149 build
`open_contra` as a **set comprehension** over `c["issue_id"]`, so duplicate issue ids collapse and the
loop at line 150 visits each affected issue exactly once regardless of how many contradictions it
carries. **The enforced gate is per-issue by construction and cannot be per-contradiction.**
`ioc-extraction-reliability`'s three open contradictions therefore cost it exactly what one would —
the case a per-contradiction gate would bite hardest, now priced, and the answer is zero extra cost.

This does **not** settle [4]. Its substance is that `prompts/t4_assess.md` step 3 and `config.yml`
line 35 prescribe a **subtraction** while the validator implements a **ceiling**, and that conflict is
untouched. I applied the ceiling and refused the subtraction, as cycle 16 ruled and every T4 since has
done. **Thirty-second cycle awaiting a human.**

### What cycle 39's work did to `consistency-calibration-as-failure-mode`

I verified this against `graph.json` and `index.json` rather than against the handoff, and the handoff
was accurate on every item I checked. Cycle 39 closed `ctr-0003` and `ctr-0005` and opened `ctr-0011`
against the same issue, so the **net G3 effect is zero, not headroom**.
...
0.3225 worse than generation 0.294). One row has the metrics disagreeing outright: extraction attack
vector is the *best*-calibrated extraction cell on zero-shot ECE (0.13) and the *worst* on zero-shot
Brier (0.46). So the calibration leg is not merely single-source and single-model — it is **internally
inconsistent between the two metrics its one source reports**. Per step 5, that is downward pressure
on a score already at 2, and it is why a 3 was never in view.
...
and gets caught"*, 11 Jan 2026.

**Why this one.** From the state's point of view it was the only source that had **never had a G2 at
all** — untouched since collection at cycle 1 — and it is the sole or primary support for four of the
five `candidate_resolutions` on `institutional-incident-real-world-impact`, the only issue above 2.
Cycle 39 named it the largest unexamined risk in the base. I am the T4 that scores that issue.

**On the relationship to cycle 40.** Cycle 40 ran this same G2 and reached the same three limbs, then
aborted with its state reverted, so no contradiction ever landed. **I did not inherit that finding.**
Rule (xiv) says a correction is a claim and must be re-verified like any other, and rule (xiii) says a
false contradiction is more expensive than a missed one — so I re-fetched the article and re-derived
all three limbs myself. The two cycles agree limb for limb. **This is the first time in this project
that a source-check has been independently replicated by a different cycle**, and it is worth
recording as a positive result about the method rather than only as a defect count.

### Rule (xv), new: the fetch refused verbatim reproduction, and a verification-mode prompt got it anyway

My first fetch asked for the entire article body verbatim, per rule (i). It **declined on copyright
grounds** and returned a summary — which under rule (ix) is worthless. The second fetch, same URL,
asked eight numbered items each to be answered **PRESENT or ABSENT**, with instructions to quote for
each PRESENT *only the single full sentence containing it, exactly as printed*, and to answer
CANNOT READ if the body was not visible. That returned whole sentences with their subordinate clauses
intact — which is exactly what rule (iv) now demands. **New rule (xv): when a fetch refuses whole-body
reproduction of a news source, switch to verification mode rather than accepting a summary.**

**Access hazard, and the validator cannot see it.** The canonical stored URL now serves a **heise+
paywall teaser**. Cycle 40 found the id-only URL form returns HTTP 404 and that appending
**`?seite=all`** to the canonical URL returns the full body; I used that form. `validate_state.py`
lines 125–127 check G1 liveness for **new sources only**, and the canonical URL still returns HTTP 200
— so this source would pass any liveness check while serving a page containing none of the content the
state cites. **Always fetch src-0004 as `…11136978.html?seite=all`.**
...
Confirmed PRESENT and exact this cycle, as whole printed sentences: *"According to Der Spiegel
magazine, 26 out of 492 footnotes in one of the reports were incorrect"*; the annual-budget sentence
below; the `"human errors"` / `"minor editorial revisions"` pair; the APT29 / Midnight Blizzard
sentence.

### Three defects, all in this base's account of the source → **`ctr-0012` opened**

**Limb (1), the material one — an omitted subordinate clause.** The article reads, one sentence:

> Enisa, which has an annual budget of around 27 million euros, admitted the errors when asked by Der
> Spiegel magazine, speaking of "deficiencies" for which it takes responsibility.
...
`candidate_resolutions[0]`, not in `candidate_resolutions[3]`. Every rendering in this base keeps the
two phrases that make ENISA look deflecting and drops the admission sitting in the same sentence.

The narrow reading survives and I did not over-correct: ENISA **did** attribute cause to human error
and **did** confine the AI's role to "minor editorial revisions", so candidate 0's wording — the
response "did not fully acknowledge AI *attribution*" — is literally true and is not disputed. What
does not survive is `key_claims[2]`'s unqualified gloss "institutional under-acknowledgment", and the
*force* of candidate 3's EY-vs-ENISA contrast. On the source's own words **both institutions publicly
accepted responsibility.**

This is a new sub-shape of the rule (iv) partial-capture class. That class had bitten on unpulled
**rows** (`ctr-0003`) and unpulled **columns** (cycle 39's Brier finding). This is the first time the
omitted material is a **subordinate clause of the very sentence being quoted**. **Rule (iv) is amended:
pull the whole row, the whole column, and the whole sentence.**

It is also a clean counterexample to rule (viii)'s sufficiency. Every stored string in src-0004 matched
exactly. **String-matching found nothing, because the defect was entirely in what was not stored.**

**Limb (2) — over-attribution of report titles.** `key_claims[0]` opens "Two ENISA 'Threat Landscape
2025' reports (published Oct/Nov 2025)…". Asked directly, the fetch returned **ABSENT**: the article
names neither report and refers to them only as two reports published "last October and November". The
titles are recoverable from src-0009 and src-0010, which candidate 1 correctly does — but the key_claim
credits src-0004 with a specification it does not make, and the plural is wrong in substance because
src-0010 is *ENISA Sectorial Threat Landscape: Public Administration*, not a "Threat Landscape 2025"
report. Non-load-bearing; recorded. **Rule (v) compliance on this ABSENT verdict:** confirmed by three
independent reads — cycle 40's full-body fetch, this cycle's summarising fetch (which likewise refers
only to "two of its reports"), and this cycle's verification-mode fetch answering ABSENT directly.

**Limb (3) — a precision drift that runs against the state's own interest.** `key_claims[1]` and
candidate 0 render the fingerprint as **prose** ("a passage attributed the name 'APT29' to Microsoft's
own naming convention"). The source describes a **fabricated hyperlink**: *"For example, a link to a
Microsoft page about the Russian hacker group APT29 also contained this name – but Microsoft itself
refers to the group as Midnight Blizzard."* That is a hallucinated *citation target*, not a factual
naming error in body text. The corrected reading is **stronger** evidence of LLM fabrication than the
state's version, so no candidate is weakened — but the two are different findings.

**Provenance, recorded for the first time.** The article attributes its load-bearing facts to Der
Spiegel in **three** separate places ("According to Der Spiegel magazine"; "when asked by Der Spiegel
magazine"; "Der Spiegel quotes Neumann"). src-0004 is confirmed at source as a **three-layer chain** —
researchers → Der Spiegel → heise. This **hardens** rather than revises the limit already in
`scores.json` that the ENISA AI-causation leg is "reported-but-not-primary". spiegel.de remains
unreachable to this agent.

**Reported plainly, as required: this is the eleventh defect in nineteen source-checks, and the defect
class is now eleven-way** — spliced quotations, unverifiable numbers, unsupported interpretive glosses,
partial table capture, correct-but-hollow entries, correct-source-corrupted-downstream,
over-restriction, over-generalisation, metric-identity, false-positive correction, and new here:
**omitted subordinate clause**.

**I did not act on rule (xiii) carelessly.** The key sentence was returned as an exact whole-sentence
quotation by a fetch explicitly instructed to answer ABSENT rather than infer, and it was independently
reproduced by cycle 40 from a different rendering. This is not a contradiction opened on the strength
of a summary.
...
- `last_assessed_cycle`: 36 → **41**.
- `assessed_cycle`: 36 → **41** on all eight entries.
- A `[CYCLE-41 T4 …]` block **appended** to all eight rationales. No prior text rewritten, no evidence
  array altered. All eight retain non-empty `evidence` (validator line 138).
- **No score changed.** Seven at 2, one at 3.

**`state/issues/graph.json`**
- **`ctr-0012` opened** against `institutional-incident-real-world-impact` (9,783 chars) — the first
  contradiction ever filed against that issue. Three limbs, a four-step resolution path, the
  `?seite=all` access hazard, the verification-mode fetch technique, and the three-layer provenance
  finding. **Nothing else in the file was touched**: no candidate, open_question or attempts array was
  modified. A T4 has no standing to rewrite candidates, and `ctr-0012`'s repairs are explicitly
  assigned to a T3 or T1.
...
Every JSON edit was validated with `jq -e . <file> > /dev/null` and read back with `jq -r` on the
specific fields added. Post-edit G3 check run explicitly: the five issues carrying open contradictions
score 2, 2, 2, 2 and 3 — all `≤ 3`, so no validator error.

---

## Next task rationale

State machine T4 → **T5 (select)**, `attempt_count` 0, `target_issue` null.

**The refresh rule fires on the cycle-42 T5** (42 % 7 == 0), so that T5 must queue a **T1 (collect)**,
not a T3 — the first collect since cycle 22. I put that finding at the top of the queue entry in
capitals, with the derivation shown, and told the T5 to re-derive it from `prompts/t5_select.md` and
`config.yml` itself and to trust its own arithmetic over mine if they differ. I also told it the
`max_new_sources: 5` cap, the G1 URL requirement and the `src-*.md` file requirement, since the T1 it
writes must carry them.

I gave it the full score table with `created_cycle`, `attempts`, `depends_on` and open contradictions
for all eight issues, since step 3 requires it to show tie-break arithmetic and step 20 requires a
ranking table in its log — and told it to re-read both files rather than trust my copy.

Tie-break reminders passed on with the warning that cycle 37 found its *own* handoff had misstated
them: (a) upstream first, (b) attempt penalty +1 per attempt within the last 5 cycles, (c) older
`created_cycle` first. At cycle 42 the only issue plausibly taking (b) is
`consistency-calibration-as-failure-mode` (attempt at 39, three cycles old); I told the T5 to derive
the window itself and say which reading it used.

I flagged the degeneracy plainly, because it is evaluation data: **seven issues at 2 and one at 3
means the tie-break is the selector.** And I restated the starvation proof at full strength —
`automated-triage-under-refusal` has `attempts []` in its 26th cycle, has lost seven consecutive
selections, and cycle 34 proved it can never be selected under tie-break (c) while the
`created_cycle=2` issues stay tied. **This is the cycle where that costs most**, because a T1 is
exactly the task that would discharge the curl/HackerOne item that seven cycles have called the
highest-value uncollected source in the project. A T5 must apply the policy as written and must not
invent an anti-starvation rule; [30] and [55] await a human. But the arithmetic must be recorded so
the exclusion stays auditable.

For the T5's own G2 I recommended **src-0006**, now the stalest source in the base (cycle 18), whose
never-read per-task scoring definitions are load-bearing for *two* issues at once —
`task-dependent-reliability-framing`'s candidate 0 and `extraction-vs-reasoning-ordinal-axis`'s route 1
— and would either secure or destroy the only surviving support on both.

---

## Budget

- Fetches: 2 (src-0004 whole-body → refused on copyright, returned a summary; src-0004
  verification-mode → eight items answered, all whole-sentence quotations). Both on the `?seite=all`
  form. Searches: 0.
- File reads: 8 (`next_task.json`, `meta.json`, `config.yml`, `t4_assess.md`, `t5_select.md`,
  `src-0004.md`, `validate_state.py` lines 125–159, plus three paged reads of `scores.json`, which now
  exceeds the Read tool's 25,000-token cap).
- `jq` projections: ~10 — the cheap route for `graph.json` (~210 KB) and `scores.json`.
- Edits: 11 (8 rationale appends + `last_assessed_cycle` + `assessed_cycle` replace_all + 1 graph
  append), 2 queue writes, 1 log write. **No failed Edit anchors** — the `jq -r … | .[-100:]` tail
  projection worked for all eight, as at cycles 39 and 40.
- Turns used: roughly 30 of 50.
