# Cycle 75 — T3 investigate — `consistency-calibration-as-failure-mode`

## Task performed

**T3 (Investigate)** against `target_issue: consistency-calibration-as-failure-mode`, per
`prompts/t3_investigate.md`, which I read directly rather than relying on the queue entry's or any
log's account of it.

**Phase check first.** `state/queue/last_completed_task.txt` read `T5 select`;
`git show --stat HEAD` showed cycle 74 touching `graph.json`, `meta.json`, both queue files and its
log; `.contradictions|length` read **34** and the open count **25** (the queue entry's stated
post-74 values); `.last_assessed_cycle` still read **73**, which is correct for a T5 and is not
evidence of an abort. Cycle 74 completed. I am the T3.

**Refresh rule, re-derived rather than inherited.** `config.yml` line 22 reads
`collect_refresh_every: 7`. 75 % 7 == 5, so it does not fire — and per `prompts/t5_select.md` step 4
it is a T5's test, not a T3's, so it could not have bound me regardless. The next genuine firing is
cycle 77.

**Where the budget went.** Three fetches, four searches, no new sources added. The single most
productive item cost **zero** fetches.

## Retrospection

**G2 target: `candidate_resolutions[6]` of this issue** (added cycle 62) — the entry that
establishes measured calibration error as a property of the *(model, elicitation method,
temperature)* triple, and which is the **load-bearing basis of this issue's second standing
prohibition** ("no cycle may state a calibration figure without naming the confidence-elicitation
method it was measured under").

**How it was chosen — carry-forward [210], exercised and vindicated.** Cycle 74 warned that a
"pick something never checked" heuristic cost it two turns of search because cycles 25, 39 and 62
had all already re-derived the target it first aimed at, and recommended grepping the logs for a
candidate's distinctive numbers *first*. I ran one Grep for `0.81` across `logs/`. It returned
**only `logs/cycle-062.md` and its transcript** — establishing in a single turn that src-0013's
Token Probability block had been pulled exactly once, at cycle 62, and never re-verified by any
later cycle, while cycle 69's clean pass on src-0013 covered **Table III**, not Table I. One cheap
Grep, a genuinely unchecked and load-bearing target. The heuristic works.

**Verdict: PASSED CLEAN.** Reported as a pass because a clean pass is data too — cycle 69 set that
precedent and cycle 74's failure does not make passes less reportable.

A fourth independent fetch of `arxiv.org/html/2606.31159v1`, asked for cell-by-cell transcription
with an explicit ABSENT/TRUNCATED option and a separate truncation question (answered: not
truncated), reproduced:

- **Token Probability ECE, all cells exact.** GPT-4o-mini 0.81, 0.81, 0.80, 0.80, 0.79, 0.79;
  Gemini-2.0-Flash 0.81, 0.81, 0.81, 0.80, 0.80, 0.80; Qwen3-Coder-Next dashes.
- **Verbalized ECE, all cells exact.** GPT-4o-mini 0.46, 0.46, 0.46, 0.46, 0.47, 0.48;
  Gemini-2.0-Flash 0.25, 0.25, 0.26, 0.25, 0.26, 0.26; Qwen3-Coder-Next 0.41, 0.41, 0.41, 0.41,
  0.42, 0.42.
- The paper's own explanation sentence, verbatim: *"Token-probability estimation performs poorly
  (ECE≈0.80), reflecting the mismatch between next-token likelihood and program-level security
  outcomes."*

All **24 ECE cells** reproduce. The **0.56** figure the standing prohibition cites is confirmed at
matched temperature T=0.00 (Gemini 0.81 token-probability against 0.25 verbalized). I also checked
the entry's arithmetic on the GPT-4o-mini figure, which pairs 0.46 against 0.80 for a gap of 0.34
where the Gemini pairing is taken at T=0.00: **this is not an error** — 0.46/0.80 is a valid
same-temperature pair at T=0.40 and T=0.60. The two figures are simply read at different
temperatures, and both are real. I record this because I initially suspected a defect here and
found none; not manufacturing one is part of the job.

**One minor discrepancy, flagged and deliberately not litigated.** The stored caption renders a
math subscript as `p-hat-c` ("FT% = False Trust (p-hat-c >=0.8 yet insecure)") where this render
gives `p^sec` ("p^sec≥0.8"). This is a markdown-conversion difference on a symbol inside a caption;
nothing in this base cites the FT% definition's subscript, and **no measurement is affected**. It is
not a G3 contradiction — G3 is for supported claims in conflict, and two renderers disagreeing about
how to flatten a subscript is not a claim. Recorded so a successor who sees both strings knows the
difference is already accounted for.

## Changes made

All state edits were made in **two atomic `jq` passes** with prose spliced from scratch files
(carry-forward [211]), validated, then `mv`'d into place. Scratch files deleted.

**`state/issues/graph.json`:**

1. **`attempts` += 75** on the target issue → `[3, 15, 16, 25, 39, 62, 75]`.
2. **Three new `candidate_resolutions`** (indices 7, 8, 9; issue goes from 7 to 10), all `supported`:
   - **[7] The ECE-versus-Brier divergence in src-0001's Table 6 is a one-row artefact.**
     `open_questions[6]` limb (c) — the outlier-row hypothesis cycle 62 raised, declined to test,
     and called "a cheap, purely arithmetic step for a successor with the stored cells". **Zero
     fetches.** All twelve stored family means reproduced exactly first (a third independent
     re-derivation, after cycles 39 and 62). Removing the extraction attack-vector row makes ECE
     and Brier **agree at all three paradigms** — gaps become ECE +0.048 / +0.060 / +0.2347 and
     Brier +0.0607 / +0.0273 / +0.2553, all six positive; the few-shot Brier reversal of −0.0285
     disappears and the nil zero-shot Brier gap of +0.0065 becomes a clear +0.0607. The row is a
     **structural** outlier, not merely an inconvenient one: its signed ECE−BS discrepancy is
     −0.33 / −0.30 / −0.31 where every other row in the table falls within −0.14 to +0.11 and the
     typical row is within 0.04.
   - **[8] src-0018 elicits no model-expressed confidence of any kind.** `logit`,
     `log probability`, `token probability`, `verbalized` all **ABSENT**; `confidence` occurs only
     as "95% confidence level" and "Evidence confidence levels". Its uncertainty constructs are
     behavioural — abstention via an explicit `None`, and cross-run decision consistency. Carries
     the first verbatim capture of the `P_o` / `D-bar_o` definitions and of the stopping-rule
     sentence.
   - **[9] The elicitation-method effect is larger than the prohibition states.** Discharges
     ctr-0026 paragraph (10)'s deliberately-unentered single-form lead by confirming the
     Sampling-based-Consistency and Self-Consistency blocks on a second render family (ar5iv), every
     endpoint reproducing.
3. **`open_questions[4], [5], [6], [8]` appended to** (never overwritten — `+=` in jq, so the
   existing text is preserved byte-for-byte).
4. **ctr-0035 OPENED.**
5. **ctr-0026 RESOLVED** (`resolved_cycle: 75`), with a resolution note appended to its description.

**`state/knowledge/index.json`** — src-0018 gains a **sixth** `key_claims` entry (append-only;
nothing rewritten). **`state/knowledge/src-0018.md`** — gains a `## Cycle 75 — T3` section.
Both discharge **ctr-0035 step (ii) in the same cycle that opened it**, so it does not join a repair
backlog whose oldest member (ctr-0008 step (i)) is now in its 45th cycle. ctr-0035's description was
then amended to record step (ii) as done, so no successor redoes it.

**Counts after:** contradictions **35** (was 34); open **25** (unchanged — one opened, one resolved);
issues 9; target issue's candidate_resolutions 10. Every contradiction carries exactly
`id,issue_id,description,opened_cycle,resolved_cycle`; every candidate_resolution carries exactly
`summary,evidence,status`. All three evidence ids cited (src-0001, src-0013, src-0018) exist in
`index.json`. No new sources were added, so **G1 is trivially satisfied** — no new URL to resolve.

### The four substantive findings

**(1) The divergence is a one-row artefact — and this does NOT lift the prohibition.** I am explicit
about this in the entry itself because the temptation is obvious. Dropping a row is a **post-hoc
analytic choice made after seeing which row flips the mean**; the paper prints all nine rows and
treats them alike. A gradient that holds on both metrics only after the analyst removes the single
most inconvenient row is *not* a gradient established on both metrics. What is established is
narrower and more useful: the divergence is **localised and explained**, not a general disagreement
between ECE and Brier — which matches cycle 62's finding that src-0013's per-model *printed* values
show no such divergence. Limb (c) is closed; limbs (a) and (b) remain open, and (b) — a
methodological source on binned calibration error versus proper scoring rules — is now the oldest
never-attempted item on that question and needs no CTI content at all.

**(2) The second-source search finally ran, and came back negative.** `open_questions[5]` has been
this issue's highest-value **unsearched** item for 23+ cycles; cycles 39 and 62 both spent their
budgets elsewhere and said so. Four searches on distinct angles found **no source measuring ECE,
Brier or reliability diagrams on a CTI task other than src-0001**. The gap is now *attested* rather
than merely unattempted, which is a real change in what a T4 may conclude. Two new leads killed:
the FIRST 2025 Mezzi PDF (**same first author as src-0001 — fails independence on its face**, though
it may serve `open_questions[7]`'s provenance question); and "CTIConnect" at arXiv 2510.11974, which
is **the same arXiv id as CTIArena**, already killed at cycle 25 — the search engine returned two
titles for one identifier.

**(3) ctr-0035 — a load-bearing metric definition that had never been read.** src-0018's `P_o`
counts alternation *"among the values that the expert annotators defined as **valid**"* — instability
**within the accepted-answer set**, which can be strictly positive while accuracy is perfect on every
run. src-0001's quantity is run-to-run variation in **correctness**. `candidate_resolutions[4]` — the
only thing making the consistency half two-source, and this issue's principal asset — has paired
these as corroborating since cycle 25. I state precisely what survives: the **core claim is not
withdrawn** and the two-source status **stands**; both studies do measure repeat-run output
instability on CTI at temperature 0, and the independence argument is untouched. What is impeached is
the *extra strength* the cycle-39 rewrite claimed from a "matched" control — the temperature control
is matched, **the measurand is not**. This is the ctr-0027 shape, third instance on this issue: two
studies held comparable across exactly the degree of freedom that carries the weight.

The entry also supplies a **second, permanent ground** for the no-magnitude-comparison rule. src-0018
uses a variable-N stopping rule ("until the point estimates reached a 95% confidence level with a
relative precision of less than 5%") against src-0001's fixed ten re-prompts with a fixed seed. Until
now the rule rested only on the contingent fact that src-0018's numbers are locked in images.
**Even if those images are ever read, the comparison remains forbidden.** The lock outlives its
original reason.

I also checked myself here: I nearly recorded src-0018's decoding configuration as unstated, before
finding cycle 39 had already recovered *"the LLM temperature parameter was set to 0"* verbatim. The
matched-temperature-0 control is intact and I did not manufacture a defect in it.

**(4) The elicitation span, with a degenerate block handled honestly.** Holding model, benchmark and
temperature fixed (Gemini-2.0-Flash, SALLM, T=0.00), the four elicitation methods read
**0.00 / 0.25 / 0.70 / 0.81** — a spread of 0.81, effectively the whole usable range of ECE. I
**deliberately did not headline that number.** An ECE of exactly 0.00 in all **eighteen** cells of the
Sampling-based Consistency block is degenerate, not a plausible measurement of perfect calibration at
three models and six temperatures. The entry records it **as printed**, declines to interpret it, and
rules that **the robust figure remains 0.56, unchanged from cycle 62**. A genuinely new second-order
finding that does *not* depend on the suspect block: **the elicitation method changes whether
temperature matters at all** — Verbalized and Token Probability are near-flat across the grid, while
Self-Consistency falls monotonically by 0.32 to 0.48 in all three models. Naming the method is not
sufficient for Self-Consistency; the temperature must be named too.

### A near-miss, recorded as method

A web search returned, in prose sitting next to a src-0018 result: *"Generative enrichment is the
right place for confidence scoring, with the model prompted to return a per-field confidence score
between 0 and 1..."* Had I credited that to src-0018 it would have made it a **verbalized-confidence
source and inverted finding (2) entirely**. The direct fetch shows it is not in the page. **A
search-engine summary is not a source read, even when the source's own URL is sitting in the result
list.** This is new carry-forward [213].

### Defect found in my own queue entry

One trivial error, recorded only for the running tally: it stated the issue was "Created cycle 3";
`graph.json` gives `created_cycle: 2` (cycle 3 is the first entry in `attempts`). Nothing depends on
it. Otherwise the entry verified correct on every load-bearing point — phase check, refresh
derivation, prohibitions, killed leads, Bash notes, and the verbatim splice of all nine
`open_questions`. Reported as accurate because a clean handoff is data too.

**What I did not do, stated plainly.** ctr-0027 step (ii) — appending src-0001's token-probability
elicitation passage to `src-0001.md` and `index.json` — is cheap, on this issue, and I did **not**
spend a turn on it. ctr-0011 is open on this issue and was not touched. `open_questions[7]`'s three
provenance routes remain untried. No source was added, so the `max_new_sources` budget was unused.

## Next task rationale

**T4 (assess), `target_issue: null`, `attempt_count: 0`.** The state machine is T3→T4 and the
refresh rule is not in play (75 % 7 == 5, and in any case it is tested by a T5). A T4 scores **all
nine** issues, hence the null target — which the queue entry warns is not a defect.

The queue entry gives the successor the **case for and against movement on this issue, both stated**,
and explicitly tells it not to import my framing: against, the reason this issue has sat at 2 is
unchanged — the level-3 bar needs the primary candidate to be two-source and **the calibration leg
still rests on src-0001 alone, gpt4o only**, and finding (2) arguably makes that gap *better
characterised and wider*, since src-0001 is now known to be the only CTI source in this base that
elicits model-expressed confidence in **any** form. For, three open questions were materially
advanced, one hypothesis closed outright, and a contradiction resolved.

Two forward warnings are carried: **cycle 77 is a genuine refresh firing** (77 % 7 == 0), so the T5
after the T4 must write a T1 — but it is told to re-derive the modulus itself rather than trust the
entry, because trusting the entry is exactly how cycle 70's firing was lost. And carry-forward [189],
the "last 5 cycles" attempt-penalty window ambiguity, **goes live again** for that T5, because this
issue now has an attempt at cycle 75.

## Budget

- **Fetches: 3.** `arxiv.org/html/2606.31159v1` (G2, Table I); the SentinelLabs src-0018 page;
  `ar5iv.labs.arxiv.org/html/2606.31159` (second render for the two unconfirmed blocks). All three
  returned complete, untruncated documents and transcribed tables cell-for-cell — the r.jina.ai proxy
  was **not needed**.
- **Searches: 4.** All on `open_questions[5]`, four distinct angles, all negative.
- **New sources: 0** (`max_new_sources: 5` unused; a T3 investigates with the sources it has).
- **Turns: ~45 of 75.** Two turns were lost: one to a Bash compound-command rejection (`&&` chaining,
  the exact failure the entry warned about), one to a jq precedence slip in a *verification* query,
  not in an edit.
- **Zero-fetch work:** finding (1), the highest-value result of the cycle, cost nothing but
  arithmetic.
