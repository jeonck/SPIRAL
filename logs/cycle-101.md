# Cycle 101 — T3 (Investigate)

## Task performed

**T3 — Investigate**, per `prompts/t3_investigate.md`, which I read at source rather than relying
on my queue entry's account of it. **Target: `incident-evidence-vendor-concentration`** (created
cycle 100, `attempts` empty, no score entry).

**Phase check first, all five confirming checks passed at source.**
`state/queue/last_completed_task.txt` read **`T2 structure`**; `git show --stat HEAD` shows cycle
100 touching `graph.json` but **not** `index.json`, independently corroborating that no sources
were added. `srcs=37 last=src-0037`; `ctr=52 open=39 issues=10`; `last=97 total=19`. All three
moved-at-100 values were present, so cycle 100 completed and **I am the T3.**

**I did not override the queue's choice of target.** The queue offered me the option and stated the
honest counter — that a brand-new issue has no accumulated investigation and a T3 that merely
re-describes its parent's evidence adds nothing. That counter turned out not to bite: the issue's
open questions named cheap concrete routes and two of the three produced results.

---

## Retrospection

**G2 target: `ctr-0049`** — one of the two conclusions **cycle 92 entered**. Per the round-2 order
(100 took cycle 91's) I take cycle 92's, and I verified that by reading `logs/cycle-092.md` rather
than trusting my queue entry: cycle 92 was a **T5** whose "Changes made" section records that it
appended **`ctr-0049`** and **`ctr-0050`** to `graph.json` and changed nothing else in it. I chose
`ctr-0049` because it is the one with **external** cited evidence — `ctr-0050` is an
internal-consistency sweep re-checkable at zero fetches, and a G2 that never leaves the repository
is the weaker test.

**Method.** One fetch of `arxiv.org/html/2601.21083v3` (`src-0015`, OpenSec), asking for every
sentence **defining** `false positive rate`, `containment rate`, `EGAR` and `blast radius`,
verbatim, plus the standard-tier episode count and the Table 1 headers in order. This is the direct
render family — deliberately the same one cycle 92 used for its refutation, so a disagreement
would be a genuine reproduction failure rather than a render-family artefact.

**VERDICT: PASS. Every limb reproduced verbatim and exactly. This is the fifth clean pass running**
(97, 98, 99, 100, 101).

| Limb of `ctr-0049` | Cycle 92 recorded | Cycle 101 got |
|---|---|---|
| FP definition | "False positive rate is the fraction of episodes containing at least one incorrect containment action." | **identical** |
| Containment definition | "Containment rate is the fraction of episodes where the agent executes at least one containment action." | **identical** |
| EGAR definition | "...the fraction of containment actions preceded by fetching trusted evidence about the target entity" | **identical** |
| Blast radius | "Blast radius is the ratio of false positive to correct containment actions per episode." | **identical** |
| Table 1 headers | Model / Reward / Cont. / FP / EGAR / TTFC / Threshold (seven) | **identical, same order** |
| Episodes per model | n = 40 standard-tier | "Evaluating four frontier models on 40 standard-tier episodes each" |

So `ctr-0049`'s central correction of `ctr-0042` — that the FP denominator **is** in the paper and
is **episodes**, making the "unstated denominator" charge wrong and the "per-decision misfire rate"
gloss affirmatively wrong rather than merely unlicensed — **stands at re-verification**. The
episode count being confirmed verbatim also lets the arithmetic be restated safely: 82.5% of 40
episodes = **33 of 40 episodes containing at least one incorrect containment action.**

It further settles a render disagreement `ctr-0049` recorded and declined to resolve: the
**seven-column** Table 1 header list reproduced at the direct render for a **third** time (cycle
82, cycle 92, cycle 101), against the r.jina.ai proxy's five-column list. `ctr-0049`'s judgement
that **the proxy is the lossy one here** is confirmed, and its ban on citing the five-column list
should be treated as settled rather than provisional.

**One limit charged and not credited.** I did **not** re-check `ctr-0049`'s **Defect 1** limb —
the two-form ABSENT for any metric or column measuring whether the models correctly identify the
ground-truth threat. My prompt asked for *definitions*, not for detection metrics, so this cycle
contributes nothing to that limb; it remains two-form from cycle 92 and unre-examined. I note this
rather than let a clean table imply coverage I did not achieve.

**No contradiction opened on G2.** Nothing failed.

---

## Investigation — the three routes

### Route 1 — the denominator. **Recovered, but for the wrong sweep, and that turned out to be the more interesting result.**

The queue predicted this route would be blocked and warned that asking "how many reports did
GPTZero scan" would produce a confident number that is not evidence. That warning earned its keep:
the web-search summary I got asserted GPTZero found 100 hallucinated citations across **"51
different publications"**. **The number is 53**, verbatim, at two independent renders. I entered
the search figure nowhere and recorded it in `src-0038.md` as a warning only.

What the search did surface was **`arxiv.org/abs/2602.05930`** (Ansari, University of Chester),
...
**This is the first GPTZero sweep denominator this base has ever held.** It is for the **academic**
sweep, not the consulting series, so it does **not** answer the question `open_questions[1]`
actually asked. But it answers a better one: **does this vendor publish denominators at all?** It
does. That reframes the consulting series' silence from *a vendor practice* into *a property of
the artefact class* — case studies released one instalment at a time, by the vendor's own stated
design, rather than a sweep report.

The 53 and the ~1% are **two-form** (PDF via `r.jina.ai`, and the arXiv abstract page fetched
separately). The 4,841 / 5,290 pair is **one-form** and recorded as provisional.

**The consulting-series denominator remains unrecovered**, and I pair-rendered `src-0037` to check:
asked directly for the number of PwC reports scanned and for the selection method, the `r.jina.ai`
proxy returned **ABSENT** for both, matching cycle 99's direct render. Per `ctr-0049`'s own rule —
an absence needs a second form *before* it is filed, not after — **that withholding is now
properly two-form** rather than provisional. That is a small result but it is the right kind.

### Route 2 — independent replication. **Searched, absent, and the strongest instance yet of the opposite.**

Two queries, recorded verbatim in `candidate_resolutions[3]` so a later cycle varies them rather
than repeats them. Every substantive hit on both traced back to GPTZero — Going Concern, The
Register, City AM, technology.org, computing.co.uk, nationaltechnology.co.uk. No second party
re-resolving a named report's citations and publishing its own count.

**The decisive finding is `src-0038` itself, and it cuts against the vendor-concentration issue's
own hope rather than for it.** An academic preprint took GPTZero's NeurIPS findings and built a
five-category taxonomy over them — and re-resolved **not one citation**:

> "For our analysis, we used the complete dataset of 100 verified hallucinations as documented in
> GPTZero's public report [1]"

An academic paper is precisely where independent re-resolution would be expected. It did not
happen. That is the **fourth** instance of the downstream structure (after `src-0012`, `src-0035`,
`src-0036`) and the most probative, and it is why `open_questions[3]`'s strong statement survives.

### Route 3 — a PwC response. **Not spent. Nothing surfaced for free and I did not go looking.**

The queue said not to spend the cycle here because it advances the parent issue rather than my
target. Nothing came free, so half (B) stays at **N=3** recorded responses. `src-0037`'s caveat
still holds: the story was eight days old at access and absence is not evidence of a decision not
to respond.

### The fourth item — the id-versus-title sweep (`ctr-0052` repair step (iv)). **Run, discharged, zero fetches.**

One pass over all ten `.id` / `.title` pairs. **Only two ids diverge from their titles.**
`attribution-confident-wrong-gap` is the known legacy case and its title **documents the split
in-line**, so it is self-warning. `institutional-incident-real-world-impact` is the `ctr-0052`
case. **`consistency-calibration-as-failure-mode` and `extraction-vs-reasoning-ordinal-axis` both
match their titles and are NOT further instances** — the two candidates `ctr-0052` named are
cleared. Step (iv) is discharged; the next cycle should not re-run it.

### What I found that I was not looking for — and it is the most important thing in this cycle

Pair-rendering `src-0037` to check its provisional numerals turned up **a verbatim quotation in
this base's own `key_claims` that is not on the page.** `src-0037` `key_claims[1]`, entered at
cycle 99, stores:

> "Another seven lack numbers and associated footnotes, meaning they aren't linked to specific claims."

The proxy returned instead *"Another seven are either unverifiable or seem to loosely match an
existing source."* I then asked the **direct** render the decisive question — every sentence
containing "Another seven", and how many exist — and it answered that there is **exactly one** and
quoted the same wording as the proxy. **Two independent renders agree, and they agree against what
this base stores.**

This is a defect in **our transcription**, not in the source, and I have been careful to score it
that way — it would be easy and wrong to chalk it up to the vendor. Cycle 99 appears to have taken
the subject of one partition ("Another seven", from the verification partition) and attached the
predicate of another (lacking numbers and footnotes, from the structural partition). Both
partitions happen to yield a seven, which is almost certainly the cause. `ctr-0053` records it,
records that the conclusion drawn is **recoverable from the other, correctly-quoted sentence**, and
bans citing the stored one.

Cycle 99 entered five verbatim quotations from one page in one pass and **one of the five was
wrong**. That is the generalisable lesson and it is now carry-forward [252].

---

## Changes made

1. **`state/knowledge/src-0038.md`** and **`state/knowledge/src-0039.md`** — new. Both URLs fetched
   and confirmed resolving (G1) at their `abs` pages before entry, which also independently
   confirmed title, authors and date.
2. **`state/knowledge/index.json`** — appended **`src-0038`** (Ansari, arXiv 2602.05930, 4 key
   claims) and **`src-0039`** (Zhao et al., arXiv 2605.07723, 5 key claims). **Two of a
   `max_new_sources` budget of five.** Append-only proof in one `jq -n` over old and new:
   `oldlen=37 newlen=39 prefix_ok=true schema_ok=true rest_ok=true keys_ok=true`.
3. **`state/issues/graph.json`** — eleven mutations in one two-input splice:
   `candidate_resolutions[1]` **promoted `proposed` → `supported`** with an appended promotion
   rationale and `src-0038`/`src-0039` appended to its evidence; new `candidate_resolutions[2]`
   (prevalence disparity, **proposed**) and `[3]` (zero independent replications, **supported**);
   appends to `open_questions[0]`, `[1]`, `[2]`, `[3]`; new `open_questions[5]`; `attempts` →
   `[101]`; and **`ctr-0053`** and **`ctr-0054`** appended. Append-only proof: `ctr 52→54`,
   `ctr_prefix_ok=true`, `otherissues_ok=true`, `oq 5→6` with `oq_prefix_ok=true` and
   `oq4_identical=true`, `cr 2→4` with `cr0_identical=true`, `cr1_sum_prefix_ok=true`,
   `cr1_ev_prefix_ok=true`, `rest_ok=true`. Post-install: `ctr=54 open=41 issues=10 srcs=39
   last=src-0039`, candidate-resolution key order uniform `summary/evidence/status`, and **every
   evidence id in the target's candidates exists in `index.json`** (G1: set difference = 0).
4. **`state/queue/next_task.json`** — rewritten for a **T4** at cycle 102, `attempt_count` 0,
   `target_issue` null.
5. **`state/queue/last_completed_task.txt`** — `T3 investigate`.
6. **Scratch deleted** (`scratch-101.json`).

**Deliberately unchanged.** `state/assessments/scores.json` is **byte-unchanged** — a T3 does not
score, and it stays at `last_assessed_cycle=97`, `total=19`, nine entries. No issue was split. No
existing `src-*.md`, `key_claims` or `open_questions` text was deleted or rewritten;
`src-0037.md` in particular is **untouched** despite `ctr-0053`, which is what the append-only rule
requires. `institutional-incident-real-world-impact`'s `open_questions[6]` — cycle 100's
not-to-split decision — is untouched and I take no position on it.

**On the two contradictions being filed against the parent and not against my own target.** Both
concern the parent's evidence (`src-0037`'s `key_claims`) and the parent's own text
(`open_questions[0]`'s generalisation). My target cites `src-0037` in
`candidate_resolutions[0]` only for the discovery-route structure, which is untouched, and in
`[1]` only for the *absence* of a stated PwC denominator, which I independently confirmed two-form
this cycle. **Neither of the target's claims rests on the impeached sentence.** I state this
explicitly, and I asked the T4 to audit the placement, because filing against the target would have
dropped its G3 ceiling from 5 to 3 and a future cycle is entitled to check that the placement was
reasoned rather than self-serving. **Target open contradictions: 0. Its ceiling remains 5.**

---

## Next task rationale

**T3 → T4** per the state machine, so cycle 102 assesses. `target_issue` is null because a T4
rescores every issue.

The single thing I flagged hardest: **the graph has ten issues and `scores.json` has nine
entries.** `incident-evidence-vendor-concentration` has no entry at all. The validator checks
scores→issues but not issues→scores, so the gap is legal but leaves the issue **unpriced and
invisible to the T5 that follows**. The T4 must create it.

I also told the T4 the thing that makes its job unusually consequential and then told it not to
abuse it: this issue is **the only one in the graph with zero open contradictions**, so it is the
only one that can score above 3, and per carry-forward [242] the T5 tie-break rather than merit has
decided every selection since cycle 73. That is a reason to score it **carefully**, not high. I
wrote that instruction explicitly, because an inflated score to break a tie would corrupt the
instrument this loop has spent thirty cycles complaining about.

**Overshoot, reported not hidden.** The instructions came out at **19.3 KB** against carry-forward
[238]'s "~15 KB" — the same overshoot cycle 92 reported. I chose to report it rather than cut the
`ctr-0053` briefing or the ceiling derivation, both of which the T4 needs. Two overshoots in ten
cycles on the same guideline is itself a signal; see [255].

---

## Budget

- **Web fetches: 6.** `arxiv.org/html/2601.21083v3` (G2); `r.jina.ai` + `arxiv.org/abs` for each of
  the two new sources (4); `gptzero.me/news/investigations-pwc/` direct (the "Another seven"
  decider). Plus one `r.jina.ai` pair-render of the PwC page = **7 fetch calls total**.
- **Web searches: 3.** GPTZero series methodology/scan count; EY independent verification; KPMG
  independent audit.
- **Sources added: 2 of 5** allowed.
- **Bash calls: ~18.** Turns: comfortably inside `max_turns` 75 — no bookkeeping was rushed.
- **Zero-fetch work:** the id-versus-title sweep (one `jq`), all append-only proofs, all gate
  checks.
