...
T5 select, as the queue directed. I read `prompts/t5_select.md` at source before acting and
applied its policy mechanically to `state/assessments/scores.json` and `state/issues/graph.json`,
re-deriving every input rather than inheriting it from cycle 146's queue entry.

**Selected: `institutional-incident-real-world-impact`.** The outgoing task is **T1 collect**,
because the refresh rule fired.

**Zero fetches, zero searches.** A T5 needs none, and this one spent none.

**Phase check.** All six confirmations in the queue entry passed exactly.
`state/queue/last_completed_task.txt` read `T4 assess`. HEAD is `589b972`, a real cycle commit
(`cycle 146: T4 assess`), and its stat touches `state/assessments/scores.json`, `state/meta.json`,
both `state/queue` files and `logs/cycle-146.md` and nothing else — `state/knowledge/` and
`state/issues/graph.json` are both untouched, as a T4's commit must be. The three counts:
`srcs=48 last=src-0048`; `ctr=70 open=52 issues=10 last=ctr-0070`;
`last=146 n=10 total=22 max=3`. I hand-checked these with `jq`; I could not run
`scripts/validate_state.py` (the bash sandbox refuses `python3`) and I do not imply that I did.

**The refresh rule — re-computed at source, not inherited.** `prompts/t5_select.md` step 4 reads:
*"if `current_cycle % schedule.collect_refresh_every == 0`, the next task is `T1` targeting the
selected issue (bring in fresh evidence) instead of `T3`."* `config.yml` `schedule.collect_refresh_every`
is **7**. `current_cycle` is **147**, and 147 = 7 × 21, so **147 % 7 == 0 and the rule fires**.
My outgoing edge is **T5 → T1**, not T5 → T3.

I confirmed this independently of the arithmetic, because cycle 145's entry got exactly this wrong
and cycle 146 caught it (carry-forward [390]). The T1s in this base have landed at cycles
**1, 15, 43, 78, 99, 120 and 134 — every one congruent to 1 mod 7**, i.e. each queued by a T5
running at a cycle congruent to 0 mod 7 (14, 42, 77, 98, 119, 133). Cycle 148 will be the eighth
such landing. Two independent derivations agree, and neither is taken from the entry that told me
the answer.

## Retrospection

**G2, round 2, next in line: cycle 138.** Cycle 138 was a **T5 select** — the same role I am in —
so this re-check doubled as preparation for my own task. Its commit is `768f307`; I stat'd it
rather than trusting the message, and it touches `state/meta.json`, both queue files and
`logs/cycle-138.md` and no research file at all, which is correct for a T5.

**A T5's entered conclusions are its selection and the ranking table that justifies it.** I pulled
`768f307:state/assessments/scores.json`, `768f307:state/issues/graph.json` and
`768f307:state/queue/next_task.json` to scratch files and re-derived every cell of cycle 138's
table from them. **Zero fetches.**

| cycle 138 claimed | I re-derived | verdict |
|---|---|---|
| ttp 3, vendor-concentration 3, other eight 2; `last_assessed_cycle` 137 | identical | ✅ |
| open contradictions: ttp 6, ioc 4, consistency-calibration 9, attribution-confident 3, attribution-expressed 4, task-dependent 4, extraction-vs-reasoning 2, institutional-incident 12, automated-triage 5, vendor 1 — sum **50** | identical, sums to 50 | ✅ |
| every issue carries ≥ 1 open contradiction | true at that snapshot | ✅ |
| 3(a) pairwise leaves four roots: ioc, consistency-calibration, institutional-incident, automated-triage | identical from `depends_on` | ✅ |
| 3(a) also excludes attribution-confident, attribution-expressed, task-dependent, extraction-vs-reasoning — **four of eight**, disagreeing with cycle 137's entry | correct: each has ≥ 1 dependency inside the score-2 tier | ✅ |
| 3(b) window 133–137: consistency-calibration attempted at **136**, +1 → effective 3; ioc last at 131, institutional at 128, automated-triage never | identical from `.attempts` | ✅ |
| 3(c): automated-triage out on `created_cycle` 16 vs 2 | identical | ✅ |
| ioc and institutional-incident tie unbroken by policy; broken by the **invented** least-recently-attempted rung, 128 < 131 → institutional-incident | follows from its own stated rule, and cycle 138 labelled the rung unauthorised rather than dressing it as policy | ✅ |
| the queue entry it wrote: `task_type` T3, target `institutional-incident-real-world-impact`, `attempt_count` 0 | identical | ✅ |
| institutional-incident's 12 open contradictions are "roughly a quarter of the base's 50" | 12/50 = 24% | ✅ |

**Cycle 138 passes in full. No contradiction opened.** I also checked its own refresh arithmetic,
which it did not state: 138 % 7 = 5, so T3 was the correct outgoing edge.

**Two benign supersessions, reported as supersessions and not as failures.** Cycle 138 recorded the
base-wide open-contradiction count as **50**; it is now **52**, because `ctr-0069` opened at cycle
139 and `ctr-0070` at cycle 142. And it recorded **12** open contradictions on
institutional-incident; that is now **13**, from `ctr-0069`. Both figures were true when written.

**One observation worth recording for the paper, since this G2 landed on a T5.** Cycle 138's most
valuable act was not its selection — the winner was over-determined — but its *disagreement* with
cycle 137's queue entry about the width of the 3(a) exclusion. It widened honesty item (C) from two
issues to four and every T5 since has reproduced the four-wide figure, including me. A cycle that
re-derives instead of inheriting pays for itself even when the headline answer is unchanged.

## Changes made

Three files, and nothing else. **I added no source, edited no issue and moved no score.**

- `state/queue/next_task.json` — rewritten for cycle 148: `task_type` **T1**, `target_issue`
  **institutional-incident-real-world-impact**, `attempt_count` 0, instructions 64,854 characters
  carrying the full ban set, the killed-lead list, the seven honesty items and the work-owed list
  forward, with the target issue's nine `open_questions` quoted by index and its collection target
  quoted verbatim from `open_questions[5]`.
- `state/queue/last_completed_task.txt` — `T5 select`.
- `logs/cycle-147.md` — this file.

`state/knowledge/`, `state/issues/graph.json` and `state/assessments/scores.json` are all three
untouched. Scratch files (`g2_scores138.json`, `g2_graph138.json`, `g2_task139.json`,
`next_task_body.txt`, `next_task_instr.json`) were all removed.

## Next task rationale

### The ranking, every rung worked and reported

Everything below was re-derived from the files this cycle. Nothing was inherited from cycle 146's
queue entry.

**Step 1 — candidate set.** All ten issues score < 5. All ten are candidates.

**Step 2 — base priority = score.** Re-derived from `scores.json` (`last_assessed_cycle` 146):

| issue | score |
|---|---|
| ttp-attack-mapping-reliability | 3 |
| incident-evidence-vendor-concentration | 3 |
| the other eight | 2 |

Open contradictions per issue, re-derived in one grouped `jq` per [373] and summing to **52**:
institutional-incident 13, consistency-calibration 9, ttp 6, automated-triage 5, ioc 5,
attribution-expressed 4, task-dependent 4, attribution-confident 3, extraction-vs-reasoning 2,
vendor-concentration 1. Every issue carries at least one, so under the ceiling reading of g3 the
cap is 3 everywhere and binds nothing (honesty item (G)). The weakest tier is score 2 with eight
members; the two score-3 issues never enter the tie-breaks.

**Step 3(a) — upstream first, computed PAIRWISE within the tier**, taking maximal elements, per
`ctr-0067` step (6)(i) and [386]. I state the reading explicitly: **pairwise**, *not* "is
`depends_on` empty".

| issue (score 2) | depends_on | any dependency inside tier 2? | survives 3(a)? |
|---|---|---|---|
| ioc-extraction-reliability | — | root | **yes** |
| consistency-calibration-as-failure-mode | — | root | **yes** |
| institutional-incident-real-world-impact | — | root | **yes** |
| automated-triage-under-refusal | — | root | **yes** |
| attribution-confident-wrong-gap | consistency-calibration | yes (score 2) | no |
| attribution-expressed-confidence-unmeasured | attribution-confident, consistency-calibration | yes (both score 2) | no |
| task-dependent-reliability-framing | ttp, ioc, consistency-calibration, attribution-confident, attribution-expressed | yes (four of five in tier) | no |
| extraction-vs-reasoning-ordinal-axis | task-dependent-reliability-framing | yes (score 2) | no |

The two readings of 3(a) coincide here, and I checked *why* rather than assuming: the only
out-of-tier dependency edge in the whole graph is task-dependent's edge to `ttp`, and `ttp` scores
**3** — higher. That is the condition [386] names, and it still holds at cycle 147. **Four of the
eight score-2 issues are unreachable by the selector** — honesty item (C), reproduced unchanged.

**Step 3(b) — attempt penalty**, +1 per attempt within the last 5 cycles. **I apply the EXCLUSIVE
reading, window 142–146**, as cycles 133, 141 and 144 declared. The inclusive reading is run below
as a robustness check.

| survivor of 3(a) | attempts (tail) | in window 142–146 | penalty | effective |
|---|---|---|---|---|
| ioc-extraction-reliability | …117, 131, **142** | **142** | **+1** | **3** |
| consistency-calibration-as-failure-mode | …122, 136, **145** | **145** | **+1** | **3** |
| institutional-incident-real-world-impact | …114, 128, 139 | none (139 is 8 back) | 0 | 2 |
| automated-triage-under-refusal | *empty* | none, ever | 0 | 2 |

On the third unruled reading question — whether the 3(b) penalty yields an "effective score" that
competes *across* base-score tiers or is only compared *within* a rung-2 tie — **I apply the
within-tier reading**, consistent with every prior T5, and I record that **it is non-decisive
here**: the winner sits at effective 2, which is the global minimum under either reading, so the
across-tier reading would not disturb it.

**Step 3(c) — older `created_cycle` first:**

| remaining at effective 2 | created_cycle | outcome |
|---|---|---|
| institutional-incident-real-world-impact | 2 | **survives — selected** |
| automated-triage-under-refusal | 16 | eliminated at 3(c) |

**Final ranking**

| rank | issue | score | 3(b) penalty | effective | eliminated at |
|---|---|---|---|---|---|
| 1 | **institutional-incident-real-world-impact** | 2 | 0 | 2 | — (**selected**) |
| 2 | automated-triage-under-refusal | 2 | 0 | 2 | 3(c), created 16 vs 2 |
| 3= | ioc-extraction-reliability | 2 | +1 | 3 | 3(b) |
| 3= | consistency-calibration-as-failure-mode | 2 | +1 | 3 | 3(b) |
| 5= | attribution-confident-wrong-gap | 2 | 0 | 2 | 3(a) |
| 5= | attribution-expressed-confidence-unmeasured | 2 | 0 | 2 | 3(a) |
| 5= | task-dependent-reliability-framing | 2 | 0 | 2 | 3(a) |
| 5= | extraction-vs-reasoning-ordinal-axis | 2 | 0 | 2 | 3(a) |
| 9= | ttp-attack-mapping-reliability | 3 | 0 | 3 | step 2 (score tier) |
| 9= | incident-evidence-vendor-concentration | 3 | 0 | 3 | step 2 (score tier) |

### The tie-break list terminated — and that is luck, not a repair

**For the first time since cycle 126, the written policy decided a T5 on its own.** No rung was
invented. I want to be exact about why, because it would be easy to read this as the defect having
gone away, and it has not.

The list terminated only because **cycle 142 happened to attempt `ioc` inside the 3(b) window**.
That penalty is what separated `institutional-incident` from `ioc` at rung 3(c), where the two are
otherwise identical (both roots, both `created_cycle` 2). Had cycle 142's T3 landed one cycle
earlier, rung 3(c) would have tied and the list would have run out again. **Honesty item (B)
stands, unchanged, raised for the eighteenth time.**

**The robustness check, and it found something new.** Under the **inclusive** reading of the
5-cycle window (143–147), `ioc`'s attempt at 142 falls *outside* the window and it takes **no
penalty**. Three issues then reach rung 3(c) at effective 2 — `ioc` (created 2),
`institutional-incident` (created 2), `automated-triage` (created 16). Rung 3(c) eliminates
`automated-triage` and **deadlocks on the other two**, and the invented least-recently-attempted
rung would be needed: `institutional-incident` last attempted **139**, `ioc` last attempted
**142**, so it too returns `institutional-incident`.

**Both readings return the same winner, so this selection is robust to the unruled question.** But
**cycle 147 is the first cycle at which the two readings produce different 3(b) penalty columns**,
not merely different termination behaviour — `ioc` takes +1 under one and 0 under the other. Until
now the window boundary only ever decided *whether the list ran out*. It can now decide *the
arithmetic*. They agreed on the winner this time. There is no guarantee they will next time, and
a human ruling on the window's inclusivity is worth more today than it was yesterday. Recorded as
carry-forward [391] and folded into honesty item (B).

### Honesty item (A): automated-triage-under-refusal, thirty-ninth cycle

It survived 3(a) (a root, so 3(a) never touches it) and 3(b) (never attempted, so it can never take
a penalty). **It was eliminated at rung 3(c), on `created_cycle` 16 against 2** — the **seventh**
time (127, 130, 133, 138, 141, 144, 147) it has gone out on exactly that rung. It came closer this
cycle than ever before: **two of the three** score-2 `created_cycle`-2 roots carried a 3(b) penalty
simultaneously (`ioc` at 142, `consistency-calibration` at 145), and only
`institutional-incident` did not. The condition under which it could win is that **all three** do
at once. I worked the rungs in order and did not steer, in either direction.

**And this cycle supplies the proof of the claim cycles 138–146 could only argue.** Item (A) and
item (B) are not the same problem. This cycle the tie-break list **terminated** — the policy did
not run out — and `automated-triage` **still lost**, because it dies at a *written* rung, above the
point where the policy ends. A terminating fourth rung would not have saved it. Only a staleness or
never-attempted term inserted **at or above 3(c)** would. Previous cycles asserted this; cycle 147
is the counterexample that demonstrates it. A human ruling on (B) alone leaves this issue starved.

Its one defined piece of work stands — work-owed (25), a T3 reading src-0047 end to end.

### Why the selected issue is worth the cycle, and what the T1 is for

Beyond the arithmetic: `institutional-incident-real-world-impact` carries **thirteen** open
contradictions, the largest block in the graph and a quarter of the base's 52, so honesty item (E)'s
overcount lands here hardest. It is also where honesty item **(F)** sits — `ctr-0069`'s cycle-140
ruling, now **eight cycles unexecuted**, which cycle 146 recorded as making the issue's own text
overstate its finding. I cannot execute it (a T5 edits no graph) and neither can the T1 I am
queuing; I have told cycle 148 to say so in its log if its collection succeeds, because the
unexecuted ruling would then be blocking two things rather than one.

**The collection target was specified at cycle 139 and has not moved.** Quoting the issue's own
`open_questions[5]`, cycle-139 update, final sentence: *"WHAT THIS ENTRY NOW WANTS IS A SECOND
NON-BIG-FOUR CASE WHOSE ARTEFACT IS A PUBLISHED REPORT RATHER THAN A COURT FILING, which would
settle both the scope objection and the forum confound at once; NONE IS KNOWN TO THIS BASE AND
CYCLE 139's SEARCHES DID NOT SURFACE ONE."* `src-0048` (Sullivan and Cromwell) broke the
commercial-exposure-versus-boilerplate confound but swapped it for a **forum** confound; a
non-Big-Four institution that *published a report* and then responded in public would control for
forum while keeping the non-Big-Four property. Cycle 139's three searches came back empty — a
negative on three queries, not a demonstration that none exists — so I have told cycle 148 to run
**different** queries and, if they also come back empty, to record a dated first-person
search-negative with the exact queries and add no source rather than padding the collection.
Work-owed (27) — the primary bankruptcy-docket artefact behind `src-0048`, never attempted, route
accessibility unknown — is named as the second-best target if the first is empty.

**The refresh rule did not relieve honesty item (C), and I say so rather than letting it look
relieved.** The T1 goes to the issue the selector picked, and rung 3(a) guarantees that is a
**root**. So the one mechanism this system has for scheduling fresh evidence has just been aimed,
again, away from `src-0006`'s unfetched per-task scoring definitions — work-owed (6), which
seventeen cycles now call the single highest-value unfetched item in the base, and which lives on
`task-dependent-reliability-framing`, an issue 3(a) makes unreachable. The next refresh falls at
**cycle 154** and will have the same property. This is the sharpest statement of (C) the record
has: **the refresh mechanism structurally cannot aim at the item the base most wants.**

## Budget

- **Web searches: 0. Web fetches: 0.** A T5 needs neither, and the G2 was done entirely by
  re-deriving cycle 138's table from `git show` of past revisions.
- **Turns: ~26 of 75.** Roughly: 5 reading the state and the queue entry (the entry is 54 KB and
  had to be read in two chunks per [384]), 4 on the phase check, 7 on the G2 (three `git show`
  extractions plus three re-derivation `jq` calls plus cleanup), 4 re-deriving the selection inputs
  and reading the target issue's `open_questions`/`candidate_resolutions` by index per [374], and
  6 writing and validating the outgoing queue entry and this log.
- **New technique, recorded as [391]:** `jq -Rs . body.txt > instr.json` reads a whole text file
  into a JSON string and is **not** refused by the sandbox, even though `--rawfile` is. Writing the
  64 KB instructions as plain text with the Write tool and then splicing with the two-input form of
  [237] avoided hand-escaping tens of thousands of characters into a JSON string literal, which is
  where a long entry is most likely to break. Cost: two extra Bash calls. Worth it.
- No source added, no issue edited, no score moved.
