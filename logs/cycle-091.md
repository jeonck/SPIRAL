...
**Topic:** `cti-ai-reliability` — Reliability assessment of AI-generated Cyber Threat
Intelligence. **Date:** 2026-08-04. **Runtime:** `claude-opus-5` / CLI `2.1.220`, per the
pin in `config.yml`.
...
The queue entry warned that cycles 70 and 83 aborted within living memory and told me not to
assume cycle 90 completed. I ran every confirming check it named and did not trust the commit
message.

| Check | Expected if 90 completed | Observed | Verdict |
|---|---|---|---|
| `state/queue/last_completed_task.txt` | `T3 investigate` | `T3 investigate` | ✅ |
| `git show --stat HEAD` | cycle-90 files incl. `src-0032.md` | `src-0032.md` +157, `src-0018.md` +75, `graph.json` ±27 | ✅ |
| `srcs` / `last` in `index.json` | `srcs=32 last=src-0032` | `srcs=32 last=src-0032` | ✅ |
| open contradictions | `open=35` | `open=35` | ✅ |
| `ctr-0018.resolved_cycle` | `[90]` | `[90]` | ✅ |
| `institutional-incident` cand / oq / attempts | `9 / 8 / [12,59,72,90]` | `9 / 8 / [12,59,72,90]` | ✅ |
| `scores.json` byte-unchanged | `last_assessed=88`, `assessed=[88]`, `total=19` | `last_assessed=88 n=9 total=19 cycles=[88]` | ✅ |

**Cycle 90 completed. I am the T4.** I read `prompts/t4_assess.md` directly rather than
inheriting the queue entry's account of it, and I re-read `scripts/validate_state.py`
lines 130–158 at source rather than inheriting the ceiling arithmetic.
...
`prompts/t4_assess.md` rubric. `state/assessments/scores.json` now carries
`assessed_cycle: 91` on every entry, `last_assessed_cycle: 91`, a new `_cycle91_note`, and a
cycle-91 block **appended** to all nine rationales.

### The scores

| Issue | Score | Δ vs 88 | Open contradictions | Ceiling | Binds? |
|---|---|---|---|---|---|
| ttp-attack-mapping-reliability | **3** | — | ctr-0015, 0016, 0023, 0031, 0045, 0047 (6) | 3 | **yes** |
| ioc-extraction-reliability | **2** | — | ctr-0001, 0004, 0010, 0020, 0028, 0038 (6) | 3 | no |
| consistency-calibration-as-failure-mode | **2** | — | ctr-0011, 0027, 0039, **0048** (4) | 3 | no |
| institutional-incident-real-world-impact | **2** | — | ctr-0025, 0030, 0032, 0041 (4) — *ctr-0018 resolved* | 3 | no |
| attribution-expressed-confidence-unmeasured | **2** | — | ctr-0024, 0029, 0034, 0043 (4) | 3 | no |
| attribution-confident-wrong-gap | **2** | — | ctr-0008, 0036, 0046 (3) | 3 | no |
| task-dependent-reliability-framing | **2** | — | ctr-0009, 0017, 0037 (3) | 3 | no |
| automated-triage-under-refusal | **2** | — | ctr-0021, 0033, 0042 (3) | 3 | no |
| extraction-vs-reasoning-ordinal-axis | **2** | — | ctr-0013, 0040 (2) | 3 | no |

**Total 19.** This is the **seventh consecutive assessment at 19** (cycles 69, 73, 76, 81, 85,
88, 91) and **no individual score has moved since cycle 73 — eighteen cycles.**

### The two issues that had genuinely new ground, priced explicitly

"Nothing moved" is not "nothing happened." Two issues changed since cycle 88 and each was
priced rather than held by default.

**(1) `institutional-incident-real-world-impact` — the only issue any cycle touched, and the
only one whose open-contradiction count *fell*.** Cycle 90 resolved `ctr-0018` (5 open → 4,
the issue's first resolution since cycle 59), added `src-0032`
(`gptzero.me/investigations/ey`, the GPTZero investigation itself), added
`candidate_resolutions[8]`, and discharged `ctr-0032` step (v) negative. Four gains, priced
one at a time in the rationale:

- **Real:** half (A) now rests on **primary artefacts for all three institutions** — ENISA
  (`src-0025/26/27`), EY (`src-0032`), curl (`src-0022`).
- **Real:** the **method** fact — *"One of our team members manually verified Hallucination
  Check's results"* — a commercial detector with human verification on top. Before cycle 90
  this base did not know which of three warrant grades the EY finding was.
- **Not a gain:** `src-0032` is **not independent of `src-0012`**. They are the primary and
  secondary report of *one* finding, so under the level-3 bar they count once.
  `candidate_resolutions[8]` carries three ids and would look multi-source to a T4 counting
  arrays. It is not.
- **Not a gain:** `src-0032` **adds no number.** The `0 of 27` string is a JavaScript
  odometer artefact, established across two renders.

**And half (B) is untouched.** The issue asks about real-world *impact*; the base measures
impact **not at all** — no decision taken, no resource misallocated, no cost, anywhere in the
source set. Everything cycle 90 added is on half (A) and **the candidate it added says so
itself.** Cycle 76's pinning stands and the score holds at 2. I endorsed the `open_questions[6]`
split for the fifth time.

**(2) `consistency-calibration-as-failure-mode` — one *more* open contradiction.** `ctr-0048`
(cycle 89) falsifies cycle 87's *stated reason*, not its conclusion;
`candidate_resolutions[10]` keeps `supported`. Cycle 90's G2 then **passed** on
`candidate_resolutions[12]`. **The juxtaposition is the genuinely new thing and I recorded it
as such:** both cycles found the *same defect shape* — a stored quotation that is a silent
truncation — and reached **opposite verdicts**. At 89 the omitted continuation falsified the
reason built on it; at 90 all four omissions were neutral or **reinforcing**, two of them
*upgrading* the candidate. So carry-forward [244] is vindicated as a rule about **what needs
checking**, not as a presumption that stored excerpts are wrong. That is why I did not demote
on `ctr-0048`.

**Neither movement is a scoring event**, and I stated it in both rationales in the same terms
so it reads as one rule: the G3 demotion is a **set comprehension over `issue_id`**, so 3-open,
4-open and 6-open are identical to it. Cycle 88 made the 4→3 statement on
`consistency-calibration`; I made the symmetric 3→4 statement there and the symmetric 5→4
statement on `institutional-incident`.

### Carry-forward [4], seventy-ninth cycle, re-derived at source

`prompts/t4_assess.md` step 3 reads the demotion as a **subtraction** ("loses
`gates.g3_contradiction_demotion` points, floor 0"). `scripts/validate_state.py` lines 144–156
implements a flat **ceiling** of `scale_max - demotion = 3` via a set comprehension over
`issue_id`, so multiplicity is invisible to it. Under the subtraction reading
`ioc-extraction-reliability` would sit at **−10** and `ttp` at **−9** on a 0–5 scale. **I
followed the validator, not the prompt, and said so in every affected rationale.** Cycles 76,
81, 85, 88 and 91 have all done the same. It still needs a human.

### A measurement about the instrument

This is the most useful thing this assessment produced, so I put it in `_cycle91_note` rather
than only in the log. Between cycle 73 (the last score movement) and cycle 91 the state gained
**5 sources** (27 → 32), **opened 15 contradictions**, **resolved 4**, and ran **five further
assessments**. **The score vector is byte-identical across all of it.** Eight of nine issues
sit at 2 and the ninth sits at its G3 ceiling, so the vector carries **no ordering
information** and the weakest-link selector has been decided by its tie-break, not by these
scores, since cycle 73.

With a flat ceiling of 3 and a level-3 bar requiring two independent sources on the primary
candidate, **this evidence base can express at most two distinct values**, and it currently
expresses them in an 8-to-1 split. I did **not** fix that by inflating anything —
`prompts/t4_assess.md` step 5 forbids it and inflation would break the selector further. It is
recorded as a defect of the rubric-plus-gate *combination*, and it is a **different** question
from carry-forward [4].

---

## Retrospection

**G2 target: cycle 81's scored assessment of `ttp-attack-mapping-reliability` (held at 3) —
specifically the UNVERIFIED EXPOSURE that assessment recorded and declined to price.**

**Why this target.** Per the round-2 order (cycle 90 took cycle 80's issue) cycle 91 takes
**cycle 81's**. I verified that against `logs/cycle-081.md` rather than trusting the queue
entry's line: cycle 81 was a **T4**, so the conclusions it entered are its nine scored
assessments, and the G2 charter admits "a supported candidate_resolution **or a scored
assessment** from a previous cycle." I took the **only score above 2** — the one the
weakest-link selector's shape depends on, and the one cycle 81 wrote the most consequential
claims about.

**Result: PASSES on every limb, and it discharges a step that had stood since cycle 77.**
**Zero fetches, zero searches** — justified below, not merely reported.

### Limb 1 — the merit claim. Exact.

Cycle 81 wrote: *"six supported candidates on src-0002, src-0007, src-0017, src-0020,
src-0021 and src-0023."* Re-derived from `graph.json`: all six `candidate_resolutions` are
status `supported`, and the **union of their evidence arrays is exactly those six ids**, no
more and no fewer. The `scores` entry's `evidence` array carries eight because it also retains
`src-0005` and `src-0018` from earlier assessments; that is a superset of what the candidates
cite, and I left it as cycles 73–88 left it.

### Limb 2 — the exposure. RUN, and NEGATIVE.

Cycle 81 recorded, and cycle 88 repeated, that **`ctr-0036` step (ii) had never been run over
this issue**. `ctr-0036` itself calls step (ii) *"THE HIGHEST-VALUE STEP HERE"* and says it
*"NEEDS NO FETCH AT ALL"*. Both assessments called it **unverified exposure**, correctly
declined to demote on it, and neither ran it. **I ran it.**

Method — one `jq` call over the whole `ttp` issue object:

| Probe | Meaning | Hits |
|---|---|---|
| `Threat Actor` | the `src-0007` rubric block itself | **0** |
| `rubric` | any rubric reading | **0** |
| `1.140` | the Attribution cell `ctr-0008` forbids differencing | **0** |
| `3.612` | its forbidden counterpart | **0** |
| `attribution` | the equivocal word | **3** |
| `Table 4` | the table both defects live in | **4** |

All **three** `attribution` hits are the `src-0023` sense — *"CTI-ATA (Attack Technique
Attribution, Report → ATT&CK entries)"*, a technique-ID linking task — **not** the `src-0007`
rubric dimension and **not** threat-actor attribution. All **four** `Table 4` hits are
`src-0007`'s ATT&CK **TTP identification precision/recall** figures (0.2787/0.2270,
0.3480/0.1759, 0.2387/0.1846, 0.1771/0.1414) or the open question of which released scorer
produced them. **None** is the *Content: Threat Actor* block.

**No propagation site on this issue.** The hold at 3 is, for the first time in ten cycles, not
shadowed by that exposure.

### Limb 3 — the second defect cycle 81 named. Also negative, and the finding is the reverse.

Cycle 81 also listed `ctr-0038` (cross-metric comparison) as a defect this issue might carry.
`ctr-0038`'s defect is **asserting** a precision-versus-F1 comparison **as a finding**. This
issue makes exactly one such juxtaposition, in `candidate_resolutions[3]` consequence (A), and
it **withdraws its own licence in the same sentence**:

> materially above src-0007's 0.2787-class figures and BELOW OR NEAR src-0002's 0.6388, but
> **NONE of those three comparisons is licensed**, because all three differ on at least one of
> the four axes in open_questions[5]. **WHAT IS LICENSED IS ONLY THE WITHIN-PAPER READING.**

`candidate_resolutions[4]` does the same for the `src-0002` differencing, citing `src-0002`'s
own metric paragraph against it. That is the **opposite** of `ctr-0038`'s defect, not an
instance of it.

### Limits charged, not credited

1. **The sweep is lexical** and scoped to the issue object, **not** the logs. A paraphrase
   avoiding all six probe strings would evade it. What is established is **NO SITE FOUND**, not
   **NO SITE EXISTS** — and I wrote it that way into the rationale.
2. It is a **presence/absence test on wording**, not an adjudication of whether this issue's
   `src-0007` readings are *correct*.
3. It discharges step (ii) for **this issue only**. `ctr-0036` step (iv) —
   `attribution-expressed-confidence-unmeasured` — remains unswept.

### Why zero fetches, stated as a justification rather than an apology

`ctr-0036` step (ii) is **by construction** a state-internal sweep and says so. The available
fetch would have re-bought `src-0007`'s Table 4 cells, which are verified at cycles 15, 21, 30
and 53 and are exactly the re-buy pattern this base's own carry-forwards warn against.
Zero-fetch G2s are precedented and blessed (cycles 79, 81, 82, per carry-forward [214]).

### What this changes beyond the target issue, and it is the useful part

The propagation of the `src-0007` `Attribution` equivocation is now known to be **selective
rather than presumptively pervasive**: **one confirmed site** (`extraction-vs-reasoning-ordinal-axis`,
via `ctr-0040`), **one confirmed clean** (`ttp`, this cycle), **one never swept**
(`attribution-expressed-confidence-unmeasured`, step (iv)). I wrote that into all three
rationales, in each case with the direction it actually points:

- On `extraction-vs-reasoning` it moves **nothing** — the defect there was already *confirmed*,
  so learning it is rarer elsewhere neither aggravates nor mitigates it. What it changes is the
  **value of the repair**: that issue is now the graph's *only* confirmed instance.
- On `attribution-expressed-confidence-unmeasured` it **raises** the value of step (iv): that
  issue is now the **last unswept candidate site in the graph**, and the sweep that settles it
  costs one `jq` call.
- On `attribution-confident-wrong-gap` — where `ctr-0036` was *opened* — it is **not
  mitigation**, and I said so explicitly so no successor reads it as such.

**No contradiction opened.** The check passed on all three limbs.

---

## Changes made

**One state file edited, spliced in a single atomic `jq` pass, plus the two queue files.**

1. **`state/assessments/scores.json`**
   - `last_assessed_cycle` **88 → 91**.
   - `assessed_cycle` **→ 91** on all nine entries (one `with_entries(...)` clause,
     carry-forward [243]).
   - A **cycle-91 block appended** to all nine `rationale` strings. No existing text rewritten.
   - New top-level **`_cycle91_note`** (2,945 chars) carrying the summary, the contradiction
     movement, carry-forward [4], and the instrument measurement.
   - **`src-0032` appended** to `institutional-incident-real-world-impact`'s `evidence` array
     (9 → 10 ids), because cycle 90 added that source and `candidate_resolutions[8]` cites it.
     **This is the only evidence-array change anywhere in the file.**
   - **No `score` value changed.**
2. **`state/queue/next_task.json`** — T5 (select) for cycle 92, `attempt_count: 0`, 15.2 KB.
3. **`state/queue/last_completed_task.txt`** — `T4 assess`.
4. `scratch-091.json` and `c88blocks.txt` were working files and are **deleted**.

**`state/issues/graph.json` and `state/knowledge/` are byte-unchanged.** A T4 may not rewrite
candidates or key_claims, and my G2 passed, so there was nothing to file. That is a completed
cycle, not an aborted one — the phase-check table in the next entry says so explicitly.

### Gate checks, run by hand (the Bash tool refuses `python3`, so the validator cannot be run)

| Gate | Check | Result |
|---|---|---|
| syntax | `jq -e .` on the spliced file | ✅ |
| schema | every entry's keys == `score,rationale,evidence,assessed_cycle` | ✅ `["score,rationale,evidence,assessed_cycle"]` |
| G1 (line 138) | every score ≥ 2 has non-empty `evidence` | ✅ all nine |
| G1 (line 140) | every evidence id exists in `index.json` | ✅ `unknown_ev=[]` |
| line 136 | every scores key is a known `issue_id` | ✅ 9/9, set unchanged |
| G3 (line 152) | no score > ceiling 3 | ✅ `max=3` |
| append-only (string) | old rationale is a **prefix** of new, all nine | ✅ `[true]` |
| append-only (array) | old `evidence[0:9]` == new prefix, `institutional` | ✅ |
| untouched siblings | other eight `evidence` arrays byte-identical | ✅ |
| untouched siblings | `_schema`, `_cycle73/76/81/85_note` byte-identical | ✅ |
| invariants | `total=19`, `ttp=3`, `n=9`, `assessed=[91]` | ✅ |

---

## Next task rationale

**Cycle 92 is a T5 (select)** by the state-machine ladder T4→T5. The entry gives it:

- **The full phase check** with the exact expected strings, including the point that
  `graph.json` is **byte-unchanged** by cycle 91 and that this is *not* an abort signal — the
  abort test is `last_assessed_cycle` still reading 88.
- **The refresh rule, to derive itself.** My reading: 92 % 7 = 1 ≠ 0, so **no refresh fires**
  and cycle 93 is a T3; next firing cycle 98; last T1 actually run was cycle 78. I told it to
  derive this from `config.yml` and `state/meta.json` rather than take my arithmetic.
- **The raw ranking data** — score, `attempts`, `created_cycle`, `depends_on` for all nine —
  with the instruction to re-derive rather than trust the table.
- **The tie-break hazard, stated without pre-deciding it.** Eight issues tie at 2, so the
  tie-breaks decide. The only attempt inside any plausible five-cycle window is
  `institutional-incident` at cycle 90 (+1); `consistency-calibration`'s cycle-87 attempt sits
  on a boundary the prompt does not specify, and carry-forward [233] records that this loop has
  alternated on tie-break readings before. I told it to **state which reading it uses and
  why**.
- **The thing the scores cannot express**: `automated-triage-under-refusal` has
  `attempts == []`, is the only issue never investigated in 91 cycles, has been named weakest
  on merit by every assessment since 76, and has been eliminated by the `created_cycle`
  tie-break ten-plus recorded times. **I am a T4; I did not select and I did not instruct it.**
  I asked it to apply the policy mechanically, show the full ranking table, and **say plainly**
  if the mechanical answer conflicts with the merit judgement rather than quietly overriding
  either.
- **Two mispricings named**: reading the resolved `ctr-0018` as headroom (it is not — set
  comprehension), and reading `src-0032` as quantitative corroboration (it adds no number, and
  it is not independent of `src-0012`).
- **Its G2 target** under the round-2 order is **cycle 82's issue**, to be verified against
  `logs/cycle-082.md` rather than trusted from my line.

**Size: 15.2 KB**, at the ~15 KB guideline of carry-forward [238].

---

## Budget

- **Fetches: 0.** Justified, not merely reported: the G2 target was `ctr-0036` step (ii),
  which is by construction a state-internal sweep and says so in its own text. The only
  available fetch would have re-bought `src-0007`'s Table 4, verified at cycles 15, 21, 30 and
  53.
- **Searches: 0.**
- **New sources: 0.** Source count stays 32.
- **Contradictions: 0 opened, 0 resolved.** 48 total, 13 resolved, 35 open.
- **Turns: ~40 of 75.** 26 Bash calls (all single-operation), 3 Read, 3 Write, 3 Edit.
- **State bytes changed:** `scores.json` only, ~452 KB → ~480 KB.
