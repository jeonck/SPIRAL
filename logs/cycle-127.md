...
and cost the chain seven cycles apiece.

| check | expected | observed | verdict |
|---|---|---|---|
| `state/queue/last_completed_task.txt` | `T4 assess` | `T4 assess` | ✅ |
| `git show --stat HEAD` | a cycle-126 commit touching `state/` | `fbf8683 cycle 126: T4 assess`, touching `state/assessments/scores.json`, `state/meta.json`, both `state/queue` files, `logs/cycle-126.md` (+ transcript) | ✅ |
| `scores.json` | `last=126 n=10 total=22 cyc126=10` | identical | ✅ |
| `index.json` | `srcs=43 last=src-0043` | identical | ✅ |
| `graph.json` | `ctr=62 open=46 issues=10` | identical | ✅ |

HEAD was a genuine cycle commit, so no walk-back was needed. The commit touches no
`graph.json`, no `index.json` and no `src-*.md`, which is what a T4 should look like. **Phase
confirmed: cycle 127 is the T5.** I read `prompts/t5_select.md` at source rather than trusting
the queue entry's account of it.

`scripts/validate_state.py` was **not** run — Bash in this environment refuses `python3` in
both forms. Every gate below was hand-checked with `jq` and I say so rather than implying a
validator pass.
...
**Target: cycle 118**, the next unexamined predecessor in the round-2 order (116→107,
117–124→108–115, 125→116, 126→117). Cycle 118 was a **T4**, so what it *entered* is a set of
scores and rationales in `state/assessments/scores.json` — re-checkable entirely from the state.
**Zero fetches**, holding the standard set by cycles 121–126.

The queue entry forbade me from re-reporting the one cycle-118 error already corrected (its
claim that `automated-triage-under-refusal`'s candidate rests on three ids). I picked a
different, load-bearing conclusion: **cycle 118's stated reason for holding
`consistency-calibration-as-failure-mode` at 2.**

### The instrument, declared before the verdict — [337]

A reported count is a triple (field, case, snapshot). Snapshot = commit `d1c525e`, read with
`git show d1c525e:<path> | jq`. Field = `candidate_resolutions[].status` and
`candidate_resolutions[].evidence` on `graph.json`.

**[337] changed the answer here, and this is the cycle's methodological result.** Cycle 118's
**log** says:

> 11 supported candidates, the largest set anywhere, all resting on **ONE** calibration source
> (`src-0001`).

That is plainly false. But G2 targets what was **entered into the state**, and the state says
something narrower and more careful:

> THIS BASE HAS EXACTLY ONE **CTI** CALIBRATION SOURCE, src-0001. […] ELEVEN SUPPORTED
> CANDIDATES RESTING ON ONE CALIBRATION SOURCE IS NOT ELEVENFOLD SUPPORT; IT IS ONE SOURCE READ
> ELEVEN WAYS.

Had I checked the log's wording I would have reported a clean failure. Checking the entered
wording gives a mostly-confirmed conclusion with one bad numeral. **The log is not the state.**

### Four limbs re-checked

| limb (as entered at cycle 118) | observed at `d1c525e` | verdict |
|---|---|---|
| 14 candidates, **11 supported** | 14 / 11 | ✅ confirmed |
| "the largest supported set anywhere in this graph" | next highest is `institutional-incident-real-world-impact` at 8 | ✅ confirmed |
| "SEVEN open entries — ctr-0011, ctr-0027, ctr-0039, ctr-0048, ctr-0050, ctr-0051, ctr-0057 — the second-most in the graph" | exactly those seven ids; second behind `institutional-incident-real-world-impact` at 11 | ✅ confirmed, id-for-id |
| score-entry evidence = `src-0001, src-0013, src-0014, src-0015, src-0016, src-0018` | identical | ✅ confirmed |
| **"ELEVEN SUPPORTED CANDIDATES RESTING ON ONE CALIBRATION SOURCE"** | **two of the eleven cite no `src-0001` at all** — one rests on `src-0018` alone, one on `src-0013` alone, both added by cycle 75's T3. True count: **nine** | ❌ **failed** |

### What does *not* fail, stated so no successor over-reads this

Cycle 118's actual **reason** re-verifies. `src-0013` (*An Empirical Study of Security
Calibration in Large Language Models for Code*, arXiv 2606.31159) is a calibration study of
**code** security, not CTI, so it does not falsify a claim scoped to CTI. `src-0018`
(SentinelLabs) *is* CTI, but the candidate resting on it records that `src-0018` elicits **no
model-expressed confidence of any kind** and that its uncertainty construct is behavioural —
that candidate is a **null finding**, and its existence *reinforces* the one-CTI-calibration-source
claim rather than undercutting it. **The direction of cycle 118's judgement is sound; only its
numerator is wrong, and the correction argues for no score change in either direction.**

This is **not a supersession** — it was wrong when written. And it **persists**: at cycle 127 the
issue carries 15 candidates, 12 supported, still exactly two without `src-0001`, so the live
figure is ten of twelve.

### Action taken

Opened **`ctr-0063`** on `consistency-calibration-as-failure-mode` (`opened_cycle` 127,
`resolved_cycle` null). The common rules require a failed re-verification to be *entered*, not
merely logged. The entry records both the failure and the four confirmed limbs, and its step
(iii) tells the selector explicitly that a bookkeeping defect **must not by itself move the
score**. Steps (i) and (ii) — appending the corrected count to `scores.json` without editing
cycle 118's append-only block, and deciding whether either `src-0001`-free candidate counts as
independent calibration support — are reserved to a T4 or T2. **A T5 does not investigate, and I
did not.**
...
**T5 — Select.** `prompts/t5_select.md` steps 1–4, applied mechanically.

### Step 0 — the field name, verified before ranking anything ([340])

The schema's field is **`depends_on`**, with the `s`. Cycle 124 queried `.depend_on`, got `null`
ten times, and concluded upstream-first "IS INERT IN THIS GRAPH". I ran the corrected query and
**five of ten issues carry a non-empty `depends_on`**. Rung 3(a) is live and does real work
below.

### Steps 1–2 — candidate set and base priority

All ten issues score < 5, so **all ten are candidates**. Eight at **2**, two at **3**
(`ttp-attack-mapping-reliability`, `incident-evidence-vendor-concentration`). The live spread is
**one bit**, so the tie-breaks do essentially all of the work.

### Step 3 — the full ranking table

Window for 3(b) is the last 5 cycles = **122–126**. I re-derived it from the `attempts` arrays
rather than copying cycle 124's or 125's numbers, as instructed.

| # | issue | score | `depends_on` non-empty? | 3(a) | attempts | in 122–126 | 3(b) penalty | effective | created | rank |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | **institutional-incident-real-world-impact** | 2 | no | **survives** | 12,59,72,90,104,114 | — | +0 | **2** | 2 | **1 — SELECTED** |
| 2 | ioc-extraction-reliability | 2 | no | survives | 9,21,35,52,65,84,93,108,117 | — | +0 | 2 | 2 | 2 |
| 3 | automated-triage-under-refusal | 2 | no | survives | *(empty)* | — | +0 | 2 | **16** | 3 — out at 3(c) |
| 4 | consistency-calibration-as-failure-mode | 2 | no | survives | 3,15,16,25,39,62,75,80,87,96,111,**122** | **122** | **+1** | **3** | 2 | 4 — out at 3(b) |
| 5 | attribution-confident-wrong-gap | 2 | **yes** | out | 16,28 | — | +0 | 2 | 2 | 5 — out at 3(a) |
| 6 | attribution-expressed-confidence-unmeasured | 2 | **yes** | out | 49 | — | +0 | 2 | 45 | 6 — out at 3(a) |
| 7 | extraction-vs-reasoning-ordinal-axis | 2 | **yes** | out | 17,18 | — | +0 | 2 | 16 | 7 — out at 3(a) |
| 8 | task-dependent-reliability-framing | 2 | **yes** | out | 6,16,**125** | **125** | **+1** | 3 | 2 | 8 — out at 3(a) |
| 9 | ttp-attack-mapping-reliability | **3** | no | — | 16,31,32,46,55,68 | — | +0 | 3 | 2 | 9 — out at step 2 |
| 10 | incident-evidence-vendor-concentration | **3** | **yes** | — | 101 | — | +0 | 3 | 100 | 10 — out at step 2 |

Reading of 3(a) as applied: an issue with an empty `depends_on` is dominated by nothing, so it
outranks every tied issue that lists an upstream. Four of the eight tied at 2 survive. This
matches cycle 125's corrected derivation of cycle 124's ranking.

3(b) fires on exactly **two** issues in the whole graph — `consistency-calibration-as-failure-mode`
(attempt at 122) and `task-dependent-reliability-framing` (attempt at 125). The latter was already
out at 3(a), so only the former is actually moved by the penalty. **The window has moved since
cycle 124 and I re-derived rather than inherited it.**

3(c) eliminates `automated-triage-under-refusal` on `created_cycle` 16 against 2.

### The policy does not terminate

That leaves a **two-way tie**: `institutional-incident-real-world-impact` and
`ioc-extraction-reliability` — same score, both survive 3(a), both take no penalty, both created
at cycle 2. **`prompts/t5_select.md` has no fourth rung.** This is carry-forward [339]/[341] and
it is now blocking a live selection for the second time in four cycles.

I broke it with a declared **ad-hoc fourth rung: least-recently-attempted** (last attempt 114 vs
117 → this issue). I chose that rung because it is *continuous with the stated policy* — 3(b)
already encodes an anti-thrashing preference against recency, and "oldest attempt first" extends
that same principle rather than importing a new one. **It is not in the prompt, and a human should
rule on it.**

I tested four other candidate rungs to see whether the choice is robust, and report the dissent
honestly:

| candidate 4th rung | institutional | ioc | winner |
|---|---|---|---|
| least recently attempted *(chosen)* | 114 | 117 | **institutional** |
| fewest attempts | 6 | 9 | **institutional** |
| most open contradictions | 11 | 3 | **institutional** |
| alphabetical by id (content-free control) | `insti…` | `ioc…` | **institutional** |
| **most open questions** | 9 | **14** | **ioc — dissents** |

Four of five agree; one dissents. The selection is robust but **not unanimous**, and I am not
claiming it is.

### Step 4 — the refresh rule, computed here and not inherited

`collect_refresh_every: 7`. **127 % 7 = 1 ≠ 0**, so cycle 127 is **not** a refresh and the next
task is a **T3**, not a T1. (7 × 18 = 126, so cycle 126 hit the modulus exactly — but the rule is
evaluated by the T5 on *its own* cycle number, not its predecessor's.) On an unbroken
T3→T4→T5 ladder the next T5s land at 130 and 133; **133 = 7 × 19, so the refresh T5 is cycle 133
and the T1 it schedules runs at 134.** If the ladder slips, recompute.

Not all issues score 5 (total 22), so the T2 branch does not fire and I did not take it.

### What I did not do

I did **not** steer the selection toward `task-dependent-reliability-framing` to reach the
`src-0006` metric-section commission, which the queue entry named as the single highest-value
item in the base and explicitly warned me not to steer toward. It came 8th, eliminated at 3(a).
The commission stays owed and is carried forward.

---

## Changes made

1. `state/issues/graph.json` — **appended one contradiction, `ctr-0063`** (G2 failure, above).
   Counts moved `ctr` 62→63, `open` 46→47. Issues, `_schema` and the first 62 contradictions are
   byte-identical; the append-only proof returned `true true true true` before the `mv`, and the
   top-level key list is unchanged. **No score, issue, source or existing contradiction was
   touched.**
2. `state/queue/next_task.json` — **T3 on `institutional-incident-real-world-impact`**,
   `attempt_count` 0, quoting all nine of that issue's current `open_questions` from their
   verbatim openings with per-question retrieval instructions.
3. `state/queue/last_completed_task.txt` — `T5 select`.
4. `logs/cycle-127.md` — this file.

**Not touched:** `scores.json`, `index.json`, any `src-*.md`. A T5 does not score, collect or
investigate.

### A note on the queue entry's quoting requirement

`prompts/t5_select.md` requires the T3 entry to *quote* the target issue's `open_questions`. This
issue's nine questions total **~55 KB** (5327 / 2813 / 6125 / 9887 / 2501 / 5429 / 10827 / 6480 /
6232 characters) and cannot be reproduced whole in a queue entry. I quoted each one's verbatim
opening, gave its index and exact length, and gave the `jq` slice command to read each in full —
plus a full verbatim quotation of the operative sentences of `open_questions[8]` item (1), which
is the commission. **I am flagging this as a partial satisfaction of the prompt's literal wording,
not a full one.** Recorded as **[342]**.

---

## Next task rationale

**T3 (investigate) on `institutional-incident-real-world-impact`**, per the state machine's
T5→T3 edge and the refresh arithmetic above.

The issue's own route ledger (`open_questions[8]`, written by cycle 114) already names the
commission, so I did not invent one:

> OPEN, CHEAP, UNTRIED, AND THE HIGHEST-VALUE ITEM ON THIS ISSUE: DIRECTLY OBSERVE WHETHER PwC
> MIDDLE EAST'S FOUR REPORTS REMAIN PUBLISHED.

It matters twice: it settles the unverified retention limb that `ctr-0060` opened and that holds
`open_questions[3]` predictor (a)'s rejection at *provisional* rather than *established*; and per
cycle 114 it is **the only route on this issue independent of GPTZero**, which is the ancestor of
every other line of evidence here. The attempts array ends at 114, so the route is still untried
at cycle 128. The KPMG equivalent has the same character.

I carried cycle 114's discipline into the entry verbatim — these are observations of *publication
state*, not of citation accuracy, they yield no count, and the T3 must record what it observed and
the date and **must not infer a cause**. Either outcome is a result. I also carried the closed-route
ledger (FT confirmed dead in both URL forms; the GPTZero Deloitte lead expressly *not* commissioned
because a fifth institution tracing to GPTZero does not move the independence bar), the bans that
sit on this issue's own evidence (`src-0037` and `src-0036` banned outright, `src-0034`'s tiles and
40-of-45 figures banned), the `open_questions[7]` numeral-collision hazard, and the reminder to
append `128` to the `attempts` array — which past T3s have forgotten, silently corrupting the
...
- **Web searches: 0. Web fetches: 0.** A T5 should spend none, and the G2 was a zero-fetch
  re-derivation from git history — the standard held by cycles 121–127.
- Bash calls: ~24 (all `jq`, `git show`, `grep -n`, `wc`, `mv`, `rm -f`).
- Tool turns: ~40 of 75.
- Files written: 4 (one `graph.json` splice via the [237] two-input pattern, two queue files, one
  log). One scratch JSON created and deleted.
