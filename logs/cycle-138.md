...
T5 (Select). Phase check first, then G2, then the selection.

**Phase check.** `state/queue/last_completed_task.txt` read `T4 assess`. `git show --stat HEAD`
gave e37d7ef *cycle 137: T4 assess*, touching `state/assessments/scores.json`, `state/meta.json`,
both `state/queue` files and `logs/cycle-137.md` — plus the harness transcript, which is not the
cycle's work. `state/knowledge/` and `state/issues/graph.json` are both untouched by cycle 137, as
a T4 requires. HEAD was a cycle commit, so no walk-back was needed.

All three confirming checks pass, each re-run at source:

| check | expected | observed |
|---|---|---|
| `scores.json` | `last=137 n=10 total=22 max=3` | `last=137 n=10 total=22 max=3` |
| `graph.json` | `ctr=68 open=50 issues=10 last=ctr-0068` | `ctr=68 open=50 issues=10 last=ctr-0068` |
| `index.json` | `srcs=47 last=src-0047` | `srcs=47 last=src-0047` |

The score total has now been frozen at 22 since cycle 118.

**Refresh rule.** 138 % 7 = 5, not 0, so the outgoing task is a T3 and not a T1. Cycle 137's queue
entry said the refresh "fires at 140"; that is wrong in a way worth recording, because 140 is a T4
and only a T5 applies the refresh rule. T5s land on 138, 141, 144, 147 — and 147 = 7 × 21 is the
first of those that is a multiple of 7. **The next T5 that can actually fire the refresh is cycle
147.** Recorded as information; not acted on.

I did not run `scripts/validate_state.py` — Bash in this environment refuses `python3`. Every gate
statement above is a hand-check with `jq`.

## Retrospection

**Target.** Per the round-2 order (137 took 128's), the next unexamined predecessor is cycle 129.
`git log --oneline | grep -n "cycle 129"` → 9c48513; `git show --stat 9c48513` confirms it is a T4
that wrote `scores.json` and added 7 lines to `graph.json` (it opened ctr-0065). Cycle 129's entered
conclusions are therefore the scores and rationales it wrote.

**The conclusion re-checked.** Cycle 129's discharge of **ctr-0063 step (i)**, recovered verbatim
from `git show 9c48513:state/assessments/scores.json`:

> at the cycle-129 snapshot this issue carries FIFTEEN candidate_resolutions, TWELVE supported, of
> which exactly TWO cite no src-0001 in their evidence array — one resting on src-0018 alone and one
> on src-0013 alone. THE TRUE LIVE FIGURE IS THEREFORE TEN OF TWELVE

**Instrument reproduced before judging it**, per [337] — the triple is (FIELD = `candidate_resolutions[].status == "supported"` and `evidence` containing `src-0001`; CASE = issue
`consistency-calibration-as-failure-mode`; SNAPSHOT = `9c48513:state/issues/graph.json`). Result:

```
n=15 supported=12 sup_with_0001=10 sup_without=2
```

and the two src-0001-free supported candidates are **index 8, evidence `src-0018`** and **index 9,
evidence `src-0013`** — both single-source, exactly as cycle 129 described them.

**Verdict: RE-VERIFIES EXACTLY.** Every number, and both identifications, reproduce on cycle 129's
own snapshot. No contradiction opened.

**Supersession, reported as such and not as a failure.** The same instrument on the *current*
snapshot gives `n=19 supported=13 sup_with_0001=11 sup_only_0001=4`. The live figure is now eleven
of thirteen, not ten of twelve. That is a state change since cycle 129 (four candidates added, one
of the new ones supported), not an error in what cycle 129 entered; cycle 137 had already re-derived
the current counts and this cycle reproduces them independently. Cycle 129's *substantive* ground
also survives intact: the only two supported candidates that do not cite src-0001 are still the
src-0018-alone and src-0013-alone pair, neither of which counts as a second CTI calibration source,
so "this base has exactly one CTI calibration source" is unmoved.

**Cost: ZERO FETCHES.** Entirely `git show … | jq`. Cycles 121–133 also spent zero, 134 spent one,
135–137 spent zero.

## Changes made

Nothing outside the queue and the log. A T5 writes no source, no graph entry and no score, and I
appended nothing to any `attempts` array.

- `state/queue/next_task.json` — T3 on `institutional-incident-real-world-impact` for cycle 139.
- `state/queue/last_completed_task.txt` — `T5 select`.
- `logs/cycle-138.md` — this file.
- `state/meta.json` — cycle counter, written by the runner.

## Next task rationale

### The ranking, every rung worked and reported

Everything below was re-derived from the files this cycle; nothing was inherited from cycle 137's
queue entry.

**Step 1 — candidate set.** All ten issues score < 5. All ten are candidates.

**Step 2 — base priority = score.** Re-derived from `scores.json`:
...
Open contradictions per issue, re-derived with one grouped `jq` and summing to 50: ttp 6, ioc 4,
consistency-calibration 9, attribution-confident-wrong-gap 3,
attribution-expressed-confidence-unmeasured 4, task-dependent 4, extraction-vs-reasoning 2,
institutional-incident 12, automated-triage 5, vendor-concentration 1. **Every issue carries at
least one**, so the g3 demotion of 2 fires on all ten and the ceiling is 3 everywhere. The weakest
tier is score 2 with eight members; the two score-3 issues never enter the tie-breaks.

**Step 3(a) — upstream first, pairwise per ctr-0067**, restricted to the score-2 tier, taking
maximal elements. `depends_on` re-derived (and the field *is* `depends_on`, per [337]):

| issue (score 2) | depends_on | any dependency in tier 2? | survives 3(a)? |
|---|---|---|---|
...
| attribution-expressed-confidence-unmeasured | attribution-confident-wrong-gap, consistency-calibration | yes (both score 2) | no |
| task-dependent-reliability-framing | ttp, ioc, consistency-calibration, attribution-confident-wrong-gap, attribution-expressed-confidence-unmeasured | yes (four of five in tier) | no |
| extraction-vs-reasoning-ordinal-axis | task-dependent-reliability-framing | yes (score 2) | no |

**I record plainly that this disagrees with cycle 137's queue entry**, which named only
task-dependent and extraction-vs-reasoning as excluded. Applied consistently, the pairwise rule also
excludes attribution-confident-wrong-gap (its sole dependency consistency-calibration is in the same
tier) and attribution-expressed-confidence-unmeasured (both of its dependencies are). **Four of the
eight score-2 issues are unreachable by the selector, not two.** That widens honesty item (C) rather
than changing the winner — none of the four would have won anyway — but it is the honest arithmetic
and it is what a successor should re-derive against.

**Step 3(b) — attempt penalty**, +1 per attempt within the last 5 cycles (window 133–137):

| survivor of 3(a) | attempts | in window | penalty | effective |
|---|---|---|---|---|
| ioc-extraction-reliability | …, 108, 117, 131 | none (131 is 7 back) | 0 | 2 |
| consistency-calibration-as-failure-mode | …, 111, 122, 136 | **136** | **+1** | **3** |
| institutional-incident-real-world-impact | …, 104, 114, 128 | none (128 is 10 back) | 0 | 2 |
| automated-triage-under-refusal | *empty* | none, ever | 0 | 2 |

consistency-calibration drops to effective 3 and is out.

**Step 3(c) — older `created_cycle` first:**

| remaining | created_cycle | outcome |
|---|---|---|
| ioc-extraction-reliability | 2 | survives |
| institutional-incident-real-world-impact | 2 | survives |
...
| 1= | ioc-extraction-reliability | 2 | 0 | 2 | tie unbroken by policy |
| 3 | automated-triage-under-refusal | 2 | 0 | 2 | 3(c), created 16 vs 2 |
| 4 | consistency-calibration-as-failure-mode | 2 | +1 | 3 | 3(b) |
...
### The tie-break defect, hit live for the third time

**The written policy does not decide this cycle.** `institutional-incident-real-world-impact` and
`ioc-extraction-reliability` are tied at score 2; both are roots, so 3(a) does not separate them;
neither carries a 3(b) penalty; both have `created_cycle` 2, so 3(c) does not separate them. And
`prompts/t5_select.md`'s tie-break list ends at 3(c).

**I broke the tie without authority.** The rule I used is **least-recently-attempted**:
institutional-incident was last attempted at cycle 128, ioc at cycle 131, so institutional-incident
is selected. This is *not policy*. It is the same ad-hoc rung cycles 127 and 130 invented, which
cycle 136's G2 independently reproduced as a genuine gap rather than an alleged one. This is its
**third unauthorised application**, and honesty item (B) is re-raised for the ninth time. A human
should rule on a fourth rung. I chose the same rule the two prior cycles used rather than a new one,
purely so the record stays auditable — that is a reason, not an authority.

### Honesty item (A): automated-triage-under-refusal, thirtieth cycle

It survived 3(a) (it is a root, so 3(a) never touches it) and 3(b) (never attempted, so it can never
take a penalty). **It was eliminated at rung 3(c), on `created_cycle` 16 against 2** — the fourth
time (127, 130, 133, 138) it has gone out on exactly that rung. The condition under which it could
win — every score-2, `created_cycle`-2 root carrying a 3(b) penalty simultaneously — was checked and
did not hold: of ioc (131), consistency-calibration (136) and institutional-incident (128), only
consistency-calibration had an attempt inside the window. I worked the rungs in order and did not
steer, in either direction.

**One thing this cycle can add that the previous twenty-nine could not.** Item (A) and item (B) are
*not the same problem*, and it would be easy to assume they are now that both fired in one cycle.
automated-triage is eliminated by a **written** rung, above the point where the policy runs out. A
fourth rung would therefore not save it. Fixing (B) does not fix (A), and only a staleness or
never-attempted term inserted **at or above 3(c)** would. A human ruling on (B) alone would leave
this issue starved.

It now has a defined piece of work — its `candidate_resolutions[1]`, a T3 reading src-0047 end to
end for the table behind the high-confidence-recall claim and for any reported escalation or
deferral rate. Carried forward, not commissioned.

### Why the selected issue is worth the cycle

Beyond the arithmetic: `institutional-incident-real-world-impact` carries **twelve** open
contradictions, the largest block in the graph and roughly a quarter of the base's 50, so honesty
item (E)'s overcount lands here hardest. Its `open_questions[8]` is a maintained **route ledger**
whose cycle-128 paragraph (5) already names the cheapest open work, so the incoming T3 spends no
turns rediscovering the agenda: (i) whether any PwC citation was in fact updated — which I read as
**blocked**, since a diff needs the archive route this base records as closed; (ii) whether KPMG ever
named a cause — cheap and untried; (iii) a **non-Big-Four commercial response** — the highest-value
missing data point since cycle 100, and the only route that could actually widen the evidence base
past the GPTZero independence bar. The T3's instructions quote paragraph (5) verbatim and flag the
independence bar, the pwc.com G1 hazard, and the twelve contradictions by id.

The outgoing entry is `task_type` T3, `target_issue` `institutional-incident-real-world-impact`,
`attempt_count` 0, and it carries the full ban set, the killed-lead list and the carry-forward list.

## Budget

Fetches: **0** (G2 and the whole selection were done at zero — `git show … | jq` throughout).
Web searches: 0. Bash calls: 15. File writes: 3 (two queue files, this log). Read-tool calls: 3.
Turns: well inside the 75 cap. One `jq` dump of `open_questions` overflowed to a persisted file at
61 KB and was re-run as an indexed head listing — recorded below as [374].
