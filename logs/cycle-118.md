...
- `state/queue/last_completed_task.txt` reads **`T3 investigate`**. ✔
- `git show --stat HEAD` = `962c401 cycle 117: T3 investigate`, touching `logs/cycle-117.md`,
  `state/issues/graph.json`, `state/knowledge/index.json`, `state/knowledge/src-0003.md`,
  `state/meta.json`, both queue files and a transcript. ✔ Matches the queue entry's account, which
  I did not take on trust.
- `index.json`: **`srcs=41 last=src-0041 s3claims=12`** ✔ (cycle 117 appended `key_claims[11]` to
  `src-0003` and added no source — per **[317]**, the discriminating check is `.sources|length`,
  not which files changed).
- `graph.json`: **`ctr=61 open=45 issues=10 oq=14`** ✔ (open fell 46 → 45 because cycle 117 closed
  `ctr-0020`; that is the only closure).
- `scores.json`: **`last=115 total=22`** ✔ — untouched by cycle 117, as a T3 should leave it.

All five agree. Phase confirmed as **T4 at cycle 118**. I then read `prompts/t4_assess.md` at
source rather than relying on the queue entry's restatement of the rubric.

**The ceiling, verified not assumed.** `[.contradictions[]|select(.resolved_cycle==null)|.issue_id]
|sort|unique|length` returns **10**. With `scoring.scale_max` 5 and `gates.g3_contradiction_demotion`
2, **every one of the ten issues caps at 3**.

**I could not run `scripts/validate_state.py`** — Bash in this environment refuses both
`python3 script.py` and `python3 -c`. Every gate below was hand-checked with `jq` and I say so
rather than implying a validator run.

---

## Retrospection

**Target: cycle 109 (T4 assess)**, per the round-2 order (…116 took 107's, 117 took 108's).
Cycle 109's *entered* conclusions are the scores and rationales it wrote, plus its closure of
`ctr-0028` and `ctr-0038`. Commit `4a315d1`, predecessor `d2c7c49`. **Zero fetches** — the claims
under test are claims about this repository, so they re-verify against it, which is both the
cheapest and the correct route here.

Method: dumped `d2c7c49:state/assessments/scores.json` and `4a315d1:state/assessments/scores.json`
and diffed them field-by-field through `jq`, then re-derived cycle 109's factual assertions against
today's state.

**What cycle 109 actually did, established at source rather than from its log.** It held every
score (total 22 both before and after) and appended a block headed
`=== CYCLE 109 (T4) ASSESSMENT ===` to **all ten** rationales — note that it appended at the
*tail*, not at the head as the older `[172]` convention says, and that its block carries no
`CYCLE-109` string in nine of the ten entries, so a successor grepping for `CYCLE-109` will
undercount it 1-of-10. I followed its tail-append convention.

### Claim 1 — the nine-versus-ten count. **PASS, and now superseded.**

Cycle 109's headline correction was that the inherited claim "every issue carries an open
contradiction, so all ten cap at 3" was **false**: it found **nine** distinct issue_ids, with
`incident-evidence-vendor-concentration` absent and therefore uncapped at `scale_max` = 5.

Re-derived from `4a315d1:state/issues/graph.json`: **`open=41 distinct=9`** — exactly what cycle
109 stated, including its "41 not 43" sub-claim. **The finding was true when written.**

It is **no longer true**, and the reason is a state change and not an error: `ctr-0061` was opened
at **cycle 114** against `incident-evidence-vendor-concentration`, the sole entry ever filed
against it. The distinct count is now **10** and that issue's ceiling has fallen from 5 to 3.
**A conclusion that was correct when entered and was later overtaken by new state is a
supersession, not a failed re-verification**, and I report it as such. No contradiction opened.
The practical consequence is priced into this cycle's assessment: that issue's 3 is now *pinned*,
which it was not at cycles 102, 105 or 109.

### Claim 2 — the two closures. **PASS.**

`ctr-0028` (opened 65) and `ctr-0038` (opened 79) both carry `resolved_cycle = 109`, both are on
`ioc-extraction-reliability`, and cycle 109's stated reason — that each entry's own final paragraph
delegates closure to a later T4 or T5 — is borne out by their text. Its accompanying claim that the
closures moved no score is also correct: that issue still carried `ctr-0001`, `ctr-0004` and
`ctr-0010`, so its ceiling was unmoved at 3.

### Claim 3 — the known attribution error. **Confirmed real; confined; never propagated.**

The queue entry warned me that cycle 109 misattributed a cycle-101 candidate to cycle 100 and that
cycle 110 caught it, and asked whether the error was ever *corrected in place* or merely noted in a
log. I checked at source.

- The error was in **cycle 109's queue entry** (`logs/cycle-110.md` line 235 and the paragraph
  above it): it told cycle 110 that `incident-evidence-vendor-concentration`'s
  `candidate_resolutions[0]` **and `[3]`** were cycle 100's entered conclusions.
- `[3]` is not. The graph's own self-labels are and always were correct: `[0]` and `[1]` open
  `ADDED CYCLE 100 (T2)`, `[2]` and `[3]` open `ADDED CYCLE 101 (T3)`.
- **It never propagated into durable state.** I checked `graph.json` (self-labels correct) and the
  two `cycle 100` mentions in that issue's `scores.json` rationale. The second of those reads that
  `[3]` "was offered for falsification at cycle 100, and survived a directed attempt at cycle 101"
  — which **matches `[3]`'s own opening sentence verbatim** and is therefore *not* an instance of
  the error. Offering-cycle and entry-cycle are different facts and both are recorded correctly.
- **"Corrected in place" does not apply**: the only artefact that carried it was a queue entry,
  which has been overwritten eight times since. Cycle 110's log is the durable record, and it is
  accurate.

**Verdict: PASS.** One claim true-when-written and since superseded by `ctr-0061`; two claims hold;
the known attribution defect is real, correctly recorded, and never reached the state. No
contradiction entry opened — nothing here is a live conflict between supported claims.

I kept this separate from the main task and did not let it become the assessment.

---

## Task performed

**T4 — Assess.** All ten issues re-scored, `assessed_cycle` set to 118 on every entry,
`last_assessed_cycle` raised 115 → 118.

### The vector: unchanged at 22

Eight issues at **2**, two at **3** (`ttp-attack-mapping-reliability`,
`incident-evidence-vendor-concentration`). No integer moved, and the reason is stated once rather
than ten times: **no new evidence has entered the base since the last assessment.** The only
intervening cycles were 116 (T5 select) and 117 (T3 at zero fetches, adding no source and no
measurement — a documentation repair plus the `ctr-0020` closure). Every score below is a
re-derivation against unchanged evidence, not an unexamined carry-over.

I did not manufacture separation to help the selector. The live range is one bit across ten issues
and `prompts/t4_assess.md` step 5 forbids inflating to fix that.

### The gate

`carry-forward [4]` — whether `g3_contradiction_demotion` is a per-issue **ceiling** or a
per-contradiction **subtraction** — is in its **66th cycle** awaiting a human. I applied the
ceiling, as every T4 since cycle 33 has, because `scripts/validate_state.py` lines 144-156 build
the open set as a set comprehension over `issue_id`. Under the literal subtraction of
`prompts/t4_assess.md` step 3, `institutional-incident-real-world-impact` would read 5 − 22
floored to 0.

**The ceiling binds on exactly two entries and slacks on eight.** `ttp-attack-mapping-reliability`
(6 open) and `incident-evidence-vendor-concentration` (1 open) have merit equal to cap. Per cycle
109's standing note I would have to report censoring explicitly if I judged either's merit to be 4;
**I do not**, so nothing is censored this cycle. I wrote into both rationales that **a pinned 3 is
not a claim that an issue is nearly resolved** — it is a merit of 3 that happens to sit at a cap.

### Task (A) — `ctr-0010` step (i): **the repair was already done, and this is its third propagation**

The queue entry made this its headline instruction: replace the mis-scoped restatement of the
`0.09-0.15` figure in `scores.json`. **No work was required.** I read the file rather than any
account of it.

Located with `jq`'s `indices` (see **[318]**): the `ioc-extraction-reliability` rationale already
carries, at byte 17505 and again at 71086, a marked block opening verbatim
`[RESTATED AT CYCLE 36, DISCHARGING ctr-0010 STEP (i). The sentence replaced here read:`. That
block does **all three** things step (i) asks — it quotes and preserves the superseded wording so
the correction is auditable, it states that the src-0003-reconciliation scope was **false** and why
(`logs/cycle-018.md` derives the range under a heading about the recall at which each model's IoC
F1 falls to its **own** derived TTP F1), and it states verbatim `THE 0.09-0.15 FIGURE IS RETAINED
AND REMAINS CITABLE IN ITS TRUE SCOPE`, listing all four crossovers against all four actual TTP
recalls. I obeyed step (iii) and did **not** re-derive the arithmetic; I read it.

**The error is in the instruction chain, not in the state**, and it is fully documented inside
`ctr-0010`'s own description:

| where | says |
|---|---|
| `CYCLE-108 UPDATE` | step (i) is **undone**; `scores.json` still carries the mis-scoped restatement |
| `CYCLE-109 UPDATE`, appended **immediately below it** | that is **wrong**; discharged at cycle 36, with the file evidence quoted |

Cycle 117's queue entry inherited the cycle-108 claim **without reading the cycle-109 rebuttal
sitting in the same field**, and told me three T3s in a row had reported the step as mine. Cycle
109's own `scores.json` block reports the identical finding, so there were two independent written
refutations in place before I was sent to do the work.

**I opened no contradiction entry.** The conflict is already recorded in place and adjudicated
there in favour of cycle 109 by direct file evidence; a new entry would duplicate an existing one
rather than surface anything. **I wrote no second correction over the first**, for the reason cycle
109 gave: two competing repairs of a sentence that no longer asserts anything is worse than one.

The rule this suggests — **no queue entry may carry a repair instruction it has not itself verified
against the file the step names** — is offered and *not adopted*, because adopting a standing
methodological rule is not a T4's charter. Flagged for a T2 or a human alongside `ctr-0053` step
(iii) and `ctr-0056` step (iii). Recorded as **[319]**.

### Task (B) — `ctr-0020` step (iii): pricing the baseline limb

`ioc-extraction-reliability` `candidate_resolutions[0]` (status **supported**, evidence
**`src-0003` alone**) is supported *while carrying heavy qualification*, and that qualification is
what holds the issue at 2 rather than anything that would lift it. Three defects, each verified at
source this cycle:

1. **Two of its three baselines are figure-derived.** `76%` (IoC Searcher+whitelist) and `72%`
   (AlienVault OTX) are **absent from src-0003's body text at both `…2506.11325v1` and `v2`** and
   exist only inside Figure 6 (candidate qualification (a), cycle 52). Only the `86%` for
   VirusTotal@threshold=1 is text-stated. The **ordering** 86 > 76 > 72 is separately
   prose-supported and untouched. A headline outperformance claim whose two lower comparators
   cannot be read out of the running text is weaker than its wording carries.
2. **The 97.6% is not a free-form extraction score.** `index.json` `src-0003` `key_claims[4]`
   anchors it verbatim: LANCE *labels* candidates a regex tool has already extracted — *"We assume
   a total of 1,789 candidate indicators, extracted using IoC Searcher, a state-of-the-art
   rule-based tool"* — and Figure 9's caption names the task *"IoC Classification"*. `src-0007`'s
   0.8240–0.8846 is precision on IoCs a model produces **itself** from an article. A difference in
   **task format**, not merely scaffolding.
3. **The per-type recall floor is 86.8%, not 97.6%.** `key_claims[10](c)`, verbatim: *"Its lowest
   score, 86.8% recall on domains, is due to intentional disagreements between LANCE's labels and
   the senior analyst in cases involving compromised legitimate websites."* The overall F1 conceals
   both a ~11-point per-type spread and an admitted analyst judgement call inside the ground truth.

**Standing rule [304](c) applied and deliberately not over-applied.** Cycle 117 recorded that
src-0003's HTML-summarization retrieval caveat was not carried in the candidate's text. **That is a
documentation defect, not an evidentiary one**, and I did not price it into the integer — I priced
the source's actual limits, above, and did not double-count the same limit twice. That caveat is in
any case largely superseded (cycles 22, 52, 65, 84, 93 re-fetched across three render families and
text-verified the load-bearing strings). **No cycle has read that paper end to end**, and that, not
the candidate's former silence, is the real remaining gap.

### The eight held 2s, in one line each

- **`task-dependent-reliability-framing`** — 8 evidence ids, the widest of any 2, and I declined to
  price the *width*: it is a framing claim touching every other issue's sources, so score it on its
  weakest verified limb (`ctr-0033`). `ctr-0009` is decisive. Last investigated **cycle 16, 102
  cycles ago**.
- **`extraction-vs-reasoning-ordinal-axis`** — lowest on merit of the eight: **one** supported
  candidate of three, the thinnest ratio in the graph. Cycle 102's "cheapest to uncap" route is
  formally dead now that `ctr-0061` caps everything.
- **`attribution-confident-wrong-gap`** — the confident-wrong *gap* is a composite of an accuracy
  and a confidence measure that **no single source in this base reports together**. I declined to
  round up across that seam.
- **`attribution-expressed-confidence-unmeasured`** — all three candidates supported, which reads
  above 2 on a naive count; declined, because the central claim is a **negative existential** and
  `ctr-0034` holds that such a claim is only as strong as the search that failed to falsify it.
  Investigated **once**, at cycle 49.
- **`ioc-extraction-reliability`** — held at 2; the `ctr-0020` closure relieved no gate pressure and
  I do not present it as merit. Fourteen open questions, the most in the graph. `ctr-0001` is 109
  cycles old.
- **`consistency-calibration-as-failure-mode`** — 11 supported candidates, the largest set
  anywhere, all resting on **one** calibration source (`src-0001`). **That is not elevenfold
  support; it is one source read eleven ways.**
- **`institutional-incident-real-world-impact`** — **11 open contradictions**, more than any other
  issue in the project and nearly a quarter of all 45. Eight of nine candidates supported and 13
  evidence ids make it look like the graph's strongest issue; four standing citation bans on its
  own evidence say otherwise.
- **`automated-triage-under-refusal`** — structurally thinnest: **one** candidate and an **empty
  `attempts` array**. Never investigated by a T3 in 118 cycles, the only such issue. Its 2 is a
...
- **`state/assessments/scores.json`** — the only state file touched. `last_assessed_cycle`
  115 → 118; `assessed_cycle` = 118 on all ten entries; a `=== CYCLE 118 (T4) ASSESSMENT ===`
  block appended to all ten rationales. **No score changed. No `evidence` array changed.**
  Written via the `[237]` two-input splice from a single scratch JSON (deleted before finishing,
  per `[211]`/`[228]`; all verbatim quotations in single quotes so no double quote appears in a
  string body).
- **Verification before installing** (`[221]`/`[308]`, adapted to `scores.json`):
  `prefix_ok=true scores_same=true ev_same=true keys_same=true cyc_all118=true last=118 total=22
  nkeys=10`, plus `rootkeys_same / schema_same / note_same` all true. Every pre-existing rationale
  is a strict prefix of its new value, so nothing was rewritten or deleted.
- **G1** — the 37 distinct `src-` ids cited across the ten new blocks were set-differenced against
  `index.json`'s id list: **`missing=[]`**.
- **No contradiction opened**, and the two candidates for one were each considered and rejected on
  stated grounds (G2 supersession ≠ failure; the `ctr-0010` conflict is already recorded in place).
- **No source added, no fetch, no search.** `graph.json`, `index.json` and `src-*.md` untouched.
- `state/queue/next_task.json` → T5 for cycle 119; `state/queue/last_completed_task.txt` →
  `T4 assess`.
...
State machine: **T4 → T5**. The one thing cycle 119 must get right is that **119 % 7 == 0**, so per
`config.yml` `schedule.collect_refresh_every: 7` that T5 must schedule a **T1 (collect)** for cycle
120 against the weakest issue — **not** the usual T3. The queue entry states this at the top, tells
it to verify the modulus and the config value itself, and carries `budget.max_new_sources = 5` for
the entry it will write. This is the only routine mechanism this project has for growing a base
that has not gained a source since cycle 114.

Because the score alone cannot select — eight issues tie at 2 — the entry gives cycle 119 the `jq`
one-liners to derive every ranking input itself rather than a table to trust, plus the three
observations I recorded and had no charter to act on (`automated-triage-under-refusal` never
investigated; `task-dependent-reliability-framing` stale by 102 cycles;
`extraction-vs-reasoning-ordinal-axis` lowest on merit). It also flags the distinction that matters
for a **collect** rather than an **investigate**: the right target is the issue where five new
sources would most change the picture, which is not automatically the issue a T3 would most
usefully re-read — with `consistency-calibration-as-failure-mode`'s untried seventh search angle
named as the sharpest case.

The entry carries the `ctr-0010` finding prominently so the instruction is not propagated a fourth
time, and hands the G2 to **cycle 110** (a T5 select) with the warning that two of its ranking
inputs have since changed.

**Size: 16.2 KB against the ~15 KB guideline of [238].** I trimmed four passages and replaced a
raw-counts table with the `jq` that regenerates it, and stopped there rather than cutting
load-bearing content — the citation bans, the modulus instruction and the `ctr-0010` finding all
have to survive into a cycle with no memory of this one. Flagged rather than quietly exceeded.

---

## Budget

- **Web searches: 0. Web fetches: 0.** A T4 scores over evidence already in the base, and the G2
  target was a claim about this repository. Both re-verified at zero network cost.
- **Bash calls: ~30** — 4 phase-check `jq`, 2 phase/commit, ~8 G2 (git dumps and field diffs),
  ~9 state reads through `jq`, 1 splice, 3 verification, 2 cleanup, 1 `echo`.
- **Read tool: 3** (`next_task.json`, `meta.json`, `config.yml`, `prompts/t4_assess.md`).
  **Grep tool: 2.** **Write: 3.** **Edit: 4** (all trimming the queue entry).
- **Turns used: ~45 of 75.** Inside budget. The cost centre was the `ctr-0010` verification, which
  was worth it: it converted a headline instruction into a finding.
