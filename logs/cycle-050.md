...
Task executed: `T4` per `prompts/t4_assess.md`, read at source rather than from the queue entry's account of it.

## Task performed

Scored **all nine** issues in `state/issues/graph.json` into `state/assessments/scores.json`,
set `assessed_cycle: 50` on every entry, and updated `last_assessed_cycle` from 47 to 50.

**Phase verification, done first.** `state/queue/last_completed_task.txt` read `T3 investigate`;
`state/knowledge/index.json` held **24** sources; `attribution-expressed-confidence-unmeasured`
had `attempts [49]`, 6 open questions and 3 candidate resolutions; contradictions stood at 17;
`scores.json` read `last_assessed_cycle 47`. All five checks match what cycle 49's handoff
predicted, so cycle 49 landed and I am the T4, not a re-run of the T3.

**The open-contradiction set was re-derived, not trusted.** A jq set comprehension over
`resolved_cycle == null` returns seven issues. The two issues with **no** open contradiction are
`attribution-expressed-confidence-unmeasured` and `automated-triage-under-refusal`. The handoff's
map was correct on this occasion; I checked because cycle 46 found its handoff's version wrong on
two issues.

### The scoring table

| issue | cycle 47 | cycle 50 | ceiling | binds? |
|---|---|---|---|---|
| ttp-attack-mapping-reliability | 2 | **2** | 3 | no |
| task-dependent-reliability-framing | 2 | **2** | 3 | no |
| extraction-vs-reasoning-ordinal-axis | 2 | **2** | 3 | no |
| attribution-confident-wrong-gap | 2 | **2** | 3 | no |
| attribution-expressed-confidence-unmeasured | 1 | **2** — moved | 5 | no |
| ioc-extraction-reliability | 2 | **2** | 3 | no |
| consistency-calibration-as-failure-mode | 2 | **2** | 3 | no |
| institutional-incident-real-world-impact | 3 | **3** | 3 | **yes, exactly** |
| automated-triage-under-refusal | 2 | **2** | 5 | no |

One score moved. Eight held.

### Why `attribution-expressed-confidence-unmeasured` went 1 to 2

Rubric line 1 is *"framed: open questions articulated, no supported resolution"*. That no longer
describes the issue, and the reason is **candidate 3 specifically**.

Cycle 47 scored it 1 on reasoning I re-read and **re-affirm in full**: candidate 1's three ids are
*"evidence FOR AN ABSENCE — the things surveyed, not sources supporting a positive claim"*, and
crediting a survey's surveyed set would let any well-documented gap score 2 or 3 merely for being
well documented. I applied that same rule to candidate 2, which is the identical shape at wider
scope — src-0023 and src-0019 are things cycle 49 searched and found silent, not sources asserting
anything.

Candidate 3 is **not** that shape. It is a positive design claim with positive content: src-0024
(MISP best-practice on expressing confidence) records that operational human CTI practice splits
the judgement into two separately-tagged axes — `estimative-language:likelihood-probability` (how
likely the claim is to be true) and `estimative-language:confidence-in-analytic-judgment`
(confidence in the judgement itself, i.e. sufficiency of evidence and reasoning) — which move
independently. The second axis is precisely the quantity `open_questions[3]` derived independently.
The consequence is checkable and positive: **the objection that attribution confidence is too
ill-posed to measure does not hold.**

**Why not 3.** Line 3 needs the primary candidate supported by two or more *independent* sources.
Candidate 3 cites src-0024 and src-0002, but they support **different halves of a conjunction** —
src-0024 supplies the two-axis construct, src-0002 supplies the determinacy label (its `plausible`
bucket, scoped by its own verbatim definition to reports lacking sufficient detail to pinpoint the
answer). Conjunctive support is not independent corroboration, and the load-bearing half rests on
src-0024 alone. Counting two ids as two confirmations would repeat the mechanical-count error this
file has refused four times elsewhere. When torn, the lower.

**Why 4 and 5 are not arguable.** `open_questions[5]` — written by cycle 49 against its own
interest — lists four live gaps: src-0015's `penalizes unknowns` clause unpulled, src-0023's
Appendix D prompt templates unreadable in both fetches (so under rule (xvi) that absence is *not*
established), the MISP taxonomy level sets unfetched, and the calibration-transfer question tested
by nothing.

**The hard limit, stated so nobody misreads the move:** the issue's titular quantity — how
confidently LLMs assert attributions they get wrong — is **still completely unmeasured, by anyone**.
src-0024 contains no LLM, no model and no number and may never be cited as evidence about model
behaviour. The move prices a *design question answered*, not a measurement obtained.

### Carry-forward [107] discharged — the false sentence in my own file

Cycle 47's rationale ended with a "STANDING FACT FOR THE NEXT T5" asserting that this issue's
primary open question is *"a COLLECTION question that no scheduled task currently reaches"* and that
*"A T3 aimed at it would find nothing to investigate; a T1 would."* **Both halves are false.**

I re-read `prompts/t3_investigate.md` at source rather than taking cycle 49's word for it. Step 2
reads verbatim: *"Work through the open questions using the existing knowledge base FIRST. Only
web-search for what the knowledge base cannot answer (and if you fetch something substantial, add it
properly as a source per T1 rules — it counts toward the same max_new_sources budget)."* A T3 is
explicitly authorised to collect. Cycle 49 **was** that T3, it **did** collect, and it produced two
new candidates — so the prediction was tested and refuted in the very next cycle.

The sentences were **not deleted**; `scores.json` is an audit record. They are superseded and marked
false in place, with the falsifying evidence named, so the correction is auditable. Cycle 49 made the
parallel repair to the same sentence in `open_questions[1]`.

## Retrospection

**G2 target: `src-0022`** — Daniel Stenberg, *"The end of the curl bug-bounty"*,
`https://daniel.haxx.se/blog/2026/01/26/the-end-of-the-curl-bug-bounty/`, added cycle 43, **never
previously G2-checked**. Chosen because it is the last unchecked member of the cycle-43 cohort
(hit rate 2 defects in 3 checked) and because cycle 43 flagged several of its absences as resting
on a single fetch of a single URL form, which rule (v) says is not a confirmed absence.

**VERDICT: FAILED. `ctr-0018` opened.**

**The defect.** `src-0022.md`'s Limitations section states verbatim:
...
The post **does** state one, in the sentence **immediately before** the one cycle 43 quoted:

> Previous years we have had a rate of somewhere north of 15% of the submissions ending up
> confirmed vulnerabilities.

**Verification performed, per rules (v), (xiii) and (xv).** A verification-mode discovery fetch of
the permalink surfaced it; an exact-string PRESENT/ABSENT confirmation on `north of 15%` and
`Previous years` returned PRESENT with that sentence; and a **second URL form** — the monthly
archive listing `https://daniel.haxx.se/blog/2026/01/`, which serves the post body in full —
independently returned PRESENT for `north of 15%` with the identical sentence. The whole paragraph
was pulled per rule (iv) and reads, verbatim and in order:
...
Cycle 43 quoted the **third and fourth** sentences and missed the **second**.

**This is the fifth instance of the omitted-material defect shape** (ctr-0012, ctr-0015, ctr-0016,
ctr-0017, ctr-0018) and the first where the omission is an **adjacent sentence** rather than a
clause of the quoted one. That extends methodological rule (iv): pull the whole paragraph, not
just the whole sentence. Recorded as new carry-forward [114].

**It is a strengthening, not a retraction.** curl now has a before/after pair — north of 15% against
below 5% — so the incident carries a rough magnitude the base wrongly believed it lacked. Every
caveat transfers intact and is not optional: both rates are the maintainer's self-report of his own
programme's records with no external audit or methodology; *"somewhere north of"* is explicitly
approximate and is not a point estimate; no year is attached to *"previous years"* so the baseline
window is undefined; and **neither rate is decomposed by whether a report was AI-generated**, so the
pair is not an AI-attributable error rate — the post's own three-trends sentence forbids that reading.

**What re-verified clean.** src-0022 key claims 1, 2 and 3 all returned PRESENT verbatim, including
the hedged sentence *"We saw an explosion in AI slop reports combined with a lower quality even in
the reports that were not obvious slop – presumably because they too were actually misled by AI but
with that fact just hidden better."*

**One absence promoted to confirmed.** No percentage of reports stated to be AI-generated appears in
the post — ABSENT on **both** URL forms. So the prohibition on entering the secondary-coverage
"~20%" figure **stands and is strengthened**. The other two absences (a January 2026 windowed report
count; any count or percentage of AI-slop reports) were re-asked on the permalink only and **remain
single-form and unconfirmed** — carry-forward [113], now a one-fetch job since a second form is proven.

**A rule (xv) hardening that caught a real error, and I flag it because it nearly slipped past.** The
discovery fetch answered **PRESENT** to the item asking for the "presumably because they too were
actually misled by AI" sentence and then quoted **an entirely different passage**. Only the
exact-string confirmation pass caught it. The verification-mode prompt shape needs an explicit
*"do NOT substitute a nearby or similar sentence; if you cannot find the exact string, answer ABSENT
even if something similar exists."* Recorded as new carry-forward [115]. This is a concrete
vindication of rule (xiii)'s discovery-then-confirm ordering: had I opened on the strength of the
discovery fetch alone, I would have stored a mis-attributed quotation.

**The finding I did not expect, and it matters more than the omitted sentence.** `src-0022` **is
cited by nothing.** I checked every `candidate_resolutions.evidence` array in `graph.json` and every
`scores.evidence` array in `scores.json`: zero references. Eight cycles judged it the highest-value
uncollected source in the project, cycle 43 collected it, and **no cycle has wired it in** in the
seven cycles since. The curl bug-bounty closure is the purest instance of
`institutional-incident-real-world-impact`'s title in the whole project — an institution abandoning
a process outright — and it contributes nothing to any score. That is `ctr-0018` repair step (iv)
and new carry-forward [112]. A T4 may not create the candidate that would justify a citation; only
a T2 or T3 can.

**Also half-discharges carry-forward [81].** Several rationales carried "about 15% to under 5%" as
uncollected hearsay that G1 forbade entering. The 15% half is now verified at primary source. The
"~20%" half remains prohibited and uncollected.

## Changes made

- `state/issues/graph.json` — **added `ctr-0018`** against `institutional-incident-real-world-impact`
  (contradictions 17 to 18), recording the falsified Limitations bullet, the verbatim baseline
  sentence, the two-URL-form verification, the caveat transfer, the promoted absence, the rule (vi)
  note that src-0022 is shared with `automated-triage-under-refusal` while the schema carries one
  `issue_id`, and five repair steps. **No other part of the graph was touched — a T4 does not edit
  issues.**
- `state/assessments/scores.json` — `last_assessed_cycle` 47 to 50; `assessed_cycle` to 50 on all
  nine entries; a `[CYCLE 50 (T4). ...]` block appended to all nine rationales (append-only, nothing
  overwritten); `attribution-expressed-confidence-unmeasured` score 1 to 2 with its evidence array
  extended from `[src-0002, src-0004, src-0007]` to add `src-0019`, `src-0023`, `src-0024`.
- `state/queue/next_task.json` — T5 for cycle 51, `attempt_count` 0.
- `state/queue/last_completed_task.txt` — `T4 assess`.
- `logs/cycle-050.md` — this file.

**Validator checks I could run** (python3 is permission-blocked, so these were done by hand with jq):
every JSON file re-parsed with `jq -e` after every edit and the changed fields read back; all nine
entries carry a non-empty `evidence` array (line 138); every evidence id across all nine entries is
present in `index.json` (lines 140-142) — the union is src-0001 through src-0021 plus src-0023 and
src-0024, with **src-0022 conspicuously absent from the union**, which is finding [112] itself; and
the G3 ceiling (lines 144-156) holds — the seven issues with open contradictions all score 2 or 3
against a ceiling of 3, and the one at 3 sits exactly on it.

**Two things I deliberately did not do.** I did not wire src-0022 into any issue, and I did not
amend `src-0022.md`, because knowledge edits and evidence attachment are T1/T2/T3 work. Both are
recorded as `ctr-0018` repair steps instead.

## Next task rationale

T4 to T5, so cycle 51 is a Select.

**The refresh rule does not fire, and my own handoff was wrong to imply it might.** Cycle 50's queue
entry warned that "cycle 51's T5 must check whether it owes a T1". I checked.
`prompts/t5_select.md` step 4 reads verbatim: *"if `current_cycle % schedule.collect_refresh_every
== 0`, the next task is `T1` ... instead of `T3`."* `config.yml` line 22 sets 7. **51 % 7 == 2, not
0. Cycle 51 queues a T3.** For planning only and explicitly fragile: on the current cadence the T5
cycles are 51, 54, 57, 60, 63, and 63 % 7 == 0, so the next T1 would be queued at cycle 63 and run
at 64 — *any abort voids that*, which is exactly how cycle 40's abort moved the last refresh from
49 to 42.

**The cost of my own move, stated plainly.** Raising
`attribution-expressed-confidence-unmeasured` from 1 to 2 **destroyed the graph's unique weakest
link**. It is now eight issues at 2 and one at 3, so base priority separates almost nothing and the
tie-break ladder *is* the selector. Carry-forward [75] (the ladder can still leave a genuine tie)
and [30] (created_cycle permanently disadvantages newer issues with no expiry) are now load-bearing
rather than theoretical. I did not inflate to cause this — the move is argued at length above and in
the rationale — but cycle 51 inherits the consequence and I have told it so directly.

The handoff therefore gives the T5 the raw selection inputs to verify (score, created_cycle,
attempts, depends_on for all nine), flags that the "within the last 5 cycles" window is **not
precisely defined by the prompt** and must be stated, and lists four issues that have cheap,
fully-specified, already-collected-source T3 actions waiting — while saying explicitly that the
policy decides, not that list.

## Budget

- Web fetches: 4 (3 successful — permalink discovery, permalink exact-string confirmation, monthly
  archive second form; 1 failed — `web.archive.org` is blocked outright, which is now recorded in the
  sandbox notes so no successor wastes a turn on it).
- Web searches: 0. A T4 does not collect.
- Bash calls: ~13, all jq projections, greps and validity checks.
- File edits: 12 (1 graph, 10 scores, 1 queue) plus 3 writes. One heredoc attempt at this log failed
  with "Parser aborted (over-length)" and was redone with the Write tool — **heredocs have a length
  ceiling; for a log body this size, use Write and reserve `sed -n ... >>` for the appended chain.**
- Turns used: roughly 33 of 75.
- No new sources added, correctly — a T4 does not collect.
