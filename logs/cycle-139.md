# Cycle 139 — T3 investigate — institutional-incident-real-world-impact

## Task performed

T3 (Investigate) on `institutional-incident-real-world-impact`, per `prompts/t3_investigate.md`
read at source. Phase check ran first and every invariant held:

| check | expected | observed |
|---|---|---|
| `state/queue/last_completed_task.txt` | `T5 select` | `T5 select` ✓ |
| HEAD `768f307` stat | cycle-138 commit, `state/meta.json` + both queue files + `logs/cycle-138.md` only | exactly that, plus the harness transcript ✓ |
| `scores.json` | `last=137 n=10 total=22 max=3` | identical ✓ |
| `graph.json` | `ctr=68 open=50 issues=10 last=ctr-0068` | identical ✓ |
| `index.json` | `srcs=47 last=src-0047` | identical ✓ |

Cycle 138 was a T5 and touched no knowledge, no graph and no scores, as a T5 must not. I am the
T3 on the edge T5→T3; after me the edge is T3→T4.

### The work item, and why this one

The issue's route ledger (`open_questions[8]`, written cycle 114, updated cycle 128) names three
open items in its paragraph (5). I read it in full at source before acting.

- **(i) whether any PwC citation was in fact updated** — **NOT ATTEMPTED, BLOCKED.** A diff needs
  an earlier PDF and the archive route (`web.archive.org`) is recorded closed. I spent no fetch
  re-opening it, as the queue entry asked.
- **(ii) whether KPMG ever named a cause** — **RUN. DATED NEGATIVE.**
- **(iii) a non-Big-Four commercial response** — **RUN. POSITIVE, AND IT IS THE FIND OF THE
  CYCLE.** This has been the highest-value missing data point on this issue since cycle 100 and
  was uncollected through nine intervening cycles.

### (iii) — the result

`src-0048`: **Sullivan & Cromwell LLP**, a US law firm and **not** a Big Four firm, apologised to
the US Bankruptcy Court in Manhattan (Chief Judge Martin Glenn, the Prince Global Holdings
wind-down) over inaccurate citations in a filing that the reporting attributes to AI. The errors
were caught by **opposing counsel, Boies Schiller Flexner LLP**.

Two things make it worth the fetches:

1. **It is the first incident data point on this issue whose discovery route does not pass through
   GPTZero.** Five cycles have recorded that adding a further institution whose evidence traces to
   GPTZero cannot relieve the independence bar, because GPTZero is the common ancestor. This one
   does not trace to GPTZero at all — no AI-detection vendor appears anywhere in the reporting.
   Cycle 128 got the first non-GPTZero *evidence* (direct observation of publishing state); cycle
   139 gets the first non-GPTZero *incident*.
2. **It is the exact case cycle 100 said the issue needed.** Cycle 100 (T2) wrote on
   ...
   whether the EY/KPMG/PwC convergence on *reviewing the circumstances, name no cause* reflects
   **commercial exposure** or **Big Four crisis-communications boilerplate**. Sullivan & Cromwell
   is commercial, is not Big Four, and **does not produce the Big Four sentence shape at all**. On
   the commercial-exposure reading it should have declined; it did not. **The tilt is toward the
   boilerplate reading.** Tilt, not settle — see the three qualifications below.

**What the firm actually said, with the boundary I refuse to blur.** The reporting states the firm
maintains *"comprehensive policies and training requirements governing the use of AI tools in legal
work"*; that *"the firm admitted those policies were not followed in this instance"*; and that an
internal secondary review *"did not identify the inaccurate citations generated by AI"*. That is a
**control-failure attribution, not a root-cause narrative.** Asked directly with no answer
supplied, **both renders returned ABSENT for a statement of what caused the inaccurate citations
and ABSENT for any named AI tool, model or vendor.** It is materially more than "we are reviewing
the circumstances", which supplies nothing — but it is not "named a cause", and I have written the
ban into the next queue entry so no successor upgrades it.

Andrew Dietderich, verbatim, and no other institution in this base does anything like it:
*"I apologize on behalf of our entire team. I also called Boies Schiller Flexner LLP on Friday to
thank them for bringing this matter to our attention and to apologize directly to them as well."*
It **thanks the party that caught the defect** — the inverse of the posture of the other four cases.

**Three qualifications, all of which I have written into the state rather than into this log only.**

- **Scope.** The artefact is a **bankruptcy court filing**, not a published report and not CTI.
  This base already flags EY/KPMG/PwC as consultancies rather than CTI publishers proper, adjacent
  to and not within the topic; a law firm's emergency motion is further out again. **I did not
  decide whether it belongs in the response population.** I entered the candidate at status
  `proposed`, not `supported`, and opened **ctr-0069** asking a T4 or T2 to rule. Excluding it
  discards the only case that can break a nine-cycle-old confound; admitting it widens the
  population a fifth time in a direction the issue title does not obviously cover. Neither move is
  free and a T3 may not make either.
- **It swaps a confound rather than removing one.** A filing before a judge carries a professional
  duty of candour and an adversary with a direct incentive to check the citations; a
  thought-leadership PDF carries neither. **Forum** is now the live alternative explanation, and no
  source in this base tests it.
- **One wire, two carriers.** `pymnts.com` and `molawyersmedia.com` carry the same story and the
  latter self-identifies as Reuters copy by named Reuters reporters. **They are not two newsrooms
  and must never be counted as two.** This base has recorded four times that outlet count is not
  independence; I am not about to violate it on my own find. This base holds **no primary
  artefact** — not the letter, not the motion, not the corrected filing, not a docket entry.

Chronology worth noting: April 2026 places this **before** EY (May), KPMG (June) and PwC (July).
It is the **earliest** commercial case in the base, not a late addition to a pattern.

### (ii) — the result

I searched for any KPMG statement, review outcome or finding published after the 12 June 2026
statement. **Every result that carried a KPMG response carried the same 12 June formulation** —
that KPMG is reviewing the circumstances surrounding the publication — which is the statement
`src-0035` already holds verbatim. So: **still no, as of 2026-08-08.**

This is recorded in `open_questions[8]` as a **dated first-person search-negative, not a sourced
claim and not a proof that KPMG made no statement.** No source was added for it and no claim about
KPMG is entered anywhere from it. `src-0035`'s own Limitations flagged this as cheap and untried;
it is now tried and negative.

The same search surfaced a genuinely new axis I have recorded as a lead and **expressly not
commissioned**: organisations *named in* the KPMG report are reported to have publicly disputed the
claims made about their own AI deployments. That is a **third-party** response — the reaction of
parties misdescribed *by* a hallucinated report — distinct from the **publisher** response this
issue tracks. I read it only in search-result summaries, fetched none of it, entered no claim, and
noted that it traces to the same GPTZero/FT reporting the independence bar already covers.

### G1 hazard found and avoided

`www.globallegalpost.com` returned **HTTP 403 to the direct fetch client** while other carriers of
the same story served plain clients. That is the `pwc.com` class the cycle-128 ledger warns about:
entering such a URL as a `src-*` risks failing the validator's URL gate and reverting the whole
cycle. I did not enter it; I recorded it as a closed route. The URL I did enter, `pymnts.com`,
served the **direct** client in full — I chose it over the richer Reuters carrier specifically
because free-to-read and plain-client-served is the lower G1 risk, and the PYMNTS piece carries
every fragment I needed anyway.

## Retrospection

**Target: cycle 130's entered conclusion.** The round-2 order put cycle 130 next in line (138 took
129's). Cycle 130 was a **T5**, so what it *entered* is its ranking and the queue entry it wrote.
Recovered from the snapshot: `git show de9b4a1:state/queue/next_task.json` → `task=T3
target=ioc-extraction-reliability created=130`. Commit `de9b4a1` stat-checked before being trusted:
it touches `state/meta.json`, both queue files and `logs/cycle-130.md` and nothing else, which is
what a T5 commit should look like.

**Instrument: full independent re-derivation of the ranking from the cycle-130 snapshot, at ZERO
FETCHES.** I re-derived the score vector, the open-contradiction counts per issue, and every
issue's `created_cycle`, `depends_on` and `attempts` from `git show de9b4a1:` copies of
`state/assessments/scores.json` and `state/issues/graph.json` — then worked the rungs of
`prompts/t5_select.md` myself before reading cycle 130's table.

My derivation, with cycle 130's reported values in brackets where checked:

- **Snapshot score vector** — `last=129`, `ttp-attack-mapping-reliability` 3,
  `incident-evidence-vendor-concentration` 3, the other **eight** all 2. Candidate set = all ten
  (`score < 5`); the two 3s eliminate at step 2. ✓ matches.
- **Open contradictions per issue** — institutional 12, consistency 10, automated-triage 5,
  attribution-expressed 4, attribution-confident-wrong-gap 3, ioc 3, task-dependent 3, ttp 6,
  extraction-vs-reasoning 2, vendor-concentration 1. Every issue carries ≥1, so the g3 demotion of
  2 fires on all ten and the ceiling is 3 everywhere. ✓ (cycle 130's table records ioc 3 and
  consistency 10 in its rung-4 comparison; both match.)
- **Rung 3(a), pairwise, restricted to the score-2 tier, maximal elements** — excluded
  `task-dependent-reliability-framing` (four of its five dependencies in tier 2),
  `extraction-vs-reasoning-ordinal-axis` (sole dependency in tier), `attribution-confident-wrong-gap`
  (sole dependency in tier), `attribution-expressed-confidence-unmeasured` (both dependencies in
  tier). **Four of eight excluded**, survivors are the four roots: ioc, consistency,
  institutional-incident, automated-triage. ✓ matches cycle 130's rows 5–8 exactly.
- **Rung 3(b), +1 per attempt in the last 5 cycles (window 125–129)** — `institutional-incident`
  attempted at 128 takes +1 → effective 3 → out. ioc (last 117), consistency (last 122) and
  automated-triage (never) take nothing. ✓ matches, including cycle 130's own note that the
  125–129 vs 126–130 window choice changes nothing here.
- **Rung 3(c), older `created_cycle` first** — `automated-triage-under-refusal` (16) eliminated
  against `created_cycle` 2. ✓ matches.
- **Terminus** — `ioc-extraction-reliability` and `consistency-calibration-as-failure-mode`, both
  score 2, both roots, both unpenalised, both `created_cycle` 2. **`prompts/t5_select.md` ends at
  3(c).** The tie is real.

**Verdict on (a): the tie cycle 130 reported IS real under the written policy — REPRODUCED.** Cycle
136 did the same exercise for cycle 127 and also reproduced the defect; this is now the second
independent reproduction, on a different pair.

**Verdict on (b): the selection DOES follow from the rule cycle 130 actually named.** It named
least-recently-attempted, stated plainly that the rung is not in the prompt and that it had no
authority to adopt it, and applied it correctly: ioc last attempted 117, consistency 122, so ioc
wins. Cycle 130 additionally tabulated five alternative rungs and reported honestly that two of
them dissent. **Cycle 130's entered conclusion RE-VERIFIES. No contradiction opened on it.**

The defect it reported, however, is confirmed and is now three-for-three: cycles 127, 130 and 138
all deadlocked and all three invented the same unauthorised fourth rung. That is standing honesty
item (B), now raised ten times.

**Fetches spent on G2: ZERO.** (Cycles 121–133 and 135–139 all zero; 134 spent one deliberately.)

## Changes made

**New source.**
- `state/knowledge/src-0048.md` — Sullivan & Cromwell / PYMNTS-CPI, accessed 2026-08-08, five key
  claims, and a Limitations section that is longer than usual on purpose: the non-independence of
  the two carriers, the absence of any primary artefact, the unresolvable quotation/paraphrase
  boundary in the two admission fragments, the out-of-domain scope, and the forum confound.
- `state/knowledge/index.json` — `src-0048` appended. `srcs` 47 → **48**.

**Issue `institutional-incident-real-world-impact`.**
- `candidate_resolutions[9]` **added**, status **`proposed`**, evidence `["src-0048"]`. Six
  numbered paragraphs; paragraph (4) states in full why it is not promoted and what a T4/T2 ruling
  either way should do to it. Candidate count 9 → **10**.
- `open_questions[5]` **extended** by a cycle-139 update (+2322 chars): the non-Big-Four case cycle
  100 asked for has been found and does not behave like the Big Four; four binding qualifications;
  and the entry's new want — a second non-Big-Four case whose artefact is a *published report*.
- `open_questions[8]` (the route ledger) **extended** by a cycle-139 update (+4799 chars): item
  (iii) discharged positive, item (ii) run and dated-negative, item (i) not attempted and still
  blocked, `globallegalpost.com` added to the closed-route list, the third-party-response lead
  recorded and not commissioned, and paragraph (6) re-naming the cheapest open work so the next T3
  need not rederive it.
- `attempts` **appended** with `139` → `[12, 59, 72, 90, 104, 114, 128, 139]`.

**New contradiction (G3).**
- `ctr-0069`, `opened_cycle: 139`, `resolved_cycle: null`. Seven numbered paragraphs. Side A is
  `candidate_resolutions[3]`'s *3 decline / 1 volunteer, and it lines up with sector*; side B is
  `src-0048`. Paragraph (3) is the scope objection, (4) records that the volunteer/decline binary
  **has no cell** for a control-failure attribution even before scope is settled, (5) is the forum
  confound, (6) states what does **not** follow (no score moves; no fact in side A is impeached;
  the tally is **not** restated as 3/2 anywhere in the state), and (7) assigns the ruling to a T4
  or T2. Contradictions 68 → **69**; open 50 → **51**.

I opened this rather than quietly appending a fifth institution to `candidate_resolutions[3]`
because that would have been picking a side on a scope question that is not a T3's to pick — G3
exists for exactly this.

**Verification before every `mv`, hand-run with `jq` (I could not run `scripts/validate_state.py`
— Bash refuses `python3` here, so the gates below are hand-checked, not validator-checked):**
- `index.json`: `srcs=48 last=src-0048 url_ok=true claims=5`, and the id sequence `src-0001…src-0047`
  intact in order.
- **Append-only proof by hash** — `jq -S -c '<region>' new | md5sum` against
  `git show HEAD:<path> | jq -S -c '<region>' | md5sum`. All 47 prior sources: `0f32856b…` both
  sides. All 68 prior contradictions + the nine untouched issues + the nine prior candidates + the
  seven untouched `open_questions`: `98d09719…` both sides. Nothing existing was rewritten.
- **Extension proof** — `oq5_prefix=true oq8_prefix=true`; both fields are strict prefix-extensions
  of their previous values.
- `graph.json`: `ctr=69 open=51 issues=10 last=ctr-0069 opened=139`; issue `cands=10
  newstatus=proposed newev=src-0048 oq=9 attempts=12,59,72,90,104,114,128,139`.
- Scratch file `scratch139.json` validated with `jq -r keys` before the splice and `rm -f`'d after.

**`state/assessments/scores.json` is untouched.** A T3 does not score. Verified: `last=137 n=10
total=22 max=3`, unchanged.

**Queue.** `state/queue/next_task.json` → T4 for cycle 140, `target_issue: null` (a T4 scores all
ten), `attempt_count: 0`, self-contained, carrying the full ban set, all carry-forward items and
the new tool notes. `state/queue/last_completed_task.txt` → `T3 investigate`.

## Next task rationale

The state machine's edge is **T3→T4** and I am the T3, so cycle 140 is a T4 with `target_issue`
null. The refresh rule is a T5's and does not touch me: T5s land on 141/144/147 and only 147 is a
multiple of 7, so cycle 147 is the next T5 that can fire a refresh. Recorded, not acted on.

Three things I have pushed at cycle 140 specifically, beyond the standing T4 job:

1. **`ctr-0069`'s scope ruling is a T4's or a T2's and I have named it as honesty item (F).** It is
   the live question on the biggest issue in the graph and it should not sit for nine cycles the
   way the item it discharges did.
2. **The g3 arithmetic moved against the issue and cycle 140 must not miss it.**
   `institutional-incident-real-world-impact` now carries **thirteen** open contradictions — the
   largest block in the graph — and graph-wide open contradictions rose 50 → 51. The demotion still
   fires on all ten issues (every issue carries ≥1), so the ceiling stays 3 everywhere, but I have
   told cycle 140 to re-derive the per-issue counts with `[373]` rather than inherit mine.
3. **A T4 can move honesty item (A) and probably should look.** `automated-triage-under-refusal`
   has never been investigated in 139 cycles, and part of why it is stuck at 2 is that nothing has
   ever been scored *into* it. Cycle 135 entered a real candidate there from `src-0047`. If that
   candidate genuinely moves the issue's resolution level, the score change is cycle 140's to make
   — and I have told it in terms to **score on the evidence, not to engineer a selector outcome,
   and to say explicitly which it did.**

**On the selection that sent me here.** Cycle 138 broke an unbreakable tie with least-recently-
attempted, the third unauthorised use of that invented rung. I record that I think the *defect* is
real — my own G2 reproduced it independently on the cycle-130 pair — while the *selection* was
reasonable and I did not re-select. The rung is still not policy and a human should rule.

## Budget

- **Web searches: 5.** **Three** aimed at item (iii) — one broad, one narrowed to security vendors,
  one on non-Big-Four consultancies and think tanks; the third returned only Big Four cases. These
  are the three the state text refers to when it says cycle 139 did not surface a non-Big-Four case
  whose artefact is a published report. **One** to confirm and date the Sullivan & Cromwell lead
  that search 2 threw up incidentally. **One** on item (ii), the KPMG post-review statement.
- **Fetches: 4.** `globallegalpost.com` direct (**HTTP 403**, recorded as a closed route);
  `pymnts.com` direct; `pymnts.com` via `r.jina.ai`; `molawyersmedia.com` direct. Two render
  families on the URL I entered, plus one render of a second carrier of the same wire.
- **Fetches on G2: 0.** Whole retrospection done from `git show <rev>:<path> | jq`.
- **New sources: 1** of the `max_new_sources: 5` budget.
- **Turns: ~40 of 75.** Comfortable; bookkeeping was written before the log, per the ordering rule.
- **Not spent:** no fetch on the closed archive route for PwC item (i); no fetch on the GPTZero
  Deloitte Australia lead; no fetch on the third-party-dispute lead.
