...
- **Task type:** T5 — Select (weakest link)
- **Queue entry executed:** `state/queue/next_task.json`, `cycle_created` 53, `task_type` T5, `target_issue` null
- **Selected issue:** `ttp-attack-mapping-reliability`
- **Next task scheduled:** T3 (investigate) — refresh rule did **not** fire
- **G2 result:** src-0012 — numbers **PASS** (fourth consecutive cycle), state's characterisation **FAILS**, **ctr-0022 opened**

---

## Task performed

I verified the phase before doing anything, as the queue entry demanded. `jq` on the three
state files returned `last_assessed_cycle` **53**, contradictions **21**, sources **24**, and
`last_completed_task.txt` reading `T4 assess`; `git show --stat HEAD` confirms cycle 53 landed
its scores.json, graph.json, index.json and src-0007.md edits. Cycle 53's T4 therefore did not
abort and **I am the T5**, not a repeat T4.

I read `prompts/t5_select.md` myself rather than taking the queue entry's account of it.

**The refresh rule did not fire.** `config.yml` `schedule.collect_refresh_every` is 7 and
step 4 of the prompt reads `current_cycle % schedule.collect_refresh_every == 0`, evaluated in
the T5 cycle. **54 % 7 = 5.** Not zero. The next task is a **T3**, not a T1.

The queue entry invited me to determine whether "the counter runs on something else entirely",
noting the last T1 was cycle 43 and that the T5s at 48 and 51 both scheduled T3s. I take the
rule literally, because it is stated literally and because cycle 41 was burned by a projection
built on something other than the stated arithmetic. Checked: 48 % 7 = 6, 51 % 7 = 2,
54 % 7 = 5 — **none of the three T5s since cycle 43 was entitled to fire the refresh**, so the
eleven-cycle gap is exactly what the stated rule produces and is not evidence that the counter
runs on something else. If the T5 cadence holds at every third cycle (57, 60, 63, …), the next
T5 landing on a multiple of 7 is **cycle 63**. I record that as a projection and explicitly
**not** as a fact: cycle 41 found a predecessor's schedule projection wrong by fourteen cycles
precisely because an abort shifted the phase, and any aborted cycle between now and then moves
it again. Recompute, do not copy.

I did **not** rescore and I did **not** add a source. A T5 selects. `scores.json` is untouched
at `last_assessed_cycle` 53; `index.json` still holds 24 sources.

---

## Retrospection

**Target chosen by staleness: `src-0012`** (`https://www.consulting.ca/news/5023/ey-canada-takes-down-study-after-apparent-ai-hallucinations`).

I verified the staleness claim rather than accepting it. `index.json` carries no
`last_verified_cycle` field for any source, so staleness lives only in the logs and in the
appended sections of the `src-*.md` files. `src-0012.md` contains G2 sections for cycle 13 and
cycle 31 and no later one; against the handoff's list of per-cycle G2 targets, cycle 31 is the
oldest last-check of any source that has ever been checked. **Twenty-three cycles.** Confirmed.

It was also the highest-value target available for a second reason: `src-0012` underwrites
`candidate_resolutions[2]` of `institutional-incident-real-world-impact`, **the only issue in
the graph scoring above 2**, so this G2 is a direct test of that score.

### Method

`consulting.ca` fetched in verification mode per rule (xv) with the full current hardening:
ten numbered items, each answered PRESENT/ABSENT, quote only the single full sentence
reproduced to its end, explicit "do NOT substitute a nearby or similar sentence", an **ITEM 0**
body-visibility probe and a **final truncation probe** (cycle 52's hardening). One item asked
for the **entire paragraph** containing "16 of 27" from its first word to its last (rule iv).
The page self-reported **not truncated**.

### What passed

**All four stored `key_claims` re-verified PRESENT and verbatim** — the fourth consecutive
cycle (12, 13, 31, 54):
...
- "GPTZero – which offers an AI detector solution that is used by hundreds of organizations in education, hiring, publishing, and legal – found that more than a half-dozen footnotes led to dead webpages or did not contain the cited information."
- "Following the publication of the GPTZero report, EY Canada removed the study "Points of Attack: Uncovering Cyber Threats and Fraud in Loyalty Systems" from its website."

The article's date re-verifies as **19 May 2026**. It states **no year** for the EY study and
contains **no direct EY quotation** — both ABSENTs exactly as cycle 31 found them, so they are
confirmed across two cycles, and the "2025" date remains sourced to Going Concern as cycle 31
established. A fourth fetch of `gptzero.me/investigations/ey` again showed the
"0 of 27 references hallucinated" scorecard against a body containing "Most were hallucinated.",
**confirming** the rendering artefact recorded since cycle 12 rather than disturbing it.

### What failed — ctr-0022

The state says in **three places** that the EY incident was discovered by the *same route* as
ENISA:

- `index.json` src-0012 `key_claims[1]`: "The discovery route matches the ENISA case exactly -- external parties clicking through cited links, not internal QA"
- the issue's `open_questions[4]`: "BOTH known incidents were discovered from the outside by parties clicking through footnotes (Westfaelische Hochschule researchers for ENISA, GPTZero for EY)"
- `candidate_resolutions[2]`: "discovered by the same route as ENISA (an outside party clicking through footnotes)"

Per **rule (xiii)** I did not open on the strength of cycle 13's log record of GPTZero's
pipeline sentence — I re-fetched `gptzero.me/investigations/ey` in verification mode and got
the artefact itself. It says, verbatim:

> "Over the past few months we've set up an automated pipeline to search for vibe citations by finding and scanning public reports from major consulting firms."

> (same paragraph, to its last word) "What we've found suggests that the vibe citing epidemic is already endemic, even among the major players."

> "Instead of releasing our results all at once, we're going to focus on one report at a time."

> "GPTZero began targeting vibe citations with our Hallucination Check tool in 2025, which we used to further investigations into a government publication, two different Deloitte reports, and prestigious machine learning / artificial intelligence conferences like NeurIPS and ICLR."

A vendor running an automated sweep over a population it chose for the purpose, releasing
results serially, is **not** "external parties clicking through cited links" and it is not the
same route as ENISA "exactly".

**What survives, stated so this is not over-read as a demolition:** both incidents were caught
**externally** and neither by the publisher's internal QA. That half of the claim is untouched
and is the true thing the state was reaching for. What fails is "matches exactly" and "clicking
through footnotes" as a description of the EY route.

**What is NOT established, stated deliberately.** I asked the page *directly* how this specific
EY report came to GPTZero's attention. It returned **ABSENT** — it describes the pipeline and
the serial-release policy but never says in terms that the pipeline surfaced this one. So the
defensible statement is that the EY report was published as one instalment of an automated,
targeted, serially-released sweep. That is weaker than "the pipeline found EY" and still flatly
incompatible with "an outside party clicking through footnotes". **Cycles 39, 43 and 51 each
caught a prior G2 *correction* that was itself false or overstated; I would rather state my
finding at its true strength than be the fourth.**

**Why it is load-bearing rather than cosmetic.** `candidate_resolutions[2]`'s entire force is
that "Two independent institutions, one public and one commercial, is materially stronger than
one for the EXISTENCE and RECURRENCE claim". Two incidents are two independent draws only if
they were found independently. One was a chance discovery; the other is a selected instalment
of a hunt *designed* to find this failure mode, ordered for release by a commercial vendor whose
product is AI detection. That confirms the phenomenon **exists** in the hunted population; it is
weaker evidence of **recurrence-as-independent-event** than the state credits, and the selection
effect is maximally adverse.

**The denominator is withheld — established at source, not assumed.** Asked directly for any
total number of reports scanned or any rate across the sweep, the page answered **ABSENT**.
GPTZero publishes numerators only. So the pipeline — which is precisely the "sweep over N
published threat reports from M agencies and vendors" that `open_questions[0]` has wanted since
cycle 12 — **cannot yield a base rate as published**. Cycle 13 logged it as "a T1 collection
lead"; no cycle in the forty-one since recorded *why* the lead cannot close the question.

### New material recovered by the rule (iv) paragraph pull

The paragraph containing "16 of 27" has a **third sentence this base has never held**:

> "The erroneous data in the EY study included references to a fabricated McKinsey report that claimed US$200 million in unredeemed loyalty rewards globally."

This is the first **concrete instance** of a hallucinated citation in this incident that the
base records — previously it held only the count. Also recovered: the stored half-dozen-footnotes
quotation is a **fragment**, and the full sentence's opening clause identifies GPTZero as a
commercial AI-detection vendor. That **corroborates an existing caveat** rather than raising a
new one, and I say so rather than inflating it into a second finding. **Cycle 54 is the fifth
consecutive cycle in which the paragraph-first-to-last-word ask surfaced material no string
check would have found.**

### Classification

This is the **rule (vi)** shape for the tenth time — the source is clean and the state's account
of it is not — and specifically the **rule (viii)** sub-shape, in its purest instance yet:
**every stored quotation matched verbatim, and the false part of the claim was never quoted from
the source at all**, so no string check and no number check could ever have caught it. That
observation is what motivates candidate rule (xix) below.

### Consequence I am forbidden from acting on

ctr-0022 materially weakens the **merit** case for `institutional-incident-real-world-impact`'s
score of **3** — the only score above 2 in the graph. **A T5 may not rescore and I have not.**
The G3 *gate* is unaffected: this issue already carried ctr-0012 and ctr-0018, the validator
builds the open set as a set comprehension over `issue_id`, so the ceiling was already 3 and a
third entry lowers nothing. **This is a merit signal for the T4 at cycle 56, and I have written
it into the next queue entry so it cannot be lost.**

---

## Changes made

1. **`state/issues/graph.json`** — opened **`ctr-0022`** against
   `institutional-incident-real-world-impact` (contradictions 21 → 22), with five repair steps
   of which only (i) is done. Validated with `jq -e`; the five schema fields read back with
   `jq -r`. **No issue, candidate, open question or `attempts` array was modified** — a T5 does
   not investigate.
2. **`state/knowledge/src-0012.md`** — appended a "Cycle 54 G2 re-verification" section.
   Nothing above it altered.
3. **`state/knowledge/index.json`** — appended **one** `key_claim` to the **existing** src-0012
   entry (4 → 6 claims; the cycle-31 record was already claim 5). **No source added; still 24.**
   Validated with `jq -e` and read back with `jq -r`.
4. **`state/queue/next_task.json`** — T3 targeting `ttp-attack-mapping-reliability`,
   `attempt_count` 0. Validated with `jq -e` and all five fields read back.
5. **`state/queue/last_completed_task.txt`** — `T5 select`.
6. **`logs/cycle-054.md`** — this file.

**Not touched:** `state/assessments/scores.json` (still `last_assessed_cycle` 53). No source
was added. No existing `src-*.md` content or `key_claims` entry was deleted or rewritten.

---

## Selection — the full ranking table

Required by `prompts/t5_select.md` ("this table is evaluation data for the paper"). **Every
input below was re-derived with `jq` from `state/issues/graph.json` and
`state/assessments/scores.json` this cycle**, not taken from the queue entry's table — cycle 46
found its own queue entry's map wrong on two issues. Both of the queue entry's maps (attempts
arrays and open-contradiction counts) in fact checked out exactly.

**Step 1 — candidate set.** All issues with score < 5. Scores are eight issues at 2 and one at
3, so **all nine are candidates**. No issue is at 0 or 1.

**Step 2 — base priority = score.** `institutional-incident-real-world-impact` (3) sorts below
the eight at 2. Base priority separates nothing else, so **the tie-break ladder is again the
entire selector** — the condition cycle 50 warned about and cycle 51 first hit.

**Step 3a — upstream first.** "An issue that others `depend_on` outranks its dependents." I
read this as **strictly pairwise**, as cycle 51 did: X outranks Y exactly when Y depends on X,
directly or transitively. It is silent between two issues neither of which depends on the other.
Depth = longest dependency chain down to a root.

**Step 3b — attempt penalty.** +1 per attempt within the last 5 cycles. The prompt does not
define the window's endpoints. **I used cycles 49–53 inclusive** relative to cycle 54 — the same
convention cycle 51 used (46–50 relative to 51) — and I flag it as a **choice, not a derivation**.

**Step 3c — older `created_cycle` first.**

| # | issue | score | depth (3a) | attempts | in 49–53 | penalty (3b) | effective | created (3c) | open ctr | rank |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | **ttp-attack-mapping-reliability** | 2 | 0 | [16, 31, 32, 46] | none | +0 | 2 | 2 | 2 | **1 (tied)** |
| 2 | consistency-calibration-as-failure-mode | 2 | 0 | [3, 15, 16, 25, 39] | none | +0 | 2 | 2 | 1 | **1 (tied)** |
| 3 | automated-triage-under-refusal | 2 | 0 | [] | none | +0 | 2 | 16 | 1 | 3 |
| 4 | ioc-extraction-reliability | 2 | 0 | [9, 21, 35, 52] | **52** | **+1** | 3 | 2 | 4 | 4 |
| 5 | attribution-confident-wrong-gap | 2 | 1 | [16, 28] | none | +0 | 2 | 2 | 1 | 5 |
| 6 | attribution-expressed-confidence-unmeasured | 2 | 2 | [49] | **49** | **+1** | 3 | 45 | 0 | 6 |
| 7 | task-dependent-reliability-framing | 2 | 3 | [6, 16] | none | +0 | 2 | 2 | 2 | 7 |
| 8 | extraction-vs-reasoning-ordinal-axis | 2 | 4 | [17, 18] | none | +0 | 2 | 16 | 1 | 8 |
| 9 | institutional-incident-real-world-impact | 3 | 0 | [12] | none | +0 | 3 | 2 | 3 | 9 |

Dependency edges used for the depth column, read from the graph:
`attribution-confident-wrong-gap` → `consistency-calibration-as-failure-mode`;
`attribution-expressed-confidence-unmeasured` → `attribution-confident-wrong-gap`,
`consistency-calibration-as-failure-mode`; `task-dependent-reliability-framing` → five issues;
`extraction-vs-reasoning-ordinal-axis` → `task-dependent-reliability-framing`. Roots:
`ttp-attack-mapping-reliability`, `ioc-extraction-reliability`,
`consistency-calibration-as-failure-mode`, `automated-triage-under-refusal`,
`institutional-incident-real-world-impact`.

### Ordering ambiguities, recorded because they are auditable

**(A) Does the penalty feed into the base score, or apply strictly after the upstream test?**
Computed both, as cycle 51 did:

- *Strict list order* (score → depth → penalty → created): depth-0 group is {TTP, IOC, CC, ATR};
  the penalty drops IOC; `created_cycle` drops ATR (16); **TTP and CC tied**.
- *Penalty-into-score* (effective score → depth → created): the effective-2 group is
  {TTP, CC, ATR, ACWG, TDRF, EVRO}; depth 0 leaves {TTP, CC, ATR}; `created_cycle` drops ATR;
  **TTP and CC tied**.

**Both readings produce the same top pair.** The selection is robust to this ambiguity and I did
not have to choose. That is now true for the second consecutive T5.

**(B) The 5-cycle window boundary.** `attribution-expressed-confidence-unmeasured`'s only
attempt is at cycle 49, which is **exactly on the boundary**: 54 − 49 = 5. Under "counts if
`c >= 49`" it takes +1; under a strict "more recent than 49" it takes +0. **It changes nothing
here** — at depth 2 the issue is outranked pairwise by `consistency-calibration-as-failure-mode`
and `attribution-confident-wrong-gap` regardless, and never reaches the contested top group. I
computed it rather than assume it, and record that the boundary case was live but not decisive.

**(C) Is "upstream" pairwise, or a degree count? This still changes the answer.** Under a
*count-of-dependents* reading, `consistency-calibration-as-failure-mode` has three dependents
and `ttp-attack-mapping-reliability` has one, so **CC would outrank TTP outright and no tie
would arise**. I rejected that reading for cycle 51's reason: "an issue that others `depend_on`
outranks **its dependents**" is possessive and relates an issue to those that depend on *it*;
counting dependents is an extrapolation the prompt does not license. **But note that this is now
the second consecutive T5 at which the two readings of tie-break (a) select different issues,
and on both occasions the count-of-dependents reading would have picked
`consistency-calibration-as-failure-mode` with no tie at all. A human should rule on this: it is
not a corner case, it is deciding the agenda.**

### The ladder ran out. Again. Stated plainly, as the prompt requires.

After (a), (b) and (c), `ttp-attack-mapping-reliability` and
`consistency-calibration-as-failure-mode` remain **exactly tied**: same score 2, same depth 0,
same penalty +0, **same `created_cycle` 2**. The stated ladder has no fourth rung.
**Cycle 54 is the second consecutive T5 to exhaust the ladder outright**, on a *different* pair
from cycle 51's. Six of the nine issues share `created_cycle` 2, so this is structural.

**Cycle 51's ad-hoc extension does not decide this tie.** I read `logs/cycle-051.md` rather than
the queue entry's account of it. Cycle 51 extended (b)'s stated rationale — "prevents thrashing
on a stuck issue" — past its five-cycle window, on two limbs that happened to agree. **Here they
split:**

- *Fewer total attempts* → **TTP** (4: [16, 31, 32, 46]) over CC (5: [3, 15, 16, 25, 39], the
  most of any issue in the graph).
- *Longer since last attempted* → **CC** (cycle 39, 15 cycles ago) over TTP (cycle 46, 8 cycles
  ago).

So the first patch to [75] is itself insufficient, **one tie after it was invented**. That is a
finding about the mechanism, not just about this cycle.

**How I broke it, disclosed as an ad-hoc extension and not as policy.** Rule (b) as written is
purely *recency*-windowed: attempts older than five cycles carry **no** penalty at all. Both TTP
(8 cycles) and CC (15 cycles) are outside that window, so **by the rule's own terms the
anti-thrashing rationale is spent for both**. Extending the window to 8 to re-separate them
would be re-applying the rule past the point where the rule itself says it stops mattering, and
I decline to do that. Once anti-thrashing is spent, I break the tie on **which issue has more
cheap, well-specified, already-actionable work**, since the point of selection is to *move* the
weakest link:

- **TTP: one single edit discharges two open contradictions.** ctr-0015 step (iii) and ctr-0016
  step (iii) demand the *same* edit to `open_questions[1]`. **I verified this at source this
  cycle** rather than trusting the queue entry — ctr-0016(iii) reads "…is now demanded by BOTH
  ctr-0015 step (iii) and this entry; a single T3 edit discharges both". It is 8 and 9 cycles
  undone. TTP also has a second named, cheap, non-collection action: the never-run check of
  whether src-0023 reports any ATT&CK/TTP task and under what scoring rule, which if it lands is
  the *third independent measurement* this issue's route to a 3 has needed since cycle 31.
- **CC's cheapest named action is src-0001's ARES provenance** (one DOI fetch). Cycle 53 showed
  the shape pays off — but also showed that **provenance is not measurement** and does not move
  a score. CC's real blockers are collection-shaped: the calibration half is single-source on
  CTI, and src-0018's numbers are **images this agent cannot read**.

A secondary and independent extension concurs: **open contradictions** as a measure of
unrepaired damage — TTP carries **2** (ctr-0015, ctr-0016), CC carries **1** (ctr-0011).

**Circularity disclosed:** my own G2 opened ctr-0022, but against
`institutional-incident-real-world-impact`, which is **neither** issue in this tie and which
ranks last on base priority regardless. My G2 had **no** influence on this selection.

**A successor T5 facing this tie is not bound by my extension**, and I have said so in the queue
entry. **[75] and [30] now need a human ruling rather than a third cycle of ad-hoc breaks.**

### One issue I want to flag against my own selection

`automated-triage-under-refusal` has **never been attempted in 54 cycles**, has a brand-new
contradiction with four open repair steps, and has unused evidence already sitting in the base
(the 55–60% priority-scoring accuracy and the Pass Rate (Score) rows recovered at cycle 53) that
measures the very task the issue is named after. It was eliminated **solely** by tie-break (c) on
`created_cycle` 16 against 2 — that is, **for being a younger issue**, which has nothing to do
with how weak or how tractable it is. I applied the ladder mechanically as the prompt directs
and I am not overriding it, but I record plainly that **the mechanism produced a result I think
is substantively questionable**, and that this is a second independent argument for [30] and
[75] needing a human. Carry-forward **[137]** is updated accordingly.

---

## Next task rationale

**T3 (investigate) targeting `ttp-attack-mapping-reliability`.** State machine T5→T3; the
refresh rule did not fire (54 % 7 = 5), so it is not a T1.

The queue entry is self-contained, quotes the issue's current `open_questions` and names the
specific verified actions in priority order: (1) the single edit to `open_questions[1]` that
discharges both ctr-0015(iii) and ctr-0016(iii), with the substance of what the repair must say
(the three kappa values are three different estimands and do not form a band) and an explicit
warning **not** to delete the impeached sentence, since cycle 46 left it standing deliberately so
the finding stays auditable; (2) ctr-0015(iv) and ctr-0016(iv); (3) the src-0023 ATT&CK-scoring-rule
check, with the caution that CTIArena's cycle-25 *kill* applies only to
`consistency-calibration-as-failure-mode` and says nothing about ATT&CK; (4) a pointer that
`open_questions[3]`'s "T1 collection lead" may now be answerable **without** collection because
src-0019 *is* the CTIBench artefact release, added at cycle 43.

It also records the trap I hit myself: `ttp-attack-mapping-reliability`'s `open_questions` run
1282–6947 characters, and a truncated `jq` slice briefly led me to conclude that ctr-0015(iii)
named the wrong index. It does not. Print field **lengths** first, then pull the whole field.

For the next G2 I named **src-0017** as the stalest source (cycle 32, twenty-three cycles) and
verified that against the per-cycle list. It is a happy alignment: src-0017 is the
CyberThreat-Eval **code repository**, it belongs to the selected issue, and `open_questions[4]`
names exactly what remains unverified about it — nothing in the paper or the artefact states
that src-0007's Table 4 TTP figures were produced by that code.

I carried forward to the next T4 (cycle 56) the two things I was forbidden to act on: that
ctr-0022 weakens the merit case for the only score above 2, and that ctr-0021's defects remain
unrepaired.
...
- **Fetches:** 2 (`consulting.ca`, `gptzero.me/investigations/ey`) — both verification-mode,
  both self-reported not truncated. No searches.
- **Turns:** ~20.
- **Sources added:** 0 (a T5 does not collect; the index stays at 24).
- **State writes:** graph.json ×1 (ctr-0022), index.json ×1 (appended key_claim), src-0012.md ×1
  (appended section), next_task.json, last_completed_task.txt, this log.
- **jq validations:** every JSON edit validated with `jq -e` and the added fields read back with
  `jq -r`.
- **Deliberately not done:** no rescoring (forbidden to a T5), no collection, no repair steps on
  any contradiction beyond ctr-0022 step (i).

### Candidate rule (xix), offered not imposed

**When the state asserts that two cases are "the same" — same route, same mechanism, same
population, same design — check the similarity claim itself at source, not merely the facts on
each side.** A similarity claim is an assertion about a *relation*; it is never quoted from
either source, so it is invisible to every string check and every number check — and it is
exactly what carries the inferential weight in a recurrence or independence argument. That is
ctr-0022: all four stored quotations matched verbatim and the relation built on top of them was
wrong. A successor should decide whether this is genuinely distinct from (viii) or a special
case of it. **I also accept cycle 53's candidate rule (xviii) as distinct from (xii)**: (xii) is
about auditing a derived number against its derivation log, (xviii) is about re-deriving a
*unit conversion* from definitions, and ctr-0021 showed the second can fail while the first
