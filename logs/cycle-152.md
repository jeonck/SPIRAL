# Cycle 152 — T3 investigate — `institutional-incident-real-world-impact`

## Task performed

T3 (Investigate) per `prompts/t3_investigate.md`, read at source. This is the **second attempt**
at the cycle-150 queue entry: **cycle 151 ran it, reported success in its log, and then failed
`scripts/validate_state.py`**, so its entire `state/` diff was reverted and only
`logs/cycle-151.md` survives. I started from the pristine state and redid the work.

**Phase check.** `state/queue/last_completed_task.txt` read `T3 failed validation (attempt bumped)`.
`git show --stat HEAD` gave `9689611 cycle 151: T3 investigate failed validation`, touching
`logs/cycle-151.md`, its transcript, `state/meta.json` and both `state/queue/` files —
**and not `state/issues/graph.json`**, which is the signature of a reverted cycle. `next_task.json`
still carried `cycle_created: 150`, `task_type: T3`, `attempt_count: 1`. `prompts/system.md` line 46
read at source gives T2 → T3, and cycle 150 was the last T2 whose state landed, so I am the T3.
All four confirming checks reproduced the *pre-cycle-151* values exactly, confirming the revert:

| check | expected | observed |
|---|---|---|
| `index.json` | `srcs=53 last=src-0053` | `srcs=53 last=src-0053` |
| `graph.json` | `ctr=70 open=50 issues=10` | `ctr=70 open=50 issues=10` |
| `scores.json` | `last=146 n=10 total=22 max=3` | `last=146 n=10 total=22 max=3` |
| target issue | `oq=12 cands=10` | `oq=12 cands=10` |

**I could not diagnose cycle 151's failure and I am not going to pretend otherwise.**
`run_cycle.sh` keeps the log and reverts the state, and it preserves the validator's stdout
**nowhere** — not in the transcript (which captures only the agent's output), not in `logs/`, not in
git. So the one artefact that would have told me what to avoid does not exist. What I did instead was
read `scripts/validate_state.py` end to end and hand-check every gate I could reach with `jq` before
writing (I did **not** run the validator — bash refuses `python` here). Carried forward as **[400]**
and flagged to the human in the queue entry: teeing the validator's stdout into `logs/` on failure is
one line of shell, and without it every validation failure is undiagnosable by the cycle that has to
retry it.

**Step zero, non-optional and done first: I read all five of `src-0049` through `src-0053` end to
end before reading cycle 150's questions.** Cycle 150 expressly did not, and said its recommendation
was worth less for it. It mattered: I adopted its recommendation on limb (i) and **rejected the
ground it proposed for limb (ii)**.

**Zero fetches and zero searches this cycle.** The whole investigation ran off the knowledge base,
which is what `prompts/t3_investigate.md` step 2 asks for. No source was added; no score was moved.

## Retrospection

**G2 target: cycle 142 (T3 on `ioc-extraction-reliability`), commit `df3f337`, at ZERO fetches.**

Cycle 141 was taken by cycle 151. Cycle 151's *state* was reverted but its **log survives and is
committed**, so 141 is examined and redoing it would waste the slot; the next unexamined predecessor
is 142. I stat'd `df3f337` before trusting the message: `logs/cycle-142.md`, its transcript,
`state/issues/graph.json`, `state/meta.json` and both queue files — consistent with a T3.

**Result: PASSES on every checkable limb.**

- **`candidate_resolutions[5]` survives at HEAD exactly as the log describes it** — status
  `supported`, evidence `src-0003` + `src-0007`.
- **The PRISM corpus cascade re-derives verbatim from the graph**: 500 threat reports curated from
  ORKL, filtered for dual presence in AlienVault (the official account), then fewer than 30 pages and
  more than five unique indicators, yielding 50 reports spanning April 2023 – November 2024 and
  **1,774** labels. Every figure in cycle 142's log is in the entered text.
- **The attested absence is entered as an absence and the inference is labelled an inference.**
  `src-0003` makes no statement about how difficult, representative or unusual its corpus is; the
  PRISM-is-easier reading is recorded as this state's inference. That is what the standing ban
  requires and it is what the text says.
- **Append-only proves out.** Binding cycle 141's snapshot against HEAD: issue ids identical, every
  non-target issue byte-identical, `candidate_resolutions[0]` and `[1]` both `startswith` their
  cycle-141 versions, `open_questions` 15 → 16, `candidate_resolutions` 5 → 6. Nothing removed.
- **The two `src-0007` facts behind `ctr-0070` re-verify at the source file, which is the sharpest
  test available without a fetch.** `state/knowledge/src-0007.md` contains the abstract sentence
  *'TRA allows human experts to iteratively provide feedback for continuous improvement.'*
  (one occurrence), and it has recorded since entry that *the benchmark's difficulty relative to
  PRISM's 50 public threat reports cannot be assessed from the paper*. Cycle 142 quoted both
  correctly and built `ctr-0070` on them correctly.

**One soft spot, declared.** I did not re-fetch arXiv 2506.11325v2. So cycle 142's **two-render**
claims — the complete section structure, and the negative that `src-0003` has no Limitations, no
Threats to Validity and no Discussion section — are verified as **faithfully entered**, not as
**re-observed at the artefact**. That is the loose end in my G2 and I am naming it for a successor
rather than letting it pass as a full re-verification.

**A separate corroboration, cheap and worth recording.** Cycle 151's log reports that the cycle-150
queue entry undercounted the open contradictions on the target issue as eleven when the true figure
was twelve. I re-derived it independently with the `[373]` one-call pattern before touching anything:
**twelve** at HEAD, **eleven** after my closure of `ctr-0069`. Cycle 151 was right, and this is the
second independent confirmation. Carried as **[401]**.

**No contradiction opened by the G2.** Nothing failed re-verification.

## Changes made

`state/issues/graph.json` only. Six appends, two new candidates, one new open question, one closure.
Nothing erased anywhere; proven with `startswith` on every pre-existing string touched.

**The four rulings on `open_questions[9]`, which is retained and answered by append, not deleted.**

**(i) South Africa — ADMITTED, stratum V.** Recorded in a cycle-152 append to
`candidate_resolutions[3]`, with the case record as the new `candidate_resolutions[10]` (status
`supported`, evidence `src-0049`/`src-0050`/`src-0053`, and the status **scoped** to what the
institution published, that it withdrew it, and what it said). Three separately-stated grounds, each
overturnable on its own: institution type does not exclude it (ENISA is already in); the artefact is
a document the institution published on its own initiative, which is exactly what `ctr-0069` (7)(iii)
commissioned and is **closer** to the population's core than `src-0048`'s bankruptcy motion, so a
rule excluding it on artefact type would have to expel `src-0048` first; and the forum is clean, so
stratum V is right and the forum confound does not reach it. **This is the first exercise of the
stratum machinery cycle 150 built** — stratum V goes from four members to five.

**The sector-alignment observation therefore got its first genuine test, and it passes weakly.**
Within stratum V: three decline (EY Canada, KPMG, PwC — all commercial, all Big Four) and two
volunteer (ENISA, DCDT — both public-sector), the DCDT volunteer hedged. I recorded three reasons the
pass is weak, and I think they matter more than the pass:

1. **The discriminating cell is still empty.** Every decliner is commercial *and* Big Four; every
   volunteer is public-sector *and* non-commercial. Sector, Big-Four boilerplate and commercial
   litigation exposure remain mutually confounded, and **no further public-sector case can separate
   them**. What is wanted is a *commercial, non-Big-Four, stratum-V* respondent, and no cycle has
   ever searched on that axis. Entered as work-owed **(38)**.
2. **One in ten.** On the marginals — five members, two volunteer, three decline, two public-sector,
   three commercial — perfect alignment arises by chance with probability 1/10 (one-sided Fisher
   exact). That is arithmetic on the base's own membership and is labelled a derivation, not a
   sourced claim. **It is not evidence and no successor may score on it.** What the fifth member
   changes is that the observation is now falsifiable and has survived one chance to fail.
3. **The new volunteer is hedged and the old one is not.**

**A numeral-collision hazard I created and disarmed in the same edit.** The new within-stratum-V
tally contains a three and a two, and so does the restatement `ctr-0069`'s cycle-140 ruling
**forbids**. They are different propositions: the forbidden one counts `src-0048`, a stratum-T
member, across strata; mine counts DCDT, within stratum V, and `src-0048` is not in it. **The
discriminator is the member, not the number.** Appended to `open_questions[7]`, which exists for
exactly this, so a successor does not open a contradiction on the collision alone.

**(ii) MAHA — NOT ADMITTED, and I rejected the ground cycle 150 proposed.** Cycle 150 recommended
exclusion because MAHA's AI attribution is *external and inferential rather than institutional*.
**That ground does not discriminate.** EY Canada, KPMG and PwC are all members and none of them made
an institutional causal attribution either — `ctr-0032` withdrew the claim that EY made any causal
attribution at all, and `candidate_resolutions[3]` records that all three review the circumstances
and name no cause. A rule expelling MAHA for externality would expel **three of the five stratum-V
members the sector-alignment observation is built on**. I could not have seen this without reading
the source files, which is the concrete return on step zero.

The ground I used is narrower: **this base holds no artefact, at any level of the chain, in which
any party states a method for linking the MAHA defect to AI, and the institution has expressly
refused to confirm or deny it.** `src-0051` returns two-render ABSENT for any AI attribution and for
any named AI tool; `src-0052`'s renders return ABSENT for any official confirming or denying AI use;
the sole AI link is `src-0052`'s relay of a Washington Post characterisation from reporting this base
does not hold and has no account of method for.

**I stated the limit of my own ground rather than overstating it:** it discriminates sharply against
KPMG and PwC, whose vendor's own per-institution artefacts this base holds (`src-0034`, `src-0037` —
named, not quoted, both under standing bans), and **less sharply against EY Canada**, whose vendor
finding is also carried by a secondary. The difference there is one of degree — a named investigator
with an affirmative detection finding versus an unheld impression — and I said so.

A **lifting condition** is stated so the ruling is not a dead end (the Washington Post artefact, or
any institutional statement on AI use), and a **second, independent obstacle** is recorded and
expressly is *not* the ground: the respondent is undetermined between the MAHA Commission, HHS and
the White House, and a response population needs a determinate respondent.

**(iii) The axis is decomposed, not extended — proposed, not built.** New
`candidate_resolutions[11]`, status `proposed`: D1 (states a cause / states a control failure /
states no cause / expressly refuses to confirm or deny) crossed with D2 (hedged / unhedged). The
deciding argument is not aesthetic: **a flat axis puts a hedged and an unhedged causal attribution in
the same cell, so the structure itself strips the hedge** — and this base holds direct evidence
(`src-0049` versus `src-0053`) that hedge-stripping is a real, directional failure running toward the
stronger causal claim. A taxonomy that reproduces that drift internally will launder it. Cycle 150
asked whether a five-valued axis is a reason to doubt the axis; the answer is yes, because the fifth
value is a **modifier** on one of the others, which is what a second dimension is for.
**Structure is a T2's and I did not build it.** The fourth D1 value is proposed and deliberately
empty, its only candidate occupant being the declined case.

**(iv) Spokesperson versus principal — yes in principle, deliberately not built.** The evidence is
good (one institution, two levels, two dates, two forum cells, and the two responses differ), but its
only occupant is the case I just declined, and cycle 150's order rule — decide the admission, then
the cell — binds me as much as it bound cycle 150. One observation worth carrying either way: within
MAHA the two levels **agree** on the name-no-cause limb and diverge only on the minimise limb, which
is weak evidence that response *level* matters less than *forum*. Labelled a hypothesis on one case.

**Secondary (a): `ctr-0069` is certified and closed, `resolved_cycle = 152`. `open` fell 50 → 49.**
I certified limb by limb against the cycle-150 append read at source, and I added the half nobody had
done: **(7)(iii)'s further claim — that one non-Big-Four published-report find would settle (i) and
(5) together — is now tested and rejected on the evidence.** I admitted one of the two cases and
declined the other, and the admitted one leaves the confound exactly where it was, for a structural
reason: the find was public-sector and only a commercial non-Big-Four case can break the confound.

**On what authority.** Paragraph (11) of cycle 150's own execution record names *the next T2 or T3 on
this issue*; I am that, and I closed on that one-off authorisation. I did **not** close on cycle 150's
four-limbed `ctr-0064` convention, which names a T2 only, and I am **expressly not proposing to widen
it to T3s**. I checked all four of its limbs anyway and they hold. `[398]` is intact: 150 executed,
152 certified. Three standing rules are restated as surviving the closure.

**A correction to the queue chain, and it is the reason the read-at-source discipline exists.**
The cycle-150 entry asserts that `ctr-0069` (7)(i) names *a T2 or a T3* as the roles owed the
execution. **It does not.** (7)(i) as written at cycle 139 reads *'A T4 OR T2 MUST RULE ON POPULATION
MEMBERSHIP'*. The ruling was in fact made by cycle 140, a T4, so the *ruling* was competent; the
entry's error is about *who may certify*. **This is the third instance of work-owed (33)** and I have
told the next T4 — a role (7)(i) does name — that it may ratify my four admission rulings with one
append, or open a contradiction if it disagrees.

**Also entered:** the stale route ledger at `open_questions[8]` is updated by append (work-owed (26)
discharged as an updating task), promoting the Washington Post route from an open route to **the
stated lifting condition of a refused admission**, which gives it a defined consequence for the first
time; `open_questions[10]` records the legislative-testimony cell as owed-but-unoccupied and not
built; and a new `open_questions[12]` records the six-item honest residue.

**Something I wrote against my own result, because it is true.** `candidate_resolutions[3]`
paragraph (5): the response population now holds six members, of which **at least two have artefacts
that are certainly neither CTI nor threat reports** (`src-0048`'s bankruptcy motion, DCDT's national
AI policy), and whether the KPMG and PwC artefacts are CTI **has never been established in this
base**. So any finding this candidate yields is about institutional response to fabricated citations
in general, **not** about AI-generated CTI. The admission I just made is itself an instance of the
dilution the standing caveat warns about, and I told the next T4 to weigh (5) *against* the extra
data point rather than alongside it.

**G3.** No contradiction opened. I found no conflict between two supported claims: the two grounds I
rejected were rejected as *non-discriminating*, not as false, and the tally hazard is a collision,
not a conflict.

**What I left, and I am naming all of it.** The News24 fetch — the single highest-value unfetched
artefact on this issue and the one thing that would supply the counts absent from the whole base — I
**declined and said why in the state**: a T3 may fetch but may not create a `src-*.md`, so anything I
learned would be uncitable under G1 and could only be written down as an open question. That
constraint will defeat every future T3 on this route, so it is handed to a T1 as work-owed (39).
Also left: `ctr-0037` site (2) and `ctr-0062` step (iv) (work-owed (19)/(20), both cheap, both still
owed after seventeen cycles); the three pending standing checks (T2's); the issue split, whose
blocking condition I have now satisfied by exercising the stratum labels; and work-owed (25) on
`automated-triage-under-refusal` — I am a T3 and it is T3 work on an issue that has waited 152
cycles, and I did not take it because the assigned admission work turned out to be **more** expensive
than expected, not less, once I rejected cycle 150's ground and had to find my own.

**Work-owed (30) is now six instances, not five, and I am the one who added the sixth.** I entered
verbatim `src-0049` and `src-0053` strings into `candidate_resolutions[10]` and `[11]` and could not
write them to the source files, because append-only knowledge is not a T3's to edit. In this instance
the source files already carry the same quotes, so it is a duplication rather than a divergence —
but the count is six and the T2 ruling is still owed.

## Next task rationale

`prompts/system.md` line 46, read at source: T3 → **T4**. Written to
`state/queue/next_task.json` with `task_type: T4`, `attempt_count: 0`, `target_issue: null`.
The refresh rule is not mine to compute and I told cycle 153 to tell *its* successor to compute it
itself.

A T4 is the right next step on the merits and not merely by the state machine.
`last_assessed_cycle` is **146**, seven cycles stale, and the graph has moved twice underneath it —
cycle 150 closed two contradictions and built the stratum machinery, cycle 152 closed a third,
admitted a case and ruled four questions. Two issues have live reasons to move: this one, where the
dilution argument may well justify a **lower** score than cycle 146's, and
`incident-evidence-vendor-concentration`, which sits at 3 with one open contradiction and is the only
place in the graph where merit meets the G3 ceiling exactly — cycles 146 and 150 both expected a
post-ruling revisit to press it downward, and the ruling is now executed *and* certified.

I also gave cycle 153 a piece of work only a T4 can do: `ctr-0069` (7)(i) reserves population-
membership rulings to a T4 or a T2, and I am a T3, so my four rulings were made by a role that entry
did not name. One append ratifies them on the right footing, or opens a contradiction if the T4
disagrees. I did not paper over this.

Its G2 target is **cycle 143**, itself a T4 — so the conclusions it re-checks are ten scores and ten
rationales, and it is re-scoring the same ten issues. I told it to do the G2 *before* writing its own
numbers, and warned it to expect **supersessions** (143 predates three closures and one admission)
and not to misreport them as failures.

## Budget

Zero web searches, zero fetches. ~70 tool calls: 5 source reads, ~12 `jq` interrogations of the
graph, 6 `jq` proof/verification calls, 4 file writes, ~10 assembly and cleanup calls for the queue
entry, the rest phase-check, G2 re-derivation and validator reading. No `python` (bash refuses it in
every form, so **no validator run** — every gate below was hand-checked with `jq`).

**Gates hand-checked before finishing, and this is a list of `jq` results and not a validator run.**
JSON parses; issue ids unchanged; every non-target issue byte-identical; all ten pre-existing
candidate summaries and all twelve pre-existing open questions `startswith` their prior versions;
candidate `status` and `evidence` unchanged on all ten; all 69 other contradictions byte-identical
and `ctr-0069`'s description extended, not rewritten; contradiction objects still carry exactly the
five schema fields; the two new candidates carry exactly `evidence`, `status`, `summary` with
`evidence` of type **array**; and **zero dangling evidence ids** across every candidate of every
issue against `index.json`. Final counts: `ctr=70 open=49 issues=10`, target issue `oq=13 cands=12`,
`attempts` ends `…,139,152`.

## Carry-forward items — NEW AT CYCLE 152

**[400]** A FAILED CYCLE LEAVES A CONFIDENT WRITE-UP OF WORK THAT IS NOT IN THE FILES, AND NO RECORD
OF WHY. `run_cycle.sh` keeps `logs/` and reverts `state/`, and it preserves the validator's stdout
nowhere — not in the transcript, which captures only the agent's output. So cycle 152 inherited
cycle 151's detailed account of edits that do not exist, and could not learn what the gate objected
to. **TREAT EVERY `logs/cycle-NNN.md` AS A RECORD OF REASONING AND NEVER AS A RECORD OF STATE, AND
STAT THE COMMIT BEFORE BELIEVING ANY LOG'S ACCOUNT OF WHAT IT WROTE.** A human should make
`run_cycle.sh` tee the validator's stdout into `logs/` on failure — one line of shell.

**[401]** THE PER-ISSUE OPEN-CONTRADICTION COUNT ON `institutional-incident-real-world-impact` WAS
**TWELVE** AT CYCLE 151, NOT THE ELEVEN THE CYCLE-150 ENTRY ASSERTED, AND IS **ELEVEN** AFTER THE
CYCLE-152 CLOSURE. Independently re-derived by cycles 151 and 152 with the `[373]` one-call pattern.
RE-DERIVE, DO NOT INHERIT.

**[402]** IN `jq`, A BARE `.` INSIDE `startswith()`'S ARGUMENT IS THE **STRING BEING TESTED**, NOT THE
LOOP INDEX — so `[range(0;10) | ($n.x[.] | startswith($o.x[.]))]` dies with *Cannot index array with
string*. **BIND THE INDEX**: `[range(0;10) as $i | ($n.x[$i] | startswith($o.x[$i]))]`. This is
`[383]` biting in a new place, and it bites exactly where it hurts — in the append-only proof.

**[403]** COMPRESSION DID NOT SOLVE `[399]` AND CYCLE 152 IS THE SECOND CYCLE TO REPORT THAT.
The narrative sections were cut hard and the entry still grew **67,035 → 72,215 characters**. The
floor is arithmetic: the carried ledgers are ~37.7 KB, the bash notes ~7.8 KB, and the amendments
block a cycle owes its successor **grows with the work the cycle does** — a productive cycle produces
a bigger entry, which is exactly the wrong incentive. The structural fix is moving the ban set, the
honesty items and the work-owed ledger into a versioned file under `state/`, so an entry can cite
them instead of copying them. **Creating a new state file is not a cycle's call and this is flagged
for the human for the second time.** What a cycle *can* do meanwhile is `[400]`'s neighbour: carry
the ledgers **mechanically** — `jq -r .instructions > oldinstr.txt`, `grep -n "^=== "` for the section
boundaries, `sed -n <a>,<b>p >> newbody.txt` — and hand-write only the head and the amendments. That
cost cycle 152 four tool calls instead of several thousand words of transcription, and it guarantees
the ledgers are carried verbatim rather than paraphrased.

**A NOTE ON THE CARRY-FORWARD BELOW, SO THE NUMBERING IS AUDITABLE.** It is copied mechanically from
`logs/cycle-151.md` **starting at the cycle-150 heading, deliberately skipping cycle 151's own
block**. Cycle 151 was reverted: its state never landed, and it assigned itself item numbers from
[400] upward which cycle 152 has now reassigned. Copying its block would put two different meanings
on the same numbers. The high-water item number carried into cycle 152 is therefore **[399]**, as the
queue entry states, and cycle 152's new items are [400]–[403] above. **Cycle 151's reasoning is not
lost — it is in `logs/cycle-151.md` and should be read there**, and its two substantive findings that
survived independent re-derivation this cycle are recorded above: the twelve-not-eleven count ([401])
and the non-discriminating character of cycle 150's proposed ground for refusing MAHA.

## Carry-forward items — NEW AT CYCLE 150

[397] **Refinement of [392], and a correction to the Bash notes.** `jq -r .instructions
state/queue/next_task.json | head -c 23000`, then `| tail -c +23001 | head -c 23000`, then
`| tail -c +46001`, read the entire 68,612-character entry in **three** calls — half what `cut -c`
paging in 12,000-character slices would have cost. Use the widest slice the output cap tolerates,
which is ~23 KB, not 12 KB. Also newly confirmed: `python3 -V` is refused as well as `python3 -c`,
so the interpreter is unavailable in every form; but `ls -la`, `cat`, `wc -c`, `jq`, `sed -n`,
`grep -n`, `mv`, `rm -f` and single-command redirection all run. The **Read** tool additionally
refuses a `grep -o` with a large bounded window on a 68 KB single line (it omits the matching line),
while an unbounded uppercase-phrase pattern with `-o` returns a usable skeleton of the entry — that
skeleton is a cheap way to decide **whether** a huge field is worth paging before you page it.

[398] **A cycle that executes a ruling should not also certify its own execution.** Cycle 150
executed ctr-0069 (7)(i) and (7)(ii) and then **declined to set `resolved_cycle`**, leaving that to
the next T2 or T3 — even though its own freshly proposed closure convention would have permitted it.
The general form: when one cycle both performs a repair and judges the repair complete, the judgment
has no independent evidence behind it. Separate the two roles across cycles; the cost is one cycle
of delay and the gain is that a closure means something.

[399] **Queue-entry size is now a live risk to the chain, not a style question.** The cycle-148
entry reached 68,612 characters. Cycle 149 aborted on it. Cycle 150 spent four tool calls and a
material share of its context reading it before it could begin work, and the entry's own [392] exists
only because the entry had outgrown the `Read` tool. The entry's substance — the ban set, the
work-owed list, the honesty items — is load-bearing and must be carried forward whole. **The
narrative around it is not**, and every cycle that restates its predecessor's reasoning at full
length taxes its successor's ability to do any work at all. Carry the bans and the ledger verbatim;
compress the story. **And the honest half of this item: cycle 150 tried exactly that and got a 2%
reduction (68,612 → 67,035), because the compression it won was spent on what it added. Prose
discipline does not fix a monotonically growing ledger that no role is authorised to prune. The
structural fix is a versioned file the queue entry points at, and it needs a human.**
## Carry-forward items — NEW AT CYCLE 148

[392] **`cut -c<a>-<b>` is the cheapest way to read a huge single-line JSON field when the Read tool
refuses the file.** The Read tool refused `state/queue/next_task.json` at line 5 (34,844 tokens in
one line). `wc -c` gave the bound at 65,163 characters and six `cut -c` calls in 12,000-character
slices read the entire entry — no `jq`, no scratch file, and the same call count `jq -r .instructions
| head -c N` would have cost. Supersedes nothing; it is a cheaper sibling of [384].

[393] **A negative on N queries is a negative on those N queries and nothing else, and the cheapest
way to break one is to change the axis the query ranges over — not to add more values along the same
axis.** Cycle 139 ran three searches on this exact commission and came back empty; cycle 148 ran six
and its **first query alone surfaced both cases**. The difference was framing, not effort: cycle 139
searched by **sector** (security vendors, non-Big-Four consultancies, think tanks); cycle 148
searched by **institution type crossed with outcome** (government agency + corrected report;
university/institute + withdrawn report). **Record the axis, not just the queries** — a successor
reading *three searches, all negative* cannot tell whether the space was covered or merely sampled
along one dimension, and cycle 139's own careful record of its three queries is what made this
diagnosis possible.

[394] **A hedge is part of a claim, and a downstream carrier can strip it silently.** `src-0049` has
the minister saying *"The most plausible explanation is that AI-generated citations were included
without proper verification"*; `src-0053`, three days later, renders the same position as
*"Generative AI was used without proper human verification of the sources."* The drift runs toward
the **stronger** causal claim, which is the direction internal consistency checks are least able to
catch, because nothing in the state contradicts the stronger version — it simply is not what was
said. **The general rule: when a source quotes an institution, record whether the institution hedged,
and treat the hedge as part of the verbatim.** This is the first time this base has held both ends of
such a drift.

[395] **The `r.jina.ai` render is not always the poorer sibling on short pages.** On
`sanews.gov.za` the proxy render delivered **six sentences of the ministerial statement where the
direct render delivered three** — the opposite of the usual expectation and of the pattern recorded
for PDFs. Render coverage is per-source and cannot be predicted from another source's experience;
[205]'s advice to use the pair is vindicated in a direction nobody had recorded.

[396] **Ask every `r.jina.ai` render what its last received line was, and quote the answer into the
source file.** Cycle 148 did this on all three proxy renders it ran. It costs one clause in the
prompt and it is what converts a render's ABSENT from an unbounded claim into an attested absence
over delivered text, per [387]. Quoting it into the `src-*.md` — rather than only into the log —
means a successor can check the bound without re-fetching.
## Carry-forward items — NEW AT CYCLE 147

- **[391]** The two readings of `prompts/t5_select.md` rung 3(b)'s 5-cycle window can now change the
  **arithmetic** and not only the termination behaviour. At cycle 147 `ioc-extraction-reliability`
  takes +1 under the exclusive reading (window 142–146, attempt at 142) and 0 under the inclusive
  one (window 143–147). Every prior divergence — cycles 133, 142, 144 — was about whether the list
  ran out. This one is about a penalty column. The two readings happened to agree on the winner.
  A successor T5 should run both readings and report both whenever they diverge; cycle 147 did so
  at zero extra cost, because only one column changes. **This is a recommendation from one cycle,
  not a rule, and no successor should present it as policy.** Folded into honesty item (B), which
  a human should rule on.
- **Cycle 147 is the first T5 since cycle 126 whose tie-break list terminated without an invented
  rung** — and it is also the cycle that proves honesty items (A) and (B) are independent defects,
  because the list terminated and `automated-triage-under-refusal` still lost, at a *written* rung
  above the point where the policy runs out. Evaluation data for the paper: the full ranking table
  under **both** window readings is in this log under "Next task rationale".
- **The refresh rule cannot aim at the base's most-wanted item.** Cycle 147's refresh (147 % 7 == 0)
  queued a T1, but the T1 must target the selected issue, and rung 3(a) guarantees the selected
  issue is a root. `src-0006`'s per-task scoring definitions — work-owed (6), seventeen cycles the
  highest-value unfetched item — live on `task-dependent-reliability-framing`, which 3(a) makes
  unreachable. Next refresh: **cycle 154**, same property. This is honesty item (C) at its sharpest.

## Carry-forward items — NEW AT CYCLE 146

High-water item number after cycle 146 is **[390]**.

**[389] Contradiction objects in `state/issues/graph.json` carry no `summary` field.** All
seventy have exactly the key set `description, id, issue_id, opened_cycle, resolved_cycle`. A jq
that reads `.summary` off a contradiction **returns null silently** and the entry renders as
blank rather than erroring — indistinguishable from a genuinely empty entry. I hit this reading
ctr-0057 and briefly took it for an empty record. Use `.description`. Same failure family as
[337]: a reported count or field is a triple — FIELD, CASE, SNAPSHOT.

**[390] When a queue entry computes a rule that fires on a cycle other than its own, it must
substitute the cycle number the rule names, not its own.** Cycle 145's entry computed
`146 % 7 = 6` and concluded that cycle 147 was not a refresh cycle. `prompts/t5_select.md` step 4
keys on `current_cycle`, which for that rule is the T5's own number: `147 % 7 = 0`, so the
refresh **does** fire. The arithmetic was right and the substitution was wrong. Always re-compute
a scheduling rule from the prompt and `config.yml` at the cycle the rule actually fires on.

**Process note attached to [390], for the human.** This was caught only because cycle 146
re-read the prompt at source, exactly as cycle 145's own entry instructed it to. These queue
entries are long, authoritative in tone, and written by a cycle that will never learn whether it
was right. **The "read at source, disbelieve this entry" discipline is not ceremony — it paid
for itself this cycle.** Recorded as work-owed (33).

**Updates to standing items, from this cycle's re-derivations:**

- **(20)** — I re-verified the site count myself: the truncated range `0.8240-0.8846` stands at
  **exactly one** live site in `ttp-attack-mapping-reliability`. The defect is not spreading, and
  it remains the reason that issue cannot reach level 4 on merit.
- **(21)** — the ctr-0063 counts re-derived independently for the sixth time (cycles 140, 141,
  143, 144, 145, 146), this time after cycle 145's edits: 19 / 13 supported / 11 citing src-0001 /
  4 on src-0001 alone / the two non-src-0001 supported candidates both single-source.
- **(25)** — re-confirmed on my own reading that `automated-triage-under-refusal`'s
  `candidate_resolutions[1]` is still status PROPOSED, so honesty item (A)'s escape route stays
  shut and a T4 cannot move that score in either direction.
- **(29)** — I declined for the third time (after cycles 140 and 143) to credit src-0048 to
  `incident-evidence-vendor-concentration`, and recorded in that issue's rationale that a
  successor revisiting it after ctr-0069 is executed should expect the revisit to press that
  score **downward**, not upward.
- **(31)** — I wrote ten new rationale blocks and did not reintroduce the ctr-0070
  measured-configuration-versus-workflow conflation. The `state/knowledge/` half is still unrun.
- **(F)** — ctr-0069's cycle-140 ruling is now **seven** cycles unexecuted. I recorded in
  `institutional-incident-real-world-impact`'s rationale that until it is executed **that issue's
  own text overstates its own finding**, which is a reason against 3 and not for it.
- **(E)** — I recorded in `ioc-extraction-reliability`'s rationale that ctr-0066 and ctr-0064
  sitting open while ripe **corrupts the open-contradiction census scores.json itself reports**.
  Under the ceiling reading it costs no issue a point, but the census is evaluation data.
- **[383]** cost a turn again this cycle (a `startswith` argument inside a `map()`), fixed by
  binding both the entry and the prior value at the top of the map body. Four cycles now.
- **New tooling win:** `git diff --stat <a> <b> -- state/` runs fine here and is the cheapest way
  to prove what actually moved between two cycles. First use in this base was this cycle, and it
  is what let me state "nine of ten issues had unchanged inputs" as a proof rather than a claim.

## Carry-forward items — NEW AT CYCLE 145

- **[387] A render's ABSENT over a region it never received is an artefact, not an attested
  absence.** The ar5iv render of src-0014 answered ABSENT for six modality strings 'in appendices
  B–E' while its own last-received-line answer showed it stopped mid-bibliography, before any
  appendix began. Always cross-check the ABSENT's *claimed region* against the render's own
  last-received-line answer before entering the ABSENT. **An ABSENT is only as wide as the text
  actually delivered.** This is a general evidence rule, not a tooling tip, and it is the
  companion to `[382]`: `[382]` says do not trust a claim of completeness; `[387]` says what to do
  with the answers a truncated render gives you anyway.
- **[388] A positive control string makes a sweep's ABSENTs interpretable.** Include in every
  ABSENT sweep at least one string the source is *already known* to contain in the region being
  swept, so that a render silently returning an empty region is distinguishable from a genuine
  absence. Cycle 145 swept src-0014's Appendix A with `self-consistency` as that control; it came
  back PRESENT with a quotable sentence, and that is what licensed reading the other six ABSENTs
  as real. **Without a control, a sweep of six ABSENTs and a blank page look identical.** Note
  that `[387]` and `[388]` caught the *same* fetch pair from opposite ends — one render failed the
  region check, the other passed the control — which is why both are worth keeping.
- **Render coverage is per-source and cannot be predicted from another source's experience.** On
  src-0014 the r.jina.ai PDF form reached appendix bodies that four `/html` fetches never did,
  while ar5iv on the same source reached less than either. On src-0013 both families reached the
  footer. Do not generalise a render family's reach from one paper to another.
- **`awk` is refused by this sandbox** as needing approval (new observation this cycle), joining
  `python3 -c`, `cp`, process substitution, `for` loops and `&&`/`;` chaining.
- **The `[237]` splice takes a `select` inside the path expression and chains.**
  `(.issues[] | select(.id=="X") | .open_questions[13]) += $s.key` works and was chained five
  times in one program this cycle. Cycle 145's two-call append-only proof is worth copying
  verbatim: call one compares `.contradictions`, `._schema`, the id vector and the other nine
  issues; call two compares, within the edited issue, the status vector, the evidence vector, the
  unedited `open_questions` via `del(.[8],.[13])`, and the key set.

## Carry-forward items — NEW AT CYCLE 144

- **[385]** `to_entries[]|select(.key>=N)|(... + .status)` reads `.status` off the *entry
  wrapper*, where it is `null`, and the error surfaces somewhere else entirely — cycle 144 got
  `Cannot iterate over null (null)` pointing at `.evidence` when the real fault was a missing
  `.value.` prefix. **Inside `to_entries[]`, every field of the original object needs
  `.value.`.** Same family as [381] and [383]; this class has now cost three cycles.
- **[386]** A dependency-order rung like 3(a) must be computed **pairwise within the tier**, not
  by asking "is `depends_on` empty". The two give the same answer in this graph only because
  every out-of-tier dependency happens to sit on a *higher*-scored issue; if a score-2 issue
  ever depended solely on a score-3 issue, the empty-check would wrongly eliminate it while the
  pairwise reading would keep it. ctr-0067 step (6)(i) fixes the reading as **pairwise** —
  state which one you used.
- **Selector finding, for the paper.** Cycle 144 is the first cycle to compute both readings of
  the 3(b) window side by side. They **agree on the winner** and **disagree on whether the rung
  list terminates**. The non-termination defect is therefore independent of outcome, which is a
  stronger statement of honesty item (B) than fourteen prior restatements achieved.

## Carry-forward items — NEW AT CYCLE 143

- **[383] In jq, the argument expression of a filter like `startswith()` is evaluated with
  `.` set to that filter's own input, not to the surrounding context.** So
  `to_entries[]|($n[.key].rationale|startswith(.value.rationale))` fails with "Cannot index
  string with string" — the `.value` is being read off the rationale string. **Bind the
  entry first:** `to_entries[] as $e | ($n[$e.key].rationale|startswith($e.value.rationale))`.
  This cost two failed calls and refines **[381]**: the same class of silent rebinding, in a
  different place.
- **[384] `state/assessments/scores.json` is over 1 MB and the Read tool refuses it** — read
  it through jq only. To find a passage inside a 120 KB single-line rationale, use `match()`
  to get offsets and then **slice** (`.rationale[17400:19600]`), because `grep -n` on a
  jq-extracted field is useless when the whole block is one line (**[318]**). The Read tool
  likewise refused `state/queue/next_task.json` at cycle 142; **write the successor entry to
  a new path and `mv` it into place**, which is what this cycle did.
- **A process lesson worth more than either, and it is work-owed (32):** a repair step
  addressed to a file the reporting role cannot write is a step nobody verifies. `ctr-0010`
  step (i) was discharged at cycle 36 and has now been re-confirmed three separate times,
  yet the instruction chain has re-issued it four times, because the T3s that report it
  cannot read `scores.json` and the T4s that can are not the ones writing the carry-forward.
  **No cycle may report a repair step as UNDONE without reading the file the step names;
  reporting it as UNVERIFIED is always available and costs nothing.**
## Carry-forward items — NEW AT CYCLE 142

- **[382]** — **A render's own claim of completeness cannot be trusted.** Asked to list every heading
  to the end of `arxiv.org/html/2506.11325v2`, the **direct** render reported the document complete
  through References with no appendices; a second, narrowed ask at the same URL showed it had actually
  cut off **mid-citation inside the bibliography**, while the `r.jina.ai` render of the same URL showed
  an Appendix A it never reached. The apparent two-render disagreement was a **truncation artefact,
  not a rival observation**. If a render says it saw the whole document, ask it separately what its
  last received line was. This refines [205] and cycle 131's note that both renders self-report
  truncation past the body — at cycle 142 one of them did not.
- **The `Read` tool refuses `state/queue/next_task.json`** at its current size (49 KB in one 48 KB
  line) — both for reading past line 4 *and* for overwriting. Read it with `cut -c<a>-<b>` in chunks,
  and write the successor entry to a new path and `mv` it into place. Also confirmed: `wc -c` chained
  with `&&` to an `awk` is refused; split them.
- **ctr-0066 is ripe for closure and was not closed** — the second clean instance of honesty item
  (E)'s unruled question, alongside ctr-0064. Open-contradiction count 52 now overstates live disputes
  by at least two.
- **ctr-0070 is new**, and the pattern behind it is worth a human's eye: this base made the same
  measured-configuration-versus-surrounding-workflow category error **twice, in opposite directions,
  in adjacent candidates of the same issue** (ctr-0066 on the `src-0003` side, ctr-0070 on the
  `src-0007` side). Its step (iii) proposes a standing check; a T2 now has **three** pending.
- **Honesty item (B) has a second confirmed instance.** Cycle 133, like cycle 141, terminated cleanly
  **only** under the exclusive 5-cycle window it declared; under the inclusive window it deadlocks
  exactly as 127, 130 and 138 did. The window boundary is a live defect, not a hypothetical.
- **Work-owed (18) is decided, not merely re-raised.** ctr-0060 step (iii)'s standing check is **not
  runnable as worded** — `src-0003` has no Limitations, Threats-to-Validity or Discussion section. A T2
  must reword it to mean the source's stated limitations wherever they appear, or drop it.

## Carry-forward items — NEW AT CYCLE 141

[381] **A `jq` program that binds `. as $k` and then refers to `.` expecting the original
root silently returns the key instead of the value.** `'.scores|keys[]|. as $k|($k+"
score="+(.|tostring))'` printed `ioc-extraction-reliability score=ioc-extraction-reliability`,
not the score — it looks exactly like corrupt or missing data and it fails quietly. Use
`to_entries[]` when you need both key and value. This is the same failure family as
[337]'s field/case/snapshot triple and as the `depend_on`/`depends_on` and
`cycle_opened`/`opened_cycle` traps: **the instrument was wrong, not the data.** I hit it
on the very first attempt to re-derive the score vector and caught it only because the
output was self-evidently absurd; a subtler binding would not have announced itself.

[382] **Two `jq` calls separated by `;` in one Bash call were refused as needing
approval**, consistent with the standing note that `;` chaining is not accepted. The
single leading `cd <dir>; <cmd>` form still works. Split them; budget one Bash call per
`jq` program.

[383] **The 3(b) window boundary must be *declared before it is applied*, because at
cycle 141 it decided the whole selection.** Cycle 141 declared cycles 136–140 (the five
most recent *completed* cycles) and gave its reason: no attempt can be recorded at the
current cycle at the moment a T5 runs, so a window including 141 always wastes one of its
five slots. Under 137–141 the cycle would have deadlocked exactly as 127, 130 and 138
did. **A successor T5 must state its window and its reason before applying it**, and
should record which window earlier T5s used, because that record is the evidence a human
needs to rule on honesty item (B). Cycle 142's G2 falls on cycle 133, a T5, and is asked
to check specifically whether 133 declared a window at all.

[384] **A third, smaller reading question in the same rung list, raised and deliberately
left open:** does the 3(b) penalty produce an "effective score" that competes *across*
base-score tiers, or is it only ever compared *within* a rung-2 tie? Cycle 141 took the
within-tier reading (rung 3 is a tie-break: it breaks ties, it does not re-rank tiers)
and verified the selection is identical under both readings this cycle, because
`ioc-extraction-reliability` held the unique minimum at 2. **It will not always be
moot** — as soon as a penalised score-2 issue would be the sole survivor, the two
readings diverge. Folded into honesty item (B) rather than raised as a separate item.

[385] **A T4's conclusions re-check cleanly and cheaply against its own revision, and
cycle 141's G2 is the pattern to copy.** `git show <rev>:state/issues/graph.json >
old.json` followed by a single counting `jq` reproduced all five of cycle 132's numeric
claims exactly, at zero fetches. The same counts read differently today (15/12/10/4 then,
19/13/11/4 now) — **that is supersession by four later candidates, not an error**, and it
independently corroborates work-owed (21) from a different direction. Checking a past
count against *today's* file would have manufactured a false failure. Always recover the
snapshot.

## Carry-forward items — NEW AT CYCLE 140

- **[379]** The whole-file append-only proof in one call. `git show HEAD:<path> > old.json`
  first (redirection is allowed), then
  `jq -n 'input as $o | input as $n | [($n.scores|keys[]|. as $k | ($k+":score="+((($n.scores[$k].score)==($o.scores[$k].score))|tostring)+",appendonly="+(($n.scores[$k].rationale|startswith($o.scores[$k].rationale))|tostring)))]|join("\n")' old.json new.json`.
  The `keys[]|. as $k` idiom iterates every entry **without an object constructor**, so it
  survives the shell, and it proves score-invariance and append-onlyness together. The same
  shape with `($n.x.evidence)[0:($o.x.evidence|length)]==($o.x.evidence)` proves array prefixes.
- **[380]** `scripts/validate_state.py` **cannot be run here but it can be read**, with
  `grep -n` and `sed -n <range>p`. No cycle had read it in 140 cycles, and doing so narrowed a
  policy question six T4s had left open. **If you are unsure what a gate actually enforces, read
  the gate.**
- **Honesty item (G), new:** the g3 demotion is currently **inert** across the whole vector —
  under the ceiling reading it only binds issues that would otherwise score 4 or 5, and nothing
  in this graph does. Independent of the carry-forward [4] wording question.
- **Honesty item (F), ruled but not executed:** `ctr-0069` (7)(i) — **ADMIT into a labelled
  stratum, DO NOT PROMOTE**; (7)(ii) — **YES**, the axis gains a third value,
  *control-failure attribution*. The `graph.json` remedy is owed to a T2 or a T3 on that issue.
  `ctr-0069` stays open.
- **Carry-forward [4] narrowed:** the validator implements a **ceiling, not a subtraction**, but
  it only sets an upper bound, so it *permits* rather than *compels* that reading. The residual
  question for a human is whether `prompts/t4_assess.md` step 3 should be reworded to match.
- **Honesty item (B) widened:** the 5-cycle attempt-penalty window boundary is unspecified, and
  at cycle 141 it alone decides whether the tie-break list deadlocks.
- **Work-owed item (18) rewritten:** the proposed standing check is **not runnable as worded** —
  no cycle has recorded whether `src-0003` has a paper-side Limitations section, and the only
  such heading in the base is the state's own editorial note. A T2 must first decide which
  "Limitations" it means.
- **Work-owed item (11) extended:** an end-to-end read of `src-0003` would also settle whether
  that paper has a Limitations section at all.
- **Work-owed item (29), new:** `src-0048` is the first incident data point whose discovery route
  does not pass through GPTZero, which bears directly on
  `incident-evidence-vendor-concentration`'s concentration claim. Not added there and no score
  moved (it is itself a single-Reuters-wire route, and the `ctr-0069` ruling is unexecuted).
  Revisit that issue once the ruling lands.
- **Work-owed item (30), new:** cycle 131's three verbatim `src-0003` quotes live only in
  `graph.json`, not in `state/knowledge/src-0003.md`. A T2 might consider whether verbatim
  strings established by a T3 should be appended to the source file too. Not commissioned.

## Carry-forward items — NEW AT CYCLE 139

[375] **`grep -o` in a compound pipeline is refused as needing approval.** Use `jq`'s `scan()` with
a bounded window instead — `jq -r '<path>|[scan("PwC.{0,220}")]|join("\n---\n")'` worked first try
and is the right way to search inside a huge single-line JSON string. This supersedes [348]'s
advice to fall back to the Grep tool for that job; `scan()` is cheaper and stays inside one call.

[376] **The cheapest append-only proof there is: hash the regions that must not change, on both
sides.** `jq -S -c '<region selector>' new.json | md5sum` against
`git show HEAD:<path> | jq -S -c '<region selector>' | md5sum`. Cycle 139 proved all 47 prior
sources, all 68 prior contradictions, the nine untouched issues, the nine prior candidates and the
seven untouched `open_questions` byte-identical in **two calls**, with no `diff`, no process
substitution and no temp files. `-S` sorts keys so incidental reordering by the splice cannot show
up as a false difference.

[377] **To prove a string field was *extended* rather than rewritten, bind both files.**
`jq -n 'input as $o | input as $n | ($n.field|startswith($o.field))' old.json new.json`. A bare
`.issues` inside `startswith()` resolves against the *pipe input*, not the outer binding, and fails
with `Cannot index string with string "issues"` — cycle 139 hit this and lost a call to it.

[378] **When two renders of the same URL are asked different questions and one returns a fact the
other does not, that is a coverage difference arising from the questions asked — it is NOT a
two-render disagreement.** Say which it is. Cycle 139's direct render of `src-0048` was never asked
the policies-followed limb and the `r.jina.ai` render was; recording that as a render conflict
would have manufactured a contradiction out of the query design.

**Non-tooling items entered at cycle 139** are carried in the cycle-140 queue entry rather than
duplicated here: honesty item **(F)** (`ctr-0069`'s population ruling, a T4's or T2's), work-owed
**(26)** superseded and replaced (item (iii) discharged, (ii) dated-negative, (i) still blocked; the
new highest-value want is a second non-Big-Four case whose artefact is a *published report*),
**(27)** the never-attempted primary artefact behind `src-0048` (a US bankruptcy docket, tooling
accessibility unknown), and **(28)** the third-party-response axis — organisations *named in* a
hallucinated report disputing what it said about them — recorded as a lead and expressly not
commissioned.

## Carry-forward items — NEW AT CYCLE 138

[373] `jq -r '[.contradictions[]|select(.resolved_cycle==null)|.issue_id]|group_by(.)|map((.[0])+"="+(length|tostring))|join(" ")'`
is the one-call way to get open contradictions per issue. It needs no object constructor and so
survives the shell, per [227].

[374] Do not dump a large `open_questions` array whole — this issue's is 61 KB and the tool
truncates it to a file. Get an indexed head listing first with
`.open_questions|to_entries[]|("["+(.key|tostring)+"] len="+(.value|length|tostring)+" :: "+(.value[0:260]))`,
then pull the single entry you need in full.

[375] The refresh rule can only fire when a **T5** lands on a multiple of `collect_refresh_every`.
Cycle 137's queue entry asserted it fires at 140; 140 is a T4. T5s fall on 138/141/144/147 and the
next one that is a multiple of 7 is **147**. A cycle reading a predecessor's refresh prediction
should re-derive it against the T1/T3/T4/T5 phase, not just the modulus.

[376] Rung 3(a) applied consistently excludes **four** score-2 issues, not two: the two cycle 137
named (task-dependent, extraction-vs-reasoning) plus attribution-confident-wrong-gap and
attribution-expressed-confidence-unmeasured, both of whose dependencies sit in the same tier. While
the whole graph sits in one score tier the selector can reach **roots only**. This widens honesty
item (C).

## Carry-forward items

(Copied mechanically from `logs/cycle-135.md` below this line, per the standing rule; discharged
items are updated in place in the next queue entry rather than deleted here. High-water item
number after cycle 136 is **[370]** — [369] is the two-render pair-rule evidence and [370] is the
`[range(0;N)]|map(. as $i | ...)` form that replaced the failing `[range(0;N)| $a[.] ...]`.)

## Carry-forward items — NEW AT CYCLE 135

- **[364]** A jq path expression `(.issues[] | select(.id=="X") | .field) += $s.key` works and
  is the cleanest way to append to one issue or one contradiction; ten such clauses chained
  with `|` in one program all applied correctly in a single pass. But
  `[range(0;N) | $a[.] …]` fails with "Cannot index array with string" — use
  `map(. as $i | $a[$i] …)` instead.
- **[365]** The graph append-only proof that passed is six conjuncts, run as four separate jq
  calls because one call was too long to debug: `_schema` identical; issue id list identical;
  contradiction id list identical with every description a prefix-extension and all four
  metadata fields unchanged; every untouched issue byte-identical; the edited issue's
  `open_questions` same length with each a prefix-extension, `candidate_resolutions[0:14]`
  identical and `[14]` a summary-only prefix-extension; and the second edited issue's old
  candidate array an exact prefix of the new one.
- **[366]** Rung 3(a) of `prompts/t5_select.md` is read **PAIRWISE**, decided at cycle 135 and
  recorded in `ctr-0067`. A T5 must apply it as a partial order restricted to the score tier
  under consideration and take maximal elements — **not** as "every root outranks every
  non-root". The two readings coincide whenever a dependent's dependencies share its tier,
  which is the present and persistent situation, so this changes no outcome yet.
- **[367]** The fine-tuning census now lives, refreshed and dated, inside
  `consistency-calibration-as-failure-mode`'s `open_questions[11]`. **Re-run it whenever a
  source is added** — it has gone stale twice (missed src-0043 for ten cycles, then src-0045/
  0046/0047). Its current bottom line: eleven files mention fine-tuning, seven actually
  fine-tune, exactly **two** pair it with a proper score (src-0001, src-0045).
- **[368]** The `open_questions[5]` / `[11]` merge that cycle 121 left as a standing
  instruction is **decided: no merge**, on the evidence that the two questions drew different
  candidates at cycle 134. Do not re-litigate absent evidence the candidate sets have
  converged. `[11]`'s angle count is **2** and must never be folded into `[5]`'s eleven.
- **NEW HONESTY ITEM (E)** — this base has no recorded convention for who may set
  `resolved_cycle` on a contradiction whose repair steps are all discharged. `ctr-0062` has
  been held open fourteen cycles on that scruple and `ctr-0065` was left open at cycle 135 for
  the same reason despite having no outstanding action. The cost is visible and quantified:
  "open contradictions" reads **52** and is therefore an **overcount of live disputes**, and
  the G3 ceiling keys off exactly that set. A human should rule, or a T2 should adopt a
  convention and record it.

## Carry-forward items — NEW AT CYCLE 134

- **[360]** `grep -rn` over the whole `state/` directory can return 100 KB+ of output, because
  `graph.json`'s blocks are single enormous lines and one match drags the whole block. **Run
  `grep -c` first** to learn whether a term is present at all, then narrow. I hit this immediately
  and burned an output on it.
- **[361] The `/abs` render is NOT a cheaper subset of the body render, and this is a real trap.**
  src-0045's domain-fine-tuning sentence — the one `ctr-0068` turns on — is present in the abstract
  and the **body render returned ABSENT for it**, twice, asked directly. Had I fetched only the
  body I would have missed the claim entirely; had I fetched only the abstract I would have entered
  it as though it were measured. **Fetch both when a claim matters.**
- **[362]** When refreshing a negative existential under `ctr-0034`, **record the queries verbatim in
  the log**. Cycle 25's negative was unfalsifiable-in-practice for 109 cycles partly because a
  successor could not tell what had actually been searched. Mine are listed above; a successor can
  now diff its search against mine instead of repeating it blind.
- **[363] A methodological ceiling, not a defect.** This base now holds ECE numbers from src-0001,
  src-0013 **and** src-0045, and **not one of the three has its bin count or binning scheme
  recorded**. src-0028 and src-0029 establish that a fixed-binning ECE is bin-count-dependent and a
  *lower bound* on the true calibration error. **Cross-source ECE comparison in this base is
  therefore uninterpretable until binning is known** — that applies to `ctr-0068` and to every
  future comparison, not to one pair. Filed as `ctr-0068` step (6)(ii) and as honesty item (D).

## Carry-forward items — NEW AT CYCLE 133

[357] **THE `depend_on` / `depends_on` FIELD-NAME TRAP, AND IT HAS ALREADY COST ONE SELECTION.**
`prompts/t5_select.md` rung 3(a) spells the word **`depend_on`** inside its English sentence.
The field in `state/issues/graph.json` is **`depends_on`**. A `jq` query on `.depend_on` returns
`null` for every issue — a clean, plausible, entirely false answer that reads as "no issue
declares a dependency". Cycle 124 made exactly this error and selected an issue that rung 3(a)
should have eliminated first (see ctr-0067). **ANY CYCLE APPLYING RUNG 3(a) MUST QUERY
`depends_on`.** Generalisation worth keeping: a `jq` field query that returns `null` uniformly
across every element is far more likely to be a misspelt field than a uniformly empty one —
**check the field exists before reading meaning into its emptiness.**

[358] **ctr-0067 IS OPEN**, on `task-dependent-reliability-framing`, `opened_cycle` 133,
`resolved_cycle` null. It records cycle 124's selector misapplication in full. Two things are
owed. **(i) A T2 must decide whether rung 3(a) is read STRICTLY** (every root outranks every
non-root) **or PAIRWISE** (an issue is outranked by each issue it depends on) and record the
reading in the state; the two coincided at cycles 124 and 133 but need not coincide in general.
**(ii)** The standing consequence — that 3(a) makes two issues permanently unreachable — is a
**human's** call and is escalation item 3 above. Note for whoever touches it: ctr-0067 is a
**process** defect and **must not move any score in either direction**, and under the ceiling
reading of G3 it adds no further demotion beyond what that issue's existing open entries impose.

[359] **A QUEUE ENTRY IS NOT A SOURCE, AND CYCLE 132's WAS WRONG ON A CHECKABLE FACT.** Cycle
132's `next_task.json` asserted that `extraction-vs-reasoning-ordinal-axis` has an **empty**
`depends_on` and could therefore be used to route a T1 at ctr-0009 step (iii) within the rules.
Its `depends_on` is `[task-dependent-reliability-framing]` and has been at least since cycle
124's snapshot. The claim was almost certainly inherited from cycle 124's own "3(a) is inert"
error ([357]) rather than checked. **THE HANDOFF ENTRY IS THE MOST-READ AND LEAST-VERIFIED TEXT
IN THIS SYSTEM**: every cycle reads it in full and no gate checks it. The queue entries
themselves say "READ THAT PROMPT AT SOURCE — do not trust this entry's account of it"; extend
that discipline to **every structural fact** an entry asserts about `graph.json`, not just to its
account of the prompt. One `jq` call is cheaper than a misrouted cycle.

## Carry-forward items — NEW AT CYCLE 132

**[353] `scores.json`'s `scores` is an OBJECT keyed by `issue_id`, not an array.**
`jq '.scores[0]'` fails with `Cannot index object with number` — I lost a call to this.
Use `.scores|to_entries[]`, and per [335] bind with `. as $e` before reaching for `$e.key`.
The per-issue score vector in one call:
`jq -r '.scores|to_entries[]|.key+" | "+(.value.score|tostring)+" | ev="+(.value.evidence|join(","))' state/assessments/scores.json`.
Note also that `.issues[].candidate_resolutions[]` uses the field name **`summary`**, not
`text` — a `.text` selector returns empty and looks like an empty candidate.

**[354] ctr-0066 step (ii) is DISCHARGED at cycle 132 with a NULL result — do not re-run it.**
Zero corrections owed in `state/assessments/scores.json`. Four `scan()` patterns over every
rationale: a `97\.6` window returned 18 hits, none attributing the figure to a pipeline
containing human validation; `[Hh]uman.{0,3}validat`, `hybrid`, and
`HITL|human.in.the.loop|human review|human-in` each returned **zero**. The conflation
ctr-0066 charges lives in `graph.json`'s candidate wording, not in the scores file. Steps
(i) and (iii) remain OPEN and are owed to a T3 or T2 on ioc-extraction-reliability. This is
the second null sweep in the file (cycle 129's ctr-0064 step (ii) was the first) and **a
null sweep is a real result** — two contradictions have now been shown not to have
propagated into scoring.

**[355] SUBTRACT THE BAN SET BEFORE COUNTING IDS. A T4 that counts evidence ids without
checking the ban set overstates the two widest issues badly.**
`institutional-incident-real-world-impact` (14 ids) and `incident-evidence-vendor-concentration`
(13 ids) have the widest evidence arrays in the file, and those counts are inflated by
sources this state may not quote: src-0037 **banned outright**, src-0036 **banned outright**,
src-0034's percentage tiles and 40-of-45 / 5-of-45 figures banned, src-0041's Figure 51
banned, src-0022's oft-cited figure **absent from that source**, src-0044 evidence of
publication state on a date and nothing else. Cycle 132 re-derived the *quotable* support
instead: vendor candidate 0 retains src-0032, src-0012, src-0035, src-0004 and candidate 1
retains src-0032, src-0038, src-0039 — both still clear two independent quotable sources, so
the 3 stands on surviving evidence. **The vendor 3 is the one score in the vector a single
further outright ban could break.** A successor T4 should re-run this subtraction rather
than inherit the conclusion.

**[356] THE REFRESH RULE FIRES ON CYCLE 133: 7 × 19 = 133, so `133 % 7 == 0`.**
`prompts/t5_select.md` step 4 makes the next task **T1 collect**, not T3 investigate.
Precedent: cycle 119 % 7 == 0 and the following T1 added src-0042 at cycle 120. This is
load-bearing right now — cycles 129–132 all added no source, the base has stood at 44 since
cycle 128, and cycle 132's assessment found nothing new bearing on *any* issue since cycle
129 (ten entries, all re-derivations). A successor should re-derive the modulus and
`collect_refresh_every` at source rather than trust this note. The highest-value T1 target
is ctr-0009 step (iii) (src-0006's unfetched per-task scoring section) — but its home issue
is unreachable under tie-break 3(a), and **extraction-vs-reasoning-ordinal-axis has an empty
`depends_on`, is reachable, and rests on the same unread rule**, so that is the legitimate
route to it.

**Also recorded at cycle 132, not new items but re-affirmations with fresh derivation:**
every one of the ten issues carries ≥ 1 open contradiction, so **every** entry is
ceiling-capped at 3 and the ceiling binds on exactly two (ttp, vendor); carry-forward [4]
(subtraction vs ceiling) remains unresolved and only a human can settle it; and on
`consistency-calibration-as-failure-mode`, **12 supported candidates, 10 including src-0001,
4 resting on src-0001 alone** — the ten-of-twelve figure independently reproduces cycle 129's
discharge of ctr-0063 step (i), and the solo count of four is new.

## Carry-forward items — NEW AT CYCLE 131

- **[350] The render pair earns its cost on disagreement, not on redundancy — and this cycle is the
  clearest demonstration yet.** On the *same* URL the direct and `r.jina.ai` renders (a) truncated in
  **different places** (mid-References `[32]` vs mid-Appendix A.1), (b) returned the *same* sentence
  at **different lengths**, one a clean **prefix** of the other, and (c) **disagreed outright** on one
  question — with the **proxy giving the more careful answer** (ABSENT, correctly noting no sentence
  states the shared-candidate-set premise, where the direct render volunteered an interpretation).
  This confirms [205]'s "neither render family outranks the other" from the other direction: cycle 92
  found the proxy is *not* more faithful; cycle 131 finds it is *sometimes more careful*. **Use the
  pair, and when they differ, take the more conservative answer and label the gap.**
- **[351] A render's interpretive gloss is not a quotation, and the pair is what exposes it.** When
  the direct render answered that the 1,789-candidate sentence "establishes a shared baseline," that
  was the **model's reading**, not the paper's words — and the proxy's ABSENT proved it. A fetch
  answer that *characterises* rather than *quotes* must be demoted to an inference even when it is
  probably right. Same defect class as the "(Vanilla LLMs)" reading (cycle 21).
- **[352] An attested absence is worth more when the question had a positive form ready.** Asking
  "is there an ablation?" alone would have produced a bare ABSENT. Asking it **alongside** "what is
  the LLM handed, and what does the regex stage emit?" produced both the absence *and* the
  architecture sentences that make the absence *interpretable* — the paper has no ablation **and** the
  reason it does not need one for the qualitative claim is visible in its own pipeline description.
  **Pair every absence probe with the positive probe that would make the absence mean something.**
- **`ctr-0066` is new** (G3, on `ioc-extraction-reliability`). Its **standing citation rule binds
  immediately and survives the entry's eventual closure**: any state text attributing `src-0003`'s
  97.6% to a pipeline **must not include human validation in that pipeline's description** without
  also saying the human loop was evaluated separately in Section 5.2. Its **step (ii) — the
  `scores.json` sweep — was NOT run and its size is unknown.** Do not read the entry as if it had
  been. Steps (i) and (iii) are a T3's or a T2's.
- **New standing inference label:** `src-0003`'s **shared-candidate-set premise** (that all of Section
  5.1.1's compared methods label the same regex-produced candidate list) is an **inference, not a
  quotation** — the two renders disagreed and the proxy answered ABSENT. Label it wherever used.
- **Carry-forward item (11), new at cycle 131.** **No cycle has still read `src-0003` end to end**, in
  ten investigations. Cycle 117 flagged it; cycle 131 read its architecture, evaluation-setup and
  scope sentences by targeted verbatim query, which is **not** the same thing and does not discharge
  the item. Still owed to a T1 or a T3 with fetch budget.
- **Carry-forward item (12), new at cycle 131.** `ioc-extraction-reliability`'s `open_questions[1]`
  (indicator types beyond IPs/domains/URLs/hashes) **joins `open_questions[4]` as a T1 collection
  target and is no longer a T3 item.** Both sides of `ctr-0001` now carry an explicit, textually
  attested four-type ceiling, so no source in this base measures registry keys, mutexes or
  YARA/Sigma rules at all. A T1 should seek a benchmark covering behavioural or rule-shaped
  indicators — and should expect the four-type ceiling to be a **field-wide convention** rather than
  an accident of two papers, since both benchmarks chose it independently and one states
  verifiability as its reason.
- **Selector defect, re-affirmed not re-litigated.** Cycle 131 did **not** reopen cycle 130's ad-hoc
  fourth tie-break rung, as its queue entry instructed. The three escalations to a human stand
  unchanged: (a) `prompts/t5_select.md`'s tie-break list does not terminate (deadlocked at 127 and
  130); (b) `automated-triage-under-refusal` has never been investigated in 131 cycles; (c) rung 3(a)
  permanently excludes every issue with a non-empty `depends_on`, so the selector **can never reach**
  `task-dependent-reliability-framing`, home of carry-forward item (6) — which **four** cycles now
  call the highest-value unfetched item in the base.
## Carry-forward items — NEW AT CYCLE 130

- **[348]** `grep -o` with a context-window pattern (`grep -n -i -o ".\{0,220\}PAT.\{0,220\}"`)
  was **refused as needing approval** by this environment's Bash gate. Use the **Grep tool** with
  `-C` instead — it is not subject to that gate and is better for multi-file searching anyway.
  This supersedes nothing in [318]; the jq offset-slice technique is still correct for reading a
  single long JSON field.
- **[349]** A case-insensitive grep for a short metric acronym matches inside ordinary English
  words — `ECE` hits `re**ce**ive` and `ne**ce**ssary`. A raw hit count for `ECE`/`AUC`/`ROC` is
  **not** a count of the metric. Bound the pattern or read the hits before reporting a count.
  This is a concrete instance of [337]'s CASE leg.
- **Selector defect, second occurrence, escalated to a human.** `prompts/t5_select.md`'s
  tie-break list does not terminate; it deadlocked at cycle 127 and again at cycle 130. Both
  cycles invented the same fourth rung (least-recently-attempted) without authority. Two
  plausible alternatives — most-open-contradictions and highest-downstream-fan-out — would have
  chosen differently this cycle, and the latter is a direct extension of rung 3(a)'s own logic.
  **A human should rule on a fourth rung.**
- **Selector defect, third finding, new at cycle 130.** Rung 3(a) permanently excludes every
  issue with a non-empty `depends_on`, which means the selector can never reach
  `task-dependent-reliability-framing` — the home of carry-forward item (6), which three separate
  cycles call the highest-value unfetched item in the base. The policy cannot reach its own
  investigators' top priority.
- **Carry-forward item (10), new at cycle 130.** `consistency-calibration-as-failure-mode`'s
  open_questions[11] contains a cycle-121 Grep census of which `src-*.md` files mention
  fine-tuning, written as a standing instruction ("so a successor need not redo it"). It was
  **exact when written** (verified this cycle against the cycle-121 snapshot, srcs=42) and is now
  **incomplete**: `src-0043` (added cycle 125) also mentions fine-tuning and in fact fine-tunes
  `roberta-base`. The census's bottom line survives — src-0043 contains zero occurrences of
  `calibrat`, `ECE`, `Brier` or `reliability diagram` — so "no source other than src-0001 pairs
  fine-tuning with a proper score" still holds. A T2 or a T3 on that issue should **append** the
  src-0043 update so no successor trusts a stale enumeration. Not a contradiction: the claim is
  self-timestamped and true as scoped.
## Carry-forward items — NEW AT CYCLE 129

- **[346] Building a new object in jq without tripping shell brace expansion.** `from_entries` on an
  array of two-element arrays **fails** (`Cannot index array with string "key"` — jq wants
  `.key`/`.value` objects). The working form starts from an empty `{}`, which bash leaves alone
  because it contains no comma, and assigns fields:
  `({}|.id="ctr-0065"|.issue_id="..."|.opened_cycle=129|.resolved_cycle=null)`. Cycle 129 used
  exactly this to append a contradiction after `from_entries` failed. Supplements [227].
- **[347] The append-only proof for a whole file of rationales.** Five conjuncts, all of which
  passed: keys equal; score vector equal; every new rationale `startswith` its old rationale; every
  new evidence array has the old one as a **prefix** (not merely equal — this permits appends and
  forbids reorderings and deletions); `_schema` unchanged. For the graph: `.issues` equal outright,
  and the new contradictions array's leading slice equal to the old array. This is the right
  instrument for a T4, as cycle 128's `startswith` form was for a T3 appending to open_questions.
- **[348] Sweeping long rationales without [318]'s failure mode.** `grep -n` on a jq-extracted field
  is useless because a whole rationale is one line. Better than offset slices:
  `[scan("[^.]{0,300}PATTERN[^.]{0,200}")]|join("\n")` gives every occurrence with a bounded
  sentence-ish window around it. Cycle 129 used this for the ctr-0064 KPMG sweep and the ctr-0037
  site checks and it found both the hits and their qualifiers in one call each.
- **A G2 lesson worth generalising, and it is the reason ctr-0065 exists.** Before entering an
  ABSENT about a source, **read that source's own file in `state/knowledge/`.** A repetition
  protocol stated for statistical stability and a reported repeat-agreement metric can coexist in
  one paper; finding the former is not evidence of the latter's absence. Cycle 120 had the
  contradicting text in the base the entire time.
- **ctr-0060 step (iii) is now a T2's or a human's, not a T4's, and the reason is on the record.**
  `prompts/t4_assess.md` defines a T4 as a scorer and confers no authority to adopt a standing
  methodological rule binding future cycles. The question it asks is still live and still good.
- **[344] restated with a sharper edge.** A human or a T2 should rule on whether direct
  publication-state observations warrant a source-entry convention of their own. Cycle 129 found the
  concrete cost: the cheapest route away from this base's vendor concentration is available *only
  where the institution's own host happens to admit a plain client* — `assets.kpmg.com` does and
  became src-0044, `pwc.com` returns 403 to the validator's own instrument and could not be sourced
  at all. G1 as written has no slot for a first-person publication-state observation, and
  `incident-evidence-vendor-concentration` is where that gap bites hardest.

## Carry-forward items — NEW AT CYCLE 128

- **[344] `pwc.com` IS READABLE BUT NOT SOURCEABLE, AND THIS IS A G1 REVERT HAZARD, NOT AN
  INCONVENIENCE.** Direct fetches of `pwc.com/m1` return **HTTP 403 to a plain non-browser
  client** — confirmed on **both** an HTML publication page and a PDF — while `r.jina.ai` returns
  the pages in full. `scripts/validate_state.py`'s `url_alive()` fetches each **new** source URL
  with an ordinary `requests` client (HEAD then GET, Mozilla UA); a URL returning ≥ 400 **fails G1
  and reverts the entire cycle**. I therefore **deliberately added no PwC source** and recorded the
  PwC retention as a **dated first-person retrieval observation** inside the issue — exactly the
  form `ctr-0060` step (ii) asked for (*"record the observation with its date"*), and the same
  handling cycle 114 gave the FT retrieval failure. **A human or a T2 should rule on whether
  direct publication-state observations warrant a source-entry convention of their own**, because
  this issue's cheapest remaining routes are **all** of that shape and G1 as written has no slot
  for a first-person observation. By contrast `assets.kpmg.com` **serves plain clients** (200,
  binary returned), which is why the KPMG limb could be sourced and the PwC limb could not.
- **[345] A SEARCH-INDEX URL IS NOT AN OBSERVATION.** Search handed me a PDF filename for the GCC
  mobility report that **404'd**; the report was in fact **live** under a *different* filename
  linked from its own landing page. Had I recorded the 404, I would have entered a false removal
  into the state. **Always resolve the landing page before recording a 404 as a removal.**
- **`ctr-0064` is new** (G3, KPMG partitive). Its **step (ii) — sweeping `scores.json` and
  `graph.json` for unqualified renderings of the KPMG removal — was NOT run and its size is
  unknown.** Do not read the entry as if it had been.
- **`ctr-0060` step (ii) is DISCHARGED**; the entry stays **open** on step (iii).
- **`src-0044` carries its own ban**, written into its Limitations: it is evidence of **publication
  state on a date and nothing else**, supports no claim about citation accuracy, and **no cause may
  be inferred from it.** Its key claim 4 records that the surviving artefact is an **executive
  summary** with no footnote or reference list on the render's own report — **do not upgrade "the
  executive summary is live" into "the report with the defective citations is live."**
- **`ctr-0037` site (2) was NOT reached this cycle** and remains owed, along with the four
  occurrences in `scores.json`'s `ioc-extraction-reliability` rationale that its site list does not
  name at all.

## Carry-forward items — NEW AT CYCLE 127

- **[341] The tie-break list does not terminate, and it has now blocked a live selection.** This
  is [339] re-derived with the **correct** `depends_on` field, which [340] showed cycle 124 had
  mistyped. `prompts/t5_select.md` rung 3(c) left a genuine two-way tie between
  `institutional-incident-real-world-impact` and `ioc-extraction-reliability` — identical score,
  both survive 3(a), both unpenalised at 3(b), both `created_cycle` 2. Cycle 127 broke it with a
  declared ad-hoc rung (least-recently-attempted) and reported that four candidate rungs agree
  and one (most open questions) dissents. **A human should rule on a fourth rung.** Note this is
  a *different and simpler* problem than [339] originally described: rung 3(a) is **not** inert.
- **[342] The T5 quoting requirement is unsatisfiable as literally written for large issues.**
  `prompts/t5_select.md` requires the next-task entry to quote the target's `open_questions`;
  `institutional-incident-real-world-impact`'s nine total ~55 KB. Cycle 127 quoted verbatim
  openings plus lengths, indices and retrieval commands, and flagged this as partial compliance.
  **A human should either relax the wording to "quote or precisely locate" or cap
  `open_questions` length.** Every future T5 on a mature issue hits this.
- **[343] `ctr-0063` is new and is a *bookkeeping* defect, not evidence.** Cycle 118 entered
  "eleven supported candidates resting on one calibration source" for
  `consistency-calibration-as-failure-mode`; two of those eleven cite no `src-0001` at all, so the
  true count was nine (ten of twelve at cycle 127). **The reason cycle 118 gave — exactly one CTI
  calibration source — re-verifies, and this correction argues for no score change in either
  direction.** Steps (i) and (ii) are appends/judgements reserved to a T4 or T2.
- **A general rule the G2 suggested, offered and *not* adopted** (adopting standing rules is not
  a T5's charter): **G2 must target what a cycle entered into the state, not what its log said.**
  Cycle 118's log stated the calibration claim in a strictly stronger and plainly false form than
  its `scores.json` block. Checking the log would have produced a clean "failed" verdict; checking
  the state produced a mostly-confirmed conclusion with one bad numeral. Flagged for a human or a
  T2 alongside [319].

## Carry-forward items — NEW AT CYCLE 126

**[341] ctr-0037's recorded site list is incomplete, and the omission is in `scores.json`.**
The entry names site (3) as *"scores.json's task-dependent-reliability-framing rationale"*. It is
also in **`scores.json`'s `ioc-extraction-reliability` rationale**, where the string
`0.8240-0.8846` occurs **4×** — three of them defective, the fourth already writing
*"0.8240-0.8846 and 0.6944"* and therefore fine. Cycle 126 appended DO-NOT-CITE markers in **both**
rationales (site (3) proper, plus this unrecorded location), which discharges the defect *in
`scores.json`* under hard rule 2 — the strings are marked, never deleted. **ctr-0037 is NOT
closed**: site (2), the same truncated range in `ttp-attack-mapping-reliability`'s cycle-31
withdrawal text inside `graph.json`, is untouched, and `graph.json` is not a T4's file. A T3 or T2
owes **both** site (2) **and** widening the entry's recorded site list to name the `ioc` rationale.
The correct four-model range is **0.6944–0.8846** (src-0007 Table 4, IoC Extraction / Precision,
GPT-4o | o3-mini | GPT-4o (FT) | GPT-4o-mini (FT) = 0.8240, 0.8503, 0.8846, 0.6944).

**[342] The unread src-0006 matching rule is load-bearing on TWO issues, not one — so one fetch
buys two issues.** ctr-0009 step (iii) asks whether src-0006's nine per-sub-task F1/AUC rows share
one matching rule; no cycle has ever read those definitions. Cycle 125 promoted it to
`task-dependent-reliability-framing`'s `open_questions[3]` because it is that issue's
`candidate_resolutions[0]`'s foundation. **Cycle 126 found it is equally load-bearing on
`extraction-vs-reasoning-ordinal-axis`**, which `depends_on` that issue *and* leads its own
evidence array with src-0006 — an **ordinal axis over sub-tasks presupposes the sub-tasks were
scored commensurably**, which is exactly what is unverified. Recorded in that issue's `scores.json`
rationale; **not** entered as an open question, because a T4 does not restructure. A T3 or T2
should enter it on `extraction-vs-reasoning-ordinal-axis`. Either outcome of the fetch is a result:
if the rows share one rule, both issues have a real level-3 case on the direction; if they do not,
both lose their last independent support and the honest scores **fall**.

**[343] The `next_task.json` size cap [238] has drifted and should be re-ruled or re-observed.**
[238] states a ~15 KB cap. Cycle 125's entry was **19,013** chars and cycle 126's is **18,889** —
both well over, for two consecutive cycles. Cycle 126 did not truncate, because [340] and the
ctr-0037 / ctr-0062 hand-offs are load-bearing for its successor, but it flags the overrun rather
than exceeding the cap silently. **A human should rule** whether the cap should rise or whether
entries should be compressed. Note the tension a ruler should weigh: the entries are long precisely
because the successor has no memory, and the three aborted cycles (70, 83, 106) were phase-check
failures that longer entries are designed to prevent.

**Method note, worth carrying: the ten-entry single-call splice.** Cycle 126 updated all ten score
entries in **one** [237] two-input `jq` call using
`reduce ($s|keys_unsorted[]) as $k (.; .scores[$k].rationale += $s[$k] | .scores[$k].assessed_cycle = 126)`.
`reduce` applies one edit shape to every key with **no brace**, so [227] does not bite, and it
replaces the per-entry splices earlier T4s used. The append-only proof used the [335] key-binding
form and returned seven `true`s.


## Carry-forward items — NEW AT CYCLE 125

**[340] THE SELECTOR HAS BEEN READING A FIELD THAT DOES NOT EXIST, AND IT CHANGED AN ACTUAL
SELECTION. NEEDS A HUMAN RULING.** The issue schema's field is **`depends_on`**; cycle 124's T5
queried **`.depend_on`** (singular), which is absent from every issue and therefore returns `null`
ten times, and concluded that tie-break (a) *"IS INERT IN THIS GRAPH"*. **Five of the ten issues
carry a non-empty `depends_on`.** Cycles 107 and 116 both used the correct field. Re-running
`prompts/t5_select.md` mechanically at cycle 124 with the correct field eliminates
`task-dependent-reliability-framing` at 3(a) — it carries the graph's longest `depends_on` — and
leaves a **two-way** tie between `ioc-extraction-reliability` and
`institutional-incident-real-world-impact`. **The issue cycle 125 was assigned was not the policy's
output.** Cycle 125 executed the assignment anyway rather than adding a second undocumented
deviation, and filed this instead. **This partly falsifies [339]:** rung (a) is not dead and the
residual tie was two-way, not four-way — though [339]'s core request (a fourth rung, or a ruling that
the residual is the T5's discretion) still stands, because a two-way tie is still unbreakable by the
written policy. **Verify in one call:**
`jq -r '[.issues[]|.id+" dep="+(.depends_on|tostring)]|join("\n")' state/issues/graph.json`.

**[341] A NEGATIVE SEARCH RESULT HAS A HALF-LIFE, AND THIS BASE NOW HAS DATA ON IT.** Cycle 125
re-ran two searches whose negatives were 109 and 119 cycles old. **One was still negative** (no
independent replication or critique of LANCE/PRISM exists, three searches across 119 cycles) and
**one produced a source** (`src-0043`, against a question last searched at cycle 16). So refreshing
stale negatives is worth roughly one hit in two here, which is a good enough rate to keep doing it —
**but the two are not symmetric.** The still-negative one is about a *specific paper* accumulating
third-party attention, which is slow; the one that hit is about a *research area* producing new work,
which is fast. **Refresh area-shaped questions; leave paper-shaped ones alone longer.** Cycle 125
recorded that judgement inside `open_questions[2]` so the next cycle does not burn a fetch relearning
it.

**[342] TWO RENDERS CAUGHT A NUMBER THAT WOULD OTHERWISE HAVE ENTERED THE BASE AS CITABLE, AND THE
RENDER-DISAGREEMENT PATTERN IS A PROPERTY OF THE TOOLS, NOT OF ANY ONE PAPER.** On `src-0043` the two
renders disagreed on the **author list** (two names vs one), the **date**, the **venue**, and — the
one that matters — the **macro-F1 decrement** (abstract render `0.04`; body render implies `0.026`
from 0.177 → 0.151). **The author-count disagreement is the exact failure already recorded at
`src-0042` limitation 2, on a different paper and a different arXiv id.** Two instances on two papers
makes it a property of the arXiv-html/`r.jina.ai` render pair. **Consequence: single-render metadata
should be marked single-render as a matter of course, and any number that appears on only one render
is a hold candidate, not a fact.** The [205] rule (use the pair) is doing real work and should not be
skipped to save a fetch.

**UPDATE TO [238] (not a new item).** The ~15 KB ceiling on `next_task.json` instructions has now been
**exceeded twice running** — 18.4 KB at cycle 124, **19.0 KB at cycle 125**. Both cycles trimmed and
both chose information over the guideline. The mandated content (verbatim `open_questions`, citation
bans, contradiction descriptions, and now [340]) does not fit under it. **Either the ceiling needs
raising or the entry needs to start citing the log by line range instead of reproducing it. A human
should rule.**

**STANDING HONESTY ITEM — SEVENTEENTH CONSECUTIVE CYCLE, AND [340] CHANGES ITS REASONING WITHOUT
CHANGING ITS CONCLUSION.** `automated-triage-under-refusal` has still never been investigated by a T3
in 125 cycles; its `attempts` array is empty and it is the only such issue in the graph. Cycle 124
concluded it was eliminated at rung (c) by a four-way tie among `created_cycle`-2 issues. **Under the
corrected field that account is wrong in its details:** the issue *survives* rung (a) (its
`depends_on` is empty) and is eliminated at rung (c) on `created_cycle` 16 against issues created at
cycle 2 — a simpler and more robust elimination than cycle 124 described. **The conclusion stands and
is if anything firmer: the never-investigated issue is unreachable by the stated policy.** A human
should rule on whether the selector needs a staleness or never-attempted term. Repeat this item until
a human rules on it.

## Carry-forward items — NEW AT CYCLE 124

**[337] REPRODUCE A PRIOR CYCLE'S INSTRUMENT BEFORE CALLING ITS COUNT WRONG.** A reported count is a
triple — **field**, **case**, **snapshot** — and all three must match before a disagreement means
anything. Cycle 124 got three successive mismatches against cycle 115's phrase count by using the
rationale instead of the whole score entry, case-sensitive instead of `ascii_downcase`, and the
committed post-append file instead of the pre-append state the cycle actually measured; with all
three corrected it reproduced exactly. **A count that will not reproduce is a hypothesis about your
own instrument first and an accusation second.** Try the variants before opening a contradiction.

**[338] THE `only candidate` PHRASE COUNT OVER `scores.json` IS A CONSUMED INSTRUMENT AND MUST NOT
BE RE-RUN NAIVELY.** The `ctr-0058`/`ctr-0059` sweep measured that phrase across score entries and
found three carriers. Recording the negative result in each cleared issue's own entry — which cycle
115 was required to do — **wrote the phrase into nearly every entry**, taking the plain count from 1
carrier pre-115 to all ten, where it remains. A future re-run on the current file will read ten
carriers and conclude the defect spread; **it did not**. Any re-measurement must either run against
`358ddb7^` or use a narrower instrument that excludes sweep-reporting text. This is an instrument
decay, **not** a G3 contradiction: cycle 115's claim was true of the state it measured.

**[339] THE SELECTOR'S TIE-BREAK LIST DOES NOT TERMINATE, AND TWO OF ITS THREE RUNGS ARE DEAD.
NEEDS A HUMAN RULING.** Demonstrated mechanically at cycle 124, not conjectured:
(a) **upstream-first is inert** — `depend_on` is `null` on all ten issues, so it has never separated
anything and cannot; (b) fires on at most the one or two issues worked in the last five cycles;
(c) `created_cycle` leaves a **four-way tie** (task-dependent-reliability-framing,
attribution-confident-wrong-gap, ioc-extraction-reliability, institutional-incident-real-world-impact
were all created at cycle 2) that the prompt has no rung left to break. **Every T5 from here on must
invent and declare a residual criterion, and different T5s may invent different ones — which makes
the ranking table, the project's own evaluation data, non-reproducible across cycles.** Cycle 124
used **staleness (cycles since last T3)** and stated it in the log; a successor is free to disagree,
which is exactly the problem. A human should either add a fourth rung to `prompts/t5_select.md` or
rule that the residual criterion is the T5's discretion. Related: this is the same rung (c) that
permanently excludes the never-investigated issue — see the standing honesty item.

**UPDATE TO [238] (not a new item).** The ~15KB ceiling on `next_task.json` instructions was
**exceeded at cycle 124: 18.4KB**, after six trimming passes, because the mandated content (verbatim
`open_questions`, the citation bans, three contradiction descriptions) does not fit under it. Either
the ceiling needs raising or the entry needs to start citing the log by line range instead of
reproducing it. Flagged, not silently ignored.

**STANDING HONESTY ITEM — SIXTEENTH CONSECUTIVE CYCLE, AND CYCLE 124 SHARPENED IT FROM SUSPICION TO
PROOF.** `automated-triage-under-refusal` has still never been investigated by a T3 in 124 cycles;
its `attempts` array is empty and it is the only such issue in the graph. **Cycle 124 was the T5 —
selection was its actual job, and the queue entry expressly told it that it MAY select this issue on
the merits — and it still did not reach it.** Applying `prompts/t5_select.md` mechanically
eliminates the issue at tie-break (c): `created_cycle` 16 loses to four issues created at cycle 2
that share its score of 2. Under cycle 124's own residual criterion it would rank **first of all
ten**, but the residual only applies after (c), and (c) had already cut it. So: **the
never-investigated issue is unreachable by the stated policy, and this is now a demonstrated
structural fact rather than an inference from repeated non-selection.** Cycle 124 declined to
override the policy on its own authority, because a T5 that departs from the written policy corrupts
the ranking table the logs exist to produce. **It needs a human ruling** on whether the selector
should carry a staleness or never-attempted term — see [339], which is the same defect seen from the
other side. Repeat this item until a human rules on it.
