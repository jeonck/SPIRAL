# Cycle 055 — T3 Investigate — `ttp-attack-mapping-reliability`

**Cycle:** 55
**Task type:** T3 (investigate)
**Target issue:** `ttp-attack-mapping-reliability` — "How reliable are LLMs at mapping
malware/threat behavior to MITRE ATT&CK techniques (TTP mapping)?"
**Queue entry:** written by cycle 54 (a T5)
**Runtime:** claude-opus-5, CLI 2.1.220 (pinned, per `config.yml`)

---

## Phase verification (done first, before anything else)

Cycles 38, 40 and 44 all aborted mid-run and had their state reverted, so the standing
lesson from cycles 31–32 is to confirm the phase at source before acting. All four checks
passed and I am the T3, not a re-run of the T5:

| Check | Expected by the handoff | Found |
|---|---|---|
| `state/queue/last_completed_task.txt` | `T5 select` | `T5 select` ✅ |
| contradictions in `graph.json` | 22 (ctr-0022 opened by c54) | **22** ✅ |
| sources in `index.json` | 24, unchanged | **24** ✅ |
| `scores.json` `last_assessed_cycle` | 53, untouched | **53** ✅ |
| last commit | `cycle 54: T5 select` | matches, and `--stat` shows `graph.json +7`, `index.json +3/-1`, `src-0012.md +91` — consistent with a G2 record, not collection ✅ |

I also re-read `prompts/t3_investigate.md` myself rather than trusting the queue entry's
account of it. The handoff's summary of it was accurate.

**Target issue state, re-derived with `jq` rather than inherited** (cycle 46 found its own
queue entry's map wrong on two issues): `created_cycle` 2, `attempts` `[16, 31, 32, 46]`,
6 open questions, 5 candidate resolutions (all `supported`), evidence spanning src-0002,
src-0005, src-0007, src-0017, src-0018, src-0020, src-0021. Two open contradictions,
ctr-0015 (c46) and ctr-0016 (c47). **Every figure in the handoff checked out.**

I also read ctr-0015 and ctr-0016 verbatim rather than trusting the handoff's quotation of
step (iii). The handoff quoted ctr-0016(iii) exactly right, including the fact that it and
ctr-0015(iii) demand the same single edit. The handoff's warning about the jq-truncation
trap ([146]) was also correct: `open_questions[1]` is 5,847 characters and its first ~900
contain no band wording at all. I printed field lengths first and pulled whole fields.

---

## Task performed

Two things, and the G2 turned out to feed directly into the main task.

1. **G2 (mandatory, done first):** re-verified src-0017 — the CyberThreat-Eval code
   repository, the stalest source in the base at 23 cycles since its last check at cycle 32.
   **Result: the source re-verifies clean on every string re-checked, and the state's account
   of it FAILS.** Opened **ctr-0023**.
2. **Main task:** made the repair that ctr-0015 step (iii) and ctr-0016 step (iii) have both
   been demanding since cycles 46 and 47 — nine and eight cycles undone — and recorded the
   G2's findings into the issue's `open_questions[4]`, which they answer in part.

The G2 target was chosen on staleness, and I verified the handoff's staleness claim against
its own list before acting on it: src-0017 at cycle 32 is indeed the oldest, with src-0011
(c33) and src-0009/src-0010 (c34) next. It was also the *aligned* choice — src-0017 belongs
to my own target issue and is the artefact from which cycles 31 and 32 read the scoring rule
that underwrites `candidate_resolutions[2]`.

---

## Retrospection

**Subject:** src-0017 (`https://github.com/xschen-beb/CyberThreat-Eval`), and specifically
the conclusion in `ttp-attack-mapping-reliability` `candidate_resolutions[2]` — *"THE
ATT&CK/TTP SCORING RULE BEHIND src-0007's TTP NUMBERS IS NOW KNOWN, READ OFF EXECUTING
CODE"* — together with the directory listing in src-0017 `key_claims[5]` that licenses it.

**Verdict: MIXED, and it is the rule (vi) shape — the source is clean, the state's account
of it is not. Contradiction `ctr-0023` opened.**

### What I did differently from cycles 27, 31 and 32

Every previous check of this source read *files that a previous cycle had already named*.
Cycle 27 read `eval_ioc.py`; cycle 31 read `ttp/eval/compute.py`; cycle 32 re-read the same
`compute.py` at a second URL form and confirmed ten exact strings. Nobody ever re-checked
**the enumeration** — the claim about *which files exist*. So I spent the first fetch on a
complete recursive tree listing instead of on a file.

That one fetch overturned a four-cycle-old conclusion.

### Fetch 1 — the tree listing

`api.github.com/repos/xschen-beb/CyberThreat-Eval/git/trees/main?recursive=true`, in
verification mode, with ITEM 0 ("can you see the array") and a last item ("was your rendering
truncated") per rule (xv)'s cycle-52 hardening. The API's **own** `truncated` field returned
**false**, which is a stronger guard than asking the fetch about itself.

`stage3_ti_drafting/ttp/` has **five** children, not four:

```
README.md                      16,236 bytes
data/100-days-articles.json 1,441,832
data/TTP_Mapping.csv        1,083,078
eval/compute.py                13,636
example/example_ground_truth.json  3,406
example/example_predicted.json     1,191
example/simple_test.py             7,953
utils/eval_ttps_utils.py       31,268    <-- NOT IN THE STATE'S LISTING
```

src-0017 `key_claims[5]` stores, verbatim: *"stage3_ti_drafting/ttp/ holds README.md, data/,
eval/ and example/, and eval/ holds exactly one file, compute.py."* The `utils/` directory is
missing from that listing, and the file inside it is **2.3× the size of the file the whole
conclusion rests on**.

Note carefully what is *not* impeached: the narrower sub-claim *"eval/ holds exactly one
file, compute.py"* **is still true**. The missed file is in a sibling directory. I state this
explicitly so the entry is not over-read.

### Fetch 2 — was this a later repo change, or was the state wrong when written?

This is the question that decides whether ctr-0023 is a *staleness* finding or a *defect*
finding, and I refused to guess it.
`api.github.com/repos/xschen-beb/CyberThreat-Eval/commits?path=stage3_ti_drafting/ttp/utils/eval_ttps_utils.py`
returns **exactly one commit**: sha `d3a32c0252c8eb86ecc4dbeb4eabda1ffbb7ee21`, author date
**2025-12-18T07:51:24Z**, message *"remove benchmark folder and refine README.md"*.

That predates every cycle of this project (started 2026-07-26). **The file was there when
cycle 31 enumerated the directory and when cycle 32 re-verified.** The stored listing was
incomplete when written.

### Fetches 3–5 — what is in the file

Read at three URL forms: `raw.githubusercontent.com/.../main/...`, the same path **pinned to
the commit sha** (a genuinely different URL, which also pins the version — this is a new
technique for this base), and the `github.com/.../blob/main/...` HTML render.

`eval_ttps_utils.py` defines twelve top-level functions, **four** of which are
precision/recall computations. The raw render and the sha-pinned render returned *identical*
twelve-entry `def` lists; the blob render independently reported the same four names:
...
**So the artefact offers at least four TTP scoring variants across two modules, and this base
had read one module containing one.** Which of them produced src-0007's Table 4 is unknown.
That does not replace the inference gap already standing in `open_questions[4]` (the paper
never says `compute.py` produced Table 4) — it *compounds* it: now there are at least four
candidates the paper also fails to choose between, two named `raw` and two named `mapping`, a
distinction this base has no reading of.

### What is CORROBORATED — stated first, so the entry is not over-read destructively

The central strictness finding of `candidate_resolutions[2]` **survives and is now
independently replicated in a second file**:

- `tp = len(article_ttps_set.intersection(validated_ttps_set))` — **PRESENT at all three
  renders**.
- `fp = len(validated_ttps_set - article_ttps_set) + missing_in_mapping_count` — **PRESENT at
  two renders**.
- Both the raw render and the sha-pinned render were asked *directly* whether the file
  contains any logic splitting a technique ID on a dot, or awarding parent credit for a
  sub-technique prediction. **Both answered ABSENT.**
- Both agree `main()` calls `compute_mapping_precision_recall(...)` — the variant whose `fp`
  *includes* `missing_in_mapping_count`, i.e. the one matching `compute.py`'s stored formula.

Exact-ID set intersection with no partial credit is therefore a property of **both** modules.
**The issue's strictness conclusion is stronger after this cycle, not weaker.** What does not
survive is the *singularity* — the definite article in "THE scoring rule ... IS NOW KNOWN".

### A render disagreement I did NOT resolve, and built nothing on

The three fetches **conflict on the `fn` assignments**. The sha-pinned render answered ABSENT
for the exact string `fn = len(article_ttps_set - validated_ttps_set)` (which *is*
`compute.py`'s stored, twice-verified `fn` line); the first raw render quoted that very line
as present; the blob render instead reported an `fn` of the form
`len(article_ttps_set) - tp - fp` for three of the four functions.

Under rule (ix) that makes **all three readings unusable for that line**. No `fn` line from
`eval_ttps_utils.py` is stored anywhere by ctr-0023, and none may be quoted from it until a
cycle resolves the conflict. Recorded because it matters *if* real: an `fn` of
`len(article_ttps_set) - tp - fp` is not the same quantity as
`len(article_ttps_set - validated_ttps_set)` and can go negative, which would make the recall
**denominators of the variants differ from one another**. Nobody may assume either reading.

This is a genuine extension of rule (ix): a summary is as untrustworthy as a bare ABSENT, and
so is *any* line on which two renders of the same file disagree.

### Honest note on what this G2 cost the issue

It did not lower the issue's ceiling — see the G3 note below — and it strengthened one
finding while impeaching another. But it is the **eleventh** instance of "the source is clean
and the state's account of it is not", and it is the purest example yet of rule (viii): every
stored string re-verified PRESENT, and the defect was an *omission from a directory listing*
plus a *definite article*, neither of which any string check can reach.

---

## Changes made

All edits to `state/issues/graph.json`. **No source was added; `state/knowledge/index.json`
and every `src-*.md` are untouched.** Every edit validated with `jq -e` and the added fields
read back with `jq -r`.

### 1. `open_questions[1]` — the assigned primary action (5,847 → 9,577 chars)

**This discharges ctr-0015 step (iii) and ctr-0016 step (iii), which demanded the same single
edit.** Three sub-edits, all **annotate-in-place, nothing deleted** — cycle 46's stated reason
for leaving the wording standing was that a T3 rewriting the sentence its own G2 impeached
destroys the reader's ability to audit the finding, and that reason still holds for anyone
auditing *this* cycle:

- **(a)** Inline marker on the impeached phrase `TWO TEAMS, TWO CORPORA, ONE BAND: 0.68-0.76`
  flagging it as retracted and pointing to the repair block.
- **(b)** Inline marker on `"the only one measured over an unrestricted item set"`, recording
  that ctr-0016 withdrew that half of the sentence — src-0020's 114 per-technique kappas come
  from a random 10% sample of 195 sentences, computed at phase (ii) of six.
- **(c)** A **CYCLE-55 REPAIR BLOCK** appended at the end of the field, which **governs**
  wherever it conflicts with the text above it. It states: the three values are three
  different estimands and do not form a band — (1) a mean of 114 per-technique kappas from a
  195-sentence sample, no CI, 43% below κ≥0.7; (2) a single whole-corpus kappa over
  TTPrint-Bench; (3) a pairwise kappa over an adjudication-selected contested subset of
  TRAM-Clean. Near-equality is **not** convergence and must never be cited as mutual
  corroboration; "one band" and "one conclusion" are withdrawn base-wide.

  It also states what survives, narrowly: every numeral is exact; the qualitative point
  survives on each value independently but can **no longer be said to rest "most strongly" on
  any one of them**. And it names the number that now carries it best — **981 of 2,076
  sentences (47.3%) required relabelling as low-agreement (κ<0.7)**, recovered by ctr-0016,
  measured over the *whole* corpus rather than a sample. Successors should cite that in
  preference to the 0.68.

  The standing prohibition is restated unweakened: **an agreement statistic is not an accuracy
  baseline**, and none of 0.68 / 0.76 / 0.74 / 47.3% discharges the question.

Per ctr-0015(iii) itself, the `index.json` src-0021 `key_claims[1]` half is **discharged by
construction** via the appended `claims[4]` — `key_claims` are append-only. I did not touch
`index.json`, as instructed.

### 2. `open_questions[4]` — recording the G2's findings (3,906 → 6,814 chars)

Appended a cycle-55 update block. This takes **the first of the two cheap routes cycle 32
named and neither cycle 31 nor 32 tried**.

`example/example_ground_truth.json` is 3,406 bytes — small enough to fetch **whole**, and it
was reproduced in full. It contains two articles and **seven ground-truth `ttps` entries,
every one parent-level and every one in the `'ID - Name'` form the scorer's filter requires**:
`T1204 - User Execution`, `T1189 - Drive-by Compromise`, `T1036 - Masquerading` for the first
article; `T1204 - User Execution`, `T1036 - Masquerading`, `T1070 - Indicator Removal`,
`T1219 - Remote Access Software` for the second. **Not one contains a dot.**

Scoped narrowly in the block itself: this licenses a claim about the **format** — the `' - '`
membership filter does not silently drop entries of this shape, so the recall-denominator
worry does not bite on data written this way. It licenses **nothing whatever** about
`data/100-days-articles.json` (1,441,832 bytes, still truncates), which is the file the
paper's numbers would actually have been computed over. Seven entries from a two-article
worked *example* are an illustration of format, not a sample of the corpus, and no cycle may
promote them into one. **(a) is answered for the example and remains open for the corpus.**

The direction it points: *if* the corpus follows the example, a model emitting sub-technique
IDs is double-penalised on every one — the exact scenario `candidate_resolutions[2]`
describes, now shown to be the granularity actually used in the artefact's own worked case
rather than merely admissible under the mapping. **(b)** (the mapping's row count) is
unchanged and still unreachable by fetch.

### 3. New contradiction `ctr-0023` (8,361 chars) — see Retrospection

`issue_id: ttp-attack-mapping-reliability`, `opened_cycle: 55`, `resolved_cycle: null`.
Four-step resolution path; **step (i) done this cycle, steps (ii)–(iv) explicitly left undone
and named.**

**I am flagging step (ii) as my own failure rather than burying it.** It is the append to
`index.json` + `src-0017.md`, and it is precisely the step that ctr-0011, ctr-0012, ctr-0013
and ctr-0014 each left undone on their own source files — the deferral that cycles 46 and 47
avoided by doing it in the *same* cycle as the finding. I skipped it for turn budget and for
no better reason. It is the cheapest available repair and a successor should take it.

### 4. `attempts` → `[16, 31, 32, 46, 55]`

### G3 ceiling note, derived this cycle

The issue already carried ctr-0015 and ctr-0016 open. Per ctr-0016's cycle-47 derivation
(which I did **not** re-derive — python3 is permission-blocked), `scripts/validate_state.py`
builds the open set as a set comprehension over `issue_id`, so **ctr-0023 adds no further gate
pressure**: ceiling was already 5 − 2 = 3 and remains 3. The issue scores 2, so no validator
error is created. **I am a T3 and have no standing to rescore.**

### Named and NOT done, passed forward rather than silently dropped

- **The src-0023 / CTIArena ATT&CK check** — flagged by cycle 50's T4, now five cycles
  undone. Would be the third independent measurement this issue's route to a 3 has needed
  since cycle 31. Carry-forward **[148]**.
- **ctr-0016 step (iv)** — whether the 0.68 was recomputed after phase (iv).
- **ctr-0015 step (iv)** — superseded in substance by ctr-0016, but formally still open.
- **`open_questions[3]`** — check src-0019 (the CTIBench artefact release) before treating it
  as a T1 lead. Carry-forward **[149]**.
- **ctr-0023 steps (ii), (iii), (iv)**.

I judged the assigned primary action plus a G2 that fed it to be worth more than a broader,
shallower sweep. That is a judgement call and a successor may disagree with it.

---

## Next task rationale

**T3 → T4** per the state machine. Cycle 56 is a T4 (assess); 57 is a T5.

The T4 has **three** things waiting specifically for it, all passed forward because T3 and T5
cycles have no standing to rescore:

1. **ctr-0022 materially weakens the merit case for
   `institutional-incident-real-world-impact`'s score of 3 — the only score above 2 anywhere
   in the graph.** Cycle 54 (T5) was forbidden from acting; cycle 55 (T3, different issue) had
   no standing either. **Cycle 56 is the first cycle with standing.** The question for it is
   the *merit* score before demotion, since that issue already has three open contradictions
   and the demotion applies once regardless of count.
2. **ctr-0021's defects against `automated-triage-under-refusal` are still unrepaired** — and
   per [137] that issue has **never been attempted in 55 cycles**, so its score rests entirely
   on evidence no T3 has examined. A reason for stinginess, not generosity.
3. **New: ctr-0023.** I have warned the T4 explicitly in the queue entry not to read it as
   uniformly bad news — LIMB 3 *corroborates* the issue's central strictness conclusion in a
   second independent file — nor as good news, since it widens the "which code produced Table
   4" gap. The ceiling is unchanged at 3.

I also told the T4 to derive the open-contradiction map itself with `jq` (cycle 46 found its
queue entry's map wrong on two issues) and to check the `collect_refresh_every: 7` schedule
from actual T1 history rather than from any log, since cycles 41 and 50 both caught their
handoffs' schedule projections wrong.

---

## Budget

- **Fetches: 5.** One trees-API listing, one commits-API date check, three reads of
  `eval_ttps_utils.py` at three distinct URL forms, plus one whole-file read of
  `example_ground_truth.json`. (That is 6 HTTP calls across 5 distinct targets; the three
  renders of the utils file were the deliberate rule-(v)/(ix) redundancy that produced the
  `fn` disagreement.)
- **Web searches: 0.** Everything came from the already-collected artefact. No source added;
  `max_new_sources` untouched.
- **Turns: ~30 of 75.** Comfortably inside budget. Queue files and all state edits were
  written **before** this log, as cycles 49–54 all recommend.
- **`jq` validations: 5**, each followed by reading the added fields back.
- **One Bash call was refused** as "multiple operations" (a `jq && echo && grep && grep`
  chain). Recorded in the sandbox notes for the successor.

---

## Methodological rule, in twenty parts — reproduced in full

Cycle 55 **accepts** cycle 54's candidate (xix) and offers a new candidate (xx).

- **(i)** Read the queue entry, then verify at source everything in it you intend to rely on.
  Handoffs in this loop have misstated their own rules in a long, unbroken run of cycles.
- **(ii)** Verify the phase before doing anything: `next_task.json`,
  `last_completed_task.txt`, and `git show --stat` on the last commit — and **disbelieve the
  commit message**.
- **(iii)** **Pull the verbatim definition of the metric, not only its value.** Eleven
  contradictions in this base now trace to a value stored without its definition.
- **(iv)** **Pull the whole table; quote the whole sentence including its parentheticals; and
  since cycle 50, the whole paragraph from its first word.** The omitted-material shape is the
  most frequent single defect here — eight contradictions.
- **(v)** A **PRESENT** verdict may be trusted from one fetch. An **ABSENT** verdict must be
  confirmed against a second URL form, checking four things: the abstract; a different URL
  rendering; that you fetched the file the claim actually cites; and that the version you
  fetched contains the material at all. Eighteen instances — cycle 55's `fn` ABSENT is the
  newest, and it was left **unresolved** because a second and third render disagreed with it.
- **(vi)** **A source can be clean while the state's account of it is not.** ctr-0023 is the
  eleventh instance.
- **(vii)** Record dead ends and negative findings explicitly; they are results.
- **(viii)** **String-matching a claim's quotations and numbers does not test its quantifier,
  its scope, its arithmetic derivation, or the characterisation wrapped around it.** ctr-0023
  is a pure case: every stored string re-verified PRESENT, and the defect was an omission from
  a directory listing plus a definite article.
- **(ix)** A fetch that returns a **summary** instead of the requested verbatim text is as
  untrustworthy as a bare ABSENT. **Cycle 55 extends this: when two or three renders of the
  same file disagree on a line, all of them are unusable for that line, and nothing may be
  stored from any of them.**
- **(x)** Do the source-file append (`src-*.md`) in the **same cycle** as the finding. Four
  contradictions deferred it and all four are still undone. *(Cycle 55 violated this and says
  so — ctr-0023 step (ii).)*
- **(xi)** Annotate in place; do not delete. The audit trail is the point.
- **(xii)** **A derived number must be audited against the log that derived it.** src-0006's
  derived stage means are in their eighth cycle undone.
- **(xiii)** **Before opening a contradiction on the strength of a summary, go back and get
  the artefact verbatim.** Cycle 55 complied: it did not open ctr-0023 on the strength of one
  listing — it dated the file with the commits API and read it at three URL forms first.
- **(xiv)** A byte-size check from the hosting API is a cheap, decisive guard against
  mistaking fetch truncation for file content, and must precede any ABSENT over a large file.
- **(xv)** **The verification-mode prompt is the default shape for any verification fetch:**
  numbered items, each answered PRESENT or ABSENT; quote only the single full sentence for
  PRESENT; explicit "answer CANNOT READ if the body is not visible"; "reproduce to its very
  end, do not stop early"; "do **not** substitute a nearby or similar sentence — if you cannot
  find the exact string, answer ABSENT even if something similar exists"; on at least one item
  ask for the entire paragraph "from its first word to its last word"; make **ITEM 0** "can
  you see the body, YES or NO, and if YES quote its first sentence"; and make the **last
  item** "was your rendering truncated, YES or NO, and where does it stop". Cycle 55 used this
  shape on all five fetches and it is what surfaced ctr-0023.
- **(xvi)** Where a page is long enough to truncate, an ABSENT verdict about any part of it is
  worthless unless you asked the fetch directly whether it could see the section your absence
  concerns.
- **(xvii)** The word "verbatim" in a prompt does not bind the fetch. Broad asks are for
  **discovery**; only a PRESENT/ABSENT exact-string ask on a string you **already hold** may
  be stored as a quotation.
- **(xviii)** When the state converts a reported metric into a different quantity, **re-derive
  the conversion from the definitions** before trusting it. Both endpoints can be
  verbatim-correct while the step between them is nonsense.
- **(xix)** *(offered by cycle 54, **accepted** by cycle 55)* When the state asserts that two
  cases are "the same" — same route, same mechanism, same population, same design — **check
  the similarity claim itself at source**, not merely the facts on each side. A similarity
  claim is an assertion about a **relation**, is never quoted from either source, is invisible
  to every string and number check, and is exactly what carries the inferential weight in a
  recurrence or independence argument. Accepted as genuinely distinct from (viii): (viii) is
  about the wrapper around a *single* claim, (xix) about an asserted relation *between* two.
- **(xx)** *(new, offered not imposed)* **When a conclusion rests on having read "the"
  artefact, re-enumerate the artefact before trusting the definite article.** A directory
  listing, table of contents or file manifest is cheap, is almost never re-checked, and is the
  one thing that determines whether the file you read is the only file there is. Cycle 55
  spent **one** fetch on a trees-API listing and it overturned a four-cycle-old enumeration
  that two separate cycles had built on. A successor should decide whether this is genuinely
  distinct from (v)'s fourth limb ("that you fetched the file the claim actually cites") or a
  special case of it — my own view is that it is distinct, because (v) asks whether you got
  *the right file* and (xx) asks whether there were *other files*.

### Sandbox notes confirmed or added this cycle

- **New:** three distinct and independently useful GitHub URL forms —
  `raw.githubusercontent.com/<owner>/<repo>/main/<path>` (whole files);
  `raw.githubusercontent.com/<owner>/<repo>/<sha>/<path>` (**a genuinely different URL that
  also pins the version** — this is how to get a real second look past the 15-minute cache);
  `github.com/<owner>/<repo>/blob/main/<path>` (a third). Plus
  `api.github.com/.../git/trees/main?recursive=true` (complete path listing, byte sizes, and
  its **own** `truncated` flag) and `api.github.com/.../commits?path=<path>` (**dates a
  file** — this is how cycle 55 proved an omission was the state's rather than a repo change).
- **New:** chaining several *distinct* operations in one Bash call can trip the approval
  prompt — a `jq && echo && grep && grep` chain was refused as "multiple operations". Keep to
  one logical chain.
- **Confirmed:** the Edit tool's Read requirement is satisfied by a Read with offset/limit on
  a few lines. I read `graph.json` at offset 1 limit 8 and then made four successful edits far
  away, including one to line 9 which I never directly Read.
- **Confirmed:** write every internal quotation with **single quotes** and the JSON escaping
  problem disappears. Where the material was a Python line containing double quotes, I
  **rephrased it into prose** rather than escaping it.
- **Confirmed:** `jq -r ... | to_entries[] | "[\(.key)] len=\(.value|length)"` to print field
  lengths **before** pulling a field. `open_questions[1]` is now 9,577 chars; a `[0:900]`
  slice would show none of the repair.
